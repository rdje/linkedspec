...
- 2026-04-10: LinkedSpec mdBook first-reader examples are now aligned with the modern helper-style story. `spec-files-and-rule-paragraphs.md` and `get-and-get-parser.md` no longer introduce the project with tiny legacy `return_a(...)` examples; they now use `return(hash(...))` / `match_text()` payloads and link into the worked `.spec` walkthrough. Remaining public-book `return_a(...)` mentions are intentional in the blind-call orchestration chapter, where the historical post-call helper is explicitly explained. Natural next book slices: shipped-spec walkthroughs or more validated helper-example cleanup.
