...
- 2026-03-22: Started Phase 4 with the first named checkpoint slice for future resume. Named `@mark(name)` now lowers into shared runtime mark storage, compatibility helper `capture_from(name)` now captures from that mark up to the left edge of the current match, and match-info propagation through `LinkedRE::or(...)` now keeps those marks visible across later matching and child-rule calls.
- 2026-03-23: Expanded the named-checkpoint docs right away because adoption depends on it. The rule-mode guide and ActionIR contract guide now spell out the exact current semantics, including that `capture_from(name)` captures to the left edge of the current match and that `@mark(name)` should be read by later slots or later child calls rather than by a same-slot action.
