...
- latest_completed_leaf: `STAGED-LINKED-PARSING.1` (`STAGED-LINKED-PARSING.1 - adopt staged linked parsing doctrine`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `STAGED-LINKED-PARSING` — ADR `0012` now defines staged linked parsing: stage-N specs may emit source-provenance text islands and later specs parse those payloads through deterministic, language-neutral parse jobs; spec imports/composition are separate from staged dispatch.
- next_action: Pick `STAGED-LINKED-PARSING.2` when this architecture track resumes: design spec-file imports/composition. If PNT returns to parity first, `RUST-PARITY.7.3` remains the active Rust frontier.
