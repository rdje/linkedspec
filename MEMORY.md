...
- latest_commit: `0a33b6b` — `SPEC-FORMAT-TERSE.1.3.4 — split operator syntax family into scalar, array, and hash leaves`. Prior committed slice: `d89838f` (`SPEC-FORMAT-TERSE.1.3.3`; handoff commit `d97c571`). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — **`.1.3.4` SPLIT DONE 2026-06-29** (operator syntax family). KM + TOOLBOX recon shows `name = value`, `items += value`, and `name["k"] = value` are RAW_PERL blockers on Perl and unsupported by Rust's expression-statement-only lifecycle AST. `.1.3.4` is now a container: `.1.3.4.1` scalar assignment, `.1.3.4.2` array append, `.1.3.4.3` hash-index assignment. No engine/book behavior change. User in a **PNT loop** (2026-06-23).
