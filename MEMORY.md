...
- latest_completed_leaf: `PERL-ACTIONIR-AST-MIGRATION.3.4` (`PERL-ACTIONIR-AST-MIGRATION.3.4 - lower return payloads from AST`; hash reported after commit). Previous HEAD before this slice: `0277102`. Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `PERL-ACTIONIR-AST-MIGRATION` — `.3.4` moved typed return payloads onto AST value/call/chain traversal before raw fallback while preserving scalar source-slot reads (`return(count)` -> `$count`) and narrow raw compatibility payloads.
- next_action: Execute `PERL-ACTIONIR-AST-MIGRATION.4`: replace statement/control lowering with AST lowering. User-defined functions remain queued and must use the AST path; final `fn <name>(...) { ... }` grammar belongs in `specs/spec.spec`, and any bootstrap-parser `fn` support is temporary migration debt to remove after text-to-AST carries the surface. Keep `SPEC-FORMAT-TERSE.3.2.2`/`.3.2.3` and `SPEC-FORMAT-TERSE.3.3` queued behind the user-directed AST/function run; do not start Julia/Dart backend code without a dedicated owning backend leaf/tree; do not start Lua backend work without an accepted decision record.
