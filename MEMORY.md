...
- latest_completed_leaf: `RUST-PARITY.7.3.7` (`RUST-PARITY.7.3.7 - hard-timeout parser construction too`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `RUST-PARITY` — `.7.3.7` closed the user-directed timeout re-debug: BNF's timeout symptom was parser construction outside the prior oracle guard (`get_parser("BNF")` ~6.4s; parse after build ~0.03s; trace reached successful parser generation), and `tools/gen_oracle_corpus.pl` now builds + parses inside the forked `ORACLE_TIMEOUT` child. `.7.3.4` triages `portmap`/`lib_reader`/`ebnf`; `.7.3.5` triages `BNF`/`DT`/`ifelse`/`operators_try`/`spec.spec`; `.7.3.6` audits RTL/plugin/legacy candidates. Staged linked parsing first prototype is complete; broader public staged surfaces remain future work.
- next_action: Pick `RUST-PARITY.7.3.4`: reproduce `portmap`, `lib_reader`, and `ebnf` structural oracle mismatches with the toolbox/oracle path and classify the first safe fixture or narrower implementation owner.
