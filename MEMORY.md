...
- 2026-04-11: `plugin/string.plg` and `plugin/genericfilter.plg` removed as pure package-backed helper-registration shells. `Plugin::String` owns `var_subst(...)` / `var_subst_test(...)`; `Plugin::GenericFilter` owns `group_by`, `group_by_port`, `group_by_ioclock`, `group_byRE`, and `dispatch(...)`; and direct repo-owned callers already use those packages. Future resume should delete package-backed `.plg` wrappers once they have no concrete repo-owned legacy caller rather than keeping compatibility names by default.
...
- 2026-04-11: GenericFilter plugin extraction landed and then advanced past the temporary wrapper step. `Plugin::GenericFilter` owns `group_by`, `group_by_port`, `group_by_ioclock`, `group_byRE`, plus `dispatch($action, ...)`; `plugin/genericfilter.plg` has been removed; and `HUtils::GenericFilter(...)` / `TableSort::GenericFilter(...)` call the package owner directly instead of `LinkedSpec::run_plugin(...)` / `get_plugin(...)` over constructed `genericfilter_*` names. Future plugin work should continue moving executable `.plg` bodies into normal `Plugin::*` owners and keep `.plg` files only as temporary legacy registration shims.
