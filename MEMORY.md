...
- latest_completed_leaf: `TOP-RULE-AS-NORMAL.3.2` (`TOP-RULE-AS-NORMAL.3.2 - lock Rust recursive top-rule values`).
- latest_commit: `7132108b` (`RUST-PARITY.9 - finalize Rust parity documentation`) until the `.3.2` commit lands; update this hash immediately after commit. Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `TRACE-OBSERVABILITY`; `TOP-RULE-AS-NORMAL` is closed: top rule is an ordinary rule entered first; Perl/Rust no-consume recursion cuts are in place; Rust recursive top/body value parity is locked by declared-variable scoping and the 91-fixture oracle corpus. `SPEC-FORMAT-TERSE` declaration-retirement `.6` and type-method `.7` lanes are closed, with no pending terse leaf.
- next_action: Pick `TRACE-OBSERVABILITY.1` — trace coverage audit before CLI/docs/coverage work.
