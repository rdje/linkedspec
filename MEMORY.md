...
- latest_completed_leaf: `PROJECT-DATA-SSD-ROOTING.3.1.2` — marker-v2 process groups close the live-descendant
  deletion RED with group drain/signals, conservative recovery, focused orphan proof, and canonical signoff.
- latest_commit: `PROJECT-DATA-SSD-ROOTING.3.1.2 - guard managed-run descendants` (this commit).
...
- current_storage_frontier: `.3.1` is complete. `.3.2` independently proves exact off-repository residue zero,
  deletes only any newly proven exact owner, and re-proves retained repository-filesystem copies usable.
...
- current_signoff: `.3.1.2` lifecycle/environment/35-route and six storage oracles pass; canonical Rust 77.46s,
  Dart 1/1, Julia 416/27.1s, primary 66x2, Phase 0 1,031/625s, KM/book/doctrines, zero runs pass.
...
- push_policy: hard lock at 300 new local commits; current counter 34/300 after this commit; never push per commit.
...
- blockers: none. in_flight_uncommitted: none after this commit. next_action: from clean `.3.1.2`, execute
  `PROJECT-DATA-SSD-ROOTING.3.2`; prove final exact residue zero, commit at 35/300, and do not push.
