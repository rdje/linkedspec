...
- activation_commit: `2dceb96e` — clean immutable-core HEAD from which Perl projection `.14.2.1.2` activated
  task-tree-first (141/300; no push); Git derives the future landing identity.
- latest_completed_leaf: `.14.2.1.2` is signoff-complete for intended landing 142/300 under its exact task-tree
  subject; immutable core `.14.2.1.1` remains the latest committed leaf until that landing (no push).
- active_leaf: `FUTURE-PARITY-BACKLOG.14.2.1.2` — signoff-complete; commit/clean mechanics only.
- active_scope: exact detached 92-row/four-family catalog plus seven aliases; handler authority initialization,
  child propagation, ActionIR/nested-method/RuleIR/handler-bridge routes, and scalar mark/cursor compatibility.
- active_progress: projection 3 top-level/202 nested and focused baselines 423/423 pass. Canonical passes containment,
  moved-root, CLI 66x2, RAM 53%, and Phase 0 1,031/1,031 in 651 seconds; Knowledge Map is 785/6,389.
- active_exclusions: admission/registration and rollout promotion, schema/semantic/generated-plan format, DSL/
  facade, mdBook support/readability, root README, push, and unrelated cleanup.
- in_flight_uncommitted: none intended after this commit; `.14.2.1.2` is fully implemented, verified, synchronized,
  and signoff-complete. Clear the brief and prove exact clean state after landing.
- next_after_clean_commit: activate admission `.14.2.1.3` task-tree-first; register the Perl consumers, promote only
  the Perl runtime row, and update sole-facing support truth after exact admission.
...
- current_storage_routing: 42 hook/doctrine/KM/canonical/backend/book/matrix/storage/path boundaries self-root and
  enter one managed run; Cargo/Dart/Julia/Lua/Python use supported root-relative wrappers.
- current_storage_backends: Perl 24, Rust 17, Dart 19, Julia 18, and Lua 17 owner proofs route project data to the
  repository volume; only documented read-only toolchain/OS resources remain external.
- current_storage_tools: tool oracle freezes 3 Python temp / 14 shell allocator / 28 Python entrypoints; mdBook
  validates and executes output from one book root. New Perl RED consumers read tracked JSON and allocate no output.
...
- current_semantic_governance: six groups / 20 responses / 128 mutations; rollout 9/9; native admission 6/6;
...
- current_typed_source: neutral/public composition plus rollout planning/correction are closed at 3/11 and 37
  mutations. Perl RED `.0`, core `.1`, and projection `.2` are signoff-complete; `.3` admission remains.
...
- push_policy: hard lock at 300 new local commits; Perl projection `.14.2.1.2` intended landing is 142/300; no push.
...
- canonical_state: `.14.2.1.2` definitive canonical CI passed containment, composed semantic/MCP admissions, CLI
  66x2, RAM 53%, and Phase 0 1,031/1,031 in 651 seconds before `local CI gate passed`.
- blockers: none; commit/clean `.14.2.1.2`, then activate `.14.2.1.3` task-tree-first; no push.
