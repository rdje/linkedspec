...
- next_action: `SPEC-LANG-REFERENCE.10.4` — reshape the `.9` §5.5 `runtime-semantics.md` Pair example to the doctrine 2-rule form (rationale: it violates the doctrine — regex on top — not that it "returns []"); re-verify through `LinkedSpec::Get`; `mdbook build` exit 0.
- queued_after: `.10.5` (audit/reshape other regex-on-top book examples: `worked-spec-walkthrough.md`, the §5.5 fixtures, etc.) → `.5.3` Array → `.5.4` Hash+Control Flow → `.5.5` (closes `.5`) → `.6` → `.7` KM cards → `.8` finalize. Then `RUST-PARITY.7.5.3`.
