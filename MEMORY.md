...
- 2026-04-10: `push_call(Rule)` is now the compact convention-over-configuration helper for "call a child rule and push the result into the current rule array"; `push_call(Rule, index)` pushes one indexed element from a shaped child return, and explicit-target `push_call(target, Rule[, index])` remains available when needed. `specs/tkgui.spec`, `specs/regdef.spec`, and the indexed quoted-string accumulator in `specs/ebnf.spec::logging_annotation` now use `push_call(...)` and remain on canonical `PUSH` ActionIR without changing parser AST shapes. Future compatibility-surface cleanup can continue with the larger raw push-call clusters in `ds_vhistory.spec` or the mixed push-plus-state-update bands in `simenv.spec`, but keep those as separate bounded slices.
