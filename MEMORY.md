...
- latest_commit: `d689cfe` (`SPEC-FORMAT-TERSE.2.2.2 - own Perl attached if blocks`). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — **`.2.2.2` implementation in-flight; frontier `.2.2.3` after commit**. User in a **PNT loop** (2026-06-23).
- next_action: Finish checks/docs for `SPEC-FORMAT-TERSE.2.2.2`, commit it, then own `.2.2.3` before any Rust attached-if parity code.
...
- blockers: NONE PNT-eligible-blocking. in_flight_uncommitted: `.2.2.2` Perl splitter/tests/book/KM/live docs until commit; known unrelated local paths `rgx` and `.claude/projects/` must remain unstaged.
