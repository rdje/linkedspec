...
- latest_completed_leaf: `SCALAREF-RETIREMENT.2` (`SCALAREF-RETIREMENT.2 - inventory scalaref retirement contract`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SCALAREF-RETIREMENT` — `.2` is done: inventory and replacement contract are locked. Function form migrates to direct nested access; receiver-dot `.scalaref(key)` migrates through named hash reads/temps. `RUST-PARITY.7.5.2` remains the committed parity fix for current Lispish behavior.
- next_action: Pick `SCALAREF-RETIREMENT.3` next: migrate shipped specs, tests, oracle fixtures, and public docs away from `scalaref(...)` before removing implementation support in `.4`. `RUST-PARITY.7.2` remains pending after/behind the retirement migration/removal track.
...
- blockers: NONE PNT-eligible-blocking. in_flight_uncommitted: none intended after `.2` commit; known unrelated local paths `rgx` and `.claude/projects/` must remain unstaged.
