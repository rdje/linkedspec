...
- 2026-04-17: RTL/FSM VHDL header generation moved out of plugin dispatch. `perl/RTLUtils.pm` now owns `RTLUtils::add_header_n_context_clause(...)`; `RTLUtils` entity/architecture generation and `FSMGen.pm` package code call it directly; and `plugin/fsmgen.plg::add_header_n_context_clause` is only a thin compatibility wrapper. This retires the repo-owned `LinkedSpec::run_plugin('add_header_n_context_clause', ...)` header path while preserving the legacy plugin name for external/legacy callers.
