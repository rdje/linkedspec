...
- latest_completed_leaf: `TRACE-OBSERVABILITY.4.5` (`TRACE-OBSERVABILITY.4.5 - close trace parity proof`; commit `e430ddce`).
- latest_commit: `e430ddce` (`TRACE-OBSERVABILITY.4.5 - close trace parity proof`). Ahead of origin remains below push threshold 300 (293); do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE.11` active by user directive 2026-07-05; it owns duck-typed `.spec` assignment semantics (`name = value` binds runtime typed values, including array/hash RHS values) and keeps delimiterless aggregate RHS sugar out of the MVP.
