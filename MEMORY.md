...
- latest_commit: `3153f9c8` (`SPEC-FORMAT-TERSE.15.2.2 - Perl reference bare-read completion in value positions`); previous `5a59b719` (`.15.2.1`). **~306 commits ahead of origin — OVER the documented 300 push threshold (LIVE_ACHIEVEMENT_STATUS); still do NOT push mid-PNT unless explicitly instructed.**
- active_work_unit: `SPEC-FORMAT-TERSE.15.2.3` (Rust parity for `.15.2.2`). Then `.15.2.4` source migration (output-preserving), then `.15.3`/`.15.4` remove `:name` ENTIRELY (no compat; user directive), `.15.5` closeout.
- next_action: Start `SPEC-FORMAT-TERSE.15.2.3`. SCOPED (static read, engine.rs): Rust `eval_expr` arm `Expr::Variable{name} => Ok(ctx.get_bare_value(name))` (`rust/linkedspec-runtime/src/engine.rs:3286`) ALREADY reads bare; `:name`=`Expr::ScalarSlot`→`get_scalar` (`:3287`). The statement-switch handler evaluates BOTH selector (`switch`, `:2652`) AND case value (`case`, `:2675`) via `eval_expr`, so `switch(kind)`/`if`/`num_*` bare reads likely ALREADY match Perl — VERIFY with the discriminating specs (build a small Rust probe/oracle fixtures; do NOT assume). KEY DIVERGENCE to resolve: Rust `case(foo)` currently reads VARIABLE foo (`:2675` eval_expr→get_bare_value), but Perl `.15.2.2` treats `case(foo)` as LITERAL "foo" (hash-key-analogous exemption). Make Rust `case(bareword)` a literal tag to match (mirror `ControlFlow::_lower_switch_case_value_expr`), keeping `case(:name)`/quoted as non-literal. This case-value divergence may be PRE-EXISTING (check if any oracle fixture uses bare case values). Prove via regenerated oracle corpus (`tools/gen_oracle_corpus.pl` — BACKGROUND, slow) + `cargo test` corpus_oracle green; Perl reference == Rust for all discriminating cases.
