...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.13.2` — Perl reference array-tree receiver block traversal landed for
  `walk_leaves`, `map_leaves`, and `reduce_leaves(initial)`; frontier moves to `.13.3` for Rust/oracle parity.
- prior_leaf: `SPEC-FORMAT-TERSE.13.1` — array-tree traversal accepted and split before parser/runtime code.
- latest_commit: HEAD containing this pointer should be
  `SPEC-FORMAT-TERSE.13.2 - implement Perl array-tree traversal`; parent before this slice is
  `45e16e31 SPEC-FORMAT-TERSE.13.1 - split array-tree traversal`.
- push_policy: check `git status -sb` for the live ahead count; do not push mid-PNT unless explicitly instructed
  or the documented 300-commit threshold policy is deliberately invoked.
- active_work_unit: `SPEC-FORMAT-TERSE.13.3` — Rust parser/runtime parity and generated oracle fixture for
  array-tree traversal receiver blocks.
- next_action: implement Rust parity for array-valued `walk_leaves`, `map_leaves`, and
  `reduce_leaves(initial)` receiver blocks, regenerate the Perl-backed oracle fixture, and verify Rust still
  preserves `.12` hash-tree behavior. `DOCTRINE-ENFORCEMENT-ADOPT.3` stays deferred, and
