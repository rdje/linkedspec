...
- latest_commit: `<this commit>` — `SPEC-FORMAT-TERSE.1.4.1 — Perl recognize terse renames set/cat/copy lowering identically to assign/concat/array_copy+hash_copy (engine + book + 4 phase0 locks)` (hash backfilled by the follow-on handoff). Prior: `5e93af3` `.1.4` split, `5e9ef57` `.1.2.2`, `b77510a` `.1.2.1`. Ahead of origin ~55 — push threshold ~300; do NOT push mid-PNT.
