...
- latest_completed_leaf: `STAGED-LINKED-PARSING.5.6` (`STAGED-LINKED-PARSING.5.6 - prove function-body staged prototype`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `STAGED-LINKED-PARSING` — first function-body staged prototype complete. ADRs `0012`-`0017` define staged doctrine, imports, parse-job annotations, registry queues, neutrality, and target AST shape. `specs/user_function_definition.spec` owns the definition shell; Perl/Rust consume that AST, preserve `body_payload`, dispatch normalized `body_parse_job` through built-in `actionir-body.spec` / `action_block`, stitch `body_ast`, and `.5.6` proves descriptor/parsed/compiled shape, runtime behavior, and provenance diagnostics. Current parsers still do not accept public `parse_job(...)`, import/provider search roots, multiple payload families, recursive staged queues, or cycle diagnostics.
- next_action: PNT returns to `RUST-PARITY.7.3` (expand remaining/harder shipped-spec oracle candidates with recorded divergence evidence) unless a new staged linked parsing leaf is explicitly split first.
