...
- 2026-04-12: String substitution graduated out of the plugin namespace. `perl/Plugin/String.pm` is gone; `perl/Text/VariableSubstitution.pm` owns `var_subst(...)` and `var_subst_test(...)`; and `Plugin::CGI::file_list_path2http(...)` calls `Text::VariableSubstitution::var_subst(...)` directly.
...
- 2026-04-11: `plugin/cgi.plg` compatibility wrapper removed. `Plugin::CGI::file_list_path2http(...)` now owns all repo-owned `file_list_path2http` usage directly, and the regression suite locks that the wrapper is gone instead of still parsing under `pplugin`. Future `.plg` extraction should delete the wrapper as soon as no concrete repo-owned legacy caller still needs the old plugin-registration name.
