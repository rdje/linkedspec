...
- latest_completed_leaf: `JULIA-BACKEND-PARITY.4.3.1` — Julia now preserves core scalar/array/hash/null/boolean/
  number shapes through typed and bare snapshots, assignments/access, checked nested writes, and entry/local
  capture maps and positions; full Julia tests pass with 554 assertions.
- prior_leaf: `JULIA-BACKEND-PARITY.4.3.0` — the broad helper/value runtime container was split by mechanism into
  core stores/captures, string/numeric, array, hash, value/control/block/callback, and no-drift leaves.
...
- latest_commit: this resume block is prepared for commit
  `JULIA-BACKEND-PARITY.4.3.1 - add Julia runtime value capture helpers`; previous committed HEAD is
  `dabc9361 JULIA-BACKEND-PARITY.4.3.0 - split Julia runtime helper families`.
- push_policy: check `git status -sb` for the live ahead count; do not push mid-PNT unless explicitly instructed
  or the documented 300-commit threshold policy is deliberately invoked.
- active_work_unit: `JULIA-BACKEND-PARITY`; current frontier after the current commit is `.4.3.2`.
- next_action: resume PNT at `JULIA-BACKEND-PARITY.4.3.2` to add the current string/scalar helper family,
  explicit lexical comparisons, numeric arithmetic/reducers/comparisons and aliases, plus compatible receiver
  chains over the `.4.3.1` copied-value/store model.
...
- blockers: none. in_flight_uncommitted: none expected after the `JULIA-BACKEND-PARITY.4.3.1` commit; do not pivot
