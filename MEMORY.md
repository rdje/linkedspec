...
- latest_completed_leaf: `RUST-PARITY.7.3.4` (`RUST-PARITY.7.3.4 - triage structural oracle mismatches`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `RUST-PARITY` — `.7.3.4` reproduced `portmap`/`lib_reader`/`ebnf` structural Rust oracle mismatches and split implementation owners. `portmap` needs runtime boolean/list-context parity (`or`, `array(flat_array(...))`) plus fluent recursive aggregation for concat; `lib_reader` first needs parser/compiler header-rest action-edge handling for regex-less top rules (`lib_file:: -> group .push` was absent from the compiled top rule); `ebnf` compiles `.push(child, rule)` chains but runtime duplicates rule headers and drops token payloads. `.7.3.5` later triages `BNF`/`DT`/`ifelse`/`operators_try`/`spec.spec`; `.7.3.6` audits RTL/plugin/legacy candidates. Staged linked parsing first prototype is complete; broader public staged surfaces remain future work.
- next_action: Pick `RUST-PARITY.7.3.4.1`: fix Rust parser/compiler handling for header-rest action edges on regex-less top rules, starting with `lib_reader`'s `lib_file:: -> group .push`, then re-run the representative `lib_reader` sattribute/cattribute oracle probes.
