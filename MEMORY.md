...
- latest_completed_leaf: `FUTURE-PARITY-BACKLOG.10.7.2.1` — parser-free Lua semantic source map.
- latest_commit: `FUTURE-PARITY-BACKLOG.10.7.2.1 - implement Lua semantic source map` (this source commit).
...
- current_lua_semantic_plan: `.10.7.1`, `.10.7.2.0`, and source `.10.7.2.1` are complete. One parser-free weak-key
  owner exposes root `semantic_index(source, options)` with strict options/UTF-8, exact private byte/scalar maps,
  arithmetic SHA-256, four ceilings, opaque/detached values, typed errors, and 377 assertions per ABI. Active
  `.10.7.2.2` owns one staged compiled-or-failed outcome, entry, diagnostic, and generated-v2 plan.
...
- current_signoff: Lua classifier/routes/identity/body/negative 1,706/179/359/166/1,542 per ABI; complete Lua
  ...
  KM 702/5,450; mdBook; canonical Rust 78.08s + Dart 1/1 + Julia 416/27.4s + reference primary 66x2 + Phase 0
  1,031/631s; reusable SSD caches retained and only explicit task scratch selected for cleanup.
...
- push_policy: hard lock at 300 new local commits; current counter 12/300 after this commit; never push per commit.
...
- blockers: none. in_flight_uncommitted: none after the `.10.7.2.1` source commit. next_action: from the clean
  boundary, implement one retained staged compiled-or-failed Lua outcome under `.10.7.2.2`; do not push.
