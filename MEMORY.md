...
- latest_completed_leaf: `JULIA-BACKEND-PARITY.7.3.2.4` — Julia primary failures/exits and the complete trace
  stdout/route/mirror/file/reset/emoji matrix are normalized around canonical JSON.
- prior_leaf: `JULIA-BACKEND-PARITY.7.3.2.3` — native execution and recursive canonical direct JSON are locked.
...
- latest_commit: this resume block is prepared for commit
  `JULIA-BACKEND-PARITY.7.3.2.4 - normalize Julia CLI failures and trace routing`; previous committed HEAD is
  `d9999233 JULIA-BACKEND-PARITY.7.3.2.3 - execute Julia primary parser requests`.
- push_policy: check `git status -sb` for the live ahead count; do not push mid-PNT unless explicitly instructed
  or the documented 300-commit threshold policy is deliberately invoked.
- active_work_unit: `JULIA-BACKEND-PARITY`; its frontier after this commit is `.7.3.2.5`.
- next_action: close Julia primary CLI conformance with direct-process success/failure/trace assertions, finalize
  the focused gate and public status, and audit all Julia/task/book/live surfaces for no drift.
...
- latest_bootstrap_read: 2026-07-10 read the full roadmap and roadmap-v2, full codebase inventory and active Julia
  ...
  frontend trace propagation, primary CLI argument/loading/execution/canonical JSON, phase-ordered stable failures,
  complete trace routing/emoji, and the 1,017/99 proof.
...
- blockers: none. in_flight_uncommitted: `.7.3.2.4` is verified and ready for its prepared commit; none expected
  afterward. Do not advance to `.7.3.2.5` until the tree is clean.
