...
- 2026-04-11: `plugin/http.plg` removed after moving the remaining real `http` file-link action into `Plugin::HTTP::print_file_links_for_conf(...)`. `Plugin::HTTP` now owns the historical CGI config lookup, host/port setup, existing-file filter, and signed URL printing directly. `plugin/lighttpd.plg` remains because it still carries the real legacy `lighttpd` action; helper-only `.plg` shadows should keep being deleted once no repo-owned caller needs the registration name.
