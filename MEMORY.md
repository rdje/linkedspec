...
- latest_completed_leaf: `PROJECT-DATA-SSD-ROOTING.2.2` — repo-local Cargo covers all 195 locked packages offline;
  all 17 Rust temp owners, generated projects, traces, and copied-binary execution use managed SSD storage.
- latest_commit: `PROJECT-DATA-SSD-ROOTING.2.2 - root Rust workspaces on SSD` (this commit).
...
- current_storage_routing: 18 hook/doctrine/KM/canonical/backend/book/matrix/storage boundaries source the
...
- current_storage_frontier: `.2.3` is active after `.2.2`: migrate Dart cache/test/generated state, verify retained
  SSD copies, delete exact old Dart-owned records, and pass complete Dart gates.
...
- current_signoff: `.2.2` standalone/reused Rust storage, offline Cargo, 18-boundary routing, path doctrine, and
  full Rust gate pass: runtime 149, corpus 105, generated classifier 105, integration 197, semantic admission,
  primary 66x2. Canonical: Rust 77.85s, Dart 1/1, Julia 416/416 27.1s, primary 66x2, Phase 0 1,031/1,031 624s;
  zero old Rust temp residue and zero managed runs. Docs/doctrines pass before commit.
...
- push_policy: hard lock at 300 new local commits; current counter 28/300 after this commit; never push per commit.
...
- blockers: none. in_flight_uncommitted: none after this commit. next_action: from clean `.2.2`, implement
  `PROJECT-DATA-SSD-ROOTING.2.3`; migrate/verify/delete exact Dart-owned data, commit at 29/300, and do not push.
