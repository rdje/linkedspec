...
- 2026-06-04 (MEMORY-ARCHITECTURE-DOC.2): Created `docs/decisions/` (layer C) + INDEX + 4 ADR records (0001 doctrine, 0002 ActionIR-ready invariant, 0003 raw-Perl-free policy, 0004 CI-disabled). Doctrine migrated from harness memory into the repo. perl -c OK; 1004 PASS. PNT frontier `.3` (demote this MEMORY.md to ≤60-line resume pointer + reconcile COMMIT.md).
- 2026-06-04 (MEMORY-ARCHITECTURE-DOC.1): Adopting the durable agent-memory architecture (`MEMORY_ARCHITECTURE.md`) — new active tree. .1 done: standard at repo root (verbatim; Knowledge Map not adopted) + README/SESSION_BOOTSTRAP pointers. perl -c OK; 1004 PASS. PNT frontier `.2` (docs/decisions layer C). NOTE: this MEMORY.md is itself being demoted to a bounded ≤60-line resume pointer in `.3`; its long history stays in git.
- 2026-06-04 (DOC-CODEBASE-ALIGNMENT.5 — TREE COMPLETE): Synced `ROADMAP.md`'s status tracker with `ROADMAP_V2.md`/reality (13 stale rows flipped: Phases 1-7 done, Backbone done, Method-like mostly done, Plugin done; Overall stays in progress). Annotated Phase 1A planning section that ActionRewriter.pm was deleted in Phase 1. 15/15 statuses now match V2. **DOC-CODEBASE-ALIGNMENT tree COMPLETE (5 leaves).** No active trees — PNT idle (proposed `PLUGIN-ACTION-MIGRATION` not activated). perl -c OK; 1004 PASS. Not pushed yet (per batch push policy).
- 2026-06-04 (DOC-CODEBASE-ALIGNMENT.4): Reconciled `ROADMAP_V2.md` Phase 1A row — ActionRewriter now historical there (Phase 1 row already recorded the deletion). DISCOVERED: `ROADMAP.md`'s status tracker is broadly stale (Phase 1/1A `mostly done`, 2-6 `in progress`, 7 `not started`, Backbone `mostly done` — all `done` per V2). Split into new leaf `.5` to sync ROADMAP.md's tracker. perl -c OK; 1004 PASS. PNT frontier: `.5`.
- 2026-06-04 (DOC-CODEBASE-ALIGNMENT.3): Scrubbed `USER_GUIDE.md` of the stale claim that `ActionRewriter.pm` is a live/compatibility module (7 refs across two clusters). Now framed as deleted in Phase 1; entrypoint = `RuleIR::EmitContext::rewrite_action_code_for_compat`. Book already clean. perl -c OK; 1004 PASS baseline. PNT frontier: `.4` (reconcile ROADMAP.md + ROADMAP_V2.md — Phase 1A status + live ActionRewriter-seam language).
...
- 2026-05-16 (PHASE2-DSL-FRONTEND.6): Completed fluent-continuation surface regression-lock. 4 new subtests (20 assertions): lifecycle markers with fluent chains, deeply nested 5+ call chains, quoted args with nested parens, empty-args method calls. Full suite: 999 tests PASS. Next frontier: `.4` (inside-block rule-start detection).
- 2026-05-16: Installed task-tree tracking workflow. Active trees: `PHASE2-DSL-FRONTEND` (Phase 2, current frontier `.1`), `PHASE1A-CLOSE-OUT` (Phase 1A), `PHASE3-EXECUTION-SEMANTICS` (Phase 3), `PHASE4-CAPTURE-MARK-API` (Phase 4), `PHASE5-RUNTIME-DIAGNOSTICS` (Phase 5), `PHASE6-DOCUMENTATION` (Phase 6), `BACKBONE-ACTION-IR-LOWERING` (Backbone Item 3), `PLUGIN-MODERNIZATION`. Proposed: `PHASE7-SELF-HOSTED-SPEC`. All start with inventory leaves. PNT selects first eligible leaf from first active tree unless user names another. PostCompact hook re-reads live-docs + `docs/TASK_TREE.md` + mdBook SUMMARY/index. to reject direct `LinkedSpec::get_plugin(...)`, `LinkedSpec::run_plugin(...)`, and `LinkedSpec::dispatch_plugin_autoload_name(...)` calls. The plugin-corpus regression now names the package-owner destination rather than the older get-plugin transition step.
- 2026-05-11: Removed the stale commented `LinkedSpec::get_plugin('setup_hold_tmax_tmin')` lookup from `plugin/stan_omap2430c_backend.plg`. Phase0 now scans all shipped `.plg` files and rejects direct `LinkedSpec::get_plugin(...)` calls, keeping repo-owned plugin files on package-owner calls rather than bridge lookup.
...
- 2026-05-10: Removed unused `Compiler::_compiled_spec_state_compiled_rule_order(...)`; compiled-rule order access no longer has a compiler-local mirror of `CompilerState`. Phase0 source-lock coverage rejects reintroducing the compiled-rule-order pass-through wrapper.
- 2026-05-10: Removed `Compiler::_compiled_spec_state_rule_info(...)`; dependency-regex validation now retrieves referenced dependency-rule metadata directly from `CompilerState`. Phase0 source-lock coverage rejects reintroducing the rule-info pass-through wrapper.
...
- 2026-05-10: Removed `Compiler::_describe_build_dependency_regex_map_rule_dependency_refs_result(...)`; the dependency-regex map validation boundary now builds invalid `dependency_refs` diagnostics directly while still sharing the contract value-kind formatter. Phase0 source-lock coverage rejects reintroducing the one-shot `dependency_refs` describer wrapper.
- 2026-05-10: Removed `Compiler::_describe_build_dependency_regex_map_rule_info_result(...)`; the dependency-regex map validation boundary now builds invalid rule-info diagnostics directly while still sharing the contract value-kind formatter. Phase0 source-lock coverage rejects reintroducing the one-shot rule-info describer wrapper.
- 2026-05-10: Removed `Compiler::_describe_build_dependency_regex_map_spec_result(...)`; the compiled-spec input normalization callback now builds invalid input diagnostics directly while still sharing the contract value-kind formatter. Phase0 source-lock coverage rejects reintroducing the one-shot dependency-regex map spec-result describer wrapper.
...
- 2026-05-09: Removed unused `Runtime::_set_runtime_ctx_last_error(...)`; runtime-owner fallback errors use the preserve-existing RuntimeContext helper, and phase0 source-lock coverage rejects reintroducing the dead direct setter wrapper.
...
- 2026-05-09: Removed `Compiler::_emit_runtime_ctx_parser_source_line(...)`; compiler parser-source emission now calls `RuntimeContext::emit_runtime_ctx_parser_source_line(...)` directly through `_call_runtime_ctx(...)`. Phase0 source-lock coverage rejects reintroducing the wrapper.
- 2026-05-09: Removed `Compiler::_clear_runtime_ctx_last_error(...)`; compiler operation-boundary and parser-invocation stale-error clearing now call `RuntimeContext::clear_runtime_ctx_last_error(...)` directly through `_call_runtime_ctx(...)`. Phase0 source-lock coverage rejects reintroducing the wrapper.
...
- 2026-05-09: Removed `Runtime::_runtime_owner_handler_source_label(...)`; runtime-owner fallback diagnostics now call `RuntimeContext::build_runtime_ctx_top_rule_handler_source_label(...)` directly through `_call_runtime_ctx(...)`. Phase0 source-lock coverage rejects reintroducing the wrapper.
...
- 2026-05-09: Added `RuntimeContext::clear_runtime_ctx_parser_source_capture(...)` and routed `run_get(...)`, `get_parser(...)`, and low-level `build_compiled_rule_table(...)` preparation through it. The helper centralizes clearing stale parser-source chunks plus removing stale emit callbacks, while `run_get_pipeline(...)` still clears only chunks so active parser-source emission remains wired during compiler output. Focused phase0 coverage now locks the reset helper directly.
- 2026-05-09: Continued the Phase 5 runtime-context owner cleanup on the standalone `build_compiled_rule_table(...)` path. The shared `RuntimeContext` preparation helper now clears stale parser-source chunks and removes stale parser-source emit callbacks there too, so reused contexts cannot carry generated parser-source state from earlier `Get(...)` / `get_parser(...)` runs into low-level rule-table diagnostics. Focused phase0 coverage locks both direct helper behavior and compiler-routed cleanup.
- 2026-05-05: Continued Phase 5 runtime-context owner cleanup. `LinkedSpec::RuntimeContext` now owns low-level `build_compiled_rule_table(...)` runtime-context preparation too, so `Compiler.pm` delegates hook normalization, stale `last_error` clearing, stale `spec_name` / `spec_path` clearing, and selected-`top_rule` seeding through the shared runtime-context owner. Phase0 now locks the direct helper behavior and the compiler routing seam.
- 2026-05-05: Re-ran the README/SESSION_BOOTSTRAP reading path and continued Phase 5 runtime diagnostics by locking reusable populated scalar-slot `runtime_ctx_ref => \$ctx` behavior through the public `LinkedSpec::Get(...)` and `LinkedSpec::get_parser(...)` entrypoints. The new regression coverage proves public callers can reuse the same materialized context hashref, preserve caller-owned fields, refresh current run identity, and replace stale `last_error` data.
- 2026-04-30: Parked further `.plg` slices per user direction and switched the active PNT thread back to Phase 5 runtime diagnostics. `LinkedSpec::RuntimeContext` now names/tests the reusable scalar-slot `runtime_ctx_ref => \$ctx` contract explicitly: an empty scalar slot is populated with the context hashref, a later call reuses that same hashref even though Perl reports the reference shape as `REF`, direct hashrefs remain supported, and scalar slots already holding non-hash references are rejected with a public contract error.
- 2026-04-30: The obsolete `plugin/tssio.plg` compatibility wrapper is gone. `Timing::SetupHold::write_tssio(...)` remains the package-owned `tssio` report/action body, phase0 locks the wrapper absence, and the direct package-owner smoke still preserves `tssio.lof`, per-path `sta_<n>.lof` sheets, banner/error strings, internal link formatting, and a no-`PPlugin` load boundary.
...
- 2026-04-30: `specs/ebnf.spec` now reports `compatibility_surface_rule_count == 0`; the final cleanup migrated `grammar_file` to helper-form child-call assignment, include readers to helper-form `entry_text()` cleanup plus split/trim/filter pipelines, `semantic_annotation` to `capture_slice()` and helper-form return payloads, and `logging_annotation` to explicit rolling `start_capture_slice()` movement plus stored `entry_group(0)` names. Phase0 locks zero compatibility metadata, source spelling, and preserved include / semantic / logging parser behavior.
- 2026-04-30: `specs/regdef.spec` now reports `compatibility_surface_rule_count == 0`; the final cleanup migrated top/reg/field array payloads to helper-form `return(a(...))` / `I.return(...)` with `array_copy(a(...))` accumulator snapshots, and migrated `ob_cb` to `return(1)`. Phase0 locks zero compatibility metadata, source spelling, and a runtime smoke for the nested `?regdef_top:` / `?reg_def:` / `?reg_fld:` AST shape.
- 2026-04-30: `specs/pplugin.spec` now reports `compatibility_surface_rule_count == 0`; the final cleanup migrated `pplugin_top` to helper-form declarations, `next()`, `assign(...)`, structured `if(is_defined(s(retv)))` aggregation flow, `return_undef()`, and `return(hash(flat_array(a(defs))))`, migrated `subdef` to helper-form `return(a(entry_named(subname), ...))`, and migrated `curlyb` to `return_undef()`. Phase0 locks zero compatibility metadata, source spelling, and preserved plugin hash/coderef smoke behavior.
- 2026-04-30: `specs/hlink_substitution.spec` now reports `compatibility_surface_rule_count == 0`; the final cleanup migrated dangling/unmatched delimiter exits to `exit_now(...)`, migrated the bracket scalar-ref return to helper-form `return(\(my $capt = capture_slice()))`, and migrated the wrapped brace return to `return(concat("{", capture_slice(), "}"))`. Phase0 locks zero compatibility metadata, source spelling, and preserved bracket/brace/raw-string parser smoke output.
- 2026-04-30: `next()` is now the canonical action-helper spelling for rule-flow skips; it lowers to runtime `next`, emits `NEXT` ActionIR metadata, and does not count as compatibility syntax, while bare `next` remains compatibility surface. `specs/tkgui.spec` now reports `compatibility_surface_rule_count == 0` after migrating `sub_gui_list` and `curlyb` to `next()`, `return_undef()`, and `return(hash(flat_array(a(sub_gui_list))))`; phase0 locks the helper contract, source spelling, and preserved parser smoke output.
...
- 2026-04-29: Fluent `.return(capture_slice_len())` now preserves canonical general-return payload semantics instead of becoming legacy label-injected return form. The block and fluent direct `capture_slice_len()` return shapes now share clean ActionIR metadata, and `specs/sdce.spec::oc_brace` spends `return(capture_slice_len())` instead of raw `$LSPOS - $IPOS - 1`; phase0 locks the helper spelling, `CAPTURE_SLICE_LEN` node coverage, and zero raw/unresolved counts.
- 2026-04-29: Generalized `return(payload)` plus fluent `.return(payload)` now recognize direct immediate text/group helper payloads such as `entry_text()` as canonical general-return payloads. This keeps documented `return(entry_text())` and fluent `I.return(entry_text())` out of raw fallback / legacy label-injected `return(label, ...)` rendering. `specs/hlink_substitution.spec::raw_string` now uses `entry_text()` instead of raw `$IMATCH`, and phase0 locks that rule as raw-fallback-free and language-agnostic ActionIR-ready.
- 2026-04-29: `BootstrapSpec.pm` no longer carries the separate `_require_bootstrap_core_pkg(...)` loader wrapper. Its meaningful local bootstrap grammar seam remains `build_bootstrap_spec(...)`, and that helper now resolves `BootstrapSpec::Core::build_bootstrap_spec(...)` directly through `LinkedSpec::OwnerDispatch::require_pkg_cb(...)` inside its `$@`-preserving body; phase0 now locks that slimmer bootstrap facade shape.
- 2026-04-29: `RuleIR.pm` no longer carries the separate `_require_data_dumper_pkg(...)` loader wrapper. Its meaningful local RuleIR dump-formatting seam remains `_dump_value(...)`, and that helper now lazy-loads `Data::Dumper` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside its `$@`-preserving body; phase0 now locks that slimmer RuleIR dump helper shape.
- 2026-04-29: `SpecEntry.pm` no longer carries the separate `_require_data_dumper_pkg(...)` loader wrapper. Its meaningful local spec-entry dump-formatting seam remains `_dump_value(...)`, and that helper now lazy-loads `Data::Dumper` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside its `$@`-preserving body; phase0 now locks that slimmer spec-entry dump helper shape.
- 2026-04-29: `Compiler.pm` no longer carries the separate `_require_data_dumper_pkg(...)` loader wrapper. Its meaningful local compiler dump-formatting seam remains `_dump_value(...)`, and that helper now lazy-loads `Data::Dumper` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside its `$@`-preserving body; phase0 now locks that slimmer compiler dump helper shape.
- 2026-04-29: `Compiler.pm` no longer carries the separate `_require_linkedre_pkg(...)` loader wrapper. Its meaningful local compiler regex seam remains `_ored_re(...)`, and that helper now lazy-loads `LinkedRE` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside its `$@`-preserving body; phase0 now locks that slimmer compiler regex helper shape.
- 2026-04-29: `BootstrapSpec/Core.pm` no longer carries the separate `_require_linkedre_pkg(...)` loader wrapper. Its meaningful local bootstrap regex seams remain `_linkedre_or(...)` and `_linkedre_ored_re(...)`, and those helpers now lazy-load `LinkedRE` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside their `$@`-preserving bodies; phase0 now locks that slimmer bootstrap-core helper shape.
- 2026-04-29: `ActionIR/CanonicalEvents.pm` no longer carries the single-use `_require_canonical_events_core_pkg(...)` loader wrapper. Its meaningful local canonicalization seam remains `_canonicalize_helper_action_ir_event(...)`, and that helper now lazy-loads `CanonicalEvents::Core` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside its `$@`-preserving delegation body; phase0 now locks that slimmer canonical-events owner shape.
- 2026-04-29: `ActionIR/StatementSplit.pm` no longer carries the single-use `_require_statement_split_core_pkg(...)` loader wrapper. Its meaningful local seam remains `_split_action_ir_statements(...)`, and that helper now lazy-loads `StatementSplit::Core` directly through `LinkedSpec::OwnerDispatch::require_pkg(...)` inside its `$@`-preserving delegation body; phase0 now locks that slimmer statement-split owner shape.
- 2026-04-29: `ActionIR/MethodLowering.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_lower_typed_declare_statement(...)`, `_normalize_method_tag_expr(...)`, `_lower_method_value_expr(...)`, `_lower_return_payload_expr(...)`, `_lower_return_general_statement(...)`, `_lower_assign_statement(...)`, `_lower_push_value_statement(...)`, `_lower_push_nonempty_statement(...)`, `_lower_regex_subst_statement(...)`, and `_lower_return_undef_statement(...)`, and those helper bodies now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer method-lowering owner shape.
- 2026-04-29: `ActionIR/ControlFlow.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain the control-flow lowering helper family from `_lower_control_flow_value_expr(...)` through `_lower_print_statement(...)`, and those helper bodies now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer control-flow owner shape.
- 2026-04-29: `ActionIR/FlowExpr.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_looks_like_array_value_expr(...)`, `_looks_like_hash_value_expr(...)`, `_lower_is_empty_expr(...)`, `_lower_defined_target_expr(...)`, and `_lower_flow_composite_expr(...)`, and those helper bodies now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer flow-expression owner shape.
- 2026-04-28: `ActionIR/ValueExpr.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_extract_scalar_symbol_name(...)`, `_extract_array_symbol_name(...)`, `_extract_hash_symbol_name(...)`, `_lower_scalar_access_key_expr(...)`, `_split_scalaref_path_segments(...)`, `_lower_scalaref_segment_expr(...)`, `_infer_scalar_container_kind(...)`, `_lower_assignment_source_expr(...)`, and `_strip_literal_delimiters(...)`, and those helper bodies now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer value-expression owner shape.
- 2026-04-22: `ActionIR/DeclareMethod.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_parse_declare_binding_entry(...)`, `_lower_declare_value_expr(...)`, `_lower_declare_initializer_expr(...)`, `_extract_declare_statement_from_method_expr(...)`, `_lower_declare_method_statement(...)`, and `_lower_assign_method_statement(...)`, and those helper bodies now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer declare-method owner shape.
- 2026-04-22: `Compiler.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_require_runtime_ctx(...)` and `run_get_pipeline(...)`, and those now validate the required runtime/pipeline dependencies inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer compiler source shape.
- 2026-04-22: `PPlugin.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seam remains `_load_legacy_registry(...)`, and that loader now validates the required parser/discovery/registry callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer legacy-registry source shape.
- 2026-04-22: `ParserFactory.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seam remains `run_get_parser(...)`, and that setup path now validates the required trace/resolve/compile callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer parser-factory source shape.
- 2026-04-22: `PluginBridge.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_lookup_plugin_name(...)` and `_dispatch_plugin_name(...)`, and those seams now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer plugin-bridge source shape.
- 2026-04-22: `ActionIR/ArrayPipeline.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_normalize_split_delimiter_expr(...)` and `_build_array_pipeline_plan_from_expr(...)`, and those seams now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer array-pipeline owner shape.
- 2026-04-22: `ActionIR/RewritePipeline.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_build_action_rewrite_rules(...)` and `_rewrite_action_code_with_diagnostics(...)`, and those seams now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer rewrite-pipeline owner shape.
- 2026-04-22: `ActionIR/Diagnostics.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seams remain `_find_unresolved_action_helpers(...)` and `_collect_action_helper_ir_nodes(...)`, and those seams now validate the required callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer diagnostics-owner shape.
- 2026-04-22: `ActionIR/CanonicalEvents.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seam remains `_build_canonical_action_ir_events(...)`, and that seam now validates the required trim/split callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer canonical-events owner shape.
- 2026-04-22: `ActionIR/StatementSplit.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seam remains `_split_action_ir_statements(...)`, and that seam now validates the required `trim_action_ir_value` callback inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer statement-split owner shape.
- 2026-04-22: `ActionIR/ScannerCore.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seam remains `_scanner_rule_dep_bindings(...)`, and that seam now validates required scanner callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer scanner-core source shape.
- 2026-04-22: `ActionIR/Contracts.pm` no longer carries a separate local `_require_dep(...)` validator wrapper. Its meaningful local seam remains `_require_lowering_deps(...)`, and that seam now validates required lowering callbacks inline instead of bouncing through a second top-level helper layer; phase0 now locks that slimmer contracts-owner source shape.
- 2026-04-22: `RuleIR/EmitContext.pm` no longer carries dead local `_require_*_pkg(...)` owner-package wrappers. Its meaningful local package-loading seam remains `_actionir_owner_package(...)`, and `_accumulate_action_rewrite_diagnostics(...)` now routes through `_call_actionir_owner('diagnostics', ...)` too, so the owner-key registry plus shared dispatcher are the only remaining package/callback-loading seams on that bridge; phase0 now locks that slimmer source shape.
- 2026-04-22: `ActionIR/ScannerCore.pm` no longer carries a separate `_scanner_rule_binding_symbols(...)` registry. Its meaningful seams remain the scanner-rule family registry plus `_scanner_dep_specs()`, and `_with_scanner_rule_family_deps(...)` now derives the rebinding symbols straight from that shared dep-spec table instead of bouncing through a second hardwired symbol list; phase0 now locks that slimmer scanner-core contract shape.
- 2026-04-22: `Compiler.pm` no longer carries a local `_call_preserving_err(...)` wrapper. Its meaningful seams remain `_dump_value(...)`, `_ored_re(...)`, `_trace_log_output(...)`, `_trace_log_dump(...)`, `_trace_should_dump(...)`, `_trace_enter(...)`, `_trace_exit(...)`, `_trace_decision(...)`, `_trace_apply_trace_options(...)`, and `_trace_level_name_for_current_verbosity(...)`, and those helper bodies now preserve caller `$@` by spending `LinkedSpec::OwnerDispatch::call_preserving_err(...)` directly instead of bouncing through a second local wrapper layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `SpecEntry.pm` no longer carries a local `_call_preserving_err(...)` wrapper. Its meaningful seams remain `_trace_enter(...)`, `_trace_exit(...)`, `_trace_decision(...)`, `_trace_log_dump(...)`, `_trace_should_dump(...)`, `_dump_value(...)`, and `_trace_runtime_mark_event(...)`, and those helper bodies now preserve caller `$@` by spending `LinkedSpec::OwnerDispatch::call_preserving_err(...)` directly instead of bouncing through a second local wrapper layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `RuleIR/EmitContext.pm` no longer carries a local `_call_preserving_err(...)` wrapper. Its meaningful seams remain `_actionir_owner_default_deps(...)`, `_call_actionir_owner(...)`, `_call_actionir_owner_with_deps(...)`, `_accumulate_action_rewrite_diagnostics(...)`, and `rewrite_action_code_for_compat(...)`, and those helper bodies now preserve caller `$@` by spending `LinkedSpec::OwnerDispatch::call_preserving_err(...)` directly instead of bouncing through a second local wrapper layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `Trace.pm`, `Validation.pm`, `ActionIR/CanonicalEvents.pm`, and `ActionIR/StatementSplit.pm` no longer carry single-use local `OwnerDispatch` wrappers. Their live helper bodies now call `LinkedSpec::OwnerDispatch` inline for Data::Dumper/Trace/core-owner loading and `$@` preservation, so this is another no-behavior-change owner-surface shrink; phase0 now locks those wrapper bodies out.
- 2026-04-22: `Runtime.pm`, `BootstrapSpec.pm`, and `ParserFactory.pm` no longer carry one-shot local `OwnerDispatch` callback-loader or `$@`-preservation wrappers. Their live orchestration helpers now call `LinkedSpec::OwnerDispatch` inline for compiler/bootstrap callback lookup and successful `$@` preservation, so this is another owner-surface shrink with no intended behavior change; phase0 now locks those wrapper bodies out too.
- 2026-04-22: `Resolver.pm` no longer carries local `_require_trace_pkg(...)` or `_call_preserving_err(...)` wrappers. Its live trace helper bodies now call `LinkedSpec::OwnerDispatch` inline for Trace loading and successful `$@` preservation, so this is another no-behavior-change owner-surface shrink; phase0 now locks those wrapper bodies out too.
- 2026-04-22: `ActionIR/Scanner.pm` no longer carries local `_require_pkg_cb(...)` or `_call_preserving_err(...)` wrappers. Its live owner helper bodies now call `LinkedSpec::OwnerDispatch` inline for `ScannerCore` callback lookup and successful `$@` preservation, so this is another no-behavior-change owner-surface shrink; phase0 now locks those wrapper bodies out too.
- 2026-04-22: `LinkedSpec::OwnerDispatch` now seeds the repo `perl` root into `@INC` as an absolute path at load time, so later lazy owner loads stay `chdir(...)`-safe. This closed the file-oriented parser regression where `get_parser(...)` could lose `ParserFactory` / `RuntimeContext` lazy loads after tests changed into a temp directory.
- 2026-04-22: `RuleIR.pm` no longer carries a local `_call_preserving_err(...)` wrapper. Its meaningful seams remain `_trace_should_dump(...)`, `_trace_log_output(...)`, `_trace_decision(...)`, and `_dump_value(...)`, and those helper bodies now preserve caller `$@` by spending `LinkedSpec::OwnerDispatch::call_preserving_err(...)` directly instead of bouncing through a second local wrapper layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `PluginBridge.pm` no longer carries a local `_call_preserving_err(...)` wrapper. Its meaningful seams remain `_exec_legacy_plugin(...)`, `_get_legacy_plugin(...)`, `_lookup_plugin_name(...)`, and `_dispatch_plugin_name(...)`, and those compatibility helpers now preserve caller `$@` by spending `LinkedSpec::OwnerDispatch::call_preserving_err(...)` directly instead of bouncing through a second local wrapper layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `BootstrapSpec/Core.pm` no longer carries a single-use local `_call_preserving_err(...)` wrapper. Its meaningful seams remain `_linkedre_or(...)` and `_linkedre_ored_re(...)`, and both now preserve caller `$@` by spending `LinkedSpec::OwnerDispatch::call_preserving_err(...)` directly instead of bouncing through a second local wrapper layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `ActionIR/StatementSplit/Core.pm` no longer carries a generic local `_require_pkg(...)` wrapper. Its meaningful seams remain `_require_statement_split_mode_pkg(...)` and `_require_method_expr_pkg(...)`, and both now lazy-load their owners by spending `LinkedSpec::OwnerDispatch::require_pkg(...)` directly instead of bouncing through a second local package-loader layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `RuleIR/EmitContext.pm` no longer carries a generic local `_require_pkg(...)` wrapper. Its meaningful seam is still `_actionir_owner_package(...)`, and that owner-key registry now lazy-loads ActionIR owners by spending `LinkedSpec::OwnerDispatch::require_pkg(...)` directly instead of bouncing through a second local package-loader layer; phase0 now locks that slimmer source shape.
- 2026-04-22: `Runtime.pm`, `BootstrapSpec.pm`, and `ActionIR/Scanner.pm` no longer carry dead local `_require_pkg(...)` wrappers, and `RuleIR/EmitContext.pm` no longer carries its dead local `_require_trace_pkg(...)` wrapper. The active seams were already `require_pkg_cb(...)` or the ActionIR owner registry, so this is another owner-surface shrink with no intended behavior change; phase0 now locks those wrapper bodies out.
- 2026-04-22: The dep-map-only ActionIR owners `RewritePipeline`, `Diagnostics`, `ValueExpr`, `FlowExpr`, `ArrayPipeline`, `ControlFlow`, `Contracts`, `MethodLowering`, and `DeclareMethod` no longer carry dead local `_require_pkg(...)` or `_call_preserving_err(...)` pass-through wrappers. Their active path was already `LinkedSpec::OwnerDispatch::build_dep_map(...)`, so this is another owner-surface shrink rather than a behavior change; phase0 now locks the missing wrapper bodies in those files.
- 2026-04-21: `perl/LinkedSpec.pm` no longer carries dead local `_require_pkg(...)` or `_call_preserving_err(...)` pass-through wrappers, and `perl/LinkedSpec/ParserFactory.pm` no longer carries dead local `_require_pkg(...)`, `_require_pkg_cb(...)`, or `_require_pkg_value(...)` wrappers. The active facade/parser-factory path was already using `LinkedSpec::OwnerDispatch` directly, so this is an owner-surface shrink rather than a behavior change; phase0 now locks both the missing wrapper bodies and the preserved `$@` contract on direct `OwnerDispatch` helper use for ParserFactory-owned lookups.
- 2026-04-18: `plugin/fsmgen.plg` no longer exposes `getop_plugin_list`. Repo-owned dynamic plugin-list parsing already uses `FSMGen::getop_plugin_list(...)` directly, the old helper name is gone from the shipped plugin registry, and phase0 now locks both the missing coderef in `fsmgen.plg` and the absence of bare `getop_plugin_list(...)` action calls across repo-owned plugin files.
- 2026-04-18: `plugin/fsmgen.plg` no longer exposes `add_header_n_context_clause`. Repo-owned RTL/FSM generation already uses `RTLUtils::add_header_n_context_clause(...)` directly, the old helper name is gone from the shipped plugin registry, and phase0 now locks both the missing coderef in `fsmgen.plg` and the absence of bare `add_header_n_context_clause(...)` action calls across repo-owned plugin files.
- 2026-04-18: `plugin/tcl4interconn.plg` is gone. Repo-owned QC flow now uses `QC::TclInterconn::append_interconnect_tcl(...)`, `load_fanx(...)`, and `fanx_info(...)` directly with no remaining bare `tcl4interconn(...)`, `tcl4fanx(...)`, or `get_fanxinfo(...)` action calls in shipped plugin files, and phase0 now locks both the removed wrapper file and the direct package-owner path.
- 2026-04-18: `tssio` moved out of the remaining `.plg` body. `perl/Timing/SetupHold.pm` owns `write_tssio(...)`; at that point `plugin/tssio.plg` was only a thin compatibility wrapper, and a later 2026-04-30 cleanup deleted that wrapper. The package-owner path preserves `tssio.lof`, per-path `sta_<n>.lof` sheets, banner/error strings, and internal link formatting without loading `PPlugin`.
- 2026-04-18: STAN OMAP2430C report writers moved out of the remaining `.plg` bodies. `perl/Timing/StanOmap2430cBackend.pm` now owns `write_potential_fp(...)`, `write_questionable_paths(...)`, and `write_frequency_summary(...)`; `plugin/stan_omap2430c_backend.plg` keeps `potential_fp`, `questionable_paths`, and `freqency_summary` only as thin compatibility wrappers; and the package-owner path preserves the historical LOF filenames, section markers, and visible legacy names without loading `PPlugin`.
- 2026-04-17: FSMGen dynamic plugin-list parsing moved out of the `.plg` body. `perl/FSMGen.pm` now owns `FSMGen::getop_plugin_list(...)` for `+type=plugin#args...` entries; at that step `plugin/fsmgen.plg::getop_plugin_list` delegated to that owner, and the newer 2026-04-18 note above records that the obsolete wrapper has since been deleted; the package function keeps `LinkedSpec::get_plugin(...)` as its default resolver plus the unresolved-name no-op fallback while allowing injected lookup for tests/package callers.
- 2026-04-17: QC flow Tcl/FANX helpers moved out of repo-owned plugin lookup. `perl/QC/TclInterconn.pm` now owns `append_interconnect_tcl(...)`, `load_fanx(...)`, and `fanx_info(...)`; `plugin/qcflow.plg::glc_xcel2hm` calls those package functions directly and no longer loads `LinkedSpec` or resolves `tcl4interconn`, `tcl4fanx`, or `get_fanxinfo` through `LinkedSpec::get_plugin(...)`. The newer 2026-04-18 note above records that the temporary `plugin/tcl4interconn.plg` wrapper has since been removed.
- 2026-04-17: RTL/FSM VHDL header generation moved out of plugin dispatch. `perl/RTLUtils.pm` now owns `RTLUtils::add_header_n_context_clause(...)`; `RTLUtils` entity/architecture generation and `FSMGen.pm` package code call it directly; and the newer 2026-04-18 note above records that the temporary `plugin/fsmgen.plg::add_header_n_context_clause` compatibility wrapper has since been deleted. This retires the repo-owned `LinkedSpec::run_plugin('add_header_n_context_clause', ...)` header path while preserving the preferred package-owner contract.
- 2026-04-17: README/SESSION_BOOTSTRAP was re-run against the current docs and `LinkedSpec.pm` load shape. `ARCHITECTURE_STATE.md` now shows `LinkedSpec::CompilerState` explicitly in the root owner tree under `Compiler`, and its `PluginBridge` section now records default dependency-map assembly through `LinkedSpec::OwnerDispatch::build_dep_map(...)`.
- 2026-04-17: `LinkedSpec::PluginBridge::_default_deps()` now assembles its registry-first / legacy-`PPlugin` fallback callbacks through `LinkedSpec::OwnerDispatch::build_dep_map(...)` instead of returning a hand-built local callback hash. This keeps the transition bridge's default dependency wiring on the same shared seam as the active parser and ActionIR owner surfaces.
- 2026-04-17: Dead local `_require_pkg_cb(...)` wrappers were removed from the ActionIR owners that already assemble dependency callbacks through `LinkedSpec::OwnerDispatch::build_dep_map(...)`: `ArrayPipeline`, `CanonicalEvents`, `Contracts`, `ControlFlow`, `DeclareMethod`, `Diagnostics`, `FlowExpr`, `MethodLowering`, `RewritePipeline`, `StatementSplit`, and `ValueExpr`. `ActionIR::Scanner` still has a local wrapper because it directly loads `ScannerCore`.
- 2026-04-17: `LinkedSpec::RuleIR::EmitContext` no longer does local `Package->can(...)` callback probes for ActionIR owner methods. Its owner-key registry remains local, but callback resolution now goes through `LinkedSpec::OwnerDispatch::require_pkg_cb(...)`, leaving `OwnerDispatch` itself as the only intentional `->can(...)` probe under `perl/LinkedSpec`.
- 2026-04-17: RTL memory-wrapper `get_log2` moved out of the legacy plugin registry. `perl/RTLUtils.pm` now owns `RTLUtils::ceil_log2(...)`; `plugin/generic_fake_memory_module.plg` and `plugin/wrapgen.plg` call that package owner directly for address-width sizing; and the former helper subdef is no longer a `.plg` registration or unqualified shared helper.
- 2026-04-17: STAN/report-timing backend `minmax_clockmx_cellcode` moved out of the legacy plugin registry. `perl/Timing/StanBackend.pm` now owns `Timing::StanBackend::clock_matrix_cell_code(...)`; `plugin/stan_backend.plg::clockmatrix` calls that package owner directly for clock-matrix summary cell links; and the former helper subdef is no longer a `.plg` registration or unqualified same-file helper. The package owner preserves min/max slack sorting, first-100 path sheet allocation, and internal workbook summary-link text.
- 2026-04-17: QC flow `qcflow_qclogdata` moved out of the legacy plugin registry. `perl/QC/Flow.pm` now owns `QC::Flow::prepare_qclog_data(...)`; `plugin/qcflow.plg::glc_xcel2hm` calls that package owner directly in both qclib/noctslib and CSV-only paths; and the former helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('qcflow_qclogdata')`. Remaining QC flow dynamic lookups are now external tcl/fanx callbacks (`tcl4interconn`, `tcl4fanx`, `get_fanxinfo`).
- 2026-04-17: QC flow `qcflow_links_n_qclog` moved out of the legacy plugin registry. `perl/QC/Flow.pm` now owns `QC::Flow::write_qclog_links(...)`; `plugin/qcflow.plg::glc_xcel2hm` calls that package owner directly in both qclib/noctslib and CSV-only paths; and the former helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('qcflow_links_n_qclog')`. Later QC flow qclog-data helper plus external tcl/fanx callbacks still had dynamic lookups to unwind at that point.
- 2026-04-17: QC flow `qcflow_clock_ctsinfo` moved out of the legacy plugin registry. `perl/QC/Flow.pm` now owns `QC::Flow::clock_cts_info(...)`; `plugin/qcflow.plg::glc_xcel2hm` calls that package owner directly for extracted clock CTS summaries; and the former helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('qcflow_clock_ctsinfo')`. Later QC flow qclog/link helpers still had dynamic lookups to unwind at that point.
- 2026-04-17: QC flow `qcflow_filter_handler` moved out of the legacy plugin registry. `perl/QC/Flow.pm` now owns `QC::Flow::filter_handler(...)`; `plugin/qcflow.plg::glc_xcel2hm` stores `\&QC::Flow::filter_handler` in `$qconf->{_filter_handler}`; and the former helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('qcflow_filter_handler')`. Later QC flow log/link/CTS helpers still had dynamic lookups to unwind at that point.
- 2026-04-17: QC flow budget-check callbacks moved out of the legacy plugin registry. `perl/QC/Flow.pm` now owns `QC::Flow::budget_check(...)` and `budget_check_single_or_zero_match(...)`; `plugin/qcflow.plg::glc_xcel2hm` passes those package coderefs through `HUtils::Recurse(...)`; and the former `qc_budget_check` / `qc_budget_check_01match_code` helper subdefs are no longer `.plg` registrations or resolved through `LinkedSpec::get_plugin(...)`. `get_fanxinfo` is still resolved at the visible action boundary and passed into the package owner; later QC flow qclog-data/link helpers still had dynamic lookups to unwind at that point.
- 2026-04-17: QC flow `qc_pushonce` moved out of the legacy plugin registry. `perl/QC/Flow.pm` now owns `QC::Flow::push_once(...)`; `plugin/qcflow.plg::glc_xcel2hm` passes `\&QC::Flow::push_once` into the budget-check recursion; and the former `qc_pushonce` helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('qc_pushonce')`. Later QC flow qclog-data/link helpers still had dynamic helper lookups to unwind at that point.
- 2026-04-17: STAN OMAP2430C `drive_nopath_check` and `nopath_check` moved out of the legacy plugin registry. `perl/Timing/StanOmap2430cBackend.pm` now owns `write_no_path_check(...)` and `record_no_path_check(...)`; the writer passes an explicit output filehandle through `HUtils::Recurse(...)`; and the former helper subdefs are no longer `.plg` registrations or resolved through `LinkedSpec::get_plugin(...)`. This completed the then-identified private STAN OMAP callback extraction cluster; a later 2026-04-18 slice moved the remaining visible report-writer bodies into the same package owner too.
- 2026-04-17: STAN OMAP2430C `drive_tckdelays` moved out of the legacy plugin registry. `perl/Timing/StanOmap2430cBackend.pm` now owns `write_tck_delays(...)`; `plugin/stan_omap2430c_backend.plg::dm_measures` calls that package owner directly; and the former `drive_tckdelays` helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('drive_tckdelays')`. The package writer preserves `stan_tcksegments.lof`, per-segment sheet filenames, workbook links, and `consolidated_ns` path-sheet sections.
- 2026-04-16: STAN OMAP2430C `stafrequency` moved out of the legacy plugin registry. `perl/Timing/StanOmap2430cBackend.pm` now owns `collect_sta_frequency(...)`; `plugin/stan_omap2430c_backend.plg::old_default` passes that package callback directly to `HUtils::Recurse(...)`; and the former `stafrequency` helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('stafrequency')`. The package callback preserves good-path frequency accumulation, negative-period/questionable path handling, and potential false-path collection.
...
- 2026-04-12: Setup/hold timing helper math moved out of the legacy plugin registry. `perl/Timing/SetupHold.pm` now owns `collect_dxcy(...)`, `calculate_path_delay(...)`, `delay_dici(...)`, `delay_dico(...)`, `delay_doci(...)`, `delay_doco(...)`, `tss_setup_hold_delay(...)`, and `tss_tmax_tmin_delay(...)`; `plugin/setup_hold_tmax_tmin.plg` and the then-current `plugin/tssio.plg` body called that package owner directly; and the former helper subdefs `DxCy`, `DiCi`, `DiCo`, `DoCi`, `DoCo`, `tss_setup_hold`, and `tss_tmax_tmin` are no longer `.plg` registrations. A later 2026-04-18 slice moved the full `tssio` action body into `write_tssio(...)` too. Future mixed-plugin cleanup should keep collapsing private helper subdefs into package owners, and should prefer explicit package-level dispatch when old helper signatures already show the data they need.
...
- 2026-04-11: `plugin/plugin.plg` removed. Its only remaining repo-owned consumer, `plugin/fsmgen.plg::getop_plugin_list`, first moved to `LinkedSpec::get_plugin($name) // sub {}` directly; the newer 2026-04-17 note above records that this parser now lives in `FSMGen::getop_plugin_list(...)` with that same default resolver and no-op fallback.
...
- 2026-04-11: Continued the plugin/resource modernization track across shipped `.plg` callers. The then-existing `plugin/plugin.plg`, plus `plugin/qcflow.plg`, `plugin/setup_hold_tmax_tmin.plg`, `plugin/stan_omap2430c_backend.plg`, and `plugin/tssio.plg`, moved to `LinkedSpec::get_plugin(...)` instead of direct `PPlugin->get(...)`; newer notes above record that `setup_hold_tmax_tmin.plg` calls `Timing::SetupHold` directly for private timing helpers, `plugin/tssio.plg` is now deleted after `write_tssio(...)` became package-owned, `plugin/plugin.plg` is removed, and FSMGen's former `.plg` dynamic plugin-list parser now lives in `FSMGen::getop_plugin_list(...)`. Regression coverage now scans all repo-owned `plugin/*.plg` files to keep direct `PPlugin->get(...)` constrained to compatibility bridge/adapter internals. Future plugin work should keep repo-owned plugin bodies behind `LinkedSpec::get_plugin(...)` / `run_plugin(...)` or package owners, not direct `PPlugin` access.
...
- 2026-04-11: Owner wrapper callback lookup cleanup landed. `Runtime`, `Compiler`, `BootstrapSpec`, `SpecEntry`, and `ActionIR::Scanner` now use `LinkedSpec::OwnerDispatch::require_pkg_cb(...)` instead of local `Package->can(...)` callback probes. A source-level regression locks those wrappers against reintroducing `->can(...)`; at that point `RuleIR::EmitContext` still had custom owner-key registry callback probes, and the newer 2026-04-17 note records that those now route through `OwnerDispatch` too.
...
- 2026-04-22: Continued the same Phase 1A / Backbone Item 3 owner-dispatch cleanup in `LinkedSpec::Compiler` and `LinkedSpec::SpecEntry`. Those compile-path owners no longer keep a generic local `_require_pkg_cb(...)` wrapper; their named bootstrap/spec-entry/validation and RuleIR/emit-context helper seams now spend `LinkedSpec::OwnerDispatch::require_pkg_cb(...)` directly. Future resume should treat those named helpers as the real callback-loader seams and avoid reintroducing another generic local callback-loader layer above them.
- 2026-04-22: Continued the same Phase 1A / Backbone Item 3 owner-dispatch cleanup in `LinkedSpec::Compiler`, `LinkedSpec::SpecEntry`, and `LinkedSpec::RuleIR`. Those compile-path owners no longer keep a generic local `_require_pkg(...)` wrapper; their Trace / `Data::Dumper` / `LinkedRE` helper seams now spend `LinkedSpec::OwnerDispatch::require_pkg(...)` directly. Future resume should treat the package-specific helpers there as the meaningful seams and avoid reintroducing another generic local package-loader layer above them.
- 2026-04-22: Continued the same Phase 1A / Backbone Item 3 owner-dispatch cleanup in three support owners. `LinkedSpec::BootstrapSpec::Core`, `LinkedSpec::ActionIR::ScannerCore`, and `LinkedSpec::PluginBridge` no longer keep single-use local `_require_pkg(...)` pass-through wrappers; `_require_linkedre_pkg(...)`, `_scanner_dispatchers(...)`, and `_load_legacy_plugin_runtime(...)` now spend `LinkedSpec::OwnerDispatch::require_pkg(...)` directly. Treat those named helpers as the remaining meaningful seams there, and do not reintroduce another generic local package-loader wrapper around them.
...
- 2026-05-16: PHASE2-DSL-FRONTEND.1 completed — 39 validation checks inventoried with line-level owning-code references. Active PNT frontier: PHASE2-DSL-FRONTEND.2 (close validator/parser drift gap with regression coverage). The compile pipeline is: `validate_spec_content` → `validate_dsl_syntax` → `bootstrap_parse` → `build_compiled_rule_table` → `build_dependency_regex_map` → `validate_dependency_regex_references`. Highest risk: the validator's `_looks_like_supported_rule_paragraph_member_line` may not recognize every construct the bootstrap grammar accepts, which is the primary defense against silent token loss. Five gap leaves queued (PHASE2-DSL-FRONTEND.2 through .6).
- 2026-05-16: PHASE2-DSL-FRONTEND.2 completed. Added 6 regression subtests (20 assertions) to `t/phase0_regression.t`. Full suite: Files=1, Tests=995, PASS. Next PNT frontier: PHASE2-DSL-FRONTEND.6 (verify full fluent-continuation surface recognition in `_looks_like_supported_rule_paragraph_member_line`), then PHASE2-DSL-FRONTEND.4 (inside-block rule-start detection), PHASE2-DSL-FRONTEND.5 (extra-colon rejection), PHASE2-DSL-FRONTEND.3 (strict_syntax option).

- 2026-05-17: PHASE7-SELF-HOSTED-SPEC.4 completed. Fixed AND++LX hang (replaced LX→E in spec_file), added 3 regression subtests (15 assertions). Full suite: 1010 PASS. Active PNT frontier: PHASE7-SELF-HOSTED-SPEC.5 (extension-surface policy). Known bootstrapping gaps: (1) AND++LX infinite loop (fixed with E), (2) no comment/blank-line skip in body_element (worked around in tests by stripping leading comments).

- 2026-05-17: PHASE7-SELF-HOSTED-SPEC.5 completed. Defined extension-surface policy: spec.spec is the required change surface for .spec language evolution. Bootstrap grammar changes are exception-only. Policy documented in spec.spec header and DEVELOPMENT_NOTES.md. PHASE7-SELF-HOSTED-SPEC tree COMPLETE (5 leaves). Full suite: 1010 PASS.
