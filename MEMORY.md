...
- 2026-03-23: Added the next complementary Phase 4 helpers for future resume again. `entry_start_pos()` and `entry_end_pos()` now expose the current immediate match left and right boundaries directly, so the immediate-match surface is now a clean trio too: `entry_text()`, `entry_start_pos()`, and `entry_end_pos()`.
