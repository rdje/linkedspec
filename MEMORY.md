...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.6.2.1` (`SPEC-FORMAT-TERSE.6.2.1 - remove declare from shipped specs`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — shipped specs no longer use active `declare(...)` / `.declare(...)`; focused compile, phase0 **1018 green**, and Rust corpus oracle **66 fixtures** pass. `.6.2.2` next migrates old helper spellings (`assign`, `push_value`, `array_copy`/`hash_copy`, `concat`); `.6.2.3` typed-wrapper/direct-shape cleanup; `.6.3` docs/corpus/examples; `.6.4` declaration compatibility policy. User backlog directive captured as `.7.1`: audit supported type methods, including string `substr()` as a receiver method.
- next_action: Pick `SPEC-FORMAT-TERSE.6.2.2`: migrate shipped-spec old helper spellings to canonical terse helper/operator forms, keeping behavior stable and leaving wrappers/constructors to `.6.2.3`; verify focused compile, phase0, Rust corpus oracle, and no-drift scans.
