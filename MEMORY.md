...
- latest_completed_leaf: `JULIA-BACKEND-PARITY.2.3` — Julia `validate_spec(...)` validates parsed source ASTs for
  top-rule presence, duplicate labels/functions, function registry shape, edge consistency, references, regex
  structure, and strict unused-rule behavior.
- prior_leaf: `JULIA-BACKEND-PARITY.2.2` — Julia `parse_spec(...)` parses core `.spec` rule paragraphs into source
  AST types, with focused tests over all checked-in specs and rule-only corpus specs.
...
- latest_commit: this resume block is prepared for commit
  `JULIA-BACKEND-PARITY.2.3 - add Julia frontend validation`; previous committed HEAD is
  `cebe92f0 JULIA-BACKEND-PARITY.2.2 - add Julia source spec parser`.
- push_policy: check `git status -sb` for the live ahead count; do not push mid-PNT unless explicitly instructed
  or the documented 300-commit threshold policy is deliberately invoked.
- active_work_unit: `JULIA-BACKEND-PARITY`; current frontier after the current commit is `.2.4`.
- next_action: resume PNT at `JULIA-BACKEND-PARITY.2.4` to consume top-level `fn` shells through the
  spec-defined `function_definition` / `function_definition_error` owner before ActionIR and runtime work.
...
- blockers: none. in_flight_uncommitted: none expected after the `JULIA-BACKEND-PARITY.2.3` commit; do not pivot unless the repo
  is handoff-ready.
