...
- 2026-04-17: STAN OMAP2430C `drive_nopath_check` and `nopath_check` moved out of the legacy plugin registry. `perl/Timing/StanOmap2430cBackend.pm` now owns `write_no_path_check(...)` and `record_no_path_check(...)`; the writer passes an explicit output filehandle through `HUtils::Recurse(...)`; and the former helper subdefs are no longer `.plg` registrations or resolved through `LinkedSpec::get_plugin(...)`. This completes the currently identified private STAN OMAP helper extraction cluster; remaining subdefs in `plugin/stan_omap2430c_backend.plg` are visible legacy actions or still-disabled report actions.
