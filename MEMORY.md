...
- latest_completed_leaf: `FUTURE-PARITY-BACKLOG.10.3.2.0` — corrected neutral static rule authority and independent
  rule-local cross-gate are clean at `40d35201` without backend behavior, rollout, or admission.
- latest_commit: `40d35201` — `FUTURE-PARITY-BACKLOG.10.3.2.0 - correct semantic static oracle`
  (ahead: 265; push at threshold 300).
- active_work_unit: `FUTURE-PARITY-BACKLOG.10.3.2.1` — private Perl static records/relations deep-equal graph,
  privacy full/limited, failed, and runtime-static neutral targets; complete focused/canonical signoff is green.
- next_action: finish durable closeout, clean generated artifacts, commit/clear/verify clean, then activate calls/
  shapes/staged/generated projection `.10.3.3` task-tree-first from the new commit.
...
- current_perl_authority_map: decoded source/map plus descriptor order/topology and runtime failure now project
  private static v1 facts; ActionIR owns later call/binding spans, staged registry owns jobs/results, generated v2
  owns plan metadata, and invocation-local typed observations remain absent. Raw bytes decode before `Get`.
...
- current_perl_foundation: `semantic_index` compiles in-memory decoded/strict-UTF-8 source once without execution;
  retains opaque source/map/outcome plus clone-safe static records; never reads a path. Public query is absent.
...
- current_signoff: `.10.3.2.1` static 5, foundation 5, semantic 6/20/53, rule-local 36/18/8+60, capability
  80/0/0, selector 59/27/0, repeated action 8/10/8+0/54, primary 66x2, KM 653/4,812, mdBook/governance,
  Phase 0 1,031/1,031 in 611 seconds, and canonical exit 0 pass. Rollout/admission stay 1/9 and 0/6.
...
- blockers: none. in_flight_uncommitted: `.10.3.2.1` projector/test/CI/docs/KM are canonical-green but uncommitted;
  final durable closeout/cleanup, commit/brief clearing, then `.10.3.3` task-tree-first activation remain.
