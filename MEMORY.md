...
- 2026-04-11: GenericFilter plugin extraction landed. `Plugin::GenericFilter` now owns `group_by`, `group_by_port`, `group_by_ioclock`, `group_byRE`, plus `dispatch($action, ...)`; `plugin/genericfilter.plg` is reduced to compatibility wrappers; and `HUtils::GenericFilter(...)` / `TableSort::GenericFilter(...)` call the package owner directly instead of `LinkedSpec::run_plugin(...)` / `get_plugin(...)` over constructed `genericfilter_*` names. Future plugin work should continue moving executable `.plg` bodies into normal `Plugin::*` owners and leave `.plg` files only as temporary legacy registration shims.
