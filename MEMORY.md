...
- 2026-04-09: Deepened `docs/fsmgen-book/src/model/fsm-source-model.md` into a real FSMGen source-shape reference. The FSMGen mdBook now documents the active paragraph families (`?define`, `?fsm`, `?top`), FSM body entries, decision-tree node kinds, hierarchy nodes, and plugin-driven mini-languages directly from `perl/FSMGen.pm` plus `plugin/fsmgen.plg`. Future resume should keep growing the FSMGen book by grounding each chapter in the actual active walkers/parsers rather than leaving scaffold prose in place.
- 2026-04-09: Added a dedicated FSMGen mdBook under `docs/fsmgen-book/` with a stable chapter hierarchy and seeded overview/usage/model/reference/development pages. Future resume should expand FSMGen documentation there first instead of scattering new FSMGen long-form docs across ad hoc top-level markdown files.
...
- 2026-04-02: Continued the real Phase 5 diagnostics line with synthetic generated-handler attribution on compile-time rule failures. `Compiler.pm` now preserves `handler_source_label` on attributed `compiler_pipeline:spec_descr`, `compiler_pipeline:build_final_descr`, and `compiler_pipeline:validate_gdata_references` failures whenever the failing rule label is already known, and the parser boundary now also preserves that same label whenever at least the selected top-rule label is known even if no exact handler variant is available yet. Future resume should treat label-only forms like `LinkedSpec::generated_handler:Top` as part of the stable structured-diagnostics contract now instead of assuming `handler_source_label` only appears on runtime-handler variant-known failures.
...
- 2026-04-01: Continued the same Phase 5 diagnostics line for future resume one stage later. `Compiler.pm` now also preserves `runtime_ctx->{last_error}{rule_label}` on default-`spec_gdata(...)` `compiler_pipeline:build_final_descr` failures when the active rule’s combined regex dependencies are being built and `_ored_re(...)` dies. If we keep pushing runtime diagnostics next, treat compile-time rule attribution as spanning both `spec_descr` and default `build_final_descr` failure paths whenever the active rule is still known.
- 2026-04-02: Continued the Phase 5 diagnostics line for future resume. `Compiler.pm` now preserves `runtime_ctx->{last_error}{rule_label}` and a more specific owning-rule `detail` string when `compiler_pipeline:validate_gdata_references` rejects the generated descriptor by returning false for one compiled rule, using new optional failure metadata exposed by `LinkedSpec::Validation::validate_gdata_references(...)`. If we keep pushing runtime diagnostics next, treat compile-time rule attribution as spanning `spec_descr`, default `build_final_descr`, and false-return `validate_gdata_references` when the owning/generated rule is still known.
...
- 2026-04-05: Continued the same Phase 5 diagnostics-attribution line into the later generic compiler seams too. `Compiler.pm` now falls back to `LinkedSpec::generated_handler:<top_rule>` for `handler_source_label` on unattributed `spec_descr`, `build_final_descr`, and `validate_gdata_references` failures when the caller already selected a top rule but no rule ownership has been pinned yet. Regression coverage now locks that directly plus through public `LinkedSpec::Get(...)` continuity, so future resume should treat late generic compiler-failure handler attribution as covered continuity now instead of assuming only rule-attributed late compiler stages preserve handler identity.
- 2026-04-06: Locked the same late generic compiler-failure attribution story through the file-oriented path too. `get_parser(...)` now has focused regression coverage proving that after spec resolution/load succeeds, a generic late `build_final_descr` failure still preserves resolved `spec_path`, selected `top_rule`, and `LinkedSpec::generated_handler:<top_rule>` in shared `runtime_ctx->{last_error}`. Future resume should treat that file-oriented late-compiler continuity as covered now instead of re-probing it manually.
