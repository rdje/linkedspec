...
- latest_completed_leaf: `STAGED-LINKED-PARSING.5.3.2` (`STAGED-LINKED-PARSING.5.3.2 - retire Rust raw function parser`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `STAGED-LINKED-PARSING` — ADRs `0012`-`0017` define staged doctrine, import/composition, `parse_job(...)` annotations, deterministic registry/dispatch queues, implementation-language neutrality, and the target function-definition staged AST shape. `.5.3.1` added `specs/user_function_definition.spec` as the executable user-function definition parser for Perl; `.5.3.2` makes Rust consume the same returned `function_definition` / `function_definition_error` AST contract and removes the core Rust raw `fn` parser bridge. Current parsers do not yet accept/execute future import/parse-job/dispatch directives.
- next_action: Pick `STAGED-LINKED-PARSING.5.4`: add the minimal neutral parse-job marker/sidecar prototype now that active Perl/Rust definition-shell bridges converge on the spec-owned AST contract. If PNT returns to parity first, `RUST-PARITY.7.3` remains the active Rust frontier.
