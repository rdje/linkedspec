...
- latest_completed_leaf: `RUST-PARITY.7.3.4.1` (`RUST-PARITY.7.3.4.1 - fix header-rest action-edge parsing`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `RUST-PARITY` — `.7.3.4.1` fixed Rust parser/compiler handling for header-rest action edges on regex-less top rules: unrecognized header tokens such as `->`, `->Child.push`, and `I.return(...)` are restored to body rest instead of swallowed as invalid mode suffixes, and Rust now follows `specs/spec.spec` optional spacing after `->`/`=>` (`[ \t]*`). `lib_reader` top dispatch now compiles and representative probes no longer collapse to `[[]]`, but capture fields are still null; `.7.3.4.4` owns runtime capture propagation for dependency-resolved child regex matches. `portmap` still needs runtime boolean/list-context parity (`or`, `array(flat_array(...))`) plus fluent recursive aggregation for concat; `ebnf` still needs fluent child/target aggregation. `.7.3.5` later triages `BNF`/`DT`/`ifelse`/`operators_try`/`spec.spec`; `.7.3.6` audits RTL/plugin/legacy candidates. Staged linked parsing first prototype is complete; broader public staged surfaces remain future work.
- next_action: Pick `RUST-PARITY.7.3.4.4`: fix runtime capture propagation for dependency-resolved `lib_reader` child regex matches so `entry_group(0/1)` in `sattribute`/`cattribute` sees real captured values instead of nulls, then re-run the representative `lib_reader` sattribute/cattribute oracle probes.
