...
- latest_completed_leaf: `PERL-ACTIONIR-AST-MIGRATION.5` (`PERL-ACTIONIR-AST-MIGRATION.5 - split fallback retirement and function handoff`; hash pending commit). Previous HEAD before this slice: `32edebe`. Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `PERL-ACTIONIR-AST-MIGRATION` — `.5.1` is next: audit the remaining supported-surface text fallback boundary before code.
- next_action: Start `PERL-ACTIONIR-AST-MIGRATION.5.1`: classify remaining fallback paths as explicit compatibility debt, supported-surface leakage to retire, or user-function handoff dependency using TOOLBOX probes and code reads before code. User-defined functions remain queued and must use the AST path; final `fn <name>(...) { ... }` grammar belongs in `specs/spec.spec`, and any bootstrap-parser `fn` support is temporary migration debt to remove after text-to-AST carries the surface. Keep `SPEC-FORMAT-TERSE.3.2.2`/`.3.2.3` and `SPEC-FORMAT-TERSE.3.3` queued behind the user-directed AST/function run; do not start Julia/Dart backend code without a dedicated owning backend leaf/tree; do not start Lua backend work without an accepted decision record.
...
- blockers: NONE PNT-eligible-blocking. in_flight_uncommitted: `.5` split docs only until commit; known unrelated local paths `rgx` and `.claude/projects/` must remain unstaged.
