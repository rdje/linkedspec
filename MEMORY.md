...
- latest_commit: `b41e047e` — `FUTURE-PARITY-BACKLOG.10.9.7.2 - close MCP implementation parent` (103/300;
  no push); commit is brief-cleared/residue-free/clean.
- latest_completed_leaf: `FUTURE-PARITY-BACKLOG.10.9.7.2`; `.10.9.7` and `.10.9` are committed closed.
- active_leaf: `FUTURE-PARITY-BACKLOG.10.10` — public semantic/MCP no-drift task-tree-first from clean `b41e047e`.
- active_scope: inventory and synchronize every public/native/backend/API/mdBook/roadmap/status/ADR/Knowledge
  projection; add rich graph/resolution/provenance/diagnostic/explain/privacy/pagination/runtime/MCP examples;
  implement omission-sensitive public governance; advance only public no-drift; close semantic parent `.10`.
- active_progress: public contract covers 28 current surfaces / nine examples / two recurring drivers / separate
  companion-book boundary; checker passes 6/20/128 at rollout 9/9 + admission 6/6. Both direct recurrences and
  canonical CI with both opt-ins pass; public docs/ADRs/cards/backend READMEs/task/roadmaps/live/book are synced.
- active_exclusions: no production parser/compiler/runtime/server/contract/binding/semantic/CLI/aggregator/legacy/
  package/SDK/service/network behavior, unrelated rollout, or push in `.10.10`.
- next_after_clean_commit: close semantic parent `.10`, then select the next roadmap-aligned pending activity;
  no push before 300.
...
- push_policy: hard lock at 300 new local commits; counter 103/300 after `b41e047e`; no push.
...
- canonical_state: `.10.10` canonical with semantic+MCP opt-ins passes all 7 doctrines, containment/moved-root,
  CLI 66x2, RAM 54%, Phase 0 1,031/1,031; optional semantic includes Rust 78.50s / Julia 27.5s, optional MCP
  includes Rust 15.33s and all six admissions, ledger 141, primary 30/30. Book 79 files/13,844 KiB; KM 758/6,147.
- blockers: none; remove exact generated output, rerun final lightweight gates, commit/clear/clean, then select the
  next roadmap-aligned pending activity.
