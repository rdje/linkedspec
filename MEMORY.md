...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.6.1` (`SPEC-FORMAT-TERSE.6.1 - own declare retirement migration`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — `.6.1` owns the directive that `declare(...)` shall not be used in spec files. Inventory found 70 active hits across 13 shipped specs. This is terse-format migration work, not a `RUST-PARITY` runtime `declare` expansion. `.6.2` migrates shipped specs first; `.6.3` sweeps public docs/corpus/tests; `.6.4` decides any remaining compatibility support after the live spec surface is clean. `RUST-PARITY.7.3.4.4` runtime capture propagation remains pending behind this active terse migration.
- next_action: Pick `SPEC-FORMAT-TERSE.6.2`: migrate shipped specs away from `declare(...)` using auto-existing variables, assignments, direct shape literals, and type-implying terse positions; verify `rg -n '(^|[.{;[:space:]])declare\(' specs` is clean, then run focused compile/oracle/phase0 checks and split exact blockers if needed.
