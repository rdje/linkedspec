...
- next_action: **`.2.2.2.1`** (pre-impl recon DONE — execute mechanically from the node). Rewrite `return_array(semantic_annotation, X)` → `return(array("semantic_annotation", X))` in the **17 B1-array subtests only** (L16582/16636/16668/16709/16750/16794/16838/16879/16920/16963/17208/17289/17373/17419/20105/21720/21784). **SCOPE HAZARD:** `return_array` = 74× (34 failing / 40 PASSING, byte-identical strings) → **line-scope, NOT global replace**; EXCLUDE the cluster-C `emit_context` site ~L12440 (→ `.2.3`). 33 spec-body (4 forms) + 1 subst-arg (L16601, drop `Top`). Spec-body subtests pin no literal (only fluent==block+fallback==0+ready) → apply identically to fluent+block; `return(array(` needs +1 close paren. Verified rewrite gives fallback=0/ready. Then `.2.2.2.2` (4 accumulator, re-dump + judgment), `.2.3` (C `emit_context` ×20), `.2.4` (F=3+G=2), `.5` green-phase0 (+ corpus tail), `.6` book `:AND`.
