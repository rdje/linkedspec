...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.6.3` (`SPEC-FORMAT-TERSE.6.3 - sweep docs and corpus declare examples`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — docs/corpus terse-surface sweep is closed. Current-facing mdBook examples and root checked-in corpus specs now use terse initialization/mutation plus `push`/`copy`/`cat`; root corpus scans/probes pass; generated Rust oracle inputs use canonical helpers where old spellings were incidental; residual old spellings are compatibility/reference-only; Knowledge Map covers the `merge_hash(copy(hash(base)), overlay)` boundary. Oracle regeneration remains **73 fixtures** with no expected-output drift, Rust `corpus_oracle` passes, and mdBook builds.
- next_action: Pick `SPEC-FORMAT-TERSE.6.4`: decide post-migration compatibility support for declaration helpers now that live spec-file/docs/corpus teaching use is gone.
