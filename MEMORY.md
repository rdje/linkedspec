...
- 2026-04-17: QC flow Tcl/FANX helpers moved out of repo-owned plugin lookup. `perl/QC/TclInterconn.pm` now owns `append_interconnect_tcl(...)`, `load_fanx(...)`, and `fanx_info(...)`; `plugin/qcflow.plg::glc_xcel2hm` calls those package functions directly and no longer loads `LinkedSpec` or resolves `tcl4interconn`, `tcl4fanx`, or `get_fanxinfo` through `LinkedSpec::get_plugin(...)`. `plugin/tcl4interconn.plg` remains a thin compatibility wrapper for the three legacy names.
