...
- latest_completed_leaf: `PERL-ACTIONIR-AST-MIGRATION.4.3` (`PERL-ACTIONIR-AST-MIGRATION.4.3 - lower block values from AST`; hash reported after commit). Previous HEAD before this slice: `eb2d295`. Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `PERL-ACTIONIR-AST-MIGRATION` — `.4.3` moved expression-valued block side effects, block-local return payloads, and final block expressions onto AST block/statement fields before legacy fallback.
- next_action: Execute `PERL-ACTIONIR-AST-MIGRATION.4.4`: lower structured control-flow statement forms from AST nodes. User-defined functions remain queued and must use the AST path; final `fn <name>(...) { ... }` grammar belongs in `specs/spec.spec`, and any bootstrap-parser `fn` support is temporary migration debt to remove after text-to-AST carries the surface. Keep `SPEC-FORMAT-TERSE.3.2.2`/`.3.2.3` and `SPEC-FORMAT-TERSE.3.3` queued behind the user-directed AST/function run; do not start Julia/Dart backend code without a dedicated owning backend leaf/tree; do not start Lua backend work without an accepted decision record.
