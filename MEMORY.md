...
- 2026-04-17: FSMGen dynamic plugin-list parsing moved out of the `.plg` body. `perl/FSMGen.pm` now owns `FSMGen::getop_plugin_list(...)` for `+type=plugin#args...` entries; `plugin/fsmgen.plg::getop_plugin_list` delegates to that owner; and the package function keeps `LinkedSpec::get_plugin(...)` as its default resolver plus the unresolved-name no-op fallback while allowing injected lookup for tests/package callers.
