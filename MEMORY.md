...
- 2026-04-11: README/SESSION_BOOTSTRAP architecture refresh completed. Current reading: `LinkedSpec.pm` is still a thin facade, active compiler/facade naming is `build_compiled_rule_table(...)` plus compiled dependency-regex state, and remaining `spec_descr` / `gdata` names are bootstrap-local internals in `LinkedSpec::BootstrapSpec` / `BootstrapSpec::Core`. Future cleanup can rename that bootstrap island, but it should not be confused with the already-migrated active compiler descriptor model.
...
- 2026-04-09: The active derived-regex descriptor path is now fully on `dependency_regex_map` / `dependency_refs` / `DEPENDENCY_REFS`. `Compiler.pm`, `CompilerState.pm`, `SpecEntry.pm`, `Validation.pm`, `RuleIR::EmitContext.pm`, the static parser fixtures, and the active regression locks now all use those names on the live compiler/runtime/public path. Treat `gdata` as historical/bootstrap vocabulary only unless the task is explicitly about older notes or bootstrap internals.
...
- 2026-04-08: Renamed the active low-level compiler seam from `spec_descr(...)` to `build_compiled_rule_table(...)` with no compatibility wrapper preserved under the old name. `Compiler.pm`, `LinkedSpec.pm`, the active regression locks, and the current docs/roadmaps now all treat compiled rule-table construction as the real concept and `compiler_pipeline:build_compiled_rule_table` as the real diagnostics stage. Future resume should treat `spec_descr` as historical vocabulary only unless the discussion is explicitly about bootstrap internals or older notes.
