...
- latest_completed_leaf: `STAGED-LINKED-PARSING.4` (`STAGED-LINKED-PARSING.4 - specify staged parser registry dispatch`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `STAGED-LINKED-PARSING` — ADRs `0012`-`0015` now define staged doctrine, import/composition, `parse_job(...)` annotations, and deterministic registry/dispatch queues. Current parsers do not yet accept/execute the future directives/helpers/queue.
- next_action: Pick `STAGED-LINKED-PARSING.5` when this architecture track resumes: implement the first narrow staged-parsing prototype. If PNT returns to parity first, `RUST-PARITY.7.3` remains the active Rust frontier.
