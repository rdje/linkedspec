...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.6.4` (`SPEC-FORMAT-TERSE.6.4 - lock declare compatibility policy`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — declaration-retirement lane `.6` is closed. `declare(...)` and declaration aliases remain accepted legacy compatibility for existing specs, but new shipped specs/current examples/root corpus examples use auto-existing variables and terse assignment/mutation forms. ADR `0018`, mdBook, task tree, roadmap, live docs, and Knowledge Map record the policy. Existing declare/no-declare oracle fixtures remain compatibility locks; no implementation support was removed in `.6.4`.
- next_action: Pick `SPEC-FORMAT-TERSE.7.1`: audit supported types and helper families before adding/backfilling methods such as string `substr()`.
