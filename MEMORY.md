...
- latest_completed_leaf: `PERL-ACTIONIR-AST-MIGRATION.4.4.4` (`PERL-ACTIONIR-AST-MIGRATION.4.4.4 - lower while controls from AST`; hash pending commit). Previous HEAD before this slice: `bcb3542`. Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `PERL-ACTIONIR-AST-MIGRATION` — `.5` is next: retire supported-surface text fallback and unblock user-defined functions on AST; split before code if the leaf is too broad.
- next_action: Start `PERL-ACTIONIR-AST-MIGRATION.5` by owning/splitting the fallback-retirement and function-call path before code. User-defined functions remain queued and must use the AST path; final `fn <name>(...) { ... }` grammar belongs in `specs/spec.spec`, and any bootstrap-parser `fn` support is temporary migration debt to remove after text-to-AST carries the surface. Keep `SPEC-FORMAT-TERSE.3.2.2`/`.3.2.3` and `SPEC-FORMAT-TERSE.3.3` queued behind the user-directed AST/function run; do not start Julia/Dart backend code without a dedicated owning backend leaf/tree; do not start Lua backend work without an accepted decision record.
...
- blockers: NONE PNT-eligible-blocking. in_flight_uncommitted: `.4.4.4` while-control AST lowering code/tests/docs/KM until commit; known unrelated local paths `rgx` and `.claude/projects/` must remain unstaged.
