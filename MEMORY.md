...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.7.2` (`SPEC-FORMAT-TERSE.7.2 - verify string method surface`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — declaration-retirement lane `.6` is closed; type-method audit `.7.1` is done; string/scalar method verification `.7.2` is done with no code change. String receivers already include `trim`, `lowercase`, `uppercase`, `replace_substr`, `rm_prefix`, `rm_suffix`, `substr`, `concat`/`cat`, `coalesce_nonempty`, `split` bridge, and terminal length/predicate helpers. Regex `substr(:target, pattern, replacement, flags)` stays statement mutation.
- next_action: Pick `SPEC-FORMAT-TERSE.7.3`: audit/backfill useful array/list, hash, and number receiver methods from `.7.1`, keeping mutation or ambiguous helpers explicit.
