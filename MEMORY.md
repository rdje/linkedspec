...
- latest_completed_leaf: `SPEC-FORMAT-TERSE.6.2.3.2` (`SPEC-FORMAT-TERSE.6.2.3.2 - retire scalar and assign spec helpers`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — authored/current `.spec` files no longer use or support `scalar(...)` scalar-slot reads or `assign(...)` assignment aliases. Scalar slots use `:name`, assignment uses `LHS = RHS` or `set(...)`, and initialized bare identifiers remember scalar/array/hash kind for later bare reads/copies. mdBook, KM, phase0, and focused Rust checks are synced.
- next_action: Pick `SPEC-FORMAT-TERSE.6.2.4`: final shipped-spec terse-surface verification and no-drift inventory before broader doc/corpus sweeps.
