...
- latest_commit: `ba79310` before `.2.2.1`; after commit, run `git log -1 --oneline` for `SPEC-FORMAT-TERSE.2.2.1 — split control-flow keyword surface`. Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `SPEC-FORMAT-TERSE` — **`.2.2.1` control-flow keyword split DONE; frontier `.2.2.2` pending ownership**. User in a **PNT loop** (2026-06-23).
- next_action: After committing `.2.2.1`, own `SPEC-FORMAT-TERSE.2.2.2` before any attached-block `if/elseif/else` code changes.
...
- blockers: NONE PNT-eligible-blocking. in_flight_uncommitted: `.2.2.1` docs/KM/book split until commit; known unrelated local paths `rgx` and `.claude/projects/` must remain unstaged.
