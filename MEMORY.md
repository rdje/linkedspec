...
- 2026-04-11: Continued the plugin/resource modernization track across shipped `.plg` callers. `plugin/plugin.plg`, `plugin/qcflow.plg`, `plugin/setup_hold_tmax_tmin.plg`, `plugin/stan_omap2430c_backend.plg`, and `plugin/tssio.plg` now use `LinkedSpec::get_plugin(...)` instead of direct `PPlugin->get(...)`; regression coverage now scans all repo-owned `plugin/*.plg` files to keep direct `PPlugin->get(...)` constrained to compatibility bridge/adapter internals. Future plugin work should keep repo-owned plugin bodies behind `LinkedSpec::get_plugin(...)` / `run_plugin(...)` or package owners, not direct `PPlugin` access.
