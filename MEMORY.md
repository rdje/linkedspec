...
- latest_completed_leaf: `TRACE-OBSERVABILITY.2` (`TRACE-OBSERVABILITY.2 - add trace CLI control`).
- latest_commit: `9085a026` (`TRACE-OBSERVABILITY.1 - audit trace coverage gaps`). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `TRACE-OBSERVABILITY`; `.2` CLI/docs control is done: `bin/linkedspec` exposes `--trace`, `--trace-file`, `--trace-mode`, `--trace-reset`, and `--trace-emoji` over the existing Perl trace API and mdBook/TOOLBOX document it. Coverage gaps remain: generated handler branch/control-flow decisions, most ActionIR owner branches, and Rust trace parity. `TOP-RULE-AS-NORMAL` is closed; `SPEC-FORMAT-TERSE` declaration-retirement `.6` and type-method `.7` lanes are closed, with no pending terse leaf.
- next_action: Pick `TRACE-OBSERVABILITY.3` — extend Perl reference trace coverage before backend parity.
...
- blockers: NONE PNT-eligible-blocking. in_flight_uncommitted: `TRACE-OBSERVABILITY.2` CLI/docs slice being committed; known unrelated local paths `rgx` and `.claude/projects/` must remain unstaged.
