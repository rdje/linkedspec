...
- latest_completed_leaf: `STAGED-LINKED-PARSING.2` (`STAGED-LINKED-PARSING.2 - specify spec import composition contract`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `STAGED-LINKED-PARSING` — ADR `0012` defines staged linked parsing; ADR `0013` defines future spec import/composition: file-scope `import "path.spec" as alias` for qualified reuse and `include "path.spec"` for structured unqualified composition. Current parsers do not yet accept those directives.
- next_action: Pick `STAGED-LINKED-PARSING.3` when this architecture track resumes: design staged parse-job annotations and AST payload metadata. If PNT returns to parity first, `RUST-PARITY.7.3` remains the active Rust frontier.
