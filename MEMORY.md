...
- 2026-04-11: `plugin/lighttpd.plg` removed after moving the remaining real `lighttpd` action into `Plugin::HTTP::run_lighttpd_for_conf(...)`. The package owner now renders `Global->lighttpd_conf`, validates the port, closes the generated temp config before launch, and uses list-form `system('lighttpd', '-f', $filename)` instead of shell-string execution. Helper/action-only `.plg` shadows should keep being deleted once no repo-owned caller needs the registration name.
- 2026-04-11: `plugin/http.plg` removed after moving the remaining real `http` file-link action into `Plugin::HTTP::print_file_links_for_conf(...)`. `Plugin::HTTP` now owns the historical CGI config lookup, host/port setup, existing-file filter, and signed URL printing directly. At that moment `plugin/lighttpd.plg` still carried the real legacy `lighttpd` action; the newer note above records that it is now gone too.
...
- 2026-04-11: Earlier HTTP package-backed helper registrations were retired instead of preserved as permanent plugin names. Repo-owned `.plg` callers in the then-existing `http.plg`, plus `rtl.plg`, `stan_backend.plg`, and `tree.plg`, were moved to `Plugin::HTTP` for `httplink`, `set_http_hostport`, and `set_http_localhost`; the newer note above records that `http.plg` itself is now gone after moving the remaining `http` action into `Plugin::HTTP`.
