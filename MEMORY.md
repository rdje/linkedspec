...
- activation_commit: `e232609b` — clean four-backend callable public-closeout HEAD from which Lua audit `.11.8.0`
  activated task-tree-first (120/300; no push); Git derives the landing identity.
- latest_completed_leaf: `.11.8.0` intends `FUTURE-PARITY-BACKLOG.11.8.0 - freeze Lua callable plan`; its audit
  is behavior-free and dependency-complete.
- active_leaf: `.11.8.0` — signoff-complete; clean-boundary activation of `.11.8.1` follows this landing.
- active_scope: land the frozen dependency-complete `.11.8.1-.4` Lua callable plan, then activate inert typed
  construction `.11.8.1`; no behavior is authorized before that task-tree-first boundary.
- active_progress: identical PUC Lua/LuaJIT probes and all signoff are complete. Neutral 7/11/9/7/4/8+20, Lua
  177/177 per ABI, CLI 66x2, corpus 105/105, KM 776/6,291, mdBook 79/14,008 KiB, and canonical Phase 0
  1,031/1,031 in 653s plus the callable matrix pass.
- active_exclusions: behavior/fixture/capability-row/MCP changes, lexical capture, broad raw fallback, second codec/
  executor, root README, history rewriting, push, and task pivot.
- in_flight_uncommitted: none after this intended landing; clear the brief and prove clean before `.11.8.1`.
- next_after_clean_commit: implement inert Lua callable construction under `.11.8.1`; no push.
...
- current_future_direction: callable governance and Lua audit `.11.8.0` are complete; Lua explicit parity continues
  through `.11.8.1-.4`; exclusion freshness is queued `.24.0-.2`; ADR `0064` remains unscheduled and ADR `0056`
  stays `.14.1-.8`.
...
- push_policy: hard lock at 300 new local commits; counter 121/300 after intended `.11.8.0` landing; no push.
...
- canonical_state: `.11.8.0` authorized CI is green through semantic/MCP, containment/moved-root, CLI 66x2,
  RAM 61%, Phase 0 1,031/1,031 in 653s, neutral+20, Perl 10, Rust 18, Dart 21, Julia 125+118+239, and explicit pass.
- blockers: none; land `.11.8.0`, clear/prove clean, then activate `.11.8.1`; no push.
