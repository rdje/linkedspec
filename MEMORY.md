...
- 2026-04-15: STAN OMAP2430C `portiming` moved out of the legacy plugin registry. `perl/Timing/StanOmap2430cBackend.pm` now owns `filter_port_timing_paths(...)`; `plugin/stan_omap2430c_backend.plg::old_default` passes that package callback directly to `HUtils::WRecurse(...)`; and the former `portiming` helper subdef is no longer a `.plg` registration or resolved through `LinkedSpec::get_plugin('portiming')`. The callback lazy-loads `TableGrep` so requiring the timing owner stays clear of eager plugin/runtime loading. Future STAN OMAP cleanup can continue moving the remaining private traversal callbacks (`stafrequency`, `drive_tckdelays`, `nopath_check`) to the same owner when their visible-action boundaries are clear.
- 2026-04-14: Hosted GitHub Actions CI is intentionally paused for LinkedSpec to avoid consuming account Actions minutes. `.github/workflows/ci.yml` keeps only `workflow_dispatch` and a disabled `if: ${{ false }}` job guard; there are no `push` or `pull_request` triggers. Local validation remains `bash tools/run_ci_local.sh`, and re-enabling hosted CI later requires restoring the automatic triggers plus removing the disabled guard.
- 2026-04-13: STAN OMAP2430C frequency-detail helpers moved out of the legacy plugin registry. `perl/Timing/StanOmap2430cBackend.pm` now owns `record_frequency_detail(...)`, `write_frequency_detail_paths(...)`, `write_frequency_detail(...)`, and `frequency_detail_filename(...)`; `plugin/stan_omap2430c_backend.plg::old_default` calls those package coderefs directly; and the former `freqency_detailed`, `freqency_detailed_paths`, `drive_freqency_detailed`, and `get_freqency_detailed_fname` helper subdefs are no longer `.plg` registrations. Future STAN OMAP cleanup can continue moving the remaining private traversal callbacks (`stafrequency`, `drive_tckdelays`, `nopath_check`) to the same owner when their visible-action boundaries are clear.
- 2026-04-13: STAN/report-timing backend setup moved out of the legacy plugin registry. `perl/Timing/StanBackend.pm` now owns `Timing::StanBackend::start(...)`, preserving the historical launch banner, `HTTP::FileAccess::set_hostport(...)`, `PathSearch->go('stan_backend_table2ss')`, and `Table2SS::UConf(...)` flow; `plugin/stan_backend.plg`, `plugin/skew.plg`, and `plugin/duty_cycle_degradation.plg` call that owner directly; and the former `stan_backend_start` helper is no longer exposed as a `.plg` registration or resolved via `LinkedSpec::get_plugin(...)`. Future mixed-plugin cleanup should keep moving private helper/setup subdefs into domain owners while leaving only visible legacy actions in `.plg` files.
- 2026-04-12: Setup/hold timing helper math moved out of the legacy plugin registry. `perl/Timing/SetupHold.pm` now owns `collect_dxcy(...)`, `calculate_path_delay(...)`, `delay_dici(...)`, `delay_dico(...)`, `delay_doci(...)`, `delay_doco(...)`, `tss_setup_hold_delay(...)`, and `tss_tmax_tmin_delay(...)`; `plugin/setup_hold_tmax_tmin.plg` and `plugin/tssio.plg` call that package owner directly; and the former helper subdefs `DxCy`, `DiCi`, `DiCo`, `DoCi`, `DoCo`, `tss_setup_hold`, and `tss_tmax_tmin` are no longer `.plg` registrations. Future mixed-plugin cleanup should keep collapsing private helper subdefs into package owners, and should prefer explicit package-level dispatch when old helper signatures already show the data they need.
- 2026-04-12: QC summary merge behavior moved out of the legacy plugin registry. `perl/QC/Summary.pm` now owns `QC::Summary::append_merged_rows(...)`; `plugin/qc_summary.plg` calls that package owner directly, no longer loads `LinkedSpec` for `qc_summary_merge`, and no longer exposes `qc_summary_merge` as a `.plg` subdef. Future mixed `.plg` cleanup should keep moving private helper subdefs into package owners instead of preserving internal helper names as dynamic plugin registrations.
- 2026-04-12: HTTP file-access behavior graduated out of the plugin namespace. `perl/Plugin/HTTP.pm` is gone; `perl/HTTP/FileAccess.pm` owns `url_for_path(...)`, `set_hostport(...)`, `set_localhost(...)`, `print_file_links_for_conf(...)`, `run_lighttpd_for_conf(...)`, and `run_httpd_for_conf(...)`; `TableScript`, `HTML::PathLinks`, `Text::VariableSubstitution`, `rtl.plg`, `stan_backend.plg`, and `tree.plg` call `HTTP::FileAccess` directly. The historical `httplink(...)`, `set_http_hostport(...)`, and `set_http_localhost(...)` helper names are retired in current repo-owned usage, and there are no live `Plugin::*` implementation owners left.
- 2026-04-12: Path-token link rendering graduated out of the plugin namespace. `perl/Plugin/CGI.pm` is gone; `perl/HTML/PathLinks.pm` owns `link_path_tokens(...)`; and `Text::VariableSubstitution::var_subst_test(...)` calls `HTML::PathLinks::link_path_tokens(...)` directly. The historical `file_list_path2http(...)` helper name is retired in current repo-owned usage.
- 2026-04-12: String substitution graduated out of the plugin namespace. `perl/Plugin/String.pm` is gone; `perl/Text/VariableSubstitution.pm` owns `var_subst(...)` and `var_subst_test(...)`; path-link rendering now lives in `HTML::PathLinks`.
- 2026-04-12: GenericFilter table grouping graduated out of the plugin namespace. `perl/Plugin/GenericFilter.pm` is gone; `perl/Table/GenericFilter.pm` owns `dispatch(...)`, `group_by(...)`, `group_by_port(...)`, `group_by_ioclock(...)`, and `group_byRE(...)`; `HUtils.pm` and `TableSort.pm` call `Table::GenericFilter::dispatch(...)` directly.
- 2026-04-12: Excel automation helper graduated out of the plugin namespace. `perl/Plugin/MSOffice.pm` is gone; `perl/MSOffice/Excel.pm` owns `start(...)`; and `spyglass.plg` calls `MSOffice::Excel::start()` directly. Keep using `Plugin::*` only as temporary extraction scaffolding when no clearer domain owner is available.
- 2026-04-12: VHDL constant helpers graduated out of the plugin namespace. `perl/Plugin/VHDLConst.pm` is gone; `perl/VHDL/ConstantEval.pm` owns `evaluate_constant_values(...)`, `substitute_hash_values(...)`, and `print_constant_values_for_conf(...)`; `mbist.plg` and `regtest.plg` call `VHDL::ConstantEval` directly. Keep using `Plugin::*` only as temporary extraction scaffolding when no clearer domain owner is available.
- 2026-04-11: Prompt helper package graduated out of the plugin namespace. `perl/Plugin/Prompt.pm` is gone; `perl/InteractivePrompt.pm` owns `yes_no(...)`; and `fxenv_helper.plg` calls `InteractivePrompt::yes_no(...)` directly. Future cleanup should keep treating `Plugin::*` as extraction scaffolding, not a final namespace when a better domain owner is obvious.
- 2026-04-11: `plugin/yesno.plg` removed. `Plugin::Prompt::yes_no(...)` now owns the interactive prompt helper, `fxenv_helper.plg` calls it directly, empty input still defaults to yes, and the no-branch array-callback path is fixed while moving out of the legacy `.plg` body.
- 2026-04-11: `plugin/vhdconst_eval.plg` removed. At that migration step the VHDL constant helpers moved into the short-lived `Plugin::VHDLConst` scaffold; the newer 2026-04-12 note above records that current callers now use `VHDL::ConstantEval`.
- 2026-04-11: `plugin/msoffice.plg` removed. At that migration step `spyglass_waive` moved to the short-lived `Plugin::MSOffice` scaffold; the newer 2026-04-12 note above records that current callers now use `MSOffice::Excel::start()`.
...
- 2026-04-11: `plugin/httpd.plg` removed after moving the historical `httpd` action into the then-current `Plugin::HTTP::run_httpd_for_conf(...)` scaffold. The action was already stale because repo-owned `conf/httpd.conf` had been intentionally removed, but the package owner preserved the behavior for callers that provide `Global->httpd_conf`, validates port/action input, closes the generated temp config before launch, and uses list-form `system('apachectl', '-k', $action, '-f', $filename)`; the newer 2026-04-12 note above records that current usage now lives in `HTTP::FileAccess`.
- 2026-04-11: `plugin/lighttpd.plg` removed after moving the remaining real `lighttpd` action into the then-current `Plugin::HTTP::run_lighttpd_for_conf(...)` scaffold. The package owner rendered `Global->lighttpd_conf`, validates the port, closes the generated temp config before launch, and uses list-form `system('lighttpd', '-f', $filename)` instead of shell-string execution; the newer 2026-04-12 note above records that current usage now lives in `HTTP::FileAccess`. Helper/action-only `.plg` shadows should keep being deleted once no repo-owned caller needs the registration name.
- 2026-04-11: `plugin/http.plg` removed after moving the remaining real `http` file-link action into the then-current `Plugin::HTTP::print_file_links_for_conf(...)` scaffold. That step preserved the historical CGI config lookup, host/port setup, existing-file filter, and signed URL printing directly. At that moment `plugin/lighttpd.plg` still carried the real legacy `lighttpd` action; the newer notes above record that it is now gone too and current HTTP file access lives in `HTTP::FileAccess`.
- 2026-04-11: `plugin/string.plg` and `plugin/genericfilter.plg` removed as pure package-backed helper-registration shells. String substitution briefly lived in `Plugin::String` before the newer 2026-04-12 note above moved current usage to `Text::VariableSubstitution`; GenericFilter briefly lived in `Plugin::GenericFilter`, and the newer 2026-04-12 note above records that current callers now use `Table::GenericFilter`. Future resume should delete package-backed `.plg` wrappers once they have no concrete repo-owned legacy caller rather than keeping compatibility names by default.
- 2026-04-11: Earlier HTTP package-backed helper registrations were retired instead of preserved as permanent plugin names. Repo-owned `.plg` callers in the then-existing `http.plg`, plus `rtl.plg`, `stan_backend.plg`, and `tree.plg`, were moved to the then-current `Plugin::HTTP` scaffold for `httplink`, `set_http_hostport`, and `set_http_localhost`; the newer notes above record that `http.plg` itself is now gone and current HTTP file access lives in `HTTP::FileAccess`.
- 2026-04-11: `plugin/cgi.plg` compatibility wrapper removed. The path-link behavior briefly lived in `Plugin::CGI::file_list_path2http(...)` before the newer 2026-04-12 note above moved current usage to `HTML::PathLinks::link_path_tokens(...)`, and the regression suite locks that the wrapper is gone instead of still parsing under `pplugin`. Future `.plg` extraction should delete the wrapper as soon as no concrete repo-owned legacy caller still needs the old plugin-registration name.
- 2026-04-11: GenericFilter plugin extraction landed and then advanced past the temporary wrapper step. At that point `Plugin::GenericFilter` owned the grouping helpers and `plugin/genericfilter.plg` had been removed; the newer 2026-04-12 note above records that current callers now use `Table::GenericFilter` directly instead of `LinkedSpec::run_plugin(...)` / `get_plugin(...)` over constructed `genericfilter_*` names.
- 2026-04-11: Continued the plugin/resource modernization track across shipped `.plg` callers. The then-existing `plugin/plugin.plg`, plus `plugin/qcflow.plg`, `plugin/setup_hold_tmax_tmin.plg`, `plugin/stan_omap2430c_backend.plg`, and `plugin/tssio.plg`, moved to `LinkedSpec::get_plugin(...)` instead of direct `PPlugin->get(...)`; the newer setup/hold note above records that `setup_hold_tmax_tmin.plg` and `tssio.plg` now call `Timing::SetupHold` directly for private timing helpers, and the newer plugin-shim note records that `plugin/plugin.plg` is now removed after its remaining caller moved to `LinkedSpec::get_plugin(...)` directly. Regression coverage now scans all repo-owned `plugin/*.plg` files to keep direct `PPlugin->get(...)` constrained to compatibility bridge/adapter internals. Future plugin work should keep repo-owned plugin bodies behind `LinkedSpec::get_plugin(...)` / `run_plugin(...)` or package owners, not direct `PPlugin` access.
- 2026-04-11: OwnerDispatch delegated owner-call cleanup landed. `LinkedSpec::OwnerDispatch::dispatch_owner_call(...)` now resolves delegated callbacks through `require_pkg_cb(...)` instead of maintaining a second internal package-`can(...)` plus symbol-call path; regression coverage locks list-context return preservation and successful `$@` preservation for that shared callback-loader route.
- 2026-04-11: Owner wrapper callback lookup cleanup landed. `Runtime`, `Compiler`, `BootstrapSpec`, `SpecEntry`, and `ActionIR::Scanner` now use `LinkedSpec::OwnerDispatch::require_pkg_cb(...)` instead of local `Package->can(...)` callback probes. A source-level regression locks those wrappers against reintroducing `->can(...)`; remaining direct `->can(...)` hits are `OwnerDispatch` itself plus `RuleIR::EmitContext`'s custom owner-key registry diagnostics.
- 2026-04-11: The bootstrap-local `spec_descr` / `gdata` naming island is now cleaned in active code. `LinkedSpec::BootstrapSpec` and `BootstrapSpec::Core` use `rule_descriptors` / `dispatch_state`, with state keys `start_token_re`, `start_rule_dispatch`, `brace_scanner_re`, and `current_rule_label`. Treat old `spec_descr` / `gdata` wording as historical compatibility/negative-test language unless a future scan proves otherwise.
- 2026-04-11: README/SESSION_BOOTSTRAP architecture refresh completed. Current reading: `LinkedSpec.pm` is still a thin facade and active compiler/facade naming is `build_compiled_rule_table(...)` plus compiled dependency-regex state. At refresh time, the remaining `spec_descr` / `gdata` names were bootstrap-local internals in `LinkedSpec::BootstrapSpec` / `BootstrapSpec::Core`; the follow-up note above records that this naming island has since been cleaned.
- 2026-04-11: Public mdBook blind-call post-processing examples now use explicit `return(array(...))` payloads instead of the historical `return_a(...)` shortcut, including the fluent post-call and block post-call examples in `docs/linkedspec-book/src/user-model/blind-calls-and-parser-orchestration.md`. Root guides plus the public book now teach visible return payloads for normal blind-call examples; legacy tagged return helpers remain compatibility references.
- 2026-04-11: Root-guide ordinary examples now align with the canonical helper story. `USER_GUIDE.md` no longer uses `return_a(...)` as filler in normal examples, and `USER_GUIDE_RuleModesAndSplit.md` no longer teaches the old raw push plus `capture_if(...)` / `CAPTURE_IF()` EBNF logging-annotation shape. Remaining `return_a(...)` / `capture_if(...)` mentions in those root guides are compatibility-list references only.
- 2026-04-11: Live VHDL return helper authoring now prefers explicit `return(array(...))` payloads over legacy tagged aliases. `specs/vhdl.spec` no longer uses `return_m`, `return_ma`, `return_a`, or `flat_array(IMATCH_LIST)` in the migrated helper-return band; replacements use `flat_array(entry_groups())` for capture groups and `array_copy(array(rule))` for current-rule accumulators. The slice also fixed MethodLowering so `flat_array(entry_groups())` / `flat_array(match_groups())` lower as real flattened array-valued helper expressions instead of runtime helper calls. Docs now show modern replacements for those compatibility helpers. Future DSL consistency work should leave the alias behavior covered as compatibility unless explicitly retiring it; the next cleanup target can move to remaining compatibility aliases or convention-based accumulator helpers outside VHDL.
- 2026-04-10: Array-edge drop authoring now prefers `drop_front(...)` and `drop_back(...)`. `tail(...)` and `drop_last(...)` remain documented compatibility aliases with direct regression coverage, but current user-facing examples, mdBook helper reference, emitted-Perl reference, flow examples, and fluent/structured regression fixtures use the explicit edge names as canonical. Future DSL consistency work should not treat this as an open naming question; track it only as eventual compatibility-alias retirement policy.
- 2026-04-10: `flat(...)` is now the canonical generic list-context splice helper name. `flatten(...)` remains a documented compatibility alias with explicit regression coverage for array/hash generic splice forms, but current docs no longer present it as a peer spelling or normal example. Next DSL consistency targets remain legacy return helper overlap and convention-based accumulator helper documentation.
- 2026-04-10: Live snapshot-helper authoring now prefers `array_copy(...)` over compatibility `array_values(...)`. Remaining shipped specs using the old spelling (`lib_reader`, `sdce`, `ds_vhistory`, `Lispish`, `hlink_substitution`, `simenv`, `vhdl`) plus current examples/source locks were migrated. Keep explicit `array_values(...)` tests/docs only where compatibility-alias behavior is the subject. Future cleanup target is eventual alias-retirement policy, not live-spec migration.
- 2026-04-10: Child-call appends are now standardized on `push(...)`, not duplicate `push_call(...)`. `push(Rule)` appends `call(Rule)` into the current rule's default accumulator array; `push(Rule, index)` appends `call(Rule)->[index]` into that array; `push(Rule, target)` and `push(Rule, target, index)` handle explicit target arrays. Internal raw-wrapper contract IDs now use `push_child_call_*_builtin`, not `push_call_*_builtin`, so descriptor metadata does not look like the removed public helper. The public mdBook documents the default accumulator convention and the older capture/tagged-return helpers that also use it. Future exhaustive DSL cleanup should scan for duplicate helper names, conflicting argument orders, and undocumented convention-based target selection; current live-roadmap audit targets are legacy return helper overlap, compatibility-alias retirement policy, and convention-based accumulator helpers.
- 2026-04-10: `push_nonempty(array(target), value)` is now the preferred modern helper for optional accumulator appends. It lowers/classifies as canonical `PUSH`, skips `undef`, empty strings, empty arrayrefs, and empty hashrefs, and preserves meaningful values such as `"0"`. `specs/ebnf.spec::logging_annotation` now uses `push_nonempty(a(logging_annotation), trim(capture_slice()))` instead of the legacy `capture_if(...)` / `CAPTURE_IF()` surface, and the public mdBook helper reference plus EBNF walkthrough document the rationale and examples. Future resume should use `push_nonempty(...)` for trimmed optional capture/child-result append patterns instead of reintroducing capture-if helpers in new authoring.
- 2026-04-10: LinkedSpec mdBook shipped-material path now also has a detailed `ebnf.spec` walkthrough at `docs/linkedspec-book/src/specs-and-corpora/ebnf-spec-walkthrough.md`. It covers `get_parser('ebnf')`, the `Expr`/`Term` AST shape, top-level include entries, `grammar_file` accumulator state, guarded body-token parsing, terminal token readers, semantic/logging annotations, return annotations, `ebnf/*.ebnf` corpus coverage, descriptor ActionIR readiness, and regression-lock coverage. This slice also fixed shared `capture_if(...)` / `CAPTURE_IF()` lowering in `perl/LinkedSpec/ActionIR/Contracts.pm` so the emitted whitespace trim regex stays literal; `t/phase0_regression.t` now locks both the trim lowering and `@log_rule("expr", "term")` runtime parsing. Natural next book slices: `vhdl.spec` walkthrough or focused domain-spec walkthroughs such as `tablegrep.spec` / `portmap.spec` / `sdce.spec`.
- 2026-04-10: LinkedSpec mdBook shipped-material path now has its first detailed shipped-spec walkthrough at `docs/linkedspec-book/src/specs-and-corpora/lispish-spec-walkthrough.md`. It covers running `get_parser('Lispish')`, the nested AST shape, head/tail and `undef` tail conventions, rule inventory, top-level dispatch, recursive `parenthesis` helper flow, token readers, comment handling, descriptor ActionIR-readiness facts, and `perl/Lispish.pm` convenience helpers. Natural next book slices: `ebnf.spec` walkthrough, `vhdl.spec` walkthrough, or smaller focused domain-spec walkthroughs such as `tablegrep.spec` / `portmap.spec` / `sdce.spec`.
- 2026-04-10: LinkedSpec mdBook first-reader examples are now aligned with the modern helper-style story. `spec-files-and-rule-paragraphs.md` and `get-and-get-parser.md` no longer introduce the project with tiny legacy `return_a(...)` examples; they now use `return(hash(...))` / `match_text()` payloads and link into the worked `.spec` walkthrough. A later 2026-04-11 cleanup also moved the blind-call orchestration chapter's normal post-call examples to explicit `return(array(...))` payloads. Natural next book slices: shipped-spec walkthroughs or more validated helper-example cleanup.
- 2026-04-10: LinkedSpec mdBook user-model path now has a worked `.spec` walkthrough at `docs/linkedspec-book/src/user-model/worked-spec-walkthrough.md`. It walks a validated `Pair::AND` helper-style parser through regex captures, zero-based `match_group(...)`, `trim(...)`, `hash(...)`, `return(...)`, inline `Get(...)`, `seek` versus `consume`, descriptor mode, `runtime_ctx_ref`, and natural evolution toward blind calls, explicit `call(...)` dataflow, or repeated `OR` extraction. Natural next book slices: per-spec walkthroughs for shipped specs, helper-family deep examples, or replacing remaining tiny legacy `return_a(...)` intro examples with linked modern explanations where appropriate.
- 2026-04-10: LinkedSpec mdBook user-model path now has a dedicated blind-call orchestration guide at `docs/linkedspec-book/src/user-model/blind-calls-and-parser-orchestration.md`. It covers `=>` versus `->`, valid and invalid blind-call syntax, label-driven sequence/choice/repetition behavior, ordered and repeated ordered wrappers, single-choice and repeated-choice wrappers, post-call block/fluent processing, the no-mixed-edge-family rule, same-line versus multiline formatting, and when explicit `assign(scalar(retv), call(Child))` dataflow is clearer. Natural next book slices: per-spec walkthroughs, end-to-end `.spec` walkthrough, or more method-level DSL reference backfill.
- 2026-04-10: LinkedSpec mdBook user-model path now has a detailed rule-mode guide at `docs/linkedspec-book/src/user-model/rule-modes-and-parse-modes.md`. It covers the separate rule-mode and parse-mode axes, baseline `:` / `::` labels, compact `:&` / `:|` / `:+` / `:*` / `:?` sigils, worded and bounded `AND` / `OR` families, choice versus sequence versus repetition, blind-call label-driven semantics, exact spelling expectations, and `seek` versus `consume` parse modes. Natural next book slices: deeper blind-call walkthroughs, per-spec walkthroughs, or additional method-level DSL references.
- 2026-04-10: LinkedSpec mdBook DSL path now has an action/lifecycle placement chapter at `docs/linkedspec-book/src/dsl/action-and-lifecycle-placement.md`. It covers rule paragraph members, `I { ... }` setup, action edges, method-chain action edges, empty action edges, blind-call edges, local match helper reads, `LS`/`LE`, `LX`, advanced `IT`/`EX`/`E`, and placement-sensitive `@capture_slice` / `@mark(name)` markers. Natural next book slices: deeper rule-mode walkthroughs, blind-call orchestration details, or shipped-spec walkthroughs.
- 2026-04-10: LinkedSpec mdBook DSL path now has a declaration helper reference chapter at `docs/linkedspec-book/src/dsl/declaration-helper-reference.md`. It covers canonical `declare(type, ...)`, declaration aliases, `I { ... }` shared-state placement, action-local declarations, initialized scalar/array/hash declarations, fluent vs structured forms, reset-vs-redeclare guidance, and common mistakes. Natural next book slices: lifecycle/control-flow placement reference, per-spec walkthroughs, or remaining repo-root ActionIR helper clusters.
- 2026-04-10: LinkedSpec mdBook DSL path now also has a method-level value/container/flow helper reference chapter at `docs/linkedspec-book/src/dsl/value-container-flow-helper-reference.md`. It covers composition sites, accessors, snapshots vs flattening, `assign`, `call`, `push_value`, `return`, scalar/string/numeric helpers, array/hash helpers, array pipelines, fallback/presence helpers, boolean composition, structured `if`, structured `switch`, debug output helpers, and worked examples. Natural next book slices: declaration helper details, per-spec walkthroughs, or deeper examples for specific DSL clusters.
- 2026-04-10: LinkedSpec mdBook DSL path now has a method-level source-boundary helper reference chapter. It covers `capture_*`, `mark_*`, `cursor_*`, `input_*`, `entry_*`, and `match_*` helper semantics, right-edge choices, stable vs advancing reads, anonymous capture boundaries, named marks, cursor/input helpers, entry vs match helpers, compatibility aliases, and worked examples. Natural next book slice: values/container/control helper reference at the same public documentation bar.
- 2026-04-10: LinkedSpec mdBook development workflow path now documents local CI/regression and docs workflow in detail: the shared `tools/run_ci_local.sh` gate, GitHub delegation from `.github/workflows/ci.yml`, tracked CI input areas, the untracked-input guard, Perl syntax and phase0 regression checks, docs-only validation, code/spec/runtime validation, public book versus continuity docs, mdBook source/build/SUMMARY workflow, documentation style expectations, and the DSL documentation bar. Natural next book slices: method-level DSL references or per-spec walkthroughs.
- 2026-04-10: LinkedSpec mdBook shipped-material path now has a public map of `specs/`, `plugin/`, `conf/`, `tablescript/`, `ebnf/`, and `t/phase0_regression.t`. It lists each shipped `.spec` with main entry rule and purpose, calls out mature/current versus historical/experimental/placeholder material, documents `.plg` as legacy transition rather than future identity, explains corpus regression over `.plg`, `.conf`, `.ts`, and `.ebnf`, ties shipped material to `tools/run_ci_local.sh`, and records the need for future per-spec walkthroughs. Natural next book slices: local CI/development workflow or method-level DSL references.
- 2026-04-10: LinkedSpec mdBook runtime-context/tracing path now has a detailed public guide. It covers `runtime_ctx_ref` hash/scalar-slot forms, context lifecycle across `Get(...)`, `get_parser(...)`, compilation, and parser invocation, structured `last_error` owner/stage payloads, file identity preservation, generated handler source labels, parser-source capture via `dump_parser_source` / `parser_source_ref`, trace configuration through `configure_trace(...)` or compile options, named trace levels, trace output modes, env vars, trace scopes/decisions, and the rule of thumb that runtime context records what failed while tracing explains how the system got there. Natural next book slices: shipped specs/corpora, local CI/development workflow, or method-level DSL references.
- 2026-04-10: LinkedSpec mdBook architecture path now has a self-contained owner-tree and module-boundary chapter. It covers `LinkedSpec.pm` as facade, `OwnerDispatch` as shared dispatch plumbing, `get_parser(...) -> ParserFactory -> Runtime -> Compiler`, `Get(...) -> Runtime -> Compiler`, `Resolver`, `RuntimeContext`, `Compiler` vs `CompilerState`, bootstrap/validation ownership, `SpecEntry` / `RuleIR` / `RuleIR::EmitContext`, `ActionIR::*`, trace separation, legacy plugin transition machinery, and a practical owner map for routing future changes. Natural next book slices: shipped specs/corpora, runtime context/tracing, local CI/development workflow, or method-level DSL references.
- 2026-04-09: LinkedSpec mdBook compiler/runtime path now explains the full pipeline, compiled state records, generated handler dispatch, and structured diagnostics. Key public-book concepts now covered: `build_compiled_rule_table(...)`, `build_dependency_regex_map(...)`, `compiled_spec_state`, `compiled_dependency_regex_state`, `compiled_descriptor_state`, `dependency_regex_map`, `dependency_refs`, `DEPENDENCY_REFS`, `runtime_ctx->{last_error}`, owner/stage families, rule labels, handler source labels, and fallback-preservation rules. Natural next book slices: architecture owner-tree deepening, shipped specs/corpora, or method-level DSL reference expansion.
- 2026-04-09: LinkedSpec mdBook DSL/action path now has public chapters for ActionIR lowering, capture/mark/source-location helpers, and value/container/control helpers. It explains raw-Perl migration rationale, the `.spec helper DSL -> canonical ActionIR -> emitted backend code` model, and helper-family choices across `capture_*`, `mark_*`, `cursor_*`, `entry_*`, `match_*`, `input_*`, `assign(...)`, `push_value(...)`, `return(...)`, container helpers, and structured flow. Natural next book slice: deepen compiler/runtime chapters or turn one DSL helper family into a method-by-method public reference chapter.
- 2026-04-09: First LinkedSpec mdBook deepening slice expanded the user-model and public-API path. The book now has concrete `.spec` rule-paragraph examples, top-level rule-start versus block-content explanation, same-line/multiline examples, exact rule-mode spelling notes, `seek`/`consume` examples, `Get(...)`/`get_parser(...)` option examples, and descriptor-shape documentation for `spec`, `dependency_regex_map`, `dependency_refs`, and `meta`. Natural next book slices: deepen ActionIR/helper-surface chapters or compiler/runtime chapters with the same example-heavy public style.
- 2026-04-09: Public-book vs continuity-doc policy is now explicit. `docs/linkedspec-book/` is the public explanation layer for LinkedSpec, while `CHANGES.md`, `DEVELOPMENT_NOTES.md`, `MEMORY.md`, roadmap notes, and `COMMIT.md` are internal continuity docs for crash recovery, handoff, and execution hygiene. Future resume should keep those roles separate.
- 2026-04-09: Started the real LinkedSpec mdBook at `docs/linkedspec-book/` and seeded the first public chapter spine. Current scaffold covers overview, user model, public API, DSL/action surface, compiler/runtime internals, shipped material, architecture, and development workflow, and it builds cleanly with `mdbook build docs/linkedspec-book`. Natural next book work is to deepen each chapter family rather than creating a second public-doc surface elsewhere.
- 2026-04-09: The active derived-regex descriptor path is now fully on `dependency_regex_map` / `dependency_refs` / `DEPENDENCY_REFS`. `Compiler.pm`, `CompilerState.pm`, `SpecEntry.pm`, `Validation.pm`, `RuleIR::EmitContext.pm`, the static parser fixtures, and the active regression locks now all use those names on the live compiler/runtime/public path. Treat `gdata` as historical vocabulary unless the task is explicitly about older notes or outward compatibility descriptor shapes.
- 2026-04-09: Active final-descriptor compiler naming is now fully `descriptor`, not `descr`. The live seam is `build_final_descriptor` / `_build_final_descriptor_state(...)` / `_compiled_descriptor_state_to_legacy_descriptor(...)`, and structured failures now report `build_final_descriptor` at both `stage` and `compiler_pipeline:build_final_descriptor`. Do not reintroduce `build_final_descr` on the active path.
- 2026-04-09: Removed the mistakenly added external-project mdBook/doc slice and restored the active documentation scope to LinkedSpec-only. Future resume should not rebuild that external-project book scaffolding here unless the project scope is explicitly redefined first.
...
- 2026-04-08: Renamed the active low-level compiler seam from `spec_descr(...)` to `build_compiled_rule_table(...)` with no compatibility wrapper preserved under the old name. `Compiler.pm`, `LinkedSpec.pm`, the active regression locks, and the current docs/roadmaps now all treat compiled rule-table construction as the real concept and `compiler_pipeline:build_compiled_rule_table` as the real diagnostics stage. Future resume should treat `spec_descr` as historical vocabulary only unless the discussion is explicitly about older notes.
- 2026-04-08: Continued the same compiler-state extraction line by clarifying the naming model around historical `gdata`. `LinkedSpec::CompilerState` now treats dependency-regex state as the preferred internal term and keeps `compiled_gdata_state`, `gdata_by_label`, and `gdata_rows` as compatibility aliases over the same payload. Future resume should use dependency-regex state as the architectural term unless the slice is specifically about outward compatibility descriptors or legacy validation APIs.
- 2026-04-08: Continued the same validation hardening line by unifying the gdata-validation engine in `Validation.pm`. Legacy `validate_gdata_references(...)` input and owner-provided descriptor-state validation views now route through one shared validation-view validator, so future resume should not reintroduce parallel copies of the same gdata/rule consistency checks.
- 2026-04-08: Continued the same compiler-state extraction line into descriptor validation view assembly too. `LinkedSpec::CompilerState` now exposes one `compiled_descriptor_state_validation_view(...)`, and `Validation.pm` now consumes that single owner-provided view instead of issuing repeated owner lookups from inside descriptor-state validation loops. Future resume should keep descriptor-state validation on that single owner-provided view seam.
- 2026-04-08: Continued the same compiler-state extraction line into descriptor-state validation views. `Validation.pm` now walks owner-provided descriptor-state rule rows, descriptor-state gdata rows, and descriptor-state by-label lookup through `LinkedSpec::CompilerState` instead of flattening descriptor state back into raw rule/gdata maps first. Future resume should keep descriptor-state validation on that owner-provided view layer.
- 2026-04-08: Extracted the compiled state model into `LinkedSpec::CompilerState`. That owner now defines compiled-spec / compiled-gdata / compiled-descriptor state construction, shape checks, and legacy projection, and both `Compiler.pm` plus `Validation.pm` now delegate their state-model work through it. Future resume should treat compiler-state ownership as a separate architectural seam now.
...
- 2026-04-07: Refactored the historical `spec_descr` / `spec_gdata` seam into one explicit compiler-owned compiled-spec state model. `Compiler.pm` now builds `compiled_spec_state` first (`kind`, `version`, `definition_order`, `rule_order`, `rules_by_label`, `redefined_rule_labels`), default `spec_gdata(...)` now enriches that state directly, `_build_final_descriptor(...)` now projects legacy `spec` / `gdata` hashes outward while exposing `meta.descriptor_model`, `meta.rule_order`, and `meta.redefined_rule_labels`, and low-level callers can request the same richer state directly with `spec_descr(..., { return_state => 1 })`. Future resume should treat compiled-spec state as the compiler's internal source of truth now instead of reasoning from the legacy spec hash first.
- 2026-04-07: Tightened the default `spec_gdata(...)` final-descriptor seam too. Malformed descriptor content there now fails with explicit contract detail instead of incidental Perl reference errors, and `run_get_pipeline(...)` preserves that as `compiler_pipeline:build_final_descriptor` with the active `rule_label` plus label-scoped `handler_source_label` continuity. Future resume should treat malformed `gdata` / dependency / referenced-`re` shapes as covered structured Phase 5 surface now instead of assuming `build_final_descriptor` only preserves thrown inner callback failures.
- 2026-04-07: Tightened the low-level `spec_descr(...)` entry-shape seam too. Malformed individual parsed entries inside an otherwise ARRAY-shaped parsed-entry list now normalize into the same `compiler_pipeline:spec_descr` payload family when `runtime_ctx_ref` is in use, with preserved `top_rule` plus label-scoped `handler_source_label` continuity when no usable parsed label exists on the malformed entry itself. Future resume should treat malformed individual parsed bootstrap entries as covered structured Phase 5 surface now instead of assuming only the outer container and callback contracts are validated there.
- 2026-04-07: Tightened the remaining low-level `spec_descr(...)` pre-entry contract seam. Invalid non-CODE `compile_spec_entry` callbacks and malformed non-ARRAY parsed-entry containers now normalize into the same `compiler_pipeline:spec_descr` payload family when `runtime_ctx_ref` is in use, with requested/seeded `top_rule` plus label-scoped `handler_source_label` continuity when available. Future resume should treat low-level `spec_descr(...)` contract validation as covered structured Phase 5 surface now instead of assuming only in-loop callback failures are normalized there.
- 2026-04-06: Tightened the low-level compiler-facing `spec_descr(...)` surface too. It now accepts `runtime_ctx_ref => \$ctx`, seeds `top_rule` from the parsed entries when available, and normalizes both thrown and malformed non-throwing `compile_spec_entry(...)` failures into the same `compiler_pipeline:spec_descr` payload family with `rule_label` and label-scoped `handler_source_label` continuity. Future resume should treat standalone `spec_descr(...)` diagnostics as covered structured Phase 5 surface now instead of assuming only `run_get_pipeline(...)` exposes compiler-owned `spec_descr` failures cleanly.
- 2026-04-06: Tightened the top-level parser boundary itself. Returned parser coderefs now reject malformed non-`SCALAR`-reference input explicitly at `runtime_parser:validate_input_ref` instead of falling through to lower-level Perl dereference failures, and focused regression coverage now locks that both on the direct `run_get_pipeline(...)` seam and through file-oriented `get_parser(...)` continuity after spec resolution/load. Future resume should treat parser-input shape validation as covered runtime-parser surface now instead of assuming top-level parser failures only begin at handler resolution or handler dies.
- 2026-04-05: Continued the same Phase 5 diagnostics-attribution line into the earlier compiler-owned seams too. `Compiler.pm` now preserves the label-only generated-handler identity `LinkedSpec::generated_handler:<top_rule>` not only on later rule-attributed failures, but also on `prepare_pipeline` and `bootstrap_parse` payloads whenever the selected top rule is already known. Regression coverage now locks that through both the direct `run_get_pipeline(...)` seam and the public `LinkedSpec::Get(...)` path. Future resume should treat compiler pre-rule handler attribution as covered structured-diagnostics surface now instead of assuming label-only handler identity starts only at later compiler stages or runtime/parser boundaries.
- 2026-04-05: Continued the same Phase 5 diagnostics-attribution line one seam earlier on the file-oriented path. `ParserFactory.pm` now preserves the label-only generated-handler identity `LinkedSpec::generated_handler:<top_rule>` not only on `parser_factory:compile_spec` fallback payloads, but also on the earlier `prepare_parser_factory`, `validate_spec_name`, `resolve_spec_path`, and `load_spec_content` failure seams whenever the selected top rule is already known. Regression coverage now locks that through both the direct `run_get_parser(...)` seam and the public `get_parser(...)` path. Future resume should treat parser-factory pre-compile handler attribution as covered structured-diagnostics surface now instead of assuming label-only handler identity starts only at compile fallback or runtime/parser boundaries.
...
- 2026-04-04: Continued the same real Phase 5 diagnostics line one step later in the parser-factory file-loading seam. `load_spec_content(...)` now reports structured failure metadata on non-throwing resolver-side open failures, and `ParserFactory.pm` now preserves that resolver-specific summary/detail on `parser_factory:load_spec_content` false-return failures instead of collapsing back to the old generic `Spec file load failed` wrapper. Future resume should treat resolver-attributed false-return unreadable-file failures as covered parser-factory diagnostics surface now instead of assuming only thrown load callbacks preserve specific text.
- 2026-04-04: Continued the same real Phase 5 diagnostics line on the parser-factory resolution seam. `resolve_spec_path(...)` now reports structured failure metadata on non-throwing resolver-side `undef` paths, and `ParserFactory.pm` now preserves that resolver-specific summary/detail on `parser_factory:resolve_spec_path` false-return failures instead of collapsing back to the old generic `Spec resolution failed` wrapper. Future resume should treat resolver-attributed false-return resolution failures as covered parser-factory diagnostics surface now instead of assuming only thrown resolver callbacks preserve specific text.
- 2026-04-04: Continued the same real Phase 5 diagnostics line on the parser-factory side. `validate_spec_name(...)` now reports structured failure metadata for non-throwing invalid-name rejections, and `ParserFactory.pm` now preserves the validator’s specific summary/detail on `parser_factory:validate_spec_name` false-return failures instead of collapsing back to the old generic wrapper string. Future resume should treat invalid-name false returns as covered parser-factory diagnostics surface now instead of assuming only thrown parser-factory callbacks preserve specific failure text.
- 2026-04-04: Continued the same real Phase 5 diagnostics line one stage earlier in the compile flow. `validate_spec_content(...)` now reports structured failure metadata for non-throwing envelope-validation rejections, and `Compiler.pm` now preserves the validator’s specific summary/detail on `compiler_pipeline:validate_spec_content` false-return failures instead of collapsing back to the old generic input-envelope banner. Malformed-first-rule failures now also preserve `rule_label` plus the label-only synthetic `handler_source_label` when that early rule label is already known. Future resume should treat attributed `validate_spec_content` false returns as covered structured-diagnostics surface now instead of assuming only later rule-level validation or descriptor-build stages can expose specific compile-time attribution.
- 2026-04-04: Continued the real Phase 5 diagnostics line back into rule-level DSL validation false-return paths. `validate_dsl_syntax(...)` now reports structured failure metadata for non-throwing validation rejections, and `Compiler.pm` now preserves the validator’s specific summary/detail plus `rule_label` and label-only `handler_source_label` when the offending rule paragraph is still known. Future resume should treat rule-attributed `compiler_pipeline:validate_dsl_syntax` failures as covered structured-diagnostics surface now instead of assuming only thrown validation callbacks or later descriptor-build stages can expose rule attribution.
- 2026-04-02: Continued the real Phase 5 diagnostics line at the remaining parser-boundary missing-entry seam. `runtime_parser:resolve_top_rule_handler` now preserves `handler_source_label` even when the selected top-rule label is known but the compiled descriptor entry itself is missing, so callers still get the stable label-only form `LinkedSpec::generated_handler:<top_rule>` before any exact handler variant can exist. Future resume should treat missing-top-rule-descriptor-entry failures as covered generated-handler attribution now instead of assuming label-only handler-source identity starts only after a handler entry or variant is present.
- 2026-04-02: Continued the real Phase 5 diagnostics line with synthetic generated-handler attribution on compile-time rule failures. `Compiler.pm` now preserves `handler_source_label` on attributed `compiler_pipeline:spec_descr`, `compiler_pipeline:build_final_descriptor`, and `compiler_pipeline:validate_gdata_references` failures whenever the failing rule label is already known, and the parser boundary now also preserves that same label whenever at least the selected top-rule label is known even if no exact handler variant is available yet. Future resume should treat label-only forms like `LinkedSpec::generated_handler:Top` as part of the stable structured-diagnostics contract now instead of assuming `handler_source_label` only appears on runtime-handler variant-known failures.
- 2026-04-02: Continued the real Phase 4 feature line with the missing whole-input right-edge position helper. `input_end_pos()` is now a first-class helper contract across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat direct whole-input right-edge position reads as covered helper surface now instead of overloading `input_len()` or falling back to raw `length($$STRING)` for boundary-oriented logic.
- 2026-04-01: Continued the real Phase 4 feature line with the missing whole-input right-edge location pair. `input_end_line()` and `input_end_col()` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat direct whole-input right-edge line/column reads as covered helper surface now instead of falling back to raw `length($$STRING)` plus newline/column math or round-tripping through explicit absolute input-end marks.
- 2026-04-01: Continued the real Phase 4 feature line with the missing whole-input read pair. `input_text()` and `input_len()` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat direct whole-input text/width reads as covered helper surface now instead of falling back to raw `$$STRING` / `length($$STRING)` or round-tripping through explicit absolute input-boundary marks.
- 2026-04-01: Continued the real Phase 4 feature line with the missing absolute input-boundary writer pair. `mark_input_start(name)` and `mark_input_end(name)` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Their lowering is also safe as standalone writer statements even when the stored boundary itself is `0`. Future resume should treat explicit whole-input boundary writes as covered helper surface now instead of improvising that meaning through `mark_here(name)`, rule-entry boundaries, or raw `0` / `length($$STRING)` plumbing.
- 2026-04-01: Continued the real Phase 4 feature line with the missing advancing current-edge width pair. `capture_take_len()` plus `capture_take_len_from(name)` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat rolling anonymous and named current-edge width reads as covered helper surface now instead of chaining a stable current-edge width read with a separate anonymous or named boundary move.
- 2026-04-01: Continued the real Phase 4 feature line with the missing advancing end-of-input tail family. `capture_take_rest()` / `capture_take_rest_len()` plus `capture_take_rest_from(name)` / `capture_take_rest_len_from(name)` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat rolling anonymous and named tail-through-end-of-input reads as covered helper surface now instead of chaining a stable tail read with a separate anonymous or named boundary move.
- 2026-04-01: Continued the real Phase 4 feature line with the missing advancing explicit two-mark width helper. `capture_take_between_len(start_mark, end_mark)` is now a first-class helper contract across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat rolling explicit two-mark width reads as covered helper surface now instead of chaining `capture_len_between(...)` with a separate mark advance.
- 2026-04-01: Continued the real Phase 4 feature line with the missing advancing through-cursor width pair. `capture_take_until_cursor_len()` and `capture_take_until_cursor_len_from(name)` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat rolling through-cursor width reads as covered helper surface now instead of chaining a stable through-cursor width read with a separate anonymous or named boundary move.
- 2026-04-01: Continued the real Phase 4 feature line with explicit start-edge human-readable aliases. `entry_start_line()` / `entry_start_col()` and `match_start_line()` / `match_start_col()` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat explicit immediate/local start-edge line and column reads as covered helper surface now instead of relying only on the older shorter `entry_line()` / `entry_col()` / `match_line()` / `match_col()` names.
- 2026-04-01: Continued the real Phase 4 feature line with the missing stable named boundary writers. `mark_entry_start(name)` and `mark_entry_end(name)` are now first-class writer-side companions to `entry_start_pos()` / `entry_end_pos()`, and `mark_match_end(name)` is now the stable writer-side companion to `match_end_pos()`, all across the contract/scanner/canonical stack with regression coverage proving both runtime behavior and no-raw-fallback lowering. Future resume should treat stable named entry/match boundary writes as covered helper surface now instead of chaining raw boundary readers into manual mark plumbing.
- 2026-04-01: Continued the real Phase 4 feature line with the missing advancing through-cursor pair. `capture_take_until_cursor()` and `capture_take_until_cursor_from(name)` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both runtime behavior and no-raw-fallback lowering. Future resume should treat rolling through-cursor capture as covered helper surface now instead of chaining a stable through-cursor read with a separate anonymous or named boundary move.
- 2026-04-01: Continued the real Phase 4 feature line with the missing anonymous/named bridge pair. `mark_capture_slice(name)` now snapshots the current anonymous capture boundary into a stable rule-local named mark, and `start_capture_slice_from(name)` now restores that stored named checkpoint back into the anonymous rolling-boundary slot, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat anonymous/named boundary bridging as covered helper surface now instead of falling back to raw `$IPOS` plus `mark_pos(...)` plumbing.
- 2026-04-01: Continued the real Phase 4 feature line with the missing anonymous advancing-read companion. `capture_take()` is now the anonymous capture-boundary counterpart to named `capture_take(name)`, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat anonymous rolling split-cursor reads as covered helper surface now instead of falling back to raw `$IPOS` span math plus a separate `$IPOS = pos $$STRING` write.
- 2026-04-01: Continued the real Phase 4 feature line with the missing live-cursor tail pair. `cursor_rest()` and `cursor_rest_len()` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should treat live parser-cursor “through end-of-input” reads as covered helper surface now instead of falling back to raw `pos $$STRING` plus `substr(...)`.
- 2026-03-31: Raised the standing documentation bar for the DSL surface. Future resume should treat extensive user-facing documentation with detailed semantics and worked examples as part of feature completion for both new and already-landed DSL methods; thin helper docs are now considered backlog that should be backfilled as families are touched.
- 2026-03-31: Continued the real Phase 4 feature line with the missing through-cursor span family. `capture_slice_until_cursor()` / `capture_slice_until_cursor_len()` now read from the current anonymous capture boundary through the live parser cursor, and `capture_until_cursor_from(name)` / `capture_until_cursor_len_from(name)` now do the same from a named checkpoint. Future resume should treat these as the explicit “right edge is current cursor” helpers instead of falling back to raw `pos $$STRING` plus `substr(...)` math.
- 2026-03-31: Continued the real Phase 4 feature line with the missing named-mark tail pair. `capture_rest_from(name)` and `capture_rest_len_from(name)` are now first-class helper contracts across the contract/scanner/canonical stack, with regression coverage proving both the runtime semantics and the no-raw-fallback ActionIR lowering. Future resume should keep new named-checkpoint “through end-of-input” reads on that helper pair instead of falling back to `mark_pos(...)` plus raw `substr(...)`.
- 2026-03-31: Continued the real Phase 4 feature line with the missing right-edge location companions. `entry_end_line()`, `entry_end_col()`, `match_end_line()`, and `match_end_col()` are now first-class helper contracts across the contract/scanner/canonical stack, with runtime regression coverage proving both the multi-line right-edge line semantics and the same-line right-edge column semantics. Future resume should treat immediate/local match right-edge reporting as complete on the helper surface now: position, line, and column all have direct readers, so new `.spec` authoring should not fall back to manual math around `entry_end_pos()` or `match_end_pos()`.
- 2026-03-31: Pivoted back to a real Phase 4 feature slice instead of another live-spec helper replay. `capture_slice_col()`, `mark_col(name)`, `cursor_col()`, `entry_col()`, and `match_col()` are now first-class 1-based column-read helpers across the contract/scanner/canonical stack, with runtime regression coverage proving the same helper-family model already used for positions and lines. Future resume should keep new same-shape column reporting on that helper family instead of reintroducing raw position-plus-newline math in `.spec` authoring.
...
- 2026-03-24: Started the first actual package-backed `.plg` extraction. This is now a historical snapshot: the old `string.plg` logic first moved into a package owner, then the temporary wrapper was later deleted once repo-owned callers no longer needed the old plugin name. Future resume should use the later policy: move logic into a package owner first, then delete the wrapper when compatibility is no longer needed.
...
- 2026-03-24: Continued the plugin/runtime modernization track by retiring repo-owned use of the legacy `_get_parser(...)` helper plugin. `Lispish.pm` now uses `LinkedSpec::get_parser('Lispish')` directly, and the remaining repo-owned `.plg` callers (`ds_vhistory.plg`, `fsmgen.plg`, and `regtest.plg`) now do the same. Historical note: `plugin/spec.plg::_get_parser` was compatibility-only at that point, and the 2026-04-11 note above records that the shim has since been removed.
- 2026-03-24: Continued the plugin/runtime modernization track by spending public plugin lookup inside smaller repo-owned plugin files. At that migration step, `qc_summary.plg` used `LinkedSpec::get_plugin('qc_summary_merge')` and `skew.plg` used `LinkedSpec::get_plugin('stan_backend_start')` instead of direct `PPlugin->get(...)`; the newer 2026-04-12 and 2026-04-13 notes above record that current `qc_summary` row merging now lives in `QC::Summary` and current STAN backend setup now lives in `Timing::StanBackend`. Future resume should keep migrating repo-owned `.plg` files with known plugin names toward package owners or explicit bridge APIs depending on whether compatibility still needs a dynamic callback.
- 2026-03-24: Continued the plugin/runtime modernization track inside lightweight repo-owned plugin files. `regtest.plg` moved to `LinkedSpec::run_plugin('hvalue_substitute', ...)` instead of instantiating `PPlugin`, and the former `string.plg` was moved through `LinkedSpec::run_plugin('file_list_path2http', ...)` before later package-owner extraction and wrapper removal. Future resume should keep steering repo-owned `.plg` files with known plugin names toward package owners or `run_plugin(...)`, leaving dynamic `plugin { PPlugin->get(...) }`-style compatibility surfaces for later.
...
- Key technical outcome:
  - `LinkedSpec::Compiler::_build_final_descriptor(...)` now owns the default `spec_gdata` callback,
...
- 2026-04-08: Continued the same compiler-state extraction line for future resume into migration-summary shaping too. `action_rewriter_migration` is now built by `LinkedSpec::CompilerState::build_action_rewriter_migration_summary(...)`, while `Compiler.pm` only logs that owner-produced summary when tracing asks for it. The owner-routing regression now traps that seam too; keep future migration-summary shaping on the state owner side.
- 2026-04-08: Continued the same compiler-state extraction line for future resume into descriptor metadata assembly as well. `_build_final_descriptor_state(...)` now gets descriptor metadata from `LinkedSpec::CompilerState::build_compiled_descriptor_meta(...)` instead of taking compiled-spec metadata and mutating in `parse_mode` / migration-summary fields locally. The owner-routing regression now traps that seam too; keep future descriptor-meta shaping on the state owner side.
- 2026-04-08: Continued the same compiler-state extraction line for future resume into by-label lookup as well. `spec_gdata(...)` now checks dependency presence and fetches dependent rule info through `LinkedSpec::CompilerState` helpers instead of probing the compiled rule map directly. The owner-routing regression now traps those by-label lookups too; keep future compiler-state consumers on owner-provided lookup helpers rather than direct map access.
- 2026-04-08: Continued the same compiler-state extraction line for future resume into ordered rule iteration as well. `Compiler.pm` now walks compiled rules through `LinkedSpec::CompilerState::compiled_spec_state_rule_rows(...)` instead of rebuilding the active rule stream locally from `rule_order + rules_by_label`. The owner-routing regression now traps that ordered iteration seam too; keep future compiler-state consumers on that owner-provided view.
- 2026-04-08: Continued the same compiler-state extraction line for future resume into read-side access as well. `Compiler.pm` trace/logging and `Validation.pm` descriptor-state validation now read compiled-state data through `LinkedSpec::CompilerState` accessors instead of peeking raw `{definition_order}`, `{redefined_rule_labels}`, or `{compiled_spec_state}` fields directly. The owner-routing regression now traps those read-side calls too; keep future slices on that same owner boundary.
- 2026-04-08: Continued the same compiler-state extraction line for future resume. `LinkedSpec::CompilerState` now also owns normalization of legacy compiled-spec and compiled-gdata hashes into explicit state records, so `Compiler.pm` no longer keeps a second local compatibility-shape conversion seam beside the extracted owner. The direct owner-routing regression now traps compiled-spec creation, compiled-spec normalization, compiled-gdata normalization, and descriptor-state validation together; resume from that assumption instead of reintroducing local normalization in `Compiler.pm`.
...
- 2026-04-01: Continued the Phase 5 diagnostics line for future resume. `Compiler.pm` now preserves `runtime_ctx->{last_error}{rule_label}` on `compiler_pipeline:spec_descr` failures when the active parsed rule entry is still known, covering both thrown `compile_spec_entry(...)` failures and non-thrown invalid descriptor tuples. If we keep pushing runtime diagnostics next, treat this as the current expectation: compile-time descriptor-build failures can now expose `rule_label`, not only runtime handler/parser failures.
- 2026-04-01: Continued the same Phase 5 diagnostics line for future resume one stage later. `Compiler.pm` now also preserves `runtime_ctx->{last_error}{rule_label}` on default-`spec_gdata(...)` `compiler_pipeline:build_final_descriptor` failures when the active rule’s combined regex dependencies are being built and `_ored_re(...)` dies. If we keep pushing runtime diagnostics next, treat compile-time rule attribution as spanning both `spec_descr` and default `build_final_descriptor` failure paths whenever the active rule is still known.
- 2026-04-02: Continued the Phase 5 diagnostics line for future resume. `Compiler.pm` now preserves `runtime_ctx->{last_error}{rule_label}` and a more specific owning-rule `detail` string when `compiler_pipeline:validate_gdata_references` rejects the generated descriptor by returning false for one compiled rule, using new optional failure metadata exposed by `LinkedSpec::Validation::validate_gdata_references(...)`. If we keep pushing runtime diagnostics next, treat compile-time rule attribution as spanning `spec_descr`, default `build_final_descriptor`, and false-return `validate_gdata_references` when the owning/generated rule is still known.
- 2026-04-02: Continued the same Phase 5 diagnostics line for future resume on the throw path too. `validate_gdata_references(...)` now also reports owning-rule metadata before rethrowing nested `validate_rule_definition(...)` exceptions, so `Compiler.pm` can preserve `runtime_ctx->{last_error}{rule_label}` on trapped `compiler_pipeline:validate_gdata_references` die paths as well. If we keep pushing runtime diagnostics next, treat generated-descriptor validation attribution as covering both false-return and nested rule-local throw failures when the owning compiled rule is still known.
- 2026-04-02: Continued the same Phase 5 generated-descriptor validation line for future resume with a structured-summary follow-up. `Compiler.pm` now reuses validation-provided `summary` text on attributed `compiler_pipeline:validate_gdata_references` failures instead of always collapsing back to the generic `Generated parser validation failed` banner. If we keep pushing runtime diagnostics next, treat that seam as preserving not only `rule_label` and detail but also the more specific validation summary when the failing rule/reference is already known.
- 2026-04-02: Continued the same Phase 5 runtime-owner line for future resume with the missing silent-undef fallback. `Runtime.pm` now treats delegated `run_get_pipeline(...)` silent-undef failures the same way it already treated delegated dies: if no deeper structured payload exists yet, it writes a fallback `runtime_owner:run_get_pipeline` error with detail `run_get_pipeline returned undef without structured runtime context`; if a deeper payload already exists, it preserves that instead. The file-oriented path now benefits automatically too, because `get_parser(...)` preserves that deeper runtime-owner payload instead of collapsing to a generic parser-factory compile fallback. If we keep pushing runtime diagnostics next, this silent-undef seam is now closed on both the direct `Runtime::run_get(...)` path and the shared `get_parser(...)` continuity path.
- 2026-04-02: Locked the adjacent parser-factory compile seam for future resume too. `ParserFactory.pm` already handled delegated `compile_spec(...)` silent-undef behavior correctly, but only the throw path and the public `get_parser(...)` preservation path were directly covered. We now also have direct regression anchors for both parser-factory fallback on silent `undef` without deeper payload and preservation of a deeper runtime-owner payload when `compile_spec(...)` returns `undef` after writing one. If we keep pushing diagnostics next, the remaining value is more likely in a new behavior seam than in another silent-undef rediscovery around parser-factory compile dispatch.
- 2026-04-02: Corrected the same Phase 5 runtime-owner silent-undef seam for future resume after a real false-positive check. `Compiler::run_get_pipeline(...)` intentionally returns `undef` on successful `parse_only` and `generate_only`, so the first fallback pass in `Runtime.pm` was too broad and could stamp `runtime_owner:run_get_pipeline` onto successful mode-only runs. `Runtime.pm` now skips that fallback on intentional `parse_only` / `generate_only` success paths, and direct regression coverage now locks both branches so future resume should treat `runtime_ctx->{last_error}` as failure-only there again.
- 2026-04-05: Continued the same Phase 5 diagnostics-attribution line into the remaining pre-rule compiler validation seams. `Compiler.pm` now falls back to `LinkedSpec::generated_handler:<top_rule>` for `handler_source_label` on `validate_spec_content` and unattributed `validate_dsl_syntax` failures when the caller already selected a top rule but rule ownership is still unknown. Regression coverage now locks that directly plus through public `LinkedSpec::Get(...)` / `get_parser(...)` continuity paths, so future resume should treat pre-rule compiler-validation handler attribution as covered continuity now instead of assuming label-only handler identity starts only after rule attribution begins.
- 2026-04-05: Continued the same Phase 5 diagnostics-attribution line into the later generic compiler seams too. `Compiler.pm` now falls back to `LinkedSpec::generated_handler:<top_rule>` for `handler_source_label` on unattributed `spec_descr`, `build_final_descriptor`, and `validate_gdata_references` failures when the caller already selected a top rule but no rule ownership has been pinned yet. Regression coverage now locks that directly plus through public `LinkedSpec::Get(...)` continuity, so future resume should treat late generic compiler-failure handler attribution as covered continuity now instead of assuming only rule-attributed late compiler stages preserve handler identity.
- 2026-04-06: Locked the same late generic compiler-failure attribution story through the file-oriented path too. `get_parser(...)` now has focused regression coverage proving that after spec resolution/load succeeds, a generic late `build_final_descriptor` failure still preserves resolved `spec_path`, selected `top_rule`, and `LinkedSpec::generated_handler:<top_rule>` in shared `runtime_ctx->{last_error}`. Future resume should treat that file-oriented late-compiler continuity as covered now instead of re-probing it manually.
...
- 2026-04-11: Continued the plugin/resource modernization track for future resume with a no-behavior-change `PluginBridge` owner-dispatch cleanup. `LinkedSpec::PluginBridge` now uses `LinkedSpec::OwnerDispatch` for bridge-local `$@` preservation, lazy `PPlugin` loading, and registered-plugin lookup through `LinkedSpec::PluginRegistry`, and the architecture regression now locks those seams directly. Future work should continue treating `PluginBridge` / `PPlugin` as transition/removal machinery, but should not reintroduce local eval-require or local `$@` restore loops there while the branch still exists.
- 2026-04-11: Continued the same plugin/resource modernization track one step lower in `PPlugin`. `_default_deps()` now uses `_load_linkedspec_parser(...)`, which dispatches to `LinkedSpec::get_parser('pplugin')` through `LinkedSpec::OwnerDispatch` and removes the local `eval { require LinkedSpec }` branch. Regression coverage now locks the preserved laziness contract: `require PPlugin` and default-dep construction still keep both `LinkedSpec.pm` and `LinkedSpec::OwnerDispatch` unloaded until the parser callback is actually invoked.
- 2026-04-11: Continued the legacy `PPlugin` cleanup for future resume by replacing the registry builder's localized `@ARGV`/diamond file slurp with `_read_plugin_file(...)`. `_build_plugin_registry(...)` now reports unreadable `.plg` files as skipped entries before parser invocation, still reports real parser errors for malformed `.plg` files, preserves file order and later duplicate override behavior, and restores caller `$@` after successful partial registry builds. Future work should keep legacy plugin cleanup on explicit IO and explicit dependency seams instead of reintroducing global reader state.
