...
- 2026-04-11: HTTP package-backed helper registrations are being retired, not preserved as permanent plugin names. `plugin/http.plg` still owns the real legacy `http` action and `plugin/lighttpd.plg` still owns the real legacy `lighttpd` action, but their thin helper registrations for `httplink`, `set_http_hostport`, and `set_http_localhost` have been removed. Repo-owned `.plg` callers in `http.plg`, `rtl.plg`, `stan_backend.plg`, and `tree.plg` now call `Plugin::HTTP` directly. Future resume should keep removing package-backed helper registration names once no repo-owned caller needs them.
