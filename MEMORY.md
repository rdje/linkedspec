...
- latest_completed_leaf: `RUST-PARITY.7.3.3.3` (`RUST-PARITY.7.3.3.3 - defer hlink scalar-ref fixtures`; hash = this commit). Ahead of origin remains below push threshold ~300; do NOT push mid-PNT.
- active_work_unit: `RUST-PARITY` — `.7.3.3.3` deferred `[abc]` and mixed `foo[bar]{baz}` hlink fixtures to `.7.3.3.4`: Perl emits scalar refs that `JSON::PP` cannot encode, and Rust currently lacks both a scalar-ref `RuntimeValue` and working execution for the shipped scalar-ref action branch. `.7.3.4` triages `portmap`/`lib_reader`/`ebnf`; `.7.3.5` triages `BNF`/`DT`/`ifelse`/`operators_try`/`spec.spec`; `.7.3.6` audits RTL/plugin/legacy candidates with the hardened timeout guard. Staged linked parsing first prototype is complete; broader public staged surfaces remain future work.
