...
- 2026-03-24: Continued the plugin/runtime modernization track by spending public plugin lookup inside smaller repo-owned plugin files. At that migration step, `qc_summary.plg` used `LinkedSpec::get_plugin('qc_summary_merge')` and `skew.plg` used `LinkedSpec::get_plugin('stan_backend_start')` instead of direct `PPlugin->get(...)`; the newer 2026-04-12 note above records that current `qc_summary` row merging now lives in `QC::Summary`. Future resume should keep migrating repo-owned `.plg` files with known plugin names toward package owners or explicit bridge APIs depending on whether compatibility still needs a dynamic callback.
