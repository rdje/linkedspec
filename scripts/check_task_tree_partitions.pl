...
if (@$records != 9) {
    push @errors, 'task-tree index must contain one metadata record and exactly eight part records';
    finish(\@errors);
}
...
push @errors, 'task-tree metadata contract mismatch'
    ...
    || ($metadata->{part_count} // 0) != 8
    || ($metadata->{semantic_part_count} // 0) != 7
...
push @errors, 'semantic part count mismatch' if $semantic_count != 7;
...
sub finish {
    ...
    print "[task-tree-partitions] PASS: 7 semantic parts, 1 immutable history part, $ids_count stable IDs; "
...
sub run_self_tests {
    my @tests = (
        ...
        ['part_14', sub { part_owns_id('14', "$TREE.14.3") && !part_owns_id('14', "$TREE.15") }],
