...
| `TASK-TREE-METADATA` | structural | `scripts/check_task_tree_metadata.sh` | completed task trees do not advertise live `Current Frontier` rows; pending nodes do not claim false activation; every exact current task ID is unique across partitioned and unpartitioned task files, excluding only index-registered immutable history; partitioned trees preserve exact source coverage, immutable history, current digests, bounded lookup, and collection/member ceilings; checker-owned closed-capability markers remain in the stable task-index section with a complete consumer census and frontier-rewrite mutation proof |
