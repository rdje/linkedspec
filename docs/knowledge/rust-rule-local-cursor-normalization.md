...
every governed diagnostic identity directly. At the normalization milestone, primary-command diagnostic projection and retired global option bytes
belonged to later Rust leaves; those removals and admission are complete in [[rust-rule-local-cursor-execution]].
...
## September 8 runtime normalization consumer reading

Startup .3.3.60 completes the 50-line runtime normalization consumer and adjacent cursor suites.
Its historical default-bare test name/message describes the former staged execution boundary; current
family-derived execution and its exact 8/0/60 neutral governance supersede that chronology. Named/invalid
AND selectors remain outside the admitted numeric exclusions and belong to existing repair .57.
