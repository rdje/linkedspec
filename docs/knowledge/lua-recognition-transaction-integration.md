...
date: 2026-08-11
status: private integration signoff complete on PUC Lua and LuaJIT; ordinary and canonical admission pending
...
reverify: "LINKEDSPEC_LUA_RECOGNITION_TRANSACTION_RED_MODE=integration bash tools/run_lua_project_data.sh puc lua/test/recognition_transaction_contract_test.lua && LINKEDSPEC_LUA_RECOGNITION_TRANSACTION_RED_MODE=integration bash tools/run_lua_project_data.sh luajit lua/test/recognition_transaction_contract_test.lua && bash tools/run_python_project_data.sh tools/check_recognition_transaction_contract.py && bash tools/run_lua_local.sh"
---
...
Explicit integration mode passes 243 assertions on each ABI, but this remains
an internal boundary. Neither private module is exported, and the exact consumer
is absent from ordinary and canonical discovery. Admission `.14.3.6.3` alone may
remove the selector and advance PUC Lua and LuaJIT in the rollout ledger.
