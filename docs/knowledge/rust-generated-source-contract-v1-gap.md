...
title: Rust generated source now satisfies the contract-v1 baseline; admission and breadth remain
answers:
...
`parse_with_trace` keep the accumulator envelope. The only baseline step left
is explicit `.3.1.3.3` admission. Rust remains partial afterward for generated
breadth until `.3.2` expands the proof from eight fixtures to all 105.
