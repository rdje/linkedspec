...
Exact `array(name)` and `hash(name)` remain parsed only until Lua supports bare replacements and tracked sources
migrate. Julia rejects and deletes those selector paths in `FUTURE-PARITY-BACKLOG.12.1.8.4`.
