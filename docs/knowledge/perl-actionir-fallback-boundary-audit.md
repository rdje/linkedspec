...
discard semantics in `.4.2.3`. After `SPEC-FORMAT-TERSE.4.2.2`, an exact-arity registered function call such as
`return(normalize(" x "))` executes in value position with zero raw fallback/unresolved helpers; wrong-arity
registered calls remain unresolved-helper diagnostics.
