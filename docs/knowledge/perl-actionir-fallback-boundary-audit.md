...
resolution and discard semantics. After `SPEC-FORMAT-TERSE.4.2.1`, even a registered function call such as
`return(normalize(" x "))` is still an unresolved helper in value position; the registry exists in descriptors,
but the evaluator is `.4.2.2`.
...
bridge without changing the bootstrap parser.
