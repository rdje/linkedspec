...
title: "SPEC-FORMAT-TERSE.1.3 ground truth — mutation surface is not one change. Scalar function form `set(name,val)` is already satisfied by .1.4; explicit array append currently works as `push_value(name,val)`, while requested `push(name,val)` collides with existing child-call `push(rule[,target])`; `set_key(name,k,v)` is a pure hash value expression in Perl paths, not a standalone mutation statement, and Rust only updates if arg0 is already a hash; operator forms `name = val`, `items += val`, `name[\"k\"] = val` pass through as raw/invalid Perl and Rust has no assignment statement AST."
answers:
...
evidence: "TOOLBOX probes 2026-06-29 (`perl -Iperl -MLinkedSpec`, `call_spec_handler_subst`, minimal `LinkedSpec::Get` runtime specs). Perl lowering: `set(name,\"ok\")` and `assign(name,\"ok\")` both -> `$name = \"ok\"`; `push_value(items,\"a\")` -> `push @items, \"a\"`; `push(items,\"a\")` passes through as raw `push(items, \"a\")`; `set_key(name,\"k\",\"v\")` alone passes through statement lowering; `assign(hash(name), set_key(hash(name),\"k\",\"v\"))` lowers to `%name = (...)`; operators `name = \"ok\"`, `items += \"a\"`, `name[\"k\"] = \"v\"` pass through unchanged. Runtime: bare `set` returns `\"ok\"`; bare `push_value` returns `[\"a\"]`; `push(items,\"a\")` fails handler compile (`Type of arg 1 to push must be array`); operator forms fail as invalid/raw Perl; `return(set_key(name,\"k\",\"v\"))` returns `{k=>v}` as a pure value, while wrapped assignment returns the same as mutation. Rust code-read: `rust/linkedspec-core/src/expr.rs` grammar is `stmt -> expr`, `expr -> primary ('.' method_call)*`, with calls/variables/indexed vars/literals/fluent chains only; no Assign/PlusEq statement node. Runtime `Engine::call_helper()` has `\"push_value\" | \"push\"` alias and `set_key` only changes when arg0 is `RuntimeValue::Hash`."
reverify: "perl -Iperl -MLinkedSpec -e 'for my $expr (q{set(name,\"ok\")},q{assign(name,\"ok\")},q{push_value(items,\"a\")},q{push(items,\"a\")},q{set_key(name,\"k\",\"v\")},q{name = \"ok\"},q{items += \"a\"},q{name[\"k\"] = \"v\"}) { my $out=LinkedSpec::call_spec_handler_subst(\"Top\",$expr); $out =~ s/\\n/\\\\n/g; print \"$expr => $out\\n\" }' && rg -n 'stmt +→ expr|enum Expr|\"push_value\" \\| \"push\"|\"set_key\"' rust/linkedspec-core/src/expr.rs rust/linkedspec-runtime/src/engine.rs"
---
...
- **Scalar function form is already satisfied.** `set(name, value)` is the terse rename of `assign(name, value)`.
  ...
  `"ok"`.
- **Explicit array append currently works as `push_value(name, value)`, not `push(name, value)`.**
  `push_value(items,"a")` lowers to `push @items, "a"` and runs. `push(items,"a")` is currently a child-call
  spelling family in Perl (`push(Rule)`, `push(Rule,target)`, indexed variants), so it passes through as raw
  Perl and fails handler compilation in the explicit-value shape. Rust already treats `"push"` as a
  `push_value` alias, which is ahead of the Perl reference for this exact shape and must not drive the contract
  before the `push(rule,target)` ambiguity is resolved.
...
- `.1.3.2` resolves the `push(name,value)` vs `push(rule,target)` collision before implementation.
