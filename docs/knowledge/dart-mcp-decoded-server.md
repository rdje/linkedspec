...
`index.capabilities.toJson()` and `index.queryNeutral(request).toJson()`; it cannot load or compile source, execute
...
ledger status deliberately remains 2/5 implementations and 2/6 runtimes: `.10.9.4.2` still owns strict raw stdio,
and `.10.9.4.3` alone may admit Dart at 3/5 plus 3/6.
