...
date: 2026-08-25
status: private authority current and dormant on both ABIs; dedicated node/carriers next
...
reverify: "bash tools/run_lua_project_data.sh puc lua/test_dormant/progressive_span_dispatch_authority_test.lua; bash tools/run_lua_project_data.sh luajit lua/test_dormant/progressive_span_dispatch_authority_test.lua; bash tools/run_lua_project_data.sh puc lua/test_dormant/progressive_span_dispatch_contract_test.lua; bash tools/run_python_project_data.sh tools/check_progressive_span_dispatch_contract.py; test \"$(rg -c 'progressive_span_dispatch_authority_test[.]lua' tools/run_lua_local.sh tools/run_ci_local.sh)\" = 0"
---
...
Lua-5.1-compatible, unexported, ActionIR-independent, and absent from every
execution carrier.
...
This core does not recognize or execute the authored `dispatch_span` form.
Dedicated node and carrier integration remain exclusively owned by
`FUTURE-PARITY-BACKLOG.14.6.6.2`.

Related facts: [[lua-progressive-span-dispatch-dormant-red]],
