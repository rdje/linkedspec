...
title: Condition truthiness still differs after Perl and Rust adopt typed truth
answers:
...
reverify: "prove -Iperl t/logical_helper_perl_contract.t && cargo test --manifest-path rust/Cargo.toml -p linkedspec-runtime --test logical_helper_contract && rg -n 'as_bool|bool _truthy|function _runtime_truthy|local function runtime_truthy' rust/linkedspec-core/src/types.rs dart/lib/src/runtime/interpreter.dart julia/src/runtime/Interpreter.jl lua/src/linkedspec/interpreter.lua"
---
...
values are true. Perl and Rust now consume the ADR `0043` target, while Dart and Julia already match its
representable truth rows and Lua retains the pre-normalization scalar-`"0"`/empty-aggregate profile.
`FUTURE-PARITY-BACKLOG.5.2.4-.6` own the remaining backend rollout across lazy controls plus eager
...
and one condition/helper truth seam. Rust `.5.2.3` applies the same policy through its typed runtime seam. The
remaining table difference and helper evaluation/arity differences are explicitly owned by `.5.2.4-.6`.
