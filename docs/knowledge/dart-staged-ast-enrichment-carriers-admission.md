...
date: 2026-08-27
status: current private Dart carrier admission; Julia current-depth authority is implemented dormant and FUTURE-PARITY-BACKLOG.14.7.6.3 is next
...
This is private backend behavior, not public `parse_job(...)` authoring. Dart rollout is complete. Julia now has
its private marker and typed provenance while one authority/policy sentinel remains RED; Lua, six-runtime
recurrence, public authoring, independent recomposition, and combined no-drift retain their existing owners.
