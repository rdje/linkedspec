...
This 65-case manifest is not yet an all-variant parity claim. Rust retains marker-only defaults, while Dart,
Julia, and Lua still reject markerless source before reaching their existing fallback helpers. Their ordered leaves
must converge to these same shared bytes before final five-backend admission.
