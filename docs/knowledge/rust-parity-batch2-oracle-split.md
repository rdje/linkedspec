...
mixed hlink fixtures because they required a neutral scalar-ref/action-payload
contract or a hlink spec migration; `SPEC-SOURCE-TERSE-CLOSEOUT.1` later supplied
that migration by making bracket payloads neutral strings. `RUST-PARITY.7.3.6` then added seven
...
- `.7.3.3.3`: done/superseded — scalar-ref representation decision deferred bracket/mixed
  fixtures with Perl JSON failure and Rust action-branch evidence; `SPEC-SOURCE-TERSE-CLOSEOUT.1`
  later resolved the hlink path by changing bracket payloads to neutral strings.
- `.7.3.3.4`: superseded — no scalar-ref contract is needed for the hlink bracket/mixed
  fixtures now active in the corpus.
