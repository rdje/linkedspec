...
timeout with per-case fork/SIGKILL instead of `alarm()`. The current frontier is
`RUST-PARITY.7.3.3`: try remaining `hlink_substitution` delimiter/link-path
fixtures with the hardened oracle guard in place.
