...
date: 2026-07-29
status: architecture, decoded implementation, and strict wire complete; admission and closeout pending under FUTURE-PARITY-BACKLOG.10.9.6.3-.4
...
`register_index`, `revoke_handle`, `dispatch`, `serve_stdio`, and `shutdown`. It accepts only an existing
...
Implementation order is generated/runtime/decoded/native-system `.1` (complete), strict stdio `.2` (complete), identical dual-ABI
