...
and generated Perl/Rust/Dart/Julia/Lua modules, validates schema/counts/order/scalar values/logical digest, and executes the fixtures
...
## September 6 generated-data reconciliation

`SESSION-STARTUP-READING.3.2.49` reconciles the first 1,500 Perl table lines already fully consumed
under `.31`, with exact 32,073-byte baseline identity. ADR `0027`, the complete 287-line checker,
and the complete 88-line Perl consumer were read for this checkpoint. The current checker regenerates
and byte-compares the neutral JSON plus all five backend modules, extending the original Perl/Rust-only
wording. Its managed temporary directory stays on the repository filesystem and is context-cleaned.

Current focused invocation `bash tools/run_python_project_data.sh tools/check_unicode_case_contract.py`
passes: Unicode 17.0.0, 1,563 lower / 1,581 upper mappings, 158 / 464 property ranges, 12 fixtures.
This proves generated byte equality and the independent neutral fixture evaluator, not fresh execution
of every backend. The separate managed Perl consumer passes 52 tests in 13 seconds. No upstream data,
generator, generated file, Unicode version, normalization rule, or locale policy changed.
