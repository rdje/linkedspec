...
remain disjoint from the legacy shared 239-name set. This is deliberate rollout staging: Julia and Lua must first
consume the unchanged contract, then `.17.5` admits all seven into the shared inventory and hardens the checker
against another symmetric omission.
