...
title: "SPEC-FORMAT-TERSE.1.2 historical ground truth — Perl bare working-variable inference was split by channel; later leaves closed current bare reads and retired old helper spellings."
answers:
...
evidence_update_2026_07_06: "Later terse leaves closed the original gaps: current Perl lowers `return(count)`, `count = entry_group(0)`, `items += value`, and `meta[key] = value` as scoped working-variable reads/writes with auto-declarations as needed. SPEC-FORMAT-TERSE.8.3 also retired old successful Perl helper spellings such as `push_value(...)`, `array_copy(...)`, and `declare(...)`; use current `push(...)`, `copy(...)`, bare reads, and assignments when reverifying current behavior."
reverify: "perl -Iperl -MLinkedSpec -e 'for my $stmt (q{return(count)}, q{count = entry_group(0)}, q{items += value}, q{meta[key] = value}) { my $out=LinkedSpec::call_spec_handler_subst(q{Top}, $stmt); $out =~ s/\\n/\\\\n/g; print \"$stmt => $out\\n\" }' && rg -n 'Channel 2 value-position reads are now closed|RHS-shape/type-inference work was split' docs/knowledge/terse-bare-working-vars-engine-gaps.md"
---
