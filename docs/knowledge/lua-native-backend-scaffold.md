...
runs 169 current tests on PUC Lua and the same 169 on LuaJIT, process-smokes the exact primary adapter, validates
the exact 105-fixture corpus, and executes it at 105/105 through the developer runner. It writes no cache or global
module state.
...
`runtime-corpus-full` status; recurring primary process/matrix admission remains `.7.2`.
...
See also [[lua-primary-cli-adapter]].
