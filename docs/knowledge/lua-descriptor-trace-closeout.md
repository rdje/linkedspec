...
This is deliberately not capability-census admission. ADR `0041` defines the manifest as an all-pass backend
surface, so Lua remains outside its 16 rows while native, corpus, primary CLI, generated-source, and generated-
subset work is incomplete. The census therefore stays at four-backend 64/0/0; `.8.4` alone may add Lua rows after
