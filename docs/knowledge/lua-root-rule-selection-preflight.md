...
date: 2026-07-18
status: behavior-free preflight complete; core `.9.1.1.2.5.1` next after clean `.5.0` commit
...
Lua does not need a new ordering model. Its authored rule order, `is_top` markers, default helper, normalized
state, and v1 generated plan already preserve enough information for ADR 0046. The blocking boundary is that
validation still requires a marker and execution has no one typed owner for selection identity and failures.
