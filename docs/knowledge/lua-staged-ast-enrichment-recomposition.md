...
date: 2026-08-28
status: current shared Lua parent closeout; six-runtime recurring proof next
...
All five private backend sources and six runtime routes remain admitted at 106 neutral mutations. The Lua source
runs exactly once per ABI in ordinary and canonical proof; native, reconstructed, generated-plan, and emitted
results remain equal and authority-free outside their host invocation. The next leaf `.14.7.8` owns one recurring
five-source/six-route proof and the typed staged row. Public `parse_job(...)` authoring remains `.14.7.9`-owned.
