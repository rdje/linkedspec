...
date: 2026-08-26
status: current private Perl admission; later backends, recurrence, and public authoring pending
...
The exact consumer is admitted once through ordinary phase-0 and once through canonical CI. Only the Perl backend
and rollout row are complete. This is private implementation evidence, not public `parse_job(...)` authoring;
public closeout remains `.14.7.9`, and Rust dormant RED `.14.7.4.0` is the next backend leaf.
