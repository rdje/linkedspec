...
Perl and Rust implement the alias surface. Dart, Julia, and Lua remain backend-owned work, so the target contract
must not be described as fully implemented until `FUTURE-PARITY-BACKLOG.16.4-.16.7` close. Rust's existing
`.contains()` missing-argument fallback is helper drift owned by `.5`; `.contains` still behaves exactly like its
parenthesized Rust twin.
...
- Tree: `docs/tasks/FUTURE-PARITY-BACKLOG.md`, design `.16.0`, contract `.16.1`, Perl `.16.2`, and Rust `.16.3`
