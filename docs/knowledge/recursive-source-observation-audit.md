...
date: 2026-08-12
status: accepted audit; numeric lineage corrected, neutral executable, and Perl Rust Dart privately admitted
...
mutation, bringing governance to 71. Rust `.14.4.3` and Dart `.14.4.4` then add one omission regression each,
bringing governance to 73 while the parent row stays pending with Perl, Rust, and Dart recorded. Julia, shared Lua,
facades, schemas, semantic/MCP, CLI, README, and public behavior remain owned by `.14.4.5-.8`.
