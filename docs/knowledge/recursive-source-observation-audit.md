...
date: 2026-08-12
status: accepted audit; numeric lineage corrected, neutral executable, and Perl Rust Dart Julia privately admitted
...
mutation, bringing governance to 71. Rust `.14.4.3`, Dart `.14.4.4`, and Julia `.14.4.5` then add one omission
regression each, bringing governance to 74 while the parent row stays pending with Perl, Rust, Dart, and Julia
recorded. Shared Lua, facades, schemas, semantic/MCP, CLI, README, and public behavior remain owned by `.14.4.6-.8`.
