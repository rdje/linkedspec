...
- At split time, Rust had no bracket/brace primary expression for shape values; current Rust shape-literal value
  parity is recorded in [[terse-rust-shape-literal-value-parity]], and current Rust target-kind parity is
...
4. `.1.2.3.5.4`: Rust parity for the accepted target-kind inference contract (landed).
