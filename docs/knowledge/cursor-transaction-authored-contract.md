...
answers:
  ...
  - "how do I write a current Perl bounded recognition transaction"
  - "how do I write a current Rust bounded recognition transaction"
...
date: 2026-08-10
status: accepted authored/static contract; current on Perl and Rust; other runtime support pending
...
The accepted authored shape, current on Perl and Rust and future on the other runtimes, is:
...
Every entered transaction rule on Perl or Rust owns a fresh invocation-frame mark table. Recursive same-label frames do
not alias one rule-label bucket; child exit cannot replace its parent's marks. Transaction rollback restores only
the owning frame snapshot, and frame exit invalidates its mark generation. Other runtimes retain their prior
behavior until their own transaction admissions.
...
These spellings and rules are current executable syntax on the independently admitted Perl and Rust backends. The
neutral artifact/checker remains the shared authority under `.14.3.1.1`; Dart, Julia, PUC Lua, and LuaJIT follow
independently, so two backend admissions still do not imply complete cross-runtime portability.
