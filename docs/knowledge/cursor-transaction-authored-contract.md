...
date: 2026-08-10
status: accepted authored/static contract; current on Perl; other runtime support pending
...
The accepted authored shape, current on Perl and future on the other runtimes, is:
...
Every entered transaction rule on Perl owns a fresh invocation-frame mark table. Recursive same-label frames do
...
These spellings and rules are current executable syntax on the admitted Perl reference. The neutral
artifact/checker remains the shared authority under `.14.3.1.1`; Rust, Dart, Julia, PUC Lua, and LuaJIT follow
independently, so Perl admission does not imply portable cross-runtime support.
