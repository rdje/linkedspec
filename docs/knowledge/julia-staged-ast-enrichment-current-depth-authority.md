...
date: 2026-08-27
status: current private one-depth API; dormant and not publicly admitted
...
Returned markers deliberately remain inert. Leaf `.14.7.6.3` owns the separate
breadth-first recursive API, active-chain cycle/decrease rules, shared
cancellation/deadline/work/call/depth/result/diagnostic authority, callback safe
points and expiry, and original-source diagnostic rebasing. Leaf `.4` retains
fresh native/reconstructed/generated/emitted carrier authority and admission.
