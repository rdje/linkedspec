...
date: 2026-08-09
status: accepted descriptor contract; migrations pending
...
The live file is still direct required-marker authority for callable, logical-helper, root-selection,
duplicate-slot, repeated-action, rule-local-cursor, and capability-exclusion closeouts. The future task file is
...
line/byte/SHA-256 evidence, immutable archive segments, and reconstruction queries. Live status becomes an
overwrite-oriented five-section snapshot. Changes and notes become 512-line/64-KiB hot shards over ordered
