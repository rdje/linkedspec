...
date: 2026-08-09
status: accepted descriptor contract; live-status and future-task migrations implemented, remaining migrations pending
...
unchanged recomposition/registry closure. The first two stores are implemented. Every move is atomic from a clean
Git boundary, accounts for the old source byte-for-byte, and rolls back only by the activation commit before
landing or an explicit revert after.
