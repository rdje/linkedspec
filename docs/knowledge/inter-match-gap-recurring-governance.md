...
title: Inter-match gap recurrence executes neutral plus private Perl, Rust, Dart, and Julia while two runtime routes remain pending
answers:
...
date: 2026-08-16
status: current governance; rollout is 5 complete + 4 pending with private Perl, Rust, Dart, and Julia admitted
tags: [capture, segmentation, recurring-gate, project-data, no-overclaim, local-ci, rollout]
evidence: "INTER-MATCH-GAP-CAPTURE.1.2 adds one repository-routed driver, canonical always-on neutral checking plus opt-in ordered governance, five topology/storage/public mutations, and exact outward-surface guards. Perl .2.4, Rust .3.5, Dart .4.5, Julia .5.5, and shared Lua .6.5 admit their complete consumers ordinarily, canonically, and once in rooted order. Current proof is 60 rejected semantic mutations plus ten Rust, ten Dart, ten Julia, and sixteen Lua admission mutations, 7 complete + 2 pending, exact neutral-plus-six-private-runtime execution, repository-volume storage, and unchanged outward guards."
...
the admitted Perl, Rust, Dart, and Julia consumers once each and reports—but does not invoke—the exact pending PUC
Lua and LuaJIT consumer routes.

Perl's full 124-test final-path consumer now passes authored/static, native-live, emitted/loaded, lifecycle,
rollback/recursion, and typed-diagnostic roles. `.2.4` registers it once in ordinary canonical execution and once
in this rooted route. Rust's exact nine-role consumer adds native/reconstructed/descriptor/generated/emitted,
lifecycle, recursion/rollback, diagnostic, and primary-command proof; `.3.5` registers it once on both routes.
Perl, Rust, Dart, and Julia regressions are independently rejected. Ten Rust, ten Dart, and ten Julia admission
mutations lock their consumer/role/registration boundaries. All six private runtime routes are admitted and the
facade remains guarded; Lua `.6.0` changes no driver or rollout state while freezing `.6.1-.6.5`.
...
neutral plus private Perl, Rust, Dart, and Julia support, route order, and repository-volume storage; it does not
claim public support. `.7` alone can promote
`recurring` and `public_no_drift` after all six runtime proofs exist.
...
set is 60 semantic mutations plus ten Rust, ten Dart, ten Julia, and sixteen Lua admission mutations, with rollout
7 complete + 2 pending.
