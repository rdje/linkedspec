...
title: Inter-match gap recurrence executes neutral plus private Perl, Rust, and Dart while three runtime routes remain pending
answers:
...
the admitted Perl, Rust, and Dart consumers once each and reports—but does not invoke—the exact pending Julia,
...
guarded; Julia `.5.0` freezes its implementation plan without changing the driver.
...
neutral plus private Perl, Rust, and Dart support, route order, and repository-volume storage; it does not claim
later backends or public support. Remaining runtime admissions are owned by `INTER-MATCH-GAP-CAPTURE.5-.6`; `.7` alone can promote
...
set is 57 semantic mutations plus ten Rust and ten Dart admission mutations, with rollout 4 complete + 5 pending.
...
Julia planning: [[inter-match-gap-julia-implementation-plan]].
