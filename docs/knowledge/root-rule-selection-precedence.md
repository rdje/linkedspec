...
admission consumer. Julia core/native/primary execution now implements the same precedence and portable failures;
its composed routes/admission remain pending, while Lua still makes no-marker fallback unreachable in validation.
Follow task subtree `.9.1.1.2` for the 4/7 rollout state; do not report markerless selection as uniform across all
backends until Julia routes/admission, Lua/LuaJIT, and final no-drift close.
