...
date: 2026-07-18
status: neutral contract plus Perl and Rust admitted; Dart routes implemented; rollout 3 complete / 4 pending
...
and Rust backends are complete. Dart now implements the same rule across core, loaded/reconstructed,
generated/emitted, traced, diagnostic, descriptor, and primary routes, but its omission-sensitive topology
admission remains `.3.3`; Julia and Lua still make their no-marker fallbacks unreachable in validation. Follow
task subtree `.9.1.1.2` for the 3/7 rollout state; do not report markerless selection as uniform across all
backends until the remaining four legs close.
