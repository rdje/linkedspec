...
title: Dart backend scaffold package lives under dart/ and has manifest IO plus AST data types
answers:
...
date: 2026-07-09
status: current
tags: [dart, backends, package, scaffold, corpus, ast, tests]
evidence: "DART-BACKEND-PARITY.1.2 creates the Dart package scaffold; DART-BACKEND-PARITY.1.3 adds corpus manifest IO; DART-BACKEND-PARITY.2.1 adds dart/lib/src/ast/spec_ast.dart and JSON round-trip tests without parser/runtime execution."
...
`dart/lib/src/ast/spec_ast.dart` defines data-only AST/staged-parse-job types that round-trip through JSON.
This is still scaffold only. It does not implement `.spec` parsing, compiled-spec state, runtime semantics,
or corpus output comparison yet. `DART-BACKEND-PARITY.2.2` starts parser implementation next.
