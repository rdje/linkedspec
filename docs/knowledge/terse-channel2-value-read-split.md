...
The Perl direct-access scalar child now treats:

```text
foo["a"][z]
```

as the same scalar array-index read as `foo["a"][scalar(z)]`. The next leaf is Rust lockstep/conformance
(`.1.2.3.4`), not a bundled RHS-shape change. See [[terse-scalar-bare-read-seams]],
[[terse-mutation-slot-bare-reads]], and [[terse-direct-access-bare-path-atoms]] for the scalar-read split
details.
