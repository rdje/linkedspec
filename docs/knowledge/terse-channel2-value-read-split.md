...
- `.1.2.3.3`: Perl scalar bare value reads. **Split 2026-06-29 by lowering seam.**
  - `.1.2.3.3.1`: return/assignment scalar source slots. **Current frontier after `.1.2.3.3`.**
  - `.1.2.3.3.2`: mutation key/RHS scalar slots.
...
requires Perl-reference child leaves and then a Rust lockstep/conformance leaf, not a bundled aggregate-read
change. See [[terse-scalar-bare-read-seams]] for the scalar-read split details.
