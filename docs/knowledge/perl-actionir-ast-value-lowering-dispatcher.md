...
- block values; at `.3.1`, statement side effects still used the compatibility path. The later `.4.3`
  owner traverses typed block/statement fields and retains compatibility fallback where required.
...
September 6 startup reading rechecked the milestone chronology against
[[perl-actionir-ast-statement-operator-lowering]], [[perl-actionir-ast-statement-call-lowering]], and
[[perl-actionir-ast-block-value-lowering]]. Their completed `.4.1`–`.4.3` work owns the later statement paths;
the original value-dispatcher milestone does not describe an outstanding statement migration.
