...
date: 2026-08-26
status: current private dormant declaration carrier; caller-frozen authority remains FUTURE-PARITY-BACKLOG.14.7.4.2-owned
...
match object, scheduler, cache, path, cancellation, deadline, budget, mutable queue, or host handle. Marker data is
not yet resolved or executed.
...
`linkedspec_staged_ast_enrichment_red`, absent from ordinary and canonical discovery. `.14.7.4.2` alone owns the
caller-frozen registry/cache and result/failure policies; `.3` owns recurrence/bounds/rebasing and `.4` owns fresh
carrier authority, admission, and Rust rollout.

Related: [[rust-staged-ast-enrichment-dormant-red]], [[general-staged-ast-current-boundary]],
[[general-staged-ast-enrichment-neutral-contract]], [[typed-source-location-cursor-algebra-direction]], and ADR
`0088`.
