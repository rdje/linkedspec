...
`NONCURRENT-HELPER-CODE-PURGE.5` closed the final no-drift audit: the remaining
Rust runtime unit-test fixture that used retired helper examples for generic
fallback coverage now uses invented unknown helper names, exact retired-helper
call-shape / label-tag / `?concat:` scans are clean, and the tree is complete.
