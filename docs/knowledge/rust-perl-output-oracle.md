...
evidence_update_2026_07_22_self_hosted_freshness: "FUTURE-PARITY-BACKLOG.10.5.0.1.2.0 runs one authoritative ORACLE_TIMEOUT=30 regeneration over all 105 fixtures after the self-hosted Unicode-label grammar change. Only the four spec_spec_* input.spec snapshots change, each to exact canonical specs/spec.spec bytes at SHA-256 43cddeaea03cfaddce941ca87f66185de1abf81e281e86c29156fbad16f6d2ce; no expected.json changes survive. The generator exposed and the same leaf repairs a Perl sparse-AND consume regression before accepting output. Rust, Dart, Julia, and Lua complete corpus routes remain 105/105."
evidence_update_2026_09_01_root_dispatch_and_projection_repair: "FUTURE-PARITY-BACKLOG.19.3.3 resolves the pre-existing capability_position_helper_surface regeneration split at the Perl typed-projection owner: absent local-match length/start/end offsets remain null and display lines/columns remain one-based 1, while a real zero-width match at offset zero stays present. The governed live and independently emitted Perl routes reproduce the committed rich record without diagnostics. The generator then removes 67 inert inline Top `/x/` rows and the source-backed control-marker row; precisely 68 derived input.spec files change while all 105 expected.json/input.txt values and manifest bytes stay fixed. Two independent complete ORACLE_TIMEOUT=30 passes have the same corpus diff digest `4df3a7eec18215dc05d823089abc2635fbd9f7df3827fc4fe5cfcafb8eba7e21`. Rust, Dart, Julia, PUC Lua, and LuaJIT each execute all 105 cases with unchanged outputs. A seven-assertion neutral guard rejects fixture regression."
evidence_update_2026_09_08_consumer_reading: "Startup .3.3.50 reads the complete 234-line / 8,478-byte corpus runner. It validates manifest format/count/unique safe names, exact fixture-directory sets and wrapped compatibility results in manifest order. September inventory remains 105 cases; this source checkpoint does not regenerate or execute them. The current Cargo reverify uses repository-managed storage."
reverify: "perl -c -Iperl tools/gen_oracle_corpus.pl; ORACLE_TIMEOUT=0 perl -Iperl tools/gen_oracle_corpus.pl 2>&1 | grep 'hard kill during parser build/parse'; ORACLE_TIMEOUT=30 perl -Iperl tools/gen_oracle_corpus.pl; rg -n '\"case_count\" : 105|capability_capture_named_surface' rust/linkedspec-runtime/tests/corpus/manifest.json; bash tools/run_cargo_local.sh test --manifest-path rust/Cargo.toml --locked --offline -p linkedspec-runtime oracle_corpus_matches_perl_reference"
---
...
loading, and operational failures passed. `.1.5.2.2` now supplies a reusable native direct-top-value execution
operation; the CLI consumes that operation rather than locally unwrapping arbitrary JSON arrays. `.1.5.2.4` has
since closed the primary command at 61/61 in default/POSIX environments.
...
cases, later terse-language leaves raised the checked-in manifest to 97 fixtures, and
`SPEC-SOURCE-TERSE-CLOSEOUT.1` raised it to 99 fixtures with the neutral hlink bracket
and mixed bracket/brace delimiter cases.
