...
title: Logical-helper audit found three truthiness profiles; Perl, Rust, and Dart now consume the target
answers:
...
Helper evaluation and arity remain divergent on Julia and Lua until their rollout leaves land.
...
Dart `.5.2.4` now replaces its audited short-circuit loop and legacy arities with pre-effect validation plus eager
values; normalized, generated-plan, compiled emitted, and primary projections agree. Julia and Lua generated
execution reproduces each backend's remaining native drift, so emission is not its source.
`FUTURE-PARITY-BACKLOG.5.2.1` ratified ADR `0043`; Perl `.5.2.2`, Rust `.5.2.3`, and Dart `.5.2.4` consume it.
Dependency-ordered Julia/Lua, generated/primary, recurring-gate, and public no-drift leaves `.5.2.5-.9` remain.
