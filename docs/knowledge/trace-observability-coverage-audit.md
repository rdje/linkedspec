...
The remaining observability gap is cross-variant trace parity proof. `.4.2` through `.4.4` have since added Rust
trace controls, compile/spec-parser/staged-dispatch events, and interpreted/generated-plan runtime branch/mark/capture
events. `TRACE-OBSERVABILITY.4.5` must still prove the mdBook-documented external trace contract across variants
before Rust can claim trace parity.
