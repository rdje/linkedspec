...
evidence: "JULIA-BACKEND-PARITY.7.1 documents native usage and 99/99; .7.2 defers generated source. .7.3.2.3 documents executable native primary requests and recursive canonical JSON. Current suite is 942; .7.3.2.4 owns final error/trace normalization."
...
The accepted interpreter-first boundary is 99/99 exact corpus outputs, 942 package assertions, and package/CLI
...
loading, and `.7.3.2.3` closes native execution/direct canonical JSON. `.7.3.2.4` remains active for normalized
errors/exits/trace routing. `JuliaFormatter`
