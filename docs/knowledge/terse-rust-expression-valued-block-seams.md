...
top-level-fat-arrow `{ key => value }` remain `Expr::HashLiteral`, while non-empty brace payloads without a
top-level `=>` become block values.
...
- parse non-empty brace payloads without top-level `=>` as `Expr::BlockValue`;
