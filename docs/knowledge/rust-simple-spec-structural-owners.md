...
also closed. `RUST-PARITY.8.1` has since split the source-emitter lane, and
`RUST-PARITY.8.2` landed the minimal generated-source scaffold and compile/run
harness. The current RUST-PARITY frontier is `.8.3`, non-repetition
