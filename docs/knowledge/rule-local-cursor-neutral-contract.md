...
answers:
  ...
  - "why is the rule local cursor migration inventory 75 files"
...
date: 2026-07-19
status: accepted neutral contract; all backends plus recurring gate admitted at 7 complete / 1 pending
...
Duplicate-slot public governance then adds its contract and checker under cursor public owner `.9.1.9`, producing
the exact current inventory of 75 files without changing cursor rollout or mutation status.
...
`lua_dual_abi`, and `recurring_five_backend_gate` are admitted in the composed rollout ledger.
...
registration. One recurring driver composes all six runtime legs, selected primary cases, and support ledgers;
seven additional mutations raise the total to 56. Public no-drift follows after the separately owned identical-
dependency-regex decision. Generated-source v1 remains the historical unmigrated-backend baseline.
