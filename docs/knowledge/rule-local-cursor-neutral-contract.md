...
the exact current inventory of 75 files. Public no-drift then admits that surface without changing inventory.
...
seven additional mutations raised the total to 56. Public contract coverage and admission add four rejected
mutations, closing the ledger at 60. Generated-source v1 remains the historical unmigrated-backend baseline.
...
See also [[rule-local-cursor-public-no-drift]].
