...
evidence: "Director directive on 2026-07-09: each LinkedSpec backend variant should have a different CLI. DART-BACKEND-PARITY.7.3 records the directive, and DART-BACKEND-PARITY.7.4 productizes the Dart-specific CLI under dart/bin/linkedspec_dart.dart. FUTURE-PARITY-BACKLOG.1.2 records the same obligation for Julia by creating JULIA-BACKEND-PARITY, whose .1.1 preflight must define the Julia-specific CLI entrypoint. Lua planning must include equivalent ownership when activated."
reverify: "rg -n 'variant-specific CLI|distinct LinkedSpec CLI|DART-BACKEND-PARITY\\.7\\.4|JULIA-BACKEND-PARITY\\.1\\.1|Julia-specific CLI|per-variant CLI|bin/linkedspec_dart\\.dart|corpus --corpus' docs/tasks/DART-BACKEND-PARITY.md docs/tasks/FUTURE-PARITY-BACKLOG.md docs/tasks/JULIA-BACKEND-PARITY.md docs/TASK_TREE.md docs/linkedspec-book/src/appendix/backend-handoff.md docs/linkedspec-book/src/overview/project-status.md dart/README.md"
---
...
`linkedspec` command as the only user-facing entrypoint for every variant.
