...
- `{}` and top-level-pair `{ key : value }` remain hash literals.
- A non-empty brace payload without a top-level hash-pair delimiter is an expression-valued block.
