...
compiled state. Generated-v2 direct/traced plus freshly persisted emitted direct/traced execution pass the
invocation-local `top_rule` to the ordinary runtime. `resolve_entry_rule` remains the only semantic decision owner.

Generated source is contract v2/format 2. Its plan retains only ordered `{label, family}` rows: authored
