...
title: Current call coverage independently checks 126 shared generic Perl contracts against 250 backend names
answers:
  ...
  - why do dedicated parse_job comments not enter the generic helper inventory
date: 2026-07-15
status: current
...
evidence_update_2026_08_28_dedicated_forms: "Progressive recurrence makes dispatch_span a private cross-backend dedicated intrinsic while keeping it outside the ordinary helper inventory. FUTURE-PARITY-BACKLOG.14.7.9 makes parse_job a public dedicated assignment annotation, also outside that inventory. The checker now names the exact sixteen-form set non-generic and subtracts it from raw corpus reverse scanning; this prevents the required canonical spec.spec comment mirrors from being mistaken for ordinary parse_job calls. A separate staged mutation deletes the subtraction guard. Shared inventory and independently covered generic counts remain 250/126."
...
2. Every shared name appears in the mdBook and in either the 105-case corpus or the exact complete named-mark
   fixture.
3. The Perl lowering contracts independently produce 126 shared generic identifier-shaped names, all of which
   must occur in the backend inventories.
...
The independent shared-generic Perl set currently excludes exactly sixteen identifier-shaped diagnostics.
...
operations plus `observe_recognition`, `recognition_checkpoint`, `recognize_once`, `recognition_commit`, and
`recognition_rollback` to remain present in Perl and absent from backend generic helper inventories. The
recognition forms are grammar-owned intrinsics with dedicated ActionIR nodes, not ordinary calls; disappearance
or accidental helper admission fails loudly.

The fifteenth classified form is private cross-backend `dispatch_span`, represented by a dedicated ActionIR node.
The sixteenth is public `parse_job`, represented by a dedicated exact-assignment annotation rather than an
ordinary helper. Both remain outside the shared generic inventory even though their backend routes are complete.
Because reverse coverage scans raw corpus text, it subtracts all sixteen classified forms after token discovery;
the comment-only canonical `parse_job(` declaration therefore cannot fabricate a generic call occurrence.
