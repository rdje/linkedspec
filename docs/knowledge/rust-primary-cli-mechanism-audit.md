...
answers:
  ...
  - did Rust remove the global parse mode execution option
...
evidence: "FUTURE-PARITY-BACKLOG.1.5.2.0 audits/splits the workspace; .1 lands 29/61; .2 lands native direct execution and historical entry/global-mode controls at 41/61; .3 lands canonical trace at 61/61; .4 closes default/POSIX and recurring verification. FUTURE-PARITY-BACKLOG.9.1.4.6 removes the global mode while retaining entry selection/direct results and reaches 63/63 in both environments."
...
`.1.5.2.2` subsequently added both to `ExecutionOptions` with `Engine::execute_value`. ADR `0044` later retired the
global override: `.9.1.4.6` leaves only optional entry selection, while every entered rule derives its own cursor
policy. The CLI remains a thin adapter and host applications retain direct results without a subprocess or
compiled-state mutation.
...
`.1.5.2.2` historically closed the native direct-result/entry/mode seam at 41/61; `.3` adds the independent canonical adapter
trace and closes the unchanged suite at 61/61. `.4` closes both option environments and adds the recurring Rust
gate. `.9.1.4.6` removes the global mode option/flag/request field and advances the expanded reference-owned
matrix to 63/63 in both environments. This was not a parser-language gap.
