...
date: 2026-07-19
status: verified historical pre-implementation boundary; all six Lua migration leaves are implemented and composed-admitted
...
evidence_update_2026_07_19_admission: "Public option removal .5 and exact dual-ABI admission .6 close the remaining migration seams. One 15-role consumer passes 119/119 on each ABI; the complete driver passes package 177/177 per ABI, primary 65/65 across four ABI/environment legs, and corpus 105/105 per ABI. Governance is 69/6+2/49."
reverify: "bash tools/run_python_project_data.sh tools/check_rule_local_cursor_contract.py; rg -n 'Pipe|parse_mode|GENERATED_SOURCE_CONTRACT|GENERATED_SOURCE_FORMAT|classify_generated_rule_family|lua_runtime:rule' lua/src/linkedspec/{spec_ast.lua,interpreter.lua,compiled_spec.lua,source_emitter.lua,spec_loader.lua,corpus.lua,primary_cli.lua}"
---
