...
answers:
  ...
  - why did the Julia temporary owner count change from 20 to 22
date: 2026-08-16
status: current
...
evidence_update_2026_09_03_write_vivification_owner: "FUTURE-PARITY-BACKLOG.19.5.1 adds julia/test/write_vivification_contract_test.jl as the exact 21st maintained mktempdir/tempdir owner. Its emitted-module proof writes beneath the managed repository-local TMPDIR and removes the exact workspace in finally. The five-package offline depot and all device, symlink, shared-depot, generated-source, trace, and cleanup boundaries remain unchanged."
evidence_update_2026_09_03_map_leaves_mutation_owner: "FUTURE-PARITY-BACKLOG.19.5.2 adds julia/test/map_leaves_mutation_contract_test.jl as the exact 22nd maintained mktempdir/tempdir owner. Its independent emitted-module proof uses the managed repository-local TMPDIR and removes the exact workspace in finally. The exact owner-array comparison was already correct, but its success sentence still hard-coded 21; this leaf derives the displayed count from that checked array so later registrations cannot silently print a stale total. The five-package offline depot and every device, symlink, shared-depot, generated-source, trace, and cleanup boundary remain unchanged."
...
`tools/test_julia_project_data_storage.sh` freezes all 22 tracked `mktempdir()`/`tempdir()` owners, checks actual
