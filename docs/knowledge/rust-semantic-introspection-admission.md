...
Rust is the second admitted implementation of `linkedspec-semantic-model-v1` and
...
calls, privacy, failed, and runtime snapshots; direct, loaded, reconstructed, generated-plan, source-emitter, and
traced execution; native/neutral JSON identity; all 20 exact response digests; privacy, pages, budgets, portable
...
status, and Rust rollout status. Eight mutations independently remove or alter those boundaries. The existing
early-admission mutation now targets Dart, so no later backend can promote without its owned composed leaf.
