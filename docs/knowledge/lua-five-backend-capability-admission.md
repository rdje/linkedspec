...
`future.generic_final_codeblock`: contextual declared Lua forms are current,
but Lua explicit callable values and arbitrary dynamic calls are not. Rust and
Dart callable parity are now complete, while Julia construction and dynamic
invocation are current and its generic contextual equivalence remains pending.
