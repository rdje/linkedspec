...
and nested assignment. It also evaluates harray literals and `hash(...)`. It does not yet dispatch the ordinary
hash helper family or hash receivers, and shared `flat(...)` currently follows the array path for every value kind.
...
- `.4.3.5.1`: `hash`, literals, `copy`, `flat`, `flat_hash`, explicit constructor splicing, and isolation.
