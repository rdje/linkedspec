...
evidence: "DART-BACKEND-PARITY.2.2 adds dart/lib/src/parser/spec_parser.dart and dart/test/spec_parser_test.dart. Tests cover focused Rust-compatible parser seams, all checked-in specs/*.spec files, and corpus input.spec files that do not start with top-level fn definitions."
reverify: "cd dart && dart test test/spec_parser_test.dart && dart analyze --fatal-infos --fatal-warnings"
---
...
source parsing, not strict validation. `DART-BACKEND-PARITY.2.3` owns validation
and strict syntax behavior. Top-level `fn name(args) { ... }` extraction/staging
remains `DART-BACKEND-PARITY.2.4`.
