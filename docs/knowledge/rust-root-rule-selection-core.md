...
rule. Effective selection is per-execution state and adds no strict-unused reference or exemption. Generated and
emitted execution still selects only an authored marker and lacks an invocation selector; `.2.2` owns those
composed routes, while `.2.3` alone may topology-check admission and promote the Rust rollout row.
