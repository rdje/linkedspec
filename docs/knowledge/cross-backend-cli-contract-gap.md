...
- Julia `bin/linkedspec_julia.jl` exposes `status` plus manifest corpus validation/execution. It reports usage
  failure as `2` but does not expose Perl's parser command contract.
