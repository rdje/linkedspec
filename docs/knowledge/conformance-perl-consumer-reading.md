...
date: 2026-09-21
status: groups 34–47 physically read; Phase0 partial through line 10823; all runtime repair prerequisites remain
...
reverify_phase0_checkpoint: "Replay .1.47 using conformance-source-reading-coverage; compare ordinals 310–337 and unchanged own Perl sources against canonical commit 87b35665e. Retained execution grants no additional reading credit."
...
Group 47 reads Phase0 lines 9845–10823. ParserFactory and public get_parser controls reject
malformed default parser, descriptor and single parse/generate-only results with mode-specific
detail. Callback-written deeper owner errors survive exceptions and undef returns. Public
managed-file controls separate Runtime fallback diagnostics from ParserFactory rejection of
malformed Runtime results. Inert callback path values and actual File::Temp fixtures retain
the distinction described by [[perl-project-data-ssd-storage]].

Runtime supplies context while Compiler owns the default parse/entry callbacks. Direct hashes
and populated scalar slots preserve identity and caller fields, clear stale errors and replace
source captures for identical repeated input. Inline failure clears stale file identity.
These controls reconcile with [[runtimecontext-boundary]]. Constructed handler presence and
source-capture equality do not establish independent generated-source or parser execution.

Single parse-only and generate-only successes deliberately return undef with last_error clear;
they differ in whether a top rule has been selected. Injected compiler exceptions and malformed
results acquire runtime_owner attribution unless a deeper compiler error already exists.
[[perl-compiler-pipeline-stage-and-mode-boundaries]] owns those stages. Combined-flag precedence
is outside these single-mode controls: [[perl-get-mode-result-validation-precedence-drift]]
and startup .42 remain open. The public missing-file control also asserts emitted diagnostic text.

Twenty-eight completed subtests retain canonical PASS at ordinals 310–337 (280 assertions),
with Phase0, own Perl sources and shipped specs unchanged from 87b35665e. Replay checks every
sequential assertion number; no new execution is claimed. The reused-context resolution-failure
test ends after eight of nine assertions; .1.48 owns the final assertion. Reading is 47/143,
60,318 fragments / 2,181,495 baseline bytes and 97 complete files. All prerequisite-gated
repairs remain open; prior exact checkpoints remain in task nodes/Git.
