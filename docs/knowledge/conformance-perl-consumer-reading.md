...
answers:
  ...
  - which Phase0 binding and newline checks retain meaningful contrast evidence
  - which Phase0 receiver tests expose absent binding host-slot leakage
  ...
  - what cursor scalar and semantic call consumers are read in group 85
  - what semantic and sparse slot consumers are read in group 86
  - does the Perl semantic observation reconstructed plan row rebuild an executor
  - what lifecycle and trace consumers are read in group 88
  - do the Perl trace consumers prove actual cold-process lazy loading
  - what trace dispatch and typed source consumers are read in group 89
  - which typed source value tests remain unread after group 89
date: 2026-09-22
status: groups34–89 physically read; seven more files complete; typed values partial through226; all repairs retain prerequisites
...
Seven trace/typed-projection consumers complete with27 top-level/402 nested results.
CLI checks exact JSON/error channels and ambient-trace isolation. EmitContext checks
canonical rejection/injection plus actual cold-process Trace absence. Live generated
branch probes remain distinct from four source-checked repetition templates.
[[perl-trace-wrapper-source-observation-gap]] owns the cross-handler assertion gap;
eleven process helpers retain .2.16 ownership. Startup .24 exception-state repair
remains open. Typed projections pin92 rows/seven aliases and exact live/generated
Unicode and absent-match results, including a present zero-width control. Generated
packages share the process. Immutable values are partial through226; .1.90 owns the
remaining diagnostics and authority tests. Staged evidence stays in
[[conformance-staged-perl-consumer-reading]].

The no-execution query controls replace Runtime::run_get and establish unchanged
answers without compilation; exact parser-callback interception is not exercised.
Generated-plan validation is the published minimal-plan contract, not a second
executor-construction path. No new runtime defect follows from these boundaries.

Source-observation scope: [[phase0-cursor-source-observation-gap]].
Process failure scope: [[phase0-subprocess-capture-status-and-pipe-gap]].
Nested copy scope: [[perl-semantic-call-copy-observation-gap]].
Known scalar and semantic repairs remain separate from these finite fixtures.

Reading is 87/143 groups, 106,904 fragments / 4,673,539 baseline bytes and 120 complete files.
