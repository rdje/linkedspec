...
date: 2026-09-21
status: groups 34–58 physically read; Phase0 partial through line 23758; all observation-gap and runtime repairs retain prerequisites
...
reverify_phase0_checkpoint: "Replay .1.58 using conformance-source-reading-coverage; verify ordinals 559–576 and nine seven-tag loops against unchanged ec10be6b sources. Keep current-slot-shape observations separate from code equivalence and retain the canonical .2.10 gap reproduction."
...
Group 58 reads Phase0 lines 22259–23758. It completes fluent outer-switch readiness
and covers mixed attached/plain branch carriers across structured and fluent forms.
The first seven-tag fluent outer-switch loop compares metadata without a code-slot
assertion. Mixed-carrier pairs include uppercase-slot comparisons under existing .2.10.

Five marker-switch nested-flow families cover marker if, marker switch, composite
if, inline switch and composite if/elseif. Their assertions pin metadata, unresolved
lists, fallback and explicit undef slot shape. Outer/nested marker switches expect
SWITCH2/ENDSWITCH2; marker outer plus two inline switches expects SWITCH3/ENDSWITCH1.
These representation-specific expectations differ from earlier inline-outer counts.
Lifecycle RETURN totals include the separate action-edge return.

A deeper inline outer switch contains if/elseif and mode/mode2/kind/kind2 switches.
Its structured/attached-if comparisons explicitly describe current slot shape.
That narrow claim differs from generated-code equivalence; the latter remains
unsupported by absent-slot comparisons, as documented in
[[phase0-code-slot-equivalence-observation-gap]]. No target parser executes here.

Eighteen completed subtests retain canonical PASS at ordinals 559–576 with 160
direct assertions, including 63 nested results. Nine seven-tag loops separately
verify 476 inner assertions: three loops use eight per tag, five use seven and
one uses nine. The next marker-outer counterpart stops inside its first fixture,
at if( in the outer default body; .1.59 owns the suffix. Reading is 58/143,
73,253 fragments / 2,895,669 baseline bytes and 97 complete files. No new defect
or target execution is established. All .2.5–.2.11 and earlier repairs remain.
