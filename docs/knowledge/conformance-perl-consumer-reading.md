...
date: 2026-09-22
status: groups 34–88 physically read; six more files complete; trace CLI partial through 47; all repairs retain prerequisites
...
Six staged/lifecycle/trace files are complete; trace CLI is read only through 47.
Staged carrier detail belongs to [[conformance-staged-perl-consumer-reading]].
Lifecycle twins check exact bootstrap, source, spans, duplicate order and malformed
fields; self-hosting preserves seven reserved markers. Generated Perl runs in the
same process. Trace tests compare lowering text and selected trace events; their
cold children observe Trace absence both before and after owner calls. They do
not execute every lowered fragment. Four process helpers reproduce existing
status/pipe defects, now owned by .2.16; preserve the cold-process assertions.
