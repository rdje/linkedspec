...
date: 2026-09-21
status: groups 34–40 physically read; Phase0 partial through line 2566; all runtime repair prerequisites remain
...
reverify_phase0_checkpoint: "Replay .1.40 using conformance-source-reading-coverage; compare ordinals 50–79 and unchanged own Perl sources against canonical commit 87b35665e. Retained execution grants no additional reading credit."
...
Group 40 reads Phase0 lines 1527–2566. Eight source-shape validation tests are now
complete, including the crossing ParserFactory check. Actual OwnerDispatch calls resolve
synthetic callbacks, preserve requested dependency keys and list returns, and build mixed
callback/value bundles. Already-loaded package fixtures do not prove first-time disk loads.

Twelve EmitContext owner-map tests substitute map builders and implementation callbacks.
They check the requesting package, returned markers, local entrypoints and successful
caller error-state preservation. Other wrappers cover tracing, dumping, LinkedRE,
runtime/bootstrap, SpecEntry, compiler, plugins and injected ParserFactory orchestration.
These tests execute the wrapper paths while deliberately replacing their collaborators;
they do not establish the behavior of those replaced compiler/scanner/lowering functions.
[[ownerdispatch-shared-seam]] remains the structural owner for the shared helpers.

Thirty completed subtests retain unchanged canonical PASS at ordinals 50–79, totaling
181 assertions; Phase0 and LinkedSpec-owned Perl sources match 87b35665e. No fresh runtime
execution is claimed. The next ActionIR dependency-builder test plans 24 assertions, but
only imports and callback fixture setup are read; .1.41 owns every assertion and the suffix.
Current reading is 40/143, 52,061 fragments / 1,723,739 baseline bytes and 97 complete files.
Update this checkpoint in place; prior ranges/evidence remain in task nodes and Git.
