...
all 105 fixtures. Rust remains partial only because that diagnostic has not yet
completed zero-failure closeout and strict recurring-gate admission. This is
not an API, semantic-role, or newly discovered emitter gap.

`FUTURE-PARITY-BACKLOG.3.2.0` completes all-105 classification. Later `.3.2`
leaves own zero-failure closeout and full-manifest recurring admission.
