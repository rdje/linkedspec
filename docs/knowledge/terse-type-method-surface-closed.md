...
No `SPEC-FORMAT-TERSE` leaf is currently pending. `TRACE-OBSERVABILITY.1` and `.2` have since closed;
`TRACE-OBSERVABILITY.3` has since split, and PNT returns to `TRACE-OBSERVABILITY.3.1` unless a new terse
leaf is split or another active tree is reprioritized.
