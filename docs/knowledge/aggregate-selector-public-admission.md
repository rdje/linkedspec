...
is 64 public files. It composes the runtime/source retirement checker so public and executable admission cannot
...
## September 13 Rust integration-page inventory

`BACKEND-INTEGRATION-GUIDES.2.1` adds exactly
`docs/linkedspec-book/src/public-api/integration-rust.md` to public discovery.
Focused preflight reproduces expected62/observed63 before the correction at
`tools/check_public_aggregate_selector_surface.py:14`. The checker changes only
that expected cardinality and the classified-reference count; all prior classifier,
migration, mutation and composed checker code remains byte-identical. The new page
adds no selector reference. Exact discovery also finds three existing references
in the Julia callable-validation limitation introduced by6308ff4e24: two inline
retired spellings and one invalid fenced example. A blank paragraph boundary
separates that fence from the surrounding invalid-language explanation, so
`sentence_at` correctly requires classification context inside the fence.

The full Julia example and open JULIA-STARTUP-READING.2.1 repair are retained. An
explicit invalid/retired comment classifies the fenced occurrence; the current
reference count is35. The original32 other references, zero current examples,
five contrasts and eleven contrast mutations remain intact. Earlier evidence
counts above are dated results, not the current public inventory. This is a
public documentation repair, not a Julia validator repair or new runtime proof.
