...
title: Round 2 control-flow keyword surface is split by current portable support: marker/composite if and inline switch work cross-backend, Perl attached if now works, Rust attached if and other attached forms need separate leaves
answers:
...
- **Portable cross-backend today:** statement-marker `if(cond); ... elseif(cond); else(); ... endif()` and inline-composite
  lazy `if(cond, body, elseif(...), else(...))` / `switch(expr, case(...), default(...))`.
- **Perl attached-if today:** compact attached-block `if(cond) { ... } elseif(cond2) { ... } else { ... }`
  lowers through ActionIR and runs without raw fallback after `.2.2.2`; Rust parity is `.2.2.3`.
