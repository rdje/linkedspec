...
evidence_update_2026_08_30_five_backend_projection: "Later Dart, Julia, and Lua parity leaves made attached if/elseif/else, when/otherwise, switch/case/default, and while controls plus lazy inline if/switch current on all five backends. FUTURE-PARITY-BACKLOG.23.2 repairs the formal grammar's surviving Perl/Rust-only wording and extends capability conformance with four omission/stale-injection mutations bound to `language.current_mdbook_surface`, whose five backend states are all pass."
