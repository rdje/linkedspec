...
title: Round 2 control-flow keyword surface: attached if/when/switch and inline if/switch are portable; while remains the open leaf
answers:
...
- **Next leaf:** `while(cond) { ... }` with explicit progress/iteration safety.
...
5. `while(cond) { ... }` with explicit progress/iteration safety. FRONTIER.
