...
title: Round 2 control-flow keyword surface: attached if/when/switch and inline if/switch are portable; while is split by Perl reference and Rust parity
answers:
...
- **Split today:** `while(cond) { ... }` is host code on Perl and has no Rust statement-loop runtime.
- **Next leaf:** `.2.2.6.1` — Perl reference `while(cond) { ... }` with deterministic iteration safety.
