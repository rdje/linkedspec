...
date: 2026-08-07
status: private core implemented by FUTURE-PARITY-BACKLOG.14.2.5.1; projection and admission pending
...
reverify: "LINKEDSPEC_LUA_TYPED_SOURCE_RED_MODE=core bash tools/run_lua_project_data.sh puc lua/test/typed_source_location_contract_test.lua && LINKEDSPEC_LUA_TYPED_SOURCE_RED_MODE=core bash tools/run_lua_project_data.sh luajit lua/test/typed_source_location_contract_test.lua && LINKEDSPEC_LUA_TYPED_SOURCE_RED_MODE=projection bash tools/run_lua_project_data.sh puc lua/test/typed_source_location_contract_test.lua 2>&1 | rg -q 'Lua typed source RED: missing linkedspec.typed_source_projection_rows' && LINKEDSPEC_LUA_TYPED_SOURCE_RED_MODE=projection bash tools/run_lua_project_data.sh luajit lua/test/typed_source_location_contract_test.lua 2>&1 | rg -q 'Lua typed source RED: missing linkedspec.typed_source_projection_rows' && ! rg -q 'typed_source_location_contract_test[.]lua' tools/run_lua_local.sh tools/run_ci_local.sh"
---
...
`tools/run_lua_project_data.sh` with ABI `puc` or `luajit`. The two core combinations now pass 133/133 assertions.
The two projection combinations stop at the same next private boundary:

```text
Lua typed source RED: missing linkedspec.typed_source_projection_rows
```

The core is a private direct module and is neither exported by `linkedspec/init.lua` nor routed into an execution
context. Projection catalog lookup remains ordered after that core require, so `.14.2.5.2` owns the one honest
missing `linkedspec.typed_source_projection_rows` boundary without a consumer rewrite.
...
the private core is now implemented, while routing and admission remain separate durable frontiers.

## Links

- Owners: [[FUTURE-PARITY-BACKLOG]] `.14.2.5.0.2` (frozen consumer) and `.14.2.5.1` (private core).
