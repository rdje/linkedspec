...
date: 2026-07-21
status: current private compiled projection; public query added separately; observations and admission pending
...
`capabilities`/`query` are now supplied by `.10.3.4`; runtime observations, the composed Perl consumer, and ledger
admission remain `.10.3.5-.10.3.6`, so neutral rollout stays 1/9 and backend admission 0/6.
