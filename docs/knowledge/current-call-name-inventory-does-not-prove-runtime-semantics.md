...
evidence_update_2026_07_10: "FUTURE-PARITY-BACKLOG.1.6.1.2.2.1.1 through `.1.3` repair the Rust, Dart, and Julia pure fixtures: numeric predicate results, direct-literal flat splicing, and explicit working-array statement transforms now match Perl. Rust position `.2.1` then adds explicit match-presence state, exact absent-local-match null/empty/default projection, numeric named-presence values, and a real zero-width-at-zero no-regression lock. Rust passes 137 library tests, 193 integration tests, and the unchanged 99-case oracle. Dart passes the governed pure fixture, 152 package tests, 61 shared CLI cases in both environments, and unchanged 99-case corpus. Julia passes the governed pure fixture, 1,020 assertions, 61 shared CLI cases in both environments, and unchanged 99-case corpus. Dart position `.2.2` is next; the overall card remains a confirmed gap until every mechanism/backend and final 105-case admission close."
