...
evidence_update_2026_07_10: "FUTURE-PARITY-BACKLOG.1.6.1.2.2.1.1 and `.1.2` repair the Rust and Dart pure fixtures: numeric predicate results, direct-literal flat splicing, and explicit working-array statement transforms now match Perl. Rust passes the governed fixture, 191 integration tests, and unchanged 99-case oracle. Dart passes the governed fixture, 152 package tests, 61 shared CLI cases in both environments, and unchanged 99-case corpus. Julia `.1.3` is the next pure-value child; the overall card remains a confirmed gap until every mechanism/backend and final 105-case admission close."
