...
evidence_update_2026_07_10: "FUTURE-PARITY-BACKLOG.1.6.1.2.2.1.1 through `.1.3` repair the Rust, Dart, and Julia pure fixtures; position `.2.1` through `.3` align exact absent-local-match values and zero-width locks on all three. Rust marker-control `.3.1` then adds known/executable `i`/`elif` aliases while confirming existing matched-case/default exclusion. Rust passes 137 library tests, 194 integration tests, and unchanged 99-case oracle; Dart passes 154 package tests, 61 shared CLI cases in both environments, and unchanged 99-case corpus; Julia passes 1,022 assertions, 61 shared CLI cases in both environments, and unchanged 99-case corpus. Dart marker-control `.3.2` is next; the overall card remains a confirmed gap until every mechanism/backend and final 105-case admission close."
