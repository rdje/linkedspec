...
evidence_update_2026_07_10: "FUTURE-PARITY-BACKLOG.1.6.1.2.2.1.1 through `.1.3` repair the Rust, Dart, and Julia pure fixtures; position `.2.1` through `.3` align absent-local-match values and zero-width locks on all three. Rust marker-control `.3.1` adds executable `i`/`elif`; Dart `.3.2` groups marker switch siblings into a nesting-aware first-match/default chain. Rust passes 137 library tests, 194 integration tests, and unchanged 99-case oracle; Dart passes 155 package tests, 61 shared CLI cases in both environments, and unchanged 99-case corpus; Julia passes 1,022 assertions, 61 shared CLI cases in both environments, and unchanged 99-case corpus. Julia marker-control `.3.3` is next; the overall card remains a confirmed gap until every mechanism/backend and final 105-case admission close."
