...
date: 2026-07-15
status: accepted direction; contracts and Perl/Rust/Dart mechanisms complete; Julia/Lua/admission pending
...
Perl, Rust, and Dart nested assignment implement write-only vivification: an absent root or missing intermediate
is created from the evaluated string/integer selector kind. Julia and Lua still require existing intermediate
...
ADR `0036` accepts two portable mechanisms, now implemented on Perl, Rust, and Dart:
...
same-receiver behavior belong to `.19.1`. Perl, Rust, and Dart implement `map_leaves!`; Julia/Lua implementations
and public admission follow in `.19.5-.19.9`. Until those leaves land, only Perl, Rust, and Dart may claim current
write vivification or bang receiver mutation.
