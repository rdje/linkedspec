...
date: 2026-07-15
status: accepted direction; nested writes complete on all five backends, Lua map_leaves bang and portable/public admission pending
...
evidence_update_2026_09_03_julia_map_leaves: "FUTURE-PARITY-BACKLOG.19.5.2 implements the unchanged map_leaves! mechanism on Julia through one typed receiver/callback/continuation carrier, stable scoped binding identities, pre-evaluation receiver guards, copied original-shape traversal, atomic publication, detached results, and post-commit continuation. The permanent suite passes 496 assertions across native, reconstructed, generated-plan, emitted-module, and CLI routes. Both Lua mechanisms and portable/public admission remain pending."
...
All five backends implement write-only nested-assignment vivification: an absent root or missing intermediate is
created from the evaluated string/integer selector kind. Every backend keeps dense arrays and refuses wrong-kind
coercion. Perl, Rust, Dart, and Julia ActionIR accept only the ratified `map_leaves!` receiver form; Lua still
rejects `!` as a method suffix.

ADR `0036` accepts two portable mechanisms. Nested writes now run on all five backends; receiver mutation runs on
Perl, Rust, Dart, and Julia:
...
same-receiver behavior belong to `.19.1`. Perl, Rust, Dart, and Julia implement `map_leaves!`; Lua receiver
mutation and public admission follow in `.19.6.2-.19.9`. Until those leaves land, those four backends may claim
both current mechanisms; Lua may claim nested writes only, and portable/public admission remains pending.
