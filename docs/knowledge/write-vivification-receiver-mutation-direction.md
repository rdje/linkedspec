...
date: 2026-07-15
status: accepted direction; contracts and both Perl mechanisms complete; remaining backends/admission pending
...
array/harray shapes. Every backend keeps dense arrays and refuses wrong-kind coercion. Perl's ActionIR grammar now
accepts only the ratified `map_leaves!` receiver form; the other backends still reject `!` as a method-name suffix.

ADR `0036` accepts two portable mechanisms, now implemented on the Perl reference:
...
same-receiver behavior belong to `.19.1`. Perl implements `map_leaves!`; Rust/Dart/Julia/Lua implementations and
public admission follow in `.19.3-.19.9`. Until those leaves land, only Perl may claim either current write
