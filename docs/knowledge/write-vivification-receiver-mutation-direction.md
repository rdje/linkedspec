...
date: 2026-07-15
status: accepted direction; contracts and both Perl/Rust mechanisms complete; remaining backends/admission pending
...
Perl and Rust nested assignment implement write-only vivification: an absent root or missing intermediate is
created from the evaluated string/integer selector kind. Dart, Julia, and Lua still require existing intermediate
array/harray shapes. Every backend keeps dense arrays and refuses wrong-kind coercion. Perl and Rust ActionIR now
accept only the ratified `map_leaves!` receiver form; the other backends still reject `!` as a method-name suffix.

ADR `0036` accepts two portable mechanisms, now implemented on Perl and Rust:
...
same-receiver behavior belong to `.19.1`. Perl and Rust implement `map_leaves!`; Dart/Julia/Lua implementations
and public admission follow in `.19.4-.19.9`. Until those leaves land, only Perl and Rust may claim current write
vivification or bang receiver mutation.
