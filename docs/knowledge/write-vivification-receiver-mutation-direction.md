...
date: 2026-07-15
status: accepted direction; both mechanisms complete through Julia; Lua and portable/public admission pending
...
array/harray shapes. Every backend keeps dense arrays and refuses wrong-kind coercion. Perl, Rust, Dart, and Julia
ActionIR accept only the ratified `map_leaves!` receiver form; Lua still rejects `!` as a method suffix.

ADR `0036` accepts two portable mechanisms. Nested writes and receiver mutation are now implemented on Perl,
Rust, Dart, and Julia:
...
same-receiver behavior belong to `.19.1`. Perl, Rust, Dart, and Julia implement `map_leaves!`; Lua and public
admission follow in `.19.6-.19.9`. Until those leaves land, those four backends may claim both current mechanisms;
Lua and portable/public admission remain pending.
