...
date: 2026-08-17
status: dormant RED frozen; no Rust production behavior or rollout admitted
...
reverify:
  - "RUSTFLAGS='--cfg linkedspec_progressive_span_dispatch_red' bash tools/run_cargo_local.sh test --offline --manifest-path rust/Cargo.toml -p linkedspec-runtime --test progressive_span_dispatch_contract -- --nocapture"
  - "bash tools/run_cargo_local.sh test --offline --manifest-path rust/Cargo.toml -p linkedspec-runtime --test progressive_span_dispatch_contract"
...
the private immutable registry/invocation/source-view authority, `.14.6.3.2` owns the dedicated node and four
carriers, and `.14.6.3.3` alone may route the unchanged consumer and promote Rust rollout. Until those leaves land,
private Perl remains the sole backend implementation and neutral + Perl remains rollout 2/9.
