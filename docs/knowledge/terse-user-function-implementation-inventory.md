...
`SPEC-FORMAT-TERSE.3.3.4` later closed legacy spelling cleanup and full assignment-expression docs/oracle
closure. No concrete `SPEC-FORMAT-TERSE` PNT-eligible leaf remains after that closure; future backend leaves are
deferred or blocked by explicit roadmap decisions.
