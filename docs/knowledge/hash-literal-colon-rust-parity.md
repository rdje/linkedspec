...
The 2026-09-07 source-reading checkpoint `SESSION-STARTUP-READING.3.3.6` confirms
that `parse_hash_literal` parses both the key and value with `parse_expr`. A bare
`key` therefore remains an expression referring to that binding; it is not silently
converted into the literal string `"key"`. Use `{ "key" : value }` when literal
