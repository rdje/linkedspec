...
- `MethodLowering` now consumes AST for supported value/call/receiver-chain nodes, typed
  ...
  source/match/body/default/end nodes. Raw fallback remains for untyped compatibility
  payloads and the while lowering family that a later `.4.4` leaf still owns.
