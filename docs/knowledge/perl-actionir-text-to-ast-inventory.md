...
- `MethodLowering` now consumes AST for supported value/call/receiver-chain nodes, typed
  return payloads, and assignment/mutation operator statements, while raw fallback remains
  for untyped compatibility payloads and later statement families.
...
nodes, and control/return/print nodes with source spans. Standalone expression statements
drop their value silently.
