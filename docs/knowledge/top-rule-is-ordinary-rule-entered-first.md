...
date: 2026-06-23
status: confirmed runtime mechanics; ADR 0046 selection admitted on Perl and Rust with later variants pending
...
- **The feature surface is shared.** After a rule is selected as the parser entry point, a `::` rule
  ...
  when no marker exists. Perl now implements and admits that exact order; Rust, Dart, Julia, Lua, and final
  five-backend admission remain tracked under `FUTURE-PARITY-BACKLOG.9.1.1.2.2-.6`.
