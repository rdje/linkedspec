...
date: 2026-08-01
status: Perl and Rust runtimes admitted; Dart immutable core implemented but unadmitted; Julia, PUC Lua, and LuaJIT pending
...
text or engine reference. Its two value tests pass, and the full dormant consumer now stops only at the absent
projection catalog owned by `.14.2.3.2`. No helper route or admission has moved, so `dart_runtime` and rollout
remain pending at 5/9/39.
