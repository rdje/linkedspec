...
evidence: "PROJECT-DATA-SSD-ROOTING.4.1 adds scripts/check_project_data_storage_locality.sh as the registered PROJECT-DATA-STORAGE structural doctrine. It scans tracked code, tests, tools, configuration, README/Toolbox/public-book guidance, and only executable reverify lines in Knowledge cards. It rejects operating-system-temp, developer-home, unrooted cache/depot/build/output, and concrete external storage defaults while accepting explicit caller inputs, inert path/privacy fixtures, external executables/system libraries, rejection-test probes, root-relative operands, and repository-derived storage. Twenty-two embedded rejected/accepted cases run on every invocation. The registry now contains six doctrines, and hostile outside-cwd routing covers 36 entrypoints. Toolbox TAP/diff/focused-test captures now use a checkout-derived diagnostic directory. Direct, registry, routing, and all six storage oracles pass with zero managed runs."
...
fallbacks, developer-home package caches, unrooted storage fallbacks, temporary allocator templates, active config
destinations, and documented output paths. It deliberately accepts root-relative operands, checkout-derived
storage, explicit caller inputs, inert privacy/path values, necessary external executables and libraries, and
hostile external locations read only by rejection tests. Twenty-two embedded examples keep both sides of that
boundary mutation-sensitive.
...
scanning. Structural enforcement parent `.4.1` is complete; process-level opened-path proof `.4.2` follows.
