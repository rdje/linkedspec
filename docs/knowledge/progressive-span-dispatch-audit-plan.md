...
date: 2026-08-17
status: executable neutral authority, all six private runtime admissions, recurrence, and public no-drift complete without an outward API
...
evidence_update_2026_08_17_dart_admission: "FUTURE-PARITY-BACKLOG.14.6.4.3 moves the unchanged seven-group carrier consumer to ordinary Dart discovery and registers its exact tracked path, log marker, and invocation once in canonical CI. Only Dart admission and rollout advance. Governance is 4/9 complete with 103 mutations, 9 Rust + 8 Dart carrier paths, 2 Julia/Lua guard groups/8 paths, 10 outward guards, and 26 diagnostics. The authority-only consumer remains dormant; typed progressive, Julia/Lua, recurrence, generated format, public inventory, and outward surfaces stay pending or unchanged."
...
evidence_update_2026_08_25_lua_admission: "FUTURE-PARITY-BACKLOG.14.6.6.3 moves the unchanged 178-assertion Lua carrier to ordinary discovery once per ABI and adds one exact canonical route per ABI. Only PUC Lua and LuaJIT rollout advance, making governance 7/9/112 with 9 Rust + 8 Dart + 9 Julia + 9 Lua carrier paths, zero backend guards, 10 outward guards, and 26 diagnostics. The 273-assertion-per-ABI authority proof remains dormant; production, generated format, typed recurrence, public inventory, and outward surfaces remain unchanged."
evidence_update_2026_08_25_lua_recomposition: "FUTURE-PARITY-BACKLOG.14.6.6.4 independently reruns carrier 178/178 and authority 273/273 on both Lua ABIs, complete ordinary Lua, all four admitted peer consumers, and every neutral/direct-dependent checker without production, test, fixture, executable-contract, generated-format, or CI-topology changes. It corrects only stale current Lua-pending projections while preserving dated evidence. Shared Lua parent .14.6.6 closes at 7/9/112; typed recurring proof .14.6.7 is next."
evidence_update_2026_08_25_typed_recurrence: "FUTURE-PARITY-BACKLOG.14.6.7 adds tools/check_progressive_span_dispatch_six_runtime.sh as one repository-routed fail-fast driver. It executes the neutral checker, Perl, cfg-enabled Rust, Dart, Julia, PUC Lua, and LuaJIT in exact order, then typed-source, generated-source, capability, and language support ledgers. Five immutable backend source groups form six runtime routes because the shared Lua consumer runs once per ABI. Twelve topology/storage/rollout mutations plus two new reason-checked stale-projection denials advance typed governance to 12 complete / 2 pending / 170 and promote only progressive_span_dispatch; behavioral governance remains 7/9/112 and public no-drift remains .14.6.8-owned. Git blame identifies commit 78bc66e1 as the origin of the stale Perl-only 2/9 capability-guide paragraph; later admissions omitted that current projection, so this slice corrects and mutation-locks it without rewriting dated evidence."
evidence_update_2026_08_25_public_closeout: "FUTURE-PARITY-BACKLOG.14.6.8 proves the completed .14.6.7 driver conflicts with its still-pending behavioral recurring row, binds that unchanged driver, and promotes public_no_drift. Governance is 9/9/116 plus public 6 documents / 12 stale-current denials / 10 outward paths / 60 omission-and-injection mutations. No runtime, consumer, generated format, shared helper, facade, schema, semantic/MCP, CLI, or README behavior moves."
reverify:
...
Private Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT progressive behavior is admitted. All five source backends
recognize `dispatch_span(...)` as one
...
Dart `.14.6.4.2` provides the same logical-only shape through `ActionProgressiveDispatchSpanExpr`, an opaque host
seed, static recognition-effect closure, and live-token defense. Native, reconstructed, generated-plan, and
independently analyzed emitted source agree. `.14.6.4.3` admits that unchanged consumer through ordinary discovery
and one exact canonical route.

Julia `.14.6.5.2` supplies the corresponding logical-only node and fresh-seed carrier through native,
...
Lua `.14.6.6.2` supplies the same dedicated node and fresh-seed behavior through native, normalized-JSON
reconstructed, generated-plan, and independently loaded emitted-module routes. Admission `.14.6.6.3` runs the one
Lua-5.1-compatible 178-assertion consumer once per ABI in ordinary discovery and once per ABI through exact
canonical routes. Its separate 273-assertion-per-ABI authority proof remains dormant.
...
executable neutral contract/checker; Perl `.2`, Rust `.3`, Dart `.4`, Julia `.5`, and both Lua runtime rows `.6.3`
were privately admitted at rollout 7/9/112 before closeout. Julia's exact historical dormant RED `.0`, private authority/core `.1`,
dormant carriers `.2`, canonical admission `.3`, and independent recomposition `.4` are complete. Shared Lua RED
`.6.0`, private authority `.6.1`, dormant carriers `.6.2`, dual-ABI admission `.6.3`, and independent
recomposition `.6.4` are complete. `.7` binds five source groups to six runtime routes and promotes only typed
`progressive_span_dispatch`, making typed truth 12/2/170. `.8` corrects the behavioral recurring row and closes
public projection/no-drift at 9/9/116 plus public 6/12/10/60 without exporting a facade or consuming combined
final row `.14.8`.
