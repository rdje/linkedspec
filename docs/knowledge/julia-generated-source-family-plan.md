...
title: Julia generated-source v2 has exact ten-family authoritative cursor and structural dispatch
answers:
...
evidence_update_2026_07_18_cursor_v2: "FUTURE-PARITY-BACKLOG.9.1.6.4 keeps the exact ten names and label/family-only rows, adds contract-first v2 validation, derives the neutral five-seek/five-consume map, corrects compact Pipe to OR, and removes the generated-v1 forced-seek engine. Focused emitter proof is 65/65; complete Julia is 3,133 plus the frozen help mismatch."
reverify: "bash tools/run_julia_project_data.sh --project=julia --startup-file=no --history-file=no -e 'using LinkedSpecJulia, JSON3, Test; const REPO_ROOT=pwd(); include(\"julia/test/source_emitter_test.jl\")'"
---

Julia uses the exact generated-source-v2 family names: `default`, `or_acode`, `and_single_acode`, `and_acode_seq`,
...
`validate_generated_rule_plan_v2(...)` first rejects a non-v2 contract, then distinguishes row-count,
ordered-label, known-family mismatch, and unknown-family failures. Unknown names are rejected before
expected-family comparison.
The validated plan becomes a per-label typed family map in the runtime execution context. Every generated root and
nested rule reads that map, derives seek or consume, and selects regex/acode or blind/bcode dispatch from it;
native execution has no map and retains compiled-family selection. The plan therefore controls execution rather
than serving as metadata only, while serializing no separate cursor field.
...
[[julia-runtime-rule-interpreter]], and [[julia-generated-source-v2-rule-local-cursor]].
