...
evidence: "SPEC-FORMAT-TERSE.6.2.3.2 retired authored `scalar(...)` and `assign(...)`. At that time `:name` was the explicit scalar-slot path, but SPEC-FORMAT-TERSE.15.2.4 migrated current sources to bare reads and SPEC-FORMAT-TERSE.15.3 hard-retired Perl `:name` support. Current Perl authoring reads variables by bare name in value positions and uses `name = value` / `set(name, value)` for assignment; explicit `array(name)` / `hash(name)` targets mark aggregate-storage mutation boundaries. Rust `Expr::ScalarSlot` remains separately owned by SPEC-FORMAT-TERSE.15.4."
