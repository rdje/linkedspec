...
date: 2026-09-21
status: exact baseline decomposition preserved; physical reading 56/143, 97 files complete and 87 groups remain; one approved current-source delta
...
`CONFORMANCE-SOURCE-READING.1.56` reads 10 windows, 1,500 fragments and 64,289 baseline bytes.
Cumulative reading is 56/143, 70,552 fragments, 2,767,515 baseline bytes and 97 complete files.
Phase0 is partial through line 21057, after eight assertions of the LX flat-list test.
[[conformance-perl-consumer-reading]] records nested-switch metadata and the reading boundary.
[[phase0-code-slot-equivalence-observation-gap]] supersedes earlier generated-code equality
interpretations: changed payloads pass absent-slot comparisons. New .2.10 owns that repair;
.2.11 owns the source-capture recipe error. Retained canonical proof covers 18 subtests with
148 direct assertions, including 74 nested results with 557 verified inner assertions.
No production runtime defect or repair is claimed. The pinned 113-byte inventory delta stays
separate; these ranges total 2,767,628 current bytes. Next .1.57 reads Phase0 lines 21058–22258.
