...
date: 2026-09-21
status: exact baseline decomposition preserved; physical reading 44/143, 97 files complete and 99 groups remain; one approved current-source delta
...
`CONFORMANCE-SOURCE-READING.1.44` reads 11 windows, 1,190 fragments and 65,521 baseline bytes.
Cumulative reading is 44/143, 57,321 fragments, 1,985,093 baseline bytes and 97 complete files.
Phase0 is partial through line 7826, before the final owner-default-error assertion.
[[conformance-perl-consumer-reading]] distinguishes validation-only acceptance, typed marker
lowering, compiler failure payloads and runtime-context identity controls. Thirty-seven completed subtests retain unchanged
PASS at 87b35665e (295 assertions). All runtime repairs remain open.
The pinned 113-byte current cursor-inventory delta stays separate from historical reading;
the same current ranges total 1,985,206 bytes. Next .1.45 reads Phase0 lines 7827–8871.
