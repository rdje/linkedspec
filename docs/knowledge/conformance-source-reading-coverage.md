...
date: 2026-09-21
status: exact baseline decomposition preserved; physical reading 63/143, 97 files complete and 80 groups remain; one approved current-source delta
...
`CONFORMANCE-SOURCE-READING.1.63` reads 9 windows, 1,500 fragments and 52,166 baseline bytes.
Cumulative reading is 63/143, 80,753 fragments, 3,179,902 baseline bytes and 97 complete files.
Phase0 is partial through line 31258, inside the first outer-family composite-if inline-switch fixture.
[[conformance-perl-consumer-reading]] records selected-node checks in outer-family switch comparisons and their
observation limits. [[phase0-code-slot-equivalence-observation-gap]] remains the
code-equivalence limitation; all .2.5–.2.11 and earlier repairs stay open.
Retained proof covers ten subtests with 90 direct assertions, including 35 nested results
with 252 verified inner assertions. No new defect or target execution is claimed.
The pinned 113-byte inventory delta stays separate; these ranges total 3,180,015 current
bytes. Next .1.64 reads Phase0 lines 31259–32758.
