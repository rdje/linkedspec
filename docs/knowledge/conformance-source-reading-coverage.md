...
date: 2026-09-21
status: exact baseline decomposition preserved; physical reading 42/143, 97 files complete and 101 groups remain; one approved current-source delta
...
`CONFORMANCE-SOURCE-READING.1.42` reads 11 windows, 1,397 fragments and 65,472 baseline bytes.
Cumulative reading is 42/143, 54,631 fragments, 1,854,695 baseline bytes and 97 complete files.
Phase0 is partial through line 5136, before the AND+ comparison's first result assertion.
[[conformance-perl-consumer-reading]] distinguishes resolution, runtime-error, descriptor
and execution proof. Forty-one completed subtests retain unchanged PASS at 87b35665e
(353 assertions). All runtime repairs remain open.
The pinned 113-byte current cursor-inventory delta stays separate from historical reading;
the same current ranges total 1,854,808 bytes. Next .1.43 reads Phase0 lines 5137–6636.
