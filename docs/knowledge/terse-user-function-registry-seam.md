...
evidence: "SPEC-FORMAT-TERSE.4.2.1 added specs/spec.spec function_definition grammar and spec_file dispatch; added LinkedSpec::UserFunctionRegistry; added compiled_spec_state function_order/functions_by_name and public descriptor functions/meta.function_order/meta.function_count; phase0 subtest user_function_registry_descriptor_seam proves descriptor projection, body ActionIR AST capture, duplicate/builtin/rule/parameter diagnostics, and that registered value calls remain unresolved until .4.2.2."
...
Registered calls are still not executable in `.4.2.1`. A value-position call to a
registered function still reports unresolved-helper metadata with zero raw fallback.
Perl value-call execution starts at `SPEC-FORMAT-TERSE.4.2.2`; standalone discard and
purity hardening remain `.4.2.3`.
