...
- `.4.5.3`: done — runtime interpreter branch, lifecycle, dispatch, recursion-cutoff, cursor-control, and
  source-boundary trace instrumentation;
- `.4.5.4`: done — no-drift closeout across Dart README/CLI status, mdBook, live docs, task-tree index, and
  Knowledge Map.

The active executable frontier after `.4.5.4` is `DART-BACKEND-PARITY.5.1`.
