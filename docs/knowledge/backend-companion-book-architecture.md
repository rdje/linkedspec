...
evidence: "ADR 0040 and FUTURE-PARITY-BACKLOG.21 adopt the architecture. Lua admission .8.4 satisfies the five-backend parity prerequisite; BACKEND-COMPANION-BOOKS.1 remains pending and no companion scaffold exists yet."
...
The books have not been scaffolded. Full five-backend parity is now complete; `BACKEND-COMPANION-BOOKS.1+`
remains pending explicit activation, beginning with a read-only inventory before any scaffold or content move.
Rollout then establishes shared structure, independent builds, navigation, canonical-owner metadata, and
mechanical duplication/drift gates before any content is migrated or populated.
