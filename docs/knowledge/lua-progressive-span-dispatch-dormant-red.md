...
date: 2026-08-25
status: historical dual-ABI RED preserved as evidence; current carriers admitted and independently recomposed on both ABIs
...
evidence_update_2026_08_25_admission: "FUTURE-PARITY-BACKLOG.14.6.6.3 moves the current GREEN consumer into ordinary and canonical dual-ABI discovery and advances only PUC Lua/LuaJIT to rollout 7/9/112. The historical .0 RED remains dated evidence; no dormant carrier duplicate remains."
evidence_update_2026_08_25_recomposition: "FUTURE-PARITY-BACKLOG.14.6.6.4 independently reruns the admitted carrier at 178/178 and separate authority at 273/273 on each ABI without executable movement. It preserves this dated RED evidence, closes shared Lua parent .14.6.6 at unchanged rollout 7/9/112, and hands off typed recurring proof .14.6.7."
reverify: "git show 84178bd82f238338e6119fa3e9ad2b6d82170fae:lua/test_dormant/progressive_span_dispatch_contract_test.lua | rg -n 'sole intentional RED boundary|missing exclusive progressive_dispatch_span node'; bash tools/run_lua_project_data.sh puc lua/test/progressive_span_dispatch_contract_test.lua; bash tools/run_lua_project_data.sh luajit lua/test/progressive_span_dispatch_contract_test.lua; bash tools/run_python_project_data.sh tools/check_progressive_span_dispatch_contract.py"
---

# Shared Lua progressive span-dispatch RED

The current final-path consumer is
`lua/test/progressive_span_dispatch_contract_test.lua`. It is one
Lua-5.1-compatible source executed unchanged by PUC Lua and LuaJIT. Leaf `.0`
recorded its exact historical 85-pass/one-RED boundary. Carrier `.2` made the
expanded consumer GREEN at 178/178 per host, and admission `.3` moved it into
ordinary and canonical discovery without a dormant duplicate.
...
The consumer also locks the neutral 7/9/112 inventory, nine admitted carrier
paths, both complete Lua rollout rows, normalized reconstruction, the unchanged
one-row generated-v2 plan, emitted identity, and exact dual-ABI discovery. The
package facade and outward surfaces remain free of the private intrinsic.

Child `.14.6.6.1` owns one private authority shared by both ABIs and proven
separately at 273/273 per host. `.2` owns the dedicated node and four carriers;
`.3` owns their ordinary/canonical dual-ABI admission and rollout. `.4` independently
recomposes those owners unchanged and closes the parent.
