...
broader model, and `@capture_gaps` as the accepted future neutral directive. `@capture_gaps` is not current
syntax. Its executable contract must decide prefix, tail, empty-span, failure/backtracking, recursion,
typed-span, diagnostics, and compatibility policy before implementation.
...
only coupling. Numeric/unindexed forms remain compatible. The ratified future syntax declares
