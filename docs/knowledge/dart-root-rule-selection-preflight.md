...
date: 2026-07-18
status: superseded by dart-root-rule-selection-core; retained behavior-free causal baseline
...
Core leaf `.3.1` has now replaced marker-required validation with one-or-more-rule validation, made the
compiled-state resolver own portable zero and unknown failures, preserved strict graph semantics, and published
immutable descriptor identity. Route work `.3.2` proves loaded, normalized, generated direct/traced, and emitted
