...
- **Internal positions stay byte-based**: `ctx.pos`, `capture_start`, `marks`, the regex
  ...
  all **byte** offsets into `ctx.input` (the regex engine works in bytes). Slicing the input
  between two of these (e.g. `capture_slice`, `capture_from`) is panic-safe and text-correct.
