...
evidence: "JULIA-BACKEND-PARITY.7.3.2.0 audits Julia's exports, primary CLI, corpus adapter, parser/compiler/runtime, diagnostics, trace sinks, and JSON use. Existing native seams are reusable, but five separately verifiable mechanisms are missing; .7.3.2.1 is active first."
...
Runtime trace instrumentation exists, but source parsing, validation, compilation,
function-shell parsing, and staged dispatch do not yet emit through it. Accepting
primary `--trace*` flags before closing that gap would give the option a narrower
meaning in Julia than in other variants.

`JULIA-BACKEND-PARITY.7.3.2` is therefore a parent with five ordered mechanism groups:

1. `.7.3.2.1` — compile/parser/function-shell/staged trace coverage;
2. `.7.3.2.2` — exact arguments plus source/input loading and named resolution;
