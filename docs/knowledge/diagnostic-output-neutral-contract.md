...
evidence: "ADR 0042 ratifies linkedspec-diagnostic-output-v1. capability_conformance/diagnostic_output_contract.json encodes three helpers, 11 render rows, five invalid arities, six semantic scenarios, and an exact rollout ledger; tools/check_diagnostic_output_contract.py independently evaluates those fixtures and rejects eight drift mutations. FUTURE-PARITY-BACKLOG.5.1.2-.6 complete all five native legs, and .5.1.7 completes generated propagation plus exact quiet primary projection."
...
The rollout ledger now records all five native legs and generated/primary projection as `complete` (six complete,
two pending). Recurring admission `.5.1.8` and public no-drift `.5.1.9` remain.
