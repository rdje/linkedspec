...
reverify: "bash tools/run_cargo_local.sh test --locked --offline --manifest-path rust/Cargo.toml -p linkedspec-runtime --test source_boundary_compatibility_aliases; bash tools/run_python_project_data.sh tools/check_typed_source_location_contract.py"
---
...
At that repair milestone, Rust typed-value admission remained separate. The subsequent rollout is recorded
in [[typed-source-location-runtime-rollout-plan]]; fresh 2026-09-07 neutral proof is 14 complete/zero pending,
with 231 mutations.
...
## 2026-09-07 source catalog reconciliation

`SESSION-STARTUP-READING.3.3.28` independently decodes both JSON catalogs in runtime.rs: 92 unique
canonical names (47 capture/mark, 30 entry/match, 11 input/cursor, four cursor control) and exactly seven
aliases. The five capture mappings above plus entry_named_map→entry_map and match_named_map→match_map
match the current catalog. This is exact source inventory and fresh neutral contract proof, not a new
native alias run; the historical carrier execution evidence above remains explicitly dated.
