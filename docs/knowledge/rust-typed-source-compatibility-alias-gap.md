...
date: 2026-08-01
status: neutral targets corrected; confirmed Rust parity gap awaits five-alias implementation
...
The Perl reference does not return `undef` for those calls. It lowers them to the canonical `capture_slice`,
`capture_slice_len`, `capture_rest_len`, `start_capture_slice`, and `capture_slice_len` projection families. In the
...
choice: Rust must match Perl. The audit also proved the first two neutral target labels were wrong; their correct
preferred targets are `capture_slice()` and `capture_slice_len()`. The neutral correction now records and checks
those exact identities. The Rust parity implementation must land next, before the typed-source RED.

## Links

- Owners: [[FUTURE-PARITY-BACKLOG]] `.14.2.2.0.1-.0.2`.
