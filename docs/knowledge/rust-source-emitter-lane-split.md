...
title: RUST-PARITY.8 is split into narrow source-emitter leaves; Rust generated source now embeds a validated family plan before direct execution
answers:
...
`RUST-PARITY.8.2` / `.8.3` / `.8.3.1`.** The Rust source-emitter work is not one implementation
...
- `.8.2`: done — minimal emitter API plus generated-source compile/run
  harness. The generated module embeds serialized `CompiledSpec` and delegates
  `parse(input)` through `Engine`.
- `.8.3`: split/active — non-repetition handler-family emission is now
...
- `.8.3.1`: done — `CompiledRule` preserves parsed `RuleMode`, generated
  ...
  validates that plan before delegating through `Engine`.
- `.8.3.2`: current executable frontier — direct default/OR acode generated
  execution.
...
replacing the delegation with direct generated handlers starts in `.8.3.2`.
