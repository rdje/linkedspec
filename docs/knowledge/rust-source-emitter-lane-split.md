...
title: RUST-PARITY.8 is split into narrow source-emitter leaves; Rust generated source now directly runs non-REP acode and bcode families
answers:
...
`RUST-PARITY.8.2` / `.8.3` / `.8.3.1` / `.8.3.2` / `.8.3.3` / `.8.3.4`.** The Rust source-emitter work is not one implementation
...
- `.8.3.5`: current executable frontier — non-repetition generated-family
  matrix closeout.
...
`.8.3.2`, `.8.3.3`, and `.8.3.4` started replacing fallback execution with
direct generated-family handling for acode and bcode families.
