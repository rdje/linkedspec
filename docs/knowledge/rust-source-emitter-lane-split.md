...
title: RUST-PARITY.8 is split into narrow source-emitter leaves; Rust generated source now directly runs all non-REP families
answers:
...
`RUST-PARITY.8.2` / `.8.3` / `.8.3.1` / `.8.3.2` / `.8.3.3` / `.8.3.4` / `.8.3.5`.** The Rust source-emitter work is not one implementation
...
- `.8.4`: current executable frontier — repetition handler-family emission and
  termination guards.
...
`.8.3.2`, `.8.3.3`, `.8.3.4`, and `.8.3.5` replaced fallback execution for
every non-REP generated family.
