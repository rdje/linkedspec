...
title: RUST-PARITY.8 is split into narrow source-emitter leaves; Rust now has a minimal generated-source scaffold, while non-REP direct emission starts with .8.3.1 metadata/plan work
answers:
...
`RUST-PARITY.8.2` / `.8.3`.** The Rust source-emitter work is not one implementation
...
- `.8.3.1`: current executable frontier — rule-mode/family metadata plus
...
split. `.8.2` adds a source-emitter module and compile/run harness only;
replacing the delegation with direct generated handlers starts after `.8.3.1`
establishes the generated family-plan metadata.
