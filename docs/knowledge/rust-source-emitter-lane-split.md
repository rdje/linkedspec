...
title: RUST-PARITY.8 is split into narrow source-emitter leaves; Rust generated source now directly runs acode families
answers:
...
`RUST-PARITY.8.2` / `.8.3` / `.8.3.1` / `.8.3.2` / `.8.3.3`.** The Rust source-emitter work is not one implementation
...
- `.8.3.4`: current executable frontier — direct AND/OR bcode generated
  execution.
...
`.8.3.2` and `.8.3.3` started replacing fallback execution with direct
generated-family handling for acode families.
