...
title: RUST-PARITY.8 is split into narrow source-emitter leaves; Rust generated source now directly runs default/OR acode
answers:
...
`RUST-PARITY.8.2` / `.8.3` / `.8.3.1` / `.8.3.2`.** The Rust source-emitter work is not one implementation
...
- `.8.3.2`: done — generated parse enters the plan-aware executor; `Default`
  and `OrAcode` families run directly while later-family lanes still fall back.
- `.8.3.3`: current executable frontier — direct AND acode generated execution.
...
`.8.3.2` started replacing fallback execution with direct generated-family
handling for default/OR acode.
