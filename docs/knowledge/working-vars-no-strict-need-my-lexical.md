...
title: "Generated rule handlers run WITHOUT `use strict`, and a rule's I-block + all edges + LX compile into ONE lexical scope. A `.spec` working variable needs one preamble `my`; auto-existence now supplies it for current terse refs, while historical declare(type,name) evidence explains the original hazard."
answers:
...
evidence_update_2026_07_06: "SPEC-FORMAT-TERSE.8.3 retired Perl `declare(...)` lowering, so current reverify should use terse references such as `items += value; return(copy(array(items)))` to inspect auto-supplied preamble declarations. The non-strict handler and one-lexical-scope facts remain current; the declare examples are historical evidence for why auto-existence was needed."
evidence_update_2026_07_07: "SPEC-FORMAT-TERSE.8.4 completed Rust hard-retirement of declaration helpers and short aggregate wrapper aliases. Current authoring and reverify examples should use bare scalar reads, `array(...)` / `hash(...)`, assignment, `set(...)`, `push(...)`, and `copy(...)`; references to `declare(...)`, `s(...)`, `a(...)`, and `h(...)` in this card are historical implementation evidence."
reverify: "grep -c 'use strict' perl/LinkedSpec/SpecEntry.pm   # 0 -> generated handlers are non-strict; then dump a current terse spec such as items += value; return(copy(array(items))) via dump_parser_source to see `my @items ;` once in the preamble"
---
...
- **Collect rule-wide:** the original implementation scanned every action-code block of a rule for typed wrapper references
  ...
  read). Current authoring uses bare scalar reads and the long aggregate wrappers; short aliases are retired.
