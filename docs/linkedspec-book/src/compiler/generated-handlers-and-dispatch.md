...
therefore owns explicit > first marker > first rule for every route without an emitter-local selection algorithm.

Generated-plan validation runs before entry selection. After a valid plan, an unknown selector becomes
...
selection trace are identical on PUC Lua and LuaJIT. Root-selection admission now composes generated direct,
generated traced, emitted direct, and emitted traced roles in one exact 15-role shared-source consumer on each ABI
without adding another resolver. Cursor leaf
