...
ADR 0030 adopts the next versioned signature, now implemented by the Perl reference:
...
arguments into ordered temporaries before binding fixed values and a new rest array. Fixed calls still require
exact arity; variadic calls require at least `min_arity`. Rust, Dart, Julia, and Lua remain rollout work, so this is
current reference behavior but not yet portable multi-backend behavior.
