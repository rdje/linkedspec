...
The setup/hold timing migration shows the same pattern across two still-shipped legacy actions. `plugin/setup_hold_tmax_tmin.plg` still exposes `setup_hold_tmax_tmin`, and `plugin/tssio.plg` now keeps `tssio` only as a thin compatibility wrapper, but their private timing helpers are no longer dynamic `DxCy`, `DiCi`, `DiCo`, `DoCi`, `DoCo`, `tss_setup_hold`, or `tss_tmax_tmin` plugin subdefs, and the `tssio` report body no longer lives in the legacy plugin file. `Timing::SetupHold` owns those formulas, dispatch helpers, and `write_tssio(...)` directly.
