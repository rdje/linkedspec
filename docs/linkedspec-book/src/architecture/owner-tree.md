...
Within those lowering owners, `DeclareMethod` now treats its declare/assign helper routines as the direct callback-validation seams instead of carrying a second top-level `_require_dep(...)` wrapper above them. `ControlFlow` now does the same for the branch/output lowering helper family that owns if/switch marker lowering plus fluent `say(...)` / `print(...)` lowering. `MethodLowering` now follows that same shape for method/value/assignment/return lowering.
