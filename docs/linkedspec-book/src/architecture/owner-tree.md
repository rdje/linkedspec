...
In the same direction, `Runtime`, `Compiler`, `BootstrapSpec`, and `ParserFactory` no longer keep one-shot local pass-through wrappers for the single compiler/bootstrap callback or `$@`-preservation calls inside their main orchestration helpers. `Runtime`, `ParserFactory`, and compiler top-rule-only diagnostics also ask `RuntimeContext` for top-rule generated-handler labels directly at their fallback diagnostics boundaries instead of keeping one-shot label wrappers. `ParserFactory` also validates its required trace-level values directly in `run_get_parser(...)` instead of keeping a one-shot value-dependency validator. Those live bodies now spend the real setup seams directly, which keeps meaningful ownership visible instead of hiding it behind wrapper names that only had one caller.
