...
`LinkedSpec::RuntimeContext` is the shared owner for runtime-context preparation as well as payload storage. Inline `Get(...)`, file-oriented `get_parser(...)`, compiler-pipeline parser-source setup, and low-level `build_compiled_rule_table(...)` diagnostics now route their runtime-context setup through that owner instead of each hand-normalizing the hook.
