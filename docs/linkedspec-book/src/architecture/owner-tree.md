...
Repo-owned `.plg` files should not reach into `PPlugin->get(...)` or direct `LinkedSpec` plugin-bridge dispatch helpers for private helper reuse. Shipped legacy plugin files now call explicit package owners for those migrated helper families, while package owners that genuinely need dynamic callback resolution can inject or default to `LinkedSpec::get_plugin(...)` outside the `.plg` corpus. The FSMGen dynamic plugin-list parser follows that pattern: the old `plugin/fsmgen.plg::getop_plugin_list` helper wrapper is gone, and `FSMGen::getop_plugin_list(...)` uses `LinkedSpec::get_plugin(...)` only as its default resolver.
