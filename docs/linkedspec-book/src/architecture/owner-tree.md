...
Seven small but useful examples of that owner-shape cleanup are `LinkedSpec::ActionIR::Contracts`, `LinkedSpec::ActionIR::ScannerCore`, `LinkedSpec::ActionIR::StatementSplit`, `LinkedSpec::ActionIR::CanonicalEvents`, `LinkedSpec::ActionIR::Diagnostics`, `LinkedSpec::ActionIR::RewritePipeline`, and `LinkedSpec::ActionIR::ArrayPipeline`: those files now keep `_require_lowering_deps(...)`, `_scanner_rule_dep_bindings(...)`, `_split_action_ir_statements(...)`, `_build_canonical_action_ir_events(...)`, `_find_unresolved_action_helpers(...)`, `_collect_action_helper_ir_nodes(...)`, `_build_action_rewrite_rules(...)`, `_rewrite_action_code_with_diagnostics(...)`, `_normalize_split_delimiter_expr(...)`, and `_build_array_pipeline_plan_from_expr(...)` as their local dependency seams and no longer expose second top-level `_require_dep(...)` validator wrappers beside them.
