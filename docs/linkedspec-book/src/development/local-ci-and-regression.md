...
were immutable measured debt. All four are now current bounded collections: live status under ADR `0067`, task
...
perl tools/read_document_history.pl --surface engineering_notes --grep 'needle'
...
recursive and zero/one/two-regex structural cases; and rejects all 189 registered mutations.
...
The gate also requires and ordinarily executes Rust's exact value/projection and recursive-observation consumers:
...
The gate also requires and executes Julia's exact value/projection and seven-group recursive-observation consumers
through repository-managed Julia storage:

```bash
bash tools/run_julia_project_data.sh --project=julia \
  --startup-file=no --history-file=no -e \
  'using LinkedSpecJulia, JSON3, Test; include("julia/test/typed_source_location_contract_test.jl"); include("julia/test/recursive_observation_contract_test.jl")'
```
...
It runs the typed checker, the complete neutral plus six-runtime gap driver, recognition ownership, and the
...
Julia's recursive-observation consumer runs beside that value/projection consumer:

```bash
bash tools/run_julia_project_data.sh --project=julia \
  --startup-file=no --history-file=no -e \
  'using LinkedSpecJulia, JSON3, Test; include("julia/test/recursive_observation_contract_test.jl")'
```

Its seven groups and 30 assertions prove the private dedicated node, static and transaction-effect closure,
falsey-safe detached records, UTF-8-code-unit to Unicode-scalar projection, direct/action-edge cursor ownership,
all terminal outcomes, fresh rejected-attempt identity, no retained history, and native/reconstructed/generated-
plan/emitted-module convergence.
...
Every evidence path must exist. The separate exclusion ledger is now schema v2 with exactly one governed record:
deprecated Perl plugin machinery is `legacy` under pending `.6` with nullable `retention_authority`. An open
legacy record requires null, while completed legacy ownership requires an existing repository-relative durable
retention authority. Portable parse-job authoring is current and absent from the exclusion ledger.

The checker derives unique task ids and leading status enums from tracked task sources, locks exact exclusion
content/order, and rejects 19 in-memory schema, classification, retention, task-status, content, and
satisfied-record mutations. Completed semantic/MCP, rule-local cursor, and parse-job authoring narratives are
absent rather than mislabeled as future. At the original exclusion-governance boundary,
