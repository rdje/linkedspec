...
Julia's exact dormant final-path command is recorded in the [progressive span-dispatch status](../dsl/capture-marks-and-source-locations.md): 55 assertions pass and only the missing dedicated-node assertion fails; ordinary/canonical discovery omit it, and `.14.6.5.1-.4` separately own authority, carriers, admission, and recomposition.
