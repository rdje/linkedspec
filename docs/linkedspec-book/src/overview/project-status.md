...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, exhaustive neutral language/helper proof `.1.6.1`, and exact four-backend outward descriptors `.1.6.2` are closed. The validated 15-capability census currently classifies its 60 states as 52 pass, one partial, and seven gap. Typed Rust diagnostics `.1.6.3.1` are closed and final admission `.2` is active; later leaves own native named/file resolution for Rust/Dart/Julia and Dart frontend/compiler/staged trace. `.1.6.6` closes non-codegen parity; generated-source `.3` remains separately mandatory before Lua.
