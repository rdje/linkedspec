...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5` and all non-codegen capability work `.1.6` are closed. The validated 15-capability census classifies its 60 states as 57 pass, one partial, and two gaps. All three non-pass states belong only to generated parser source—Rust partial, Dart/Julia gaps—and are owned by active `.3` before Lua.
...
- **Generated-source convergence** — the interpreter oracle remains primary. `.3.1.2` repairs Perl's independently loaded source with canonical dependency reconstruction, public/legacy deterministic emission, validated family plans, trace/error/identity roles, and exact indexed execution. Rust covers every family and the accepted eight-case subset; `.3.1.3.1` now adds v1 identity/metadata/typed errors while preserving string adapters. Exact neutral family-plan rejection and trace roles remain under `.3.1.3.2`, followed by admission `.3.1.3.3` and full 105-case breadth `.3.2`. Dart/Julia have no emitter. Census stays 56/2/2. Host-source bytes may differ; observations may not.
