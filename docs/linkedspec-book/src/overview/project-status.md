...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, exhaustive neutral language/helper proof `.1.6.1`, exact outward descriptors `.1.6.2`, structured diagnostics `.1.6.3`, and exact four-backend native resolution `.1.6.4` are closed. The validated 15-capability census classifies its 60 states as 56 pass, one partial, and three gap. Dart full-pipeline trace `.1.6.5` is split into frontend/compiler, function/staged, and composed-admission leaves; frontend/compiler `.1` is active. `.1.6.6` closes non-codegen parity; generated-source `.3` remains separately mandatory before Lua.
