...
by lifting their options to Dart `RegExp`; deeper PCRE structural
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  diagnostic run. The basic regex-dialect bridge is done, while deeper PCRE structural constructs remain routed to
  a follow-up; the next Dart frontier is missing helper/action surfaces, followed by recursion/output, residual
  parity, and cross-backend gates.
