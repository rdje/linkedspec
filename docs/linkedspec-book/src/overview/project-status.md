...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  jobs, rule modes, body elements, edges, and fluent calls. `Pkg.instantiate()`, `Pkg.test()`, CLI help/status, and
  corpus validation commands pass with a writable depot. The active frontier is `.2.2` for parsing `.spec` text
  into the source AST.
