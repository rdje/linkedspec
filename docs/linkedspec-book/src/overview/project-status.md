...
## Current rollout guidance is contract-locked

`FUTURE-PARITY-BACKLOG.23.2` reconciles the remaining current-facing rollout claims with their executable owners.
Semantic introspection remains 9/9 with six runtime admissions and 128 mutations. Rule-local cursor and bare-edge
normalization are current on all five backends at 8/0; standalone lifecycle shorthand is current through six
runtime routes; logical truthiness/helpers are 8/0; attached and inline structured controls are current on all five
backends; and root selection is 7/0 with explicit selector > first `Rule::` > first ordinary `Rule:` precedence.

The same leaf turns those statements into omission/stale-injection failures. Current narrow inventories are cursor
30 documents / 28 denials / 60 mutations, standalone lifecycle 15 / 7 / 14, logical helpers 19 / 14 / 26, root
selection 24 / 18 / 54, and inter-match gaps 8 documents / 15 denials / 10 outward guards / 34 public mutations.
The structured-control marker is bound to the all-pass `language.current_mdbook_surface` capability through four
mutations. Dated rollout evidence remains historical. No parser, compiler, runtime, fixture, generated format,
capability state, facade, schema, semantic/MCP field, CLI, or public API behavior changes.
...
## Standalone lifecycle-block parity is complete

ADR `0094` and `FUTURE-PARITY-BACKLOG.15.0` ratify a rule-item-leading `{ ... }` as exact shorthand for
`I { ... }`, with the same authored position, code, ordering, and diagnostics. `.15.1` established Perl/Rust;
`.15.2` completed Dart, Julia, dual-ABI Lua, the self-hosted grammar, recurring proof, and public no-drift. The
shorthand is current on all five backends through six runtime routes, including exact source/opening-line
provenance, explicit/bare duplicate order, inert legacy Rust `PlainBlock`, and direct/serialized/generated proof.
Attached edge blocks and function/callable/nested braces remain unchanged. The explicit `I` marker remains a
clear spelling, not a portability requirement.
...
Physical Dart reading has completed all 55 children, including the complete ActionIR parser,
...
Write-vivification and dormant progressive-authority tests are also complete. All 115
files total 80,297 reading fragments / 2,471,305 bytes. The current 20-test selection
and two neutral checks pass; earlier results remain in their
...
the independent .3.1/.3.2 audit is complete and ADR0114 closes startup `.3.4` reading. Julia reading closes all 52 owned groups under explicit ADR0117 approval after the passing independent audit and complete component gate. ADR0116 admits the finite evidence allowance; ADR0115 remains limited to history.
...
**Completed child 49 evidence:** the twelve-role
...
for the eight-case accepted subset; `.1.50` now completes the harness and fixtures.
...
plus staged function loading and structured failures; directories stand in for
non-regular files. Parser/validator corpus checks skip initial function definitions
and establish parsing/validation for their selected rule sources, not execution.

Staged consumer reading reaches line 291. Its prefix checks current public
...
**Completed child 54 evidence:** exact retired
...
consumer. Running that test adds no unread suffix credit. That leaf reached 54/55
...
**Julia reading is complete and independently verified:** all 52 committed groups
account for 95 files, 75,984 lines and 2,693,170 bytes. The approved ADR0117 closeout
reverifies each source scope, comprehension/result record and first-parent activation.
All 122 touched fact cards and 80 pending repair nodes remain intact.

The unchanged complete Julia component gate passes 12,903 assertions, storage for
22 Julia owners and five locked package trees, primary CLI process conformance,
and all 105 corpus fixtures. This closeout rechecks its recorded evidence; it does
not claim a new gate run or resolve the known limitations below. The private
progressive authority consumer retains separate proof and dormant discovery.

The director explicitly approved a one-time reading-only canonical CI/receipt
exception. It closes only Julia reading and allows Lua decomposition to proceed,
while preserving all repairs, normal hooks and later verification/push requirements.
...
and 10,485,760 bytes in total, with the existing 128-file, 8,000-line/member and
1,048,576-byte/member safeguards. Every partition-checker run compares all five
...
143 bounded groups covering 8,257,059 baseline decoded bytes. Sixty-one groups account for
77,753 fragments and 3,067,280 baseline bytes, including 97 complete files. Phase0 reading
reaches line 28258. Structured-argument, attached and plain marker switch branches contain
multi-case switches, composite if/elseif and deeper alternating marker controls. Their
metadata checks inspect builds, fallback, helper/node coverage and readiness. Independent
selectors and undef branch values remain descriptor inputs; branch execution is unproved.

**The earlier generated-code equivalence claim remains withdrawn:** absent uppercase
code slots cannot establish emitted-code equality. This range identifies further assertions
labelled identical code output within the existing .2.10 repair inventory. That repair owns
meaningful code/behavior comparisons and a changed-payload negative control. Repair .2.11
owns the invalid scalar dereference of source chunks; explicit scalar capture works.

Eleven completed subtests retain canonical proof with 107 direct assertions, including 42
nested results whose inner plans verify 364 assertions. The next deep-marker action pair is
partially read after five of thirteen assertions; .1.62 continues. MethodExpr .2.5,
...
is established. The pinned 113-byte inventory delta and all repair prerequisites remain.
...
or repair tests: reading now reaches 61/143, `.1.62` is next, and the test and recipe
...
Rust checkpoint is pushed at `ad290bdb4`. The
[Perl integration guide](../public-api/integration-perl.md) now provides explicit
...
The [Lua integration guide](../public-api/integration-lua.md) supplies exact module
paths, separate native products for PUC Lua and LuaJIT, and a native word consumer.
...
surrounding claims; reading and passing checks do not close those repairs.
The four pinned gzip inputs retain explicit decompressed-text coordinates for
later reading. This checkpoint changes no language behavior or admission status.
...
**Lua named-capture and scoped-callback verification:** `.1.42` records 17 complete
selected groups per installed host, 34 total. Boundary capture seeks the earliest usable
...
value controls were partly read in that leaf and are completed below.
...
allowed body: a false condition succeeds, while another true condition raises the
exact typed limit error. Valid logical helpers evaluate operands eagerly in order.

The exact 55-case scalar numeric fixture passes. Word/symbol aliases and numeric
receivers share results; comparison and aggregate-reducer receivers stay terminal.
Reducers preserve inputs and retain their specific empty/invalid outcomes. These
finite cases do not close the separately tracked integer-overflow limitation.

Exact retired selectors reject in ordinary, dead, deferred-function, fluent and
caller-mutated compiled state. Retained constructors and the uniform-binding
fixture execute. Mutation results remain independent snapshots; registered rules
keep ambiguous push precedence, child push reuses its selected result, and wrong
accumulator kinds retain typed fields. Statement `set_key` reports a binding-kind
mismatch; direct incompatible indexed writes report a nested-write conflict.

Array/hash construction preserves ordered evaluation and explicit splice intent.
Receiver copy evaluates once and retains compatible continuation. Sorted harray
views preserve key/value correspondence and null-valued membership; copied
...
**Lua source, graph and native-observation verification:** the complete source
suite passes 382 assertions per installed host. It verifies strict UTF-8, exact
SHA digests, byte/scalar boundaries, LF/CR and combining-character coordinates,
source ceilings, typed range/option errors and detached protected values. Active
file/environment/clock sentinels and an authored failing action confirm the
constructor avoids those operations on the tested path.

The static graph suite passes 64 assertions per host at exactly 12 records,
14 relations and seven source references. Duplicate Child slots stay distinct;
parent matchers do not become extra slot records. Full fixture equality, source
spans, value shapes, detached copies and neutral Default repetition pass. Previously
reported source-correlation gaps remain separately owned.

Runtime derivation passes 269 assertions per host for a fresh observed snapshot,
the twentieth query digest, exact event evidence and unchanged static bases.
Instrumentation measures one materialization and no input hashing. Its malformed
fixtures have the six-case coverage limitation below. A separate complete helper
prefix loads; later remaining-static behavior tests await their owned reading.

The full native-observation suite passes 121/121 on LuaJIT and repeats **120/121,
FAIL, exit 1** on installed PUC Lua 5.5 for the known nil-callback identity change.
Unicode events, invocation-local sinks, no-sink observation silence, diagnostic
separation, reentrancy and healthy trace writers retain their expected behavior.
Across the four full suites the exact total is 1,671/1,672 assertions; 48 fixture
diagnostic observations and two helper loads are counted separately.

The required notes rollover preserves every historical byte. The retained
249-line root and new immutable segment reconstruct the prior root with its
...
field. Six passing negative cases therefore test wrong types instead of missing
fields. An independent probe confirms that genuinely absent fields already reject
correctly on both hosts. Repair `.2.34` owns explicit removal and fixture
preconditions that catch this mistake; production rejection behavior is unchanged.
...
covering digests, canonical frames, schema fixtures, cloned values and response
identity. The embedded data deliberately includes examples for all five backends.
These checks validate the generated binding; remaining source reading and complete
...
**Lua staged completion verification:** a fresh run of the unchanged staged
consumer passes 890 assertions per installed host, 1,780 total. A separate set of
102 boundary observations measures diagnostic sizes, guarded marker traversal,
capture storage and seed admission. Ordinary and maximum-exact exhausted call
counts reject before callbacks. A sparse snapshot array has different copy
...
All previous source, task, Knowledge, decision and immutable-history evidence is
preserved. Independent and production-function models agree that four rollovers
per history collection fit the admitted slots. The complete actual candidate plus
the full 57-unit reserve passes, including unchanged root and member limits.
Production validators pass 125 threshold and 54 authorization cases.
...
With a 64-byte allowance, one failure retains 188 bytes and two failures retain
375 bytes; the second record reports an already exhausted allowance. The remaining
counter reaches zero, but the retained records still exceed the limit. Larger
256-byte and 4096-byte controls fit. These measurements count compact UTF-8 JSON
records and exclude duplicate sidecars and list wrappers. Lua `.2.26.1-.3` own
repair and independent carrier proof, coordinated with the shared Dart `.2.17.1`
accounting decision and Julia `.2.14`. The current evidence comes from private
recursive-host calls; the existing positive staged suites still pass.

**Known Lua marker limitation — special detachment mishandles cycles and null:**
ordinary cyclic objects reject promptly and ordinary null values survive.
Marker-shaped host values follow a separate walk: root and nested cycles reach
the diagnostic probe’s fixed instruction guard, and an extension containing null
is rejected. False and valid acyclic marker controls pass. Lua `.2.27.1-.3` own
cycle-aware traversal, null preservation and independent affected-carrier proof.
The probes use tiny host-created values with bounded execution; they do not
establish an ordinary authored-spec loop. Atomic marker node accounting remains
the existing behavior, and arbitrary extension fields are not thereby admitted
as valid declaration options.

**Known Lua capture limitation — private range storage accepts malformed input:**
direct host calls can store negative, reversed, fractional or non-finite capture
endpoints. Extra and sparse range-list members are dropped, and an infinite
capture index is treated as absent. Valid copied ranges remain isolated from
caller edits. Native matching supplies PCRE endpoint pairs, and staged declaration
converts retrieved endpoints through typed source authority before using the span;
these probes do not establish a failure from ordinary spec input. Lua `.2.28.1-.3`
own the producer/consumer invariant, validated storage and independent provenance
proof. Exact replay for these three limitations:
`docs/knowledge/lua-staged-completion-reading-and-boundary-gaps.md`.
...
Execution-seed construction also copies a snapshot before validation. An extra
aliases-array member rejects during direct freezing but is omitted and accepted
by seed construction. Sparse-tail behavior differs with the host’s array length;
the evidence retains the PUC acceptance and LuaJIT rejection. No factory executes
in those controls. Lua `.2.25.1/.2` own full membership validation and independent
supported-carrier proof. These controlled host-table cases do not establish data
loss from ordinary spec text or valid serialized JSON. Exact array-copy evidence:
`docs/knowledge/lua-validator-staged-prefix-reading-and-array-gaps.md`; the seed
observations are in `docs/knowledge/lua-staged-completion-reading-and-boundary-gaps.md`.
...
The staged authority constructor similarly replaces `total_calls=false` with zero.
...
reading credit is claimed. Capacity commit `30723f0f0` restored the reading frontier; current progress is above.
...
payload version through staging, compilation and execution. Mutually consistent
line99 metadata is also accepted for a function actually on line1. Ordinary source
parsing emits the valid comparison node. Julia `.2.22.1-.3` own strict metadata,
source-derived coordinates and supported-carrier/public proof. Exact controls:
`docs/knowledge/julia-function-projection-metadata-gaps.md`.

**Known Julia limitation — lifecycle regex balance validation:** compact
`I.return(matches("{", /{/))` preserves complete code but fails brace validation.
The validator counts regex braces as structural braces; quoted braces and the
ordinary regex comparison succeed. Julia `.2.21.3` owns this distinct downstream
repair, required alongside outer collection `.2.21.1` before `.2.21.2` recurrence.
The exact diagnostic replay above also preserves genuine missing-brace rejection.

**Known Julia limitation — compact literal arguments:** `I.return("(")` can
be rewritten to `return()` and produce null, while `I.return(")")` truncates
and rejects. Their braced forms return the literals correctly. The argument
extractor counts parentheses inside strings. Julia `.2.20.1/.2` own lexical
extraction and supported-route/public proof.

**Known Julia limitation — regex braces in outer blocks:** a regex closing
brace can terminate an outer lifecycle block too early. For example,
`I { return(matches("}", /}/)) }` is truncated and rejected, while the ordinary
regex control works. Julia `.2.21.1/.2` own outer scanning and downstream balance
review. Unterminated explicit and shorthand blocks remain correctly rejected.
Exact controls and mechanisms: `docs/knowledge/julia-spec-lexical-boundary-defects.md`.
...
and strict validation. Inline headers have the same gap. Standalone body fluents also discard suffixes:
`.Töp()` compiles as method `T`, and `.öp()` compiles with an empty method name.
The measured `I`,
...
in semantic queries. The compiled edge remains unindexed, while the query reports
a regex-slot target shape and a `selects_regex` relation. A substring inside
`OtherChild[0]` also triggers the measured error. Julia `.2.18.1/.2` own typed
selector repair and supported-route/public proof. Exact controls and source
mechanism: `docs/knowledge/julia-semantic-authored-selector-gap.md`.
...
and still permits an attempt. A missing token therefore produces a child slot
event before `recognition_token_expected`. Repeating an attempt, including after
commit, runs the child again before reporting the token error. A one-character
Child on input `xx` proves both matches at positions 1 and 2. A valid checkpoint,
single attempt and commit returns `ok` with only the first child match.
Julia `.2.7.1/.2.7.2` own validation before child execution and static-sequence/carrier
verification. Native and generated-plan conveniences agree; fresh emitted routes
and enabled trace behavior remain unmeasured. Exact replay:
`docs/knowledge/julia-recognition-attempt-preflight-gap.md`.
...
passes 35 frames, 10 raw inputs, 10 lifecycle cases and 76 rejected mutations.
Runtime loading verifies the embedded bundle digest. Host registration binds opaque
handles to immutable native indexes, caller authorization, expiry and lower-only
query budgets. Evidence: `docs/knowledge/julia-mcp-decoded-server.md`.

**Known Julia limitation — MCP pattern matching:** a 43-character handle followed
by a newline (for example, `"A"` repeated 43 times plus `"\n"`) passes schema
validation and receives `linkedspec_mcp_handle_unavailable`. The malformed string
should receive JSON-RPC invalid-params error `-32602`; the neutral validator and
host handle-length check reject it. A trailing CRLF or extra ASCII character is
already rejected. Direct digest-schema validation has the same final-LF gap.
Julia `.2.4.1/.2.4.2` own complete matching and decoded/stdio regression proof.
Exact reproduction: `docs/knowledge/julia-mcp-pattern-terminal-newline-gap.md`.

Six paired decoded/stdio requests also confirm the previously owned metadata-error
ordering difference in Julia. Startup `.36` remains its shared repair owner.
Evidence: `docs/knowledge/perl-mcp-validation-error-order-drift.md`.
These passing finite suites do not close either defect or grant full Julia signoff.
...
its independent audit and complete component gate pass, and ADR0117 closes reading, with the remaining Lua/supporting
...
line. Every baseline byte is unchanged and accounted for exactly once.

Write tests cover exact syntax and astral spans, evaluation order, structural
failures after RHS evaluation, non-creating reads, detached aggregates, fresh
function locals and malformed compiled carriers. The expression-failure fixture
injects its exception through the diagnostic sink and checks identity and stopped
evaluation. A representative nested write also runs through reconstruction,
generated plans, inline CLI and a fresh offline emitted package.

The private progressive authority test covers neutral views, grants, cancellation,
chains, all 26 diagnostic codes, scalar rebasing, shared invocation counters,
expired requests and detached bounded results. Its permissive nested cases do not
prove inheritance of narrower callback grants or budgets; startup `.37` retains
the measured defect. Its copied parent-state fixture is separate from engine
integration. The authority test remains private and outside ordinary discovery;
the admitted carrier test runs separately.
...
startup `.3.4` reading. Approved containment .12 enabled Julia .1.4; current Julia
reading progress is tracked above. Existing defects and the remaining codebase/book/policy
...
**Known Dart limitation — regex braces:** `/(})/` is accepted alone but becomes unsupported action
syntax inside an attached `if`. Independently, lifecycle balance validation counts regex braces
as block syntax: even a programmatically constructed spec with `/}/` fails validation. Ordinary
grouped-regex and quoted-pattern controls execute successfully. The exact five-case probe and
...
reconstruction of every earlier note and manifest record. At the capacity checkpoint, the retained note file
was 253 lines / 27,043 bytes; the manifest is 26 lines / 15,618 bytes. All 22
...
Containment .9 committed at 9c644ecb with exact canonical proof: all nine doctrines,
both CLI environments at 66/66, and Phase 0 at 1,032/1,032. Its clean handoff resumed
Dart reading. Parser behavior and prior defect owners remain unchanged.
...
The private top-level seam proves equal native, normalized-descriptor, generated-plan, and independently loaded
...
discovery. Shared Lua `.14.7.7.0-.4` now freeze, implement, and admit one stable Lua-5.1 source and its exclusive inert marker,
live-range-proven typed provenance, caller-frozen pure resolver/cache, complete-depth target reservation/order/
...
five admitted backend regressions advance it to 75. Dedicated observation recurrence adds twelve topology,
storage, registration, and recurrence regressions for the current total of 87.
...
Their neutral artifact/checker is executable at 134 current + 4 dedicated ActionIR rows, 250 call rows, and 58
...
lossless-gap and transaction-safety composition, progressive recurrence advances their rollout to 12 complete /
2 pending. The transaction row projects the separately owned 9/9 six-runtime recognition authority; staged
dispatch and combined public no-drift retain their existing owners.
...
rollout change. Lossless-gap composition `.14.5.1` then binds the separately owned 9/0/63 plus public 8/15/10/34
gap authority to the already-detached same-source half-open `gap_span`, adds one rooted composed route and twelve
drift mutations, and advances only `lossless_gap_composition`. Corrective `.14.6.0.1` then binds the independently
complete recognition authority—9/9 rollout, 58 mutations, five backend sources over six runtime routes, closed
effects and cursor-progress proof—into `transaction_safety` for 11/3/152 truth at that boundary. Progressive
recurrence `.14.6.7` binds five sources to six routes plus four support ledgers, promotes only
`progressive_span_dispatch`, and made typed truth 12/2/170 at that boundary without behavior or public-surface
movement. Final `.14.8` now makes current aggregate truth 14/0/231.
...
same detached value without moving the parent cursor. The exact consumer passes 178/178 on both ABIs;
...
passes 125 assertions and now runs once in canonical CI; admission `.2.3` promotes only Perl while generated
format, typed rollout, shared helper inventories, recurrence, and outward surfaces remain unchanged.
...
and five-backend repeated-action composition. Recurrence then passes Perl 7, Rust 7, Dart 7, Julia 30, and Lua 43
per ABI plus all support ledgers through `tools/check_recursive_observation_six_runtime.sh`.
...
and canonical CI executes that exact target once. Its admission advanced only the Rust row.

Dart now has an admitted package-private end-to-end implementation. Four dedicated non-eager ActionIR nodes,
...
authority boundary passes 187 assertions per ABI, and the unchanged admitted consumer passes 246 on each: four
...
and independently loaded emitted source. Lua keeps those migration spellings outside the shared 250-name current
...
modules, and semantic signatures. Every route compiles and runs the same typed record through the ordinary
interpreter; emitted source adds no host closure, callable codec, or route-specific executor.

An ordinary bound `cb(args)` still runs after static callables with once-only arguments, copied/restored fixed/rest
bindings, live caller nonparameters, local results/access/chaining, exact typed failures, and ordered recursion.
Built-in helper/receiver/tree final slots now accept contextual, explicit, or bound callbacks through the same
executor. Callback lookup happens before scoped `value` is installed, and distinct nested anonymous callbacks do
not become a false helper-name recursion cycle. Only ordinary bound names participate in recursion tracking.

The focused consumer passes 449 assertions per ABI across construction, invocation, contextual equivalence,
native/reconstructed/generated/fresh-emitted identity, exact generated wrapper/runtime detail, corrupt-payload
...
with 54 rejected mutations and a 24-document public contract. Run
...
- **Write-vivification and explicit receiver mutation rollout** - ADR `0036` and `FUTURE-PARITY-BACKLOG.19`
  ...
  pre-evaluation guarded write forms, and strict nested-write composition. Lua `.19.6.1-.2` now implement both
  mechanisms through shared typed native, reconstructed, generated-plan, emitted-module, primary-CLI, and
  user-function-body routes on PUC Lua and LuaJIT. Nested writes pass 438 assertions per ABI; `map_leaves!` passes
  530 per ABI with all 18 guarded write forms and unchanged six-callback/one-continuation composition. Parent
  `.19.6` is closed. `.19.7` admits two exact capability rows and advances the current census to 20 capabilities /
  100 pass / 0 partial / 0 gap. `.19.8` completes exact six-runtime recurrence through one project-routed driver,
  three support ledgers, 17 topology mutations, and exact-one canonical CI. `.19.9` publishes the exact current
  examples, adds recurring public no-drift, and closes parent `.19`. Its mandatory notes rollover is bounded by ADR `0101` at 23 collection files / 22
  manifest lines without changing any byte or aggregate ceiling.
...
- **Julia callable-codeblock construction and invocation** - `FUTURE-PARITY-BACKLOG.11.6.1` recognizes exact `{|params| body }`
  and `{|| body }` before Julia's harray/eager-block classifier and preserves the neutral eight-field record as
  inert recursively copied data. Fixed/final-rest signatures, typed ActionIR, exact source, containing
  Unicode-character literal/body spans, nested literals, all nine malformed codes, user-function transport,
  compiled JSON, generated plans, normalized reconstruction, a freshly loaded emitted module, and semantic
  `codeblock` shapes agree. Deferred bodies are excluded from eager contract and removed-selector scans; no Julia
  closure or environment capture exists. Completed `.11.6.2` adds static-precedence bound-name dispatch,
  once-only left-to-right arguments, recursively copied/restored fixed/rest bindings, live nonparameter caller
