...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  direct/mutual recursion. The full suite passes with 671 assertions, package status is
  `runtime-user-functions`, and `.5.3` descriptor-shape parity is active.
