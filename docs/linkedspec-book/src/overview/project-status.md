...
- **Generated-source convergence** — the interpreter oracle remains primary. `.3.1.2` repairs Perl's independently loaded source with canonical dependency reconstruction, public/legacy deterministic emission, validated family plans, trace/error/identity roles, and exact indexed execution. Rust covers every family and an eight-case subset, but not all 105; Dart/Julia have no emitter. Census stays 56/2/2 until active Perl admission `.3.1.3`, then Rust `.3.2`, Dart `.3.3`, Julia `.3.4`, and `.3.5`. Host-source bytes may differ; observations may not.
