...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 5 complete / 4 pending: Perl, Rust, Dart, and Julia are admitted privately, while the two Lua runtimes and both public rows remain pending. Shared Lua `.6.1` now implements logical source identity, pinned-Unicode named/anonymous declarations, selector/directive provenance, exact diagnostics, compiled slot/directive/five-field identity, and one 178-assertion dual-ABI consumer guarded by ten dormancy mutations. `.6.2-.6.5` retain the one recognition-frame/token native path, normalized/descriptor/generated-v2 execution, fresh-process emitted proof with storage 18→19, and existing-primary/nine-role admission. Existing rule-slot events remain post-action/pre-`LE`; dedicated capture-enabled preselection alone precedes `LS`. The checker stays at 58 mutations plus ten Rust, ten Dart, ten Julia admission, and ten Lua dormancy mutations. Legacy descriptors/refs, format 2 `{label,family}` plans, 246 supported ActionIR names, storage 18, recognition 137/246/58, 122 public helpers, typed source 9/5/114, and every outward surface remain unchanged.
