...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Governance is current at 5 complete / 4 pending: Perl, Rust, Dart, and Julia are admitted privately, while PUC Lua, LuaJIT, recurring, and public rows remain pending. Shared Lua behavior-free plan `.6.0` freezes five leaves: authored/static/compiled metadata and dormancy; one recognition-frame/token native path; normalized/descriptor/generated-v2 carriers; fresh-process dual-ABI emitted proof with storage 18→19; then existing-primary/nine-role admission. Existing rule-slot events remain post-action/pre-`LE`; dedicated capture-enabled preselection alone precedes `LS`. The checker stays at 58 mutations plus ten Rust, ten Dart, and ten Julia admission mutations. Loaded/inline identity, legacy descriptors/refs, format 2 `{label,family}` plans, 246 supported ActionIR names, recognition 137/246/58, 122 public helpers, typed source 9/5/114, and every outward surface remain unchanged.
