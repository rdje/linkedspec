...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 4 complete / 5 pending: Perl, Rust, and Dart are admitted privately, while later runtimes and both public rows remain pending. The checker retains 57 mutations plus ten Rust and ten Dart admission mutations. Behavior-free Julia `.5.0` now freezes `.5.1-.5.5`: logical/authored metadata and dormancy; private state on the existing recognition authority; normalized reconstruction, compatible descriptors, and unchanged-v2 generated execution; one repository-routed emitted host; then existing-primary exact nine-role admission. Julia behavior and rollout do not move in `.5.0`; both Lua ABIs and both public rows remain pending. Recognition stays 137/246/58 with 122 public helpers; typed source stays 9/5/114; generated format 2 `{label,family}` and legacy descriptor/reference fields remain unchanged; outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
