...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred/future work. Lua input/live-cursor
  ...
  v1/v2-only generated scaffold scope against outward descriptor v3 and splits exact-v1/v2/v3 emitter core
  `.8.1.1` from fresh-process PUC Lua/LuaJIT proof `.8.1.2`. `.8.1.1` is active, no emitter is claimed yet, and
  the capability census remains four-backend 64/0/0 until `.8.4`.
  Cross-backend
...
- **Generated-source convergence** — closed for Perl, Rust, Dart, and Julia when the census reached 60/0/0; later punctuation-light admission makes the broader live census 64/0/0. The interpreter oracle remains primary. Those four backends pass contract v1. Rust's strict recurring classifier compiles/runs all 105 generated fixtures. Dart and Julia each have exact ten-family direct routing, four plan rejections, portable trace roles, isolated all-family proof, and contract-sourced interpreter-first 8/105 host admission. Julia additionally locks Unicode-safe strict-UTF-8/hex payloads and typed metadata/errors. Host-source bytes may differ; observations may not. Lua already passes native 169/169, primary CLI 61x2, and interpreter corpus 105/105. Planning `.8.1.0` found that the original scaffold text predated outward descriptor v3, so `.8.1.1` now owns deterministic source from the complete fixed-v1/variadic-v2/final-codeblock-v3 effective state and `.8.1.2` owns fresh-process PUC Lua/LuaJIT proof. No Lua emitter is claimed before those leaves, and final admission remains `.8.4`.
