...
- **Generated-source convergence** — the interpreter oracle remains primary. Perl, Rust, and Dart pass contract v1. Rust's strict recurring classifier compiles/runs all 105 generated fixtures. Dart has exact ten-family direct routing, four plan rejections, portable trace roles, isolated all-family proof, and contract-sourced interpreter-first 8/105 host admission. Julia now has deterministic v1 emission, strict-UTF-8/hex Unicode-safe payloads, typed metadata/errors, direct/traced entrypoints, and an isolated 18-assertion include/run/failure proof. Census remains 59/0/1 until Julia's active family-plan/direct execution `.3.4.2` and later manifest admission pass. Host-source bytes may differ; observations may not.
