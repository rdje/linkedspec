...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 5 complete / 4 pending: Perl, Rust, Dart, and Julia are admitted privately, while the two Lua runtimes and both public rows remain pending. The checker retains 58 mutations plus ten Rust, ten Dart, and ten Julia admission mutations. Julia `.5.1-.5.5` implements logical source identity, pinned-Unicode declarations/selectors, exact static diagnostics, private same-recognition-authority state/lifecycle, normalized reconstruction, compatible descriptor projections, same-runtime generated-v2 execution, independently loaded emitted direct/traced proof, and an exact nine-role admission through the existing primary adapter. Explicit proof retains 105 metadata + 33 native + 46 carrier + 105 emitted assertions across ten value and two typed-error modules; storage is exact 20 owners / 5 packages with cleanup. Loaded/inline identity, legacy descriptors/refs, format 2 `{label,family}` plans, and the exact 246 supported ActionIR names remain unchanged. Recognition stays 137/246/58 with 122 public helpers; typed source stays 9/5/114; outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
