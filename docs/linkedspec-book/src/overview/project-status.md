...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  rollout, and `.6.2.1` adds ordered named/offset/limit selection plus bounded runner PASS/FAIL reporting. Full
  tests now pass with 745 assertions and status `runtime-corpus-selection`; `.6.2.2` starter fixtures 0–39 are
  active. Unbounded CLI execution and full parity remain later work.
