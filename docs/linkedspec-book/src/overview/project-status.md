...
- **Generated-source convergence** — the interpreter oracle remains primary. `.3.1` is closed: Perl's independently loaded contract-v1 source is admitted pass, and Rust's identity/metadata/errors, exact ten-family plan/four rejections, direct result, portable trace, and all-family execution are admitted. The scalable `.3.2.0` classifier compiles/runs all 105 generated fixtures with zero failures, and `.3.2.1` closes the empty repair inventory; census remains 57/1/2 until `.3.2.2` makes that proof a strict recurring admission gate. Dart/Julia have no emitter. Host-source bytes may differ; observations may not.
