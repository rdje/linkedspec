...
- **Generated-source convergence** — the interpreter oracle remains primary. Perl and Rust pass contract v1. Rust's strict recurring classifier compiles/runs all 105 generated fixtures, the contract checker prevents ignore/conditional drift, and complete package/CLI admission gates pass. Dart's deterministic v1 scaffold now passes isolated offline compile/run and its complete 178-test/61x2/105 gate; family-plan `.3.3.2` is active before corpus admission. Census remains 58/0/2 and Julia follows. Host-source bytes may differ; observations may not.
