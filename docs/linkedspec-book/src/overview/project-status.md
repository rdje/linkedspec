...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred/future work. Lua input/live-cursor
  ...
  `runtime-corpus-primary-cli`; final no-drift `.7.3` closes parent `.7`, generated-source scaffold `.8.1` is
  active, and the capability census remains four-backend 64/0/0 until `.8.4`.
  Cross-backend
