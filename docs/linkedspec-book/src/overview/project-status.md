...
parsing; deeper PCRE structural constructs remain split follow-up work. Full shipped 99-fixture corpus parity is
still in progress.
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  diagnostic run. The basic regex-dialect bridge and helper/action bridge are done, while deeper PCRE structural
  constructs remain routed to a follow-up; the next Dart frontier is recursive/default-mode semantics, followed by
  residual parity and cross-backend gates.
