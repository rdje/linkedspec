...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  The full suite passes with 588 assertions, package status is `runtime-diagnostics`, and the active frontier is
  `.4.5.2` for trace controls/events/sinks.
