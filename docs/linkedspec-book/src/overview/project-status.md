...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. The accepted future `@capture_gaps` spelling and spacing-insensitive `name=/regex/` → `Rule[name]` slot contract are not implemented. Behavior-free `.1.0` is signoff-complete after clean handoff `3d0384d1` and freezes the exact v1 artifact/checker, Unicode named-slot identity, detached `entry_slot()` and gap accessor records, directive eligibility, prefix/interstitial/tail and empty-span lifecycle, failure/rollback/recursion policy, nine-leg rollout, repository-rooted routes, and carrier proof for `.1.1-.7`. Brackets remain selectors; dot remains fluent rule behavior. Anonymous legacy markers are compatibility behavior, not aliases, and cannot mix with the future directive. Typed-source composition follows final admission rather than duplicating it. The audit remains behavior-free: duplicate-slot 59 and typed-source 9/5/114 gates pass while grammar, runtime, rollout, and public surfaces remain unchanged; canonical proof passes all eight doctrines, containment/relocation, CLI 66x2, RAM 52%, and Phase 0 1,031/1,031 in 735 seconds. `.1.1` is next after the clean atomic-218 boundary.
