...
- **Generated-source convergence** — the interpreter oracle remains primary. Perl and Rust pass contract v1. Rust's strict recurring classifier compiles/runs all 105 generated fixtures, the contract checker prevents ignore/conditional drift, and complete package/CLI admission gates pass. Census is 58/0/2; Dart scaffold `.3.3.1` is active and Julia follows. Host-source bytes may differ; observations may not.
