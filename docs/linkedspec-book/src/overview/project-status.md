...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 4 complete / 5 pending: Perl, Rust, and Dart are admitted privately, while later runtimes and both public rows remain pending. The checker retains 57 mutations plus ten Rust and ten Dart admission mutations and ten Julia dormancy mutations. Julia `.5.1` implements logical source identity, pinned-Unicode named/anonymous declarations, selector/directive validation, exact static diagnostics, and compiled slot/directive/five-field edge provenance. Julia `.5.2` now adds private same-recognition-authority gap state, lifecycle, rollback/nesting, scalar spans, and four private zero-argument accessors behind that dormant consumer. The helpers do not widen the exact 246 supported ActionIR names. Reconstruction/descriptors/generated execution `.5.3`, emitted proof `.5.4`, and primary/admission `.5.5` remain pending. Recognition stays 137/246/58 with 122 public helpers; typed source stays 9/5/114; generated format 2 `{label,family}` and legacy descriptor/reference fields remain unchanged; outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
