...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. `FUTURE-PARITY-BACKLOG.1.1` scoped Dart into `DART-BACKEND-PARITY`; `DART-BACKEND-PARITY.7.5` closes that scoped milestone. `FUTURE-PARITY-BACKLOG.1.2` scoped Julia into `JULIA-BACKEND-PARITY`; `JULIA-BACKEND-PARITY.1.1` completed the Julia toolchain/package-layout preflight, so active Julia work advances to `.1.2` for the minimal package scaffold. Lua remains scheduled after Julia. Each backend variant is expected to own a distinct LinkedSpec CLI entrypoint. The backlog also parks a future spec-derived closed-loop validation arc: both a parser and a stimuli generator should be derived from the same `.spec` source of truth, with design work required before any implementation. A separate parked language-design arc captures the corrected edge-default model: AND rules should default bare entries to blind-call sequence semantics, while OR/default rules should default bare entries to action-edge regex-dispatch semantics.
...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  official current stable release and recorded the intended `julia/` package layout, package/test commands,
  optional formatter/linter commands, `julia/bin/linkedspec_julia.jl` CLI, and `julia/bin/corpus_runner.jl` corpus
  runner. The active frontier is `.1.2` for the minimal package scaffold.
