...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Dart and Julia both have green 99/99 interpreter milestones. ADR `0023` owns the stricter completion boundary. Julia `.7.3.2.3` now closes native primary execution and canonical direct JSON after trace/argument preparation; `.7.3.2.4` is active for normalized failures/exits/trace routing before conformance. Global `.1.5` owns Perl/Rust/Dart/four-backend CLI convergence, `.1.6` the full public capability census, and `.3` public generated-source parity. Lua follows after current-backend convergence and inherits both native embedding and exact interface gates.
...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  primary execution and recursively key-sorted direct JSON with 22 focused assertions; 942 package assertions and
  99/99 pass. `.7.3.2.4` is active for normalized failures/exits/trace routing. Public generated source remains deferred to `.3` and blocks a complete Julia
