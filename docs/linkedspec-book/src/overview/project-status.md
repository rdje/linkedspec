...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Perl is privately admitted through authored, live, and generated execution. Rust `.3.1-.3.4` now implement exact Unicode-17 named/anonymous declarations, all three selector forms, directive eligibility/diagnostics, compiled source/line plus five-field provenance, native invocation state, ordinary reconstruction, compatible descriptors, separate generated-plan parity, and independently compiled emitted direct/traced parity. The final Rust consumer remains explicitly ignored until `.3.5`, so primary and canonical/recurring/facade execution remain absent. Inter-match gap-capture governance is current at 2 complete / 7 pending: Perl is admitted privately, while later runtimes and both public rows remain pending. The neutral checker remains at 56 semantic mutations and adds ten local Rust dormancy mutations. Recognition remains 137 rows / 246 calls / 58 mutations with 122 public helpers; typed source remains 9/5/114; generated plan v2 stays exact `{label,family}` and outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
