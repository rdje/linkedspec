...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  ActionIR bodies, and descriptor function order/count through the same executable compiled state. The full suite
  passes with 691 assertions, package status remains `runtime-user-functions`, `.5` is closed, and `.6.1`
