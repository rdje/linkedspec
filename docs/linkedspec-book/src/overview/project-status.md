...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. `FUTURE-PARITY-BACKLOG.1.1` scoped Dart into `DART-BACKEND-PARITY`; `DART-BACKEND-PARITY.7.5` closes that scoped milestone. `FUTURE-PARITY-BACKLOG.1.2` scoped Julia into `JULIA-BACKEND-PARITY`; `JULIA-BACKEND-PARITY.1.1` completed the Julia toolchain/package-layout preflight, `.1.2` created the minimal Julia package scaffold, `.1.3` added manifest-backed corpus IO/drift guards, `.2.1` added source AST/data types with neutral JSON projection, `.2.2` added the source parser, `.2.3` added frontend validation and strict syntax behavior, `.2.4` added spec-shaped top-level `fn` shell projection, `.3.1` added typed helper/action AST parsing, `.3.2` added ActionIR contract resolution and diagnostics, `.3.3` added the user-function registry seam with staged function-body parse-job records, `.3.4` added compiled-spec/interpreter-state records plus descriptor projection, `.4.1` added runtime regex matching and match-state tracking, `.4.2` added first executable compiled-rule dispatch, `.4.3.0` split helper/value work by mechanism, `.4.3.1` added core typed stores, assignments/access, snapshots, and capture maps/positions, `.4.3.2` added current string/scalar and numeric helpers plus compatible receiver chains, `.4.3.3` added copied array pipelines, split/flatten/reducer bridges, and statement-only end mutations, `.4.3.4` added copied hash views/transforms, explicit splicing, direct assignment, and statement-only named set-key mutation, `.4.3.5` added expression-valued blocks, attached/marker/inline controls, immediate with-blocks, and scoped hash/array tree callbacks, `.4.3.6` closed helper/value no-drift at 567 assertions without a runtime correction, and `.4.4` added explicit cursor save/restore, entry/local rewinds, character-based cursor/input helpers, and non-consuming boundary capture at 581 assertions. `.4.5.0` split diagnostics/trace before code, `.4.5.1` added structured runtime diagnostics, and `.4.5.2` added ordered trace levels, environment/config controls, structured events, stdout/route/mirror sinks, and output-preserving traced entrypoints at 617 assertions. `.4.5.3` added rule/regex/dispatch/lifecycle/recursion/cursor/boundary trace events with output identity at 631 assertions, and `.4.5.4` closed diagnostics/trace no-drift without a source correction. `.5.1` added deterministic built-in ActionIR-body dispatch, portable cache/compiled/result records, and immutable body_ast stitching at 662 assertions. `.5.2` added registered exact-arity execution with eager caller args, fresh typed local stores, receiver/drop behavior, and recursion diagnostics at 671 assertions. `.5.3` locks neutral function payload/job/AST and descriptor metadata through executable compiled state at 691 assertions. `.6.1` adds controlled manifest-to-runtime execution, one-level wrapped structural comparison, optional trace lines, structured diagnostics, and all-fixture reporting at 715 assertions with status `runtime-controlled-corpus`. `.6.2.0` splits the 99-fixture rollout into bounded selection/reporting, starter 0–39, middle non-function 40–67, shipped-spec/parser-smoke 68–98, and spec-defined function-shell owners; `.6.2.1` is active. Lua remains scheduled after Julia. Each backend variant is expected to own a distinct LinkedSpec CLI entrypoint. The backlog also parks a future spec-derived closed-loop validation arc: both a parser and a stimuli generator should be derived from the same `.spec` source of truth, with design work required before any implementation. A separate parked language-design arc captures the corrected edge-default model: AND rules should default bare entries to blind-call sequence semantics, while OR/default rules should default bare entries to action-edge regex-dispatch semantics.
- **Dart backend parity** - `DART-BACKEND-PARITY` is complete for the scoped interpreter-first Dart milestone. Its strategy is
  ...
  and curated-corpus proof prerequisites. Dart-specific CLI productization is complete: `bin/linkedspec_dart.dart`
  owns help text plus a `corpus` command that validates or executes the manifest-backed corpus through Dart
  parse/compile/runtime, while `bin/corpus_runner.dart` remains a compatibility wrapper. No active Dart frontier
  remains in this tree; generated Dart source stays deferred to a future split proof lane.
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  passes with 715 assertions and package status `runtime-controlled-corpus`. `.6.2.0` splits the 99-fixture rollout
  into bounded selection/reporting, starter 0–39, middle non-function 40–67, shipped-spec/parser-smoke 68–98, and
  spec-defined function-shell owners; `.6.2.1` is active. CLI `--execute` and full parity remain later work.
