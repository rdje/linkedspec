...
- **Generated-source convergence** — the interpreter oracle remains primary. `.3.1` is closed: Perl's independently loaded contract-v1 source is admitted pass, and Rust's identity/metadata/errors, exact ten-family plan/four rejections, direct result, portable trace, and all-family execution are admitted. The scalable `.3.2.0` classifier compiles/runs all 105 generated fixtures with zero failures; census remains 57/1/2 until `.3.2.1-.2` close classification and make it a strict recurring admission gate. Dart/Julia have no emitter. Host-source bytes may differ; observations may not.
