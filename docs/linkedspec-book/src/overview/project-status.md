...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Unicode 17 full-default casing and the typed scalar-to-text contract are exact across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT. Lua scalar/string parity is closed at 76/76 on both ABIs with strict PCRE2-backed `matches`, pure split, scalar regex mutation, and explicit array split replacement. Perl, Rust, Dart, and Julia now pass all 55 strict scalar numeric v1 cases; Lua numeric admission is ready. The variadic audit confirms purposefully unbounded built-ins already use open upper bounds, while user functions require a versioned grammar/staged/descriptor/runtime signature; neutral contract design is active before backend code. Exact broader shipped cases remain unchanged under dependency-complete phase 6.
