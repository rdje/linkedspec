...
window is now 22/31 green and split into focused residual leaves after the regex-dialect, helper/action,
recursion/default-mode, portmap result-shape, hlink delimiter/capture, and helper mutation/text-normalization
bridges. Basic
...
recursive top-rule, hlink, tablegrep, simenv, and `lib_reader` parser-smoke fixtures pass. Deeper PCRE structural
constructs and legacy structural-output mismatches remain split follow-up work. Full shipped 99-fixture corpus
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  diagnostic run and is now 22/31 green. The basic regex-dialect bridge, helper/action bridge,
  recursive/default-mode bridge, portmap result-shape bridge, hlink delimiter/capture bridge, and helper
  mutation/text-normalization bridge are done, while deeper PCRE structural constructs remain routed to a
  follow-up; the next Dart frontier is legacy structural smoke output parity, followed by residual closeout and
  cross-backend gates.
