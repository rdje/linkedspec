...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred/future work. Lua input/live-cursor
  ...
  measures exact offsets 0-39 plus 99-104 at 45/46 on both ABIs: every governed capability fixture and 39/40 core
  fixtures pass. Offset 20 alone exposes nested-assignment key/index segment-kind erasure; repair `.6.1.1`, library
  execution `.6.1.2`, core admission `.6.1.3`, and capability/no-drift `.6.1.4` are dependency-ordered. The
  capability census remains 64/0/0. Cross-backend
...
- **Lua staged-function frontier** - Planning `.5.1.0` separates minimal action-body dispatch `.5.1.1`, fixed-v1
  ...
  `.6.1.0` measures the owned windows at 45/46 on both ABIs and activates nested segment-kind repair `.6.1.1`.
