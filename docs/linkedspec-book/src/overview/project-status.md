...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred/future work. Lua input/live-cursor
  ...
  `.5.1.1` passes 130/130; fixed-v1 execution `.5.1.2` passes 133/133 with public status
  `runtime-user-functions-fixed-v1`; variadic-v2 state `.5.1.3.1` is active, and full native-loading/frontend/
...
- **Lua staged-function frontier** - Planning `.5.1.0` separates minimal action-body dispatch `.5.1.1`, fixed-v1
  ...
  fixed calls, copied fresh stores, local returns, composition, and typed failure fences at 133/133; `.5.1.3.1` is
  active. Descriptors/full trace remain `.5.3`, generated Lua remains `.8`, and explicit callable literals/bound
  calls remain `.11.7`.
