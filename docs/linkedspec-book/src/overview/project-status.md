...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Dart and Julia both have green 99/99 interpreter milestones. ADR `0023` owns the stricter completion boundary. `.1.5.1.1` now provides the reusable byte-exact manifest/runner and backend-neutral Perl help baseline; `.1.5.1.2` owns strict argument/usage cases before success, failure, and trace families. Julia remains active/delegated rather than complete. Global `.1.6` owns the full capability census and `.3` generated-source parity. Lua follows current-backend convergence and inherits both native embedding and exact interface gates.
