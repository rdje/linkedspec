...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, all non-codegen capability work `.1.6`, and generated-source `.3` are closed at 60 pass / zero partial / zero gap for Perl, Rust, Dart, and Julia. Lua now has a dependency-free native scaffold, strict typed JSON, exact 105-case manifest/fixture validation, and lossless typed source/provenance AST data on PUC Lua and LuaJIT. Parser execution remains absent; universal rule parser `.2.2` is active.
