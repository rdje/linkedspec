...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Dart and Julia both have green 99/99 interpreter milestones. ADR `0023` owns the stricter completion boundary. Julia `.7.3.3` closes its local primary-CLI no-drift audit at `runtime-corpus-primary-cli`, but the Julia tree remains active/delegated rather than complete. Global `.1.5` owns Perl/Rust/Dart/four-backend CLI convergence, `.1.6` the full public capability census, and `.3` public generated-source parity; `.1.5.1` is next. Lua follows current-backend convergence and inherits both native embedding and exact interface gates.
