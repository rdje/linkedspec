...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Perl, Rust, and Dart are privately admitted across native, reconstructed/generated/emitted, lifecycle, recursion/rollback, diagnostics, and primary roles. Inter-match gap-capture governance is current at 4 complete / 5 pending: Perl, Rust, and Dart are admitted privately, while later runtimes and both public rows remain pending. Dart `.4.1-.4.5` provide pinned-Unicode authored provenance, private same-authority native state/accessors, normalized reconstruction, separate compatible descriptor metadata, direct/traced generated-plan execution, independently analyzed/executed emitted libraries, and the exact once-only primary/admission ledger. The neutral checker is 57 semantic mutations plus ten Rust and ten Dart admission/regression mutations. Recognition remains 137 rows / 246 calls / 58 mutations with 122 public helpers; typed source remains 9/5/114; generated plan v2 and legacy descriptor/reference fields stay unchanged, and outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
