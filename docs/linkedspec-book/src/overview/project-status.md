...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. The Perl reference now accepts spacing-insensitive `name=/regex/`, `Rule[name]`, and static `@capture_gaps` metadata with exact Unicode 17 identity and generated dependency provenance; it does not yet execute gap state, accessors, tails, or loaded generated gap behavior. Inter-match gap-capture governance is current at 1 complete / 8 pending, with no runtime or public admission. The executable format-1 neutral artifact locks 55 semantic and governance mutations, while 10 separate dormancy mutations fence the Perl consumer outside canonical and recurring runtime routes. `.2.2` owns same-guard live state, `.2.3` emitted/loaded parity, and `.2.4` Perl-only admission. Brackets remain selectors; dot remains fluent rule behavior. Anonymous legacy markers are compatibility behavior, not aliases, and cannot mix with the directive. Typed-source composition follows final admission rather than duplicating it.
