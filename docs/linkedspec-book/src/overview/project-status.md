...
- **Generated-source convergence** — the interpreter oracle remains the primary cross-variant correctness gate. `.3.1.0` proved Perl's captured source recompiles but loses dependency-regex alternative indexes and is not standalone-equivalent. Rust covers every current structural family and compile/runs an eight-case manifest subset, but not all 105; Dart and Julia have no emitter. Census is 56/2/2. Contract v1 `.3.1.1` is canonically gated; active Perl repair `.3.1.2` precedes admission, Rust `.3.2`, Dart `.3.3`, Julia `.3.4`, and `.3.5`. Host-source bytes may differ; generated execution, results, diagnostics, tracing, and source identity may not.
