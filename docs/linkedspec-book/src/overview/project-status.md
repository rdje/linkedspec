...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, all non-codegen capability work `.1.6`, and generated-source `.3` are closed at 60 pass / zero partial / zero gap for Perl, Rust, Dart, and Julia. Lua now has a dependency-free native scaffold, strict typed JSON, exact 105-case manifest/fixture IO, typed source/provenance AST data, public parsing/validation over all 21 shipped and 102/105 rule-only corpus sources, strict-unused checks, exact 239-name collision coverage, and spec-owned top-level function-node projection with no raw scanner on PUC Lua and LuaJIT. Frontend `.2` is closed and typed ActionIR `.3.1` is active; corpus/runtime/primary CLI execution remains absent.
