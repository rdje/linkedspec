...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 5 complete / 4 pending: Perl, Rust, Dart, and Julia are admitted privately, while the two Lua runtimes and both public rows remain pending. Shared Lua `.6.1-.6.2` now implement logical authored/compiled metadata plus invocation-local candidate/tail state, detached child entry identity, four private accessors, lifecycle timing, nesting, and rollback on the existing recognition frame/token. One 211-assertion dual-ABI consumer includes 33 native assertions and remains guarded by ten dormancy mutations. `.6.3-.6.5` retain normalized/descriptor/generated-v2 execution, fresh-process emitted proof with storage 18→19, and existing-primary/nine-role admission. Existing rule-slot events remain post-action/pre-`LE`; dedicated capture-enabled preselection alone precedes `LS`. The checker stays at 58 mutations plus ten Rust, ten Dart, ten Julia admission, and ten Lua dormancy mutations. Legacy descriptors/refs, format 2 `{label,family}` plans, 246 supported ActionIR names, storage 18, recognition 137/246/58, 122 public helpers, typed source 9/5/114, and every outward surface remain unchanged.
