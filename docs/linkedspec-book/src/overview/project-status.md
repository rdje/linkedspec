...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 5 complete / 4 pending: Perl, Rust, Dart, and Julia are admitted privately, while the two Lua runtimes and both public rows remain pending. Shared Lua `.6.1-.6.4` now implement logical authored/compiled metadata; invocation-local candidate/tail state, detached child entry identity, four private accessors, lifecycle timing, nesting, and rollback on the existing recognition frame/token; normalized reconstruction; detached compatible descriptor projections; same-engine direct/traced generated-v2 execution; and ten value/two typed-error modules independently loaded in fresh PUC-Lua and LuaJIT children. One 362-assertion dual-ABI consumer includes 33 native, 46 carrier, and 105 emitted assertions and remains guarded by ten dormancy mutations. `.6.5` retains existing-primary/nine-role admission. Existing rule-slot events remain post-action/pre-`LE`; dedicated capture-enabled preselection alone precedes `LS`. The checker stays at 58 mutations plus ten Rust, ten Dart, ten Julia admission, and ten Lua dormancy mutations. Legacy descriptor fields/refs, format 2 `{label,family}` plans, 246 supported ActionIR names, storage 19/three native modules, recognition 137/246/58, 122 public helpers, typed source 9/5/114, and every outward surface remain unchanged.
