...
`push(child, index)` preserves EBNF logging payloads. Dart full-corpus parity is now 99/99 green; the next Dart
frontier is verification integration and cross-backend handoff cleanup.
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  forms. The next Dart frontier is wiring the green corpus gate into the local verification story, followed by
  cross-backend gates.
