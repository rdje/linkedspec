...
- **Generated-source convergence** — the interpreter oracle remains primary. Perl, Rust, Dart, and Julia pass contract v1; census is 60/0/0. Rust's strict recurring classifier compiles/runs all 105 generated fixtures. Dart and Julia each have exact ten-family direct routing, four plan rejections, portable trace roles, isolated all-family proof, and contract-sourced interpreter-first 8/105 host admission. Julia additionally locks Unicode-safe strict-UTF-8/hex payloads and typed metadata/errors. Final four-backend closeout `.3.5` is active. Host-source bytes may differ; observations may not.
