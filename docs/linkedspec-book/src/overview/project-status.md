...
- **Generated-source convergence** — the interpreter oracle remains primary. `.3.1` is closed: Perl's independently loaded contract-v1 source is admitted pass, and Rust's identity/metadata/errors, exact ten-family plan/four rejections, direct result, portable trace, all-family execution, and eight-case subset are admitted. Census is 57/1/2; Rust remains partial solely for generated breadth 8/105, with scalable classifier `.3.2.0` active. Dart/Julia have no emitter. Host-source bytes may differ; observations may not.
