...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. ADR `0023` owns exact user behavior/CLI, ADR `0024` canonical trace, and ADR `0025` strict preserved UTF-8 text. `.1.5.1.6.0` proves decoded Perl native Unicode is correct and isolates mojibake to the adapter; `.6.1` runner hex bytes, `.6.2` Perl decoding/fixtures, and `.6.3` no-drift precede Rust `.1.5.2`. Dart and Julia remain scoped 99/99 milestones, not complete parity. Global `.1.6` owns capability census and `.3` generated-source parity. Lua follows convergence.
