...
effective/basis trace, portable loader/generated failures, generated-plan-first validation, and unchanged v1
...
- **Generated-source convergence** — closed for Perl, Rust, Dart, and Julia when the census reached 60/0/0; punctuation-light admission later raised the four-backend census to 64/0/0, and Lua `.8.4` now makes the broader live census five-backend 80/0/0. The interpreter oracle remains primary. All five backends pass the shared contract-v1 semantic capability ledger; current Perl, Rust, and Dart artifacts identify generated-source v2 for rule-local cursor reconstruction. Rust's strict recurring classifier compiles/runs all 105 generated fixtures. Dart, Julia, and Lua each have exact ten-family routing, four plan rejections, portable trace roles, isolated all-family proof, and contract-sourced interpreter-first 8/105 host admission; Lua runs fresh PUC Lua and LuaJIT hosts and preserves exact v1/v2/v3 callable state. Host-source bytes may differ; observations may not.
