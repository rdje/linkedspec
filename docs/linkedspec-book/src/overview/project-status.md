...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Perl and Rust are privately admitted across authored, live, reconstructed/generated/emitted, and primary roles. Inter-match gap-capture governance is current at 3 complete / 6 pending: Perl and Rust are admitted privately, while later runtimes and both public rows remain pending. Dart `.4.1-.4.3` now implement pinned-Unicode authored provenance, private same-authority native state/accessors, normalized reconstruction, separate compatible descriptor metadata, and direct/traced generated-plan execution behind a final-path library-skipped consumer. Emitted `.4.4` and primary/admission `.4.5` remain pending. The neutral checker stays at 56 semantic mutations plus ten Rust admission/regression and ten Dart dormancy mutations. Recognition remains 137 rows / 246 calls / 58 mutations with 122 public helpers; typed source remains 9/5/114; generated plan v2 and legacy descriptor/reference fields stay unchanged, and outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
