...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Dart and Julia both have green 99/99 interpreter milestones. ADR `0023` owns the stricter completion boundary. Julia `.7.3.3` closes local no-drift but leaves the tree active/delegated. `.1.5.1.0` now splits Perl/neutral CLI convergence after proving implicit aliases/positionals/environment policy and failure-stdout drift; `.1.5.1.1` owns the reusable manifest/runner and exact help baseline. Global `.1.6` owns the full capability census and `.3` generated-source parity. Lua follows current-backend convergence and inherits both native embedding and exact interface gates.
