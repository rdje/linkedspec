...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. The Perl reference accepts spacing-insensitive `name=/regex/`, `Rule[name]`, and `@capture_gaps` metadata with exact Unicode 17 identity and generated dependency provenance. Private native-live execution now supplies same-guard prefix/interstitial/tail state, detached entry-slot identity, gap accessors, rollback, and recursion isolation; recognition is 137 rows / 246 calls / 58 mutations with 122 public helpers. This private-live slice is signoff-complete at Phase 0 1,031/1,031 in 756 seconds and the exact opt-in neutral-plus-six-pending gap route. Inter-match gap-capture governance is current at 1 complete / 8 pending, with no runtime or public admission. The executable format-1 neutral artifact locks 55 semantic and governance mutations, while 10 separate dormancy mutations fence the Perl consumer outside canonical and recurring runtime routes. `.2.3` owns emitted/loaded parity, and `.2.4` Perl-only admission. Brackets remain selectors; dot remains fluent rule behavior. Anonymous legacy markers are compatibility behavior, not aliases, and cannot mix with the directive. Typed-source composition follows final admission rather than duplicating it.
