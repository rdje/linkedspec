...
- **Terse `.spec` format evolution** — the active language-evolution track now supports auto-existing working variables, canonical helper renames (`set`, `cat`, `copy`), scalar/array/hash mutation operators, typed primitive literals, newline-or-semicolon statement separators, direct nested access such as `payload["children"][0]["name"]`, and bare scalar reads in return/assignment source slots, mutation key/RHS slots, and direct path atoms such as `return(i)`, `set(out, i)`, `out = i`, `items += value`, `meta[key] = value`, and `payload["children"][i]`.
