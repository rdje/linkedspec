...
Their neutral artifact/checker is executable at 133 current + 4 dedicated ActionIR rows, 246 call rows, and 58
...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. The Perl reference accepts spacing-insensitive `name=/regex/`, `Rule[name]`, and `@capture_gaps` metadata with exact Unicode 17 identity and generated dependency provenance. Private native-live execution supplies same-guard prefix/interstitial/tail state, detached entry-slot identity, gap accessors, rollback, and recursion isolation; signoff-complete `.2.3` carries those values, lifecycle, provenance, rollback, recursion, typed diagnostics, direct entry, and legacy behavior through emitted and independently loaded source in five groups / 138 internal assertions. Generated plan v2 remains unchanged. Admission `.2.4` registers that complete private consumer in ordinary CI and the rooted recurring route without changing outward surfaces; parent `.2` landed as atomic 226 at `eceb15ac`. Behavior-free Rust preflight `.3.0` signoff-completely maps exact parser/compiler/runtime/reconstruction/descriptor/generated/emitted/primary seams into `.3.1-.3.5`: Rust currently supports numeric selectors but not named declarations/selectors, the directive, or gap accessors; no Rust behavior or rollout has moved, and `.3.1` waits for clean atomic 227. Inter-match gap-capture governance is current at 2 complete / 7 pending: Perl is admitted privately, while later runtimes and both public rows remain pending. The format-1 neutral artifact locks 56 semantic/governance mutations, including Perl regression and premature Rust promotion. Recognition remains 137 rows / 246 calls / 58 mutations with 122 public helpers. Brackets remain selectors; dot remains fluent rule behavior. Anonymous legacy markers are compatibility behavior, not aliases, and cannot mix with the directive. Typed-source composition follows final admission rather than duplicating it.
