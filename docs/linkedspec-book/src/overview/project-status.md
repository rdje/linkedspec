...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, exhaustive neutral language/helper proof `.1.6.1`, exact outward descriptors `.1.6.2`, and structured diagnostics `.1.6.3` are closed. The validated 15-capability census now classifies its 60 states as 56 pass, one partial, and three gap. Native resolution contract plus Rust/Dart/Julia APIs `.1.6.4.1-.4` are closed; final admission `.5` is active. A later leaf owns Dart frontend/compiler/staged trace. `.1.6.6` closes non-codegen parity; generated-source `.3` remains separately mandatory before Lua.
