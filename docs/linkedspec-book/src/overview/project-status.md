...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5` is closed at 4x2x61, and `.1.6.0` closes a validated 15-capability census over 60 backend states: 47 pass, five partial-proof, and eight gap. Active `.1.6.1` owns exhaustive neutral language/helper proof; later leaves own Rust descriptor projection, Rust structured runtime diagnostics, native named/file resolution for Rust/Dart/Julia, and Dart frontend/compiler/staged trace. `.1.6.6` closes non-codegen parity; generated-source `.3` remains separately mandatory before Lua.
...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  stderr/exit, and stdout/route/mirror/file/reset/emoji behavior with 75 focused assertions; 1,020 package
