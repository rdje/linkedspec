...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  continuations, markers, comments, and block boundaries. `Pkg.instantiate()`, `Pkg.test()`, CLI help/status, and
  corpus validation commands pass with a writable depot. The active frontier is `.2.3` for frontend validation and
  strict syntax behavior.
