...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  The full suite passes with 581 assertions, package status remains `runtime-cursor-boundary`, and the active
  frontier is `.4.5.1` for structured runtime diagnostics.
