...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  parse-result-neutral diagnostic output and advances simenv/history beyond unsupported `print`. Full tests pass
  with 780 assertions, status is `runtime-corpus-diagnostic-output`, the window remains 18/31, and `.6.2.4.3` is
  active.
