...
window is measured and split for regex-dialect, helper/action, recursion/output, and residual parity work; full
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  diagnostic run; the next Dart frontier is the regex-dialect bridge, followed by helper/action, recursion/output,
