...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Unicode 17 full-default casing and the typed scalar-to-text contract are exact across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT. Lua scalar/string parity is closed at 76/76 on both ABIs with strict PCRE2-backed `matches`, pure split, scalar regex mutation, and explicit array split replacement. Perl, Rust, Dart, and Julia now pass all 55 strict scalar numeric v1 cases; Lua numeric admission is ready. ADR 0030 and the neutral callable-signature contract adopt `fn name(fixed, ...rest)` with a typed rest array and versioned records. Perl, Rust, and Dart now implement the unchanged grammar/staged/descriptor/native/generated-execution contract; Julia is active next, with Lua dependency routing at closeout. Exact broader shipped cases remain unchanged under dependency-complete phase 6.
