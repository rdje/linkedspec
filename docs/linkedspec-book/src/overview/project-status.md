...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  parsing, and keeps direct `parse_spec(...)` rule-only. `Pkg.instantiate()`, `Pkg.test()`, CLI help/status, and
  corpus validation commands pass with a writable depot. The active frontier is `.3.1` for typed helper/action AST
  parsing.
