...
- **Generated-source convergence** — the interpreter oracle remains primary. Perl and Rust pass contract v1. Rust's strict recurring classifier compiles/runs all 105 generated fixtures. Dart's deterministic v1 emitter now has exact ten-family direct routing, four pre-execution plan rejections, portable trace roles, isolated all-family host proof, and a complete 180-test/61x2/105 gate. Accepted-subset admission `.3.3.3` remains active, so census stays 58/0/2; Julia follows. Host-source bytes may differ; observations may not.
