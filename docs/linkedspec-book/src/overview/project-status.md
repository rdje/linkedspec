...
window is now 19/31 green and split into focused residual leaves after the regex-dialect, helper/action,
recursion/default-mode, portmap result-shape, and hlink delimiter/capture bridges. Basic
...
`items = []`, so the tclite, recursive top-rule, hlink, and tablegrep parser-smoke fixtures pass. Deeper PCRE
structural constructs remain split follow-up work. Full shipped 99-fixture corpus parity is still in progress.
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  diagnostic run and is now 19/31 green. The basic regex-dialect bridge, helper/action bridge,
  recursive/default-mode bridge, portmap result-shape bridge, and hlink delimiter/capture bridge are done, while
  deeper PCRE structural constructs remain routed to a follow-up; the next Dart frontier is helper mutation/text
  normalization parity, followed by the structural-output leaf, closeout, and cross-backend gates.
