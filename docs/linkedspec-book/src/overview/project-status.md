...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. The Perl reference accepts spacing-insensitive `name=/regex/`, `Rule[name]`, and `@capture_gaps` metadata with exact Unicode 17 identity and generated dependency provenance. Private native-live execution supplies same-guard prefix/interstitial/tail state, detached entry-slot identity, gap accessors, rollback, and recursion isolation; signoff-complete `.2.3` carries those values, lifecycle, provenance, rollback, recursion, typed diagnostics, direct entry, and legacy behavior through emitted and independently loaded source in five groups / 138 internal assertions. Generated plan v2 remains unchanged. Canonical proof passes Phase 0 at 1,031/1,031 and exact opt-in neutral-plus-six-pending routing. Recognition is 137 rows / 246 calls / 58 mutations with 122 public helpers. Inter-match gap-capture governance is current at 1 complete / 8 pending, with no runtime or public admission. The format-1 neutral artifact locks 55 semantic/governance mutations and 10 separate dormancy mutations fence the Perl consumer outside canonical and recurring runtime routes. `.2.4` alone owns Perl admission. Brackets remain selectors; dot remains fluent rule behavior. Anonymous legacy markers are compatibility behavior, not aliases, and cannot mix with the directive. Typed-source composition follows final admission rather than duplicating it.
