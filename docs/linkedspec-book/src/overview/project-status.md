...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Unicode 17 full-default casing, typed scalar-to-text, and strict scalar numeric v1 are exact across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT. Lua numeric word aliases, symbol callees, number receiver composition/terminality, strict copied-array reducers, ordered copied constructors/literals, explicit flat splicing/concatenation, copied selection/order/membership/uniqueness, transform/join/PCRE2 pipelines, typed mutation/child-result flow, and exact copied `[tag, item, fields...]` record construction pass the 99/99 dual-ABI gate. All 34 non-callback array names and six numeric terminals are closed under `.4.3.4`; all 13 ordinary harray names close at 103/103 under `.4.3.5`. Eager blocks, lazy inline `if`/`switch`, attached/marker if and switch statements, and attached while pass through `.4.3.6.3.3`: selected-only branches, local return, nested markers, one-time switch subjects, state-visible loop conditions, and typed malformed/runaway controls are locked. `.4.3.6.4` adds copied final-codeblock metadata and cleanup-safe helper/receiver `with` in attached and parenthesized form, raising both Lua ABIs to 112/112. `.4.3.6.5.1.0` splits a reproduced Perl callback append-RHS scope collision into reference repair `.1.1` before Lua harray execution `.1.2`. Lua follows Perl-reference truthiness, switch scalar equality, and inner-loop `next`; truthiness, switch comparison/ranges, alias shapes, exact-limit timing, and `next` drift are routed to helper-caveat owner `.5`. General user-function final blocks remain `.5.1`, and first-class callable block values remain future `.11.7`. ADR 0030 and the neutral callable-signature contract adopt `fn name(fixed, ...rest)` with a typed rest array and versioned records. Perl, Rust, Dart, and Julia implement the unchanged grammar/staged/descriptor/native/generated-execution contract. Lua native parity is routed to `.5.1`, descriptors to `.5.3`, and generated preservation/execution/admission to `.8.1-.4`. Exact broader shipped cases remain unchanged under dependency-complete phase 6.
  ...
  five-backend/two-Lua-ABI command. Lua callback work is split at `.4.3.6.5.1.0`; reference repair `.1.1` is active
  before Lua harray execution `.1.2`.
