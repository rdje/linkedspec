...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 4 complete / 5 pending: Perl, Rust, and Dart are admitted privately, while later runtimes and both public rows remain pending. The checker retains 57 mutations plus ten Rust and ten Dart admission mutations and ten Julia dormancy mutations. Julia `.5.1-.5.3` now implement logical source identity, pinned-Unicode named/anonymous declarations, selector/directive validation, exact static diagnostics, private same-recognition-authority gap state/lifecycle, normalized reconstruction, fresh detached descriptor projections, and direct/traced generated-v2 execution through the same runtime. Explicit proof is 105 metadata + 33 native + 46 carrier assertions. Loaded descriptors retain basename logical identity while inline descriptors retain `inline`; legacy `resolved_edges`, `{label,idx}` refs, format 2 `{label,family}` plans, and the exact 246 supported ActionIR names remain unchanged. Emitted proof `.5.4` and primary/admission `.5.5` remain pending. Recognition stays 137/246/58 with 122 public helpers; typed source stays 9/5/114; outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
