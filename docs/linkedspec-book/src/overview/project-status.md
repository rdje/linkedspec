...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, exhaustive neutral language/helper proof `.1.6.1`, exact outward descriptors `.1.6.2`, structured diagnostics `.1.6.3`, exact four-backend native resolution `.1.6.4`, and Dart full-pipeline trace `.1.6.5` are closed. The validated 15-capability census classifies its 60 states as 57 pass, one partial, and two gaps. `.1.6.6` closes non-codegen parity; generated-source `.3` remains separately mandatory before Lua.
