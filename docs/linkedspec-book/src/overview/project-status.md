...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Unicode 17 full-default casing and the typed scalar-to-text contract are exact across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT. Lua passes 76/76 on both ABIs with strict PCRE2-backed `matches`, pure split, scalar regex mutation, and explicit array split replacement. Focused string public no-drift is active under `.4.3.2.2.5.1`; exact shipped cases remain unchanged under dependency-complete phase 6 because they first require later control, output, array, and child-flow owners.
