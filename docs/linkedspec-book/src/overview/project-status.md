...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  `.7.1` has closed public commands, native examples, status, and limitation alignment. `.7.2` owns the separate
  generated-source decision; current trace parity remains runtime-scoped rather than compile/parser-wide.
