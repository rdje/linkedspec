...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Inter-match gap-capture governance is current at 4 complete / 5 pending: Perl, Rust, and Dart are admitted privately, while later runtimes and both public rows remain pending. The checker retains 57 mutations plus ten Rust and ten Dart admission mutations and ten Julia dormancy mutations. Julia `.5.1-.5.4` now implement logical source identity, pinned-Unicode declarations/selectors, exact static diagnostics, private same-recognition-authority state/lifecycle, normalized reconstruction, compatible descriptor projections, same-runtime generated-v2 execution, and independently loaded emitted direct/traced proof. Explicit proof is 105 metadata + 33 native + 46 carrier + 105 emitted assertions across ten value and two typed-error modules; storage is exact 20 owners / 5 packages with cleanup. Loaded/inline identity, legacy descriptors/refs, format 2 `{label,family}` plans, and the exact 246 supported ActionIR names remain unchanged. Primary/admission `.5.5` remains pending. Recognition stays 137/246/58 with 122 public helpers; typed source stays 9/5/114; outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
