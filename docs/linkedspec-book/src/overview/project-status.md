...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  offsets 68–98: 31/31 exact outputs with stable endpoints and zero failures. Full tests pass with 816 assertions,
  status is `runtime-corpus-shipped`, and `.6.2.5` is active for three routed top-level function fixtures.
