...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Dart and Julia both have green 99/99 interpreter milestones. ADR `0023` owns the stricter completion boundary. Julia primary CLI repair is split into trace, arguments/IO, execution/JSON, failure/routing, and conformance leaves; `.7.3.2.1` is active for compile/parser/staged tracing. Global `.1.5` owns Perl/Rust/Dart/four-backend CLI convergence, `.1.6` the full public capability census, and `.3` public generated-source parity. Lua follows after current-backend convergence and inherits both native embedding and exact interface gates.
...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  `.7.3.2.1` is active for compile/parser/staged trace coverage. Public generated source remains deferred to `.3`
  and blocks a complete Julia capability-parity claim.
