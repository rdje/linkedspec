...
- **Inter-match gap capture direction** — ADR `0045` recovers historical “super split” as automatic prefix/interstitial source-gap access around repeated OR/default action edges. Target rules own referenced regex slots and lifecycle code; the enclosing rule owns selection and gap orchestration. Perl and Rust are privately admitted across authored, live, reconstructed/generated/emitted, and primary roles. Inter-match gap-capture governance is current at 3 complete / 6 pending: Perl and Rust are admitted privately, while later runtimes and both public rows remain pending. Behavior-free Dart `.4.0` proves its current named/directive syntax is raw-invalid, its accessors are unknown helpers, and repeated `LS` precedes selection; it freezes authored/static `.4.1`, native `.4.2`, reconstructed/descriptor/generated `.4.3`, emitted `.4.4`, and primary/admission `.4.5` without changing behavior or that rollout. The neutral checker stays at 56 semantic mutations plus ten local Rust admission/regression mutations. Recognition remains 137 rows / 246 calls / 58 mutations with 122 public helpers; typed source remains 9/5/114; generated plan v2 stays exact `{label,family}` and outward surfaces do not move. Brackets remain selectors, dot remains fluent behavior, and legacy markers remain separate compatibility behavior.
