...
- **Future backend parity backlog** - `FUTURE-PARITY-BACKLOG` owns deferred parity work. Exact CLI `.1.5`, all non-codegen capability work `.1.6`, and generated-source `.3` are closed at 60 pass / zero partial / zero gap for Perl, Rust, Dart, and Julia. Lua now has a dependency-free native scaffold, strict typed JSON, exact 105-case manifest/fixture validation, typed source/provenance AST data, and a public rule parser passing all 21 shipped specs plus 102/105 rule-only corpus sources on PUC Lua and LuaJIT. Validation `.2.3` is active; corpus/runtime/primary CLI execution remains absent.
