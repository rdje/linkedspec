...
- **Generated-source convergence** — the interpreter oracle remains the primary cross-variant correctness gate. Rust generated source already covers every current structural family and compile/runs an eight-case manifest subset, but not the complete 105-case manifest; Dart and Julia have no emitter. `FUTURE-PARITY-BACKLOG.3.0` split the work into active neutral executable contract `.3.1`, Rust full-manifest breadth `.3.2`, Dart `.3.3`, Julia `.3.4`, and exact four-backend admission `.3.5`. Host-language source bytes may differ; user-observable generated execution, results, diagnostics, tracing, and source identity may not.
