...
- **Julia backend parity** - `JULIA-BACKEND-PARITY` is the active second future-backend lane. It starts from the
  ...
  passes with 715 assertions, package status is `runtime-controlled-corpus`, and `.6.2` recoverable manifest batches
  are active. CLI `--execute` and full 99-fixture parity remain later work.
