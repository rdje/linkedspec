...
structural constructs remain split follow-up work, and `ds_vhistory_version_entry` is routed to residual closeout
because its checked null object-name oracle conflicts with the current scalar-held direct-access surface. Full
shipped 99-fixture corpus parity is still in progress.
...
- **Dart backend parity** - `DART-BACKEND-PARITY` is the active first future-backend lane. Its strategy is
  ...
  structural constructs remain routed to a follow-up, and the next Dart frontier is residual parser-smoke closeout
  for `ds_vhistory_version_entry`, followed by cross-backend gates.
