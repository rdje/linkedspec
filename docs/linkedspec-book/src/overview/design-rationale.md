...
contract. Perl, Rust, and Dart implement nested creation and `map_leaves!`; Julia implements nested creation but
not yet the bang form. The verified non-bang control remains identical on Perl, Rust, Dart, Julia, and Lua;
Julia, PUC Lua, and LuaJIT retain their generic parser-invocation failure for the bang form.
