...
- clearer non-plugin/domain owners such as `HTTP::FileAccess`, `InteractivePrompt`, `HTML::PathLinks`, `Text::VariableSubstitution`, `VHDL::ConstantEval`, `MSOffice::Excel`, `QC::Flow`, `QC::Summary`, `Timing::SetupHold`, `Timing::StanBackend`, `Timing::StanOmap2430cBackend`, and `Table::GenericFilter` now carry real behavior that used to live in `.plg` files,
...
- setup helpers reused across legacy action files can move too; `plugin/stan_backend.plg`, `plugin/skew.plg`, and `plugin/duty_cycle_degradation.plg` still expose visible actions, but the former `stan_backend_start` setup helper now lives in `Timing::StanBackend::start(...)` rather than the dynamic plugin registry,
