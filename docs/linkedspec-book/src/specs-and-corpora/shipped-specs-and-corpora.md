...
- extracted package owners such as `Plugin::String`, `Plugin::CGI`, `Plugin::GenericFilter`, and clearer non-plugin/domain owners such as `InteractivePrompt`, `VHDL::ConstantEval`, and `MSOffice::Excel` now carry real behavior that used to live in `.plg` files,
