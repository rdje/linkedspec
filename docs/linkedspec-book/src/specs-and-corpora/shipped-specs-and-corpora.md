...
- extracted package owners such as `Plugin::CGI`, plus clearer non-plugin/domain owners such as `InteractivePrompt`, `Text::VariableSubstitution`, `VHDL::ConstantEval`, `MSOffice::Excel`, and `Table::GenericFilter`, now carry real behavior that used to live in `.plg` files,
