...
- extracted package owners such as `Plugin::String`, `Plugin::CGI`, and clearer non-plugin/domain owners such as `InteractivePrompt`, `VHDL::ConstantEval`, `MSOffice::Excel`, and `Table::GenericFilter` now carry real behavior that used to live in `.plg` files,
