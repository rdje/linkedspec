...
- extracted package owners such as `Plugin::String`, `Plugin::CGI`, `Plugin::GenericFilter`, `Plugin::MSOffice`, and clearer non-plugin/domain owners such as `InteractivePrompt` and `VHDL::ConstantEval` now carry real behavior that used to live in `.plg` files,
