...
- extracted package owners such as `Plugin::String`, `Plugin::CGI`, `Plugin::GenericFilter`, `Plugin::MSOffice`, `Plugin::VHDLConst`, and clearer non-plugin owners such as `InteractivePrompt` now carry real behavior that used to live in `.plg` files,
