...
lifecycle fluent chains. Perl and Rust additionally accept the six standalone aliases `else`, `endif`, `default`,
`endcase`, `endswitch`, and `next`, plus a generic final zero-argument receiver segment such as `.trim`; Dart,
Julia, and Lua rollout is explicitly owned by `FUTURE-PARITY-BACKLOG.16`. This narrow surface does not
