...
That distinction is deliberate. `push(...)` says "append exactly what I computed." The explicit guard says "append the computed value only if it survived the emptiness filter." The older `push_nonempty(...)` helper now emits a retired-helper diagnostic on the Perl reference; Rust compatibility remains separately owned by `SPEC-FORMAT-TERSE.8.4`.
