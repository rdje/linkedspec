...
> `set_key(meta, "key", expr)` on Perl, Rust, and Dart, but it is not the same runtime contract: its evaluated string selector
> chooses harray, while a nonnegative integer chooses array. `meta[key] = value` reads working values `key` and
> `value`, and in value positions yields the updated root snapshot. Julia and Lua retain the earlier
...
> Reads retain their existing direct-access interpretation and never create state. For assignment on Perl, Rust, and Dart, every
...
> scalar-held array/harray value. Perl, Rust, and Dart create an absent root and missing intermediates when the
> selector kind determines their shape; bound null and wrong kinds are conflicts, and arrays remain dense.
> Structural failures throw typed diagnostics and commit no partial path. Julia and Lua retain the
...
| `meta[key_expr] = expr` | update one typed path and yield the updated root snapshot when used as an expression | on Perl, Rust, and Dart, an evaluated string selects harray and a nonnegative integer selects array; use `set_key` when harray-only intent must be explicit. Julia and Lua retain their earlier hash-index boundary until admission. |
