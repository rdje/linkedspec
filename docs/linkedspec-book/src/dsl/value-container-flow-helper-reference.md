...
Use `next()` when a rule edge should consume a recognized item, such as a comment, and then skip adding a value to the current accumulator. Perl, Rust, Dart, and Julia also accept the punctuation-light standalone alias `next`; keep `next()` in cross-backend source until the Lua parity lane closes:
