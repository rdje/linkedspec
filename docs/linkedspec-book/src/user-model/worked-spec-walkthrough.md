...
snapshot of that accumulator with `array_copy(array(Top))` — that snapshot (a list) is the parser's
...
- Let the entry rule collect each returned payload (`-> Pair .push`) and return the snapshot (`LX { return(array_copy(array(Top))) }`).
