...
full 95-fixture corpus remains the Rust interpreter oracle gate unless a later
...
boundary: interpreter parity is the 95-fixture corpus contract, while generated
...
95 fixtures, including the `with(...) { ... }` helper and `.with() { ... }` receiver trailing block cases, the two minimal shipped `tclite.spec` cases restored by the
