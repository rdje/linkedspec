...
recursive/default-mode bridges, reached 13/31 green after the portmap result-shape bridge, and is now 19/31 green
after the hlink delimiter/capture bridge. Residual non-PCRE work is split into focused helper mutation/text
normalization, legacy structural smoke output, and closeout leaves. The portmap/action-edge result-shape leaf is
...
scalar-held lists, all five hlink fixtures pass, and `tablegrep_simple_term` also passes. The basic regex-dialect
...
and is now 19/31 green. The basic regex-dialect bridge, helper/action bridge,
recursive/default-mode parser-smoke bridge, portmap result-shape bridge, and
hlink delimiter/capture bridge are done; the next Dart leaf is helper
mutation/text normalization parity, and full shipped 99-fixture corpus parity
remains later.
