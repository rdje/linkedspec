...
| Full native pipeline trace | Perl, Rust, and Julia propagate a caller-owned emitter through frontend/compiler/staged/runtime phases; Dart begins at the interpreter. Audit `.1.6.5.0` splits frontend/compiler `.1`, function/staged `.2`, and composed admission `.3`. | `.1.6.5.1` active |
