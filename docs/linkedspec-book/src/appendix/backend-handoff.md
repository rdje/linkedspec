...
`.10.9.4.0` consumes that clean handoff: a generated private-part binding/runtime, same-process server around
caller-owned `SemanticIndex`, core secure entropy/monotonic expiry, strict duplicate-safe/canonical stdio, exact
admission, and no-change closeout are frozen under `.1-.4`. No Dart production server exists yet; `.10.9.4.1` is
the next implementation owner.
