...
> execution. The Julia consumer now passes 207 assertions after freezing the promoted Lua, recurring, and public-
> no-drift rows; it is included in ordinary Julia discovery and executed once directly by
...
> 246 per ABI across four non-eager nodes, recursive effect closure, cursor-only progress, native/reconstructed/
...
> 246-assertion source once per ABI in ordinary discovery and once per ABI in canonical CI. Only PUC Lua and LuaJIT
> advanced in that admission.
>
> The exact six-runtime recurring proof is current. `tools/check_recognition_transaction_six_runtime.sh` runs the
...
cancellation/error/policy/canonical/shutdown rules. Any later one-endpoint aggregator or legacy adapter is outside
...
The Lua handoff is complete through `.10.9.6.1-.4` under ADR `0061`. It reuses only protected Lua
...
and one two-function C99 entropy/monotonic-time module is compiled for each ABI. Strict `.2` scans number tokens
before decoding and reads borrowed interactive streams one byte at a time under the 1 MiB bound. One exact source
and consumer serve PUC Lua and LuaJIT. Do not add `/dev/urandom`, `math.random`, wall-clock, LuaRocks,
MCP SDK, network/
async transport, executable, source bootstrap, semantic cache, ABI-specific source, or a second server. Formal
status is 5/5 implementations + 6/6 runtimes with 114 rejected mutations; no-change `.4` closes parent
`.10.9.6`, and recurring rollout remains `.10.9.7`.
...
since reached the same private admission boundary; Julia/Lua consumers remain pending under `.14.7.6-.7`. Dart
...
19/19 in ordinary/canonical admission and governance rejects 90 mutations while only Dart rollout advances.
Generated format and public behavior remain unchanged; Julia `.14.7.6.0` is next.
