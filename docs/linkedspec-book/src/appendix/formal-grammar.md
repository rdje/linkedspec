...
version-2 signature record are adopted by ADR 0030 and the neutral callable-signature contract. The Perl reference
implements the syntax now; Rust, Dart, Julia, and Lua remain under the active `FUTURE-PARITY-BACKLOG.4.2-.4`
rollout, so portable multi-backend specs must wait for those admissions.

On the Perl reference, these calls illustrate the binding boundary:
