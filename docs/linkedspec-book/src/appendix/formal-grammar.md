...
The one exception now adopted as a versioned next surface is a final rest parameter:
...
```text
all_values()                  # items = []
all_values(1, [2], false)     # items = [1, [2], false]
collect("head")               # prefix = "head", items = []
...
When `direct_access` is used as `nested_assignment` on Perl, Rust, Dart, or Julia, every segment is retained as an ordinary typed
...
no partial path. Lua retains the earlier rule that every intermediate already exists until its implementation
and admission leaves complete. Read semantics remain non-creating on every backend.
...
array_expr.map_leaves() { return(value) }
                        — array-tree receiver block traversal over scalar/hash leaves; Perl/Rust/Dart current
binding_identifier.map_leaves!() { block }
                        — Perl/Rust/Dart/Julia current receiver mutation; one bare existing harray/array binding, dedicated
...
binding_identifier.map_leaves!() { block }
                        — Perl/Rust/Dart/Julia current; callback binds value, path (also @path on Perl), depth, and key or
...
are excluded. Lua does not yet accept this form.
