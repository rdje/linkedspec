...
When `direct_access` is used as `nested_assignment` on any backend, every segment is retained as an ordinary typed
...
no partial path. Read semantics remain non-creating on every backend.
