...
> `set_key(name, key, value)` auto-exist from a **bare** name too. On Perl, Rust, Dart, and Julia, typed-path assignment
> `name[key] = value` chooses the absent root from the evaluated selector kind instead of fixing it from syntax;
> Lua retains its prior hash-target behavior until admission. `copy(name)` snapshots the current array or harray
...
- **Returns**: read form returns the selected value or `undef`; assignment returns the updated root on success.
  Perl, Rust, Dart, and Julia structural failures throw the typed v1 diagnostic; Lua returns null at its prior boundary.
- **Behavior**: Reads any-depth mixed hash/array paths directly from a structured payload without creating state.
  Assignment uses the same spelling. On Perl, Rust, Dart, and Julia, each bracket expression is evaluated and its runtime kind selects
...
- **Edge cases**: Reads return `undef` when a segment does not exist or the current value has the wrong container
  kind. Perl, Rust, Dart, and Julia writes create an absent root and missing intermediates only when evaluated selectors determine the
  ...
  Lua retains the earlier requirement that intermediates already exist and returns null on a
...
- **Operator form**: `name[key] = value` is a separate typed-path contract on Perl, Rust, Dart, and Julia. An evaluated string has the
  same field-update effect, while a nonnegative integer selects an array. It yields an independent updated root
  snapshot in value positions; bare key/RHS identifiers read scalar working variables. Lua retains the earlier
  hash-index interpretation until admission. Receiver-dot `name.set_key(key, value)` remains pure.
...
- **Behavior**: Typed path assignment on Perl, Rust, Dart, and Julia. It evaluates the selector and value once, chooses harray for a
  string or array for a nonnegative integer, mutates an isolated root copy, then commits and returns a detached
  updated root. It is not identical lowering to harray-only `set_key`. Lua retains the earlier hash-index
  contract until admission.
- **Examples**: `meta["stage"] = "normalized"`, `meta[cat("source", "_kind")] = kind`,
  `meta[field_name] = field_value`, `items[position] = field_value`.
- **Edge cases**: The left side is a bare typed binding. On Perl, Rust, Dart, and Julia, absence creates the root chosen by the selector;
...
- **Boundary**: `set_key(name, key, value)` mutates the named working harray. On Perl, Rust, Dart, and Julia,
  `name[key] = value` is typed-path assignment and may instead select array for an integer; it yields the updated
  root snapshot in value positions. Lua retains its earlier hash-index meaning until admission.
...
- **Syntax**: Perl, Rust, Dart, Julia, and Lua accept standalone bare `next`; it is the same zero-argument operation as `next()`.
...
| `meta[key] = value` | `set_key(meta, key, value)` for explicit harray intent | typed-path assignment operator on Perl, Rust, Dart, and Julia (string→harray, nonnegative integer→array); Lua retains its earlier hash-index boundary. Bare key/RHS identifiers read scalar working variables; value positions yield the updated root snapshot. |
| `payload["items"][0]["name"] = value` | direct nested access assignment | mutates a scalar-held array/harray path. Perl, Rust, Dart, and Julia create absent/unambiguous missing containers from evaluated string/integer selectors; Lua still requires existing intermediates until admission. Wrong kinds are never coerced and array gaps remain invalid. |
...
working variables. As a Perl, Rust, Dart, or Julia lvalue, every segment is instead classified from its evaluated value kind. A direct nested path mutates a scalar-held array/harray value tree with the transitional
backend rules described above: vivifying on Perl, Rust, Dart, and Julia, with checked existing-path behavior on Lua.
...
snapshot. Perl, Rust, Dart, and Julia typed path failures throw exact diagnostics; Lua retains its earlier null result.
