...
> `set_key(name, key, value)` auto-exist from a **bare** name too. On all five backends, typed-path assignment
> `name[key] = value` chooses the absent root from the evaluated selector kind instead of fixing it from syntax.
> `copy(name)` snapshots the current array or harray
...
- **Edge cases**: Reads return `undef` when a segment does not exist or the current value has the wrong container
  kind. Writes create an absent root and missing intermediates only when evaluated selectors determine the
  ...
  Primitive literals and engine locals such as `[true]` or
...
### Five-backend receiver mutation: `map_leaves!`
...
- **Backend status**: current on Perl under `FUTURE-PARITY-BACKLOG.19.2.2`, Rust under `.19.3.2`, Dart under
  ...
  portable capability row, `.19.8` completes exact six-runtime recurrence, and `.19.9` closes current public
  no-drift.
...
- **Operator form**: `name[key] = value` is a separate typed-path contract on all five backends. An evaluated string has the
  same field-update effect, while a nonnegative integer selects an array. It yields an independent updated root
  snapshot in value positions; bare key/RHS identifiers read scalar working variables. Receiver-dot
  `name.set_key(key, value)` remains pure.
...
- **Behavior**: Typed path assignment on all five backends. It evaluates the selector and value once, chooses harray for a
  string or array for a nonnegative integer, mutates an isolated root copy, then commits and returns a detached
  updated root. It is not identical lowering to harray-only `set_key`.
- **Examples**: `meta["stage"] = "normalized"`, `meta[cat("source", "_kind")] = kind`,
  `meta[field_name] = field_value`, `items[position] = field_value`.
- **Edge cases**: The left side is a bare typed binding. Absence creates the root chosen by the selector;
...
- **Boundary**: `set_key(name, key, value)` mutates the named working harray. On all five backends,
  `name[key] = value` is typed-path assignment and may instead select array for an integer; it yields the updated
  root snapshot in value positions.
...
| `meta[key] = value` | `set_key(meta, key, value)` for explicit harray intent | typed-path assignment operator on all five backends (string→harray, nonnegative integer→array). Bare key/RHS identifiers read scalar working variables; value positions yield the updated root snapshot. |
| `payload["items"][0]["name"] = value` | direct nested access assignment | mutates a scalar-held array/harray path. All five backends create absent/unambiguous missing containers from evaluated string/integer selectors. Wrong kinds are never coerced and array gaps remain invalid. |
...
working variables. As a lvalue on any backend, every segment is instead classified from its evaluated value kind.
A direct nested path mutates a scalar-held array/harray value tree with the vivifying rules described above.
...
snapshot. Typed path failures throw exact diagnostics on every backend.
