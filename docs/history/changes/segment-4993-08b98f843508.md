...
- Activated task-tree-first from clean atomic-251 commit `c33f6664`; no grammar, parser, compiler, runtime,
  carrier, generated-plan, primary, rollout, facade, schema, CLI, README, capability, or typed-source behavior
  changed before or during this behavior-free audit.
- The neutral checker passes exact 7 complete / 2 pending / 60 semantic mutations. The rooted driver passes
  neutral, Perl 124, Rust 1, Dart 5, Julia 319, PUC Lua 392, and LuaJIT 392 in exact order, proving the complete
  private implementation boundary without re-deriving backend mechanisms.
- ADR `0079` freezes `.7.1` as recurring-only admission at 8/1/61; `.7.2` as the four-call staged-private→public language
  promotion (shared 246→250, public Perl 122→126, recognition calls 246→250 with 137 nodes unchanged), exact
  compatibility/public teaching, gap 9/0/63, and public 6 documents / 12 stale denials / 10 guards / 29 mutations;
  `.7.3` as independent no-change closeout and typed-source `gap_composition` handoff.
- Found and root-caused a documentation-governance defect: one correct required marker could coexist with stale
  surrounding prose. The capture chapter and Toolbox still claimed Lua skips and local-CI still presented 3/6 as
  current even though the rooted driver executes all six runtime routes. Corrected only those current facts;
  `.7.2` owns omission-sensitive stale-claim proof so the blind spot cannot recur.
- Preserved the exact compatibility boundary: legacy anonymous markers remain accepted non-alias behavior;
  named marks and explicit helpers remain independent; `@emit_gaps` and forced AST emission remain absent.
- The complete record crosses the bounded change hot shard's 90% line threshold. ADR `0078` reviews only the
  finite collection 18→19 file and manifest 17→18 line controls before the official content-addressed rollover;
  every root, byte, per-segment, aggregate, lifecycle, verifier, storage, runtime, and public control remains.
- Focused proof, rendered mdBook, Knowledge 840/7,102, bounded-history/task/memory checks, and all nine doctrines
  pass. Canonical proof passes CLI 66x2, RAM 56%, Phase 0 1,031/1,031 in 752 seconds, exact neutral plus all-six-
  runtime gap routing, receipt generation, and exit 0; the finalized staged record is rerun receipt-bound.
