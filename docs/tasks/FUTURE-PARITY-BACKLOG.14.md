...
- ID: `FUTURE-PARITY-BACKLOG.14.6.1`
  Status: `done; canonical-signoff-complete` (2026-08-17; task-tree-first from clean corrective commit `27f9c87f`; intended atomic 259; no push)
  Goal: Select the private authored progressive form and add an executable backend-neutral contract/checker for contiguous direct-span dispatch, source-view rebasing, pre-registered identity/top rule, isolated child state, detached result, ceilings, cancellation, cycle/progress, transaction-effect rejection, diagnostics, and no implicit loading.
  Depends on: `.14.6.0.1`, ADR `0080`, current typed-source and recognition-transaction authorities.
  Acceptance: retrieve ADR `0080`, progressive Knowledge, typed/recognition contracts, staged-registry precedents, loader/capability/cancellation authorities, canonical neutral-contract precedents, and task ownership before archaeology; select one private fail-closed authored form without enabling any backend; write checker-first RED for the absent neutral progressive authority; add an independently executable contract/checker covering immutable pre-registration, one contiguous direct same-source span, explicit top rule, rebased bounded child view, isolated parent state, detached result, narrowing capability/policy/source-detail/resource ceilings, inherited cancellation/deadline/remaining budget, strictly decreasing same-identity cycles plus depth/call limits, uncommitted-recognition rejection, diagnostics, rollout ownership, and zero implicit path/loading; add reasoned semantic/topology/current-boundary mutations and canonical tracked execution; keep typed `progressive_span_dispatch` pending until `.7`; change no parser/compiler/runtime/facade/schema/semantic/MCP/CLI/README behavior; synchronize ADR/Knowledge/Toolbox/capability/roadmaps/live/mdBook; pass focused direct proof plus the canonical gate required by neutral governance registration; commit, clear the brief, and land clean before `.2`.
  Verification tier: `canonical`
  Focused checks: `checker-first RED/GREEN; typed and recognition direct dependents; canonical registration/order; capability/language; mdBook; Knowledge; task metadata; bounded histories; README/memory; all doctrines; no-production-diff; exact diff`
  Canonical trigger: `yes — adds a new executable neutral authority/checker and always-on canonical registration even though backend behavior and typed rollout remain pending`.
  Checklist: [x] clean activation/task ownership; [x] committed authority retrieval; [x] authored/neutral decision; [x] checker-first RED; [x] executable contract/checker; [x] mutation and canonical registration; [x] focused/canonical proof; [x] roadmap/Knowledge/book/live synchronization; [x] atomic commit/clean handoff.
  Authority evidence: clean corrective atomic 258 `27f9c87f`; ADRs `0012`, `0015`, `0056`, and `0080`; progressive/staged/typed/recognition Knowledge; current typed 11/3/152 and recognition 9/9/58; five narrow staged registries; neutral recognition precedent `.14.3.1.1`; canonical/project-data registration; capability/language and sole-facing book were retrieved before artifact design.
  Decision: reserve `value = dispatch_span("expr-v1", "Expr", span)` as one dedicated `PROGRESSIVE_DISPATCH_SPAN` future expression. Parser identity/top rule are normalized literals; `span` is a bare exact four-field local direct carrier; effect is `parser_registry_or_staged_dispatch`; result is detached; failure is fail-only; no backend is enabled.
  RED evidence: the complete independent checker existed before the artifact and exited 1 only at exact `ContractError: progressive span-dispatch contract is missing`.
  GREEN/signoff evidence: `linkedspec-progressive-span-dispatch-v1` passes 2 immutable registry entries, 2 sources/8 rebased views, 6 authority-minimum, 6 cancellation/deadline/budget, 8 decreasing-chain/bound, 4 isolation/detachment cases, 5 backend guard groups/17 implementation paths plus 10 outward guards, 26 diagnostics, neutral 1/9 rollout, and 86 mutations. Canonical CI requires/invokes it exactly once; typed remains pending; the rejected effect has empty current node/call rows; every backend guard omits both future tokens; outward guards deny private spelling/node/rollout exposure; and Perl returns the unsupported sentinel. Typed 11/3/152, recognition 137/250/58, capability 80/0/0, language 250/105+1/126, project-data tool locality 3/14/31, and JSON/CI/diff pass; Knowledge is 842/7,124; task metadata is 535 IDs at the exact 5,000-line bounded part; mdBook is inspected 79 files/15,024 KiB and removed; histories, memory, README, all doctrines, exact staging, receipt-bound canonical CI, leaf commit, brief clear, and clean handoff pass before `.2` activation.
  Commit: `FUTURE-PARITY-BACKLOG.14.6.1 - add neutral progressive span dispatch authority`
