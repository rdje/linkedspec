...
- ID: `FUTURE-PARITY-BACKLOG.14.6.2.0`
  Status: `done; focused-signoff-complete` (2026-08-17; task-tree-first from clean neutral atomic 259 `a0a33c32`; intended atomic 260; no push)
  Goal: Establish the exact dormant Perl RED and freeze the implementation/carrier plan before production behavior; Depends on: `.14.6.1`; Acceptance: retrieve committed ADR/Knowledge/contract and current Perl registry/lowering/runtime/generated authorities before archaeology; add or freeze one exact repository-owned failing semantic consumer without changing production behavior; prove current private syntax/node rejection and map `.1-.3` to disjoint authority/core, carrier integration, and admission changes.
  Verification tier: `focused`
  Focused checks: exact 83/1 dormant RED, syntax/routing absence, neutral/typed/recognition/generated direct dependents, Knowledge, task metadata, book, histories, README/memory, nine doctrines, no-production-diff, exact diff.
  Canonical trigger: `none — dormant unrouted test and durable planning only; no production, executable governance, CI topology, admission, generated format, storage, path, doctrine, or public movement`.
  Checklist: [x] ownership [x] authority retrieval [x] Toolbox probes [x] exact RED [x] carrier seam [x] `.1-.3` split [x] durable synchronization [x] focused signoff [x] clean handoff.
  Evidence: final-path consumer derives complete neutral 2/2+8/6/6/8/4/26/1-of-9/86 inventory, passes 83 then fails only on missing `PROGRESSIVE_DISPATCH_SPAN`; current descriptor has ASSIGN/RETURN, one unresolved helper, zero raw dependencies; generated-v2 has one sentinel and loads null; staged adapter rejects `expr-v1` at resolve; consumer absent from CI.
  Commit: `FUTURE-PARITY-BACKLOG.14.6.2.0 - freeze Perl progressive dispatch RED`
