...
- ID: `FUTURE-PARITY-BACKLOG.14.6.3`
  Status: `active` (2026-08-17; dormant RED `.3.0` focused-signoff-complete for intended atomic 264; private authority `.3.1` next; no push); Goal: Split, implement, and privately admit Rust parity across native, reconstructed, generated-plan, and independently compiled emitted carriers; Depends on: `.14.6.2`; Children: `.14.6.3.0-.14.6.3.3`.
- ID: `FUTURE-PARITY-BACKLOG.14.6.3.0`
  ...
  Verification tier: `focused`
  Focused checks: exact dormant Rust RED and zero ordinary/canonical discovery; staged-registry and four-carrier probes; progressive/typed/recognition/generated/capability/language direct dependents; task/index, Knowledge, mdBook, histories, README/memory, nine doctrines, no-production-diff, and exact diff.
  Canonical trigger: `none — dormant outer-cfg test and durable planning only; no production, executable governance, CI topology, admission, generated format, dependency/toolchain, storage/path/doctrine infrastructure, or public movement`.
  Checklist: [x] clean ownership/activation [x] committed authority retrieval [x] Toolbox-led Rust seam audit [x] exact final-path RED [x] ordinary/canonical dormancy proof [x] `.1-.3` plan freeze [x] durable synchronization [x] focused verification [x] atomic commit/clean handoff.
  Evidence: one outer-cfg consumer derives neutral 2/2+8/6/6/8/4/26 and rollout 2/9/86, proves staged `expr-v1` resolve rejection, and carries the exact authored assignment through native, reconstructed, generated-plan, and independently compiled emitted Rust execution. All four currently return `null` through the same generic `Expr::Call`; serialized/emitted forms retain exactly one `dispatch_span` call and no dedicated node. The opt-in run passes every pre-boundary assertion then fails only on missing `progressive_dispatch_span` / `PROGRESSIVE_DISPATCH_SPAN`; ordinary discovery runs zero tests and canonical CI omits the target. Focused Rust/direct-dependent, Knowledge, book, task/history/memory/doctrine, no-production-diff, and exact-diff checks pass without changing rollout 2/9, typed 11/3/152, recognition 138/250/58, generated format, other backends, or outward surfaces; `.3.1-.3` exclusively own authority, carriers, and admission. Commit: `FUTURE-PARITY-BACKLOG.14.6.3.0 - freeze Rust progressive dispatch RED`.
- ID: `FUTURE-PARITY-BACKLOG.14.6.3.1`
  Status: `pending`; Goal: Add the private immutable Rust registry, invocation, rebased-source-view, cancellation/budget, cycle/progress, policy-ceiling, and typed diagnostic authority without ActionIR/carrier or rollout movement; Depends on: `.14.6.3.0`.
