...
- ID: `FUTURE-PARITY-BACKLOG.14.5`
  Status: `active` (2026-08-13; behavior-free boundary audit `.14.5.0` signoff-complete; `.14.5.1` waits for `INTER-MATCH-GAP-CAPTURE.7`)
...
- ID: `FUTURE-PARITY-BACKLOG.14.5.0`
  Status: `done; signoff-complete` (2026-08-13; task-tree-first from clean recursive-observation public-closeout commit `d26e4d4e`; intended atomic 217/300; no push); Goal: Audit the committed named-slot, typed-source, historical-gap, marker-divergence, and cross-tree authorities and freeze the exact ownership/dependency handoff before gap syntax or implementation work; Depends on: `.14.4.8`, `INTER-MATCH-GAP-CAPTURE.0`, completed duplicate-regex-slot identity rollout; Acceptance: prove clean activation; retrieve and hash ADRs `0045`/`0047`/`0056`, both owning trees, Knowledge facts, contracts, and recurring gates before archaeology; use Toolbox probes to verify numeric identity, named-slot absence, historical Perl gaps, marker divergence, typed spans, and carriers; preserve `INTER-MATCH-GAP-CAPTURE.1-.7` as sole syntax/lifecycle/compatibility/backend owner and `.14.5.1` as post-admission typed-source composition only; freeze prerequisites/evidence/no-overclaim/clean-pivot policy; change no grammar/parser/compiler/runtime/descriptor/generated/helper/value/schema/semantic/MCP/CLI/README/rollout/current claim; synchronize task/index, roadmaps, ADR/Knowledge, mdBook, and live layers; pass focused/book/doctrine/canonical signoff; commit `FUTURE-PARITY-BACKLOG.14.5.0 - freeze lossless gap handoff`; clear the brief and land clean before switching trees; Checklist: [x] clean activation/task ownership; [x] committed authority retrieval; [x] Toolbox-led current audit; [x] cross-tree ownership freeze; [x] no behavior/public overclaim; [x] verify/commit/clean. Activation evidence: `.14.4.8` landed cleanly at `d26e4d4e` as atomic 216/300 with empty status/diffs, zero-byte brief, fresh Knowledge, no rendered-book residue, and no background result. Authority evidence: committed duplicate/typed contracts `ecef5cd5`/`a9e2ca7`, ADRs `0045` `8cf73db2`, `0047` `e9edc734`, `0056` `c37fefb6`, gap/marker/typed Knowledge cards `ad1a41b`/`4da68ef0`/`4b91b43`, both owning task trees `2a17d755`/`97dfdd6`, and exact checker/driver authorities were retrieved before archaeology. Audit evidence: `LinkedSpec::Get(..., return_descriptor => 1)` resolves current `Document[1]` to action-owned regex slot 1; current `Document[section]` rejects with unsigned-integer guidance and `section=/S/` rejects as unsupported rule-paragraph content. The corrected historical Perl `@move_pos` probe on `preHgapSmoreFtail` returns exact `[[pre,H],[gap,S],[more,F]]` and no automatic tail; the earlier cumulative result was host `qq{}` interpolation of an unescaped marker, not a project defect. Current source confirms Perl anonymous markers are unconditional rule-level LE code, Lua markers are post-action preceding-slot events, and native Rust/Dart/Julia do not execute marker members. Design freeze: brackets alone select slots; numeric selectors preserve positional compatibility, named selectors preserve stable identity and may share one typed resolved slot while retaining source provenance. Dot remains fluent behavior; neither selector-dot aliases nor whitespace-only first-method attachment are accepted. Ownership freeze: `INTER-MATCH-GAP-CAPTURE.1-.7` exclusively owns grammar, lifecycle, compatibility, six-runtime implementation, carriers, and public admission; only after `.7` may `.14.5.1` promote typed `gap_composition`, while combined no-drift remains `.14.8`. Focused evidence: the post-edit duplicate-slot driver passes 5 fixtures / 2 diagnostics / 6 runtime rows / 7+0 rollout / 21 documents / 11 forbidden claims / 59 mutations, Perl 12, Rust one exact 15-role test, Dart exact, Julia 121, PUC Lua/LuaJIT 112 each, CLI 5x2, strict Rust generated 105/105, capability 80/0/0, and language 246/105+1/122. The post-edit typed-source driver passes neutral 9/5/114, Perl 17, Rust observation 7 plus typed 4, Dart +11, Julia 127+30, PUC Lua/LuaJIT 240+43 each, and all support ledgers. The first duplicate recomposition correctly exposed a condensed task-index row missing the unrelated capability-exclusion closeout marker; restoring the preserved `.24.2` claim makes the exact rerun pass. The first canonical attempt correctly exposed a bounded MEMORY rewrite that omitted the unrelated closed repeated-action next owner `FUTURE-PARITY-BACKLOG.10.1`; restoration makes the focused 8-mode / 10-special / 54-mutation contract pass. The book renders 79 files / 14,580 KiB with exact selector/ownership text, Knowledge passes 829 facts / 6,931 questions, both bounded history shards remain below rollover thresholds, and rendered output is removed. Definitive canonical evidence: the sandboxed restart reaches only the expected outer-harness nested-sandbox denial at representative process containment; the unchanged permission-authorized run passes all eight doctrines, six-family repository containment, all-five-anchor relocation, CLI 66/66 in both option environments, RAM 68%, and Phase 0 1,031/1,031 in 771 seconds through exact `[ci] local CI gate passed` and exit 0. Commit evidence is the intended atomic 217 landing with the required subject, zero-byte brief, empty diffs/status, fresh Knowledge, and no rendered-book residue before the clean pivot.
- ID: `FUTURE-PARITY-BACKLOG.14.5.1`
  Status: `pending`; Goal: After `INTER-MATCH-GAP-CAPTURE.7` closes exact six-runtime lossless segmentation, compose its unchanged named-slot/gap-span evidence into typed-source `gap_composition` and a dedicated recurring proof; Depends on: `.14.5.0`, `INTER-MATCH-GAP-CAPTURE.7`; Acceptance: reuse the separate program's syntax/lifecycle/compatibility/carrier/public evidence; add no second implementation or migration owner; promote only `gap_composition`; preserve final `recurring_public_no_drift` for `.14.8`; recompose runtime/support ledgers and close `.14.5`.
...
- ID: `FUTURE-PARITY-BACKLOG.14.8`
  Status: `pending`
  Goal: Recompose six-runtime admissions and recurring governance, close examples/tooling/mdBook/Knowledge Map alignment, reject stale-helper and unsafe-cursor drift, and complete public no-drift for the full authoring model.
