...
- ID: `FUTURE-PARITY-BACKLOG.14.5.1`
  ...
  Verification tier: `canonical`; Focused checks: `checker-first RED; typed 10/4/126; gap and typed six-runtime matrices; recognition/generated/capability/language; outside-CWD; mdBook 79/14,984; Knowledge 841/7,112; nine doctrines`; Canonical trigger: `yes — executable governance and local-CI topology change`.
  ### Acceptance Checklist
  - [x] **REPRODUCE / ISSUE** — The final checker oracle rejects the committed contract before contract/driver/CI edits with exact `ContractError: lossless-gap composition contract is missing`.
  - [x] **ROOT CAUSE (WHY + WHERE)** — Knowledge/ADR/TOOLBOX plus both executable contracts prove all six `gap_span` carriers already provide detached typed same-source half-open Unicode-scalar spans; only the incomplete typed rollout row and dedicated composed route were absent.
  - [x] **FIX** — Bound unchanged gap 9/0/63 + public 6/12/10/29 to existing `gap_span`; added rooted composed driver/canonical switch; twelve regressions promote only `lossless_gap_composition` for typed 10/4/126.
  - [x] **ADDRESSED (verified)** — Typed checker, dedicated composed driver, complete gap and typed matrices, support ledgers, outside-CWD routing, `mdbook build`, Knowledge, history pressure, memory, nine doctrines, containment/relocation, CLI 66x2, Phase 0 1,031/1,031, and the opt-in canonical route pass.
...
- ID: `FUTURE-PARITY-BACKLOG.14.6.2`
  Status: `active` (2026-08-17; split task-tree-first from clean neutral commit `a0a33c32`; dormant RED `.0` focused-complete, authority `.1` next); Goal: Implement and privately admit the Perl reference through exact RED, registry/source-view authority, live/generated carriers, and ordinary/canonical admission; Depends on: `.14.6.1`; Children: `.14.6.2.0-.14.6.2.3`.
...
- ID: `FUTURE-PARITY-BACKLOG.14.6.2.1`
  Status: `pending`; Goal: Implement immutable Perl registry resolution, bounded rebased source-view authority, cancellation/budget/cycle/progress enforcement, isolated child execution, detachment, and exact private diagnostics; Depends on: `.14.6.2.0`; Planned tier: `focused`.
