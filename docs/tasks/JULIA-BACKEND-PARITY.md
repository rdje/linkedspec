...
- Julia exposes its own distinct LinkedSpec CLI entrypoint; it must not rely on the Perl, Rust, or Dart CLI names as
  the only user-facing command. Distinct executable names must expose the same user-visible command contract:
  options and meanings, positional arguments, output/error behavior, and exit semantics are identical across
  variants.
...
- ID: `JULIA-BACKEND-PARITY.7`
  Status: `active`
  Goal: Close documentation, generated-source follow-up, strict user-facing CLI parity, and handoff alignment.
...
- ID: `JULIA-BACKEND-PARITY.7.3`
  Status: `active`
  Goal: Close Julia user-facing parity without hiding feature or CLI drift behind the scoped corpus milestone.
  Children: `.7.3.0`, `.7.3.1`, `.7.3.2`, `.7.3.3`

- ID: `JULIA-BACKEND-PARITY.7.3.0`
  Status: `done`
  Goal: Audit and split the strict user-facing parity closeout before repair work.
  Acceptance: Inspect every implemented variant CLI from source; compare command purpose, options, positional
    arguments, outputs/errors, and binary availability; split decision, Julia repair, and no-drift work before
    changing implementation.
  Verification: `PASS` - Perl `bin/linkedspec` is a parser CLI with source/input, parser-mode, trace, canonical-JSON,
    and 1/2 exit semantics. Dart's primary CLI and Julia's primary CLI are corpus/status adapters with different
    options and 64/2 error conventions. The Rust workspace declares no binary target. The verified contract gap is
    split into `.7.3.1` contract/routing, `.7.3.2` Julia CLI alignment, and `.7.3.3` honest no-drift closeout.
...
- ID: `JULIA-BACKEND-PARITY.7.3.2.0`
  Status: `done`
  Goal: Audit and split Julia primary-CLI alignment by implementation mechanism before code.
  Acceptance: Identify native library seams and every missing ADR `0023` mechanism; split trace prerequisites,
    argument/source/input handling, execution/canonical JSON, normalized errors/trace routing, and direct-command
    conformance into recoverable leaves.
  Verification: `PASS` - Julia already exposes rule/staged source parsing, compilation, runtime top-rule/parse-mode,
    structured diagnostics, trace configs/sinks, and corpus JSON utilities. Its primary CLI still owns only
    status/corpus dispatch; no named-spec resolver, parser-option model, canonical key-sorted result serializer, or
    compile/parser trace instrumentation exists. `.7.3.2.1` through `.7.3.2.5` own those mechanisms before repair.
  Commit: `JULIA-BACKEND-PARITY.7.3.2.0 - split Julia primary CLI alignment`

- ID: `JULIA-BACKEND-PARITY.7.3.2.1`
  Status: `done`
  Goal: Close Julia compile/spec-parser/staged trace coverage required by the primary trace options.
  Acceptance: Opt-in structured trace events cover source parse/validation/compile, function-shell parsing, and
    staged dispatch through existing levels/config/sinks; default APIs remain quiet and output-compatible.
  Verification: `PASS` - one optional caller-owned `LinkedSpecTraceEmitter` now propagates through rule parsing,
    validation passes, compiled-state construction, spec-driven function-shell parsing/projection, runtime shell
    execution, and staged normalize/queue/resolve/load/compile/execute phases. Twenty-eight focused assertions prove
    scopes, decisions, failure exits, routed output, disabled quietness, and traced/untraced identity; the complete
    868-assertion package suite and 99/99 focused corpus gate pass.
...
- ID: `JULIA-BACKEND-PARITY.7.3.2.3`
  ...
  Verification: `PASS` - prepared requests now compose rule-first/spec-driven-function fallback parsing, traced
    compilation, runtime construction with source identity and parse controls, and direct top-rule execution. A
    recursive serializer emits compact JSON with every nested object key sorted lexicographically and one newline.
    Twenty-two focused assertions, the complete 942-assertion package suite, direct canonical-output smoke, and
    99/99 corpus gate pass.
...
- The prepared request now flows through the same native library path used by corpus execution. Rule-only
  `parse_spec(...)` runs first; only `SpecParseException` selects the spec-driven top-level-function parser. The
  same optional emitter propagates through parse, validation, compilation, staged function bodies, and runtime.
- `LinkedSpecRuntimeEngine` receives the selected seek/consume mode plus exact spec name/path identity;
  `runtime_execute(...)` receives the optional top-rule and literal/file-loaded input. The CLI serializes
  `RuntimeParseResult.value`, the direct backend-neutral top-rule value, rather than the corpus-only wrapped
  `output` field.
- `_write_primary_cli_canonical_json(...)` recursively handles null, booleans, strings, numbers, arrays/tuples,
  and string-keyed objects. Every object level is lexicographically sorted before compact encoding, so host `Dict`
  insertion order cannot affect stdout. Success adds exactly one newline.
...
- [x] **REPRODUCE / ISSUE** — Valid canonical requests loaded exact source/input but always returned the temporary
  “parser execution is not connected” failure.
- [x] **ROOT CAUSE (WHY + WHERE)** — The primary adapter had no composition from its typed request into the
  already-shipped rule/staged parser, compiler, engine, runtime, and direct result shape.
- [x] **FIX** — Compose the native phases with one emitter and add a recursive key-sorted canonical JSON writer
  over the direct top-rule value.
- [x] **ADDRESSED (verified)** — Twenty-two focused assertions cover both source paths, runtime controls, direct
  shape, nested canonicalization, file loading, and routed tracing.
...
- At this leaf boundary, valid prepared requests deliberately stopped before execution; `.7.3.2.3` has since
  connected those requests to native execution and canonical JSON.
...
- [x] **LOCKSTEP** — At this leaf boundary, mdBook, Julia README, roadmap/task/live docs, architecture, and
  Knowledge Map described preparation only and advanced the sole frontier to `.7.3.2.3`.

## `JULIA-BACKEND-PARITY.7.3.2.1` Frontend/Compiler/Staged Trace Proof

Implementation evidence recorded on 2026-07-10:

- `julia/src/trace/Trace.jl` now loads before trace-aware frontend owners. Existing trace levels, configs, emitter,
  stdout/route/mirror sinks, reset behavior, and event rendering remain the single trace mechanism.
- `parse_spec(...)`, `validate_spec(...)`, and `compile_spec(...)` accept optional caller-owned `trace` emitters.
  Low-level scopes enclose each operation; medium decisions report parse outcomes, every validation pass/skip,
  function-registry construction, compiled rules, and dependency-regex construction. Failure paths pair every
  emitted enter with an error-bearing exit.
- Spec-driven function parsing propagates the same emitter through parser-spec parse/compile, cache selection,
  runtime execution of `specs/user_function_definition.spec`, neutral-node projection, stripped rule parsing, and
  staged body stitching. No raw Julia function scanner or alternate traced path was introduced.
- Staged jobs emit a low-level queue scope, medium normalization/sort/job scopes, and resolve/load/compile/execute
  decisions. `dispatch_function_body_parse_jobs(...)` and every public stitching/parse composition accept and
  propagate the same optional emitter.
- Omitting `trace` retains the direct quiet implementations. A disabled emitter records/writes nothing. Focused
  descriptor/spec comparisons prove traced and untraced results are identical; routed-file output uses the existing
  sink rather than a frontend-specific writer.

## `JULIA-BACKEND-PARITY.7.3.2.1` Acceptance Checklist

- [x] **REPRODUCE / ISSUE** — `.7.3.2.0` proved runtime-only tracing left primary trace options semantically
  incomplete for source parsing, validation, compilation, function-shell parsing, and staged dispatch.
- [x] **ROOT CAUSE (WHY + WHERE)** — Trace types loaded after frontend owners and only runtime entrypoints accepted
  emitters; parser/compiler/staged APIs therefore had no shared propagation path.
- [x] **FIX** — Load the existing trace owner before frontend definitions and add one optional emitter through every
  native parse/validate/compile/function-shell/staged composition, with balanced scopes and phase decisions.
- [x] **ADDRESSED (verified)** — Twenty-eight focused assertions cover success/failure events, route sinks,
  default-disabled quietness, and result identity; the complete Julia suite passes with 868 assertions.
- [x] **NO REGRESSION** — `tools/run_julia_local.sh` passes CLI smokes and all 99 corpus fixtures exactly; generated
  depot compilation artifacts were removed afterward.
- [x] **LOCKSTEP** — mdBook, Julia README, roadmap/task/live docs, architecture, and Knowledge Map describe the new
  trace boundary; `.7.3.2.2` is the sole active Julia frontier.

## `JULIA-BACKEND-PARITY.7.3.2.0` Primary-CLI Mechanism Audit

Audit evidence recorded on 2026-07-10:

- `parse_spec(...)` handles rule-only sources; `parse_spec_with_staged_user_function_definitions(...)` and the
  corpus path provide spec-driven top-level function fallback. `compile_spec(...)`, `LinkedSpecRuntimeEngine`, and
  `runtime_execute(...)` already accept the data needed for `--top-rule` and `--parse-mode`.
- `LinkedSpecTraceConfig` already supports numeric/named levels, stdout/route/mirror sinks, file reset, and emoji;
  the runtime is instrumented. Julia still explicitly lacks compile/spec-parser/staged trace events, so merely
  accepting CLI trace flags would not satisfy their cross-variant meaning.
- `RuntimeDiagnostic` carries stable owner/stage/spec/path/top/rule/handler fields, but the primary CLI has no
  normalized formatter or stage-to-exit boundary.
- `julia/src/cli/LinkedSpecJuliaCli.jl` only dispatches `status` and `corpus`. It has no ADR `0023` option parser,
  named/current/repo spec resolver, input loader, parser execution, or canonical result writer.
- `JSON3.write(...)` is used for corpus tooling but does not establish Perl-style lexicographically key-sorted
  canonical JSON for arbitrary nested result dictionaries. Direct CLI output needs its own locked serializer.
- These are separate failure domains and validation surfaces; `.7.3.2` is therefore a parent rather than a single
  implementation leaf.

## `JULIA-BACKEND-PARITY.7.3.2.0` Acceptance Checklist

- [x] **REPRODUCE / ISSUE** — Audited Julia exports, primary CLI, corpus composition, parser/compiler/runtime,
  diagnostics, trace configuration/sinks, JSON use, and current tests from source.
- [x] **ROOT CAUSE (WHY + WHERE)** — The CLI was scaffolded/productized around corpus rollout before ADR `0023`;
  the native runtime grew behind it without a parser-oriented command adapter or compile/parser trace layer.
- [x] **FIX / SPLIT** — Split five implementation/conformance mechanisms before changing code.
- [x] **ADDRESSED (verified)** — Every known Julia primary-CLI gap has one ordered child owner.
- [x] **NO REGRESSION** — Read-only audit/task split; no Julia source/test behavior changed.
- [x] **LOCKSTEP** — `.7.3.2.1` is the sole active leaf and preserves ADR `0023` as the target contract.
...
- ADR `0006` already requires every backend to have the same features, runtime semantics, and `.spec`
  compatibility. The director clarified that this sameness is evaluated from the user's point of view.
- Distinct variant executable names remain valid, but their interfaces must be identical: command structure,
  option list and meanings, positional arguments, output/error behavior, and exit semantics.
- Perl `bin/linkedspec` is currently the only parser-oriented CLI. It selects spec source and input, configures
  top-rule/parse-mode/trace controls, emits canonical JSON, and uses exit `1` for runtime failure / `2` for usage.
- Dart `linkedspec_dart.dart` currently loads or executes a manifest corpus and uses exit `64` for usage errors.
  Julia `linkedspec_julia.jl` currently exposes `status` and corpus commands with exit `2` for usage errors.
  Neither primary CLI exposes Perl's parser command contract.
- The Rust workspace contains library crates only and declares no binary target. The ADR `0006` CLI reach table
  therefore describes intended reach, not the implemented current surface.
- `.7.3` cannot truthfully close as a simple documentation sync. Decision/routing, Julia CLI repair, and final
  no-drift are separate recoverable leaves; cross-backend repair ownership must be durable before Julia code.

## `JULIA-BACKEND-PARITY.7.3.0` Acceptance Checklist

- [x] **REPRODUCE / ISSUE** — Read the four implemented backend CLI/package surfaces and compared their actual
  command purposes and contracts.
- [x] **ROOT CAUSE (WHY + WHERE)** — Earlier planning interpreted “distinct CLI” as per-backend command ownership
  but never specified one shared interface schema, allowing corpus/status scaffolds to be productized as primary
  Dart/Julia CLIs while Rust retained no CLI target.
- [x] **FIX / SPLIT** — Split durable contract/routing, Julia implementation, and honest final no-drift leaves
  before changing any backend behavior.
- [x] **ADDRESSED (verified)** — Source evidence names every current gap and `.7.3.1` is the sole active leaf.
- [x] **NO REGRESSION** — Planning/audit only; parser/compiler/runtime/CLI behavior is unchanged.
- [x] **LOCKSTEP** — The task tree distinguishes the proven 99/99 interpreter corpus milestone from the newly
  clarified exact user-facing parity obligation.
...
- `2026-07-10`: Completed `.7.3.2.2` Julia primary arguments/resolution/loading. The primary module accepts only
  ...
  assertion package suite, direct CLI checks, and 99/99 pass. `.7.3.2.3` became active there and has since closed
...
- `2026-07-10`: Completed `.7.3.2.1` Julia frontend/compiler/staged tracing. Optional caller-owned emitters now
  ...
  868-assertion package suite, CLI smokes, and 99/99 corpus gate pass. `.7.3.2.2` is active.
- `2026-07-10`: Completed `.7.3.2.0` Julia primary-CLI mechanism audit/split. Existing native parser/compiler/
  runtime, diagnostics, and runtime trace seams are reusable, while compile/parser trace, arguments/resolution,
  execution/canonical JSON, error/routing, and direct conformance need separate leaves. `.7.3.2.1` is active; no
  behavior changed.
...
- `2026-07-10`: Completed `.7.3.0` read-only strict user-facing parity audit. ADR `0006` already requires the same
  ...
  `.7.3` is split before repair; `.7.3.1` is active for the durable contract and cross-backend routing.
