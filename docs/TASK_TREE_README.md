...
| `ROADMAP_V2.md` | High-level status board; links active lanes to active task trees. |
...
```text
docs/tasks/PHASE2-DSL-FRONTEND.md
docs/tasks/PARSER-DIAGNOSTICS.md
docs/tasks/SPEC-BOOTSTRAP.md
docs/tasks/PLUGIN-CLEANUP.md
```
...
- ID: `PHASE2-DSL-FRONTEND.1`
  Status: `pending`
  Goal: `Inventory current DSL frontend validation rules and error surfaces.`
  Acceptance: `The task file lists current validation points, error paths, gaps, and the next executable leaf.`
...
| --- | --- | --- | --- |
| 1 | `PHASE2-DSL-FRONTEND.1` | `pending` | Need an accurate surface inventory before hardening. |
```
...
- Do not duplicate the whole task tree into `ROADMAP_V2.md`.
...
[ ] ROADMAP_V2.md links active roadmap lane(s) to active task tree(s).
