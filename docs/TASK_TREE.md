...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed. Perl static projection `.10.3.2.1` is clean at `e0aeee63`. Generated-plan correction `.10.3.3.0` proves the calls artifact is v2 family `default`, not stale/illegal `and_acode`; semantic governance is 6/20/55 without behavior or admission. Corrected Perl calls/staging `.10.3.3.1` follows its clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
