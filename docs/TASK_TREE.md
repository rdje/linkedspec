...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.4.4` done 2026-07-09 - Dart `push(Child)` current-accumulator semantics are aligned, `regdef_nested_register_fields` passes, `ds_vhistory_version_entry` is routed with direct-access/oracle evidence, and the parser-smoke window is 23/31 green. Next frontier `.6.2.4.4.5` is residual parser-smoke closeout. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
