...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia staged `.14.7.6.3` focused-signoff-complete | `Overall roadmap - future parity backlog` | Julia's dormant consumer is 386 GREEN/one exact `.14.7.6.4` carrier/admission RED after private breadth-first recurrence, strict lineage, shared resources, expiring safe points, and typed source rebasing. `.4` is next after atomic `.3`; rollout, formats, and public behavior remain fixed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
