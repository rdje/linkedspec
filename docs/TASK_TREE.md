...
| `TRACE-OBSERVABILITY` | `active` | `Overall roadmap — engine observability / developer experience` | User directive 2026-06-19: discoverable CLI trace control + comprehensive "see everything" trace (function enter/exit, if/switch/case branches). Framework EXISTS (`Trace.pm`: enter/exit/decision/levels/sinks; env/per-call control works). `.1` audit showed trace is not exhaustive; `.2` added discoverable `bin/linkedspec` CLI flags and docs. `.3` is split before code: helper seam, generated non-REP dispatch, generated repetition, compile/ActionIR scopes, then coverage closeout. Frontier `.3.1`, then `.3.2`–`.3.5`, then future backend parity split. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. `TRACE-OBSERVABILITY.3` has since split; PNT returns to `TRACE-OBSERVABILITY.3.1` unless a new terse leaf is split or another active tree is reprioritized.
...
`RUST-PARITY` unblocked `TOP-RULE-AS-NORMAL.3.2`; that leaf has since closed, `TRACE-OBSERVABILITY.1` has completed its read-only audit, `TRACE-OBSERVABILITY.2` has added CLI trace control, and `.3` has split before code, so PNT advances to `TRACE-OBSERVABILITY.3.1`.
