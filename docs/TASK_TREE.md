...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart source/outcome parent `.10.5.1` is signoff-complete from clean scanner-repair commit `1529ca81`: strict source mapping, compiled-or-failed authority, exact privacy/isolation/host-state proof, and untracked-source no-drift all pass without semantic promotion. Static projection `.10.5.2` is next after the clean closeout commit. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
