...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua `.10.7` is closed: one exact twelve-role consumer passes 408 assertions unchanged on PUC Lua and LuaJIT, and semantic governance is 6 groups / 20 responses / 98 mutations at rollout 6/9 and admission 6/6. Recurring six-runtime composition `.10.8` is the next pending task-tree-first slice. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
