...
| `MDBOOK-DESTINATION-ROOT-ALIGNMENT` | `active` / safety intake `.0` signoff-complete | `Project-data locality / mdBook workflow safety` | Render verification exposed one exact mismatch: relative custom destinations validate from the book root but execute from the repository root. `.0` durably freezes the reproduced bases and smallest repair/RED plan without behavior change; `.1` follows the clean commit to align execution with validation, lock real split/equals/compact CLI regressions, and close before typed-source Perl work resumes. | [docs/tasks/MDBOOK-DESTINATION-ROOT-ALIGNMENT.md](docs/tasks/MDBOOK-DESTINATION-ROOT-ALIGNMENT.md) |
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; typed source rollout correction `.14.2.0.1` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; typed source-location neutral/public parent `.14.1`, runtime plan `.14.2.0`, and live correction `.14.2.0.1` are closed. Current typed-source truth is 3/11 with 37 mutations and exact backend admission owners. mdBook destination safety intake `.0` is signoff-complete and `.1` repairs the seam before Perl begins. Knowledge Map is 785/6,377; the sole-facing rendered-readability audit remains independently queued and nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
