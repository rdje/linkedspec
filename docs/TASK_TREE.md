...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia staged `.14.7.6.1` focused-signoff-complete | `Overall roadmap - future parity backlog` | Julia's dormant final-path consumer is 131 GREEN/one exact `.14.7.6.2` authority RED after `.1` added the exclusive marker, strict static closure, and native-capture-proven direct/ordered-derived provenance. Atomic `.1` landing precedes task-tree-first `.2`; ordinary/canonical discovery, rollout, authority/policies, formats, and public behavior remain fixed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
