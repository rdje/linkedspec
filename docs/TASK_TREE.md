...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl/Rust/Dart/Julia are 61/61 default/POSIX. `.1.5.4.2` closes Julia's independent canonical trace; `.1.5.4.3` is active for one warmed recurring four-command gate and CLI-lane closeout. `.10.1` parks introspection/MCP; `.11.1` parks generic codeblocks/`with`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | Local proof is 1,019 assertions/nine processes/99 fixtures; Julia now passes 61/61 shared CLI cases default/POSIX. Recurring matrix `.1.5.4.3`, capability census `.1.6`, and generated-source `.3` remain. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
