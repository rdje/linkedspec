...
| `FUTURE-PARITY-BACKLOG` | `active` / MCP `.10.9.7.1.1.2` all-twenty consumer proof signoff-complete from clean `8352d535` | `Overall roadmap - future parity backlog` | The five existing consumers prove capabilities plus all nineteen queries preserve direct-native/MCP text/structured/decoded/digest identity on Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT. Complete canonical proof is green through Phase 0 1,031/1,031 in 661s; only the clean 100/300 commit remains. Rollout stays 5/5 implementations + 6/6 runtimes pending/114 until routed promotion `.1.1.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
