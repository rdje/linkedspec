...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current surface `.1.6.1`, descriptors `.1.6.2`, and structured diagnostics `.1.6.3` are closed. Native resolution contract `.1.6.4.1` is fixed; Rust native API `.1.6.4.2` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
