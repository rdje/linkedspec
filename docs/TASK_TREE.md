...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart callable-codeblock construction `.11.5.1` signoff-complete; `.11.5.2` next after clean commit | `Overall roadmap - future parity backlog` | Dart now has exact inert eight-field literal/signature/body/source/Unicode-span state across native, compiled, generated, emitted, user-function, and semantic roles; focused 8/8, complete Dart 362, storage 18/47, CLI 66x2, corpus 105/105, and canonical Phase 0 1,031/1,031 pass. Dynamic invocation remains `.11.5.2`; no push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
