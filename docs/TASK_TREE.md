...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.4` — `.7.3.7` closed the user-directed timeout re-debug by moving parser construction inside the oracle hard-timeout child; next triage `portmap`/`lib_reader`/`ebnf` structural divergences | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
