...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.1` done 2026-07-09 - Dart now has seek/consume regex matching, stable alternative identity, capture/named-capture records, char-offset projections, entry/local match registers, cursor state, and zero-progress detection. Next frontier `.4.2` starts rule dispatch and lifecycle order. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
