...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Root selection retains final five-backend recurring/public no-drift at 7/0; duplicate-slot recurring/public no-drift is closed at 7/0/59 through the exact six-runtime recurring driver. Public cursor no-drift `.9.1.9` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
