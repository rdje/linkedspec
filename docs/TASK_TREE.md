...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust root core leaf `.9.1.1.2.2.1` is done with ordered native selection and full signoff; composed loaded/generated/emitted/trace route leaf `.2.2` is next but remains pending until the clean commit boundary. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
