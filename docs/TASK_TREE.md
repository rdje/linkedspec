...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `TRACE-OBSERVABILITY.1` unless a new terse leaf is split or another active tree is reprioritized.
...
`RUST-PARITY` unblocked `TOP-RULE-AS-NORMAL.3.2`; that leaf has since closed, so PNT advances to `TRACE-OBSERVABILITY.1`.
