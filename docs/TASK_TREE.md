...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua reusable corpus execution `.6.1.2` passes controlled typed non-aborting proof at 160/160 on both ABIs; permanent core window `.6.1.3` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | `.6.1.2` validates before selection and records automatic wrapped execution without aborting later fixtures at 160/160 per ABI; exact ordered core offsets 0-39 `.6.1.3` are active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
the developer CLI remains validation-only. Permanent ordered core window `.6.1.3` is active.
