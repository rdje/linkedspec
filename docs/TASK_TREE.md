...
| `ROADMAP-DRIFT-RECONCILE` | `active` | `Overall roadmap — documentation and book sync` | Created 2026-06-23 to OWN deferred `ROADMAP.md` / `ARCHITECTURE_STATE.md` drift. User `PNT` reactivated the lane on 2026-07-08 after `SPEC-FORMAT-TERSE.14` closed. **`.1` DONE 2026-07-08** — long-form `ROADMAP.md` now names the current `SPEC-FORMAT-TERSE` / ADR `0007` terse direction, stops presenting `declare(...)` as current authoring, records `LEGACY-VHDL-RETIRE` + `NONCORE-QUARANTINE` (`plugin/` gone, 13 `.plg` under `noncore/plugin`, `perl/` core-only except `PPlugin.pm` compatibility), updates phase0 to `PASS 1..1026`, records the 95-fixture Rust oracle, and states `.spec` = universal contract / Perl = reference / Rust = implemented lockstep variant / Julia+Dart = future variants. Frontier → **`.2`** (`ARCHITECTURE_STATE.md` refresh). | [docs/tasks/ROADMAP-DRIFT-RECONCILE.md](docs/tasks/ROADMAP-DRIFT-RECONCILE.md) |
