...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart typed capture `.10.5.5.1`, immutable topology-validated observed-index derivation `.2`, and generated/emitted direct/traced propagation `.3` now preserve the exact twentieth response, results, diagnostics, trace bytes, exit omission, callback identity, and generated-source v2/format 2. Composition closeout `.10.5.5.4` is active; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
