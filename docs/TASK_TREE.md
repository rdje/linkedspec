...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Selector retirement `.12.1` is re-closed through 56-file follow-up `.12.1.10`. Lua scalar numeric calls, receivers, aggregate reducers, and array terminals pass 91/91 on both ABIs; focused numeric/public no-drift `.4.3.3.4` is active. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Scalar numeric calls/receivers and strict aggregate reducers pass 91/91 on both ABIs; focused numeric/public no-drift `.4.3.3.4` is active, with later variadic/native/descriptor/generated obligations still explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
