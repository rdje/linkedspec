...
syntax contract and backend rollout after a clean pivot. The contract plus Perl and Rust implementations are
complete; Dart `.16.4` is next.
...
implements the contract on Perl with exact live/generated execution, and `.16.3` adds Rust typed/native/
serialized/emitted/generated execution. Dart `.16.4` is next; Rust's pre-existing `.contains()` missing-argument
default is delegated to helper owner `.5`; Lua
