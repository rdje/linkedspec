...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart generic contextual final blocks `.11.5.3` and parent `.11.5` signoff-complete; landing commit pending | `Overall roadmap - future parity backlog` | Exact final-only metadata plus one signature-governed helper/user-function/receiver/tree normalization pass complete across native/reconstructed/generated/emitted Dart. Complete Dart and canonical gates pass; `.11.6.1` may activate only after `.11.5.3` lands from clean `74725ff0`, the brief is cleared, and Git is clean. No capability/MCP movement or push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
