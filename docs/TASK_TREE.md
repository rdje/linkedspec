...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl immutable query `.10.3.4` is complete and canonical-green from `b4805f23`: all 19 static response digests, 26 request/error boundaries, primary 66x2, and Phase 0 1,031/1,031 pass. Runtime observation/routes `.10.3.5` is next after the clean commit; admission remains later. The repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
