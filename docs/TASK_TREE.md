...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua scaffold planning `.8.1.0` corrects v1/v2/v3 scope and splits emitter `.8.1.1` from isolated host proof `.8.1.2`; `.8.1.1` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | CLI parent `.7` is closed; generated-source planning `.8.1.0` splits exact v1/v2/v3 emission `.8.1.1` from fresh-process PUC/LuaJIT proof `.8.1.2`, and `.8.1.1` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
| `BACKEND-COMPANION-BOOKS` | `proposed` / dependency-gated | `Documentation architecture - backend implementation companions` | `.0` adopts one normative neutral mdBook plus linked Perl/Rust/Dart/Julia/Lua implementation companions; `.1` is a read-only inventory after current backend parity, and no scaffold/content migration exists yet. | [docs/tasks/BACKEND-COMPANION-BOOKS.md](docs/tasks/BACKEND-COMPANION-BOOKS.md) |
...
Index note 2026-07-16: `LUA-BACKEND-PARITY.8.1.0` corrects the older v1/v2-only emitter leaf against ADR `0041`'s
later final-codeblock-v3 union. Deterministic effective-state emission/metadata/errors/entrypoints is `.8.1.1`;
fresh-process PUC Lua/LuaJIT valid/corrupt load, run, and cleanup is `.8.1.2`. `.8.1.1` activates without source,
runtime, CLI, corpus, status, generated-capability, or census change.
