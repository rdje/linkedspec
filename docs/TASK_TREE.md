...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Perl preflight `.2.0` signoff-complete from clean `db299789`; `.2.1` next after atomic 222 lands | `.spec language evolution / lossless segmentation and source preservation` | Neutral parent `.1` landed unchanged as atomic 221/300. Toolbox-led `.2.0` freezes exact `.2.1` parsing/metadata+dormant RED, `.2.2` same-guard live state/accessors+recognition-effect synchronization, `.2.3` emitted/loaded parity, and `.2.4` Perl-only admission. Canonical proof is green while current rollout remains 1 complete + 8 pending / 55 mutations; recognition stays 133/246/58, public helper coverage 122, typed source 9/5/114, and every planned syntax/runtime/outward surface remains absent in this leaf. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
