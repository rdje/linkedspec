...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua harray audit `.4.3.5.0` splits construction, views, transforms, mutation, and no-drift; construction/identity `.4.3.5.1` is active and callbacks remain `.4.3.6`. Flat/concat/hash arity, negative counts, dropped-transform rebinding, invalid-join, and implicit child-push result drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array family is closed at 99/99. Harray mechanisms are split by `.4.3.5.0`; copied construction/explicit splicing `.4.3.5.1` is active. Tree callbacks and later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
