...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.1.2` after the `.1.1` preflight closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.1.1` done 2026-07-10 - Julia 1.12.6 is installed through Homebrew and matches the official current stable release; the intended `julia/` package layout, package/test commands, optional formatter/linter commands, Julia-specific CLI, and corpus-runner entrypoint are recorded. Active frontier advances to `.1.2` for the minimal package scaffold. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.1.1` and now sits at `JULIA-BACKEND-PARITY.1.2`. Generated Dart source remains a future
split proof lane.
