...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Native Perl MCP behavior-free owner/seam audit `.10.9.2.0` is complete from clean neutral-contract commit `9307524a`; ADR `0057` freezes generated-contract, registry, strict-wire, and security owners without server behavior. In-process registry/decoded dispatch `.10.9.2.1` activates only after the clean plan commit, followed by stdio/lifecycle `.2`, exact admission `.3`, and no-change closeout `.4`. Director-approved ADR `0056` remains pending under `.14.1-.8`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
