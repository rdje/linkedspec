...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl MCP `.10.9.2.1` is complete: generated filesystem-free binding, private schema runtime, secure opaque registry, exact decoded discovery/list/call/cancel dispatch, adversarial proof, and canonical signoff. Strict wire/lifecycle `.2` is next; exact admission `.3` and no-change closeout `.4` remain ordered, while ADR `0056` stays pending under `.14.1-.8`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
