...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Shared Lua decoded-server leaf `.10.9.6.1` is implementation/signoff complete from clean `315af596`: 111 + 210 assertions per ABI, complete dual-ABI Lua/cross-runtime/canonical proof, and 94 governance mutations pass. Its commit/brief-clear/clean boundary is next; only then may strict stdio `.10.9.6.2` activate. Formal status stays 4/5 implementations + 4/6 runtimes with rollout pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
