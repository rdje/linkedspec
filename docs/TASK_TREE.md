...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust static v1 projection `.10.4.2` is signoff-complete from clean `c92172b8`; calls/staging `.10.4.3` follows only after its clean commit. Semantic rollout/admission remain 2/9 and 1/6. Startup mdBook drift is queued separately under `.23`. repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
