...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart progressive private authority `.14.6.4.1` focused-signoff-complete; four carriers `.14.6.4.2` next | `Overall roadmap - future parity backlog` | Neutral, Perl, and Rust remain admitted at 3/9/95. Dart now has an ActionIR-independent immutable registry/invocation/rebased-view authority with all 26 diagnostics and a dormant GREEN focused proof; the unchanged final-path consumer remains +4/-1 only at the missing dedicated node. Commit `.14.6.4.1`, then implement four carriers `.4.2` and admission `.4.3`; Julia/Lua `.5-.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
