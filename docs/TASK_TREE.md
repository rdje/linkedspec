...
| `DART-BACKEND-PARITY` | `done` / `closed` (scoped milestone) | `Overall roadmap - future backend parity (Dart first)` | `.7.5` closed the 99/99 interpreter-first Dart tree. ADR `0023` later proved complete public parity remains global work: `FUTURE-PARITY-BACKLOG.1.5.3` owns Dart's exact primary CLI and `.1.6`/`.3` own capability/codegen gaps. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.2.3` brings Rust to 61/61 with canonical trace; `.1.5.2.4` is active for recurring-gate/default-POSIX no-drift closeout. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | `.7.3.3` done 2026-07-10 - exact local 1,017/nine-process/99-fixture proof is aligned. The tree remains active, not complete, through global `.1.5` CLI identity, `.1.6` capability census, and `.3` generated-source work. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
surfaces agree on the strict 61-case Perl reference, Rust `.1.5.2.3` passes all 61 unchanged cases, and
`.1.5.2.4` is the active recurring-gate/default-POSIX no-drift frontier against the unchanged
...
and a thin MCP transport that owns no semantic behavior. The active backend frontier is Rust `.1.5.2.4`.
