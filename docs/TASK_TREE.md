...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia authority `.14.6.5.1` focused-signoff-complete | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart remain admitted at 4/9/103. Julia `.14.6.5.0-.1` preserve the exact 55-pass/one-RED final path while adding one unexported immutable authority and complete dormant proof; carrier leaf `.2` is next. Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
