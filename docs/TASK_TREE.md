...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.4.3` done 2026-07-09 - Dart statement-form helper mutation and split replacement are aligned, `simenv_multiline_value` plus both `lib_reader` fixtures pass, and the parser-smoke window is 22/31 green. Next frontier `.6.2.4.4.4` is legacy structural smoke output parity. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
