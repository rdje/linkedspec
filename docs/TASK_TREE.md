...
| `TASK-TREE-METADATA-HYGIENE` | `active` | `Overall roadmap — durable architecture / task-tree hygiene` | Created 2026-07-07 to own the user-requested audit of non-closed task trees and stale per-file task metadata. **`.0` DONE** (tracking-only): central live non-closed trees are `SPEC-FORMAT-TERSE`, `STAGED-LINKED-PARSING`, `DOCTRINE-ENFORCEMENT-ADOPT`, `SPEC-LANG-REFERENCE`, and `ROADMAP-DRIFT-RECONCILE`, with their frontiers empty/deferred/paused at `.0` creation time. The user then reactivated `SPEC-FORMAT-TERSE.14`; that lane is closed by `.14.5`, and `.1` top-level metadata reconciliation is done; the next PNT-eligible hygiene leaf is **`.2`** stale frontier/verification rows, while `.3` gate decision remains deferred. | [docs/tasks/TASK-TREE-METADATA-HYGIENE.md](docs/tasks/TASK-TREE-METADATA-HYGIENE.md) |
