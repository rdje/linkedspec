...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Dart parent `.4` landed as atomic 239 at `43ed1c8f`; behavior-free Julia `.5.0` active | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, and Dart run ordinarily and in the rooted route at gap 4/5/57 plus ten Rust and ten Dart admission mutations. Julia `.5.0` audits the current absent syntax/accessors and freezes `.5.1-.5.5`: metadata/dormancy, same-authority native state, normalized/descriptor/generated carriers, independent emitted proof, and primary/nine-role admission. No Julia behavior or rollout moves in `.5.0`; Lua, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
