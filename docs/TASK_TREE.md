...
| --- | --- | --- | --- | --- |
| `PPLUGIN-WALKTHROUGH-DRIFT` | `active` | `Overall roadmap — documentation and book sync` | Created 2026-07-07 to own a narrow mdBook drift before edits: the pplugin walkthrough still describes an older below-1.0000 descriptor readiness/compatibility-surface status even though the current `pplugin` descriptor reports 1.0000 ready, zero blocked rules, and zero compatibility-surface rules. Tracking leaf `.0` is done; frontier is `.1` for the walkthrough/Knowledge/live-doc correction. | [docs/tasks/PPLUGIN-WALKTHROUGH-DRIFT.md](docs/tasks/PPLUGIN-WALKTHROUGH-DRIFT.md) |
