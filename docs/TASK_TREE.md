...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.2.2` after the Julia AST-data boundary. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.2.1` done 2026-07-10 - Julia now has source AST/data types and neutral JSON projection for spec files, functions, source spans, staged parse jobs, rule modes, body elements, edges, and fluent calls. Active frontier advances to `.2.2` for parsing `.spec` rule paragraphs into those types. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.2.1` and now sits at `JULIA-BACKEND-PARITY.2.2`. Generated Dart source remains a future
split proof lane.
