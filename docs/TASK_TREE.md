...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed. Strict source/map/outcome `.10.3.1` is clean at `0558c65a`. Static correction `.10.3.2.0` is signoff-complete: default rules are `or`/`seek`, compiled no-edge ownership is `none`, and rule-local cross-contract proof rejects 53 mutations without behavior/admission. Its clean commit precedes Perl projection `.10.3.2.1`; rollout/admission stay 1/9 and 0/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
