...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.5.2` after terminating control advanced simenv to statement mutation. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.5.1` done 2026-07-10 - immediate explicit/default `exit_now(...)` termination retains structured attribution and advances simenv to its earlier statement-mutation prerequisite. Full tests pass with 801 assertions and status `runtime-corpus-exit-now`; shipped smoke remains 25/31 and `.6.2.4.5.2` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.2.3` have since landed; `.6.2.4.5.2` is active.
...
`runtime-corpus-exit-now`, and `.6.2.4.5.2` is active.
