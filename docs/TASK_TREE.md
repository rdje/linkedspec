...
| `NONCORE-QUARANTINE` | `active` (**current focus**) | `Overall roadmap — keep only portable/cross-variant code (non-core quarantine)` | `.1` dependency inventory + `.2` relocate done 2026-06-19: **41 KEEP / 36 non-core `.pm`; 13 `.plg` non-core** (shipped specs reference 0 plugins/cross-specs → the whole legacy domain island is unreachable; zero core→domain edges). **Relocate (`git mv`) to `noncore/`, NOT delete** (user: keep refactor/port/publish/delete options open; ledger `noncore/README.md`). 12 zero-ref modules moved. `.3` = domain clusters + their phase0 subtests (clears the back-half hangs) → `.4` = 13 `.plg` → `.N` = plugin machinery (POSTPONE) → `.V` verify green phase0. | [docs/tasks/NONCORE-QUARANTINE.md](docs/tasks/NONCORE-QUARANTINE.md) |
