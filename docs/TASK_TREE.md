...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.3.1` — `.8.3` split the non-repetition generated-family lane before code; next carry rule-mode/family metadata and emit the generated non-REP family plan | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.3.1` unless a new terse leaf is split.
