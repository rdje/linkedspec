...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua staged `.14.7.7.5` canonical-signoff-complete; `.14.7.8` next | `Overall roadmap - future parity backlog` | The committed shared Lua-5.1 source independently recomposes at 888/888 on PUC Lua and LuaJIT with exact once-per-ABI topology, peer projections, and 106-mutation governance unchanged. Parent `.14.7.7` closes without executable movement; recurring five-source/six-route proof `.14.7.8` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
