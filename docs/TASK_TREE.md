...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Behavior-free Dart semantic audit `.10.5.0` is complete from clean Rust admission `0de66ae9`. It freezes shared self-hosted label/corpus-freshness/current-bridge closure `.10.5.0.1`, Dart label parity `.10.5.0.2`, then semantic source/static/calls/query/runtime/admission `.10.5.1-.6`; four stale `spec_spec_*` inputs mean 105/105 does not yet prove current canonical grammar. No executable behavior or ledger changed; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
