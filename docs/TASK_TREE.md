...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Perl typed-source RED `.14.2.1.0` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; typed source-location neutral/public parent `.14.1`, runtime plan `.14.2.0`, and live correction `.14.2.0.1` are closed. Current typed-source truth is 3/11 with 37 mutations and exact backend admission owners. Perl `.14.2.1.0` freezes independent missing-core and missing-92-row-projection consumers while current neutral/mark/cursor/generated behavior stays green; `.14.2.1.1` is the next clean core-value leaf. Knowledge Map is synchronized; sole-facing support stays unchanged until admission `.14.2.1.3`, and rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
