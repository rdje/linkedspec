...
| `FUTURE-PARITY-BACKLOG` | `active` / blocked | `Overall roadmap - future parity backlog` | Diagnostic-output `.5.1` is closed at 8/0. Logical audit `.5.2.0`, neutral ADR `0043`/contract `.5.2.1`, and Perl typed-AST/runtime `.5.2.2` are complete at 1/7. AND/OR cursor audit `.9.1.0` rejects global override and exposes default drift; decision `.9.1.1` awaits the blind-call child-ownership answer before Rust `.5.2.3` resumes. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
