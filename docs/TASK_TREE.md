...
| `FUTURE-PARITY-BACKLOG` | `active` / callable public no-drift `.11.7.2` signoff-complete from clean `c86f32fa` | `Overall roadmap - future parity backlog` | Exact 23-document/nine-forbidden-claim/20-mutation lockstep, mdBook 79 files / 13,996 KiB, KM 775/6,283, and canonical Phase 0 1,031/1,031 in 659s plus the callable matrix pass. Parent `.11.7` is closed; land clean before activating Lua audit `.11.8.0`. Exclusion freshness stays `.24.0-.2`; no behavior/capability-row/MCP/root-README/push movement. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
