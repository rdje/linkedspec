...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.2.4` after the Julia source-validation boundary. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.2.3` done 2026-07-10 - Julia `validate_spec(...)` now validates parsed source ASTs for top-rule presence, duplicate labels/functions, edge consistency, references, regex structure, function registry shape, and strict unused-rule behavior. Active frontier advances to `.2.4` for top-level `fn` shell projection. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.2.3` and now sits at `JULIA-BACKEND-PARITY.2.4`. Generated Dart source remains a future
split proof lane.
