...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl typed records execute dynamically; audit `.11.3.3.0` found no final-codeblock declaration schema, so design `.11.3.3.1` is active before behavior. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
acceptance or the callback's own parameters. Declaration design `.11.3.3.1` is active before Perl behavior `.2`;
no parser, registry, lowering, runtime, fixture, or generated behavior changed.
