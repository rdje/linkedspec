...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl/Rust/Dart are 61/61. Dart `.1.5.3.3` closes canonical trace default/POSIX; `.1.5.3.4` is active for recurring gate/no-drift. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
