...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage `.1` plus Perl/Rust/Dart/Julia `.2.1-.2.4` are complete. Julia now supplies a source-bearing five-package offline depot, proves 17 temporary owners plus generated/trace paths stay on repository storage, migrates 85 current commands, and deletes both exact old depots plus the exact former-checkout shared-log record. `.2.5` is the clean Lua storage frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
