...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart-native label rollout `.10.5.0.2` is split into `.0-.4`; generated classifier/validator/prefix foundation `.0` is complete, and `.1` is the next pending parser/external-AST validation frontier after its clean commit. Semantic governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
