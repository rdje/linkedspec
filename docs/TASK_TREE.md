...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia static-authority plan `.10.6.3.0` is complete and verified: five exact targets, authored/compiled correlation, neutral normalization, privacy fences, full Julia/matrix/canonical proof, and cleanup pass without behavior. Graph implementation `.10.6.3.1` is next only after the clean plan commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
