...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Shared executable-label/corpus-freshness/current-bridge parent `.10.5.0.1` is split as generated class/checker `.0`, canonical grammar+Dart structural execution `.1`, and refreshed corpus/cross-runtime closeout `.2`; `.10.5.0.1.2.0` is done in its clean commit with canonical four-input freshness and the Perl sparse-AND repair green across full corpus/canonical routes. `.2.1` remains pending until that commit, then owns label-boundary/five-runtime closeout. Semantic governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
