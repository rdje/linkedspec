...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage plus Perl/Rust/Dart/Julia/Lua `.1-.2.5` are complete. Lua roots all 13 allocation owners, dual-ABI native modules, generated output, and traces on repository storage; exact old Lua workspace residue is zero. `.2.6` is the Python/shell/book/Knowledge Map/conformance/tool storage frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
