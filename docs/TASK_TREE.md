...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed. Semantic-introspection `.10.2` is active from clean design commit `056d413b`. ADRs `0049`/`0050`, six neutral fixture groups, 20 digest-locked responses, 50 rejected mutations, source/privacy/budget/explanation/runtime proof, and canonical registration are implemented without backend behavior; final canonical signoff/commit precede Perl `.10.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
`.10.2` commit boundary remains before Perl `.10.3` can activate.
