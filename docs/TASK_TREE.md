...
| `FUTURE-PARITY-BACKLOG` | `active` / shared Lua `.14.6.6.3` canonical-signoff-complete; `.4` next | `Overall roadmap - future parity backlog` | One unchanged 178-assertion carrier is ordinary once per Lua ABI and has one exact canonical route per ABI; only PUC Lua/LuaJIT advance, making private progressive rollout 7/9/112. The separate authority remains dormant at 273/273 per ABI; `.4` will independently recompose and close the Lua parent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
