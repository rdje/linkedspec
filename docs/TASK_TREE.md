...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Array audit found current statement-only result prose superseded by admitted uniform binding; `.12.1.11` is active before Lua array code. Numeric parity remains closed at 91/91. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array mechanisms are split under `.4.3.4.0`; copied construction `.4.3.4.1` waits for uniform-binding result-doc repair `.12.1.11`. Numeric parity remains closed at 91/91, with later variadic/native/descriptor/generated obligations explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
yet several current book/KM/backend summaries still teach the old rule. `FUTURE-PARITY-BACKLOG.12.1.11` is active
to repair and gate that cross-cutting drift before Lua construction `.4.3.4.1` begins.
