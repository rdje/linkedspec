...
syntax contract and backend rollout after a clean pivot. The contract plus Perl, Rust, Dart, and Julia
implementations are complete; Lua `.16.6` is next.
...
emitted/generated execution, `.16.4` adds Dart typed/native/generated/emitted/CLI execution, and `.16.5` adds the
same Julia paths. Lua `.16.6` is next; Rust/Dart/Julia pre-existing `.contains()` missing-argument outcomes are
delegated to helper owner `.5`; Lua
