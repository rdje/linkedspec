...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia `.7.3.2.3` closes native primary execution and canonical JSON after trace/argument preparation. Active PNT is `.7.3.2.4` normalized failures/exits/trace routing; `.1.5`, `.1.6`, and `.3` own global convergence before Lua. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.7.3.2.3` done 2026-07-10 - native rule/function parsing, runtime controls, recursively key-sorted direct JSON, 942 assertions, and 99/99 green. `.7.3.2.4` is active for normalized failures/exits/trace routing. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
package assertions, direct canonical primary smoke, and 99/99 pass; `.7.3.2.4` is active for normalized failures,
