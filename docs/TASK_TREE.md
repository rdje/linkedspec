...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.5.3` after registered user-function runtime execution landed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.5.2` done 2026-07-10 - Julia executes registered exact-arity calls before helper fallback with eager caller arguments, fresh typed local stores, receiver continuation, standalone result drop, and structured recursion cycles; full tests pass with 671 assertions and status `runtime-user-functions`. Active frontier advances to `.5.3` descriptor-shape parity. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
pass with 671 assertions and status `runtime-user-functions`; `.5.3` descriptor-shape parity is active.
