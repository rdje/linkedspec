...
| `FUTURE-PARITY-BACKLOG` | `active` / MCP `.10.9.7.1.1.0` authorized repair plan | `Overall roadmap - future parity backlog` | Real Perl all-twenty spend proves 17/20 identities and three shared schema/default-policy blockers. The director authorized exact repair on 2026-07-30. Active `.1.1.0` freezes atomic contract/bindings/five-server correction `.1`, all-twenty six-runtime evidence `.2`, routed promotion `.3`, and unchanged closeout `.4`; current state remains 5/5 implementations + 6/6 runtimes, rollout pending/114 until proof earns promotion. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
