...
| `STAGED-LINKED-PARSING` | `active` | `Overall roadmap — .spec language model / parser composition` | `STAGED-LINKED-PARSING.5.3` — `.1` adopted staged linked parsing doctrine, `.2` specified future import/composition, `.3` specified future parse-job annotations, `.4` specified deterministic registry/dispatch queue semantics, `.5.1` selected user-function body text as the first prototype payload family, and `.5.2` audited the function-definition AST shape/harness before code. Next preserve source provenance for function-body payload parse jobs. | [docs/tasks/STAGED-LINKED-PARSING.md](docs/tasks/STAGED-LINKED-PARSING.md) |
