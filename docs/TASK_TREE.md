...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Planning `.0` freezes ADR `0053`, exact live/tracked/cache/tool inventories, necessary-only cross-volume access, verified immediate old-data deletion, and the `.1-.5` migration/enforcement order. `.1.1` is the clean implementation frontier for repo-derived scratch/cache roots. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
