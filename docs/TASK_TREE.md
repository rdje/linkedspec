...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl, Rust, Dart, and Julia hard-reject exact selectors with complete no-drift gates; Lua `.12.1.8.5` is active. Toolbox repair `.13.1` and structural/progressive/staged authoring arc `.14.1-.4` remain queued behind selector retirement. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
