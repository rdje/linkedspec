...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.2.3` after the Julia source-parser boundary. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.2.2` done 2026-07-10 - Julia `parse_spec(...)` now parses core `.spec` rule paragraphs into source AST types, including headers/modes, regex slots, lifecycle blocks, action/blind-call edges, fluent continuations, markers, comments, and block boundaries. Active frontier advances to `.2.3` for frontend validation and strict syntax behavior. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.2.2` and now sits at `JULIA-BACKEND-PARITY.2.3`. Generated Dart source remains a future
split proof lane.
