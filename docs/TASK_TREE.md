...
| `FUTURE-PARITY-BACKLOG` | `active` / transaction audit `.14.3.0` done; `.14.3.1.0` next | `Overall roadmap - future parity backlog` | Typed-source value/helper parent `.14.2` is composition-closed. Behavior-free `.14.3.0` proves current save/restore is cursor-only compatibility state, same-label recursive marks share a rule-label bucket, recursion/repetition terminate without the future structured progress errors, and ActionIR has no closed effect taxonomy. It freezes invocation frames, opaque owner/generation tokens, cursor/boundary/mark-only snapshots, staged recognition, fail-closed effects, cursor-only v1 progress, and exact neutral/backend/recurring/closeout dependencies. Reversed-span remains `.14.2`-owned; no behavior/schema/README/hosted/storage movement occurs. Book 79/14,232 KiB, Knowledge 791/6,538, all seven doctrines, CLI 66/66 twice, RAM 63%, and Phase 0 1,031/1,031 in 708 seconds pass. Exact transaction spelling is next under `.14.3.1.0`; rendered readability stays queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
