...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed. Static correction `.10.3.2.0` is clean at `40d35201`; Perl `.10.3.2.1` has complete static 5 / foundation 5 / semantic 6/20/53 / primary 66x2 / Phase 0 1,031 canonical signoff. Public query/admission stay absent at 1/9 and 0/6; calls/staging `.10.3.3` follows the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
