...
| `FUTURE-PARITY-BACKLOG` | `active` / callable admission `.11.8.4` signoff-complete; clean landing next | `Overall roadmap - future parity backlog` | Five-backend/six-runtime callable proof, 22 mutations, 25 public documents, satisfied-exclusion removal, unchanged 80/0/0 census, and parent `.11.8`/`.11` closeout are green from `917038e7`. Land as 125/300, clear/prove clean, then activate queued exclusion-freshness audit `.24.0`; no push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
