...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, source/outcome `.10.7.2`, private static `.10.7.3`, and calls/staging/generated `.10.7.4` are composition-closed. Query audit `.10.7.5.0` freezes 19 hashes/26 boundaries; private `.10.7.5.1` now matches nine exact non-traversal hashes with protected recursive values at focused 1,080 per ABI and no public query name. Private traversal/limits `.10.7.5.2` is next. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
