...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion-freshness `.24.0` signoff-complete | `Overall roadmap - future parity backlog` | Exact audit from clean `47b40c7a` retains plugin legacy, re-owners valid future parse-job work from superseded `.2` to active `.14`, removes satisfied semantic/cursor narratives later, and freezes schema-v2 disposition/retention/task-status governance plus 24 mutations without behavior change. Focused/KM/book/doctrines/canonical pass at 80/0/0, 783/6,343, 79/14,056 KiB, RAM 52%, and Phase 0 1,031/1,031 in 651s. Land 126/300, then activate `.24.0.1`; no push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
