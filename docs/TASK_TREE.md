...
| `FUTURE-PARITY-BACKLOG` | `active` / staged audit `.14.7.0` signoff-complete; diagnostic `.14.7.1` next | `Overall roadmap - future parity backlog` | The audit proves one-depth `actionir-body.spec` function staging on five backends/six runtimes, distinguishes inert raw policy strings from implemented function stitching, assigns Perl/Rust/Dart compile-context loss to `.1`, and freezes neutral `.2`, backend `.3-.7`, recurrence `.8`, public `.9`, and recomposition `.10` without behavior. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
