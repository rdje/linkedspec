...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.1` after the shipped-smoke boundary was measured at 10/31 and split. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.0` done 2026-07-10 - shipped-spec/parser-smoke fixtures 68–98 start at 10 passed / 21 failed and are split into owned mechanism leaves before source changes. Status remains `runtime-corpus-middle` at 757 assertions; `.6.2.4.1` anonymous capture-boundary helpers are active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.3` have since landed; `.6.2.4.1` is active.
...
behavior changes; `.6.2.4.1` is active.
