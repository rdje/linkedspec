...
| `FUTURE-PARITY-PARTITION-CAPACITY` | `active` / `.0` focused-signoff-complete; canonical `.1` next | `Repository continuity / bounded task evidence` | Clean Dart closeout `3ccaf7c3` exposes `.14` exactly at its 5,000-line member ceiling before Julia `.14.6.5` can be task-split. ADR `0084` now freezes unchanged pressure ceilings/IDs, one bounded `.14.6.5-.14.8` member, exact source accounting, all topology consumers, and rollback. `.1` implements atomically; `.2` independently recomposes; no backend/parser behavior moves in this tree. | [docs/tasks/FUTURE-PARITY-PARTITION-CAPACITY.md](docs/tasks/FUTURE-PARITY-PARTITION-CAPACITY.md) |
...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart `.14.6.4` closed at `3ccaf7c3`; Julia `.14.6.5` blocked only by bounded partition capacity | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart are admitted at 4/9/103. Dart's private authority, dedicated node, four carriers, ordinary discovery, and exact canonical route are current without outward or typed-row movement. The `.14` task member is exactly 5,000 lines, so `FUTURE-PARITY-PARTITION-CAPACITY.0-.2` must add and recompose one equally bounded `.14.6.5-.14.8` member before Julia task splitting; Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
- Capability exclusion public closeout `.24.2` is complete, and Capability exclusion freshness is public-closed
  under `FUTURE-PARITY-BACKLOG.24`.
