...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `TOP-RULE-AS-NORMAL.3.2` unless a new terse leaf is split.
...
`RUST-PARITY` unblocks `TOP-RULE-AS-NORMAL.3.2`, which is the next PNT leaf.
