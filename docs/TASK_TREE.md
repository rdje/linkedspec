...
| `FUTURE-PARITY-BACKLOG` | `active` / Perl `.14.3.2` complete; Rust `.14.3.3.0` next | `Overall roadmap - future parity backlog` | Typed-source `.14.2`, neutral transaction `.14.3.1`, and Perl transaction `.14.3.2` are composition-complete at the current atomic 187/300 candidate from clean activation parent `e173bbcb`. Admission `.14.3.2.3` canonically executes the exact 51-test consumer once, promotes only `perl` to rollout 2/9, and updates current guidance without changing production implementation bytes. Rust dormant RED `.14.3.3.0` is the next task-tree-first activation after atomic 187 lands and the clean handoff is proved. `TASK-TREE-METADATA-HYGIENE.5` separately owns active-frontier prevention; rendered readability remains queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
