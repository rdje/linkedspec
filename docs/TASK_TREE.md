...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia source/outcome plan `.10.6.2.0` freezes exact text/byte input, source ceilings/map, detached compiled outcome, descriptor-alias/privacy/no-execution boundary, and `.1-.3` implementation order without behavior or ledger change. `.10.6.2.1` follows only after the clean plan commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
