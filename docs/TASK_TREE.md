...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.4.1` — `.7.3.4` reproduced `portmap`/`lib_reader`/`ebnf` structural mismatches and split implementation owners; next fix parser/compiler handling for `lib_reader`'s regex-less top-rule header-rest action edge | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
