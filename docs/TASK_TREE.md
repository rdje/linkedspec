...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Migration/residue `.1-.3` and enforcement `.4` are complete. `PROJECT-DATA-STORAGE` has 28 cases and 38 routed boundaries; relocated Perl/Rust/Dart/Julia/Lua/tool containment, all six storage oracles, eight affected parity drivers, and canonical CI pass. Final closeout `.5` is next after the clean `.4.2` commit. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
