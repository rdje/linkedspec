...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart semantic foundation behavior-free split `.10.5.1.0` is complete from clean `ecdc1ac7`; strict copied input/private Unicode source map `.10.5.1.1` is next after its clean commit, followed by compiled-or-failed authority `.2` and composed closeout `.3`. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
