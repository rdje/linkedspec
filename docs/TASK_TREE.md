...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust semantic parent `.10.4` is signoff-complete through one exact 12-role consumer at governance 6/20/73, rollout 3/9, and admission 2/6. Commit `.10.4.6` cleanly, then activate Dart adapter `.10.5` task-tree-first. Startup mdBook drift remains queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
