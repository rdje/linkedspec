...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 2/6; Rust `.9.1.4.0-.1` are committed and `.2` is verified. Live per-rule execution `.3` activates only after the clean `.2` boundary. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
