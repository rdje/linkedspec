...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Punctuation-light parent `.16` is complete: ADR 0033 plus `.16.0-.16.6` ratify and implement the narrow aliases, and `.16.7` admits them at capability 64/0/0 with a recurring five-backend/two-Lua-ABI proof. Parenthesis-free `if`/`while` headers remain excluded. Rust/Dart/Julia/Lua `.contains()` arity drift is owned by `.5`; future generated Lua preservation remains `.8.1-.8.4`. Lua block/control/contextual-built-in/tree-callback parent `.4.3.6` closes at 114/114 through no-drift `.4.3.6.6`; user-function final blocks remain `.5.1`, explicit callable values remain `.11.7`, and capture/cursor `.4.3.7` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks, controls, metadata-governed `with`, and deterministic scoped harray/array traversal close at 114/114 through `.4.3.6.6`; capture/cursor audit `.4.3.7.0` splits six runtime mechanisms plus no-drift and advances input/cursor `.4.3.7.1`, while seven documented mark helpers require a cross-backend inventory owner before `.4.3.7.3`. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
