...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `active` / live-history migration `.1` active from clean `dc8dd896` | `Repository architecture / documentation sustainability` | Descriptor `.0` landed as atomic 176/300. `.1` transfers all 13 live-history capability consumers to indexed ADR `0067`, preserves the exact clean 14,872-line source in four immutable manifest segments, installs query plus the eighth `DOCUMENT-HISTORY` doctrine, replaces the stable root with a five-section <=256-line / 32-KiB current view, and ratchets `live_status` from debt to current only through exact staged transition authority. `.2` follows only after `.1` commits and proves clean. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
