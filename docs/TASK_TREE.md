...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Julia carrier `.5.3` focused-signoff-complete for intended atomic 243 | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, and Dart run ordinarily and in the rooted route at gap 4/5/57 plus ten Rust and ten Dart admission mutations. Julia `.5.1-.5.3` now add exact authored/static/compiled provenance, same-recognition-authority native execution, normalized reconstruction, detached compatible descriptors, and same-engine generated-v2 proof behind ten dormancy guards while preserving exact 246 supported ActionIR names and format 2 plans. Independent emitted `.5.4`, primary/admission `.5.5`, Lua, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
