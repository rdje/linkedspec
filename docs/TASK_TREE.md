...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.3.42` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; codebase No; physical mdBook Yes, formal .4 pending. Callable/named-mark reading and corpus prefix complete. Containment `.5` removes verified duplicates; next `.3.3.43` after proposed generic-rule intake. Existing repairs and rgx exclusion remain. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `done` / `.5` duplicate compaction complete | `Repository architecture / documentation sustainability` | Exact batch identities, duplicate subjects and retained per-leaf notes are verified; source/repair fields, all stable IDs and every limit remain unchanged. `.5` requires exact staged canonical proof before landing. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
