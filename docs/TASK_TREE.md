...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, source/outcome `.10.7.2`, private static `.10.7.3`, calls/staging/generated `.10.7.4`, immutable typed/raw-neutral query `.10.7.5`, and runtime observation `.10.7.6` are composition-closed. No-change `.10.7.6.4` passes unchanged focused 1,964 per ABI plus complete signoff from clean `04ab4fec`; exact ordered dual-ABI admission `.10.7.7` follows only after the clean closeout commit. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
