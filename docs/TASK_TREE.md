...
| `FUTURE-PARITY-BACKLOG` | `active` / shared Lua `.14.6.6.1` focused-signoff-complete; `.2` next | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart + Julia remain admitted at 5/9/106. One private Lua-5.1 authority passes 273/273 on PUC Lua and LuaJIT while the separate final path remains 85-pass/one-RED and all carriers stay absent. `.2` next owns the dedicated node and four dormant dual-ABI carriers. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
