...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact Dart MCP admission `.10.9.4.3` is signoff-complete from clean `b3269975`: one ordered twelve-role consumer advances only Dart to 3/5 implementations + 3/6 runtimes while rollout remains pending. No-change closeout `.10.9.4.4` may activate only after the clean admission commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
