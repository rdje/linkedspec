...
| `FUTURE-PARITY-BACKLOG` | `active` / progressive `.14.6` closed through public `.14.6.8`; staged `.14.7` next | `Overall roadmap - future parity backlog` | Progressive behavior is 9/9/116 and public no-drift is 6/12/10/60 after binding recurrence to the committed five-source/six-runtime driver. Typed governance remains 12/2/170; no outward API is admitted. Staged AST enrichment `.14.7` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
