...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | ADR `0044` / `.9.1.1.1` ratifies rule-local cursor policy, mode-sensitive bare edges, migration, descriptor/generated-v2, and conformance; implementation `.9.1.2-.9` is pending. Dart logical `.5.2.4` is verified for its owning commit; Julia `.5.2.5` is the next clean frontier. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
