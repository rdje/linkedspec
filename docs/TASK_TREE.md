...
| `FUTURE-PARITY-BACKLOG` | `active` / shared Lua `.14.6.6.4` canonical-signoff-complete; `.14.6.7` next | `Overall roadmap - future parity backlog` | Both committed 178-assertion carriers and 273-assertion authorities independently recompose across PUC Lua/LuaJIT with governance fixed at 7/9/112 and no executable movement. Shared Lua `.14.6.6` closes; typed recurring five-source/six-runtime proof `.14.6.7` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
