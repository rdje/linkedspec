...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current surface `.1.6.1`, descriptors `.1.6.2`, diagnostics `.1.6.3`, native resolution `.1.6.4`, and Dart full-pipeline trace `.1.6.5` are closed. Non-codegen closeout `.1.6.6` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
