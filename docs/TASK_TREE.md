...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Dart cursor option/CLI `.9.1.5.5` has full 260/63x2/105, 66-file mutation, and canonical signoff; clean commit is the only remaining boundary. Then root-selection decision `.9.1.1.2.0` is next: explicit selector > first authored `::` > first authored `:`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
