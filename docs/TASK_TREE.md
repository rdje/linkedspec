...
| `DART-BACKEND-PARITY` | `done` / `closed` (scoped milestone) | `Overall roadmap - future backend parity (Dart first)` | Global proof is now 178 tests, 105 corpus fixtures, exact 61x2 CLI, admitted full-pipeline trace, and a deterministic isolated v1 emitter scaffold. Generated family-plan and corpus admission remain in global `.3`. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source census is 58/0/2: Perl and Rust pass. Dart scaffold `.3.3.1` is green; family-plan/direct execution `.3.3.2` is active before Dart admission, Julia, and exact closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
