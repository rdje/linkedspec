...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Diagnostic-output audit `.5.1.0` is done without behavior changes. It found distinct arity/evaluation/format/transport/process-control drift and split neutral contract `.5.1.1`, five native backends `.2-.6`, generated/CLI `.7`, symmetric gate `.8`, and no-drift `.9`. `.5.1.1` is active; logical truthiness `.5.2` plus ADR `0036`/`.19` remain later-owned. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
