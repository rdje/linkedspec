...
| `BACKTRACK-SURFACE-RUST-ALIGNMENT` | `active` | `.spec language evolution / backend parity no-drift` | `.1` done 2026-07-09 - broad backtrack helper names are replaced by explicit `save_cursor()` / `restore_cursor()` and `rewind_match_start()` / `rewind_entry_start()` across Perl, Rust, and Dart. Active frontier `.2` lands zero-width/lookahead boundary support for all present and future variants. | [docs/tasks/BACKTRACK-SURFACE-RUST-ALIGNMENT.md](docs/tasks/BACKTRACK-SURFACE-RUST-ALIGNMENT.md) |
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.4` done 2026-07-09 - Dart runtime first landed cursor rewinds and char-based cursor/input helpers; `BACKTRACK-SURFACE-RUST-ALIGNMENT.1` supersedes the public names with explicit cursor controls. Next frontier `.4.5` implements runtime diagnostics and trace controls. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
