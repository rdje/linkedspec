...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage `.1` is complete through lifecycle commit `18c64726`. Perl `.2.1` now routes 24 temporary owners, explicit traces and the primary matrix through managed SSD storage; 65 exact old CLI workspaces were hash-verified, exercised, and deleted. `.2.2` is the clean Rust storage/cache frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
