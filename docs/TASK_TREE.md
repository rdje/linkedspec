...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5` and exhaustive current-surface `.1.6.1` are closed. Descriptor audit `.1.6.2.0` found Perl's stale outward model tag despite a real `compiled_descriptor_state` owner; active `.1.6.2.1` reconciles that identity before Rust projection `.2` and final admission `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
