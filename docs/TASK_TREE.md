...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Rust typed admission `.14.2.2.3` signoff-complete/commit-pending | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location rollout is 5/9 with 39 mutations. `.14.2.2.3` removes only test-local dormancy, requires and ordinarily executes the exact four-test Rust value/projection consumer, and promotes only `rust_runtime`; production source, UTF-8-byte registers, results, schemas, identities, semantic/MCP state, and DSL remain unchanged. Focused 4/4, complete Rust/corpus/generated/17-owner storage/CLI 66x2, real mdBook, isolated rendered HTML paragraphs, all seven doctrines, canonical containment/relocation, CLI 66x2, RAM 36%, and Phase 0 1,031/1,031 in 669 seconds pass. Dart authority/RED `.14.2.3.0` follows only after commit/clean; rendered readability stays queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
