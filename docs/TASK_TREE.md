...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current surface `.1.6.1`, descriptors `.1.6.2`, structured diagnostics `.1.6.3`, and native resolution `.1.6.4` are closed. Dart frontend/compiler `.1` and function/staged `.2` trace pass; composed admission `.1.6.5.3` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | Current proof is 1,040 assertions/nine processes/105 fixtures; Julia passes the exact current language corpus, 61/61 default/POSIX, and the recurring four-backend matrix. Remaining capability rows and generated-source `.3` remain. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
