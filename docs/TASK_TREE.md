...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua core values pass 69/69; pure scalar/string `.4.3.2.1` is active before regex/split/mutation `.2`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Four-kind stores/access/capture reads pass 69/69 on both ABIs; scalar/string pure evaluation and regex-aware mutation are split, with `.4.3.2.1` active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
