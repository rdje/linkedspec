...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.4` done 2026-07-09 - Dart runtime now executes BACKTRACK/IBACKTRACK cursor rewinds and char-based cursor/input helpers. Next frontier `.4.5` implements runtime diagnostics and trace controls. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
...
Index note 2026-07-09: `DART-BACKEND-PARITY.4.4` is done. Dart runtime execution now supports `BACKTRACK()`
as a cursor rewind to the current local match start, `IBACKTRACK()` as a cursor rewind to the initial/entry
match start for the current context, and char-based cursor/input helpers including `cursor_pos`, `cursor_rest`,
`input_slice`, and `input_end_pos`. The director clarified that the `I` in `IBACKTRACK` is the initial-match
context also exposed by the `I` lifecycle. Active frontier advances to `DART-BACKEND-PARITY.4.5` for runtime
