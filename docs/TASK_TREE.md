...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; neutral Rust prerequisite `.14.2.2.0.1` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location rollout remains 4/10 with 38 mutations. Clean audit `.14.2.2.0` proved five Rust aliases return null and two neutral targets were arity-wrong. `.0.1` now maps the rule-start aliases to anonymous `capture_slice` / `capture_slice_len` and checks exact Perl diagnostic/IR/runtime-lowering identity without changing any runtime or the already-correct mdBook. CLI 66x2, RAM 50%, and Phase 0 1,031/1,031 pass. After its clean commit, `.0.2` implements all five Rust aliases and `.0.3` freezes typed RED before `.1-.3`; rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
