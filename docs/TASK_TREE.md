...
| `SCALAREF-RETIREMENT` | `active` | `Overall roadmap — .spec language evolution / compatibility retirement` | `SCALAREF-RETIREMENT.3` — migrate shipped specs, tests, oracle fixtures, and public docs away from `scalaref(...)` using the `.2` replacement contract | [docs/tasks/SCALAREF-RETIREMENT.md](docs/tasks/SCALAREF-RETIREMENT.md) |
...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.2` — `.7.5.2` landed Lispish `scalaref(retv, {content})` parity and the `lispish_x_y` fixture is active; next expand the structurally simple oracle corpus batch | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
Index note 2026-07-02: `RUST-PARITY.7.5.2` is now done. Rust parses/evaluates Lispish legacy
`scalaref(retv, {content})` paths, contains child-return accumulator pushes, supports the Lispish aggregate-wrapper
assignment clear, and the `lispish_x_y` fixture is active in the 63-fixture oracle corpus. The `RUST-PARITY`
frontier advances to `.7.2`.
...
named hash temporary plus `scalar(hash(temp), key)` or `temp["key"]` for expression receiver-dot cases. Frontier
advances to `.3` migration.
