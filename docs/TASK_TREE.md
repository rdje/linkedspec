...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed; neutral semantic contract `.10.2` is committed at `a891d7af`. Perl authority map `.10.3.0` is signoff-complete after exact decoded-source/descriptor/ActionIR/staged/generated/diagnostic/runtime seam probes, a dependency-safe `.10.3.1-.10.3.6` split, and canonical 1,031/608s without behavior; strict source/outcome `.10.3.1` waits for the clean audit commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
Neutral rollout remains 1/9 and backend admission 0/6; `.10.3.1` waits for the clean audit commit.
