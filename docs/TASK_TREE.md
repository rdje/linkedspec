...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.5` — `.8.4` closed direct REP generated families; next integrate generated-source validation with the oracle/corpus contract | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.5` unless a new terse leaf is split.
...
work starts. `.8.4` has since closed direct REP generated-family execution; current frontier advances to `.8.5`
for oracle/corpus validation integration.
