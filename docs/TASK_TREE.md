...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.3.3` done 2026-07-09 - Dart runtime now executes array helper family, array receiver chains, regex split/filter bridges, array numeric reducers, and statement-only end mutations. Next frontier `.4.3.4` starts hash helper family and hash receiver/mutation behavior. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
