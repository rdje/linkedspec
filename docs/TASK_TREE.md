...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua staged `.14.7.7.4` canonical-signoff-complete; `.5` next | `Overall roadmap - future parity backlog` | One shared Lua-5.1 source passes 888/888 independently on PUC Lua and LuaJIT and now runs exactly once per ABI through ordinary/canonical proof. Neutral governance is 106 mutations with every private lifecycle/rollout row complete; `.14.7.7.5` retains independent admitted-route recomposition and parent closure. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
