...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia no-change closeout `.10.9.5.4` is signoff-complete from clean `f8fd4a51`: unchanged owner recomposition and canonical CI preserve 4/5 implementations + 4/6 runtimes, close parent `.10.9.5`, and correct one stale duplicate mdBook status paragraph. Shared Lua planning `.10.9.6.0` follows only after the closeout commit/brief-clear/clean proof. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
