...
| `FUTURE-PARITY-BACKLOG` | `active` / `.19.3.2` next | `Overall roadmap - future parity backlog` | Rust `.19.3.1` implements the frozen nested-write contract across typed parsed/serialized/generated/emitted carriers and passes exact canonical signoff without portable/public admission. Rust `map_leaves!` `.19.3.2` is the next clean-boundary leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
