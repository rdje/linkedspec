...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.2.2` after helper regex flags moved shipped smoke to 18/31. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.2.3` done 2026-07-10 - strict shared helper regex compilation preserves `imsx`, ignores `go`, rejects unknown flags, and closes portmap constant. Full tests pass with 772 assertions and status `runtime-corpus-helper-regex-flags`; shipped smoke is 18/31 and `.6.2.4.2.2` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.2.3` have since landed; `.6.2.4.2.2` is active.
