...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 3/5 at 68 governed files; Rust `.9.1.4` closes at `288da21a`. Dart `.9.1.5.0` records the exact preflight and dependency-safe `.1-.6` split before behavior code. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
