...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua staged `.14.7.7.0` focused-signoff-complete; `.1` next | `Overall roadmap - future parity backlog` | One shared Lua-5.1 source now passes 153 assertions and reaches one exact marker/provenance RED independently on PUC Lua and LuaJIT. Neutral governance is 98 mutations; ordinary/canonical discovery and both Lua rollout rows remain pending while `.14.7.7.1` owns annotation/provenance. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
