...
| `REPO-ROOT-PATH-PORTABILITY` | `active` | `Repository architecture / checkout relocation invariance` | Rust runtime repair `.1.1` and all eight legacy config/source repairs `.1.2` are complete; current leaf `.1.3` owns only the 12 frozen Julia Knowledge Map reverify commands. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
