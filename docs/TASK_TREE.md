...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Rust alias parity `.14.2.2.0.2` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location rollout remains 4/10 with 38 mutations. After clean audit `.0` and neutral correction `.0.1`, `.0.2` routes all five missing Rust capture aliases through canonical arms and proves Unicode, mutation/absence, native, reconstructed, generated-plan, and independently emitted carriers. Complete Rust/corpus/storage/CLI 66x2, real mdBook, all doctrines, canonical RAM 50%, and Phase 0 1,031/1,031 in 653 seconds pass. Commit/clean remain before `.0.3` freezes typed RED and `.1-.3` implement/admit Rust typed values; rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
