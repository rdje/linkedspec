...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2` after controlled corpus execution landed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.1` done 2026-07-10 - public controlled corpus execution composes validation, parse/compile/runtime, wrapped structural comparison, trace/diagnostic capture, and all-failure reporting; 24 focused assertions bring the full suite to 715 with status `runtime-controlled-corpus`. Active frontier advances to `.6.2` recoverable manifest batches. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
bring the full suite to 715 with status `runtime-controlled-corpus`; `.6.2` recoverable manifest batches are active.
