...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.2` — `.8.1` split the HandlerIR/CompiledSpec-to-Rust-source emitter lane after `.7.4` finalized the **88-fixture** manifest-backed oracle corpus guard; next add the minimal generated-source scaffold and compile/run harness | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.2` unless a new terse leaf is split.
