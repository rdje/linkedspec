...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed. Perl calls/staging parent `.10.3.3` is complete: corrected generated family/spec identity plus the private exact 22-record/25-relation projection pass semantic 6/20/57, five-backend primary 5x2x66, and Phase 0 1,031/1,031. Immutable Perl capabilities/query leaf `.10.3.4` is next after the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
