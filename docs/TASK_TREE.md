...
| `FUTURE-PARITY-BACKLOG` | `active` / `.19.3.3` canonical-complete, `.19.3.4.0` next | `Overall roadmap - future parity backlog` | Rust `.19.3.1-.2` implement frozen nested-write and `map_leaves!` contracts across typed generated/emitted carriers. Oracle/root-target semantics `.19.3.3` is canonical-signoff-complete across all backends; controlled macOS first-launch/cache-interference classification `.19.3.4.0` is next before Rust parent closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
