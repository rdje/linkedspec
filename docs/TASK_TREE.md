...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact 12-role Dart semantic admission `.10.5.6` composes strict source, compiled/failed/runtime snapshots, loaded/reconstructed/generated/emitted/traced routes, native/neutral JSON, all 20 digests, bounded queries, immutability, and host-leak denial. Only Dart advances: governance is 6/20/81 at rollout 4/9 and admission 3/6; parent `.10.5` is closed and Julia `.10.6` is next after the clean handoff. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
