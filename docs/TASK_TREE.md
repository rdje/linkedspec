...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0 and neutral cursor `.9.1.2` is committed at 1/7. Perl preflight `.9.1.3.0` is committed; typed family/bare-edge normalization `.9.1.3.1` is implemented and verified, with live cursor execution `.9.1.3.2` next after the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
