...
| `DART-BACKEND-PARITY` | `done` / `closed` (scoped milestone) | `Overall roadmap - future backend parity (Dart first)` | `.7.5` closed the 99/99 interpreter-first Dart tree; global `.1.5.4.3` now also closes exact four-backend CLI identity. `.1.6`/`.3` own remaining capability/codegen gaps. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, census `.1.6.0`, current-call audit `.1.6.1.0`, newline splitting `.1.6.1.1`, `endswitch` termination `.1.6.1.2.1`, and all pure/position/control plus Rust/Dart capture repairs are closed. Active `.1.6.1.2.2.4.3` completes Julia capture/mark semantics. Final admission must reconcile two governed Perl capture contracts omitted by the provisional 237-name shared inventory. Later leaves own descriptor/diagnostics/resolution/trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | Local proof is 1,023 assertions/nine processes/99 fixtures; Julia passes the exact pure/position/control sources, 61/61 default/POSIX, and the recurring four-backend matrix. Capability census `.1.6` and generated-source `.3` remain. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
