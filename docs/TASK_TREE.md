...
| `STAGED-LINKED-PARSING` | `active` | `Overall roadmap — .spec language model / parser composition` | `STAGED-LINKED-PARSING.5.3.2` — `.1` adopted staged linked parsing doctrine, `.2` specified future import/composition, `.3` specified future parse-job annotations, `.4` specified deterministic registry/dispatch queue semantics, `.5.1` selected user-function body text as the first prototype payload family, `.5.2` audited the function-definition AST shape/harness before code, and `.5.3.1` added `specs/user_function_definition.spec` as the executable user-function definition AST parser consumed by the Perl registry, with linked body-island rules for nested braces/strings/comments/regex literals. Next retire remaining host-language definition parser bridges, starting with Rust. | [docs/tasks/STAGED-LINKED-PARSING.md](docs/tasks/STAGED-LINKED-PARSING.md) |
