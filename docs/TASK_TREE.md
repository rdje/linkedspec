...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust `.14.7.4.2` current-depth authority focused-signoff-complete | `Overall roadmap - future parity backlog` | Rust now has private caller-frozen pure resolution/cache, isolated deterministic current-depth execution, detached atomic stitching, and all result/failure policies behind the unchanged dormant consumer; ordinary/canonical discovery and rollout remain unchanged, and `.14.7.4.3` owns recurrence, bounds, and source-rebased diagnostics. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
