...
| `LEGACY-VHDL-RETIRE` | `active` | `Overall roadmap — keep only portable/cross-variant code (retirement)` | `.1` read-only inventory **done** 2026-06-18 (3 modules + 6 dependent `.plg` + phase0 smoke blocks ≈ 6,701 lines; zero core dependency; catastrophic regex at `RTLUtils.pm:746`). Removal leaves `.2`–`.5` **blocked** pending user removal-scope confirmation. Clears `RTLUTILS-REGEX-HANG` → unblocks `SPEC-FORMAT-TERSE`. | [docs/tasks/LEGACY-VHDL-RETIRE.md](docs/tasks/LEGACY-VHDL-RETIRE.md) |
