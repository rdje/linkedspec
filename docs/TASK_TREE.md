...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua helper breadth is split into nine ordered implementation/closeout leaves; four-kind stores/access `.4.3.1` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Compiled dispatch passes 66/66 on both runtimes; core values, scalar/string, numeric, array, harray, blocks/controls, stateful capture/cursor, diagnostic output, and no-drift have separate owners. `.4.3.1` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
