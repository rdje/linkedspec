...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.3.5` — `.8.3.4` landed direct generated execution for AND/OR bcode; next close the non-repetition generated-family matrix before REP work | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.3.5` unless a new terse leaf is split.
...
dispatch and OR bcode first-match dispatch; blind-edge tail execution is shared with the interpreted path. Current
frontier advances to `.8.3.5` for the non-repetition generated-family matrix closeout before REP work.
