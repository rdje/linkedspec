...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua dynamic invocation `.11.8.2` signoff-complete; clean landing precedes `.11.8.3` | `Overall roadmap - future parity backlog` | Post-static `cb(args)`, colon-keyword rejection, evaluated-value access, once-only copied fixed/rest frames, live caller state, local results, exact failures, and ordered recursion pass 232 assertions on each Lua ABI. Complete Lua, sole-facing mdBook, doctrines, canonical Phase 0 1,031/1,031, and the exact callable matrix pass. Emitted-route identity remains `.11.8.3`; five-backend/public/capability admission remains `.11.8.4`; no root-README/push movement. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
- Callable-codeblock parent `.11.7` is closed at four admitted backends with 20 governance mutations and 23 public
  documents. Lua construction and general bound calls are current under `.11.8.1-.2`; route proof/admission remain
  `.11.8.3-.4`, and lexical capture remains deferred.
