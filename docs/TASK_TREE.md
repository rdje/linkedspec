...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.4.4` — `.7.3.4.1` fixed parser/compiler handling for header-rest action edges and optional `->`/`=>` spacing; next fix runtime capture propagation for `lib_reader` dependency-resolved child regex matches | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
