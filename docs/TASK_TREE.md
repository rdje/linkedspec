...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Perl typed-source admission `.14.2.1.3` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; typed source-location neutral/public `.14.1`, runtime plan/correction `.14.2.0-.0.1`, and Perl `.14.2.1.0-.3` are signoff-complete. Admission `.3` canonically registers both Perl consumers, promotes only `perl_runtime`, advances truth to 4/10 with 38 mutations, and aligns the sole-facing book; focused 10-test and rendered-book proofs pass with canonical CLI 66x2 / RAM 53% / Phase 0 1,031/1,031 in 650 seconds. Rust authority/RED `.14.2.2.0` is next only after a clean `.3` commit; rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
