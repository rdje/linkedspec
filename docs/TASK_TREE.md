...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Rust typed RED `.14.2.2.0.3` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location rollout remains 4/10 with 38 mutations. After clean alias parity `.0.2`, `.0.3` freezes one layered dormant Rust consumer: ordinary discovery is zero-test green, while the core cfg fails only with `E0432` for absent `source_location`; the later projection cfg owns exact 92+7 and unchanged carrier shapes. Complete Rust/corpus/storage/CLI 66x2 and canonical CLI 66x2 / RAM 49% / Phase 0 1,031 in 653s pass without production, neutral, schema, rollout, root-README, or mdBook behavior change. Commit/clean remain before immutable core `.1`; rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
