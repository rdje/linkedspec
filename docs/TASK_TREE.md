...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.2.1` after the `.1` foundation closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.1.3` done 2026-07-10 - Julia now has JSON3-backed manifest IO and drift/file guards over the checked-in 99-fixture corpus while `--execute` remains rejected. The `.1` foundation container is closed; active frontier advances to `.2.1` for source AST/data types. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.1.3` and now sits at `JULIA-BACKEND-PARITY.2.1`. Generated Dart source remains a future
split proof lane.
