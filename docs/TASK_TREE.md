...
| `SPEC-LANG-REFERENCE` | `active` | `Overall roadmap — documentation and book sync` | **ACTIVE 2026-07-08** — `.10.5.6` fixed `user-model/rule-modes-and-parse-modes.md`: regex-owning mode examples now use single-colon labels, `::` remains no-regex entry/dispatcher framing, and seek/consume snippets use a verified 2-rule wrapper. Next leaf: `.10.5.7`, fixing `user-model/regex-in-spec.md` regex-on-`::` fragments while preserving capture teaching. | [docs/tasks/SPEC-LANG-REFERENCE.md](docs/tasks/SPEC-LANG-REFERENCE.md) |
