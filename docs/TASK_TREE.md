...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart generated binding/decoded server `.10.9.4.1` is complete and commit-pending from clean planning commit `b503dcc9`: focused Dart 10/10, package 347/347, canonical CI, and unchanged 2/5 + 2/6 pending admission all pass. Strict stdio `.10.9.4.2` is next only after the clean commit; admission `.3` and closeout `.4` remain excluded. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
