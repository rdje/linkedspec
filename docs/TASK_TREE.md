...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.4` — `.7.3.6` landed seven RTL/plugin/legacy shipped-spec safety smokes (`regdef`, `tablegrep`, `simenv`, `vhdl`, `ds_vhistory`, `pplugin`, `tkgui`) and routed richer mismatches to follow-up; next finalize the oracle corpus guard | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.7.4` unless a new terse leaf is split.
