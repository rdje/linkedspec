...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Recurring semantic composition `.10.8` is complete at 6 groups / 20 responses / 105 mutations, rollout 7/9, and admission 6/6. One routed driver composes all six admitted consumers, three 5x2 primary cases, three support ledgers, and canonical opt-in without a new semantic model or CLI surface. Thin MCP `.10.9` is next after the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
