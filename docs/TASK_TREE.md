...
| `FUTURE-PARITY-BACKLOG` | `active` / MCP `.10.9.7.1.0` blocker verified; director decision pending | `Overall roadmap - future parity backlog` | Real Perl all-twenty spend proves capabilities plus 16/19 query identities, then exposes shared MCP schema/default-policy blockers for `calls_symbols_and_shapes`, `source_ceiling_forbidden`, and `unsupported_contract`. Exploratory code is removed; formal state remains 5/5 implementations + 6/6 runtimes, rollout pending/114. Recommended `.1.1` repairs the neutral contract and identical five-server policy seam before exact all-twenty proof; weakening the claim is not recommended. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
