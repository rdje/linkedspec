...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.2.1` lands the Rust command boundary at 29/61 exact cases; `.1.5.2.2` is active for reusable entry/mode/direct-result execution. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
surfaces agree on the strict 61-case Perl reference, Rust `.1.5.2.1` lands its exact boundary at 29/61, and
`.1.5.2.2` is the active execution frontier against the unchanged
...
and a thin MCP transport that owns no semantic behavior. The active backend frontier is Rust `.1.5.2.2`.
