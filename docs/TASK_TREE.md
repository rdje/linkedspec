...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 2/6; Rust preflight `.9.1.4.0` fixes the exact `.1-.7` split and makes local-gate hardening `.9.1.4.1` the next clean frontier before behavior. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
