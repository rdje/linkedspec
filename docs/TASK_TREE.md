...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua harray construction plus deterministic count/key/value/membership views pass 101/101 on both ABIs; copied transforms `.4.3.5.3` are active and callbacks remain `.4.3.6`. Flat/concat/hash arity and harray list order, negative counts, dropped-transform rebinding, invalid-join, and implicit child-push result drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array family is closed at 99/99. Harray construction plus deterministic count/key/value/membership views pass 101/101; copied transforms/receiver chains `.4.3.5.3` are active. Tree callbacks and later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
`undef` claim. Both Lua ABIs pass 101/101; copied transforms/receiver chains `.4.3.5.3` are active.
