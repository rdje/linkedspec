...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.3` done 2026-07-09 - Dart now resolves tclite/default-mode action-edge regex dispatch and recursive rule-local aggregate reset semantics; the parser-smoke window is 7/31 green. Next frontier `.6.2.4.4` is residual shipped-spec parser-smoke output/helper parity, with deeper PCRE constructs routed to `.6.2.4.6`. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.8.0` done 2026-07-09 - director's single-source `.spec` parser/stimuli roundtrip idea is captured as low-priority future design leaf `.8.1`; Julia `.1.2` remains gated until the Dart scoped milestone is reached. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
