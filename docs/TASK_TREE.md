...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua's complete 34-name non-callback array family and six numeric terminals close at 99/99 under `.4.3.4`; harray helpers `.4.3.5` are active and tree callbacks remain `.4.3.6`. Flat/concat arity, negative counts, dropped-transform rebinding, invalid-join, and implicit child-push result drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | All 34 ordinary array names and six numeric terminals pass 99/99 on both ABIs; public result prose and its recurring guard are aligned. Harray helpers `.4.3.5` are active; tree callbacks and later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
