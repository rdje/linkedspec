...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Rust root-selection preflight `.9.1.1.2.2.0` is complete at an exact 64/65 twice baseline with no behavior change; after its clean commit, core validation/resolution `.2.1` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
