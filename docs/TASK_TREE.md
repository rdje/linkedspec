...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Planning `.0` is `74138903`; initializer `.1.1` is `e33ed191`; routing `.1.2` is `671592ef`. Lifecycle `.1.3` adds checkout-isolated collision-safe runs, exact cleanup/retention, and guarded recovery. `.2.1` is the clean Perl migration/deletion frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
