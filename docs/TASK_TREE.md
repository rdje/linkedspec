...
| `FUTURE-PARITY-BACKLOG` | `active` / Perl recursive observation `.14.4.2` signoff-complete as intended atomic 210 from clean `6e3b77c0` | `Overall roadmap - future parity backlog` | Perl privately admits `observe_recognition` through one dedicated node/static policy, the existing invocation authority, detached carrier, and live/generated execution. Exact six-runtime typed-source composition is GREEN at 8/6/71 with only Perl on the pending observation row; recognition remains complete at 129+4/246/58. Book 79/14,508 KiB, Knowledge 822/6,843, all eight doctrines, containment/relocation, CLI 66x2, RAM 50%, and Phase 0 1,031/1,031 in 723 seconds pass; Perl must land clean before Rust `.14.4.3` activates. Typed-source `.14.8`, task metadata `.5`, and rendered readability remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
