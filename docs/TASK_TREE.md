...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua runtime diagnostics/trace no-drift closes parent `.4.4` at 129/129 on both ABIs; staged-function/native-loading `.5.1` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Exact native runtime diagnostics, controls/sinks, and interpreter events close parent `.4.4` at 129/129 on PUC Lua and LuaJIT; staged-function/native-loading `.5.1` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
