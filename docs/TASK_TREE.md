...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper `.5.2` is closed at 8/0 and neutral cursor `.9.1.2` is committed at 1/7. Perl `.9.1.3` is split before code; gate-safe mechanism/CLI ownership audit `.9.1.3.0` is verified and normalization `.9.1.3.1` follows after clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
