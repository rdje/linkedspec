...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.3.63` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; codebase No; physical mdBook Yes, formal .4 pending. Emitter/loader/staged-prefix consumers reconciled; classifier proof scope corrected under .77; routing signal .79 and manifest .78 remain owned; SimEnv repair .76 owned. Next .3.3.64; containment .7 owns later Dart capacity; rgx reading exclusion remains. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `active` / `.6` complete, `.7` future capacity | `Repository architecture / documentation sustainability` | `.6` preserves 264 records in 260 nodes plus six retained captions, corrects a proven historical reference and saves 251 lines/8,210 bytes overall. Focused documentation proof; unchanged limits. `.7` follows Rust closeout before Dart ownership. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
