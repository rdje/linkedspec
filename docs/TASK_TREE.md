...
| `STAGED-LINKED-PARSING` | `active` | `Overall roadmap — .spec language model / parser composition` | `STAGED-LINKED-PARSING.3` — `.1` adopted staged linked parsing doctrine and `.2` specified future spec-file import/composition: `import "path.spec" as alias` for qualified reuse and `include "path.spec"` for structured namespace merge, both separate from staged runtime payload parsing. Next design parse-job annotations/AST payload metadata. | [docs/tasks/STAGED-LINKED-PARSING.md](docs/tasks/STAGED-LINKED-PARSING.md) |
