...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Diagnostic-output audit/contract `.5.1.0-.1`, all five native sink/event repairs `.5.1.2-.6`, generated/primary projection `.5.1.7`, and symmetric recurring gate `.5.1.8` are done. Public no-drift `.5.1.9` is active; logical truthiness `.5.2` plus ADR `0036`/`.19` remain later-owned. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
