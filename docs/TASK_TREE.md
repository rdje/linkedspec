...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, opaque source/outcome `.10.7.2`, private static projection `.10.7.3`, and calls/staging/generated `.10.7.4` are composition-closed. Six committed calls suites recompose at exact focused 920 per ABI and private 22/25/10. Behavior-free immutable-query audit `.10.7.5.0` is next. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
