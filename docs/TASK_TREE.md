...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Punctuation-light parent `.16` is complete at capability 64/0/0 and condition headers remain parenthesized. Named-mark `.17.1-.17.4` align all five backends; Lua passes the exact native/serialized contract at 119/119 on both ABIs, and final shared admission/hardening `.17.5` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks/controls/callbacks close at 114/114; input/cursor `.4.3.7.1` passes 115/115; anonymous capture `.4.3.7.2` passes 116/116; earliest-boundary `.4.3.7.5` passes 117/117; and complete named-mark foundation `.17.4` passes 119/119 on both Lua ABIs. `.4.3.7.3` consumes that store after `.17.5` admission. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
