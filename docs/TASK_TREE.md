...
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks, controls, metadata-governed `with`, and deterministic scoped harray/array traversal close at 114/114 through `.4.3.6.6`; Unicode input/live-cursor views and explicit controls `.4.3.7.1` pass 115/115 on both Lua ABIs and advance anonymous capture-boundary `.4.3.7.2`, while seven documented mark helpers remain under cross-backend owner `.17` before `.4.3.7.3`. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
