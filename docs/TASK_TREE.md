...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.3` after diagnostic output advanced both routed fixtures. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.2.2` done 2026-07-10 - trace-routed `print`/`print_each`/`say` preserve parse output and advance simenv/history to successor-owned mechanisms. Full tests pass with 780 assertions and status `runtime-corpus-diagnostic-output`; shipped smoke remains 18/31 and `.6.2.4.3` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.2.3` have since landed; `.6.2.4.3` is active.
...
`runtime-corpus-diagnostic-output`, `.6.2.4.2` closes, and `.6.2.4.3` is active.
