...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua harray construction plus deterministic count/key/value/membership views pass 101/101 on both ABIs. Bare base/overlay transforms were revalidated after uniform binding in `.4.3.5.3.0`; Lua implementation `.4.3.5.3.1` is active and callbacks remain `.4.3.6`. Helper caveats remain durably routed to `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array family is closed at 99/99. Harray construction plus deterministic count/key/value/membership views pass 101/101. Transform contract revalidation `.4.3.5.3.0` is done and Lua implementation `.4.3.5.3.1` is active. Tree callbacks and later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
