...
| `FUTURE-PARITY-BACKLOG` | `active` / shared Lua `.14.6.6.0` focused-signoff-complete; `.1` next | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart + Julia remain admitted at 5/9/106. Shared Lua is split into dormant RED, private authority, dormant carriers, dual-ABI admission, and recomposition. One Lua-5.1-compatible consumer is exact at 85-pass/one-RED on PUC Lua and LuaJIT without discovery, production, format, rollout, typed, or outward movement; `.1` owns private authority next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
