...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart-native label primitives `.10.5.0.2.0`, parser/validator consumption `.1`, and exact downstream identity `.2` are complete; negative rejection and unrelated-identifier isolation `.3` is next after the clean `.2` commit. Semantic governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
