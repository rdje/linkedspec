...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.3.19` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; `.31` preserves forward reading and repair ownership. Reads 1,493 engine lines / 55,743 bytes covering traversal, callbacks and value blocks; six paired cases plus direct native diagnostics own final-assignment guard repair .58. Next `.3.3.20`: engine continuation, lines 7879–9278. Codebase No; physical mdBook Yes, formal .4 alignment pending; `rgx` excluded. `.7` blocks recovery/purge; `.29` belongs to policy adoption `.5`. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
