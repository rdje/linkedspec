...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.4` done 2026-07-09 - Dart runtime first landed cursor rewinds and char-based cursor/input helpers; `BACKTRACK-SURFACE-RUST-ALIGNMENT` supersedes the public names with explicit cursor controls and adds the portable non-consuming boundary helper. Next frontier `.4.5` implements runtime diagnostics and trace controls. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
