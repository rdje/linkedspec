...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua exact tagged-record construction `.4.3.4.5` is closed at 99/99; complete array/public no-drift `.4.3.4.6` is active. Flat/concat arity, negative counts, dropped-transform rebinding, and invalid-join drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Governed split reuse, one-time carried fields, copied `[tag, item, fields...]` records, and receiver composition pass 99/99 on both ABIs; complete array/public no-drift `.4.3.4.6` is active. Later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
