...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Uniform-binding selector retirement `.12.1` is closed and publicly admitted. Existing active Lua scalar-numeric `.4.3.3.1.4` resumes; inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Uniform-binding execution and selector rejection pass 88/88 on PUC Lua and LuaJIT; scalar numeric `.4.3.3.1.4` resumes now that selector retirement is complete. Future variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
