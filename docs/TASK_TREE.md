...
| `FUTURE-PARITY-BACKLOG` | `active` / Perl `.14.7.3.2` focused-signoff-complete; `.14.7.3.3` next task-tree-first after the intended atomic commit | `Overall roadmap - future parity backlog` | `.14.7.3.2` is complete at 133 GREEN top-level checks/one recursive-authority RED with caller-frozen resolution/cache, deterministic v2 identity, current-depth isolated execution, all result/failure policies, and no route/rollout/public movement; `.14.7.3.3` owns recursive queues, bounds, and source-rebased diagnostics next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
