...
| `FUTURE-PARITY-BACKLOG` | `active` / public mutation closeout `.19.9` next | `Overall roadmap - future parity backlog` | All five backends implement frozen nested writes and `map_leaves!`; `.19.7` admits their two exact capability rows at current census 100/0/0, and `.19.8` completes exact six-runtime recurrence through one governed driver plus 17 topology mutations. Public-current no-drift, parent closeout, and push remain `.19.9`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
| `RUST-DEPENDENCY-WARNING-ZERO` | `proposed` / non-blocking; intake `.0` focused-signoff-complete | `Repository quality / Rust dependency and generated-source hygiene` | Reproduced canonical builds report 1,870 `pgen` and 26 `rgx-core` warnings. `.0` durably owns the director-requested cleanup; `.1-.7` census unique causes, repair authored/generator/dependency/direct-crate sources, integrate explicit upstream pins, and enforce zero warnings without broad suppression. Public mutation closeout `.19.9` remains the current PNT frontier while this cleanup stays queued. | [docs/tasks/RUST-DEPENDENCY-WARNING-ZERO.md](docs/tasks/RUST-DEPENDENCY-WARNING-ZERO.md) |
