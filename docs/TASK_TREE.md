...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl MCP strict wire/lifecycle `.10.9.2.2` is complete with exact UTF-8/JSON-line preflight, canonical LF emission, prepared cancellation, sanitized optional logging, EOF/I/O cleanup, adversarial proof, and canonical signoff. Exact admission `.3` is next after the clean commit; no-change closeout `.4` remains ordered, and ADR `0056` stays pending under `.14.1-.8`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
