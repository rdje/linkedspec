...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Shared executable-label parent `.10.5.0.1` is complete: one pinned generated class feeds all 12 canonical sites, four corpus inputs are freshness-locked, physical token boundaries reject truncation, and current grammar passes exact 5x2 proof. Next PNT frontier is pending Dart-native label rollout `.10.5.0.2`; semantic governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
