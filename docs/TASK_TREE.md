...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Dart admission `.14.2.3.3` signoff-complete, atomic commit pending | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Dart projection landed clean at `3513508a`; admission `.14.2.3.3` moves its unchanged four-test consumer into ordinary discovery, registers the exact repository-wrapped target once in canonical CI, promotes only `dart_runtime`, and locks 6 complete / 8 pending / 40 mutations with independent path/dormancy/complete-to-pending checks. Focused 4/4 and complete Dart format 98/0, fatal analysis, ordinary 383, storage 20/47, CLI 66x2, and corpus 105/105 pass. The sole-facing book now distinguishes admitted internal Perl/Rust/Dart values/projections from future public typed values and Julia/Lua admissions; its 79-file/14,164-KiB render preserves separate paragraphs and command blocks. Knowledge Map is 786/6,432; all seven doctrines pass. Definitive canonical CI passes capability 80/0/0, typed Perl 10 plus Rust/Dart 4/4, composed semantic/MCP, containment/relocation, CLI 66x2, RAM 73%, and Phase 0 1,031/1,031 in 685 seconds before the exact pass marker. Atomic commit 156/300, brief clearing, and clean proof remain before task-tree-first Julia `.14.2.4.0`; rendered readability stays queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
