...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia staged `.14.7.6` closed; Lua `.14.7.7` next | `Overall roadmap - future parity backlog` | Julia's 491/491 consumer now executes four fresh-authority production routes and is admitted exactly once in ordinary/canonical topology. Julia rollout is complete at 5/9 with 97 mutations; formats and public behavior remain fixed while shared Lua `.14.7.7` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
