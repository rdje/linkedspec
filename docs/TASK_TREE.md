...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5` and census `.1.6.0` are closed. Active `.1.6.1.0` audits 237 current ActionIR call names and splits the exposed Perl newline-separator coverage gap to `.1.6.1.1`; `.1.6.1.2` then completes exhaustive neutral proof. Later leaves own Rust descriptor/diagnostics, native named/file resolution, and Dart full-pipeline trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
