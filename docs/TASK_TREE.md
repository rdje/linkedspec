...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Selector retirement `.12.1` is re-closed through backend-README follow-up `.12.1.10`; all 56 public files are recurring-gated at zero current examples. Lua numeric aliases/symbols/number receivers `.4.3.3.2` resume. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Strict scalar numeric v1 passes all 55 cases on PUC Lua and LuaJIT, with exact six-runtime admission and an 89/89 dual-ABI gate. Numeric aliases/symbols/number receivers `.4.3.3.2` are active; future variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
