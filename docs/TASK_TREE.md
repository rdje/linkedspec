...
| `FUTURE-PARITY-BACKLOG` | `active` / governance repair `.22` canonical-signoff-complete | `Overall roadmap - future parity backlog` | `.22` governs 8 closed-state families / 12 exact markers / 15 consumers outside the mutable frontier, proves four rewrite/deletion mutations, and removes bounded-memory handoff coupling. Select the next pending leaf only after its clean pushed closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
