...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | All 13 ordinary Lua harray names close at 103/103 under `.4.3.5`; codeblock/control/tree-callback parent `.4.3.6` is active. Rename collision drift joins the helper caveats durably routed to `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array family is closed at 99/99 and all 13 ordinary harray names close at 103/103. Codeblock values, controls, generic trailing blocks, and tree callbacks are active under `.4.3.6`; later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
