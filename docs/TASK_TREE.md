...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Punctuation-light parent `.16` is complete: ADR 0033 plus `.16.0-.16.6` ratify and implement the narrow aliases, and `.16.7` admits them at capability 64/0/0 with a recurring five-backend/two-Lua-ABI proof. Parenthesis-free `if`/`while` headers remain excluded. Rust/Dart/Julia/Lua `.contains()` arity drift is owned by `.5`; future generated Lua preservation remains `.8.1-.8.4`. Lua built-in final blocks/scoped `with` close at 112/112 through `.4.3.6.4`; deterministic harray callbacks close at 113/113 and shared root-kind array callbacks close at 114/114 through `.4.3.6.5`; no-drift `.4.3.6.6` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks, controls, and metadata-governed helper/receiver `with` pass 112/112 through `.4.3.6.4`; reference authored-value repair passes full phase0, and deterministic scoped Lua harray/array walk/map/reduce close at 114/114 through shared root-kind dispatcher `.4.3.6.5`; no-drift and dependency routing `.4.3.6.6` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
and root-kind array callbacks `.5.2` close at 114/114 on both ABIs. No-drift `.4.3.6.6` is active. The punctuation-light contract, all five backend
