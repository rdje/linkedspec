...
| `FUTURE-PARITY-BACKLOG` | `active` / `.19.2.1` focused-signoff-complete; `.19.2.2` next | `Overall roadmap - future parity backlog` | Neutral write/bang/composition `.19.1.1-.3` are frozen at 105 write / 167 map / 593 composition mutations. `.19.2.1` implements the unchanged nested-write v1 contract on Perl only: typed evaluated paths, absent/null presence, dense isolated creation, diagnostics, detachment, and rule/function state. Rust/Dart/Julia/Lua and public capability remain unchanged; Perl `map_leaves!` follows. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
