...
| `PROJECT-DATA-SSD-ROOTING` | `done` / `closed` | `Repository architecture / project-data storage locality` | `.5` closes migration at zero old-volume residue with exact retained inventories, every backend gate, primary 660/660, Unicode 10/10, all maintained focused parity matrices, scalar numeric 55/55, process containment, and canonical/public proof. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
| `REPO-ROOT-PATH-PORTABILITY` | `active` / next after clean storage commit | `Repository architecture / checkout relocation invariance` | Remediation `.1.1-.1.3` and structural doctrine `.2.1` are complete. Recurring relocated-process oracle `.2.2` activates after the clean `PROJECT-DATA-SSD-ROOTING.5` commit and consumes the final repo-filesystem storage environment. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
