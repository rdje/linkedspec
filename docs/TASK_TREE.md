...
| `TRACE-OBSERVABILITY` | `active` | `Overall roadmap — engine observability / developer experience` | User directive 2026-06-19: discoverable CLI trace control + comprehensive "see everything" trace (function enter/exit, if/switch/case branches). Framework EXISTS (`Trace.pm`: enter/exit/decision/levels/sinks; env/per-call control works). `.1` audit showed trace is not exhaustive; `.2` added discoverable `bin/linkedspec` CLI flags and docs. Generated handler branches, most ActionIR owner branches, and Rust trace parity remain gaps. `.1` DONE 2026-07-04; `.2` DONE 2026-07-04; frontier `.3` Perl reference coverage extension, then future backend parity split. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. `TRACE-OBSERVABILITY.2` has since closed; PNT returns to `TRACE-OBSERVABILITY.3` unless a new terse leaf is split or another active tree is reprioritized.
...
`RUST-PARITY` unblocked `TOP-RULE-AS-NORMAL.3.2`; that leaf has since closed, `TRACE-OBSERVABILITY.1` has completed its read-only audit, and `TRACE-OBSERVABILITY.2` has added CLI trace control, so PNT advances to `TRACE-OBSERVABILITY.3`.
