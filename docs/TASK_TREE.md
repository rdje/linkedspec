...
| `FUTURE-PARITY-BACKLOG` | `active` / Perl progressive RED `.14.6.2.0` focused-complete; authority `.1` next | `Overall roadmap - future parity backlog` | From clean neutral atomic 259 `a0a33c32`, the unrouted final-path Perl consumer passes 83 then fails only on absent `PROGRESSIVE_DISPATCH_SPAN`. `.14.6.2.1` next owns registry/source-view/cancellation/budget/chain/isolation/detachment authority; `.2` owns four carriers and `.3` admission. Rust/Dart/Julia/Lua `.14.6.3-.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
