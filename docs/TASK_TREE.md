...
| `FUTURE-PARITY-BACKLOG` | `active` / audit `.14.6.0` signoff-complete; correction `.14.6.0.1` next | `Overall roadmap - future parity backlog` | From clean atomic-256 `8880d1b6`, audit `.14.6.0` proves no active dispatch and only five narrow staged body adapters. It freezes pre-registered rebased-span authority and `.0.1` correction → `.1` neutral → `.2-.6` backends → `.7` recurrence → `.8` public order. `.0.1` owns the missed typed transaction-safety promotion before behavior; staged `.14.7` and combined `.14.8` remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
