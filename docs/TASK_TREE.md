...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Shared Lua closeout `.10.9.6.4` is signoff-complete from clean `4eb008d8`: unchanged neutral plus five-implementation/six-runtime recomposition preserves 5/5 + 6/6 pending/114, canonical CI passes Phase 0 1,031/1,031 in 652 seconds and the complete dual-ABI Lua gate, and parent `.10.9.6` is closed without production, test, fixture, contract, ledger, or user-visible movement. Commit/brief-clear/clean proof precedes recurring `.10.9.7`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
