...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia `.19.5.1` complete, `.19.5.2` next | `Overall roadmap - future parity backlog` | Perl, Rust, and Dart implement both frozen mutation mechanisms. Julia `.19.5.1` implements nested write-vivification through typed native, reconstructed, generated, emitted-module, and CLI routes; Julia `map_leaves!` `.19.5.2` is next. Lua and portable/public admission remain pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
