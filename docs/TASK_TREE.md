...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Public Dart `capabilities`, typed `query`, and raw-neutral `queryNeutral` now share one detached-projection evaluator and match all 19 static digests plus 26 validation boundaries. Composition-only closeout `.10.5.4.4` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
