...
| `FUTURE-PARITY-BACKLOG` | `active` / five-backend implementation `.19.2-.6` complete; `.19.7` next | `Overall roadmap - future parity backlog` | All five backends implement frozen nested writes and `map_leaves!`. Lua `.19.6.1-.2` passes 438 write and 530 bang assertions per ABI plus its complete gate; parent `.19.6` is closed. Portable/public admission `.19.7` remains pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
