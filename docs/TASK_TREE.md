...
| `FUTURE-PARITY-BACKLOG` | `active` / callable-codeblock Rust parent `.11.4` closed; clean closeout in progress | `Overall roadmap - future parity backlog` | `.11.4.3` closes one metadata registry, attached/parenthesized candidate normalization, typed staged/described final kinds, and shared helper/user/receiver/tree execution. Neutral 7/11/9/7/4/8, Perl 10/10, Rust 18/18, full Rust operational proof, Knowledge Map 761/6,175, mdBook, seven doctrines, and canonical Phase 0 1,031/1,031 in 639 seconds pass. Commit/brief-clear/clean proof precedes durable direction routing and Dart `.11.5.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
