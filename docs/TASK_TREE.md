...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia admission `.14.6.5.3` canonical-signoff-complete | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart + Julia are admitted at 5/9/106. Julia `.14.6.5.0-.3` preserve the historical RED, private authority, four fresh-authority routes, and admit the unchanged 62-assertion carrier ordinarily/canonically while the 210-assertion authority matrix stays dormant. Independent recomposition `.14.6.5.4` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
