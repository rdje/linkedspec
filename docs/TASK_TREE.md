...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.7.4` done 2026-07-09 - Dart-specific CLI productization is complete, with `bin/linkedspec_dart.dart corpus` routing through the manifest-backed Dart runtime harness. Next frontier `.7.5` final no-drift closeout. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
