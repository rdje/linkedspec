...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact twelve-role Julia MCP admission `.10.9.5.3` is signoff-complete from clean strict-stdio commit `ddb040f0` and awaits its commit/brief-clear/clean boundary. Julia alone advances to 4/5 implementations + 4/6 runtimes; no-change closeout `.10.9.5.4` follows only afterward. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
