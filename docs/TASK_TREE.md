...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia strict synchronous MCP stdio `.10.9.5.2` is signoff-complete from clean decoded-server commit `ae212a7e` and awaits its commit/brief-clear/clean boundary. Exact twelve-role Julia admission `.10.9.5.3` follows only afterward; the ledger remains 3/5 implementations + 3/6 runtimes pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
