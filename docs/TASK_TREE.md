...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua `.8.4` closes the backend rollout at 16 capabilities and 80/0/0. Diagnostic-output alignment `.5.1` is the next PNT-eligible leaf after a clean pivot; it remains pending, and logical truthiness `.5.2` plus ADR `0036`/`.19` remain later-owned. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
