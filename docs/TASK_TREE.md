...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.4.0` done 2026-07-09 - the 7/31 parser-smoke residuals are split into narrow non-PCRE leaves. Next frontier `.6.2.4.4.1` is portmap/action-edge child result shape parity; deeper PCRE constructs stay routed to `.6.2.4.6`. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
