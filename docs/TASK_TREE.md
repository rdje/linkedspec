...
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` triage + `.3`/`.4` engine fixes (ADR `0008`) + cluster `.2` (108 STALE re-blessed) all **DONE** → subtests **1-940 GREEN**. **`.5` SPLIT 2026-06-22:** `.5.1` **DONE** — subtest-941 `corpus_regression` ("No tests run"/exit-255) determined a **real stale ref, not a natural stop** (`NONCORE-QUARANTINE.3` rmdir'd `plugin/` but left the `.plg` dataset → `opendir … or die`); removed it (TEST-ONLY). This **EXPOSED `.5.2`** (frontier, **BLOCKED**): the conf/tablescript **Lispish** corpus parse **catastrophically backtracks** (374 CPU-min on a 392B file; ~21/22 conf files; ebnf healthy) — needs a user direction decision (quarantine vs fix vs guard). `.5.3` (gate flips) + `.6` (book `:AND`) blocked behind green phase0. See [[lispish-corpus-catastrophic-backtracking]]. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
