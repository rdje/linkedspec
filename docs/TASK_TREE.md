...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.2` — `.7.3.1` split the remaining/harder shipped-spec oracle batch into narrow lanes after `.7.2` recorded concrete divergences; next debug the referenced timeout/hang risk with LinkedSpec's toolbox and trace surfaces before broad corpus expansion | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
