...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.2.4` closes Rust at 61/61 default/POSIX. Dart `.1.5.3.0` splits its 0/61 corpus-command replacement; `.1.5.3.1` is the active exact boundary/loading leaf. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
