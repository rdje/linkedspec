...
| `FUTURE-PARITY-BACKLOG` | `active` / `.19.1.1` focused-signoff-complete; `.15.3` next | `Overall roadmap - future parity backlog` | Future-only nested-write contract `.19.1.1` freezes evaluated typed segments, dense atomic creation, exact diagnostics/spans, detached results, read exclusions, and 105 mutations without backend admission. Focused proof exposed one stale Lua `next()` fixture now owned by `.15.3`; after that clean repair, neutral bang contract `.19.1.2` resumes. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
