...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl semantic parent `.10.3` closes with `.10.3.6`: exact 12-role consumer 18, semantic governance 6/20/65, rollout 2/9, admission 1/6, primary 66x2, and Phase 0 1,031/1,031 in 633s. Rust semantic adapter `.10.4` is the next PNT-eligible leaf after the clean commit. The repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
