...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Recurring semantic composition `.10.8` is active task-tree-first from clean Lua closeout `e335ea8b`. Its behavior-free plan freezes one exact six-runtime driver, three primary no-drift cases, support ledgers, canonical opt-in, seven mutations, and recurring-only promotion from baseline 6/20/98 at 6/9 + 6/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
