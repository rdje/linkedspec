...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.9` — `.8.5` closed generated-source all-family plus manifest-backed corpus-subset validation; next documentation/finalization | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.9` unless a new terse leaf is split.
...
oracle/corpus subset integration; current frontier advances to `.9` for documentation/finalization.
...
is closed and the frontier advances to `.9` for documentation/finalization.
