...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.3.2` — `.8.3.1` landed generated rule-mode/family metadata plus plan validation; next emit direct default/OR acode execution | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.3.2` unless a new terse leaf is split.
...
advances to `.8.3.2` for direct default/OR acode generated execution.
