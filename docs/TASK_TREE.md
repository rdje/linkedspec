...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). Implementation leaves `.1.x`+ **gated/blocked**: need a usable `t/phase0_regression.t` (hung by `RTLUTILS-REGEX-HANG`). Migration policy resolved (gradual-alias). Gate is unblocked by `LEGACY-VHDL-RETIRE` (retire the hanging subsystem). | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
| `LEGACY-VHDL-RETIRE` | `active` | `Overall roadmap — keep only portable/cross-variant code (retirement)` | `.1`/`.2`/`.3` **done** 2026-06-18 — subsystem retired (3 modules + 6 `.plg` + phase0 smoke ≈ 6,701 lines; user-confirmed "full closure"). **RTLUtils hang CLEARED** (hang was at subtest 110 `add_header_n_context_clause`/`RTLUtils.pm:104`, not line 746). `.4`/`.5` **blocked**: removing it unmasked a SECOND pre-existing hang (`HTML::PathLinks::link_path_tokens`, subtest 131) + back-half failures → phase0 still not green; `SPEC-FORMAT-TERSE` gate stays blocked pending the `NONCORE-QUARANTINE` cleanup. | [docs/tasks/LEGACY-VHDL-RETIRE.md](docs/tasks/LEGACY-VHDL-RETIRE.md) |
| `NONCORE-QUARANTINE` | `active` | `Overall roadmap — keep only portable/cross-variant code (non-core quarantine)` | **Relocation DONE 2026-06-19** (`.1`–`.4`): all 36 non-core `.pm` + 13 `.plg` `git mv`'d to `noncore/` (ledger `noncore/README.md`); 37-subtest legacy-migration block excised from phase0 (996→959); `perl/` is now **core-only**; `perl -c` core+test OK. **NEW BLOCKER (not from this work): ~173 PRE-EXISTING back-half core-engine test failures** revealed (phase0 ran the long-dark back half — structural mismatches, `method_like`×75/`named_mark`×25/`emit_context`×21/…; engine bytes unchanged → masked by the old hang). `.V`/green-phase0 + `SPEC-FORMAT-TERSE` + `LEGACY-VHDL-RETIRE.4/.5` blocked on triaging the 173 (stale-vs-real; needs its own tree — user decision). `.N` machinery POSTPONE. | [docs/tasks/NONCORE-QUARANTINE.md](docs/tasks/NONCORE-QUARANTINE.md) |
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` read-only triage **DONE** 2026-06-19: 173 failing subtests triaged → **108 STALE (re-bless) / 65 REAL (engine-fix), across 2 distinct engine defects.** STALE: A parser-collection-shape (8, retired auto-tag), B `method_like` (75, retired `return_*`/`RETURN_A`), C `emit_context` (20, removed `Deps::*` seams), F migration-summary (3, `return(1)` resolved), G (2, `return(1)` AST shape). REAL: **Defect #1 AND-rule action-codegen** (63 — multi-edge AND + `return` edge emits `SCALAR(0x…)Rule` ⇒ compile-fail top / `[]` child) + **Defect #2 input-boundary regression** (2 — `Runtime.pm` wrapper derefs before the guard). Next: `.2` re-bless stale; `.3`/`.4` engine fixes **blocked on user OK to touch the reference engine**; `.5` green-phase0 verify. Gates green phase0 + `SPEC-FORMAT-TERSE`. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
