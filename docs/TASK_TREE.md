...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.2.41` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; `.31` preserves forward reading and repair ownership. Completes recognition/observer runtime reading and preserves required bounded change history. Next `.3.2.42`: semantic call projection and index construction. Codebase/book No; `rgx` excluded. `.7` blocks recovery/purge; `.29` belongs to policy adoption `.5`. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
