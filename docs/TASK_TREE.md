...
| `FUTURE-PARITY-BACKLOG` | `active` / mdBook drift `.23` closed; `.19.1.1` next | `Overall roadmap - future parity backlog` | `.23.1-.2` restore meaningful migration teaching, reconcile current rollout/backend truth, expand narrow no-drift governance, and close `.23` at the canonical batch boundary without runtime movement. `.19.1.1` is the next task-tree-first implementation frontier. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
