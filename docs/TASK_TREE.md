...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.3` done 2026-07-09 - Dart now passes the non-`fn` middle helper/control/receiver corpus fixtures; the three top-level `fn` fixtures are routed to `.6.2.5` for spec-defined function-shell corpus execution. Next frontier `.6.2.4` is the shipped-spec/parser-smoke corpus batch. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
