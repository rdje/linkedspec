...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 3/5 at 68 governed files; Rust `.9.1.4` closes at `288da21a`. Dart descriptor `.9.1.5.3` is committed at `39338616`; generated-source v2 `.9.1.5.4` has full signoff and awaits its clean commit before `.9.1.5.5`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
