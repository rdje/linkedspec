...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl callable-codeblock `.11.3` is complete; active `.12.1` removes spec-facing `array(name)` / `hash(name)` selector and mutation semantics before Rust codeblock or resumed Lua work. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
