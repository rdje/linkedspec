...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart typed record/source query kernel `.10.5.4.1` now matches nine exact static response digests through immutable package-private values over fresh detached projection data. Relation traversal/pages/logical limits `.10.5.4.2` are next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
