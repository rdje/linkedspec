...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.0` done through Julia `.7.3.1`; Julia `.7.3.2.1` now closes frontend/compiler/staged trace. Active PNT is `.7.3.2.2` exact arguments and source/input loading; `.1.5`, `.1.6`, and `.3` own global convergence before Lua. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.7.3.2.1` done 2026-07-10 - one optional emitter covers parse/validate/compile/function-shell/staged phases with 868 assertions and 99/99 green. `.7.3.2.2` is active for exact arguments and source/input loading. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.7.3.2.2` is active for exact primary arguments plus source/input resolution and loading.
