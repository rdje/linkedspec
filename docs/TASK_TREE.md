...
| `FUTURE-PARITY-BACKLOG` | `active` / `.19.2.2` decision-blocked from clean `6b960624` | `Overall roadmap - future parity backlog` | `.19.2.1` implements nested writes on Perl. The `.19.2.2` audit confirms two frozen bang-composition helper cases assume caller capture that the admitted pure function contract forbids. Recommended repair uses existing explicit-target `set` and caller-scoped `.with` fixtures; implicit function capture would require a separate five-backend decision. No bang behavior changed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
