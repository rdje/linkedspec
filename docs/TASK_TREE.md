...
| `FUTURE-PARITY-BACKLOG` | `active` / migration-contrast repair `.23.1` focused-signoff-complete | `Overall roadmap - future parity backlog` | `.23.1` restores two exact rejected selector examples plus five ordered old-to-new mappings and guards them through 11 semantic mutations; public proof is 61 files / 32 historical references / zero current examples. Broader current-status reconciliation `.23.2` is the next clean frontier. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
