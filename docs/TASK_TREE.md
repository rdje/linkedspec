...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0 and neutral cursor `.9.1.2` is committed at 1/7. Perl preflight `.9.1.3.0` and normalization `.9.1.3.1` are committed; live rule-local cursor `.9.1.3.2` is fully verified and closing through its commit before descriptor `.9.1.3.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
