...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed; neutral semantic contract `.10.2` and Perl authority map `.10.3.0` are clean through `e679a3eb`. Strict source/map/outcome `.10.3.1` is signoff-complete: opaque `semantic_index` construction, strict UTF-8/canonical bytes, exact spans, typed validation, and immutable compiled-or-failed outcomes pass canonical proof without records/query/admission. Only its clean commit remains before static projection `.10.3.2`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
- rule-local cursor recurring/public no-drift at 75 files / 8 complete + 0 pending / 60 mutations is closed. At
  ...
  child; repeated-action public closeout has since completed it at 8/0/54.
...
state while malformed constructor input is typed. Exact canonical signoff passes; static projection `.10.3.2`
waits only for the clean foundation commit.
