...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua inert construction `.11.8.1` signoff-complete; clean landing precedes `.11.8.2` | `Overall roadmap - future parity backlog` | Exact eight-field literals/signatures/Unicode spans, nine malformed codes, deferred bodies, offset-preserving copies, user-function/compiled/generated/emitted-effective-state transport, and semantic signatures pass 168 assertions on each Lua ABI. Complete Lua, neutral+20, KM 777/6,301, mdBook 79/14,028 KiB, all seven doctrines, and canonical Phase 0 1,031/1,031 plus the callable matrix pass. Dynamic invocation remains `.11.8.2`; no capability-row/MCP/root-README/push movement. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
- Callable-codeblock parent `.11.7` is closed at four current backends with 20 governance mutations and 23 public
  documents. Lua inert construction is now current under `.11.8.1`; general bound calls/route proof/admission
  remain `.11.8.2-.4`, and lexical capture remains deferred.
