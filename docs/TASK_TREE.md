...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.4.5` done 2026-07-09 - `ds_vhistory_version_entry` is split with public-parser, descriptor-handler, scalar-held indexed-read, and leading-newline evidence; no Dart runtime behavior changed. Next frontier `.6.2.4.4.6` resolves the leading-newline public-parser/oracle boundary before final parser-smoke closeout. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.9.0` done 2026-07-09 - director's corrected AND/OR edge-default model is captured as future design leaf `.9.1`; `.8.1` still parks the single-source parser/stimuli idea, and Julia `.1.2` remains gated until the Dart scoped milestone is reached. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
