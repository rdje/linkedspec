...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia intrinsic ordinary runtime `.9.1.6.2` is fully verified: live/loaded/normalized/recursive/traced entered-rule policy is exact with bounded v1/outer compatibility. Its clean commit is required before descriptor v1 `.9.1.6.3`; root admission `.4.3` follows cursor closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
