...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current-surface `.1.6.1`, and canonical descriptor identity `.1.6.2.1` are closed. Perl/Dart/Julia now expose the composing and nested model identities consistently; active `.1.6.2.2` adds Rust's outward projection before final admission `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
