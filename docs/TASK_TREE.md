...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua decision `.5.3.0.1` resolves descriptor/full-trace admission policy through ADR `0041`; executable descriptor contract/emission `.5.3.1` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Decision `.5.3.0.1` adopts ADR `0041`: neutral final-codeblock outward descriptor v3, four-backend census preservation through `.5.3-.7`, and sole all-pass Lua admission at `.8.4`. Executable descriptor contract/emission `.5.3.1` is next, followed by one-emitter trace `.5.3.2` and no-drift `.5.3.3`. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
