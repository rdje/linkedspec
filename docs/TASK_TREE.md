...
is closed; the remaining `SPEC-FORMAT-TERSE` exhaustion frontier moves to `.13`.
...
hashes as leaves, and binds scoped `value`, `index`, `path`, `depth`, plus reduce-only `acc`. Frontier advances to
...
continuations after `walk_leaves` / `map_leaves`. Full phase0 passes `1..1028`; frontier advances to
...
after `walk_leaves` / `map_leaves`. The generated oracle corpus is now 97 fixtures after
`terse_13_3_array_tree_traversal_receiver_blocks`, and the Rust oracle passes. Frontier advances to
