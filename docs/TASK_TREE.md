...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua attached/marker if and switch controls close at 107/107 through `.4.3.6.3.2`; attached while `.4.3.6.3.3` is active, user-function block dispatch stays `.5.1`, and first-class callable values stay future `.11.7`. Alias-shape, truthiness, switch equality/ranges, and rename-collision drift remain helper caveat `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager/inline controls and attached/marker if and switch statements pass 107/107 through `.4.3.6.3.2`; attached while `.4.3.6.3.3` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
outside-branch marker execution drift are routed to backlog `.5`; attached while `.4.3.6.3.3` is active.
