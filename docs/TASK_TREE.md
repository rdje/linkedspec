...
| `README-STABILITY-POLICY` | `active` | `Repository architecture / documentation sustainability` | `.0` is committed at `adcc89fe`. `.1` owns the exact 105-line / 5,072-byte landing page, root policy, hard 128-line / 6,144-byte budgets, and registered `README-STABILITY` implementation; full canonical signoff passes and its per-leaf commit remains. `.2` closes unchanged and returns to MCP. Project-level licensing is separately proposed in `.3` for director decision. | [docs/tasks/README-STABILITY-POLICY.md](docs/tasks/README-STABILITY-POLICY.md) |
