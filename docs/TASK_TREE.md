...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `active` / `.5` pending | `Repository architecture / documentation sustainability` | `.0-.4` complete; `.5` compacts proven duplicate startup chronology before reading resumes. Task collection is approaching its unchanged 80,000-line bound. Activate only after clean startup `.3.3.42`. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
