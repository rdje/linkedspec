...
| `REPO-ROOT-PATH-PORTABILITY` | `active` | `Repository architecture / checkout relocation invariance` | Remediation `.1.1-.1.3` and structural doctrine `.2.1` are complete: tracked parent text plus all five primary runtime anchors are mechanically gated. Current leaf `.2.2` owns the recurring relocated-process oracle and final closeout. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
