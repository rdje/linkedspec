...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.5.2` after staged function-body dispatch landed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.5.1` done 2026-07-10 - Julia resolves the built-in ActionIR-body provider, executes jobs in stable order, records portable cache/compiled/result metadata, and immutably stitches `body_ast`; full tests pass with 662 assertions and status `runtime-staged-registry`. Active frontier advances to `.5.2` user-function runtime execution. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.5.2` registered user-function runtime execution is active.
