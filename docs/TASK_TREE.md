...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Lua emitted `.6.4` focused-signoff-complete from atomic 249 `b7708cde` | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, Dart, and Julia run ordinarily and in the rooted route at gap 5/4/58. Shared Lua `.6.1-.6.4` now carries authored/static/compiled metadata, one recognition-frame/token native path, normalized reconstruction, compatible descriptors, same-engine generated-v2 execution, and ten value/two typed-error modules independently loaded in fresh dual-ABI children behind exact dormancy. Storage is 19/three native modules; `.6.5` retains existing-primary/nine-role admission. Existing rule-slot events retain post-action/pre-`LE` timing. Both Lua runtimes, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
