...
| `SPEC-LANG-REFERENCE` | `active` | `Overall roadmap — documentation and book sync` | **ACTIVE 2026-07-08** — `.6` added a verified marker-form capture/mark cross-example, corrected placement-sensitive named-mark examples to use `mark_here(...)` where action-local timing is needed, and recorded KM fact `split-boundary-marker-action-timing`. Next leaf: `.7`, durable `.spec`-language KM fact-card coverage. | [docs/tasks/SPEC-LANG-REFERENCE.md](docs/tasks/SPEC-LANG-REFERENCE.md) |
