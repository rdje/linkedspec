...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua compiled-rule dispatch `.4.2` passes 66/66 on both runtimes; helper-family split `.4.3.0` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Modes, action/blind edges, lifecycle flow, local result/control channels, direct output, and recursion/progress guards pass 66/66 on PUC Lua/LuaJIT; helper-family split `.4.3.0` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
