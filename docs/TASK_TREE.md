...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current surface `.1.6.1`, descriptors `.1.6.2`, and structured diagnostics `.1.6.3` are closed. Native contract plus Rust/Dart/Julia APIs `.1.6.4.1-.4` are closed; final admission `.1.6.4.5` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
