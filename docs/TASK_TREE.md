...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Strict shared Lua stdio leaf `.10.9.6.2` is implementation- and signoff-complete from clean `b2e886de`: 111 + 210 + 247 assertions per runtime, complete Lua package/CLI/corpus/storage proof, 98 governance mutations, and canonical CI pass. Commit, brief-clear, and clean proof are next; exact admission `.10.9.6.3` may activate only afterward. Formal status stays 4/5 implementations + 4/6 runtimes with rollout pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
