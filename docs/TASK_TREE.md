...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Semantic toolbox guard `.10.4.0.1` is complete: exact 6/20/65, rollout 2+7, admission 1+5, runtime, and Perl 12-role claims have executable omission/wrong-value protection. The next frontier `.10.4.0.2` awaits the director's Unicode-versus-ASCII label-contract choice; Rust rollout/admission remain 2/9 and 1/6. The repeated-action recurring/public no-drift is closed at 8 complete / 0 pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
