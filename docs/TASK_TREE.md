...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | ADR `0044` / `.9.1.1.1` ratifies rule-local cursor policy, mode-sensitive bare edges, migration, descriptor/generated-v2, and conformance; implementation `.9.1.2-.9` is pending. Dual-ABI Lua logical `.5.2.6` is verified at 5/3 rollout and closing in its exact commit before generated/primary `.5.2.7`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
