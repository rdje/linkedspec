...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.4` — `.7.3.3.3` deferred bracket/mixed hlink scalar-ref fixtures behind a neutral scalar-ref/action-payload owner; next triage `portmap`/`lib_reader`/`ebnf` structural divergences | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
