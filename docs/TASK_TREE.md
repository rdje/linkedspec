...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, opaque source/outcome `.10.7.2`, private static projection `.10.7.3`, and calls/staging/generated `.10.7.4` are composition-closed. Behavior-free immutable-query audit `.10.7.5.0` freezes one detached authority, exact vocabulary, 19 static hashes, 26 raw boundaries, dual-ABI representation policy, and `.1-.4` order. Private non-traversal kernel `.10.7.5.1` is next. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
