...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua audit `.11.8.0` signoff-complete; inert construction `.11.8.1` next | `Overall roadmap - future parity backlog` | Retrieved authorities and identical PUC Lua/LuaJIT RED probes freeze one focused consumer and exact `.11.8.1-.4` construction, dynamic invocation, existing-emitter identity, and five-backend admission boundaries without behavior. Canonical Phase 0 1,031/1,031 plus the callable matrix pass; the sole-facing mdBook is synchronized. Exclusion freshness stays `.24.0-.2`; no capability-row/MCP/root-README/push movement. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
- Callable-codeblock parent `.11.7` is closed at four current backends with 20 governance mutations and 23 public
  documents; Lua explicit values/general bound calls remain `.11.8` and lexical capture remains deferred.
