...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Perl `.2.4` and parent `.2` signoff-complete from clean atomic 225 at `45460329` for intended atomic 226; Rust `.3` next after clean landing | `.spec language evolution / lossless segmentation and source preservation` | The full Perl consumer runs once ordinarily and once in the rooted neutral-first route; only `perl_runtime` is complete. Gap governance is 2/7/56; canonical CI passes Phase 0 1,031/1,031 in 765 seconds and exact neutral-plus-Perl routing. Five later runtime routes, recurring/public rows, typed composition, generated-plan v2, and all outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
