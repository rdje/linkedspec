...
| `FUTURE-PARITY-BACKLOG` | `active` / recurring mutation proof `.19.8` next | `Overall roadmap - future parity backlog` | All five backends implement frozen nested writes and `map_leaves!`; `.19.7` admits their two exact capability rows at current census 100/0/0. Recurring six-runtime proof is next under `.19.8`; public-current no-drift, parent closeout, and push remain `.19.9`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
