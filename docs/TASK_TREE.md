...
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` triage + `.3`/`.4` engine fixes (ADR `0008`) + cluster `.2` (108 STALE) + `.5.1` (plugin dataset) + **`.5.2`** all **DONE** → subtests **1-940 GREEN** and **`corpus_regression` (941) GREEN**. **`.5.2`:** the corpus "hang" root cause was **NOT a regex** (corrected) — the Lispish parser never returns `undef`, so `parse_with_lispish_multi`'s `while(1)` loop spun; added a forward-progress guard (TEST-ONLY); 76/76 corpus files ok; suite now reaches subtest 960. Running past corpus revealed **3 TEST-ONLY dark-tail failures → frontier `.5.4`** (960 stale plugin-dir `opendir` like `.5.1`; 952/953 `return(1)`→scalar `1` `?Top:` re-bless) → green phase0 → `.5.3` gate flips → `.6` book `:AND`. See [[lispish-corpus-catastrophic-backtracking]]. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
