...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current surface `.1.6.1`, descriptors `.1.6.2`, structured diagnostics `.1.6.3`, and native resolution `.1.6.4` are closed. Dart frontend/compiler trace `.1.6.5.1` passes; function/staged `.1.6.5.2` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
