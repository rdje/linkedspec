...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart-native label rollout `.10.5.0.2` has generated primitives `.0` and native parser/validator consumption `.1` complete; exact downstream artifact/selector/diagnostic/trace/loader identity `.2` is next after the clean commit. Semantic governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
