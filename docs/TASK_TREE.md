...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Rust behavior-free `.3.0` signoff-complete for intended atomic 227 | `.spec language evolution / lossless segmentation and source preservation` | The admitted Perl consumer runs ordinarily and in the rooted route at gap 2/7/56. `.3.0` freezes `.3.1-.3.5` metadata/native/generated/emitted/admission ownership without behavior or rollout movement; exact signoff is green from clean `eceb15ac`. `.3.1` remains pending until commit/brief/clean proof. Five later runtime routes, recurring/public rows, typed composition, generated-plan v2, and all outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
