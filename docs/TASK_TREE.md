...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust Unicode-label prerequisite `.10.4.0.2` is signoff-complete: ADR `0051`, generated Unicode 17 `XID_Continue` classification, exact Rust routes, and canonical proof preserve `Töp` without semantic promotion. Source/outcome foundation `.10.4.1` is next after the clean commit; rollout/admission remain 2/9 and 1/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
