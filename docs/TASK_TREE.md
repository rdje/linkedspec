...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.5.3` done 2026-07-09 - Dart runtime tracing now emits parse/rule scopes, regex decisions, action/blind child-dispatch decisions, lifecycle marks, cursor/source-boundary marks, and recursion-cutoff decisions while preserving output. Next frontier `.4.5.4` closes diagnostics/trace no-drift before staged runtime work. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
