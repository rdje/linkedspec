...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.2.2` lands reusable Rust entry/mode/direct-result execution at 41/61; `.1.5.2.3` is active for the remaining 20 canonical trace cases. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
surfaces agree on the strict 61-case Perl reference, Rust `.1.5.2.2` lands exact execution at 41/61, and
`.1.5.2.3` is the active trace frontier against the unchanged
...
and a thin MCP transport that owns no semantic behavior. The active backend frontier is Rust `.1.5.2.3`.
