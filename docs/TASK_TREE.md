...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua exact descriptors `.5.3.1` and one-emitter native-pipeline trace `.5.3.2` are complete at 155/155 on both ABIs; census-preserving no-drift `.5.3.3` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Exact outward descriptors and caller-owned full-pipeline trace are complete on PUC Lua and LuaJIT at 155/155 with status `native-full-pipeline-trace-v1`; no-drift `.5.3.3` is active and `.8.4` remains sole all-pass census admission. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
