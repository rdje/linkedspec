...
| `FUTURE-PARITY-BACKLOG` | `active` / `.15.3` canonical-signoff-complete; `.19.1.2` next | `Overall roadmap - future parity backlog` | `.15.3` repairs one false-positive Lua `next()` fixture through action-edge ownership, passes complete 178x2 plus standalone 109x2, and admits engineering-notes segment `4987` at finite 21/20 capacity without behavior drift. Future-only nested-write `.19.1.1` remains frozen at 105 mutations; neutral `map_leaves!` contract `.19.1.2` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
