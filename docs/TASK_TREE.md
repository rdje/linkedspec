...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Perl typed-source core `.14.2.1.1` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; typed source-location neutral/public parent `.14.1`, runtime plan `.14.2.0`, live correction `.14.2.0.1`, Perl RED `.14.2.1.0`, and immutable core `.14.2.1.1` are closed/signoff-complete. Current typed-source truth is 3/11 with 37 mutations and exact backend admission owners. The private immutable source/value/error core makes only the value consumer green; the 92-row projection consumer remains exact RED for next leaf `.14.2.1.2`. Sole-facing support stays unchanged until admission `.14.2.1.3`, and rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
