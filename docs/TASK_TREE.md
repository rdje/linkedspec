...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Punctuation-light parent `.16` is complete at capability 64/0/0 and condition headers remain parenthesized. Lua capture/cursor audit `.4.3.7.0` exposes seven documented current mark helpers outside every governed backend inventory; new cross-backend lane `.17` owns an exact neutral contract, Perl/Rust/Dart/Julia/Lua rollout, and independent symmetric-omission gate hardening before Lua named-mark closure. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks, controls, metadata-governed `with`, and deterministic scoped harray/array traversal close at 114/114 through `.4.3.6.6`; Unicode input/live-cursor views and controls `.4.3.7.1` pass 115/115, and all 16 anonymous capture calls `.4.3.7.2` pass 116/116 on both Lua ABIs. Independent earliest-boundary `.4.3.7.5` is next while seven documented mark helpers remain under cross-backend owner `.17` before `.4.3.7.3`. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
