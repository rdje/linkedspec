...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.4` done 2026-07-10 - ADR `0022` makes native in-memory host-language embedding primary; variant CLIs are thin adapters. Julia shipped no-drift is now 31/31 and active PNT is `JULIA-BACKEND-PARITY.6.2.5`; Lua `.1.3` inherits the native-module gate after Julia. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.6` done 2026-07-10 - one permanent complete-window regression locks shipped smoke at 31/31 exact outputs. Full tests pass with 816 assertions and status `runtime-corpus-shipped`; `.6.2.5` is active for routed top-level function fixtures. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.6` have since landed; `.6.2.5` is active.
...
with 816 assertions, status is `runtime-corpus-shipped`, `.6.2.4` closes, and `.6.2.5` is active.
