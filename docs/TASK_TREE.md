...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua interpreter-corpus parent `.6` is closed with ordered 105/105 library/runner execution at 167/167 per ABI and status `runtime-corpus-full`; primary CLI adapter `.7.1` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | `.6.3` permanently executes all 105 fixtures through the library and developer runner at 167/167 on both ABIs; parent `.6` is closed and primary CLI adapter `.7.1` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
