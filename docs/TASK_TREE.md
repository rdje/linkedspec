...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` (**current focus**) | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). Implementation gate CLEARED 2026-06-22 (`t/phase0_regression.t` green + `tools/run_ci_local.sh` EXIT 0; gradual-alias migration). **`.1.1` SPLIT 2026-06-23** → `.1.1.1` (Perl reference auto-existing variables) + `.1.1.2` (Rust lockstep parity) after a `dump_parser_source` ground-truth pass (too broad for one slice; design in Decisions + KM [[working-vars-no-strict-need-my-lexical]]). **`.1.1` DONE 2026-06-24** (Perl `.1.1.1` auto-existing wrapper-referenced working vars + Rust `.1.1.2` lockstep parity). **`.1.2` SPLIT** by inference channel; **`.1.2.1` DONE 2026-06-24** — Perl Channel 1 arg-position bare working-var auto-existence (extended the `.1.1.1` collector: bare `assign`→`$`, `push_value`/`push_nonempty`→`@`; all 20 specs byte-identical; +3 phase0 locks → **971 green**; gate EXIT 0; book taught; child-append/`.push` target + bare hash deferred to Channel 2). **Frontier → `.1.2.2`** (Rust lockstep parity for `.1.2.1`). | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
