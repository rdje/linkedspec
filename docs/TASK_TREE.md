...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, source/outcome `.10.7.2`, private static `.10.7.3`, calls/staging/generated `.10.7.4`, and immutable typed/raw-neutral query `.10.7.5` are composition-closed. Static query remains exact at 19 hashes/26 boundaries/focused 1,492 per ABI without runtime observation or promotion; behavior-free observation audit `.10.7.6.0` follows. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
