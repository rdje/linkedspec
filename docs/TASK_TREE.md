...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua descriptor `.5.3.1` now governs and emits the exact fixed-v1/variadic-v2/final-codeblock-v3 union; one-emitter native-pipeline trace `.5.3.2` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Exact outward descriptor union/emission `.5.3.1` is complete on both Lua ABIs, with Perl's existing final-codeblock record aligned to v3 and the census unchanged. One-emitter full-pipeline trace `.5.3.2` is next, followed by no-drift `.5.3.3`; `.8.4` remains sole all-pass Lua census admission. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
