...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common/backend/tool migration, reconciliation, descendant remediation, and final independent zero-residue proof `.1-.3` are complete. Both runtime-derived old temporary roots and bounded shared metadata remain empty after all six storage oracles; no final deletion target existed. Structural storage doctrine `.4.1` is active. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
