...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust `.19.3` complete, Dart `.19.4.1` next | `Overall roadmap - future parity backlog` | Rust `.19.3.1-.2` implement both frozen mutations and `.19.3.3` restores cross-backend root-target semantics. Controlled `.19.3.4.0` classifies older-artifact launch delay as external macOS policy/cache state and requires no `.19.3.4.1` repair. Dart nested write-vivification `.19.4.1` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
