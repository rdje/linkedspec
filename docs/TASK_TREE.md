...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl/Rust/Dart are 61/61. `.1.5.4.1` advances warmed Julia to 42/61 with exact help, strict UTF-8, and stable phase errors; `.1.5.4.2` is active for the 19 canonical-trace residuals before the four-command driver. `.10.1` parks introspection/MCP; `.11.1` parks generic codeblocks/`with`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | Local proof is 1,019 assertions/nine processes/99 fixtures. Global `.1.5.4.1` advances exact CLI identity to 42/61; canonical trace `.2`, capability census `.1.6`, and generated-source `.3` remain. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
