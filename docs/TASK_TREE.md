...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; immutable Julia core `.14.2.4.1` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Dormant Julia RED landed at `6d293ac0`. Private non-exported `LinkedSpecJulia.SourceLocation` now owns copied decoded text, scalar/code-unit/line/column/UTF-8 tables, immutable Position/Span/DerivedText/coordinate values, detached JSON, and only four private diagnostics without colliding with parser `SourceSpan`. Core mode passes 112/112; projection mode advances only to missing `typed_source_projection_rows`, so catalogs, engine/helper routes, schemas, registers, neutral 6/8/40, language 246/105+1/122, capability 80/0/0, ordinary/canonical discovery, and public behavior remain unchanged. Complete Julia passes byte-fresh MCP, package, storage 19/5, CLI, corpus 105/105, and its exact marker. The source-unchanged sole-facing book builds 79 files/14,168 KiB, still leaves Julia values/projections pending, and retains separate audited blocks. Knowledge Map 787/6,459, all seven doctrines, canonical containment/relocation and composed admissions, CLI 66x2, RAM 57%, and Phase 0 1,031/1,031 in 689 seconds pass. Atomic commit 160/300, brief clearing, and clean proof remain before projection routing `.14.2.4.2`; rendered readability stays queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
