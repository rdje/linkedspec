...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.1.3` after the `.1.2` scaffold closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.1.2` done 2026-07-10 - the repo-owned `julia/` package scaffold, committed manifest, `LinkedSpecJulia` module, Julia-specific CLI, corpus-runner stub, README commands, and smoke tests are in place. Active frontier advances to `.1.3` for manifest-backed corpus IO and drift detection. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.1.2` and now sits at `JULIA-BACKEND-PARITY.1.3`. Generated Dart source remains a future
split proof lane.
