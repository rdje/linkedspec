...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Rust typed projection `.14.2.2.2` signoff-complete/commit-pending | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location rollout remains 4/10 with 38 mutations. `.14.2.2.2` routes the exact 92 canonical helpers plus seven aliases through one per-input immutable Rust authority while retaining UTF-8-byte registers and external results. Nested projection 4/4, core 2/2, ordinary zero-test, alias carrier 1/1, complete Rust/corpus/generated/storage/CLI 66x2, real mdBook/isolated HTML paragraphs, Knowledge Map 786/6,409, and all seven doctrines pass. Canonical CI passes containment/relocation, CLI 66/66 twice, RAM 51%, and Phase 0 1,031/1,031 in 654 seconds; only commit/clean remains. Admission/promotion is exclusively `.3`, and rendered readability stays queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
