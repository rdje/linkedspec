...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI parent `.1.5` is closed by a recurring warmed 4x2x61 matrix. Capability census `.1.6` is active; `.3` remains generated-source parity. `.10.1` parks introspection/MCP; `.11.1` parks generic codeblocks/`with`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
