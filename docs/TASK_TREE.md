...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.3` — `.8.2` landed the minimal generated-source scaffold and isolated compile/run harness after `.8.1` split the HandlerIR/CompiledSpec-to-Rust-source lane; next emit non-repetition handler families | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.3` unless a new terse leaf is split.
