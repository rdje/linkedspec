...
| `FUTURE-PARITY-BACKLOG` | `active` / `.19.3.1` next | `Overall roadmap - future parity backlog` | Perl `.19.2.2` implements `map_leaves!`, repairs the stale typed-root checker anchors exposed by canonical attempt one, passes the corrected exact canonical gate, and closes parent `.19.2` without portable/public admission. Rust nested-write `.19.3.1` is the next clean-boundary leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
