...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0 and neutral cursor `.9.1.2` is committed at 1/7. Perl `.9.1.3.0-.3` is complete through descriptor v1; generated-source v2 `.9.1.3.4` is next only after the clean descriptor commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
