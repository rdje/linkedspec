...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust `.14.7.4` closed; Dart `.14.7.5.0` next | `Overall roadmap - future parity backlog` | Rust `.14.7.4.0-.4` now provides and privately admits the complete staged-AST path through fresh native, reconstructed, validated generated-plan, and independently compiled emitted carriers. Neutral governance is 84 mutations with Perl and Rust complete; public/format/later-backend truth is unchanged. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
