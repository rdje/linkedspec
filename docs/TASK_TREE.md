...
| `FUTURE-PARITY-BACKLOG` | `active` / neutral `.14.7.2` canonical-signoff-complete from clean `54c65df8`; Perl `.14.7.3.0` next after commit | `Overall roadmap - future parity backlog` | `.14.7.2` freezes executable neutral staged-AST syntax/provenance/pre-resolution/cache/policy/queue/bounds/detachment/carrier/diagnostic authority through 72 mutations with every backend and public rollout leg pending; `.14.7.3.0` next freezes Perl's dormant RED. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
