...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / reading `.3.2.1` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; five facade/invocation owners read; `.3.2.2` splits the remaining 84 Perl paths. Codebase/book remain No; `rgx` is excluded. `.7` repairs EPERM-as-dead cleanup after reading and before recovery/purge or mutation setup. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
