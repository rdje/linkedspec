...
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks/controls/callbacks close at 114/114; input/cursor `.4.3.7.1` passes 115/115; all 16 anonymous capture calls `.4.3.7.2` pass 116/116; and non-consuming earliest-boundary `.4.3.7.5` passes 117/117 on both Lua ABIs. Cross-backend named-mark `.17.1` is next before Lua consumes `.17.4` in `.4.3.7.3`. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
cross-backend complete named-mark `.17.1` becomes the next frontier before Lua `.4.3.7.3`.
