...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.3.2` — `.7.3.3.1` split hlink delimiter fixtures after `[abc]`/mixed outputs exposed a Perl scalar-ref JSON representation gap; next add the JSON-safe `{abc}` curly-brace fixture | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
