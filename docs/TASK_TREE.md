...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia callable-codeblock dynamic invocation `.11.6.2` signoff-complete; landing commit pending | `Overall roadmap - future parity backlog` | From clean `3a38338d`, `.11.6.2` implements static-precedence bound calls, once-only arguments, caller-context fixed/rest binding and results, exact failures/recursion, and native/serialized/generated/emitted identity. Focused 125/125 + construction 239/239 + variadic 55/55, package/local Julia, KM 770/6,251, book 79/13,972 KiB, seven doctrines, CLI 66x2, and Phase 0 1,031/1,031 in 655 seconds pass. Only the workflow commit/clean proof remain before `.11.6.3`; no capability/MCP movement or push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
