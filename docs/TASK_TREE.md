...
| `INTER-MATCH-GAP-CAPTURE` | `active` / shared Lua planning `.6.0` from atomic 245 `a6ff2614` | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, Dart, and Julia run ordinarily and in the rooted route at gap 5/4/58. Lua `.6.0` freezes `.6.1-.6.5`: authored/static/compiled metadata plus dormancy, one shared recognition-frame/token native path, normalized/descriptor/generated-v2 carriers, fresh-process dual-ABI emitted proof with storage 18→19, and existing-primary/nine-role admission. Existing rule-slot events retain post-action/pre-`LE` timing. Both Lua runtimes, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
