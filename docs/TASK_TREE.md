...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua `.10.7.1-.6` are composition-closed. Exact dual-ABI admission `.10.7.7` is active task-tree-first from clean `17348041`: one Lua-5.1-compatible twelve-role consumer must run unchanged on both ABIs, both rows must share its topology, nine mutations lock it, and only Lua may advance from 6/20/89 at 5/9 + 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
