...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart immutable query parent `.10.5.4` is composition-closed: public capabilities/typed/raw-neutral evaluation matches all 19 static digests plus 26 validation boundaries over detached projection data. Typed runtime observation `.10.5.5` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
