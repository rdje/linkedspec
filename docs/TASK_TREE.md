...
| `INTER-MATCH-GAP-CAPTURE` | `active` / executable-neutral audit-plan `.1.0` signoff-complete; `.1.1` next after atomic 218 | `.spec language evolution / lossless segmentation and source preservation` | `.1.0` signoff-completely freezes the exact future v1 artifact/checker, Unicode named-slot identity, `@capture_gaps` eligibility, typed accessors, prefix/interstitial/tail and empty-span lifecycle, failure/rollback/recursion, nine-leg rollout, repository-rooted routes, mutations, diagnostics, and carrier obligations without changing behavior. Canonical proof passes doctrines, containment/relocation, CLI 66x2, RAM 52%, and Phase 0 1,031/1,031 in 735 seconds. `.1.1-.1.3` own neutral artifacts/governance/closeout; `.2-.7` own six runtimes and public admission. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
