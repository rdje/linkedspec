...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.3.11` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; `.31` preserves forward reading and repair ownership. Reads compiled types, pinned Unicode labels and AST-validation entrypoints; reconciles dated evidence. Next `.3.3.12`: remaining validation, descriptors and cursor test prefix. Codebase No; physical mdBook Yes, formal .4 alignment pending; `rgx` excluded. `.7` blocks recovery/purge; `.29` belongs to policy adoption `.5`. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
