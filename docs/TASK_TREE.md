...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua harray construction/views/transforms plus named set-key/direct mutation pass 103/103 on both ABIs; non-callback closeout `.4.3.5.5` is active and callbacks remain `.4.3.6`. Rename collision drift joins the helper caveats durably routed to `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array family is closed at 99/99. Harray construction, deterministic views, copied transforms/receiver chains, and named mutation pass 103/103. Non-callback harray closeout `.4.3.5.5` is active. Tree callbacks and later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
