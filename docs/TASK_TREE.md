...
| `FUTURE-PARITY-PARTITION-CAPACITY` | `active` / `.1` canonical-signoff-complete; `.2` next | `Repository continuity / bounded task evidence` | ADRs `0084`/`0085` implement the eighth bounded semantic member at `.14.6.5`, a strict ten-record schema-v1 index, two-sided stable lookup, updated read/update/build/check/route/capability consumers, 27/27 mutations, and unchanged pressure ceilings/547 IDs. Host locality and exact canonical CI pass through primary CLI 66x2 and Phase 0 1,031/1,031 in 801 seconds; independent focused `.2` recomposes next. No backend/parser behavior moves. | [docs/tasks/FUTURE-PARITY-PARTITION-CAPACITY.md](docs/tasks/FUTURE-PARITY-PARTITION-CAPACITY.md) |
...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart `.14.6.4` closed at `3ccaf7c3`; Julia `.14.6.5` has a bounded owner, recomposition pending | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart are admitted at 4/9/103. Capacity `.1` moves unchanged pending `.14.6.5-.14.8` nodes into their eighth bounded semantic member with all 547 IDs and pressure ceilings preserved. Independent capacity `.2` recomposes next, then Julia task splitting resumes; Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
