...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia callable-codeblock construction `.11.6.1` signoff-complete; `.11.6.2` next after clean commit | `Overall roadmap - future parity backlog` | Dart parent `.11.5` is committed clean. Julia `.11.6.1` now owns exact inert literal/signature/body/source/Unicode-span state, brace disambiguation, native/serialized/generated/emitted/semantic identity, 239/239 focused proof, complete Julia and canonical gates, and the clean handoff contract. Dynamic invocation `.11.6.2` remains pending until this leaf lands clean; no capability/MCP movement or push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
