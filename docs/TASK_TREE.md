...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust native MCP behavior-free audit/split `.10.9.3.0` is active task-tree-first from clean Perl closeout `4473a812`. It owns exact native/embedding/transport/security seam evidence and `.1-.4` dependency-safe planning only; no Rust server, dependency, fixture, API, status, or behavior may move. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
and canonical signoff, and no code or behavior movement; `.10.9.3.1` follows only after the clean plan commit.
