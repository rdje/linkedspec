...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl/Rust are 61/61. Dart `.1.5.3.1` replaces the corpus primary and passes the exact 29-case boundary subset default/POSIX; `.1.5.3.2` is active for 11 direct results before trace. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
