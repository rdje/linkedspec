...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.3.1` after the Julia frontend container. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.2.4` done 2026-07-10 - Julia now consumes spec-shaped `function_definition` / `function_definition_error` nodes, strips function spans before rule parsing, preserves staged body parse-job sidecars, and keeps direct `parse_spec(...)` rule-only. Active frontier advances to `.3.1` for typed helper/action AST parsing. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.2.4` and now sits at `JULIA-BACKEND-PARITY.3.1`. Generated Dart source remains a future
split proof lane.
