...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.2` done 2026-07-09 - Dart now has the first runtime interpreter over compiled rules: default/AND/OR/repetition dispatch, action-edge and blind-call child dispatch, lifecycle execution, explicit returns, `retv`, accumulators, recursion cutoffs, bounded repetition, and zero-progress cutoffs. Next frontier `.4.3` expands the runtime value/helper families. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
