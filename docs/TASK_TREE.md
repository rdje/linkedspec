...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart strict copied input/private Unicode source map `.10.5.1.1` is complete from clean `9260c526`, including source ceilings and dependency-free digest/offline-caller proof; compiled-or-failed authority `.10.5.1.2` is next after its clean commit, followed by composed closeout `.3`. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
