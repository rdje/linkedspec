...
| `DART-BACKEND-PARITY` | `done` / `closed` (scoped milestone) | `Overall roadmap - future backend parity (Dart first)` | Global proof is now 180 tests, 105 corpus, exact 61x2 CLI, admitted full-pipeline trace, deterministic v1 emission, and exact ten-family direct execution/four rejections. Generated manifest admission remains global `.3.3.3`. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Census is 58/0/2. Perl/Rust pass. Dart scaffold and ten-family direct execution are green; accepted-subset admission `.3.3.3` is active before Julia and exact closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
