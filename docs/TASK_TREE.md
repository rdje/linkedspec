...
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` triage + `.3`/`.4` engine fixes (ADR `0008`) + cluster `.2` (108 STALE) + `.5` (incl. `.5.3.2.1` status reconciliation + `.5.3.2.2` narrative-doc/book drift sync) all **DONE** → **`t/phase0_regression.t` 960/960 GREEN** end-to-end **and** `bash tools/run_ci_local.sh` EXIT 0; `LEGACY-VHDL-RETIRE` + `NONCORE-QUARANTINE` closed. **Only open leaf: `.6`** (book `:AND` reconciliation — the now-fixed `::AND`+regex+return form vs the book's "Body rule only"/"no regex on top" idiom; **likely a short user policy check**). See [[lispish-corpus-catastrophic-backtracking]]. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
