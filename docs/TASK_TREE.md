...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.1.6.2` closes strict Perl UTF-8 at 61/61; `.1.5.1.6.3` is active for reference no-drift before Rust. `.10.1` parks semantic introspection/MCP; `.11.1` parks the generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
61/61 in default/POSIX environments; `.6.3` is active for final reference no-drift before Rust.
...
and a thin MCP transport that owns no semantic behavior. The active backend frontier is `.1.5.1.6.3`.
