...
| `FUTURE-PARITY-BACKLOG` | `active` / `.14.3.2.2` Perl integration complete; `.14.3.2.3` admission next | `Overall roadmap - future parity backlog` | Typed-source `.14.2` and neutral transaction `.14.3.1` are composition-closed. Private Perl authority `.14.3.2.1` is committed at `8138bea5`; `.14.3.2.2` adds four dedicated ActionIR nodes, recursive static effect closure, live/generated invocation state, falsey-safe results, and cursor-only typed progress behavior. Its final-path consumer is GREEN but remains outside ordinary/canonical discovery; canonical signoff passes CLI 66x2, RAM 58%, and Phase 0 1,031/1,031 in 727 seconds. Atomic 186/300 is complete; explicit admission `.14.3.2.3` stays next and rollout remains 1/9. `TASK-TREE-METADATA-HYGIENE.5` separately owns active-frontier prevention; rendered readability remains queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
