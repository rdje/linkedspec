...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` (**current focus**) | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). Implementation gate CLEARED 2026-06-22 (`t/phase0_regression.t` green + `tools/run_ci_local.sh` EXIT 0; gradual-alias migration). **`.1.1` SPLIT 2026-06-23** → `.1.1.1` (Perl reference auto-existing variables) + `.1.1.2` (Rust lockstep parity) after a `dump_parser_source` ground-truth pass (too broad for one slice; design in Decisions + KM [[working-vars-no-strict-need-my-lexical]]). **`.1.1` DONE 2026-06-24** (Perl `.1.1.1` auto-existing wrapper-referenced working vars + Rust `.1.1.2` lockstep parity). **`.1.2` SPLIT** by inference channel; **`.1.2.1`+`.1.2.2` DONE 2026-06-24 — Channel 1 complete on BOTH variants.** `.1.2.1` (Perl): extended the `.1.1.1` collector (bare `assign`→`$`, `push_value`/`push_nonempty`→`@`; all 20 specs byte-identical; +3 phase0 locks → **971 green**; book taught). `.1.2.2` (Rust): REQUIRED a Rust engine change (unlike `.1.1.2`) — `resolve_scalar_target`/`resolve_array_target` now map a bare target to the working var (`allow_bare` gate); 2 oracle fixtures + 4 integration tests; cargo 248→252; clippy zero-new. Child-append/`.push` target + bare hash deferred to Channel 2 (`.1.2.3`+, pending `.1.5`). `.1.2` stays `active` (Channel 2 pending). **`.1.4` SPLIT 2026-06-24** → `.1.4.1` (Perl reference) + `.1.4.2` (Rust lockstep parity) after a TOOLBOX-first `call_spec_handler_subst` ground-truth pass: the three terse spellings `set`/`cat`/`copy` are currently UNRECOGNIZED, and the change spans separable Perl + Rust ownership (ADR 0006 lockstep) with three shapes — `cat`=pure rename (`_normalize_method_name`), `set`=statement-level (`Contracts.pm`/`DeclareMethod`), `copy`=unified array-vs-hash dispatch (`MethodLowering`); KM [[terse-helper-rename-lowering-sites]]. **`.1.4.1` DONE 2026-06-24** (Perl reference — `set`/`cat`/`copy` now lower **byte-identically** to `assign`/`concat`/`array_copy`+`hash_copy` in every position: `cat`/`set` via `_normalize_method_name`; `set` statement-level recognition extended at the contract + IR-event scanner + bare-arg auto-`my` collector; `copy` via a dedicated array-then-hash dispatch + every declare-init / return-payload / FlowExpr-source / type-inference recognizer. All 20 specs byte-identical; `set`==`assign` ASSIGN node; terse spec runs == canonical twin end-to-end; +4 phase0 locks → **975 green**; gate EXIT 0; ratio 1.0000; book taught 3 pages; KM [[terse-helper-rename-lowering-sites]] updated). **Frontier → `.1.4.2`** (Rust lockstep parity — `Engine::call_helper()`). | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
