...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.5` — `.7.3.4.3` added action-edge child/target aggregation parity plus `portmap_concatenation`, `ebnf_expression_rules`, and `ebnf_logging_annotation`; next triage `.7.2` null/action-parser candidates (`BNF`, `DT`, `ifelse`, `operators_try`, `spec.spec`) | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.7.3.5` unless a new terse leaf is split.
