...
| `FUTURE-PARITY-BACKLOG` | `active` / callable-codeblock Rust parent `.11.4` closed; Dart `.11.5.1` queued after direction routing | `Overall roadmap - future parity backlog` | `.11.4.3` is committed at `4aebf906` after neutral 7/11/9/7/4/8, Perl 10/10, Rust 18/18, complete Rust operational proof, Knowledge Map 761/6,175, mdBook, seven doctrines, and canonical Phase 0 1,031/1,031 in 639 seconds. Behavior-free `SPEC-LANGUAGE-SELF-CONTAINMENT.0` now routes the approved long-horizon direction before Dart activation. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
| `MEMORY-COMMIT-POINTER-ENFORCEMENT` | `active` / `.0` signoff-complete; `.1` next after clean commit | `Repository continuity and doctrine enforcement` | ADR `0065` replaces the impossible committed self-hash premise with `activation_commit`: exact current `HEAD` before a leaf commit and exact `HEAD^1` afterward, while Git owns current identity. History proves 31/31 parseable recent pointers carry first parent and none carry self; Knowledge Map 763/6,193, mdBook, all seven doctrines, CLI 66x2, and Phase 0 1,031/1,031 pass. `.1` implements enforcement; Dart `.11.5.1` stays parked through `.2`. | [docs/tasks/MEMORY-COMMIT-POINTER-ENFORCEMENT.md](docs/tasks/MEMORY-COMMIT-POINTER-ENFORCEMENT.md) |
