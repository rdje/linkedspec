...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Diagnostic-output `.5.1` is closed at 8/0. Logical audit `.5.2.0` and neutral ADR `0043`/contract `.5.2.1` are complete; rollout remains 0/8 and Perl typed-AST/lowering `.5.2.2` is active before Rust/Dart/Julia/Lua/generated/gate/public `.3-.9`. ADR `0036`/`.19` remain later-owned. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
