...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact shared Lua admission `.10.9.6.3` is signoff-complete from clean `15f537df`: one identical 202-assertion consumer passes on PUC Lua and LuaJIT, 114 mutations lock exactly 5/5 implementations + 6/6 runtimes with shared rollout pending, and canonical CI passes through Phase 0 1,031/1,031 in 659 seconds plus the complete dual-ABI Lua gate. Commit/brief-clear/clean proof precedes no-change closeout `.10.9.6.4`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
