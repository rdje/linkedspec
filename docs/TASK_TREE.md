...
| `README-STABILITY-POLICY` | `active` | `Repository architecture / documentation sustainability` | `.0` audits the complete 1,615-line / 159,437-byte README before any trim, freezes lossless canonical routing, a reviewed 105-line / 5,072-byte prototype, hard 128-line / 6,144-byte budgets, and registered-doctrine design in ADR `0063`. `.1` adopts content/enforcement after `.0` commits; `.2` closes unchanged and returns to MCP. Project-level licensing is separately proposed in `.3` for director decision. | [docs/tasks/README-STABILITY-POLICY.md](docs/tasks/README-STABILITY-POLICY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Recurring MCP architecture leaf `.10.9.7.0` is signoff-complete from clean shared Lua closeout `d05e17ed`. ADR `0062` records the audit finding that native semantic admission proves all twenty cases while current MCP consumers directly compare only capabilities plus one representative query. `.1` is the next dependency-ordered MCP leaf: strengthen those same identity roles, compose one routed six-runtime gate, and only then promote thin transport; formal state remains 5/5 + 6/6 pending/114. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
