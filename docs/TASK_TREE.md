...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust progressive private authority `.14.6.3.1` complete; four-carrier integration `.14.6.3.2` next | `Overall roadmap - future parity backlog` | Rust now has private immutable registry/invocation/rebased-view authority with exact neutral diagnostics and dormant focused proof. The unchanged four-carrier RED still fails only on the missing dedicated node, ordinary/canonical discovery remains inert, and progressive stays 2/9/86. Implement `.14.6.3.2` next; admission `.3.3`, Dart/Julia/Lua `.14.6.4-.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
