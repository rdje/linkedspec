...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.5.1` after action-edge child push moved shipped smoke to 25/31. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.4` done 2026-07-10 - all four action-edge child-push forms close the four spec.spec smokes and route EBNF quote mutation to `.5.2`. Full tests pass with 793 assertions and status `runtime-corpus-action-edge-child-push`; shipped smoke is 25/31 and `.6.2.4.5.1` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.2.3` have since landed; `.6.2.4.5.1` is active.
...
`runtime-corpus-action-edge-child-push`, and `.6.2.4.5.1` is active.
