...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, census `.1.6.0`, 237-name audit `.1.6.1.0`, newline splitting `.1.6.1.1`, `endswitch` termination `.1.6.1.2.1`, and all pure/position/control plus Rust capture repairs are closed. Active `.1.6.1.2.2.4.2` completes Dart capture/mark semantics. Later leaves own Julia capture, admission, descriptor/diagnostics/resolution/trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
