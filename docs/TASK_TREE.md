...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Behavior-free Dart MCP native-seam audit `.10.9.4.0` is complete from clean Rust closeout `7f44d2a1`; ADR `0059` freezes generated-contract, in-process server, security/wire, admission, and no-change closeout ownership under `.1-.4` without behavior or ledger movement. Generated binding/decoded server `.10.9.4.1` is next after the clean plan commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
