...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart immutable query parent `.10.5.4` is composition-closed. Runtime-observation audit `.10.5.5.0` freezes exact slot/result seams, absent-sink and no-query authority, generated-wrapper exception identity, and implementation order; typed live capture `.10.5.5.1` is active. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
