...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.3.4` after the Julia user-function registry leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.3.3` done 2026-07-10 - Julia now has ordered user-function registry records, staged body parse-job queues, immutable body-AST stitching, and registry-aware exact-arity ActionIR user-call classification. Active frontier advances to `.3.4` for compiled-spec and interpreter-state construction. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.3.3` and now sits at `JULIA-BACKEND-PARITY.3.4`. Generated Dart source remains a future
split proof lane.
