...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart strict source map `.10.5.1.1` is committed at `ae870152`; staged compiled-or-failed authority `.10.5.1.2` is signoff-complete from that clean base with exact native diagnostics, entry identity, generated-v2 plan, and no target execution. Composed foundation closeout `.10.5.1.3` is next after its clean commit. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
