...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust calls/staging/generated projection `.10.4.3` is signoff-complete from clean `c1a7e413`; immutable capabilities/query `.10.4.4` follows after the clean commit. Semantic rollout/admission stay 2/9 and 1/6. Startup mdBook drift is queued separately under `.23`. repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
