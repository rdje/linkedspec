...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; typed source-location runtime plan `.14.2.0` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; typed source-location neutral/public parent `.14.1` is composition-closed. `.14.2.0` root-causes stale public rollout rows and freezes separate source authority, immutable value/projection boundaries, exact backend module/tests, and RED/core/projection/admission children without behavior. After its clean commit, correction `.14.2.0.1` follows before Perl. Knowledge Map is 785/6,375; the sole-facing rendered-readability audit remains independently queued and nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
