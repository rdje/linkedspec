...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Scalar numeric drift is split; neutral contract `.4.3.3.1.1` is active before Lua implementation. Temporary compatibility retirement is durably owned by `.12.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | String helpers close at 76/76; scalar numeric cross-backend drift is measured and neutral contract `.4.3.3.1.1` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
