...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.5.3` after statement mutation moved shipped smoke to 30/31. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.5.2` done 2026-07-10 - statement regex mutation closes both EBNF, both lib_reader, and simenv while preserving pure slicing. Full tests pass with 808 assertions and status `runtime-corpus-statement-mutation`; shipped smoke is 30/31 and `.6.2.4.5.3` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.2.3` have since landed; `.6.2.4.5.3` is active.
