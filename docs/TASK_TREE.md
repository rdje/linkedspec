...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Thin MCP parent `.10.9` means one exact contract, five native implementations, and six runtime admissions. Architecture `.10.9.0` is complete; ADR `0055` and protocol-policy `.10.9.1.0` select modern MCP `2026-07-28` stdio; machine artifact `.10.9.1.1` pins the neutral schema/payload/corpus/JSONL bundle. Independent checker `.1.2` is next, followed by contract closeout `.1.3`, Perl `.2`, Rust `.3`, Dart `.4`, Julia `.5`, shared Lua `.6`, and six-runtime `.7`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
