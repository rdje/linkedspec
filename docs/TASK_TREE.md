...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.3.3` — `.8.3.2` landed direct generated execution for default/OR acode; next emit direct AND acode execution | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.3.3` unless a new terse leaf is split.
