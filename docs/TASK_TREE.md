...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.3` — `.7.3.2` verified the historic RTLUtils timeout is retired from current core and hardened the oracle generator to enforce per-case fork/SIGKILL timeouts; next try remaining `hlink_substitution` delimiter/link-path fixtures | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
