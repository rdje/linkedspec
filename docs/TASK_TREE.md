...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Lua carrier `.6.3` canonical-signoff-complete from atomic 248 `4e625a9f` | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, Dart, and Julia run ordinarily and in the rooted route at gap 5/4/58. Shared Lua `.6.1-.6.3` now carries authored/static/compiled metadata, one recognition-frame/token native path, normalized reconstruction, compatible descriptors, and same-engine generated-v2 execution behind exact dormancy. `.6.4-.6.5` retain fresh-process dual-ABI emitted proof with storage 18→19 and existing-primary/nine-role admission. Existing rule-slot events retain post-action/pre-`LE` timing. Both Lua runtimes, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
