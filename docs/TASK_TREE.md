...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia core `.9.1.1.2.4.1` is signed off: neutral 79, exact primary 32/65x2, corpus 105, 4/7 governance, and canonical Phase 0 1,031 pass without route/admission promotion. Routes `.2` activate only after the clean `.1` commit; cursor `.9.1.6` and admission `.3` remain dependency-ordered. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
