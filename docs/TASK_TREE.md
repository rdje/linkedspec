...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart calls audit `.10.5.3.0` freezes the exact 22/25 registry/ActionIR/sidecar/generated authority map and bounded `.1-.3` split without behavior. Typed functions/helpers/calls/bindings `.10.5.3.1` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
