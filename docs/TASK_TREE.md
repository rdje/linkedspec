...
| `TRACE-OBSERVABILITY` | `active` | `Overall roadmap — engine observability / developer experience` | User directive 2026-06-19: discoverable CLI trace control + comprehensive "see everything" trace (function enter/exit, if/switch/case branches). Framework EXISTS (`Trace.pm`: enter/exit/decision/levels/sinks; env/per-call control works). `.1` audit showed trace was not exhaustive; `.2` added discoverable `bin/linkedspec` CLI flags and docs. `.3` split and closed Perl reference coverage through generated-handler, RuleIR, EmitContext, ActionIR pipeline, compact lowerer, MethodLowering, and compile/ActionIR closeout leaves. `.3.5` closed trace no-drift/contract docs and split required backend parity. `.4.1` mapped the neutral mdBook trace contract onto Rust entrypoints and owner boundaries before code. `.4.2` added Rust trace controls, levels, sinks, event primitives, and traced entrypoints while preserving default quiet behavior. `.4.3` added Rust compile/spec-parser/staged-dispatch trace events. `.4.4` added Rust interpreted-runtime and generated-plan runtime branch/lifecycle/mark-capture events. Frontier `.4.5` closes cross-variant trace parity proof and the future-variant checklist. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. `TRACE-OBSERVABILITY.3` has since split, `.3.1` through `.3.5` have closed, `.4.2` has added Rust trace controls, `.4.3` has added Rust compile/spec-parser trace events, and `.4.4` has added Rust runtime trace events; PNT returns to `TRACE-OBSERVABILITY.4.5` unless a new terse leaf is split or another active tree is reprioritized.
...
`RUST-PARITY` unblocked `TOP-RULE-AS-NORMAL.3.2`; that leaf has since closed, `TRACE-OBSERVABILITY.1` has completed its read-only audit, `TRACE-OBSERVABILITY.2` has added CLI trace control, `.3` has split before code, `.3.1` through `.3.5` have closed, `.4.2` has added Rust trace controls, and PNT advances to `TRACE-OBSERVABILITY.4.3`.
