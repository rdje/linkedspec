...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia staged `.14.7.6.2` focused-signoff-complete | `Overall roadmap - future parity backlog` | Julia's dormant consumer is 309 GREEN/one exact `.14.7.6.3` recursive-authority RED after `.2` added caller-frozen pure resolution/cache, one complete deterministic marker depth, sibling isolation, detached atomic stitching, and all policies. Atomic `.2` landing precedes task-tree-first `.3`; carriers/admission, rollout, formats, and public behavior remain fixed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
