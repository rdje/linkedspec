...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.3.27` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; `.31` preserves forward reading and repair ownership. Reads the remaining primary CLI and recognition authority; passes 66 CLI cases and qualifies the Rust invalidation guard. Next `.3.3.28`: recognition runtime suffix and value runtime prefix. Codebase No; physical mdBook Yes, formal .4 alignment pending; `rgx` excluded. `.7` blocks recovery/purge; `.29` belongs to policy adoption `.5`. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
