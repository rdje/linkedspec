...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | No-change Dart MCP closeout `.10.9.4.4` passes focused/canonical signoff from clean `956ea5e9` and closes parent `.10.9.4` without behavior or ledger movement. Julia `.10.9.5` is the next task-tree-first frontier only after the clean closeout commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
