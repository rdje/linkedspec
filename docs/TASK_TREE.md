...
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` triage DONE (173 → 108 STALE / 65 REAL, 2 engine defects). User **authorized BOTH engine fixes** 2026-06-21 (ADR `0008`). **`.3`+`.4` DONE 2026-06-21 — BOTH engine defects fixed** (#1 AND-rule action-codegen `\$"`→verbatim in `HandlerVariantEmitter.pm`; #2 input-boundary guard in `Runtime.pm`); full phase0 **173 → 109 failing** (64 cleared, 0 regressions). Frontier → **`.2.1`** re-bless cluster A (TEST-ONLY), then `.2.2`–`.2.4` (B/C/F/G stale), then `.5` green-phase0 verify (+ `corpus_regression` subtest-941 tail), then `.6` book `:AND` reconciliation. Gates green phase0 + `SPEC-FORMAT-TERSE`. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
