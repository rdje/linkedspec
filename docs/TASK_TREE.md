...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Rust native runtime `.3.2` signoff-complete for intended atomic 229 | `.spec language evolution / lossless segmentation and source preservation` | The admitted Perl consumer runs ordinarily and in the rooted route at gap 2/7/56. Rust authored/static `.3.1` landed at `95127e1d`; `.3.2` now implements invocation-local native gap state, lifecycle, rollback/nesting, and private accessors while the final consumer remains ignored. `.3.3` is next after the clean atomic-229 boundary; `.3.3-.3.5` retain generated/descriptor, emitted, and primary/admission ownership. Five later runtime routes, recurring/public rows, typed composition, generated-plan v2, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
