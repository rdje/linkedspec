...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. ADR `0044` / `.9.1.1.1` ratifies rule-local cursor policy and mode-sensitive bare edges; executable neutral contract and exact migration inventory `.9.1.2` are verified at 1/7, with Perl `.9.1.3` next after the clean commit boundary. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
