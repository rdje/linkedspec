...
| `FUTURE-PARITY-BACKLOG` | `active` / behavior-free callable closeout/Lua-routing audit `.11.7.0` signoff-complete from clean `3767b8c3` | `Overall roadmap - future parity backlog` | Neutral and focused Perl/Rust/Dart/Julia callable proof, the unchanged complete dual-ABI Lua gate, Knowledge Map 772/6,262, mdBook, all doctrines, and canonical Phase 0 1,031/1,031 are green. Exact Lua gaps/reuse seams and `.11.7.1-.2` / `.11.8.0-.4` owners are durable. Workflow landing and clean handoff precede `.11.7.1`; no behavior/capability/MCP movement or push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
