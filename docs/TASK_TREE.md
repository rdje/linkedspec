...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.2` after bounded corpus selection/reporting landed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.1` done 2026-07-10 - Julia supports ordered named/offset/limit library selection and bounded runner PASS/FAIL reporting with strict selector/unbounded guards. Thirty added assertions bring full tests to 745 and status `runtime-corpus-selection`; `.6.2.2` starter fixtures 0–39 are active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` has since landed; `.6.2.2` is active.
...
full tests to 745 with status `runtime-corpus-selection`; `.6.2.2` starter fixtures 0–39 are active.
