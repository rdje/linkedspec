...
| `STAGED-LINKED-PARSING` | `active` | `Overall roadmap — .spec language model / parser composition` | `STAGED-LINKED-PARSING.2` — `.1` adopted staged linked parsing doctrine before implementation: stage-N specs may emit source-provenance text islands, and later specs parse those payloads through a deterministic parse graph; spec imports/composition and staged parse dispatch are separate features. Next design spec-file imports/composition. | [docs/tasks/STAGED-LINKED-PARSING.md](docs/tasks/STAGED-LINKED-PARSING.md) |
