...
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks, controls, metadata-governed `with`, and deterministic scoped harray/array traversal close at 114/114 through no-drift/dependency leaf `.4.3.6.6`; final user-function codeblocks remain `.5.1`, explicit callable values remain `.11.7`, and capture-slice/mark/input/cursor helper family `.4.3.7` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
