...
| `FUTURE-PARITY-BACKLOG` | `active` / MCP `.10.9.7.1.1.4` signoff-complete; clean commit pending | `Overall roadmap - future parity backlog` | From clean `de46b26a`, `.1.1.4` recomposes semantic 6/20/110, MCP 35/10/10/76, six consumers, complete/141 governance, primary 30/30, and canonical Phase 0 1,031/1,031 unchanged. Implementation parents `.1.1` and `.1` are closed; `.10.9.7.2` follows the clean commit (101/300, no push). | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
