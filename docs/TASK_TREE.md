...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart private static parent `.10.5.2` is composition-closed: five exact construction targets, clone/privacy/isolation, and complete Dart/public/canonical proof pass without query, runtime, rollout, or admission promotion. `.10.5.3` is next for exact calls/bindings/staged/generated provenance. Governance remains 6/20/73 at rollout 3/9 and admission 2/6; startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
