...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). **Implementation gate CLEARED 2026-06-22** — `t/phase0_regression.t` 960/960 green + `tools/run_ci_local.sh` EXIT 0 (via `PHASE0-BACKHALF-TRIAGE`); migration policy resolved (gradual-alias). Leaves `.1.x`+ are now **PNT-eligible** (first: `.1.1` auto-existing variables). | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
| `LEGACY-VHDL-RETIRE` | `active` | `Overall roadmap — keep only portable/cross-variant code (retirement)` | `.1`/`.2`/`.3` **done** 2026-06-18 — subsystem retired (3 modules + 6 `.plg` + phase0 smoke ≈ 6,701 lines; "full closure"). **RTLUtils hang CLEARED** (subtest 110 `add_header_n_context_clause`/`RTLUtils.pm:104`). **`.4` DONE 2026-06-22** (RTLUtils hang cleared + full local gate green via `PHASE0-BACKHALF-TRIAGE`; the subtest-131 `HTML::PathLinks` hang became moot — its smoke was excised when `NONCORE-QUARANTINE.3` relocated the module). `.5` **pending** (blocker cleared) — doc/book/KM drift sync executed cross-tree by `PHASE0-BACKHALF-TRIAGE.5.3.2.2`. | [docs/tasks/LEGACY-VHDL-RETIRE.md](docs/tasks/LEGACY-VHDL-RETIRE.md) |
| `NONCORE-QUARANTINE` | `active` | `Overall roadmap — keep only portable/cross-variant code (non-core quarantine)` | **Relocation DONE 2026-06-19** (`.1`–`.4`): all 36 non-core `.pm` + 13 `.plg` `git mv`'d to `noncore/` (ledger `noncore/README.md`); 37-subtest legacy-migration block excised from phase0 (996→959); `perl/` is now **core-only**. **`.V` blocker CLEARED 2026-06-22** — the ~173 back-half failures were owned + resolved by `PHASE0-BACKHALF-TRIAGE` (phase0 **960/960 green** + `tools/run_ci_local.sh` EXIT 0). `.V` now `pending`: verification + `.4`/`SPEC-FORMAT-TERSE` blocker clears landed in `PHASE0-BACKHALF-TRIAGE.5.3.2.1`; remaining doc/book/KM sync → `.5.3.2.2`, then `.V` is `done`. `.N` machinery POSTPONE. | [docs/tasks/NONCORE-QUARANTINE.md](docs/tasks/NONCORE-QUARANTINE.md) |
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` triage + `.3`/`.4` engine fixes (ADR `0008`) + cluster `.2` (108 STALE) + `.5.1`/`.5.2`/`.5.4` + `.5.3.1` all **DONE** → **`t/phase0_regression.t` 960/960 GREEN** end-to-end **and** `bash tools/run_ci_local.sh` EXIT 0. Frontier: **`.5.3.2` split 2026-06-22** → `.5.3.2.1` (status & continuity reconciliation — flips `NONCORE-QUARANTINE.V`/`LEGACY-VHDL-RETIRE.4`/`SPEC-FORMAT-TERSE` gates + index + KM + live docs) → `.5.3.2.2` (narrative-doc + book drift sync = the deferred `LEGACY-VHDL-RETIRE.5` body) → `.6` (book `:AND` reconciliation). See [[lispish-corpus-catastrophic-backtracking]]. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
