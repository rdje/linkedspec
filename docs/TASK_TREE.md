...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua root preflight `.9.1.1.2.5.0` is complete: all four PUC Lua/LuaJIT default/POSIX primary legs are exactly 31/65, with one root-owned and 33 cursor-owned failures. Commit `.5.0` cleanly, then implement core `.5.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
