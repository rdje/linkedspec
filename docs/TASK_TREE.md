...
| `FUTURE-PARITY-BACKLOG` | `active` / MCP parent `.10.9` signoff-complete in `.10.9.7.2`; commit pending | `Overall roadmap - future parity backlog` | Focused and canonical MCP-opt-in recomposition close `.10.9.7.2`, `.10.9.7`, and `.10.9` without behavior or authority movement. Public semantic/MCP no-drift `.10.10` follows only after the clean leaf commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
