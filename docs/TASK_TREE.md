...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Perl root-selection preflight `.9.1.1.2.1.0` has complete behavior-free mapping and canonical proof; its clean commit is the only boundary before core `.1.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
