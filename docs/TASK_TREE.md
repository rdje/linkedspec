...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua compiler layer `.3` passes 55/55 on both runtimes; regex/match-state adapter `.4.1` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Ordered effective rules/functions, mode/dependency/payload state, registry-aware ActionIR, typed failures, and exact outward descriptors pass 55/55 on PUC Lua/LuaJIT; regex/match-state adapter `.4.1` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
