...
| `PUBLIC-STATUS-DRIFT-SYNC` | `active` | `Overall roadmap — documentation and book sync` | Created 2026-07-07 to own public status/mdBook drift found during the bootstrap/context pass before edits: `overview/project-status.md` still describes Rust parity as an ongoing follow-on, while `appendix/backend-handoff.md` still names an older Rust oracle corpus count. `.0` is tracking-only; frontier is `.1` to update only the discovered public status/handoff pages against `ROADMAP_V2.md`, `MEMORY.md`, and `rust/linkedspec-runtime/tests/corpus/manifest.json` (93 fixtures). | [docs/tasks/PUBLIC-STATUS-DRIFT-SYNC.md](docs/tasks/PUBLIC-STATUS-DRIFT-SYNC.md) |
