...
| `STAGED-LINKED-PARSING` | `active` | `Overall roadmap — .spec language model / parser composition` | `STAGED-LINKED-PARSING.4` — `.1` adopted staged linked parsing doctrine, `.2` specified future import/composition, and `.3` specified future `parse_job(text_expr, options)` annotations plus source-aware sidecar metadata. Next design parser registry and dynamic next-stage dispatch. | [docs/tasks/STAGED-LINKED-PARSING.md](docs/tasks/STAGED-LINKED-PARSING.md) |
