...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, census `.1.6.0`, 237-name audit `.1.6.1.0`, newline splitting `.1.6.1.1`, `endswitch` termination `.1.6.1.2.1`, and all pure/position repairs are closed. Active `.1.6.1.2.2.3.1` aligns Rust marker aliases/control. Later leaves own remaining control, captures, descriptor/diagnostics/resolution/trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
