...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Director-approved thin MCP parent `.10.9` means one exact contract, five native implementations, and six runtime admissions. Behavior-free architecture leaf `.10.9.0` is complete; exact executable contract `.10.9.1` is next, followed by Perl `.2`, Rust `.3`, Dart `.4`, Julia `.5`, shared Lua `.6`, and recurring six-runtime closeout `.7`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
