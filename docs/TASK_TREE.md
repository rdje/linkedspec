...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart query audit `.10.5.4.0` freezes fresh detached projection as the sole evaluator authority, 19 static digests / 26 neutral boundaries, and a `.1-.4` implementation split. Typed record/source kernel `.10.5.4.1` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
