...
| `FUTURE-PARITY-BACKLOG` | `active` / Perl `.14.7.3.1` focused-signoff-complete from exact clean dormant commit `4a982f21` | `Overall roadmap - future parity backlog` | `.14.7.3.1` advances the unrouted oracle to 120 GREEN/one authority RED through one private exclusive marker, opaque v2 sidecar, strict literal options, and typed direct/ordered-derived provenance; `.14.7.3.2` owns resolution/cache/result/failure policy next after the atomic handoff. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
