...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia carriers `.14.6.5.2` canonical-signoff-complete | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart remain admitted at 4/9/103. Julia `.14.6.5.0-.2` preserve the historical RED, add private authority, and make four routes GREEN at 62/62 while dormant; `.2` also finitely admits mandatory history segment `4992` under ADR `0086` with canonical proof. Admission `.3` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
