...
Current director priority: complete the canonical CONSUMER-REPORT-DELIVERY publication boundary, then pursue upstream LS-004 under RGX-CONSUMER-BUILD-REPORTS.1. Seven original requirements have verified local remedies; remote Git establishes availability. Targeted startup follows ADR0123. SEMULITH/LS-001–003 were
recovered by startup .85 from integration .8.1. Quoted LF repair .83.2.1 is verified; ADR0124 defines the kind/strict contract; compiler repair .45 is closed; document grammar .83.2.2 is verified on six runtimes; native file delivery and independent public-loader admission close .83 under SEXPR-DOCUMENT-INTEGRATION.1-.2; startup .46/.47 are fixed; separately discovered .86.4.3 remains pending behind consumer delivery.
ARCHOGEN/LS-002 complete-document/all-form parsing and ARCHOGEN/LS-003 atom kinds
share the verified ADR0124 delivery under closed startup .83 with SEMULITH/LS-002.
Earlier full-reading prerequisites are superseded
only as a blanket gate; preserve real task dependencies and the separate audit.

## Active Task Trees

| Tree | Status | Roadmap lane | Current frontier | File |
| --- | --- | --- | --- | --- |
| `SEXPR-DOCUMENT-INTEGRATION` | `done` / `closed` | `Native integration / ARCHOGEN and SEMULITH reports` | `.1` delivers native files; `.2` independently admits all six public-loader routes (37 cases/21 groups each), native files and legacy compatibility. Startup .83 closes; continuation belongs to the active startup tree. | [docs/tasks/SEXPR-DOCUMENT-INTEGRATION.md](docs/tasks/SEXPR-DOCUMENT-INTEGRATION.md) |
| `CONSUMER-REPORT-DELIVERY` | `done` / local delivery | `SEMULITH / ARCHOGEN blockers` | .1 verifies current native remedies/migration; .2 reproduces open LS-004 publicly; .3 requires exact canonical landing, clean push and remote read-back. Tested baseline f60a70df3; Git remains publication authority. RGX-CONSUMER-BUILD-REPORTS.1 owns the remaining upstream repair; unrelated startup .86.4.3 stays pending. | [docs/tasks/CONSUMER-REPORT-DELIVERY.md](docs/tasks/CONSUMER-REPORT-DELIVERY.md) |
| `SESSION-STARTUP-READING` | `active` / targeted startup | `Session continuity / repair selection` | ADR0123 targets2–5 minute recovery and thorough task-specific reading. .85 recovers SEMULITH/LS-001–003; .83.2.1 fixes quoted LF across six runtime routes; ADR0124/.83.1 fixes the design; .45 closes compiler rejection; .83.2.2 verifies the document grammar; .83 is closed by verified grammar, native files and independent public-loader admission. Startup .46 fixes UTF-8 diagnostics and .47.1 fixes argument validation; .47.2 preserves source and values; .47.3 closes the parent; .49 preserves regex boundaries; non-slash .86.1 and division .86.2 are verified; Perl .86.4/.86.5 remain open; Contract audit .86.4.2.4 withdraws the unsupported regex-type/precedence premise; supported helper validation .86.4.3 is next. Both rejected candidates remain archived; .87.1/.87.2 own measured helper gaps. LS-003 remedies are verified. Full audit remains separate. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
| `DART-STARTUP-READING` | `active` / reading complete / repairs open | `Session required reading / startup .3.4` | Dart reading closed under ADR0114; all 69 repair nodes retain their owners. Current startup repair is supported Perl helper-pattern validation SESSION-STARTUP-READING.86.4.3 after the .86.4.2.4 regex-type audit under ADR0123. Historical fixtures remain retained without further reading. | [docs/tasks/DART-STARTUP-READING.md](docs/tasks/DART-STARTUP-READING.md) |
| `JULIA-STARTUP-READING` | `active` / reading complete / repairs open | `Session required reading / startup .3.5` | Julia reading closed under ADR0117 at 9824c0971; all 80 repair nodes remain pending. Current startup frontier is SESSION-STARTUP-READING.86.4.2 for Perl multiline regex action segmentation after .86.4.1 diagnosis under ADR0123. | [docs/tasks/JULIA-STARTUP-READING.md](docs/tasks/JULIA-STARTUP-READING.md) |
| `LUA-STARTUP-READING` | `active` / reading complete / repairs open | `Session required reading / startup .3.6` | ADR0119 closes all 51 audited reading children and startup .3.6. All 35 repair roots/145 pending nodes and failed/excluded validation remain intact. Supporting reading and checker correction complete under ADR0120; current frontier is SESSION-STARTUP-READING.86.4.2 under ADR0123. No dependency build or canonical pass claimed. | [docs/tasks/LUA-STARTUP-READING.md](docs/tasks/LUA-STARTUP-READING.md) |
| `SUPPORTING-SOURCE-READING` | `active` / required reading closed; six grammar/literal repair roots remain | `Session required reading / startup .3.7` | .1/startup .3.7 close under ADR0120 after checker .2.6. All158 original sources/174 ranges, three reading commits and explicit historical exclusions reconcile. Nine pending repair nodes remain exact. Current frontier is SESSION-STARTUP-READING.86.4.2 under ADR0123. | [docs/tasks/SUPPORTING-SOURCE-READING.md](docs/tasks/SUPPORTING-SOURCE-READING.md) |
| `CONFORMANCE-SOURCE-READING` | `active` / deferred separate audit | `Source audit` | Retain89/143 groups,109904 fragments/4784266 baseline bytes and133 complete files. Next unread .1.90; ADR0123 removes the blanket implementation prerequisite. Current frontier is SESSION-STARTUP-READING.86.4.2 after document, UTF-8 diagnostic, mutation-validator, outer-source, regex and symbol-call repairs. | [docs/tasks/CONFORMANCE-SOURCE-READING.md](docs/tasks/CONFORMANCE-SOURCE-READING.md) |
...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `done` / .15 approved conformance allowance | `Repository architecture / documentation sustainability` | ADR0122 admits exactly fourteen granted controls; preserves prior source/history/tasks and forecasts remaining98 units. Production156/61 checks and ordinary staged canonical proof govern landing; canonical admission ec10be6b precedes resumed conformance reading. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
...
| `PARSER-AUTHORING-APIS` | `proposed` / named-argument direction approved and parked | `Future parser authoring and semantic tooling` | Four unactivated investigations: libraries/generics, native builders, fileless MCP authoring/debugging and named user-function arguments. `.4` records approved strict named binding, positional rest, written evaluation order and parameter-name compatibility; defaults remain separate. Four design/planning children are owned; current startup reading continues. | [docs/tasks/PARSER-AUTHORING-APIS.md](docs/tasks/PARSER-AUTHORING-APIS.md) |
...
| `RUST-DEPENDENCY-WARNING-ZERO` | `proposed` / non-blocking | `Repository quality / Rust dependency warning output` | Build output reports1870 pgen/26 rgx-core warnings. Pending work uses RGX published interfaces and upstream reports only; no dependency implementation inspection, reconstruction, patches or pin changes. LinkedSpec-owned fixes remain local. | [docs/tasks/RUST-DEPENDENCY-WARNING-ZERO.md](docs/tasks/RUST-DEPENDENCY-WARNING-ZERO.md) |
| `BACKEND-INTEGRATION-GUIDES` | `done` / all five native guides independently verified | `Phase 6 documentation and adoption / native integration` | Common entry, five guides and maintained setup/deployment/error consumers are complete. Final .7 repeats native proof and requires canonical clean commit/push. RGX public integration only; strict Lispish startup .83, wrapper startup .7 and upstream report remain open. Integration 87b35665e is pushed; reading continues under the conformance frontier. | [docs/tasks/BACKEND-INTEGRATION-GUIDES.md](docs/tasks/BACKEND-INTEGRATION-GUIDES.md) |
| `RGX-CONSUMER-BUILD-REPORTS` | `pending` / upstream-owned | `Rust consumer integration` | `.1` tracks misleading bootstrap progress reporting through RGX public-command evidence and upstream response. No dependency implementation inspection or changes. | [docs/tasks/RGX-CONSUMER-BUILD-REPORTS.md](docs/tasks/RGX-CONSUMER-BUILD-REPORTS.md) |
...
- rule-local cursor recurring/public no-drift at 76 files / 8 complete + 0 pending / 60 mutations is closed. At
