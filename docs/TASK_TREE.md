...
syntax contract and backend rollout after a clean pivot. The contract plus Perl, Rust, and Dart implementations
...
emitted/generated execution, and `.16.4` adds Dart typed/native/generated/emitted/CLI execution. Julia `.16.5`
is next; Rust/Dart pre-existing `.contains()` missing-argument outcomes are delegated to helper owner `.5`; Lua
