...
| `README-STABILITY-POLICY` | `active` / routing-pressure implementation `.4.1` signoff-complete | `Repository architecture / documentation sustainability` | Original `.0-.2` adoption remains closed. Audit `.4.0` is committed at `6a5d1dcc`; `.4.1` implements the fenced local policy, strict 20-surface/62-route registry, unconditional resulting-tree checker, stale `test_input/` correction, and 32/32 mutation oracle while preserving README at 105/128 lines and 5,057/6,144 bytes. Definitive signoff passes CLI 66x2 and Phase 0 1,031/1,031 in 694 seconds; atomic commit precedes unchanged `.4.2`, then containment `.0` and transaction `.14.3.1.1`. Project licensing remains independent proposed `.3`. | [docs/tasks/README-STABILITY-POLICY.md](docs/tasks/README-STABILITY-POLICY.md) |
