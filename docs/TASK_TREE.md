...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart Unicode-label prerequisite `.10.5.0.2.0-.4` is closed with exact classifier/native/identity/negative/composed proof and no semantic promotion; after the clean `.4` commit, Dart semantic source-map foundation `.10.5.1` is next. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
