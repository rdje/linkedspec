...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 2/6; Rust preflight `.9.1.4.0` is committed, and `.9.1.4.1` now runs complete core tests before runtime. Typed family/edge normalization `.9.1.4.2` is the next clean frontier. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
