...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; typed source-location neutral artifact `.14.1.1` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. From clean `c2d9eadf` (intended 133/300; no push), `.14.1.1` adds the exact v1 neutral contract/checker and truthful status docs. Executable source proof corrects the preliminary helper classification to 92 canonical projections, seven callable aliases, and two internal contract ids; the contract fixes 3/7/6/3 fixtures, 8+8 transitions, six recursive/four structural cases, 31 diagnostics, 36 mutations, and 14 rollout legs without runtime/schema/syntax behavior. Focused, book, doctrine, and canonical Phase 0 1,031/1,031 proof pass. `.14.1.2` is next after the clean landing. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
