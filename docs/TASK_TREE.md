...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.2` done 2026-07-09 - Dart now bridges the missing shipped-smoke helper/action surfaces; remaining failures are explicit recursion/default-mode/output mismatches, diagnostic `exit_now(...)` branches, and routed PCRE structural constructs. Next frontier `.6.2.4.3` is recursive/default-mode parser-smoke semantics. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
