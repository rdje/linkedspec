...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | ADR `0044` / `.9.1.1.1` ratifies rule-local cursor policy, mode-sensitive bare edges, migration, descriptor/generated-v2, and conformance; implementation `.9.1.2-.9` is pending. Logical recurring gate `.5.2.8` is verified at 7/1 and closing; public no-drift `.5.2.9` follows after the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
