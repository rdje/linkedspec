...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Root selection is closed at 7/0 and cursor admission `.9.1.8` is clean at `a1911ec6`. Duplicate-slot audit `.9.1.8.1.0` is signoff-complete; neutral `.1` waits for its clean commit before activation. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
