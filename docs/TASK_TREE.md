...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.0` done through Julia `.7.3.1`; exact Julia CLI work is now split. Active PNT is `.7.3.2.1` compile/parser/staged trace; `.1.5`, `.1.6`, and `.3` own global convergence before Lua. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.7.3.2.0` done 2026-07-10 - native seam audit splits exact primary CLI work into trace, arguments/IO, execution/JSON, error/routing, and conformance mechanisms. `.7.3.2.1` is active for compile/parser/staged trace. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
is active for compile/parser/staged tracing, followed by argument/IO, execution/JSON, error/routing, and direct
