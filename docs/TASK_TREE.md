...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage and all Perl/Rust/Dart/Julia/Lua/tool migrations `.1-.2.6` are complete. Tool output now freezes 3 Python/12 shell/19 checker owners, validates bytecode/map/book/TAP/oracle storage, and leaves both old roots at zero exact residue. `.3.1` reconciles every frozen source/destination/deletion record. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
