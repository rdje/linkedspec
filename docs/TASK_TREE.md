...
| `TRACE-OBSERVABILITY` | `active` | `Overall roadmap — engine observability / developer experience` | User directive 2026-06-19: discoverable CLI trace control + comprehensive "see everything" trace (function enter/exit, if/switch/case branches). Framework EXISTS (`Trace.pm`: enter/exit/decision/levels/sinks; env/per-call control works). `.1` audit showed trace is not exhaustive; `.2` added discoverable `bin/linkedspec` CLI flags and docs. `.3` split the coverage lane before code; `.3.1` added the reusable generated-handler branch helper contract; `.3.2` wires non-repetition generated handler dispatch decisions; `.3.3` wires repetition generated handler paths; `.3.4` split compile/ActionIR coverage into sub-leaves; `.3.4.1` added RuleIR planning trace decisions; `.3.4.2` added EmitContext owner-bridge and rewrite-orchestration trace decisions; `.3.4.3` added scanner/canonical/diagnostic/rewrite-pipeline trace decisions; `.3.4.4` added compact lowerer trace decisions; `.3.4.5` added MethodLowering trace decisions; `.3.4.6` closed compile/ActionIR trace coverage. Frontier `.3.5` overall closeout, then future backend parity split. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. `TRACE-OBSERVABILITY.3` has since split, `.3.1` through `.3.4.6` have closed; PNT returns to `TRACE-OBSERVABILITY.3.5` unless a new terse leaf is split or another active tree is reprioritized.
...
`RUST-PARITY` unblocked `TOP-RULE-AS-NORMAL.3.2`; that leaf has since closed, `TRACE-OBSERVABILITY.1` has completed its read-only audit, `TRACE-OBSERVABILITY.2` has added CLI trace control, `.3` has split before code, and `.3.1` through `.3.4.6` have closed, so PNT advances to `TRACE-OBSERVABILITY.3.5`.
