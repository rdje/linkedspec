...
| `TRACE-OBSERVABILITY` | `active` (`.5.1` done; `.5.2-.3` pending) | `Overall roadmap — engine observability / developer experience` | Original leaves `.1-.4.5` remain complete. Corrective `.5.1` restores ordinary/traced progressive static-validation equality without changing valid results or trace schemas. `.5.2` next restores gap-aware interpreted/generated `child_dispatch` lifecycle events; `.5.3` then repairs the stale private progressive-authority route assertion left by later contract admission and closes `.5`. Rust does not renew the complete trace-parity claim until `.5.2` closes. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
