...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua native PCRE2 matching `.4.1` passes 60/60 on both runtimes; compiled rule interpreter `.4.2` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Dual-ABI disposable PCRE2 matching, stable seek/consume, captures, Unicode positions, entry/local registers, progress, JSON, and typed failures pass 60/60 on PUC Lua/LuaJIT; compiled rule interpreter `.4.2` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
