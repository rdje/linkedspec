...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia `.19.5` complete, Lua `.19.6.1` next | `Overall roadmap - future parity backlog` | Perl, Rust, Dart, and Julia implement both frozen mutation mechanisms. Julia `.19.5.1-.2` preserve typed nested-write and identity-guarded `map_leaves!` state through native, reconstructed, generated-plan, emitted-module, and CLI routes. Lua and portable/public admission remain pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
