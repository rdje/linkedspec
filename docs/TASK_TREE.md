...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua typed ActionIR contracts `.3.2` pass 46/46 on both runtimes; function registry `.3.3` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Current-name ActionIR contracts, aliases, generic diagnostics, and registry-first seam pass 46/46 on PUC Lua/LuaJIT; concrete function registry `.3.3` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
