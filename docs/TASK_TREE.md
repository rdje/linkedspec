...
| `INTER-MATCH-GAP-CAPTURE` | `active` / `.7.2` canonical-signoff-complete from clean atomic 253 `d8ed944c` | `.spec language evolution / lossless segmentation and source preservation` | Recurring and public no-drift are current at 9/0/63: neutral, Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT pass in unchanged rooted order. Exactly four calls move to shared 250/public Perl 126; recognition is 137/250/58 and final public governance is 6/12/10/29. Legacy markers remain divergent, marks/helpers independent, `@emit_gaps` absent, and format 2, implementation, facade/schema/MCP/CLI/README/capability, storage, and typed source remain unchanged. No-change closeout/handoff `.7.3` is next after atomic 254. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
