...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Rust private admission `.3.5` and parent `.3` signoff-complete from clean atomic 231 `c3326f6d` | `.spec language evolution / lossless segmentation and source preservation` | Perl and Rust run ordinarily and in the rooted route at gap 3/6/56 plus ten Rust admission mutations. Rust's exact nine-role primary/admission path and full signoff are green without generated-plan-v2 or outward movement; parent `.3` closes for intended atomic 232. Four later runtime routes, recurring/public rows, typed composition, generated-plan v2, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
