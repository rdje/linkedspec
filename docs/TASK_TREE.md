...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.5.2` done 2026-07-09 - Dart now has ordered trace levels, config/env controls, structured event primitives, stdout/routed-file/mirror sinks, reset/truncate behavior, and traced runtime entrypoints that preserve parse output. Next frontier `.4.5.3` adds runtime branch/lifecycle/source-boundary trace instrumentation. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
