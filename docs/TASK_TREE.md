...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust `.14.7.4.1` private marker/provenance focused-signoff-complete | `Overall roadmap - future parity backlog` | Rust now lowers the exact private annotation to one inert detached marker with strict literal options and typed direct/ordered-derived provenance across four dormant logical carriers; ordinary/canonical discovery and rollout remain unchanged, and `.14.7.4.2` owns caller-frozen authority after two pre-existing verification defects receive clean-pivot task ownership. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
| `TRACE-OBSERVABILITY` | `active` (`.5.1-.2` done; `.5.3` pending) | `Overall roadmap — engine observability / developer experience` | Original leaves `.1-.4.5` remain complete. Corrective `.5.1` restores ordinary/traced progressive static-validation equality. `.5.2` restores exactly one existing `child_dispatch` / result pair on normal and gap-aware interpreted/generated entry; trace controls pass 11/11 with unchanged results and full Rust trace parity is current again. `.5.3` next repairs the stale private progressive-authority route assertion left by later contract admission and closes `.5`. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
