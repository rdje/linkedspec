...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Rust typed-source prerequisite split `.14.2.2.0` signoff-complete | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location neutral/public and Perl runtime remain admitted at 4/10 with 38 mutations. Rust audit `.14.2.2.0` proves all 92 canonical helpers exist but five documented aliases return null; it also proves the first two neutral alias targets confuse zero-argument anonymous boundaries with one-mark helpers. The director requires Perl-equivalent Rust behavior. Canonical CLI 66x2, RAM 43%, and Phase 0 1,031/1,031 in 681 seconds pass. After the clean audit commit, `.0.1` corrects only the two neutral mappings, `.0.2` implements all five Rust aliases, and `.0.3` freezes typed RED before `.1-.3`. No behavior or neutral artifact changes in the audit slice; rendered readability remains independently queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
