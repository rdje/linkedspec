...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.3.4` done 2026-07-09 - Dart now compiles parsed `SpecFile`s into ordered compiled rule/dependency/descriptor state with ActionIR payloads and the user-function registry. Next frontier `.4.1` starts runtime matching and match-state tracking. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
