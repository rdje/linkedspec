...
| `BACKEND-INTEGRATION-GUIDES` | `active` / Rust complete, Perl/Dart/Julia leaves and Lua setup verified | `Phase 6 documentation and adoption / native integration` | Rust ad290bdb4 pushed. Perl .1.1-.1.2, Dart .3.1-.3.2, Julia .4.1-.4.2 and Lua .5.1 are verified; ADR0121 admits six approved history controls; next Lua .5.2 deployment. Strict Lispish repair stays startup .83. Finish common navigation and independent .7, then conformance .1.35. | [docs/tasks/BACKEND-INTEGRATION-GUIDES.md](docs/tasks/BACKEND-INTEGRATION-GUIDES.md) |
