...
| `REPO-ROOT-PATH-PORTABILITY` | `active` | `Repository architecture / checkout relocation invariance` | Rust runtime repair `.1.1` is complete and its copied-binary relocation proof is green; current leaf `.1.2` owns only the eight frozen legacy machine-bound source/config paths. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
