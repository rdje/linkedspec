...
| `FUTURE-PARITY-BACKLOG` | `active` / callable-codeblock Rust parent `.11.4`; dynamic invocation leaf `.11.4.2` signoff-complete | `Overall roadmap - future parity backlog` | Inert construction `.11.4.1` is clean at `91958022`. `.11.4.2` exact bound resolution, caller stores, copied/restored fixed/rest bindings, local results/chaining/discard, typed failures/recursion, and native/serialized/generated/emitted identity pass focused, complete Rust, canonical, book, Knowledge Map, doctrine, and cleanup proof. Commit/clean handoff precedes generic contextual final-block `.11.4.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
