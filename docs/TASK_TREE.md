...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `done` / `.4` composition-closeout signoff-complete | `Repository architecture / documentation sustainability` | `.0-.3` landed through atomic 179/300. `.4` independently recomposes history 34/34 across 3 surfaces / 21 segments, task metadata 26/26 over 510 IDs, rollover 12/12, all eleven transferred consumers, outside-CWD retrieval, and unchanged normal routing at 20 surfaces / 62 routes / 32/32. Corrected canonical proof passes Rust semantic 85.71s, Julia semantic 416/29.9s, CLI 66x2, and Phase 0 1,031/673s; atomic 180/300 commit remains. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
...
| `FUTURE-PARITY-BACKLOG` | `active` / behavior-free lossless-gap handoff `.14.5.0` signoff-complete from clean `d26e4d4e`; intended atomic 217 | `Overall roadmap - future parity backlog` | Recursive observation remains closed at typed-source 9/5/114. `.14.5.0` audits numeric slot identity, named-surface absence, historical Perl gaps, marker divergence, and both recurring gates without changing behavior, rollout, or public claims. It freezes `INTER-MATCH-GAP-CAPTURE.1-.7` as the next clean tree and sole named-slot/gap implementation owner; `.14.5.1` resumes typed `gap_composition` only after that tree closes. Brackets remain selectors and dot remains fluent rule behavior. Canonical proof passes containment/relocation, CLI 66x2, and Phase 0 1,031/1,031; land atomic 217 cleanly before pivot. Final typed-source `.14.8`, task metadata `.5`, and rendered readability remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
| `INTER-MATCH-GAP-CAPTURE` | `proposed` / clean pivot selected after `FUTURE-PARITY-BACKLOG.14.5.0` | `.spec language evolution / lossless segmentation and source preservation` | `.0` is done and all prerequisites are satisfied. `.1-.7` are the sole next owner of named-slot grammar, `@capture_gaps` lifecycle/compatibility, prefix/interstitial/tail and empty spans, failure/commit/recursion, six-runtime implementations, carriers, and public admission. Numeric brackets remain positional compatibility, named brackets provide stable identity, and dot is not a selector alias. Typed-source composition follows only after `.7`. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
