...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, opaque source/outcome `.10.7.2`, and private static projection `.10.7.3` are composition-closed. Calls/staging/generated audit `.10.7.4.0` is complete at exact 6/6 -> 18/16 -> 22/25 ownership; typed core `.10.7.4.1` is next. Semantic governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
