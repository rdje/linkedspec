...
| `FUTURE-PARITY-BACKLOG` | `active` / callable-codeblock Rust parent `.11.4`; `.11.4.1` signoff-complete | `Overall roadmap - future parity backlog` | From clean semantic closeout `6cadd6a0`, `.11.4.1` completes inert typed `{|params| body }` construction, exact spans/signature/body validation, compiled/serialized/descriptor/emitted-source round-trip, and full Rust/canonical proof. It executes no literal body or variable call. Dynamic invocation `.11.4.2` is next only after the clean `.11.4.1` commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
Index note 2026-07-30: from clean semantic closeout `6cadd6a0`, `FUTURE-PARITY-BACKLOG.11.4.1` completes inert
Rust typed callable-codeblock construction/state and emission with focused, full Rust, and canonical proof. Exact
dynamic caller-context invocation remains `.11.4.2`, while attached/parenthesized generic final-block closeout
remains `.11.4.3`; neither may activate before the `.11.4.1` clean commit.
