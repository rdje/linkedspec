...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` (**current focus**) | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). Implementation gate CLEARED 2026-06-22 (`t/phase0_regression.t` green + `tools/run_ci_local.sh` EXIT 0; gradual-alias migration). **`.1.1` DONE 2026-06-24** (auto-existing wrapper-referenced working vars on Perl + Rust). **`.1.2` SPLIT** by inference channel; **`.1.2.1`+`.1.2.2` DONE 2026-06-24 — Channel 1 complete on BOTH variants** (Channel 2 deferred behind `.1.5`). **`.1.4` DONE 2026-06-29** (helper renames closed on both variants; Perl reference + Rust parity, phase0 **975 green**). **`.1.3` DONE 2026-06-29** — mutation surface closed by mechanism: scalar `set(name,val)` already satisfied by `.1.4`; `push(target,value)` explicit append with child-call precedence; `set_key(name,key,value)` statement mutation while value form stays pure; operator family closed with scalar `name = value`, array `items += value`, and hash `name[key] = value` for explicit key/value expressions. **`.1.5` SPLIT 2026-06-29** — audit showed literal parity, call spacing, separator semantics, and direct nested access are separate leaves. **`.1.5.2` DONE 2026-06-29** — primitive literals are typed values on Perl/Rust, with JSON booleans and Rust statement-form `if(false)` gating. **`.1.5.3` DONE 2026-06-29** — call spacing locked: optional whitespace before `(` works at supported helper/value sites, while no-parenthesis helpers remain out of scope. **`.1.5.4` DONE 2026-06-29** — statement separators locked: newline-or-semicolon, same-line multiple statements require `;`, nested semicolons protected. **`.1.5.5` SPLIT 2026-06-29** — direct nested access divided into explicit-segment implementation and bare-segment/Channel-2 coordination. **`.1.5.5.1` DONE 2026-06-29** — explicit direct access landed (`foo["a"][9]["b"][scalar(z)]`), with bare path atoms later handled by Channel 2. **`.1.5.5.2` SUPERSEDED/MERGED 2026-06-29** — bare direct-access atoms belong to global Channel 2, not a direct-access-local rule. **`.1.2.3` SPLIT 2026-06-29** — Channel 2 value reads divided by aggregate/scalar surfaces. **`.1.2.3.1` DONE 2026-06-29** — Perl aggregate bare value reads auto-exist safely (`array_copy`/`copy` -> `@`, `hash_copy` -> `%`; phase0 984 green). **`.1.2.3.2` DONE 2026-06-29** — Rust aggregate bare value reads now match Perl via oracle + integration locks. **`.1.2.3.3` SPLIT 2026-06-29** — Perl scalar bare reads divided into source slots, mutation key/RHS slots, and direct path atoms. **`.1.2.3.3.1` DONE 2026-06-29** — Perl scalar source-slot bare reads landed (`return(NAME)`, `set(out, NAME)`, `out = NAME`; phase0 985 green). **`.1.2.3.3.2` DONE 2026-06-29** — Perl mutation-slot bare reads landed (`items += value`, `set_key(meta,key,value)`, `meta[key] = value`; phase0 986 green). **`.1.2.3.3.3` DONE 2026-06-29** — Perl direct-access bare path atoms landed (`foo["a"][z]` -> `$foo->{"a"}->[$z]`; phase0 987 green). **`.1.2.3.4` DONE 2026-06-29** — Rust scalar bare-read parity landed via parser/runtime/oracle locks; corpus 28 fixtures. **`.1.2.3.5` SPLIT 2026-06-29** — RHS-shape/type inference split by mechanism with no engine/book behavior change. **`.1.2.3.5.1` DONE 2026-06-29** — Perl shape-literal value expressions landed (`[value]`, `{ key => value }`; phase0 **988 green**, mdBook updated). **`.1.2.3.5.2` DONE 2026-06-29** — Perl direct RHS shapes now infer aggregate bare targets (`items = [value]` -> `@items`, `meta = { key => value }` -> `%meta`); explicit `scalar(...)` remains scalar payload assignment. **`.1.2.3.5.3` DONE 2026-06-29** — Rust shape-literal value parity landed (`[]`, `[value]`, `{ key => value }`) via parser/runtime/oracle locks; corpus 30 fixtures. **`.1.2.3.5.4` DONE 2026-06-29** — Rust RHS target-kind inference parity landed (`items = [value]` -> array working var, `meta = { key => value }` -> hash working var; explicit `scalar(...)` remains scalar payload assignment); corpus 32 fixtures. **`.1.6` DONE 2026-06-29** — array end-mutation methods landed (`push_back`, `push_front`, `pop_back`, `pop_front`); phase0 **990 green**, corpus 33 fixtures, Round 1 closed. **`.2.1.1` DONE/SPLIT 2026-06-29** — expression-valued blocks ground truth split by reference/parity/early-return semantics; **`.2.1.2` DONE 2026-06-29** — Perl-reference core block values landed; **`.2.1.3` DONE 2026-06-29** — Rust parser/runtime parity for core block values landed; corpus **34 fixtures**. **`.2.1.4` DONE 2026-06-30** — block-local early return landed on Perl/Rust; corpus **35 fixtures**. **`.2.2.1` DONE/SPLIT 2026-06-30** — control-flow keyword surface split. **`.2.2.2` DONE 2026-06-30** — Perl attached-block if landed. **`.2.2.3` DONE 2026-06-30** — Rust attached-block if parity landed; attached if is now portable on Perl/Rust; corpus **36 fixtures**. **`.2.2.4` DONE 2026-06-30** — `when/otherwise` aliases now normalize to portable attached `if/else` on Perl/Rust; corpus **37 fixtures**. **`.2.2.5` SPLIT/OWNED 2026-06-30** — attached `switch/case/default` split into `.2.2.5.1` Perl separator/source lock and `.2.2.5.2` Rust parity. **`.2.2.5.1` DONE 2026-06-30** — Perl compact attached switch branches now lower with no host residue and phase0/local CI pass. **`.2.2.5.2` DONE 2026-06-30** — Rust attached switch parser/runtime parity landed; corpus **38 fixtures**. **`.2.2.6` SPLIT/OWNED 2026-06-30** — attached `while(cond) { ... }` split into `.2.2.6.1` Perl reference loop/safety and `.2.2.6.2` Rust parity; **Frontier → `.2.2.6.1`**. | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
