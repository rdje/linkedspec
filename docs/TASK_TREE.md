...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.3.3` after the Julia ActionIR contract resolver leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.3.2` done 2026-07-10 - Julia now resolves typed ActionIR calls, receiver methods, structural assignments, controls, nested arguments, block values, shapes, access expressions, and raw fallback nodes into canonical helper/control contract and diagnostic records. Active frontier advances to `.3.3` for the user-function registry and staged function-body parse-job records. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.3.2` and now sits at `JULIA-BACKEND-PARITY.3.3`. Generated Dart source remains a future
split proof lane.
