...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4` after the middle non-function corpus passed 25/25. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.3` done 2026-07-10 - non-function manifest windows 40–56, 58–59, and 62–67 pass 25/25 unchanged, with exact top-level function routes and a permanent regression. Full tests pass with 757 assertions and status `runtime-corpus-middle`; `.6.2.4` shipped-spec/parser-smoke fixtures 68–98 are active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.3` have since landed; `.6.2.4` is active.
...
`runtime-corpus-middle`; `.6.2.4` shipped-spec/parser-smoke fixtures 68–98 are active.
