...
| `INTER-MATCH-GAP-CAPTURE` | `active` / `.1.1` neutral artifact/checker complete as atomic 219/300 | `.spec language evolution / lossless segmentation and source preservation` | `.1.0` landed the exact future v1 plan at `31f3e664`. `.1.1` passes exact absent-artifact RED then executable-neutral GREEN: 8+10 authored fixtures, 3 sources, 16 transitions, 10 segmentations, 9 diagnostics, 50 rejected mutations, rollout 1 complete + 8 pending, and full canonical signoff. No named-slot, directive, accessor, parser/runtime, README, or public behavior is current. `.1.2` follows only from the clean atomic-219 boundary; `.1.3` closes neutral governance; `.2-.7` own six runtimes and public admission. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
