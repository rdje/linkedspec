...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.4.2` done 2026-07-09 - Dart `call(...)` refreshes `retv`, scalar-held array appends are visible, all five hlink fixtures plus `tablegrep_simple_term` pass, and the parser-smoke window is 19/31 green. Next frontier `.6.2.4.4.3` is helper mutation/text normalization. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
