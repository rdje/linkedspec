...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5` and exhaustive current-surface `.1.6.1` are closed: corrected 239-name coverage and the 105-case corpus pass unchanged on Perl/Rust/Dart/Julia, with the strict checker recurring in local CI. Active `.1.6.2` adds Rust's outward compiled-descriptor projection; later leaves own diagnostics/resolution/trace, and generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
