...
| `REPO-ROOT-PATH-PORTABILITY` | `active` | `Repository architecture / checkout relocation invariance` | Audit `.0` completed ADR `0052` plus exact inventories/split without behavior change; current leaf `.1.1` replaces only Rust compile-time checkout discovery and turns the copied-binary relocation RED green. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
