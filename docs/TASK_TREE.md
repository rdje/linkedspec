...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` (**current focus**) | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). Implementation gate CLEARED 2026-06-22 (`t/phase0_regression.t` green + `tools/run_ci_local.sh` EXIT 0; gradual-alias migration). **`.1.1` DONE 2026-06-24** (auto-existing wrapper-referenced working vars on Perl + Rust). **`.1.2` SPLIT** by inference channel; **`.1.2.1`+`.1.2.2` DONE 2026-06-24 — Channel 1 complete on BOTH variants** (bare arg-position targets auto-exist; Rust `resolve_scalar_target`/`resolve_array_target` parity; Channel 2 deferred behind `.1.5`). **`.1.4` DONE 2026-06-29** (helper renames closed on both variants): `.1.4.1` Perl reference recognized `set`/`cat`/`copy` at every canonical-name site, all 20 specs byte-identical, +4 phase0 locks → **975 green**, book taught 3 pages; `.1.4.2` Rust `Engine::call_helper()` parity recognizes `"assign" \| "set"`, `"concat" \| "cat"`, and unified `"copy"` with 2 oracle fixtures + 3 integration locks, cargo/clippy/phase0/full gate green, no book change. **Frontier → `.1.3`** (mutation surface — function + operator spellings; scope/split before code). | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
