...
| `INTER-MATCH-GAP-CAPTURE` | `active` / `.7.0` canonical-signoff-complete from clean atomic 251 `c33f6664`; `.7.1` next | `.spec language evolution / lossless segmentation and source preservation` | Neutral plus Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT pass ordinarily and in rooted order at gap 7/2/60. `.7.0` verifies that exact boundary, corrects stale current Lua-skip prose, and freezes recurring `.7.1` at 8/1/61; public language/compatibility/no-drift `.7.2` at shared calls 250, public Perl 126, and gap 9/0/63 plus public 6/12/10/29; then no-change closeout/handoff `.7.3`. Focused/canonical proof passes without grammar/runtime/rollout/outward movement. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
