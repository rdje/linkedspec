...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.5.1` done 2026-07-09 - Dart now has the minimal staged registry provider for function-body parse jobs: deterministic `actionir-body.spec` resolution, `action_block` compile/cache records, stable queue execution, and `body_ast` stitching. Next frontier `.5.2` implements user-function runtime execution. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
