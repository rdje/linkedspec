...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua core four-kind values/access/capture reads pass 69/69 on both runtimes; scalar/string `.4.3.2` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Four-kind stores/snapshots, checked mixed access/assignment, current-edge `retv`, and complete entry/match reads pass 69/69 on PUC Lua/LuaJIT; scalar/string `.4.3.2` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
