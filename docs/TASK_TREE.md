...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia exact identity `.10.6.1.2` is complete from clean `732eb1e1`: ten unique labels pass 130 new/focused 1,885, Julia 5,596/primary/105, 5x2x66, ten-leg manifest, canonical Rust 80.21s + Dart 1/1 + primary 66x2 + Phase 0 1,031/645s, and 1.57-GB cleanup without production/governance change. Negative/isolation `.10.6.1.3` is next after the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
