...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.3.0` done 2026-07-09 - broad Dart runtime helper/value work is split into core value/store/capture helpers, string/number helpers, array helpers, hash helpers, value-block/control/tree traversal helpers, and no-drift closeout. Next frontier `.4.3.1` starts core value/store behavior and capture helper reads. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
