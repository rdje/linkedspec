...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / reading `.3.2.12` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; sixteen Perl files plus EmitContext prefix read; `.3.2.13` reads its suffix next. `.12` owns confirmed bare/explicit execution-order drift. Codebase/book No; `rgx` excluded. Repairs `.7`–`.12` follow reading before mutation setup. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
