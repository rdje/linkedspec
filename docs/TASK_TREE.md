...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, source/outcome `.10.7.2`, private static `.10.7.3`, calls/staging/generated `.10.7.4`, and immutable typed/raw-neutral query `.10.7.5` are composition-closed. Native capture `.10.7.6.1` and immutable observed-index derivation `.10.7.6.2` are complete at focused 1,884 per ABI with full signoff; generated/emitted propagation `.10.7.6.3` follows only after the clean commit. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
