...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua core offsets 0-39 are permanently 40/40 at endpoint 1/1 through `.6.1.3`, with 161/161 focused tests per ABI; capability/no-drift `.6.1.4` is active. Diagnostic drift `.5.1` and logical truthiness/Perl-lowering drift `.5.2` are dependency-gated; ADR `0036`/`.19` remains post-parity. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | `.6.1.3` permanently locks exact ordered core offsets 0-39, all wrapped outputs, and endpoint 1/1 at 161/161 per ABI; governed capability/no-drift `.6.1.4` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
production source, corpus data, public status, CLI, capability row, or offsets 40-104 changed; `.6.1.4` is active.
