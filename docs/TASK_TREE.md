...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart-native label primitives `.10.5.0.2.0`, parser/validator consumption `.1`, exact downstream identity `.2`, and negative/identifier isolation `.3` are signoff-complete; composed signoff `.4` is next after the clean `.3` commit. Semantic governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
