...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.1.6.1` adds exact neutral `bytes_hex` files; `.1.5.1.6.2` is active for Perl strict decoding and valid/invalid fixtures before closeout/Rust. `.10.1` parks semantic introspection/MCP; `.11.1` parks the generic final-codeblock argument correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
and a thin MCP transport that owns no semantic behavior. The backend frontier is now `.1.5.1.6.1`.
