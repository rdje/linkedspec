...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Root-selection `.9.1.1.2.0` has complete ADR `0046`, neutral 8/3/3-case, 5-backend, 24-mutation, mdBook, and canonical proof at 1/7; its clean commit is the only boundary before Perl `.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
