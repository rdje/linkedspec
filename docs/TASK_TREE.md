...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua transform/join/PCRE2 pipelines `.4.3.4.3` are closed at 95/95; mutation/child flow `.4.3.4.4` is active. Flat/concat arity, negative counts, dropped-transform rebinding, and invalid-join drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Copied array transform/join/PCRE2 pipelines and seven standalone rebindings pass 95/95 on both ABIs; mutation/child flow `.4.3.4.4` is active. Later tagged/closeout and variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
