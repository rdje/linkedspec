...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8.4` — `.8.3.5` closed the non-repetition generated-family matrix; next emit repetition handler families and termination guards | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8.4` unless a new terse leaf is split.
...
work starts. Current frontier advances to `.8.4` for repetition handler families and termination guards.
