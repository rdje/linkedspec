...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart typed calls core `.10.5.3.1` deep-equals the neutral 18/16 non-staged subset with exact functions/helpers/bindings/source/resolution/shapes and private isolation. Staged/generated exact 22/25 completion `.10.5.3.2` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
