...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, census `.1.6.0`, 237-name audit `.1.6.1.0`, newline splitting `.1.6.1.1`, and `endswitch` termination `.1.6.1.2.1` are closed. Diagnostic `.2.2.0` exposes 100/105 and splits four semantic families; Rust pure `.2.2.1.1` is closed, active `.2.2.1.2` aligns Dart. Later leaves own descriptor/diagnostics/resolution/trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
