...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 3/5 at 68 governed files; Rust `.9.1.4` closes at `288da21a`. Dart `.9.1.5.2` is committed at `7dea1f6b`; `.9.1.5.3` cursor descriptor v1 is complete with full signoff and awaits its clean commit before generated v2 `.9.1.5.4`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
