...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.1.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.1.1` pending 2026-07-09 - verify Julia toolchain availability and define the repository-owned Julia package, test, corpus-runner, and variant-specific CLI layout before source implementation. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
has since completed in `FUTURE-PARITY-BACKLOG.1.2`; the active backend frontier is
`JULIA-BACKEND-PARITY.1.1`. Generated Dart source remains a future split proof lane.
...
LinkedSpec CLI in package planning, and deferred generated Julia source to a later proof decision. The next active
frontier is `JULIA-BACKEND-PARITY.1.1` for Julia toolchain/package-layout preflight.
