...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart staged/generated completion `.10.5.3.2` deep-equals the complete private 22/25 calls projection with distinct payload/job/result provenance, selected generated-plan identity, and isolation. Composed calls closeout `.10.5.3.3` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
