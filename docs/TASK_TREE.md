...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Perl authored/static `.2.1` signoff-complete from clean `8f826923`; atomic 223 ready | `.spec language evolution / lossless segmentation and source preservation` | Permanent grammar/reference parsing, private Unicode-17 identity, named selector execution, exact diagnostics, five-field edge metadata, and generated dependency provenance are implemented. The 108-assertion dormant consumer and 10 no-admission mutations pass. Canonical signoff passes all eight doctrines, containment/relocation, CLI 66x2, RAM 65%, Phase 0 1,031/1,031 in 755 seconds, and the exact opt-in route. Live gap state/accessors/generated gap execution remain `.2.2-.2.3`; `.2.4` alone admits Perl. Rollout stays 1 complete + 8 pending / 55 neutral mutations; recognition stays 133/246/58, public helpers 122, typed source 9/5/114, and outward surfaces remain unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
