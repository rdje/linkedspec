...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, exhaustive current surface `.1.6.1`, and exact compiled descriptors `.1.6.2` are closed. Rust diagnostic audit `.1.6.3.0` found the real string-result engine boundary; typed implementation `.1.6.3.1` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
