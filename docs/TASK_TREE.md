...
| `FUTURE-PARITY-BACKLOG` | `active` / `.14.3.1.1` restored after documentation containment | `Overall roadmap - future parity backlog` | Typed-source value/helper parent `.14.2` is composition-closed. Behavior-free `.14.3.0` audits current cursor/mark/recursion seams; committed `.14.3.1.0` ratifies exact future recognition transactions, falsey-safe staged results, a closed 9-allowed/11-rejected effect lattice, linear tokens, invocation-framed marks, and cursor-only v1 progress without executable/current-public behavior. Executable neutral authority `.14.3.1.1` resumes after the clean README-policy and bounded-document containment priorities. `TASK-TREE-METADATA-HYGIENE.5` separately owns the startup-discovered active-frontier prose gap; rendered readability remains queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
