...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia's five-target private static parent `.10.6.3` is a verified no-change closure candidate through `.3.3` from clean base `556e5ae2`. Behavior-free calls/staging/generated plan `.10.6.4.0` is next only after the closeout commit is clean. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
