...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart strict MCP stdio `.10.9.4.2` is signoff-complete from clean decoded-server commit `4a044914`; its clean commit is the only remaining step before exact Dart admission `.10.9.4.3` activates. The ledger remains 2/5 implementations + 2/6 runtimes, rollout pending, with 46 mutations rejected. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
