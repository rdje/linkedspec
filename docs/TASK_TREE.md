...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia generic contextual final-codeblock equivalence `.11.6.3` signoff-complete; parent `.11.6` closed from clean `bc85c0fa` | `Overall roadmap - future parity backlog` | Julia `.11.6.1-.3` now cover inert construction, dynamic invocation, and exact final-only metadata plus one governed attached/parenthesized helper/user/receiver/tree normalization across native/reconstructed/generated/emitted roles. Focused, Julia-local, documentation, governance, and canonical proof pass; the workflow commit/clean boundary remains before four-backend no-drift/Lua routing `.11.7` activates. No capability/MCP movement or push. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
