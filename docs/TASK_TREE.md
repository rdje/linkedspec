...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua block/control/callback work is mechanism-split under `.4.3.6`; expression blocks `.4.3.6.1` are active, user-function block dispatch stays `.5.1`, and first-class callable values stay future `.11.7`. Rename collision drift remains helper caveat `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array and ordinary harray families are closed at 99/99 and 103/103. `.4.3.6.0` splits eager blocks, controls, contextual built-ins/`with`, and tree callbacks; expression-valued blocks `.4.3.6.1` are active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
