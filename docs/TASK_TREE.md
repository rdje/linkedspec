...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust progressive dormant RED `.14.6.3.0` complete; private authority `.14.6.3.1` next | `Overall roadmap - future parity backlog` | Rust's exact four-carrier outer-cfg consumer fails only on the missing dedicated node, stays absent from ordinary/canonical discovery, and freezes authority/carrier/admission in `.14.6.3.1-.3` without moving progressive 2/9/86. Implement `.14.6.3.1` next; Dart/Julia/Lua `.14.6.4-.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
