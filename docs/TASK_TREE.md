...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart runtime-observation parent `.10.5.5` is composition-closed across typed capture, immutable twentieth-digest derivation, and every generated/emitted/traced route with exact non-interference and unchanged v2/format 2. Exact composed Dart admission `.10.5.6` is active; governance remains 6/20/73 at rollout 3/9 and admission 2/6 until that consumer passes. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
