...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.6` after public-parser leading trivia moved shipped smoke to 31/31. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.5.3` done 2026-07-10 - public-parser leading blank/comment skipping closes history without weakening indexed reads. Full tests pass with 810 assertions and status `runtime-corpus-leading-trivia`; shipped smoke is 31/31 and `.6.2.4.6` is active for no-drift. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.5.3` have since landed; `.6.2.4.6` is active.
...
`runtime-corpus-exit-now` at that boundary; `.6.2.4.5.2` and `.5.3` have since closed, and `.6.2.4.6` is active.
...
`runtime-corpus-leading-trivia`, and `.6.2.4.6` is active for final no-drift.
