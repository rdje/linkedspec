...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1` and opaque source/outcome foundation `.10.7.2` are composition-closed. `.10.7.3.1` implements exact private graph 12/14/7; behavior-free ceiling-boundary correction `.10.7.3.2.0` is done and remaining static/isolation `.10.7.3.2.1` is active, with semantic governance unchanged at 6/20/89, rollout 5/9, and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage and all backend/tool migrations `.1-.2.6` are complete. Reconciliation `.3.1.1` confirms frozen off-repository residue zero, deletes one missed exact same-SSD target-era root after canonical cache/use proof, and reruns all six storage oracles. It also exposes a marker-v1 live-descendant deletion RED; remediation `.3.1.2` is the next frontier before final residue `.3.2`. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
| `REPO-ROOT-PATH-PORTABILITY` | `active` / paused behind SSD storage migration | `Repository architecture / checkout relocation invariance` | Remediation `.1.1-.1.3` and structural doctrine `.2.1` are complete. Recurring relocated-process oracle `.2.2` resumes after `PROJECT-DATA-SSD-ROOTING.5` so it proves the final repo-filesystem storage environment. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
