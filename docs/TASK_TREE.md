...
| `TASK-TREE-METADATA-HYGIENE` | `active` | `Overall roadmap — durable architecture / task-tree hygiene` | Created 2026-07-07 to own the user-requested audit of non-closed task trees and stale per-file task metadata. **`.0` DONE** (tracking-only): central live non-closed trees are `SPEC-FORMAT-TERSE`, `STAGED-LINKED-PARSING`, `DOCTRINE-ENFORCEMENT-ADOPT`, `SPEC-LANG-REFERENCE`, and `ROADMAP-DRIFT-RECONCILE`, with their frontiers empty/deferred/paused at `.0` creation time. The user then reactivated `SPEC-FORMAT-TERSE.14`; that lane is closed by `.14.5`. **`.1` DONE** top-level metadata reconciliation; **`.2` DONE** stale frontier/verification/commit row reconciliation. The next PNT-eligible hygiene leaf is **`.3`** doctrine/check decision. | [docs/tasks/TASK-TREE-METADATA-HYGIENE.md](docs/tasks/TASK-TREE-METADATA-HYGIENE.md) |
