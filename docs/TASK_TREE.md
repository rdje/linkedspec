...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `active` / descriptor leaf `.0` signoff-complete pending atomic 176/300 | `Repository architecture / documentation sustainability` | Clean `0bcb5a36` activates `.0`. Tool-assisted audit proves seven live-status capability families and six exact future-task machine consumers prevent blind truncation/splitting. ADR `0066` freezes strict root-relative manifests, byte-exact Git reconstruction, a 256-line live view, 512-line changes/notes shards, seven semantic future parts plus immutable legacy history, executable lookup/freshness gates, atomic rollback, and `.1-.4` order without moving content. `.1` follows only after `.0` commits and proves clean. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
