...
| `SPEC-LANG-REFERENCE` | `active` | `Overall roadmap — documentation and book sync` | **ACTIVE 2026-07-08** — `.10.5.7` fixed `user-model/regex-in-spec.md`: regex examples now use no-regex `Top::` wrappers plus single-colon regex-bearing rules, while numbered/named capture and compaction teaching remains verified. Next leaf: `.10.5.8`, auditing/fixing `user-model/blind-calls-and-parser-orchestration.md`. | [docs/tasks/SPEC-LANG-REFERENCE.md](docs/tasks/SPEC-LANG-REFERENCE.md) |
