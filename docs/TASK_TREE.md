...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | The final five-backend recurring/public no-drift `.9.1.1.2.6` and parent `.9.1.1.2` are signoff-complete; the clean per-leaf commit is the only boundary before selecting the next roadmap leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
