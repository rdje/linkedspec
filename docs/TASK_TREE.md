...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common/backend/tool migration and reconciliation `.1-.3.1` are complete. Marker-v2 process-group lifecycle closes the live-descendant deletion RED with group drain/signals, conservative recovery, all six storage oracles, and canonical proof. Final independent off-repository residue proof `.3.2` is active. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
