...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / Rust .3.3.67 complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; full codebase No; physical mdBook Yes, formal .4 pending. Perl/Rust complete. Dart .1.35 closes 35/55 children, 50,528 fragments / 1,551,233 bytes; validation/package/action-parser tests complete, contract tests through 203. All 35 tests and gap/duplicate checks pass. Ten reconstructed controls own .2.23: two null-name wrong-slot acceptances, six valid/rejecting controls and two census inputs. Earlier owners/gates and capacity 9c644ecb remain; next .1.36 while .3.4 stays pending. rgx reading exclusion remains. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
| `DART-STARTUP-READING` | `active` / .1.35 complete | `Session required reading / startup .3.4` | Reading is 35/55 children, 50,528 fragments / 1,551,233 bytes; validation/package/action-parser tests complete, contract tests through 203. All 35 tests and neutral gap/duplicate checks pass. Ten reconstructed controls own null named-selector .2.23 with two children; two wrong-slot acceptances, six valid/rejecting controls and two census inputs. Prior owners retain evidence. Next .1.36 follows the focused clean commit; repairs remain gated. | [docs/tasks/DART-STARTUP-READING.md](docs/tasks/DART-STARTUP-READING.md) |
...
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `done` / .9 engineering-history capacity | `Repository architecture / documentation sustainability` | Committed canonically at 9c644ecb under ADR0111: approved files 26→27 and manifest lines 25→26; exact 207-line rollover, prior-history reconstruction and 22 validator executions pass. All nine doctrines, both CLI environments 66/66 and Phase 0 1,032/1,032 pass; the receipt was promoted and clean handoff returned to Dart reading. Other limits and startup gates remain. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
