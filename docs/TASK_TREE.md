...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia Unicode core `.10.6.1.1` is complete from clean `dfcc1ca7`: generated/checker-owned 806-range classification, all five parser routes, four-role validation, focused 1,755, Julia 5,466/primary/105, 5x2x66, ten-leg manifest, and canonical Rust 79.93s + Dart 1/1 + primary 66x2 + Phase 0 1,031/663s pass. Exact identity `.10.6.1.2` is next after the clean commit; governance stays 6/20/81 at rollout 4/9 and admission 3/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
