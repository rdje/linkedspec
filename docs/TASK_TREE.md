...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage `.1` and Perl `.2.1` are complete. Rust `.2.2` now supplies a complete 195-package offline Cargo cache and proves 17 temporary owners, generated projects, traces, and copied-binary execution stay on repository storage; exact old Rust temp residue is zero. `.2.3` is the clean Dart storage frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
