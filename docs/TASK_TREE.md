...
| `PERL-ACTIONIR-AST-MIGRATION` | `active` (**current focus**) | `Overall roadmap — compiler architecture / variant contract` | User directive 2026-07-01: Perl must move from ActionIR text-to-text lowering to Rust-style text-to-AST, and every backend must use typed AST/IR before lowering/execution. **`.0` DONE** — ADR `0011`, mdBook backend-handoff/pipeline/formal/architecture updates, KM card, and live docs adopt the doctrine before code. **`.1` DONE** — inventoried the raw-string boundaries in `StatementSplit`, `MethodExpr`, scanners, contracts, canonical events, `RewritePipeline`, `MethodLowering`, and `RuleIR::EmitContext`; locked a Rust-aligned AST node set and replacement order. **`.2` DONE** — added `LinkedSpec::ActionIR::AST` / `AST::Parser` plus focused parser tests behind existing behavior. **`.3` SPLIT** — value/receiver lowering divided into `.3.1` non-call value dispatcher, `.3.2` AST helper-call composition, `.3.3` AST receiver chains, and `.3.4` AST return-payload traversal/diagnostics. **Frontier -> `.3.1`** introduce the AST value-lowering dispatcher for non-call value nodes. | [docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md](docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md) |
