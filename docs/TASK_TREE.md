...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Migration/residue `.1-.3` and structural enforcement `.4.1` are complete. Registered `PROJECT-DATA-STORAGE` scans current tracked storage/command surfaces with 22 classifier cases; six doctrines, 36 hostile-routed boundaries, and all six storage oracles pass. Process-level opened-path proof `.4.2` is active. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
