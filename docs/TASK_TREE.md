...
| `FUTURE-PARITY-BACKLOG` | `active` / typed-source `.14.2.6` signoff-complete; `.14.2.7` next after clean landing | `Overall roadmap - future parity backlog` | Typed-source truth is 8/6/53: one routed driver composes neutral, Perl, Rust, Dart, Julia, PUC Lua, LuaJIT, and three support ledgers. Five backend source groups map to six runtime routes; exact paths, commands, order, multiplicity, support checks, and `LINKEDSPEC_RUN_TYPED_SOURCE_MATRIX` are mutation-locked. The combined recurring/public no-drift row remains pending for `.14.8`; no fifteenth row, behavior/schema/README, hosted workflow, or storage root is added. Full recurrence/routing, book 79/14,204 with separate rendered blocks, Knowledge 790/6,526, all seven doctrines, canonical containment/relocation, CLI 66x2, RAM 78%, and Phase 0 1,031/1,031 in 669 seconds pass. Atomic commit 169/300 is the remaining landing step before `.14.2.7`. Rendered readability stays queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
