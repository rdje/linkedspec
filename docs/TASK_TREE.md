...
| `FUTURE-PARITY-BACKLOG` | `active` / typed recurrence `.14.6.7` canonical-signoff-complete; public `.14.6.8` next | `Overall roadmap - future parity backlog` | One repository-routed driver binds five immutable progressive sources to six ordered runtime routes and all four support ledgers. Typed governance is 12/2/170 after promoting only `progressive_span_dispatch`; behavioral governance remains 7/9/112 and public no-drift `.14.6.8` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
