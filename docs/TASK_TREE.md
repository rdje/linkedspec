...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust authority audit `.10.4.0` is complete from base `90e5e701`: exact owners/gaps, the neutral-Unicode versus published/Rust-ASCII label conflict, stale-toolbox guard leaf, and dependency split are durable with full Rust/canonical proof. `.10.4.0.1` is the next clean-boundary frontier to repair/guard semantic toolbox current state before the director decision at `.10.4.0.2`. Rollout/admission remain 2/9 and 1/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
