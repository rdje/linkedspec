...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact Perl MCP admission/ledger `.10.9.2.3` is complete from clean strict-stdio commit `f09c7c11`: its 12-role consumer and shared ledger/checker pass unchanged 35/10/10/4/4 inventories at 1/5 implementations, 1/6 runtimes, rollout pending, 28 mutations, and full canonical signoff. No-change recomposition `.4` follows only after the clean `.3` commit; ADR `0056` remains pending under `.14.1-.8`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
