...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Dart carrier `.4.3` signoff-complete from clean atomic 236 `0b074e1c` | `.spec language evolution / lossless segmentation and source preservation` | Perl and Rust run ordinarily and in the rooted route at gap 3/6/56 plus ten Rust admission mutations. Dart `.4.1-.4.3` now carry authored/static metadata, private native execution, normalized reconstruction, compatible descriptors, and generated-plan execution behind the dormant final consumer and ten Dart mutations. Independently emitted `.4.4` follows; primary/admission `.4.5`, Julia/Lua, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
