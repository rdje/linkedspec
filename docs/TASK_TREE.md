...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | AND/OR cursor audit `.9.1.0` rejects global override and exposes default drift. Director capture `.9.1.1.0` fixes child-owned modes with no parent propagation; exact contract ratification `.9.1.1.1` is next before Rust logical `.5.2.3` resumes. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
