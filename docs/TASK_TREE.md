...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia `.14.6.5` closed; shared Lua `.14.6.6` next | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart + Julia are admitted at 5/9/106. Julia `.14.6.5.0-.4` preserve the historical RED, private authority, four fresh-authority routes, unchanged ordinary/canonical carrier admission, separate dormant authority matrix, and independent no-change recomposition. Shared Lua `.14.6.6` is next. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
