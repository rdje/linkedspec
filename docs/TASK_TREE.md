...
| `TRACE-OBSERVABILITY` | `active` | `Overall roadmap — engine observability / developer experience` | User directive 2026-06-19: discoverable CLI trace control + comprehensive "see everything" trace (function enter/exit, if/switch/case branches). Framework EXISTS (`Trace.pm`: enter/exit/decision/levels/sinks; env/per-call control works), but `.1` audit shows it is undiscoverable from a CLI and not exhaustive: generated handler branches, most ActionIR owner branches, and Rust trace parity remain gaps. `.1` DONE 2026-07-04; frontier `.2` CLI+docs, then `.3` Perl reference coverage extension, then future backend parity split. | [docs/tasks/TRACE-OBSERVABILITY.md](docs/tasks/TRACE-OBSERVABILITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `TRACE-OBSERVABILITY.2` unless a new terse leaf is split or another active tree is reprioritized.
...
`RUST-PARITY` unblocked `TOP-RULE-AS-NORMAL.3.2`; that leaf has since closed, and `TRACE-OBSERVABILITY.1` has completed its read-only audit, so PNT advances to `TRACE-OBSERVABILITY.2`.
