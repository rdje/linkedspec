...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, source/outcome plan `.10.7.2.0`, and strict source owner `.10.7.2.1` are complete. Staged compiled-or-failed outcome `.10.7.2.2` is active, with semantic governance unchanged at 6/20/89, rollout 5/9, and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
