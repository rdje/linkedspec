...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / reading `.3.2.10` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; fifteen Perl files read; `.3.2.11` reads RuleIR.pm next. Validation and existing edge/capture owners reconcile; `.11.1`–`.11.3` own diagnostic source repairs. Codebase/book No; `rgx` excluded. Repairs `.7`–`.11` follow reading before mutation setup. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
