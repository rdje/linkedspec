...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart recursive observation `.14.4.4` as intended atomic 212 from clean `91a2b05c` | `Overall roadmap - future parity backlog` | Rust `.14.4.3` landed private four-carrier recursive observation at `91a2b05c`. Dart `.14.4.4` solely owns the next admission and is composed-GREEN across dedicated lowering, static/effect policy, existing invocation authority, detached UTF-16-to-scalar projection, four carriers, and ordinary/canonical/storage governance. Typed source is 8/6/73 with Perl, Rust, and Dart on the pending observation row; recognition stays 129+4/246/58. Doctrine/canonical signoff and commit must precede Julia `.14.4.5`. Typed-source `.14.8`, task metadata `.5`, and rendered readability remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
