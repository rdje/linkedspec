...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.6.2.4.4.6` done 2026-07-09 - Dart mirrors the Perl public parser's leading blank/comment-line skip, so `ds_vhistory_version_entry` passes and the shipped-smoke window is 24/31 green with only PCRE structural regex blockers remaining. Next frontier `.6.2.4.5` is final parser-smoke no-drift closeout. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
