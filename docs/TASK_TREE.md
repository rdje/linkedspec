...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, census `.1.6.0`, 237-name audit `.1.6.1.0`, newline splitting `.1.6.1.1`, `endswitch` termination `.1.6.1.2.1`, all pure-value repairs, and Rust/Dart position repairs are closed. Active `.1.6.1.2.2.2.3` aligns Julia empty-local-match positions. Later leaves own marker control, captures, descriptor/diagnostics/resolution/trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | Local proof is 1,020 assertions/nine processes/99 fixtures; Julia passes 61/61 default/POSIX and the recurring four-backend matrix. Capability census `.1.6` and generated-source `.3` remain. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
