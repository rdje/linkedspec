...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 2/6; Rust `.9.1.4.0-.3` are committed through `9fffe9bd`. Verified descriptor `.4` projects cursor v1 family/policy/resolved edges; its clean commit precedes generated v2 `.5`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
