...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, opaque source/outcome `.10.7.2`, and private static projection `.10.7.3` are composition-closed. Calls audit `.10.7.4.0` freezes exact ownership, `.10.7.4.1` implements typed 18/16, and staged/generated `.10.7.4.2` completes exact private 22/25/10 on both ABIs. No-change `.10.7.4.3` is next. Governance remains 6/20/89 at rollout 5/9 and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
