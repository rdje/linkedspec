...
| `FUTURE-PARITY-BACKLOG` | `active` / Lua nested-write `.19.6.1` complete; `.19.6.2` next | `Overall roadmap - future parity backlog` | All five backends implement frozen nested writes. Perl, Rust, Dart, and Julia also implement `map_leaves!`; Lua `.19.6.1` passes 436 assertions per ABI plus its complete gate. Lua bang and portable/public admission remain pending. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
