...
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` read-only triage IN PROGRESS 2026-06-19: root-cause the ~173 pre-existing back-half core failures (unmasked by NONCORE-QUARANTINE; NOT from that work). Trace via `LINKEDSPEC_TRACE_LEVEL=debug`. **Cluster A (parser collection-shape, ~10+) = STALE tests** (engine returns `[1,1]` per `return(1)`; tests expect old `['?Rule:',[]]` tagged shape). Clusters B–E pending: `method_like`×75, `emit_context`×21, capture/mark/entry families. Then decide re-bless vs engine-fix. Gates green phase0 + `SPEC-FORMAT-TERSE`. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
