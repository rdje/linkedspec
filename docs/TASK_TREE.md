...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Dart emitted `.4.4` signoff-complete from clean atomic 237 `4c8ab7ee` | `.spec language evolution / lossless segmentation and source preservation` | Perl and Rust run ordinarily and in the rooted route at gap 3/6/56 plus ten Rust admission mutations. Dart `.4.1-.4.4` now carry authored/static metadata, private native execution, normalized reconstruction, compatible descriptors, generated-plan execution, and independently analyzed/executed emitted libraries behind the dormant final consumer and ten Dart mutations. Primary/admission `.4.5`, Julia/Lua, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
