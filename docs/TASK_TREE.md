...
| `README-STABILITY-POLICY` | `done` / `closed` | `Repository architecture / documentation sustainability` | `.0` plans at `adcc89fe`; `.1` commits the exact 105-line / 5,072-byte landing page, root policy, hard 128-line / 6,144-byte budgets, and registered `README-STABILITY` doctrine at `ca846e7a`; `.2` recomposes them unchanged under a second canonical gate and closes adoption. MCP `.10.9.7.1` resumes after clean closeout. Project-level licensing remains the independent proposed `.3` director decision. | [docs/tasks/README-STABILITY-POLICY.md](docs/tasks/README-STABILITY-POLICY.md) |
...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Julia alias parity `.14.2.4.0.1` signoff-complete, atomic commit pending | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Julia alias parity activates from clean audit commit `57c62ce4`: one seven-row contract adapter makes all neutral spellings known and canonicalizes them to existing preferred interpreter branches, with no shared 246-name, typed-source, register, schema, or identity movement. Exact RED is 0/7 known and 7/7 unsupported; GREEN is 141/141 across Unicode/boundary/named-map behavior plus native/loaded/reconstructed/generated/emitted carriers. Complete Julia passes byte-fresh MCP, package, storage 19/5, CLI, corpus 105/105, and its exact marker. Neutral stays 6/8/40, language 246/105+1/122, and capability 80/0/0. The sole-facing book includes Julia in all seven aliases while explicitly leaving Julia typed values/projections pending; its 79-file/14,168-KiB HTML keeps separate blocks and is removed. Knowledge Map stays 787/6,450 and all seven doctrines pass. Definitive canonical CI passes typed Perl 10 plus Rust/Dart 4/4, composed semantic/MCP, relocated containment, moved/outside-CWD execution, CLI 66x2, RAM 66%, and Phase 0 1,031/1,031 in 700 seconds before the exact marker. Atomic commit 158/300, brief clearing, and clean proof remain before dormant typed RED `.14.2.4.0.2`; rendered readability stays queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
