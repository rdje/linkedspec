...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Planning `.0` is committed at `74138903`. Initializer `.1.1` defines ignored scratch/cache roots, current-file root discovery, pre/post-create device validation, standard temp/Cargo/Dart/Julia exports, and hostile outside-cwd proof. `.1.2` is the clean routing frontier for hooks, gates, generators, book, and backend runners. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
