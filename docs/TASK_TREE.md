...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia behavior-free authority audit `.10.6.0` is verified/done: typed semantic owners, missing source/query/observation seams, exact Unicode host drift/bypass, and `.10.6.1-.7` split are durable. Governance remains 6/20/81 at rollout 4/9 and admission 3/6. After the clean audit commit, `.10.6.1.0` freezes the generated Julia Unicode classifier/route plan before behavior. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
