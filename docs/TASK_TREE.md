...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Root selection retains final five-backend recurring/public no-drift at 7/0. Julia duplicate-slot `.5` is signoff-complete at 5+2/41 with package/canonical/cleanup proof; its prepared commit is the sole boundary before Lua `.6`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
