...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart outcome authority `.10.5.1.2` is committed at `1e1fd368`. Canonical closeout found and repaired the aggregate-selector scanner's untracked-file blind spot under `.10.5.1.3.0`; that child is signoff-complete from the clean base, and parent `.10.5.1.3` resumes after its clean commit. Governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift stays queued under `.23`; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
