...
| `SCALAREF-RETIREMENT` | `active` | `Overall roadmap — .spec language evolution / compatibility retirement` | `SCALAREF-RETIREMENT.5` — final no-drift sweep after `.4` removed Perl/Rust `scalaref(...)` recognition/execution | [docs/tasks/SCALAREF-RETIREMENT.md](docs/tasks/SCALAREF-RETIREMENT.md) |
...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.2` — `.7.5.2` landed temporary Lispish parity, then `SCALAREF-RETIREMENT.3/.4` migrated the fixture to direct access and removed `scalaref`; next expand the structurally simple oracle corpus batch after the retirement tree closes | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
