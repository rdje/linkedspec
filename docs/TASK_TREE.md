...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.4.3` — `.7.3.4.2` added runtime boolean helper/list-context parity plus isolated regex-dispatch alternatives, and added `portmap_bare`/`portmap_bit`/`portmap_slice`/`portmap_constant`; next fix action-edge fluent child/target aggregation for `ebnf` and `portmap` concatenation | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
