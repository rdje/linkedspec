...
| `FUTURE-PARITY-BACKLOG` | `active` / continuity correction `.22.1` canonical-signoff-complete | `Overall roadmap - future parity backlog` | `.22.1` closes task-status-consistent memory handoffs at 4 fields / 3 valid cases / 6 rejected mutations. Conflicting duplicate `RUST-FUNCTIONAL-PARITY.7` definitions remain separately queued under `.22.2`, the next clean frontier before `.23.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
