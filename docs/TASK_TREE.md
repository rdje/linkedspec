...
| `FUTURE-PARITY-BACKLOG` | `active` / Julia staged `.14.7.6.0` focused-signoff-complete | `Overall roadmap - future parity backlog` | Perl, Rust, and Dart staged-AST parents are closed and privately admitted. Julia's final-path consumer is dormant at 86 GREEN/one exact marker/provenance RED, ordinary/canonical discovery remains absent, rollout is pending, and neutral governance rejects 92 mutations. `.14.7.6.1` is the clean-handoff next leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
