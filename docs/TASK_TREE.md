...
| `SPEC-LANG-REFERENCE` | `active` | `Overall roadmap — documentation and book sync` | **ACTIVE 2026-07-08** — `.10.5.9` fixed `dsl/action-and-lifecycle-placement.md`: regex-bearing examples use normal `:` rules, entry-match lifecycle code is separated from local-slot action code, lifecycle examples use explicit `return(...)`, and the Perl lifecycle handler-shape caveat is tracked. Next leaf: `.10.5.10`, fixing `dsl/capture-marks-and-source-locations.md` worked examples. | [docs/tasks/SPEC-LANG-REFERENCE.md](docs/tasks/SPEC-LANG-REFERENCE.md) |
