...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.8` — `.7.4` finalized the manifest-backed oracle corpus guard after `.7.3.6` brought the corpus to **88 fixtures**; next implement the code-generation emitter | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.8` unless a new terse leaf is split.
