...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Julia `.5.5` and parent `.5` canonical-signoff-complete for intended atomic 245 | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, Dart, and Julia run ordinarily and in the rooted route at gap 5/4/58 plus ten Rust, ten Dart, and ten Julia admission mutations. Julia `.5.1-.5.5` proves exact metadata, same-recognition-authority native execution, normalized/descriptor/generated carriers, independently loaded emitted modules, and existing-primary/nine-role admission. Storage remains 20/5; supported ActionIR stays 246 and format stays 2. Both Lua runtimes, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
