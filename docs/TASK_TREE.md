...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.4` done 2026-07-10 - ADR `0022` makes native in-memory host-language embedding primary; variant CLIs are thin adapters. Julia generated source is deferred to future `.3`; active PNT is `JULIA-BACKEND-PARITY.7.3.1` after the strict CLI-parity audit; Lua `.1.3` follows Julia. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.7.3.0` done 2026-07-10 - source audit proves current variant CLIs are not interface-equivalent: Perl parses, Dart/Julia manage corpora/status, Rust has no binary. `.7.3.1` owns the exact user-facing contract and repair routing. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
