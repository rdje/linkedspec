...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia cursor preflight `.9.1.6.0` is verified: exact seams/boundaries and `.1-.6` are frozen without behavior. Its prepared clean commit is the only boundary before normalization `.1`; root admission `.4.3` follows Julia cursor closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
