...
| `SPEC-LANG-REFERENCE` | `active` | `Overall roadmap — documentation and book sync` | **ACTIVE 2026-07-08** — `.7` added canonical KM cards for output/return shape, regex backend features, rule-mode semantics, lifecycle/`retv`, and capture/mark taxonomy, and extended the edge syntax card for action-vs-blind dispatch. Next leaf: `.8`, final whole-book consistency + close. | [docs/tasks/SPEC-LANG-REFERENCE.md](docs/tasks/SPEC-LANG-REFERENCE.md) |
