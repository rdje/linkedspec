...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.3.6` — `.7.3.5` closed null-output/action-parser triage: `BNF`/`DT`/`ifelse`/`operators_try` were not promoted as semantic fixtures, the `operators_try` quoted-brace warning is fixed, and four `spec.spec` smokes are active; next audit RTL/plugin/legacy shipped specs under the hardened timeout guard | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
...
`SPEC-FORMAT-TERSE` frontier is empty. PNT returns to `RUST-PARITY.7.3.6` unless a new terse leaf is split.
