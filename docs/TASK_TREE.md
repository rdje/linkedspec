...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0. Cursor rollout is 3/5 at 68 governed files; Rust `.9.1.4` closes at `288da21a`. Dart `.9.1.5.1` closes typed family/bare-edge normalization plus portable diagnostics with canonical signoff; live execution `.2` is next after the clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
