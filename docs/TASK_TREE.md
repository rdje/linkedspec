...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.5.1.6.3` closes the exact 61-case Perl reference; `.1.5.2` is active for the missing Rust primary command. `.10.1` parks semantic introspection/MCP; `.11.1` parks generic final-codeblock correction and `with` disposition. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
surfaces agree on the strict 61-case Perl reference, and Rust `.1.5.2` is the active frontier against the unchanged
...
and a thin MCP transport that owns no semantic behavior. The active backend frontier is Rust `.1.5.2`.
