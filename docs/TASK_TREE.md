...
| --- | --- | --- | --- | --- |
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.2.4` done 2026-07-09 - Dart now projects spec-returned `function_definition` AST nodes into `FunctionDefinition` records without raw-scanning `fn` source. Next frontier `.3.1` starts helper/action AST parsing. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
