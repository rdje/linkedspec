...
| --- | --- | --- | --- | --- |
| `SPEC-FORMAT-TERSE` | `active` (**current focus**) | `Overall roadmap — .spec language evolution (terse format)` | `.0` done 2026-06-18 (ratified — ADR `0007`). Implementation gate CLEARED 2026-06-22 (`t/phase0_regression.t` green + `tools/run_ci_local.sh` EXIT 0; gradual-alias migration). **`.1.1` DONE 2026-06-24** (auto-existing wrapper-referenced working vars on Perl + Rust). **`.1.2` SPLIT** by inference channel; **`.1.2.1`+`.1.2.2` DONE 2026-06-24 — Channel 1 complete on BOTH variants** (Channel 2 deferred behind `.1.5`). **`.1.4` DONE 2026-06-29** (helper renames closed on both variants; Perl reference + Rust parity, phase0 **975 green**). **`.1.3` SPLIT 2026-06-29** by mutation mechanism: scalar `set(name,val)` already satisfied by `.1.4`; **`.1.3.2` DONE** — `push(target,value)` explicit append with child-call precedence; **`.1.3.3` DONE** — `set_key(name,key,value)` statement mutation while value form stays pure; **`.1.3.4` SPLIT 2026-06-29** — operator family decomposed into scalar assignment, array append, and hash-index assignment; **`.1.3.4.1` DONE** — scalar `name = value` statement assignment; **`.1.3.4.2` DONE** — array `items += value` statement append for explicit RHS expressions. **Frontier → `.1.3.4.3`** (hash `name[key] = value`). | [docs/tasks/SPEC-FORMAT-TERSE.md](docs/tasks/SPEC-FORMAT-TERSE.md) |
