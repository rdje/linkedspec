...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua attached/marker if-family controls close at 106/106 under `.4.3.6.3.1`; switch-family statements `.4.3.6.3.2` are active, user-function block dispatch stays `.5.1`, and first-class callable values stay future `.11.7`. Alias-shape, truthiness, and rename-collision drift remain helper caveat `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager/inline controls and attached/marker if-family statements pass 106/106 through `.4.3.6.3.1`; switch-family statements `.4.3.6.3.2` are active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
`FUTURE-PARITY-BACKLOG.5`. Switch-family statement execution `.4.3.6.3.2` is active.
