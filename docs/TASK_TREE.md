...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Closed status: repeated-action recurring/public no-drift is closed. Generated-plan correction `.10.3.3.0` is clean at `c07ba618`. Calls spec-identity correction `.10.3.3.1.0` derives exact `calls_and_staging` from caller logical identity and advances semantic governance to 6/20/57 without behavior/admission. Fully corrected Perl projection `.10.3.3.1.1` follows its clean commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
