...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / decomposition `.3.2.2` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; five Perl owners read; all remaining Perl ranges owned in 52 leaves; `.3.2.3` reads the dependency group next. Codebase/book remain No; `rgx` excluded. `.7` repairs EPERM-as-dead cleanup after reading and before recovery/purge or mutation setup. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
