...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua lazy inline controls close at 105/105 under `.4.3.6.2`; attached/marker if-family `.4.3.6.3.1` is active, user-function block dispatch stays `.5.1`, and first-class callable values stay future `.11.7`. Truthiness and rename collision drift remain helper caveat `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager blocks and lazy selected-value controls pass 105/105 under `.4.3.6.2`; attached/marker if-family controls `.4.3.6.3.1` are active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
execution `.4.3.6.3.1` is active. Lua follows Perl-oracle truthiness pending cross-backend normalization in
