...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Rust authored metadata `.3.1` signoff-complete for intended atomic 228 | `.spec language evolution / lossless segmentation and source preservation` | The admitted Perl consumer runs ordinarily and in the rooted route at gap 2/7/56. From clean `4a95e02a`, `.3.1` adds exact Rust declarations/selectors, directive validation, compiled provenance, and ten-mutation dormancy governance; the ignored final consumer has no canonical/recurring/facade or live execution. `.3.2-.3.5` retain native/generated/emitted/admission ownership. Five later runtime routes, recurring/public rows, typed composition, generated-plan v2, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
