...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua Unicode `.10.7.1`, source/outcome plan `.10.7.2.0`, strict source `.10.7.2.1`, and compiled-or-failed outcome `.10.7.2.2` are complete. No-change foundation closeout `.10.7.2.3` is next, with semantic governance unchanged at 6/20/89, rollout 5/9, and admission 4/6. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
