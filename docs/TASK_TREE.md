...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.4.1` after the Julia compiled-spec state leaf. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.3.4` done 2026-07-10 - Julia now compiles parsed specs into ordered compiled-state records with dependency refs, dependency-regex rows, action payload AST/contracts, function registry projection, mode metadata, and descriptor JSON. Active frontier advances to `.4.1` for regex matching and match-state tracking. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.3.4` and now sits at `JULIA-BACKEND-PARITY.4.1`. Generated Dart source remains a future
split proof lane.
