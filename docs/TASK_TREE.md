...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Rust source/outcome foundation `.10.4.1` is complete with strict copied-source mapping and opaque compiled-or-failed authority; static v1 projection `.10.4.2` follows. Semantic rollout/admission remain 2/9 and 1/6; repeated-action recurring/public no-drift is closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
