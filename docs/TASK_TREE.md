...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua statement controls including attached while close at 108/108 through `.4.3.6.3.3`; built-in final blocks/scoped with `.4.3.6.4` are active, user-function block dispatch stays `.5.1`, and first-class callable values stay future `.11.7`. Alias, truthiness, switch equality/range, while limit/next, and rename-collision drift remain helper caveat `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager/inline controls, attached/marker if and switch, and attached while pass 108/108 through `.4.3.6.3.3`; built-in final blocks/scoped with `.4.3.6.4` are active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
`next`, and typed exact-limit rule attribution. Guard timing and `next` drift join backlog `.5`; current built-in
final blocks/scoped with `.4.3.6.4` are active.
