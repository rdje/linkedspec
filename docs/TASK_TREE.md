...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Diagnostic-output `.5.1.0-.9` is closed at 8/0. Logical audit `.5.2.0` records three truthiness profiles/two Perl mechanisms and is complete; executable neutral contract `.5.2.1` is active before backend/generated/gate/public `.2-.9`. ADR `0036`/`.19` remain later-owned. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
