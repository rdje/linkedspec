...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua typed mutation/child flow `.4.3.4.4` is closed at 98/98; tagged records/remaining bridges `.4.3.4.5` are active. Flat/concat arity, negative counts, dropped-transform rebinding, and invalid-join drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Typed accumulators, cached whole/indexed child push, and uniform append/end/split flow pass 98/98 on both ABIs; tagged records/remaining bridges `.4.3.4.5` are active. Later closeout and variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
