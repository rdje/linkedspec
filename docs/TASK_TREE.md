...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia MCP generated binding/runtime/secure decoded server `.10.9.5.1` is signoff-complete from clean plan commit `1b19589d`: 48 binding/runtime + 139 decoded assertions, complete Julia local proof, 65 admission mutations, and canonical CI pass without stdio or ledger movement. Strict synchronous stdio `.10.9.5.2` is next only after the clean `.1` commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
