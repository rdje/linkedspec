...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.4.3` after first Julia runtime rule dispatch. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.4.2` done 2026-07-10 - Julia executes compiled default/AND/OR/repetition modes, lifecycle flow/events, action/blind children, `retv`, explicit returns, narrow accumulators/capture reads, output projection, and recursion/progress guards. Active frontier advances to `.4.3` for split helper/value runtime families. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`JULIA-BACKEND-PARITY.4.2` and now sits at `JULIA-BACKEND-PARITY.4.3`. Generated Dart source remains a future
split proof lane.
...
`JULIA-BACKEND-PARITY.4.3`, whose broad helper/value container must split into safe owned batches before code.
