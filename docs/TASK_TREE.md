...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua copied construction/splicing `.4.3.4.1` is closed at 92/92; selection/order/membership `.4.3.4.2` is active. Flat/concat arity drift is durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Ordered copied construction and explicit splicing pass 92/92 on both ABIs; copied selection/order/membership `.4.3.4.2` is active. Later transform/mutation/tagged/closeout and variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
