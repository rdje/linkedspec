...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.3.5` done 2026-07-09 - Dart runtime now executes value blocks, attached/inline controls, helper/receiver `with` trailing blocks, and hash/array tree traversal callbacks. Next frontier `.4.3.6` closes helper/value no-drift before BACKTRACK work. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
