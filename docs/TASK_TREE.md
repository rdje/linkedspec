...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart typed capture `.10.5.5.1` and immutable topology-validated observed-index derivation `.10.5.5.2` now match the exact twentieth response digest without result/trace/diagnostic drift or base-index mutation. Generated/emitted route propagation `.10.5.5.3` is active; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
