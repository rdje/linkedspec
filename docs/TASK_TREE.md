...
| `FUTURE-PARITY-BACKLOG` | `active` / Dart `.14.6.4` closed at `3ccaf7c3`; capacity composition-closed; Julia `.14.6.5` next | `Overall roadmap - future parity backlog` | Neutral + Perl + Rust + Dart are admitted at 4/9/103. Capacity `.0-.2` preserve all 547 IDs and pressure ceilings while composition-closing the eighth `.14.6.5-.14.8` semantic member. Julia `.14.6.5` task splitting resumes next; Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` remain separate. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
