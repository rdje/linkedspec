...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart typed record/source `.10.5.4.1` plus traversal/pages/logical limits `.10.5.4.2` now match all 16 successful static semantic-query digests package-privately. Public typed/raw-neutral query `.10.5.4.3` is next; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
