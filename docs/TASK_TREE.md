...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua harray construction/views plus copied merge/set/rename/drop/pick receiver chains pass 102/102 on both ABIs; named mutation `.4.3.5.4` is active and callbacks remain `.4.3.6`. Rename collision drift joins the helper caveats durably routed to `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Array family is closed at 99/99. Harray construction, deterministic views, and copied transforms/receiver chains pass 102/102. Named set-key/direct mutation `.4.3.5.4` is active. Tree callbacks and later variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
