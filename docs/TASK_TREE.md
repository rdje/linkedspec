...
| `FUTURE-PARITY-BACKLOG` | `active` / Rust recursive observation `.14.4.3` signoff-complete as intended atomic 211 from clean `2d937d83` | `Overall roadmap - future parity backlog` | Rust `.14.4.3` has its private four-carrier implementation, final-path regression, exact 8/6/72 typed-source composition, 79-file book, Knowledge 823/6,856, eight-doctrine proof, and canonical CLI 66x2 / RAM 46% / Phase 0 1,031/1,031 in 744 seconds complete; atomic commit remains. Perl and Rust alone are current on the pending observation row; recognition stays 129+4/246/58. Dart `.14.4.4` cannot activate before Rust lands clean. Typed-source `.14.8`, task metadata `.5`, and rendered readability remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
