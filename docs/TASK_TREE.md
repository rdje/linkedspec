...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Julia private native `.5.2` focused-signoff-complete for intended atomic 242 | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, and Dart run ordinarily and in the rooted route at gap 4/5/57 plus ten Rust and ten Dart admission mutations. Julia `.5.1` landed exact authored/static/compiled provenance at atomic 241; `.5.2` now adds same-recognition-authority native gap state/accessors/lifecycle behind ten dormancy guards while preserving exact 246 supported ActionIR names. Reconstruction/descriptor/generated `.5.3`, independent emitted `.5.4`, primary/admission `.5.5`, Lua, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
