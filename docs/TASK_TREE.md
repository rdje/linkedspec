...
| `PERL-ACTIONIR-AST-MIGRATION` | `active` (**current focus**) | `Overall roadmap — compiler architecture / variant contract` | User directive 2026-07-01: Perl must move from ActionIR text-to-text lowering to Rust-style text-to-AST, and every backend must use typed AST/IR before lowering/execution. `.0`–`.2` adopted doctrine, inventoried the Perl text-lowering boundaries, and added the additive `LinkedSpec::ActionIR::AST` parser seam. `.3` is complete for values, helper calls, receiver chains, and return payloads. `.4.1`–`.4.4.4` are complete for assignment/mutation statements, helper-call statements/returns, block-value statements, and structured control-flow lowering from typed AST nodes. `.5.1` audited the fallback boundary; `.5.2` now lowers supported standalone value statements as discarded `VALUE_DROP` nodes; `.5.3.1` retired `s(...)`, `a(...)`, and `h(...)` wrapper aliases in favor of canonical `scalar(...)`, `array(...)`, and `hash(...)`; `.5.3.2` now diagnoses return/value-position unknown typed calls instead of leaking generated host calls while leaving standalone unknown calls for the function registry. **Frontier -> `.5.4`** `fn` grammar ownership/proof lock. | [docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md](docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md) |
