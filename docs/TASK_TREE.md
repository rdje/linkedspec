...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.3.3` done 2026-07-09 - Dart now builds an ordered user-function registry from staged `FunctionDefinition` sidecars, exposes body parse jobs, and resolves exact-arity user calls before helper fallback. Next frontier `.3.4` builds the compiled-spec/interpreter state model. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
