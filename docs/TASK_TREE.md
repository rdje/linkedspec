...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Source migration `.12.1.7.1-.3` is complete at zero executable selector positives; Perl hard rejection `.12.1.8.1` is next. Toolbox repair `.13.1` and structural/progressive/staged authoring arc `.14.1-.4` are queued behind selector retirement. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
