...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Selector retirement `.12.1` is re-closed through 56-file follow-up `.12.1.10`. Lua numeric word/symbol calls and number receivers are complete; aggregate reducers/array terminals `.4.3.3.3` are active. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Strict scalar numeric v1 remains exact across six runtimes; aliases, symbols, and receiver chains pass 90/90 on both Lua ABIs. Aggregate numeric reducers/array receiver terminals `.4.3.3.3` are active; future variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
