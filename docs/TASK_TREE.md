...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.2` — `.7.5.2` landed temporary Lispish parity, then `SCALAREF-RETIREMENT.3/.4/.5` migrated, removed, and closed `scalaref`; next expand the structurally simple oracle corpus batch | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
