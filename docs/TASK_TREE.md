...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Logical helper parent `.5.2` is closed at 8/0: native/generated behavior, recurring admission, and public no-drift agree. ADR `0044` / `.9.1.1.1` ratifies rule-local cursor policy and mode-sensitive bare edges; implementation `.9.1.2` follows after the clean logical closeout commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
