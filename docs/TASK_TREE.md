...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.4` done 2026-07-10 - ADR `0022` makes native in-memory host-language embedding primary; variant CLIs are thin adapters. Julia is now 99/99 and active PNT is `JULIA-BACKEND-PARITY.6.4`; Lua `.1.3` inherits the native-module gate after Julia. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.3` done 2026-07-10 - one atomic library regression and direct unbounded CLI pass all 99 fixtures in order with exact outputs. Full tests pass with 840 assertions and status `runtime-corpus-full`; `.6.4` is active for verification wiring. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
840 assertions, status is `runtime-corpus-full`, and `.6.4` is active for verification wiring.
