...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5`, census `.1.6.0`, 237-name audit `.1.6.1.0`, and universal newline splitting `.1.6.1.1` are closed. `.1.6.1.2.0` split the residual generated `endswitch` terminator seam; active `.2.1` repairs it before `.2.2` completes strict neutral proof. Later leaves own Rust descriptor/diagnostics, native named/file resolution, and Dart full-pipeline trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` (delegated global obligations) | `Overall roadmap - future backend parity (Julia second)` | Local proof is 1,019 assertions/nine processes/99 fixtures; Julia passes 61/61 default/POSIX and the recurring four-backend matrix. Capability census `.1.6` and generated-source `.3` remain. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`endswitch()`: `} }` is followed directly by `return`. `.1.6.1.2.1` is active for contract-aware terminator repair;
