...
| `FUTURE-PARITY-BACKLOG` | `active` / correction `.14.6.0.1` signoff-complete; neutral `.14.6.1` next | `Overall roadmap - future parity backlog` | From clean audit atomic 257 `1a6e0b1f`, corrective `.0.1` projects recognition 9/9/58 into typed `transaction_safety` at 11/3/152 without behavior or outward movement. Progressive order remains `.1` neutral → `.2-.6` backends → `.7` recurrence → `.8` public; staged `.14.7` and combined `.14.8` remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
