...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.3.2` done 2026-07-09 - Dart runtime now executes string/scalar helpers, explicit `str_*` lexical comparisons, numeric helpers, numeric aliases/symbol callees, and compatible receiver chains. Next frontier `.4.3.3` starts array helper family and array receiver/mutation behavior. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
