...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5` and all non-codegen capability work `.1.6` are closed at census 57/1/2. The only non-pass states are generated source (Rust partial, Dart/Julia gaps), all owned by active `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
