...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Planning `.0` is `74138903`; initializer `.1.1` is `e33ed191`. Routing `.1.2` makes 14 hook/gate/generator/book/backend entrypoints self-initialize repo-filesystem storage and proves hostile outside-cwd selection. `.1.3` is the clean lifecycle/concurrency frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
