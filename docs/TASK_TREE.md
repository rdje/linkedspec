...
| `RUST-PARITY` | `active` | `Phase 9 — Rust variant (parity follow-on)` | `RUST-PARITY.7.5.2` — action-edge fluent `.7.5.3` reconciled done from `SPEC-FORMAT-TERSE.2.3.3.*`; remaining shipped recursive-spec blocker is Lispish `scalaref(retv, {content})` | [docs/tasks/RUST-PARITY.md](docs/tasks/RUST-PARITY.md) |
