...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.4` done 2026-07-10 - ADR `0022` makes native in-memory host-language embedding primary; variant CLIs are thin adapters. Julia is 99/99 with focused verification and aligned public docs; active PNT is `JULIA-BACKEND-PARITY.7.2`; Lua `.1.3` follows Julia closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.7.1` done 2026-07-10 - mdBook commands, native rule/function examples, `runtime-corpus-full` 99/99 status, and generated-source/trace/tooling limitations are aligned. Full tests pass with 840 assertions; `.7.2` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
