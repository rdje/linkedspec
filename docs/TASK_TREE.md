...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Root selection retains final five-backend recurring/public no-drift at 7/0/54, duplicate-slot recurring/public no-drift is closed at 7/0/59, and rule-local cursor recurring/public no-drift at 75 files / 8 complete + 0 pending / 60 mutations. ADR `0048` accepts explicit-repetition per-hit action collections and scalar pipe after two independent four-newer-backend defects were isolated; `.9.1.10.1-.7` split neutral through public rollout before code. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
