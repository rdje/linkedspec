...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / reading `.3.2.13` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; seventeen Perl files read; `.3.2.14` reads HandlerVariantEmitter next. `.12`/`.13` own edge order and repeated blind-target identity. Codebase/book No; `rgx` excluded. Repairs `.7`–`.13` follow reading before mutation setup. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
