...
| `REPO-ROOT-PATH-PORTABILITY` | `active` | `Repository architecture / checkout relocation invariance` | Remediation `.1.1-.1.3` is complete: Rust runtime root discovery, eight legacy config/source owners, and 12 Julia Knowledge Map commands are portable. Current leaf `.2.1` owns the structural doctrine and mutation-sensitive self-tests. | [docs/tasks/REPO-ROOT-PATH-PORTABILITY.md](docs/tasks/REPO-ROOT-PATH-PORTABILITY.md) |
