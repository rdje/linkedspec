...
| `FUTURE-PARITY-BACKLOG` | `active` / task-ID uniqueness `.22.2` canonical-signoff-complete | `Overall roadmap - future parity backlog` | `.22.2` repairs the sole conflicting legacy definition and makes 1,718 exact current IDs unique across 96 partitioned/unpartitioned task files through 10 mutations; one tracked-index-registered immutable history file is excluded. `.23.1` is the next clean frontier. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
