...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Behavior-free Julia MCP preflight `.10.9.5.0` is signoff-complete from clean Dart closeout `98cbe61a`; ADR `0060` and the Knowledge plan freeze generated Base64 data, native synchronous server, strict number-preserving wire, security/API/authority fences, and exact `.1-.4` order. Generated binding/runtime/decoded server `.10.9.5.1` is the next clean-boundary activation. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
