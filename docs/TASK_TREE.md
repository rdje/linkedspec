...
| `INTER-MATCH-GAP-CAPTURE` | `active` / Dart parent `.4` signoff-complete from clean atomic 238 `6a554312` for intended atomic 239 | `.spec language evolution / lossless segmentation and source preservation` | Perl, Rust, and Dart run ordinarily and in the rooted route at gap 4/5/57 plus ten Rust and ten Dart admission mutations. Dart `.4.1-.4.5` carry exact authored/native/reconstructed/descriptor/generated/emitted/primary behavior through one nine-role consumer. ADR `0074` preserves all prior guards and adds the missing Dart regression; exact canonical CI closes parent `.4`. Julia `.5` follows after commit/brief/clean proof; Lua, recurring/public, typed composition, and outward surfaces remain pending or unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
