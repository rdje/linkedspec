...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Generated-source `.3` is closed at 60/0/0. Lua ordered function/body-job registry `.3.3` passes 50/50 on both runtimes; compiled state `.3.4` is active. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Ordered immutable function/body-job records, exact resolution, body-AST stitching, isolated four-kind frames, and recursion diagnostics pass 50/50 on PUC Lua/LuaJIT; compiled state `.3.4` is active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
