...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | No-change Rust MCP closeout `.10.9.3.4` recomposes every committed owner from clean admission commit `13d9ce17`, preserves 2/5 implementations + 2/6 runtimes with rollout pending and 39 mutations, passes focused/canonical signoff, and closes parent `.10.9.3`. Dart `.10.9.4` follows only after the clean closeout commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
