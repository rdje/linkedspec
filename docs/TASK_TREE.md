...
| `DART-BACKEND-PARITY` | `done` / `closed` (scoped milestone) | `Overall roadmap - future backend parity (Dart first)` | Global proof is now 175 tests, 105 corpus fixtures, exact 61x2 CLI, and admitted full-pipeline trace. Non-codegen `.1.6` is closed; only generated-source `.3` blocks complete Dart parity. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Perl/Rust v1 baseline admission is closed at census 57/1/2. Rust's explicit full-manifest classifier is 105/105 green and zero-failure closeout is closed; strict recurring admission `.3.2.2` is active before Dart/Julia emitters and exact closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
