...
| `SPEC-LANG-REFERENCE` | `active` | `Overall roadmap — documentation and book sync` | **ACTIVE 2026-07-08** — `.10.5.8` fixed `user-model/blind-calls-and-parser-orchestration.md`: blind-call `::` wrappers remain no-regex, regex-owning action-edge examples use single-colon labels, and the mixed-edge negative example is clarified. Next leaf: `.10.5.9`, fixing `dsl/action-and-lifecycle-placement.md` worked examples. | [docs/tasks/SPEC-LANG-REFERENCE.md](docs/tasks/SPEC-LANG-REFERENCE.md) |
