...
| `PERL-ACTIONIR-AST-MIGRATION` | `active` (**current focus**) | `Overall roadmap — compiler architecture / variant contract` | User directive 2026-07-01: Perl must move from ActionIR text-to-text lowering to Rust-style text-to-AST, and every backend must use typed AST/IR before lowering/execution. `.0`–`.2` adopted doctrine, inventoried the Perl text-lowering boundaries, and added the additive `LinkedSpec::ActionIR::AST` parser seam. `.3` is complete: non-call values, helper calls, receiver chains, and return payloads now consume typed AST fields before raw fallback. `.4` is split for statement/control lowering; `.4.1` operator statements, `.4.2` helper-call statements/returns, and `.4.3` block-value side effects are done. `.4.4` is split for structured control flow; `.4.4.1` parses attached/marker control forms into typed AST nodes and **`.4.4.2` DONE 2026-07-01** lowers if/when/otherwise forms from typed condition/body nodes. **Frontier -> `.4.4.3`** lower `switch`/`case`/`default` statement forms from typed source/match/body/default nodes. | [docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md](docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md) |
