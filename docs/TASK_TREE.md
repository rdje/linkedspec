...
| `INTER-MATCH-GAP-CAPTURE` | `active` / emitted-and-loaded Perl parity `.2.3` signoff-complete from clean atomic 224 at `34d02e0c` for intended atomic 225; `.2.4` next | `.spec language evolution / lossless segmentation and source preservation` | Authored/static `.2.1`, private native-live `.2.2`, and generated/loaded `.2.3` are complete. Exact gap rollout remains 1/8/55 plus ten dormancy locks because `.2.4` alone admits Perl. Canonical `.2.3` proof passes Phase 0 1,031/1,031 and exact opt-in routing; public helpers remain 122, typed source remains 9/5/114, and generated-plan v2 plus outward surfaces remain unchanged. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
