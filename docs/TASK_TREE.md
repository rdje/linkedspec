...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.2.3` after eager logical helpers moved shipped smoke to 17/31. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.2.1` done 2026-07-10 - eager `and`/`or`/`not` closes three portmap cases plus tablegrep and routes portmap constant's no-op helper-regex flag residual. Full tests pass with 772 assertions and status `runtime-corpus-logical-helpers`; shipped smoke is 17/31 and `.6.2.4.2.3` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.2.1` have since landed; `.6.2.4.2.3` is active.
...
`runtime-corpus-logical-helpers`; shipped smoke is 17/31 and `.6.2.4.2.3` is active.
