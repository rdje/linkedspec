...
| `FUTURE-PARITY-BACKLOG` | `active` / `.14.3.2.0` dormant Perl RED | `Overall roadmap - future parity backlog` | Typed-source value/helper parent `.14.2` and neutral transaction parent `.14.3.1` are composition-closed. Perl leaf `.14.3.2.0` now freezes one final-path but canonically dormant consumer: the accepted four-form source reaches descriptor construction, 49 assertions pass, and exactly one fails because four dedicated transaction nodes are absent while three helpers remain unresolved and rollback remains raw. Neutral rollout stays 1/9; no production or public behavior moves. Private Perl authority `.14.3.2.1` follows only after atomic 184/300 lands clean. `TASK-TREE-METADATA-HYGIENE.5` separately owns active-frontier prevention; rendered readability remains queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
