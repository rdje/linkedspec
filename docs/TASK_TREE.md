...
Index note 2026-07-08: `SPEC-FORMAT-TERSE.12` is reactivated by explicit user directive. `SPEC-FORMAT-TERSE.12.1` is done; frontier is
`SPEC-FORMAT-TERSE.12.2` for the Perl reference implementation: accepted receiver methods,
attached-block syntax, scoped callback bindings, traversal order, array-leaf/error semantics, Perl/Rust parity,
mdBook examples, tests, oracle fixtures, and Knowledge Map facts.
...
| `ROADMAP-DRIFT-RECONCILE` | `done` | `Overall roadmap — documentation and book sync` | Three leaves done 2026-07-08: `.0` owned deferred `ROADMAP.md` / `ARCHITECTURE_STATE.md` drift, `.1` reconciled long-form `ROADMAP.md`, and `.2` refreshed `ARCHITECTURE_STATE.md` plus narrow mdBook status/count drift to current 21-spec / phase0 `1..1026` / 95-fixture Rust oracle / noncore-plugin state. | [docs/tasks/ROADMAP-DRIFT-RECONCILE.md](docs/tasks/ROADMAP-DRIFT-RECONCILE.md) |
...
| `TASK-TREE-METADATA-HYGIENE` | `done` | `Overall roadmap — durable architecture / task-tree hygiene` | Four leaves done 2026-07-08: `.0` owned the open-task-tree audit, `.1` reconciled stale top metadata, `.2` reconciled stale completed-tree frontier/verification/commit rows and classified deferred/non-goal cases, and `.3` added the `TASK-TREE-METADATA` doctrine gate for completed-tree `Current Frontier` rows. | [docs/tasks/TASK-TREE-METADATA-HYGIENE.md](docs/tasks/TASK-TREE-METADATA-HYGIENE.md) |
...
| `PERL-ACTIONIR-AST-MIGRATION` | `done` | `Overall roadmap — compiler architecture / variant contract` | All leaves `.0`–`.5.4` done 2026-07-01: text-to-AST doctrine adopted, Perl ActionIR AST parser seam landed, value/statement/control lowering moved to typed AST nodes, supported fallback leakage retired, return/value unknown calls diagnose, short wrappers retired, and `fn` grammar ownership locked to `specs/spec.spec`. PNT returned to `SPEC-FORMAT-TERSE.4.1`; `.4.1`, `.4.2.1`, `.4.2.2`, `.4.2.3`, `.4.3.1`, `.4.3.2`, `.4.4`, `.3.2.2`, `.3.2.3`, `.3.2.3.1`, `.3.2.3.2`, `.3.2.3.3`, `.3.2.3.4`, `.3.3`, `.3.3.1`, `.3.3.2`, `.3.3.3`, and `.3.3.4` are now complete/owned; later `.6` shipped-spec migration, `.7` type-method surface, `.15` colon scalar-slot retirement, and `.14` trailing blocks all closed; the current active terse frontier is `SPEC-FORMAT-TERSE.12.2`. | [docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md](docs/tasks/PERL-ACTIONIR-AST-MIGRATION.md) |
...
| `REPO-HYGIENE` | `completed` | `Overall roadmap — repository maintenance` | All 3 leaves done: `.1` tool-artifact ignore cleanup, `.2` local-only `rgx/` + `.claude/projects/` cleanup, and `.3` urgent generated-artifact cleanup removing ignored/untracked `rust/target` plus mdBook build output while preserving `rgx` corpus artifacts. | [docs/tasks/REPO-HYGIENE.md](docs/tasks/REPO-HYGIENE.md) |
