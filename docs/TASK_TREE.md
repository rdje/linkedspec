...
| `FUTURE-PARITY-BACKLOG` | `active` / recursive-observation public projection `.14.4.8` signoff-complete for atomic 216 from clean `eb8d052c` | `Overall roadmap - future parity backlog` | Six governed documents, six stale-claim denials, and ten public-surface guards advance typed-source governance to 9/5/114 without adding a fifteenth row or promoting combined `.14.8`. The exact five-source/six-runtime recurrence remains green at Perl/Rust/Dart 7 each, Julia 30, and Lua 43 per ABI plus support ledgers; no parser, runtime, facade, schema, semantic/MCP, CLI, README, or storage behavior moves. Parent `.14.4` is closed and `.14.5` is next. The repeated-action recurring/public no-drift is closed. Typed-source `.14.8`, task metadata `.5`, and rendered readability remain separate. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
