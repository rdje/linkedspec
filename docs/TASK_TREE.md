...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | ADR 0033 and `.16.0` ratify narrow punctuation-light zero-argument aliases after an exact five-backend parser audit; `.16.1` is next for the neutral positive/negative contract. Parenthesis-free `if`/`while` headers are excluded. Lua built-in final blocks/scoped with `.4.3.6.4` remain queued at a clean handoff. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
syntax contract after a clean pivot.
...
paths. Parenthesis-free condition-bearing `if`/`while` headers remain explicitly deferred. The next frontier is
`.16.1` for one neutral positive/negative contract; Lua `.4.3.6.4` remains cleanly queued.
