...
| --- | --- | --- | --- | --- |
| `SESSION-STARTUP-READING` | `active` / checkpoint `.3.3.2` complete | `Session continuity prerequisite to RUST-MUTATION-TESTING.1` | Roadmap Yes; `.31` preserves forward reading and repair ownership. Reads Rust manifests, the complete AST, and callable-contract prefix; owns precise documentation evidence. Next `.3.3.3`: callable normalization suffix and compiler prefix. Codebase No; physical mdBook Yes, formal .4 alignment pending; `rgx` excluded. `.7` blocks recovery/purge; `.29` belongs to policy adoption `.5`. | [docs/tasks/SESSION-STARTUP-READING.md](docs/tasks/SESSION-STARTUP-READING.md) |
