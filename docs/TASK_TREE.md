...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua copied selection/order/membership `.4.3.4.2` is closed at 93/93; scalar/regex transforms `.4.3.4.3` are active. Flat/concat arity and negative-count drift are durably routed to helper-caveat owner `.5`. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Copied selection/order/membership/uniqueness pass 93/93 on both ABIs; scalar/regex transform pipelines `.4.3.4.3` are active. Later mutation/tagged/closeout and variadic/native/descriptor/generated obligations remain explicit. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
