...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia Unicode planning `.10.6.1.0` is verified/done from clean audit `7bdb181f`: exact generated artifact, five parser replacements, validator roles, positive/distinct/negative/isolation routes, and gate order are frozen without behavior. Governance remains 6/20/81 at rollout 4/9 and admission 3/6. After its clean commit, `.10.6.1.1` generates and routes the classifier. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
