...
| `DART-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Dart first)` | `.4.3.1` done 2026-07-09 - Dart runtime core values now cover scalar/array/hash JSON shapes, typed wrapper snapshots, hash-index mutation, nested reads, and named/map/position capture helpers. Next frontier `.4.3.2` starts string/scalar and numeric helper families. | [docs/tasks/DART-BACKEND-PARITY.md](docs/tasks/DART-BACKEND-PARITY.md) |
