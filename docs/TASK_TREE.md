...
| `STAGED-LINKED-PARSING` | `active` | `Overall roadmap — .spec language model / parser composition` | `STAGED-LINKED-PARSING.5.2` — `.1` adopted staged linked parsing doctrine, `.2` specified future import/composition, `.3` specified future parse-job annotations, `.4` specified deterministic registry/dispatch queue semantics, and `.5.1` selected user-function body text as the first prototype payload family under a hard implementation-language-neutrality gate. Next audit function-body seams before code. | [docs/tasks/STAGED-LINKED-PARSING.md](docs/tasks/STAGED-LINKED-PARSING.md) |
