...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Uniform-binding selector retirement `.12.1` is closed and publicly admitted. Lua scalar numeric admission is complete; aliases/symbols/number receivers `.4.3.3.2` are active. Inspector `.13.1` is dependency-ready and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
