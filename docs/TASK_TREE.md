...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia cursor descriptor-v1 `.9.1.6.3` is fully verified: normalized family/policy/ownership/edge projection and direct/loaded/normalized bytes are exact. Its clean commit is required before generated-source v2 `.9.1.6.4`; root admission `.4.3` follows cursor closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
