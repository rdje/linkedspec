...
| `PROJECT-DATA-SSD-ROOTING` | `active` | `Repository architecture / project-data storage locality` | Common storage `.1`, Perl `.2.1`, Rust `.2.2`, and Dart `.2.3` are complete. Dart now supplies a complete 47-package offline cache and proves 18 temporary owners plus generated/trace paths stay on repository storage; two exact shared checkout records are deleted. `.2.4` is the clean Julia storage frontier. | [docs/tasks/PROJECT-DATA-SSD-ROOTING.md](docs/tasks/PROJECT-DATA-SSD-ROOTING.md) |
