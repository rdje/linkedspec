...
| `README-STABILITY-POLICY` | `active` / routing-pressure revision `.4.0` active | `Repository architecture / documentation sustainability` | Original `.0-.2` adoption remains closed: README is 105 lines / 5,072 bytes under hard 128-line / 6,144-byte caps and the registered `README-STABILITY` doctrine. Director-priority revision `.4` now adds transitive routing-pressure closure so reader navigation and author-overflow hints terminate at classified controlled destinations instead of moving append pressure. `.4.0` is a behavior-free exhaustive route/control audit and schema plan from clean `7c2ff407`; `.4.1` implements policy/registry/checker/mutations; `.4.2` recomposes unchanged and returns to transaction `.14.3.1.1`. Project licensing remains independent proposed `.3`. | [docs/tasks/README-STABILITY-POLICY.md](docs/tasks/README-STABILITY-POLICY.md) |
| `LIVE-DOCUMENT-PRESSURE-CONTAINMENT` | `proposed` / dependency-queued behind README `.4.2` | `Repository architecture / documentation sustainability` | README route audit measures four debt families from clean `7c2ff407`: live status 14,769 lines / 1,262,969 bytes; 85 task files / 6,204,304 bytes with one 2,720,175-byte part; changes 44,128 lines / 3,091,199 bytes; engineering notes 21,169 lines / 2,277,541 bytes. `.0` freezes retrieval/migration descriptors, `.1` bounds live status, `.2` semantically partitions future-backlog task evidence, `.3` rotates changes/notes, and `.4` ratchets registry state and closes. No leaf is eligible until README policy/registry/checker closeout `.4.2` is clean. | [docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md](docs/tasks/LIVE-DOCUMENT-PRESSURE-CONTAINMENT.md) |
...
| `FUTURE-PARITY-BACKLOG` | `active` / `.14.3.1.0` signoff-complete; `.14.3.1.1` next after README-policy priority | `Overall roadmap - future parity backlog` | Typed-source value/helper parent `.14.2` is composition-closed. Behavior-free `.14.3.0` audits current cursor/mark/recursion seams; `.14.3.1.0` ratifies exact future recognition transactions, falsey-safe staged results, a closed 9-allowed/11-rejected effect lattice, linear tokens, invocation-framed marks, and cursor-only v1 progress without executable/current-public behavior. Book 79/14,248 KiB, Knowledge 793/6,558, all seven doctrines, CLI 66/66 twice, RAM 58%, and Phase 0 1,031/1,031 in 672 seconds pass. Executable neutral authority `.14.3.1.1` is next after the director-priority README-policy adoption receives and completes its own clean task slice. `TASK-TREE-METADATA-HYGIENE.5` separately owns the startup-discovered active-frontier prose gap; rendered readability remains queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
