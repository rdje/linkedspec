...
| `PHASE0-BACKHALF-TRIAGE` | `active` (**current focus**) | `Overall roadmap — regression-gate health (back-half core failures)` | `.1` triage DONE (173 → 108 STALE / 65 REAL, 2 engine defects). User **authorized BOTH engine fixes** 2026-06-21 (ADR `0008`). **`.3` DONE 2026-06-21** — fixed Defect #1 AND-rule action-codegen (`\$"`→verbatim emit in `HandlerVariantEmitter.pm`); full phase0 **173 → 111 failing (62 cleared, 0 regressions)**. Frontier → **`.4`** (Defect #2 input-boundary guard in `Runtime.pm`), then `.2.x` re-bless the 108 STALE, then `.5` green-phase0 verify (+ the `corpus_regression` tail). Gates green phase0 + `SPEC-FORMAT-TERSE`. | [docs/tasks/PHASE0-BACKHALF-TRIAGE.md](docs/tasks/PHASE0-BACKHALF-TRIAGE.md) |
