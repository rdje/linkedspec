...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Dart typed live capture `.10.5.5.1` now exports exact immutable slot/result events plus an invocation-local sink across direct/loaded/reconstructed/traced/generated-plan engine routes without result/trace/diagnostic drift. Immutable observed-index derivation `.10.5.5.2` is active; governance remains 6/20/73 at rollout 3/9 and admission 2/6. Startup mdBook drift remains queued under `.23`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
