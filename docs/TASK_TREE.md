...
| `INTER-MATCH-GAP-CAPTURE` | `active` / `.1.2` recurring governance signoff-complete for intended atomic 220/300 | `.spec language evolution / lossless segmentation and source preservation` | `.1.0` landed the exact future v1 plan at `31f3e664`; `.1.1` landed the executable artifact/checker at `f58dfcb3`. `.1.2` adds repository-routed neutral recurrence, canonical opt-in, exact pending-route/storage/public guards, and 55 rejected mutations while rollout remains 1 complete + 8 pending. No named-slot, directive, accessor, parser/runtime, README, or public behavior is current. `.1.3` closes neutral governance from the next clean boundary; `.2-.7` own six runtimes and public admission. | [docs/tasks/INTER-MATCH-GAP-CAPTURE.md](docs/tasks/INTER-MATCH-GAP-CAPTURE.md) |
