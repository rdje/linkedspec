...
| `FUTURE-PARITY-BACKLOG` | `active` / `.14.3.2.1` private Perl authority signoff complete; `.14.3.2.2` next | `Overall roadmap - future parity backlog` | Typed-source `.14.2` and neutral transaction `.14.3.1` are composition-closed; dormant Perl RED `.14.3.2.0` landed at `59306f37` as atomic 184/300. `.14.3.2.1` adds one inside-out private authority with opaque frames/tokens, monotonic invocation/mark generations, snapshots, falsey-safe staging, terminal invalidation, exact neutral diagnostics, and 293 nested TAP assertions. Its unit proof and definitive canonical gate pass, but the final-path consumer remains unregistered and RED at the same four missing nodes; authored integration `.14.3.2.2` remains next and rollout stays 1/9. `TASK-TREE-METADATA-HYGIENE.5` separately owns active-frontier prevention; rendered readability remains queued/nonurgent. Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`; exclusion public closeout `.24.2` remains closed. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
