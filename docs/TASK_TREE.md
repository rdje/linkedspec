...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.4` done 2026-07-10 - ADR `0022` makes native in-memory host-language embedding primary; variant CLIs are thin adapters. Julia is 99/99 with repo-owned optional-SDK verification, and active PNT is `JULIA-BACKEND-PARITY.7.1`; Lua `.1.3` follows Julia closeout. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.4` done 2026-07-10 - `tools/run_julia_local.sh` owns package/CLI/99-fixture verification and shared CI opts in under `LINKEDSPEC_RUN_JULIA=1`. Full tests pass with 840 assertions and status `runtime-corpus-full`; `.7.1` is active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
only under `LINKEDSPEC_RUN_JULIA=1`, preserving SDK-independent defaults; `.7.1` is active.
