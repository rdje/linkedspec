...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Lua eager block values close at 104/104 under `.4.3.6.1`; lazy inline controls `.4.3.6.2` are active, user-function block dispatch stays `.5.1`, and first-class callable values stay future `.11.7`. Rename collision drift remains helper caveat `.5`; inspector `.13.1` and authoring arcs `.14`/`.15` remain queued. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `LUA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Lua third)` | Eager last values, block-local return, mutation, harray precedence, and receiver dispatch pass 104/104 under `.4.3.6.1`; lazy inline value controls `.4.3.6.2` are active. | [docs/tasks/LUA-BACKEND-PARITY.md](docs/tasks/LUA-BACKEND-PARITY.md) |
...
`{|params| ...}`. Lazy inline value controls `.4.3.6.2` are active.
