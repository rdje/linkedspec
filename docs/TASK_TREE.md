...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Root selection retains final five-backend recurring/public no-drift at 7/0/54, duplicate-slot recurring/public no-drift is closed at 7/0/59, and rule-local cursor recurring/public no-drift at 75 files / 8 complete + 0 pending / 60 mutations. Explicit repeated-OR action-result shape `.9.1.10` remains the AND/OR frontier. Neutral, ten-role Perl, and byte-identical 15-role Rust/Dart/Julia/dual-ABI Lua admissions are complete at 8 modes, 10 special cases, 35 mutations, and rollout 6 complete / 2 pending; recurring `.9.1.10.6` follows. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
