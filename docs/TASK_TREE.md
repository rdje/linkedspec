...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Julia behavior-free immutable-query authority audit `.10.6.5.0` is fully verified from clean calls closeout `61aa48bb`; detached authority, 19 hashes, 26 boundaries, and `.1-.4` are frozen pending the plan commit before private kernel `.1`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
