...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | `.1.2` done 2026-07-09 - Julia backend parity is scoped into `JULIA-BACKEND-PARITY`, with interpreter-first strategy and Julia-specific CLI ownership. Backlog Lua `.1.3` remains gated until Julia reaches its scoped milestone; active PNT work is delegated to `JULIA-BACKEND-PARITY.6.2.4.2.1` after anonymous capture boundaries moved shipped smoke to 13/31. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
| `JULIA-BACKEND-PARITY` | `active` | `Overall roadmap - future backend parity (Julia second)` | `.6.2.4.1` done 2026-07-10 - the complete direct anonymous capture family closes three hlink delimiter fixtures and routes EBNF logging to structural output. Full tests pass with 766 assertions and status `runtime-corpus-capture-boundaries`; shipped smoke is 13/31 and `.6.2.4.2.1` logical helpers are active. | [docs/tasks/JULIA-BACKEND-PARITY.md](docs/tasks/JULIA-BACKEND-PARITY.md) |
...
`.6.2.1` through `.6.2.4.1` have since landed; `.6.2.4.2.1` is active.
...
window is 13/31 and `.6.2.4.2.1` logical helpers are active.
