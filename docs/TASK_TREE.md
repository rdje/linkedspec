...
| `FUTURE-PARITY-BACKLOG` | `active` / exclusion public closeout `.24.2` complete; Rust typed core `.14.2.2.1` signoff-complete/commit-pending | `Overall roadmap - future parity backlog` | Capability exclusion freshness is public-closed under `FUTURE-PARITY-BACKLOG.24`. Typed source-location rollout remains 4/10 with 38 mutations. `.14.2.2.1` supplies an owned decoded-source authority plus opaque immutable Rust `Position`, `Span`, and ordered derived-text values. Exact focused 3/7/6/3, ordinary zero-test, one-error nested projection boundary, complete Rust/corpus/storage/CLI 66x2, and canonical doctrines/containment/relocation/CLI 66x2/RAM 53%/Phase 0 1,031 in 653s pass without helper routing, admission, neutral/schema/rollout, root-README, or mdBook claim changes. Commit/clean remain before projection `.2`; rendered readability stays queued/nonurgent. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
