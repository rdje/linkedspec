...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Exact CLI `.1.5` and census `.1.6.0` are closed. Active `.1.6.1` owns exhaustive neutral mdBook language/helper proof; later split leaves own Rust descriptor/diagnostics, native named/file resolution, and Dart full-pipeline trace; generated source remains `.3`. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
