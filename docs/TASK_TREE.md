...
| `FUTURE-PARITY-BACKLOG` | `active` | `Overall roadmap - future parity backlog` | Behavior-free shared Lua MCP audit `.10.9.6.0` is signoff-complete from clean Julia closeout `88bff190`: ADR `0061` freezes one exact Lua-5.1-compatible generated/runtime/server/wire source graph, one narrow C99 entropy/clock seam compiled per ABI, and independent unchanged PUC Lua/LuaJIT admission. Status remains 4/5 implementations + 4/6 runtimes with rollout pending; generated/runtime/decoded server `.10.9.6.1` follows only after the clean plan commit. | [docs/tasks/FUTURE-PARITY-BACKLOG.md](docs/tasks/FUTURE-PARITY-BACKLOG.md) |
...
documentation-only, and exact-cleanup signoff now pass; `.1` follows only after the clean plan commit.
