...
## Active Slice
- `FUTURE-PARITY-BACKLOG.9.1.1.2.4.0` — active task-tree-first after clean Dart admission commit `31cc78ae`.
  Exact default/POSIX shared primary is 31/65 twice: one markerless root failure plus 33 cursor-owned help/usage/
  trace failures. Native/generated bypass fallback works; normal validation blocks markerless loaded/normalized/
  emitted routes; failures/descriptor/trace remain legacy. Core `.1`, routes `.2`, cursor `.9.1.6`, admission `.3`
  is the frozen order. Corpus 105 passes; package exposes the known cursor help failure at 56/57. Book/KM 609/
  4,390, roadmap repair, root 34, four doctrines, mdBook, and canonical Perl root 7+5/cursor 288/reference primary
  65x2/Phase 0 1,031 in 641 seconds pass. Generated book/cache/log cleanup passes; commit remains. No Julia
  behavior or rollout state changes.
