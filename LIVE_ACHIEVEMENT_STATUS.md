...
- Capture-enabled native rules install selection/LMATCH/candidate before `LS`, retain it through target/action/
  `LE`, commit the accepted child-extended cursor before `IT`, and expose successful tails through `LX`/`EX`/`E`.
  Unflagged LS-before-selection and return-channel behavior are unchanged.
