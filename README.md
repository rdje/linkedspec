...
- Run `perl tools/check_language_capability_coverage.pl --report` for the current Dart/Julia ActionIR call-name
  ...
  outer function-record contract are closed under `.1.6.2`; diagnostic audit `.1.6.3.0` found Rust's real raw
  `Engine` string-result boundary, and typed implementation `.1.6.3.1` is active.
- Run the current backend-neutral primary CLI fixture baseline with `PERL5LIB= perl
  ...
  `.1.6.1` and exact outward descriptors `.1.6.2` are closed; typed Rust diagnostics `.1.6.3.1` is active.
