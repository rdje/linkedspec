...
backends. The exact neutral contract, ten-role Perl reference admission, and 15-role Rust admission are complete
at 3 complete / 5 pending. Rust now treats bare `OR` as minimum-one repetition, collects explicit repeated action
returns per hit across native/generated routes, preserves lifecycle authority and scalar pipe, and retains
generated-source v2; Dart, Julia, dual-ABI Lua, recurring, and public rollout remain
`FUTURE-PARITY-BACKLOG.9.1.10.3-.7`. Logical-helper
