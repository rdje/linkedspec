...
- Model scalar, array, harray/hash, and codeblock as the four portable value kinds. For callables whose signature
  ...
  reject exact selectors before execution with the portable `aggregate_selector_removed` fields. Dart rejection
  `.12.1.8.3.1` is implemented; bounded `spec.spec` variadic-regex bridge repair `.12.1.8.3.2` closes its complete
  gate before Julia.
