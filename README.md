...
- Run `perl tools/check_capability_conformance.pl` to validate the current 15-capability backend census, evidence
  ...
  Rust now has v1 identity/metadata/error alignment; exact neutral plan/trace alignment remains under `.3.1.3.2`
  before admission `.3.1.3.3`; census stays 56/2/2, followed by Rust breadth `.3.2`, Dart `.3.3`, Julia `.3.4`,
  and `.3.5`.
...
- Run `perl tools/check_language_capability_coverage.pl --report` for the current Dart/Julia ActionIR call-name
  ...
  contract `.3.1.1` is gated and Rust exact plan/trace alignment `.3.1.3.2` is active at census 56/2/2.
