...
- Run `perl tools/check_language_capability_coverage.pl --report` for the current Dart/Julia ActionIR call-name
  ...
  Dart full-pipeline trace `.1.6.5` is split: frontend/compiler `.1` is active, function/staged `.2` and composed
  admission `.3` follow.
- Run the current backend-neutral primary CLI fixture baseline with `PERL5LIB= perl
  ...
  native resolution `.1.6.4` is closed on all four backends; Dart full-pipeline trace `.1.6.5` is split with
  frontend/compiler `.1` active.
