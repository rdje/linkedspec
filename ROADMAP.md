...
- With `sdce.spec` now cleared, `portmap.spec` is the next highest blocker-reduction slice (`BLOCKED=1`, `TOP=bare_bit_slice`, `BLOCKERS=2` in the latest corpus scan); after that, only the one-rule tail specs `pplugin.spec` and `tkgui.spec` remain in the current blocker ranking.
