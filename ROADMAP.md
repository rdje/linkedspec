...
`.6.0-.4` are complete. All six private runtime rows are independently recomposed at 7/9 with 112 mutations; every admitted backend preserves private immutable
...
`progressive_span_dispatch`, and makes typed governance 12/2/170 while behavioral governance stays 7/9/112.
Public `.8`, staged `.14.7`, and combined `.14.8` retain their order. Capacity `.0` freezes, canonical `.1` implements,
...
progressive public no-drift `.14.6.8` is next. Its mandatory engineering-notes rollover publishes segment `4991`;
