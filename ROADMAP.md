...
| Inter-match gap capture | `active; .1.1 neutral artifact/checker complete as atomic 219/300` | Recover historical “super split” as automatic source-gap access between repeated OR/default action-edge matches, with target rules retaining their regex slots and lifecycle code. ADR `0045` accepts `@capture_gaps`, spacing-insensitive `name=/regex/`, and `Rule[name]` while rejecting blind-call/adjacency reinterpretation. | `.1.0` landed the behavior-free plan at `31f3e664`. `.1.1` provides `linkedspec-inter-match-gap-capture-v1` and its independent checker: exact absent-artifact RED, executable Unicode/empty/child/rollback/nested modeling, 8+10 authored fixtures, 3 sources, 16 transitions, 10 segmentations, 9 diagnostics, and 50 rejected mutations at 1 complete + 8 pending. Canonical signoff passes. The artifact is current; named slots, `@capture_gaps`, accessors, parser/runtime behavior, and public claims are not. `.1.2-.1.3` govern/recompose the neutral authority; `.2-.7` implement/admit six runtimes and public no-drift. Typed-source composition follows only after `.7`. |
