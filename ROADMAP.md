...
proves those owners unchanged, closes the Julia parent, and hands off shared Lua `.14.6.6`.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current on all six runtimes. Progressive neutral + Perl + Rust + Dart rollout is 4/9/103; Dart `.14.6.4` is closed with private immutable authority, dedicated-node/four-carrier behavior, ordinary discovery, and one exact canonical route. Capacity `.0-.2` composition-close Julia `.5`'s bounded owner; Julia `.14.6.5.0-.2` preserve the historical RED, add private immutable authority, and make four carrier routes GREEN at 62/62 while dormant. Mandatory history segment `4992` is finitely authorized by ADR `0086` at 21 files / 20 manifest lines and makes `.2` canonical. `.3-.4` own admission and recomposition. Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
