...
plus composed Perl, Rust, Dart, Julia, and dual-ABI Lua `.9.1.1.2.1-.5` are complete; only composed public
admission `.6` remains pending, so rollout is 6 complete / 1 pending. Clean preflight `.4.0`
...
139/139x2 green; package 177/177x2, primary 65/65x4, corpus 105/105x2, and governance 6/1/44 pass. Only final
public no-drift `.6` remains; current canonical Phase 0 is 1,031/1,031 in 641 seconds. Earlier route closeout passed root 7+5, cursor 288, primary 65x2, and Phase 0
...
| Root-rule selection precedence | `active; neutral + all five backends complete at 6/7; final public no-drift pending` | Explicit `--top-rule NAME` may select any declared rule and has priority over authored entry markers; otherwise the first authored `Rule::` is the default; when no marker exists, the first ordinary `Rule:` is the default. | Lua root admission `.5.3` composes the exact 15 roles from one shared source on PUC Lua and LuaJIT: topology RED 3/3x2 becomes 139/139x2, package is 177/177x2, primary is 65/65x4, corpus is 105/105x2, and root governance is 6/1/44. Authored selection fixtures use lifecycle `I`; fixed request-trace bytes retain `E`. Final recurring/public no-drift `.6` follows after this leaf's canonical commit. |
