...
| Phase 5 | `in progress` | Runtime modernization, diagnostics consistency, and reduced dynamic-eval fragility. | Runtime/diagnostic modernization has landed refactor groundwork, generated-handler failures at the `LinkedRE::or(...)` seam now raise normal eval-visible errors instead of hard exits, `run_get_pipeline(...)` now records structured failure context in injected `runtime_ctx->{last_error}`, `Runtime::run_get(...)` / `LinkedSpec::Get(...)` can now expose that runtime context through `runtime_ctx_ref => \$ctx`, `get_parser(...)` now keeps that same diagnostics continuity across parser-factory resolution/load failures and later compiler failures, parser-factory callback exceptions now also normalize into that same continuity channel while preserving deeper compile/runtime payloads when already present, `Runtime::run_get(...)` now also normalizes raw compiler-delegation dies into a fallback `runtime_owner:run_get_pipeline` payload when no deeper owner payload exists yet while still preserving richer deeper payloads when they do, the structured `last_error` payload now carries self-contained `owner_stage`, `spec_name`, and `spec_path` metadata when known, compiler-stage validation/parse/descriptor-build exceptions now also normalize into that same `compiler_pipeline` payload family (`validate_spec_content`, `validate_dsl_syntax`, `bootstrap_parse`, `spec_descr`, `build_final_descr`, `validate_gdata_references`) instead of bypassing it as raw dies, and parser invocation failures now promote into the same `runtime_ctx->{last_error}` channel as `runtime_handler` / `runtime_parser` payloads with rule-label context, but the phase-level behavior work is not complete yet. |
