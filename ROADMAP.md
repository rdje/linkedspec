...
admission remain unchanged; no-change query closeout `.10.6.5.4` is next after the clean commit.
...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus remains 105/105. Cursor, root, and repeated-action exact 15-role admissions are complete. Exact Unicode-17 labels, opaque strict source/compiled outcome, all five private static targets, and private calls/staging/generated 22/25 are composition-closed through `.10.6.4.3`. Query authority/split `.10.6.5.0`, immutable kernel `.1`, traversal/limits `.2`, and public typed/raw surface `.3` now match all 19 static hashes and all 26 malformed boundaries; no-change closeout `.4`, runtime observation `.6`, and admission `.7` remain. Governance stays 4/9 rollout and 3/6 native admission. Hand-authored root fixtures use entry lifecycle `I`; root/cursor/repeated-action recurring and public proof is closed. |
