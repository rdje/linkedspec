...
- With `pplugin.spec` now cleared, only `tkgui.spec` (`TOP=sub_gui`) remains in the current one-rule tail of the blocker ranking; `tkgui.spec` is the next deterministic slice to inspect.
