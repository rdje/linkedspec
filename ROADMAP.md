...
are executable through six fixture groups, 20 digest-locked responses, and 81 rejected mutations. Static rule
...
explanation. Perl, Rust, and Dart are each admitted through one exact 12-role consumer, so neutral rollout is 4
complete / 5 pending and backend admission is 3 complete / 3 pending. Dependency-ordered `.10.3-.10.10` own
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Lua `.8.4` closes the fifth backend at capability 80/0/0. Cursor/root/duplicate/repeated parents are closed. Semantic governance is six groups / 20 responses / 81 mutations at rollout 4/9 and admission 3/6; Perl `.10.3`, Rust `.10.4`, and Dart `.10.5` are admitted through exact 12-role consumers. Julia authority/Unicode `.10.6.0-.1`, strict source/outcome `.10.6.2`, private static projection `.10.6.3`, private calls/staging/generated `.10.6.4`, and immutable query `.10.6.5` are composition-closed without promotion. Query matches all 19 static hashes and all 26 malformed boundaries; typed capture, immutable twentieth-digest derivation, and generated/emitted propagation are implemented through `.10.6.6.3`, while no-change closeout `.4` and admission `.7` remain. `.22` owns immutable-marker coupling and `.23` owns full-book semantic drift repair. Later backends inherit pinned Unicode-label alignment; recurring proof, MCP, and public no-drift remain pending. Format/inter-match activation and mutation execution remain separate; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus remains 105/105. Cursor, root, and repeated-action exact 15-role admissions are complete. Exact Unicode-17 labels, opaque strict source/compiled outcome, all five private static targets, private calls/staging/generated 22/25, immutable static query, typed native runtime capture, observed-index derivation, and generated/emitted observation propagation are complete through `.10.6.6.3`. All 20 typed/raw-neutral hashes and all 26 malformed query boundaries now match; no-change runtime-observation closeout `.6.4` and admission `.7` remain. Governance stays 4/9 rollout and 3/6 native admission. Hand-authored root fixtures use entry lifecycle `I`; root/cursor/repeated-action recurring and public proof is closed. |
