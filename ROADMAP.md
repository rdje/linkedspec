...
are executable through six fixture groups, 20 digest-locked responses, and 65 rejected mutations. Static rule
...
explanation. Perl is admitted through one exact 12-role consumer, so neutral rollout is 2 complete / 7 pending and
backend admission is 1 complete / 5 pending. Dependency-ordered `.10.3-.10.10` own Perl/Rust/Dart/Julia/Lua,
...
now makes the checker require the exact 6/20/65, rollout 2+7, admission 1+5, runtime, and Perl-admission claims while
self-proving omission/wrong-value rejection. Completed Rust prerequisite `.10.4.0.2` implements ADR `0051` with
...
22 records / 25 relations without query or promotion; immutable query `.10.4.4` follows. Repeated canonical
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, Rust mutation testing, and backend implementation companion books. | Lua `.8.4` closes the fifth backend at capability 80/0/0. Rule-local cursor/root/duplicate-slot/repeated-action parents are closed with recurring/public proof. ADRs `0049`/`0050` and neutral `.10.2` freeze semantic introspection through six groups, 20 exact responses, and 65 mutations. Perl `.10.3` is composed-admitted through one exact 12-role consumer, all 20 responses, rollout 2/9, and native admission 1/6. Toolbox repair `.10.4.0.1` makes the checker own those current-state claims and prove omission/wrong-value rejection. Completed Rust prerequisite `.10.4.0.2` implements ADR `0051` through generated Unicode 17.0.0 `XID_Continue` classification and exact source/artifact/runtime proof. Completed source/outcome `.10.4.1` retains strict copied bytes/text, exact scalar coordinates, source ceilings, private compiled-or-failed state, entry identity, and shared generated-v2 plan without target execution or host leakage. Completed static `.10.4.2` adds exact private graph/privacy/failure/runtime-static v1 projection; completed `.10.4.3` adds exact source-preorder calls, bindings, staging directions, and generated provenance at 22 records / 25 relations without query or promotion. Immutable query `.10.4.4` follows. `.22` tracks a stable home for immutable cross-contract markers currently coupled to mutable task-index rows; `.23` owns full-book semantic drift repair and stronger migration-example guards. Later backends inherit the pinned label prerequisite before their semantic fixture admission; recurring proof, MCP, and public no-drift remain pending. Parenthesis-free conditions remain excluded. Helper owner `.5` retains diagnostic, constructor/transform/truthiness/control, `start_capture_slice()`, and zero-argument `capture_until_boundary()` drift. Format/inter-match activation remains separate; mutation execution remains explicit; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
