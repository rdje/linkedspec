...
- Phase 1A (LinkedSpec.pm modularization): In progress and far advanced.
  ...
  - Landed follow-up: `LinkedSpec.pm` now lazy-loads `Runtime`, `Compiler`, and `ActionRewriter` through `_require_pkg(...)` inside `Get(...)`, `spec_descr(...)`, and `call_spec_handler_subst(...)`, so `require LinkedSpec` no longer drags the compile pipeline and ActionIR stack into memory up front.
  ...
  - Landed follow-up: `LinkedSpec::RuleIR::EmitContext` now lazy-loads `ActionRewriter` through owner helpers, so require-only consumers of `EmitContext.pm` do not import the action rewrite owner until emit-context build paths actually need rewrite helpers.
  ...
  - Landed follow-up: `LinkedSpec::RuleIR::EmitContext` now routes rewrite-rule construction/execution through `LinkedSpec::ActionIR::RewritePipeline` and diagnostic accumulation through `LinkedSpec::ActionIR::Diagnostics` directly, so emit-context assembly no longer depends on the thin `LinkedSpec::ActionRewriter` wrapper layer for that path; `ActionRewriter` is only lazy-loaded indirectly when rewrite-pipeline dep maps resolve lowering callbacks.
