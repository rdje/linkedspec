...
| Inter-match gap capture | `active; neutral parent .1 signoff-complete in intended atomic 221/300` | Recover historical “super split” as automatic source-gap access between repeated OR/default action-edge matches, with target rules retaining their regex slots and lifecycle code. ADR `0045` accepts `@capture_gaps`, spacing-insensitive `name=/regex/`, and `Rule[name]` while rejecting blind-call/adjacency reinterpretation. | `.1.0` landed the behavior-free plan at `31f3e664`, `.1.1` the executable artifact at `f58dfcb3`, and `.1.2` rooted recurrence/no-overclaim at `0490522b`. `.1.3` independently recomposes those bytes, 55 mutations, exact six pending skips, storage, public absence, duplicate-slot 59, and typed-source 9/5/114 without behavior movement. Canonical CI passes CLI 66x2, RAM 61%, Phase 0 1,031/1,031 in 725 seconds, the exact opt-in route, and exit 0, closing neutral parent `.1`. Rollout remains 1 complete + 8 pending. Named slots, `@capture_gaps`, accessors, parser/runtime behavior, and public admission remain absent; Perl `.2` is next after the clean landing and `.3-.7` retain later runtime/public ownership. |
