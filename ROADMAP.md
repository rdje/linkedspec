...
is signoff-complete from that clean boundary; dynamic invocation `.11.6.2` is next after the workflow commit and
...
6. Drive `.spec` toward language-agnostic action semantics (no embedded Perl code-block dependency in final state).
   ...
   - ADR `0031` / `FUTURE-PARITY-BACKLOG.11.1` now settle the corrective design: callable codeblocks use
     ...
     Knowledge Map, mdBook, doctrine, and canonical signoff are green; `.11.6.2` follows only after the workflow
     commit and clean-boundary proof.
