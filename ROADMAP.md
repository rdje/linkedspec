...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus remains 105/105. Cursor, root, and repeated-action exact 15-role admissions are complete. Exact Unicode-17 labels, opaque strict source/compiled outcome, all five private static targets, private calls/staging/generated 22/25, immutable static query, and typed native runtime capture are complete through `.10.6.6.1`. Query matches all 19 static hashes and all 26 malformed boundaries; observed-index derivation/generated propagation/closeout `.6.2-.4` and admission `.7` remain. Governance stays 4/9 rollout and 3/6 native admission. Hand-authored root fixtures use entry lifecycle `I`; root/cursor/repeated-action recurring and public proof is closed. |
