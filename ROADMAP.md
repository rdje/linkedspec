...
- Recent landed follow-up: parser-source chunk allocation/configuration/emission now also lives in `LinkedSpec::RuntimeContext`, so `Runtime.pm`, `Compiler.pm`, and `SpecEntry.pm` no longer each carry their own small parser-source callback/chunk-management helpers; focused regression coverage now locks the shared helper behavior directly.
- Recent landed follow-up: `top_rule` reset/update and resolved `spec_path` assignment now also live in `LinkedSpec::RuntimeContext`, so `Runtime.pm`, `ParserFactory.pm`, and `SpecEntry.pm` no longer hand-edit those runtime-context fields directly; focused regression coverage now locks the shared helper behavior for those state fields too.
...
| Phase 5 | `in progress` | Runtime modernization, diagnostics consistency, and reduced dynamic-eval fragility. | Runtime/diagnostic modernization has landed refactor groundwork, generated-handler failures at the `LinkedRE::or(...)` seam now raise normal eval-visible errors instead of hard exits, `run_get_pipeline(...)` now records structured failure context in injected `runtime_ctx->{last_error}`, compiler-owner setup failures now also normalize into that same `compiler_pipeline` family at stage `prepare_pipeline` when callback/runtime-owner preparation fails before the main validation/parse flow, `Runtime::run_get(...)` / `LinkedSpec::Get(...)` can now expose that runtime context through `runtime_ctx_ref => \$ctx`, `get_parser(...)` now keeps that same diagnostics continuity across parser-factory resolution/load failures and later compiler failures, the runtime-context hook now also accepts direct shared hashrefs like `runtime_ctx_ref => \%ctx` in addition to scalar slots, parser-factory setup failures now also normalize into that same `parser_factory` family at stage `prepare_parser_factory` when callback/trace preparation fails before normal validation/resolution begins, parser-factory callback exceptions now also normalize into that same continuity channel while preserving deeper compile/runtime payloads when already present, `Runtime::run_get(...)` now also normalizes raw compiler-delegation dies into a fallback `runtime_owner:run_get_pipeline` payload when no deeper owner payload exists yet while still preserving richer deeper payloads when they do, the structured `last_error` payload now carries self-contained `owner_stage`, `spec_name`, and `spec_path` metadata when known, compiler-stage validation/parse/descriptor-build exceptions now also normalize into that same `compiler_pipeline` payload family (`prepare_pipeline`, `validate_spec_content`, `validate_dsl_syntax`, `bootstrap_parse`, `spec_descr`, `build_final_descr`, `validate_gdata_references`) instead of bypassing it as raw dies, parser-factory stages now also include `prepare_parser_factory`, `validate_spec_name`, `resolve_spec_path`, `load_spec_content`, and `compile_spec` in that same structured owner-story, injected `runtime_ctx` is now also the sole parser-source emission owner for `SpecEntry::compile_spec_entry(...)` instead of sharing that responsibility with a stale direct `emit_parser_source_line` dependency slot, compiler-owned parser-source chunk setup now also lives in `LinkedSpec::RuntimeContext` instead of `Compiler.pm` hand-preparing that state directly, runtime-owned `runtime_ctx_ref` plus `dump_parser_source` option preparation now also lives in `LinkedSpec::RuntimeContext` instead of `Runtime.pm` hand-orchestrating that option normalization and setup step directly, and parser invocation failures now promote into the same `runtime_ctx->{last_error}` channel as `runtime_handler` / `runtime_parser` payloads with rule-label context, including `resolve_top_rule_handler` when the selected top rule or handler coderef contract is missing, but the phase-level behavior work is not complete yet. |
