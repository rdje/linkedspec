...
are executable through six fixture groups, 20 digest-locked responses, and 57 rejected mutations. Static rule
...
explanation. Neutral rollout is 1 complete / 8 pending and backend admission is 0 complete / 6 pending.
Dependency-ordered `.10.3-.10.10` own Perl/Rust/Dart/Julia/Lua, recurring, MCP, and public rollout. Behavior-free
...
function locks preserve character-coordinate and rule-scan authority boundaries. No public query or admission
exists. Neutral rollout remains 1/9 and native admission 0/6; `.10.3.4-.10.3.6` own query, observations/routes,
and admission.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, Rust mutation testing, and backend implementation companion books. | Lua `.8.4` closes the fifth backend at capability 80/0/0 after exact native, corpus, CLI, and generated-source proof. Rule-local cursor rollout is closed at 75 files / 8 complete + 0 pending / 60 mutations; `tools/check_rule_local_cursor_five_backend.sh` composes all six runtime legs plus selected 5x2x5 primary, support, and public no-drift proof. Duplicate-slot rollout is closed at 7 complete / 0 pending and 59 mutations through `tools/check_duplicate_regex_slot_identity_five_backend.sh`. Repeated-action rollout is closed at 8 complete / 0 pending with 54 mutations. `tools/check_repeated_action_result_five_backend.sh` composes the six runtime legs, one selected 5x2 primary case, and generated/capability/language-coverage support ledgers; every backend treats bare `OR` as minimum-one `rep_acode`/`rep_bcode`, explicit repeated action returns collect per hit, lifecycle remains whole-rule authority, pipe remains scalar, and generated-source stays v2. Public no-drift closes the four complete semantic parents `.9.1.1`, `.9.1.10`, `.9.1`, and `.9`. ADRs `0049`/`0050` and neutral leaf `.10.2` freeze the immutable semantic model/query through six fixture groups and 20 exact responses. Static correction `.10.3.2.0` reaches 53 mutations; generated-plan correction `.10.3.3.0` proves exact v2 family `default` and reaches 55; spec-identity correction `.10.3.3.1.0` derives exact `calls_and_staging` from caller identity and reaches 57, all without behavior/admission. Perl `.10.3.0-.10.3.2.1` provide authority/source/static layers; fully corrected `.10.3.3.1.1` now projects all 22 calls/staging/generated records and 25 relations privately. Public query/runtime/admission remain `.10.3.4-.10.3.6`; neutral rollout remains 1 complete / 8 pending and backend admission 0 complete / 6 pending. Parenthesis-free condition headers remain excluded. Helper owner `.5` retains diagnostic, constructor/transform/truthiness/control, `start_capture_slice()`, and zero-argument `capture_until_boundary()` drift. The full-backend parity prerequisites for ADR `0034`/`.18`, ADR `0040`/`.21`, and inter-match gap capture are satisfied, while activating those programs remains separately task-tree owned. Mutation execution remains explicit and non-routine; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
