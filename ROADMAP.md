...
closes parent `.10.9.3`; Dart `.10.9.4` follows only after the clean closeout commit.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Lua `.8.4` closes the fifth backend at capability 80/0/0. Cursor/root/duplicate/repeated parents are closed. Perl `.10.3`, Rust `.10.4`, Dart `.10.5`, Julia `.10.6`, and dual-ABI Lua `.10.7` are admitted through exact 12-role semantic consumers. Recurring proof `.10.8` composes them unchanged with three 5x2 primary cases and three support ledgers; governance is six groups / 20 responses / 105 mutations at rollout 7/9 and admission 6/6. ADR `0054` fixes MCP `.10.9` as one exact contract, five native implementations, and six runtime admissions; `.10.9.0` is complete. ADR `0055` and `.10.9.1.0` select modern MCP `2026-07-28` stdio; `.10.9.1.1` pins the shared machine artifacts, `.10.9.1.2` independently rejects 68 mutations, and `.10.9.1.3` closes ordered canonical governance. ADR `0057` and behavior-free Perl `.10.9.2.0` freeze the native/wire/security seams; generated binding plus registry/decoded dispatch `.10.9.2.1` and strict stdio/lifecycle `.10.9.2.2` are complete with canonical signoff. Exact Perl admission `.10.9.2.3` is complete at 1/5 implementations, 1/6 runtimes, rollout pending, and 28 rejected mutations with canonical signoff. No-change `.10.9.2.4` and parent `.10.9.2` are complete from clean `28f84826`; focused/canonical recomposition is green. ADR `0058` and behavior-free Rust `.10.9.3.0` freeze shared generation, native decoded server, strict stdio, admission, and closeout under `.1-.4`. Rust `.10.9.3.1-.3` implement the binding, frozen runtime, secure decoded server, strict borrowed-stream stdio, and exact admission at 2/5 implementations + 2/6 runtimes with rollout pending and 39 rejected mutations. No-change `.10.9.3.4` recomposes every committed owner under focused/canonical signoff and closes parent `.10.9.3`; Dart `.10.9.4` is next after the clean closeout commit. Director-approved ADR `0056` and behavior-free `.14.0.1` now completely capture a typed source-location/cursor algebra and split pending `.14.1-.8`; no syntax or behavior moves. Public no-drift remains pending. `.22` owns immutable-marker coupling and `.23` owns full-book semantic drift repair. Format/inter-match activation and mutation execution remain separate; inspector `.13.1` and `.15` remain queued. |
