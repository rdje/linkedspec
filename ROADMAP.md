...
| Inter-match gap capture | `active; Rust parent .3 signoff-complete for intended atomic 232` | Recover historical “super split” as automatic source-gap access between repeated OR/default action-edge matches, with target rules retaining their regex slots and lifecycle code. ADR `0045` accepts `@capture_gaps`, spacing-insensitive `name=/regex/`, and `Rule[name]` while rejecting blind-call/adjacency reinterpretation. | Perl and Rust are privately admitted at governance 3/6/56 plus ten Rust admission mutations. Rust composes native, reconstructed, descriptor, generated-plan, emitted, lifecycle, recursion/rollback, diagnostic, and real primary-command roles exactly once; the mixed input `alpha, beta | gamma\n- delta` proves separator-agnostic item extraction with exact gaps. The consumer runs ordinarily, canonically, and once after Perl in the rooted route. Four later runtime routes and both public rows remain pending. Recognition remains 137/246/58, language 122 public helpers, typed source 9/5/114, generated plan v2 unchanged, and all outward surfaces guarded. |
