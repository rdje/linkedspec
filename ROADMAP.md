...
are executable through six fixture groups, 20 digest-locked responses, and 73 rejected mutations. Static rule
...
explanation. Perl and Rust are each admitted through one exact 12-role consumer, so neutral rollout is 3 complete /
6 pending and backend admission is 2 complete / 4 pending. Dependency-ordered `.10.3-.10.10` own
...
runtime-observation audit `.10.5.5.0` now freezes the exact live/generated authority map and `.1-.4` dependency
split; typed live capture `.10.5.5.1` is active and admission remains later.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Lua `.8.4` closes the fifth backend at capability 80/0/0. Cursor/root/duplicate/repeated parents are closed. Semantic governance is six groups / 20 responses / 73 mutations at rollout 3/9 and admission 2/6; Perl `.10.3` and Rust `.10.4` are admitted. Dart Unicode/source/static/calls foundations are complete. Query `.10.5.4.0-.4` is composition-closed: fresh detached projection is the sole evaluator input and public typed/raw-neutral paths match all 19 static digests plus 26 validation boundaries. Runtime audit `.10.5.5.0` freezes post-match slot/final-result capture, generated exception identity, immutable derivation, and the `.1-.4` split; typed live capture `.1` is active. `.22` owns immutable-marker coupling and `.23` owns full-book semantic drift repair. Later backends inherit pinned Unicode-label alignment; admission, recurring proof, MCP, and public no-drift remain pending. Format/inter-match activation and mutation execution remain separate; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
