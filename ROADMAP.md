...
rollout, or admission row moves. Completed negative/isolation audit `.10.7.1.3.0` finds one pre-existing dual-ABI body-fluent
suffix-loss defect: `.Töp()`/`.A·B()` validate as ASCII-prefix method `T`/`A` because the adapter discards the
unconsumed remainder. Active `.3.1` owns only whole-token repair; `.3.2` owns the exhaustive negative/trust/artifact/
runtime and adjacent-grammar proof before `.3` closes.
