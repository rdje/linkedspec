...
- With `lib_reader.spec` now cleared, `sdce.spec` is the next highest blocker-reduction slice (`BLOCKED=2`, `TOP=sdc_esplit`, `BLOCKERS=5` in the latest corpus scan); inspect whether its blockers are another helper-friendly cleanup before touching the one-rule tail specs.
