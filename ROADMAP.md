...
implementations + 4/6 runtimes, leaves shared rollout pending, and rejects 79 mutations. `.4` next closes the
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 105 mutations, rollout 7/9, and native admission 6/6. MCP ADRs `0054`/`0055` freeze one modern `2026-07-28` contract, five implementations, and six runtime admissions; neutral `.10.9.1`, Perl `.10.9.2`, Rust `.10.9.3`, and Dart `.10.9.4` are parent-closed. Julia `.10.9.5.1-.3` now ships generated/runtime/decoded/strict-stdio owners and one exact twelve-role admission consumer; the ledger is 4/5 implementations + 4/6 runtimes, rollout pending, with 79 rejected mutations. No-change Julia closeout `.4` and shared Lua `.10.9.6` follow. Director-approved typed source-location direction `.14.0.1` is complete and `.14.1-.8` remain pending. Public no-drift, `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
