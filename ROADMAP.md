...
final independent recomposition; combined `.14.8` is complete at 14/0/231. Standalone rule-item block audit
`.15.0` ratifies `{ ... }` as source-level shorthand for lifecycle `I { ... }` at the same position. `.15.1`
implements the exact neutral contract on Perl and Rust, including ordered duplicate `I` repair on Rust; current
cross-backend authors must retain the marker until remaining-backend/self-hosted `.15.2` lands.
