...
Composed route `.5.2` now passes 101 assertions per ABI across loaded/normalized, generated-v1/emitted direct/
traced, portable failure, low-trace, and plan-first routes while preserving contract v1/format 1 and the minimal
label/family plan. The exact 32/65x4 primary and 105/105x2 corpus boundaries do not move. Rollout still stays 5/7
until cursor `.9.1.7` and topology admission `.5.3` close. Canonical route closeout passes root 7+5, cursor 288,
primary 65x2, and Phase 0 1,031/1,031 in 643 seconds. Clean route commit `c3bdca44` activates behavior-free cursor
...
69/6+2/49 pass. Canonical Phase 0 is 1,031/1,031 in 647 seconds; the clean commit remains before separate root
admission.
...
| Root-rule selection precedence | `active; neutral + Perl + Rust + Dart + Julia complete at 5/7; Lua core/routes and cursor prerequisite complete` | Explicit `--top-rule NAME` may select any declared rule and has priority over authored entry markers; otherwise the first authored `Rule::` is the default; when no marker exists, the first ordinary `Rule:` is the default. | Julia admission `.4.3` is complete at exact 15 roles, package 3,428, primary 65x2, corpus 105, and 39 rejected mutations. Lua core `.5.1` passes 99 focused assertions per ABI, routes `.5.2` pass 101 per ABI, and cursor `.9.1.7` now reaches exact 119 composition assertions per ABI, package 177/177x2, primary 65/65x4, corpus 105/105x2, neutral 69/6+2/49, and canonical Phase 0 1,031/1,031. The clean cursor commit, dual-ABI root admission `.5.3`, and final recurring/public no-drift `.6` follow. |
