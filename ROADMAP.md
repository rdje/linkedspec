...
admission, and no-change closeout under `.1-.4`; ledger status remains 2/5 + 2/6 pending implementation.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 105 mutations, rollout 7/9, and native admission 6/6. MCP ADRs `0054`/`0055` freeze one modern `2026-07-28` contract, five implementations, and six runtime admissions; neutral `.10.9.1`, Perl `.10.9.2`, and Rust `.10.9.3` are parent-closed. The MCP ledger remains 2/5 implementations + 2/6 runtimes, rollout pending, with 39 rejected mutations. ADR `0059` and behavior-free Dart `.10.9.4.0` now freeze generated private-part binding/runtime, a same-process secure decoded server, strict duplicate-safe/canonical stdio, exact admission, and no-change closeout under `.1-.4`; implementation `.10.9.4.1` is next. Director-approved typed source-location direction `.14.0.1` is complete and `.14.1-.8` remain pending. Public no-drift, `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
