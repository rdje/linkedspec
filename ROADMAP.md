...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170 with transaction safety, recursive observation, lossless-gap composition, and progressive recurrence current on all six runtimes. Progressive private six-runtime admission, exact recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit `.14.7.0`, diagnostic repair `.1`, neutral authority `.2`, Perl `.14.7.3.0-.4`, and Rust dormant marker/provenance `.14.7.4.0-.1` are closed. The 143-check Perl oracle proves caller-frozen resolution/cache, breadth-first execution, policies, strict chains/shared resources, source rebasing, detachment, four fresh-authority carriers, serialized-authority absence, unchanged v1/v2 formats, and exact ordinary/canonical admission. Rust's outer-cfg consumer keeps ordinary discovery at zero tests; exact assignment-form `parse_job` now becomes one inert detached marker with strict literal options, exact text, and typed direct/ordered-derived Unicode-scalar provenance across native, reconstructed, generated-plan, and emitted logical carriers. Its sole final RED now names `.14.7.4.2` caller-frozen authority. Neutral+Perl with Rust dormant governance remains 79 mutations; Rust rollout, recurrence, public, and outward behavior stay pending. Two pre-existing Rust validation/trace defects found by focused proof must be task-tree-owned and repaired from the clean `.1` handoff before `.2`. Final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
