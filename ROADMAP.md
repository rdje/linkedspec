...
| Phase 5 | `in progress` | Runtime modernization, diagnostics consistency, and reduced dynamic-eval fragility. | Runtime/diagnostic modernization has landed refactor groundwork, generated-handler failures at the `LinkedRE::or(...)` seam now raise normal eval-visible errors instead of hard exits, `run_get_pipeline(...)` now records structured failure context in injected `runtime_ctx->{last_error}`, and `Runtime::run_get(...)` / `LinkedSpec::Get(...)` can now expose that runtime context through `runtime_ctx_ref => \$ctx`, but the phase-level behavior work is not complete yet. |
