...
are executable through six fixture groups, 20 digest-locked responses, and 89 rejected mutations. Static rule
...
explanation. Perl, Rust, Dart, and Julia are each admitted through one exact 12-role consumer, so neutral rollout is
5 complete / 4 pending and backend admission is 4 complete / 2 pending. Dependency-ordered `.10.3-.10.10` own
...
moved-root + reference 66x2 + Phase 0 1,031/656s pass. Exact dual-ABI admission `.10.7.7` is now active from clean
`17348041`: one unchanged twelve-role Lua consumer, identical PUC Lua/LuaJIT topology, nine omission mutations,
and Lua-only promotion are frozen before implementation. Governance remains 6/20/89 at 5/9 + 4/6.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Lua `.8.4` closes the fifth backend at capability 80/0/0. Cursor/root/duplicate/repeated parents are closed. Semantic governance is six groups / 20 responses / 89 mutations at rollout 5/9 and admission 4/6; Perl `.10.3`, Rust `.10.4`, Dart `.10.5`, and Julia `.10.6` are admitted through exact 12-role consumers. Lua Unicode `.10.7.1`, opaque source/outcome `.10.7.2`, private static projection `.10.7.3`, calls/staging/generated `.10.7.4`, immutable typed/raw-neutral query `.10.7.5`, and runtime observation `.10.7.6` are composition-closed without ledger movement; the complete semantic surface is exact at 20 hashes/focused 1,964 per ABI. Exact ordered dual-ABI admission `.10.7.7` is active task-tree-first from clean `17348041`, with one shared twelve-role source, identical two-row topology, nine mutations, and Lua-only promotion frozen before implementation. `.22` owns immutable-marker coupling and `.23` owns full-book semantic drift repair; recurring proof, MCP, and public no-drift remain pending. Format/inter-match activation and mutation execution remain separate; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
