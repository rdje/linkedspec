...
now has exact inert construction/state; `.11.5.2` owns dynamic invocation next.
...
6. Drive `.spec` toward language-agnostic action semantics (no embedded Perl code-block dependency in final state).
   ...
   - ADR `0031` / `FUTURE-PARITY-BACKLOG.11.1` now settle the corrective design: callable codeblocks use
     ...
     shape without a closure or body execution. Dynamic Dart `cb(args)` invocation remains `.11.5.2`; generic
     final-block equivalence remains `.11.5.3`.
