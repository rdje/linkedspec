...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Five-backend capability parity is 80/0/0. Semantic introspection remains exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6; MCP is complete/141. Typed-source internal value/helper parent `.14.2` is composition-closed at 8 complete / 6 pending / 53 mutations across all six runtimes. Behavior-free transaction audit `.14.3.0` is signoff-complete and proves save/restore is cursor-only compatibility state, same-label recursion shares one rule-label mark bucket, current recursion/repetition termination lacks the future structured progress errors, and ActionIR has no closed recognition-only effect taxonomy. It freezes monotonic invocation frames, opaque owner/generation-bound single-use tokens, cursor/boundary/invocation-mark snapshots, staged recognition, fail-closed transitive effects, cursor-only v1 progress, and exact neutral/Perl/Rust/Dart/Julia/shared-Lua/recurring/closeout dependencies without changing production, fixture, neutral artifact, schema, DSL, CLI, README, storage, hosted workflow, or current behavior. Reversed-span remains `.14.2`-owned; exact syntax/result exposure is next under `.14.3.1.0`; the combined recurring/public no-drift row remains `.14.8`. Book 79/14,232 KiB, Knowledge 791/6,538, all seven doctrines, canonical CLI 66/66 twice, RAM 63%, and Phase 0 1,031/1,031 in 708 seconds pass. The nonurgent rendered-readability tree stays queued; `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
