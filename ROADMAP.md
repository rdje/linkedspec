...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus remains 105/105. Cursor, root, and repeated-action exact 15-role admissions are complete. Repeated action results pass focused 162, package 3,711, shared primary 65/65x2, corpus 105, and the now-shared 6/2/35 neutral ledger while preserving lifecycle authority and generated-source v2. Hand-authored root fixtures use entry lifecycle `I`; root/cursor recurring proof remains closed. Dual-ABI Lua repeated-action parity is admitted; recurring proof `.9.1.10.6` is next. |
