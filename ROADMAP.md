...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus remains 105/105. Cursor, root, and repeated-action exact 15-role admissions are complete. Exact Unicode-17 labels, opaque strict source/compiled outcome, and all five private static targets are composition-closed; static closeout passes focused 389 and Julia 7,931 without public query or promotion. Calls/staging/generated authority is frozen at exact 22/25 under `.10.6.4.0`; implementation/closeout `.1-.3`, query, runtime observation, and admission remain `.10.6.4.1-.7`. Governance stays 4/9 rollout and 3/6 native admission. Hand-authored root fixtures use entry lifecycle `I`; root/cursor/repeated-action recurring and public proof is closed. |
