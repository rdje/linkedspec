...
admits the unchanged seven-group consumer through ordinary discovery and one exact canonical route. Julia/shared-Lua `.5-.6`, recurrence `.7`, public
...
stable ID preserved. The storage prerequisite is composition-closed. Julia `.14.6.5` is split into exact dormant
RED `.0`, private authority/core `.1`, dormant carriers `.2`, canonical admission `.3`, and independent
recomposition `.4`; `.1` now provides the unexported immutable registry/invocation/rebased-view core with a
complete dormant consumer while the 55-pass/one-RED final path remains exact, and `.2` is next.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current on all six runtimes. Progressive neutral + Perl + Rust + Dart rollout is 4/9/103; Dart `.14.6.4` is closed with private immutable authority, dedicated-node/four-carrier behavior, ordinary discovery, and one exact canonical route. Capacity `.0-.2` composition-close Julia `.5`'s bounded owner; Julia `.14.6.5.0-.1` freeze the 55-pass/one-RED final path and add its private immutable authority, while `.2-.4` own carriers, admission, and recomposition. Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
