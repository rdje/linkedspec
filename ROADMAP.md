...
| Phase 5 | `in progress` | Runtime modernization, diagnostics consistency, and reduced dynamic-eval fragility. | Runtime/diagnostic modernization has landed refactor groundwork, generated-handler failures at the `LinkedRE::or(...)` seam now raise normal eval-visible errors instead of hard exits, `run_get_pipeline(...)` now records structured failure context in injected `runtime_ctx->{last_error}`, `Runtime::run_get(...)` / `LinkedSpec::Get(...)` can now expose that runtime context through `runtime_ctx_ref => \$ctx`, and `get_parser(...)` now keeps that same diagnostics continuity across parser-factory resolution/load failures and later compiler failures, but the phase-level behavior work is not complete yet. |
