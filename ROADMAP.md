...
- Method-like DSL migration track: `in progress`.
  ...
  - Landed follow-up: the broader semicolon-light structured-block rule is now regression-locked on generic helper-only blocks too, so action-edge `{ ... }` chains and lifecycle `I { ... }` chains can be authored without `;` separators between top-level method statements while preserving the same lowering and migration metadata as the fluent baseline.
