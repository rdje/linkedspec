...
| Inter-match gap capture | `active; Dart parent .4 landed at 43ed1c8f; behavior-free Julia .5.0 active` | Recover historical “super split” as automatic source-gap access between repeated OR/default action-edge matches, with target rules retaining their regex slots and lifecycle code. ADR `0045` accepts `@capture_gaps`, spacing-insensitive `name=/regex/`, and `Rule[name]` while rejecting blind-call/adjacency reinterpretation. | Perl, Rust, and Dart are privately admitted at governance 4/5/57. Dart `.4.1-.4.5` provide pinned-Unicode authored/static metadata, private same-authority native execution, normalized reconstruction, compatible descriptors, direct/traced generated execution, independent emitted proof, existing-primary parity, and one exact nine-role ledger. Julia `.5.0` now freezes `.5.1-.5.5`: authored/static provenance and dormant consumer; same-recognition-authority native state; normalized reconstruction/descriptors/generated execution; one repository-routed emitted host; and existing-primary/nine-role admission. This planning leaf changes no Julia behavior or rollout. Both Lua ABIs, recurring/public rows, and every outward surface remain pending or unchanged. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, generated format 2/plan `{label,family}`, and legacy descriptor/reference fields unchanged. |
