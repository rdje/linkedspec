...
`.8`, staged `.14.7`, and combined `.14.8` retain their order. Before Julia `.14.6.5` can be task-split, independent
`FUTURE-PARITY-PARTITION-CAPACITY.0-.2` preserves every pressure ceiling and stable ID while adding and
recomposing one bounded `.14.6.5-.14.8` semantic member; this storage prerequisite changes no backend behavior.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current on all six runtimes. Progressive neutral + Perl + Rust + Dart rollout is 4/9/103; Dart `.14.6.4` is closed with private immutable authority, dedicated-node/four-carrier behavior, ordinary discovery, and one exact canonical route. Julia `.5` follows the behavior-free `FUTURE-PARITY-PARTITION-CAPACITY.0-.2` prerequisite that adds one bounded semantic task member without raising pressure limits; Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
