...
lands clean at `3a38338d`; dynamic invocation `.11.6.2` is signoff-complete from that boundary and awaits only
its workflow commit and clean-boundary proof before `.11.6.3` activation.
...
6. Drive `.spec` toward language-agnostic action semantics (no embedded Perl code-block dependency in final state).
   ...
   - ADR `0031` / `FUTURE-PARITY-BACKLOG.11.1` now settle the corrective design: callable codeblocks use
     ...
     and Phase 0 1,031/1,031 in 655 seconds complete signoff; only the workflow commit and clean proof remain
     before `.11.6.3` activation.
