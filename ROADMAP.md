...
and closes the adoption critical path. Recurring MCP `.10.9.7.1.1.4` now recomposes the committed rooted
promotion unchanged under an independent canonical opt-in gate and closes implementation parents `.1.1` and
`.1`; MCP-parent closeout `.10.9.7.2` follows its clean commit.
...
formal state is semantic 6/20/110 at rollout 8/9 + admission 6/6 and MCP 5/5 + 6/6 complete with 141 mutations.
No-change `.4` now repeats that chain plus canonical Phase 0 1,031/1,031 in 655 seconds, closes `.1.1` and `.1`
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 110 mutations, rollout 8/9, and native admission 6/6. MCP ADRs `0054`/`0055` freeze one modern `2026-07-28` contract, five implementations, and six runtime admissions; neutral `.10.9.1` and all implementation parents `.10.9.2-.6` are closed. ADR `0062` and `.10.9.7.1.0` record the former 17/20 blocker. Authorized repair `.10.9.7.1.1.1` atomically fixes the exact fact union, bounded contract string, explicit-component policy enforcement, five bindings, and five servers at MCP 35/10/10/76. All-twenty consumer proof `.2` is green across Perl/Rust/Dart/Julia/PUC-Lua/LuaJIT. Routed `.3` now composes those owners through rooted `tools/check_mcp_six_runtime.sh`, canonical opt-in, and omission-sensitive cross-ledger governance; focused proof is green and formal MCP state is 5/5 implementations + 6/6 runtimes with rollout complete/141. Unchanged closeout `.4` and MCP-parent `.2` remain ordered; the weaker claim is not selected. Director-approved typed source-location direction `.14.0.1` is complete and `.14.1-.8` remain pending. Public no-drift, `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
