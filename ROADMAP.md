...
nested-write `.19.6.1` now passes 436 assertions per ABI plus its complete gate; Lua bang and portable/public
...
requires no repository repair, closing Rust parent `.19.3`. Lua nested-write `.19.6.1` is canonical-signoff-
complete; Lua `map_leaves!` `.19.6.2` is next.
...
`.19.5.1`, and `.19.6.1`; Lua bang and portable/public admission remain pending.
...
| Rust dependency warning zero | `proposed; non-blocking` | Eliminate warning noise from maintained Rust dependency, generated-parser, and direct-crate builds by repairing each causal authority. | `RUST-DEPENDENCY-WARNING-ZERO.0` owns the reproduced 1,870-`pgen` / 26-`rgx-core` output baseline. `.1-.7` perform a unique-cause census, separate authored/generator/`rgx-core`/LinkedSpec repairs, integrate explicit nested pins, and enforce a warning-denying boundary. Broad suppression and blind bulk fixes are excluded; Lua `.19.6.2` remains the active feature frontier after intake. |
