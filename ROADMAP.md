...
`.15.0` now ratifies `{ ... }` as future source-level shorthand for lifecycle `I { ... }` at the same position;
current authors must retain the marker until Perl/Rust `.15.1` and remaining-backend/self-hosted `.15.2` land.
