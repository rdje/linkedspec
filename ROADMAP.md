...
- Frontend hardening gap:
  - `perl/LinkedSpec/Validation.pm` now covers current rule-label syntax, rule-paragraph regex-token validation, earlier mixed `->` / `=>` rule rejection, and malformed top-level edge-target indexing more faithfully than before, but it still trails other parts of the supported DSL surface.
