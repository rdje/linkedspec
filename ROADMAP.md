...
`progressive_span_dispatch`, and makes typed governance 12/2/170. Public `.8` corrects the behavioral recurring
...
1. Keep staged extraction as a first-class concept.
   - Current staged linked parsing status (2026-07-09):
     ...
     remain future runtime work. Behavior-free audit `.14.7.0` freezes their dependency
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170 with transaction safety, recursive observation, lossless-gap composition, and progressive recurrence current on all six runtimes. Progressive private six-runtime admission, exact recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit `.14.7.0`, diagnostic repair `.1`, neutral authority `.2`, Perl `.14.7.3.0-.4`, Rust `.14.7.4.0-.4`, and Dart dormant boundary `.14.7.5.0` are closed. The 143-check Perl oracle and admitted Rust consumer prove caller-frozen resolution/cache, breadth-first execution, policies, strict chains/shared resources, source rebasing, detachment, four fresh-authority carriers, serialized-authority absence, unchanged v1/v2 formats, and exact ordinary/canonical admission. Rust exact assignment-form `parse_job` becomes one inert detached marker with strict literal options, exact text, and typed direct/ordered-derived Unicode-scalar provenance. Its private frozen already-compiled registry provides pure resolution/cache, isolated current-depth policies, breadth-first recurrence, exact cycles/strict-decrease lineage, shared cancellation/resources, expiring safe points, and original-source rebasing. A host-only seed builds fresh registry/cache and recursive authority for native, reconstructed, validated generated-plan, and independently compiled emitted routes after the complete parent result; all four return equal detached output while logical emitted source contains no live authority. Dart now has one `test_dormant/` consumer: four neutral/v1/generic-carrier tests pass and one exact dedicated-marker/provenance RED remains, while production and discovery do not move. Neutral governance rejects 85 mutations, promotes only Perl plus Rust, and makes `.14.7.5.1` next. Public and outward behavior stay pending. Final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
