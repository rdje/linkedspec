...
- Phase 2 has now started with stricter rule-paragraph regex-token validation across both multiline and same-line rule bodies, and now also includes earlier rejection of malformed glued worded rule-mode suffixes such as `rule:ORX` / `rule::ANDX`, malformed glued edge-target suffixes such as `-> Rule-extra` / `=> Rule-extra`, malformed action-edge fluent starts such as `-> Rule.` / `-> Rule..push(...)`, unsupported blind-call fluent continuations such as `=> Rule .method(...)`, stray preamble text before the first rule paragraph, unsupported top-level garbage lines inside a rule paragraph, unsupported same-line rule-header filler after the rule start or leading regex cluster, malformed `@...` split-marker spellings, top-level-only rule-start detection inside open code blocks, and unclosed open blocks at EOF, but broader token/error hardening is still ahead.
- Exit criteria:
