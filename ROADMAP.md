...
3. Replace `call_spec_handler_subst()` regex-chain rewriting with a structured action rewriter.
   ...
   - Status: In progress (v1 pipeline + diagnostics + lowering-contract catalog + helper action-IR node metadata landed; full action AST/IR still planned).
...
- Continue Backbone Refactor Track action rewriter follow-up by adding structural parsing for helper argument payloads on top of the helper action-IR node metadata layer.
