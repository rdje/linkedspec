...
- With `vhdl::process_statement` now cleared, the current top blocked rules are `Lispish::parenthesis` (`raw=6`), `ds_vhistory::vhistory` (`raw=5`), and `vhdl::subprogram_body` (`raw=5`); prefer a contained `ds_vhistory` slice or the last remaining larger `vhdl::subprogram_body` rule before the larger `Lispish::parenthesis` stateful migration.
...
- Backbone Refactor Track: In progress.
  ...
  - Item 3 (`call_spec_handler_subst()` structured action rewriter): In progress.
    ...
    - Landed follow-up: `vhdl::process_statement` now lowers through canonical helper flow using `declare`, `assign`, and generalized `return(array(...))`, clearing the rule from the blocked set without adding new lowering contracts; regression lock `vhdl_process_statement_helper_flow_eliminates_raw_fallback` now verifies zero raw fallback, zero unresolved helpers, canonical node coverage, readiness, and that `vhdl` is down to only `subprogram_body`.
