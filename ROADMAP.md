...
1. Keep staged extraction as a first-class concept.
   - Current staged linked parsing status (2026-07-09):
     ...
     annotation/provenance `.14.7.3.1` and current-depth resolution/cache/policies `.14.7.3.2` are now private and
     dormant. The oracle has 133 GREEN top-level checks and one `.3` recursive-queue/bounds/rebased-diagnostics RED;
     no admission or public behavior moves. Mandatory segment `4991` is finitely authorized by ADR `0089` at
