...
Semantic-introspection design (2026-07-20): ADR `0049` / `FUTURE-PARITY-BACKLOG.10.1` accepts
...
are fixed; the outward descriptor remains a separate native compatibility projection. MCP will expose only
capabilities/query over a registered index handle and owns no compilation, path reads, semantic traversal, or
explanation. Dependency-ordered `.10.2-.10.10` own neutral contract, Perl/Rust/Dart/Julia/Lua, recurring, MCP, and
public rollout; `.10.2` is next. No current behavior changed in the design leaf.
