...
recomposition `.4`; the 55-pass/one-RED consumer is current without backend behavior, and `.1` is next.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current on all six runtimes. Progressive neutral + Perl + Rust + Dart rollout is 4/9/103; Dart `.14.6.4` is closed with private immutable authority, dedicated-node/four-carrier behavior, ordinary discovery, and one exact canonical route. Capacity `.0-.2` composition-close Julia `.5`'s bounded owner; Julia `.14.6.5.0` freezes a 55-pass/one-RED final-path consumer and `.1-.4` own authority, carriers, admission, and recomposition. Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
