...
| mdBook destination-root safety | `in progress; intake signoff-complete` | Ensure custom mdBook output is validated and executed against one repository-derived resolution base. | `MDBOOK-DESTINATION-ROOT-ALIGNMENT.0` durably owns the reproduced book-root-validation/repository-root-execution mismatch and exact RED/repair plan without behavior change. `.1` follows its clean commit to run mdBook from the book root, preserve default/absolute/environment behavior, prove all relative CLI forms and hostile rejections, then return the clean frontier to typed-source Perl work. This is independent of the nonurgent rendered-readability audit. |
