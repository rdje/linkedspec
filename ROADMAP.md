...
| Phase 5 | `in progress` | Runtime modernization, diagnostics consistency, and reduced dynamic-eval fragility. | Runtime/diagnostic modernization has landed refactor groundwork, generated-handler failures at the `LinkedRE::or(...)` seam now raise normal eval-visible errors instead of hard exits, `run_get_pipeline(...)` now records structured failure context in injected `runtime_ctx->{last_error}`, `Runtime::run_get(...)` / `LinkedSpec::Get(...)` can now expose that runtime context through `runtime_ctx_ref => \$ctx`, `get_parser(...)` now keeps that same diagnostics continuity across parser-factory resolution/load failures and later compiler failures, the structured `last_error` payload now carries self-contained `owner_stage`, `spec_name`, and `spec_path` metadata when known, compile-time descriptor-build exceptions now also normalize into that same `compiler_pipeline` payload family (`spec_descr` / `build_final_descr`) instead of bypassing it as raw dies, and parser invocation failures now promote into the same `runtime_ctx->{last_error}` channel as `runtime_handler` / `runtime_parser` payloads with rule-label context, but the phase-level behavior work is not complete yet. |
