...
| Inter-match gap capture | `active; Dart parent .4 signoff-complete for intended atomic 239; Julia .5 follows after clean landing` | Recover historical “super split” as automatic source-gap access between repeated OR/default action-edge matches, with target rules retaining their regex slots and lifecycle code. ADR `0045` accepts `@capture_gaps`, spacing-insensitive `name=/regex/`, and `Rule[name]` while rejecting blind-call/adjacency reinterpretation. | Perl, Rust, and Dart are privately admitted at governance 4/5/57. Dart `.4.1-.4.5` provide pinned-Unicode authored/static metadata, private same-authority native execution, normalized reconstruction, separate compatible descriptor projections, direct/traced generated execution, independently analyzed emitted execution, existing-primary parity, and one exact nine-role ledger. Its consumer runs ordinarily, canonically, and once after Rust in the rooted route; ten Dart admission mutations replace the dormancy fence. ADR `0074` preserves all 56 prior mutations and adds the missing Dart regression as 57. Exact canonical CI closes parent `.4`; Julia `.5` follows only after atomic-239 commit/brief/clean proof. Both Lua ABIs, recurring/public rows, and every outward surface remain pending or unchanged. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, generated plan v2 and legacy descriptor/reference fields unchanged. |
