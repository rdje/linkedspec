...
- With the small `Lispish` blockers now cleared, the current top blocked rules are `Lispish::parenthesis` (`raw=6`), `ds_vhistory::vhistory` (`raw=5`), `vhdl::subprogram_body` (`raw=5`), and `vhdl::process_statement` (`raw=4`); prefer a contained `ds_vhistory` slice or one of the two remaining larger `vhdl` rules before the larger `Lispish::parenthesis` stateful migration.
