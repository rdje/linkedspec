...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability remains 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2` and complete private Perl `.3`, Rust `.4`, and Dart `.5` parents are closed. Julia `.14.7.6.0` now owns one final-path dormant consumer: current function-body v1 and four generic observation routes pass 86 assertions before the sole missing `STAGED_PARSE_JOB_MARKER` / `staged_parse_job_v2` RED. Ordinary Julia and canonical CI omit the path, rollout and production remain unchanged, and neutral governance rejects 92 mutations after correcting stale Dart-availability prose and its invalid package-cwd command. Julia marker/provenance `.14.7.6.1` is next; shared Lua `.7`, recurrence `.8`, public `.9`, recomposition `.10`, final `.14.8`, reversed-span, hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
