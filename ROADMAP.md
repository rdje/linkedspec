...
| Inter-match gap capture | `active; .1.0 signoff-complete; .1.1 next` | Recover historical “super split” as automatic source-gap access between repeated OR/default action-edge matches, with target rules retaining their regex slots and lifecycle code. ADR `0045` accepts `@capture_gaps`, spacing-insensitive `name=/regex/`, and `Rule[name]` while rejecting blind-call/adjacency reinterpretation. | From clean `3d0384d1`, behavior-free `.1.0` signoff-completely freezes `linkedspec-inter-match-gap-capture-v1`, its independent checker, Unicode named-slot identity, selector provenance, directive eligibility, detached `entry_slot()`/gap records, exact prefix/interstitial/tail and empty-span lifecycle, post-`LE` commit, transaction/recursion isolation, diagnostics/mutations, nine rollout legs, repository-local routes, and carrier roles. Canonical proof passes doctrines, containment/relocation, CLI 66x2, RAM 52%, and Phase 0 1,031/1,031 in 735 seconds. These are planned—not implemented. `.1.1-.1.3` add and govern the neutral authority; `.2-.7` implement/admit six runtimes and public no-drift. Typed-source composition follows only after `.7`. |
