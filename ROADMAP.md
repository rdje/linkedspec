...
and LuaJIT; the bang suite passes 530 assertions per ABI. Portable/public admission remains pending.
...
close `.19.6`; portable/public admission `.19.7` is next.
...
`.19.5.1`, and `.19.6.1`; all five also implement the unchanged bang contract, while portable/public admission
remains pending.
...
| Rust dependency warning zero | `proposed; non-blocking` | Eliminate warning noise from maintained Rust dependency, generated-parser, and direct-crate builds by repairing each causal authority. | `RUST-DEPENDENCY-WARNING-ZERO.0` owns the reproduced 1,870-`pgen` / 26-`rgx-core` output baseline. `.1-.7` perform a unique-cause census, separate authored/generator/`rgx-core`/LinkedSpec repairs, integrate explicit nested pins, and enforce a warning-denying boundary. Broad suppression and blind bulk fixes are excluded; portable/public mutation admission `.19.7` is the active feature frontier. |
