...
| Root-rule selection precedence | `active; neutral + Perl + Rust + Dart + Julia complete at 5/7; Lua core/routes implemented` | Explicit `--top-rule NAME` may select any declared rule and has priority over authored entry markers; otherwise the first authored `Rule::` is the default; when no marker exists, the first ordinary `Rule:` is the default. | Julia admission `.4.3` is complete at exact 15 roles, package 3,428, primary 65x2, corpus 105, and 39 rejected mutations. Lua core `.5.1` passes 99 focused assertions per ABI; routes `.5.2` pass 101 per ABI, and cursor `.9.1.7.1-.4` now covers normalization, intrinsic execution, descriptor v1, and generated-source v2 through 106 dedicated / 2,027 focused assertions per ABI. The staged 176/177 package, 32/65x4 primary, and 105/105x2 corpus boundaries remain exact. Cursor `.5-.6`, dual-ABI root admission `.5.3`, and final recurring/public no-drift `.6` follow. |
