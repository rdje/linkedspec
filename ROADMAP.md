...
3. Structured block equivalence (Planned)
   ...
   - Control-flow design note agreed before implementation:
     ...
     - preferred staged switch direction is:
       ...
       - and marker-style `if(...) ... endif()` and marker-style `switch(...) ... endswitch()` are now explicitly tracked as allowing arbitrarily deep mutual nesting in structured block contexts, with representative deeper alternating regressions landed across action-edge and the full lifecycle family; practical limits are runtime recursion/resource limits, not a DSL-fixed nesting cap,
