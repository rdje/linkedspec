...
close `.19.6`; recurring proof `.19.8` follows completed portable capability admission `.19.7`, and public-current
no-drift plus parent closeout remain `.19.9`.
...
`.19.7` is complete at current census 100/0/0; recurrence `.19.8` and public closeout `.19.9` remain.
...
| Rust dependency warning zero | `proposed; non-blocking` | Eliminate warning noise from maintained Rust dependency, generated-parser, and direct-crate builds by repairing each causal authority. | `RUST-DEPENDENCY-WARNING-ZERO.0` owns the reproduced 1,870-`pgen` / 26-`rgx-core` output baseline. `.1-.7` perform a unique-cause census, separate authored/generator/`rgx-core`/LinkedSpec repairs, integrate explicit nested pins, and enforce a warning-denying boundary. Broad suppression and blind bulk fixes are excluded; recurring mutation proof `.19.8` is the next feature frontier. |
