...
- Phase 1A (LinkedSpec.pm modularization): In progress and far advanced.
  ...
  - Landed follow-up: `LinkedSpec::ActionRewriter` no longer imports `LinkedSpec::Deps` at load time, and the dead `LinkedSpec::Deps::declare_method_deps_for_package(...)` helper has been removed; the remaining active `LinkedSpec::Deps` surface is now limited to parser-factory wiring.
