...
1. Keep staged extraction as a first-class concept.
   - Current staged linked parsing status (2026-07-09):
     ...
     dormant routes, rollout, and exact ownership pass 72 reason-checked mutations. Perl dormant boundary
     ...
     rebased diagnostics `.3` are now private and dormant. The oracle has 141 GREEN top-level checks and one `.4`
     carriers/admission/Perl-rollout RED; no admission or public behavior moves. Mandatory segment `4991` remains
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170 with transaction safety, recursive observation, lossless-gap composition, and progressive recurrence current on all six runtimes. Progressive private six-runtime admission, exact recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit `.14.7.0`, diagnostic repair `.1`, neutral authority `.2`, and Perl private marker/provenance, current-depth policies, plus recursive authority `.14.7.3.1-.3` are closed. The dormant Perl oracle proves caller-frozen resolution/cache, deterministic breadth-first execution, all policies, strict chains/shared resources, source rebasing, detachment, smuggling rejection, and unchanged v1 through 141 GREEN top-level checks; only `.3.4` fresh carriers/admission/Perl rollout remain RED. No route, rollout, carrier, generated format, or public surface moves. Final `.14.8` no-drift, reversed-span and unrelated hygiene/readability, `.22`, `.23`, mutation-execution, inspector `.13.1`, and `.15` remain separately owned. |
