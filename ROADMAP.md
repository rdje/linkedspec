...
the clean boundary to shared Lua planning `.10.9.6.0`. Behavior-free Lua audit `.10.9.6.0` and ADR `0061` now
freeze one Lua-5.1-compatible generated/runtime/server/wire source graph, one package-private C99 secure-entropy/
monotonic-time seam compiled for both ABIs, strict pre-decode number-kind admission, and identical independent
PUC Lua/LuaJIT proof. Implementation `.1`, strict wire `.2`, exact dual-ABI admission `.3`, and no-change closeout
`.4` follow in order; the ledger remains 4/5 implementations + 4/6 runtimes with rollout pending before `.3`.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference and terse-format trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, Unicode structured text, Rust mutation testing, and backend companion books. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 105 mutations, rollout 7/9, and native admission 6/6. MCP ADRs `0054`/`0055` freeze one modern `2026-07-28` contract, five implementations, and six runtime admissions; neutral `.10.9.1`, Perl `.10.9.2`, Rust `.10.9.3`, Dart `.10.9.4`, and Julia `.10.9.5` are parent-closed. Julia's generated/runtime/decoded/strict-stdio owners and one exact twelve-role admission consumer preserve the ledger at 4/5 implementations + 4/6 runtimes, rollout pending, with 79 rejected mutations. Behavior-free shared Lua `.10.9.6.0` and ADR `0061` freeze one exact generated/runtime/server/wire implementation plus a narrow native entropy/clock seam for unchanged PUC Lua/LuaJIT admission; implementation `.1` is next and status remains 4/5 + 4/6. Director-approved typed source-location direction `.14.0.1` is complete and `.14.1-.8` remain pending. Public no-drift, `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
