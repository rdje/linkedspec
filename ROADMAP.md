...
projection and closes progressive no-drift without exporting an outward API. Staged `.14.7` is next; combined
`.14.8` retains its later owner. Capacity `.0` freezes, canonical `.1` implements,
...
mutations advance typed governance to 13/1/189 without promoting `.14.8`. Final recomposition
`.14.7.10` and combined typed `.14.8` remain separately owned.
