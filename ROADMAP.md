...
the complete dual-ABI Lua gate. Route identity/admission remain `.11.8.3-.4`. Manifest audit also queues unrelated exclusion-freshness governance
...
6. Drive `.spec` toward language-agnostic action semantics (no embedded Perl code-block dependency in final state).
   ...
   - ADR `0031` / `FUTURE-PARITY-BACKLOG.11.1` now settle the corrective design: callable codeblocks use
     ...
     failures, and ordered recursion at focused 232x2 plus complete dual-ABI Lua. Route proof stays `.3` and
     admission `.4`.
