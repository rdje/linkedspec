...
- mark writes and advancing mark updates now also call `_trace_runtime_mark_event(...)`, so high/debug trace output shows a short input excerpt with a caret under the stored checkpoint position; that includes `mark_copy(...)` too.
