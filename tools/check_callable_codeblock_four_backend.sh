...
linkedspec_project_data_enter_run "$REPO_ROOT/tools/check_callable_codeblock_four_backend.sh" "$@"
...
log() {
 printf '[callable-codeblock-four] %s\n' "$*"
}

fail() {
 printf '[callable-codeblock-four] ERROR: %s\n' "$*" >&2
 exit 1
}
...
for command in prove "$PYTHON_CMD" "$CARGO_CMD" "$DART_CMD" "$JULIA_CMD"; do
...
log "all four-backend callable-codeblock contract and authority routes pass"
