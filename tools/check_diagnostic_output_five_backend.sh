...
linkedspec_project_data_enter_run "$REPO_ROOT/tools/check_diagnostic_output_five_backend.sh" "$@"
...
log() {
 printf '[diagnostic-output-five] %s\n' "$*"
}

fail() {
 printf '[diagnostic-output-five] ERROR: %s\n' "$*" >&2
 exit 1
}
...
log "checking Perl native and generated consumers"
PERL5LIB= prove -Iperl t/diagnostic_output_perl_contract.t

log "checking Rust native and generated consumers"
"$CARGO_CMD" test --manifest-path rust/Cargo.toml -p linkedspec-runtime \
 --test diagnostic_output_contract

log "checking Dart native and generated consumers"
(
 cd dart
 "$DART_CMD" test test/diagnostic_output_contract_test.dart
)

log "checking Julia native and generated consumers"
...
log "building and checking PUC Lua native and generated consumers"
...
LINKEDSPEC_LUA_TEST_RUNTIME="$LUA_CMD" \
 "$LUA_CMD" lua/test/diagnostic_output_contract_test.lua

log "building and checking LuaJIT native and generated consumers"
...
log "checking exact quiet primary projection across five commands and two environments"
bash tools/run_primary_cli_matrix.sh --case success_diagnostic_helpers_quiet
...
log "all native, generated, primary, capability, and corpus diagnostic boundaries pass"
