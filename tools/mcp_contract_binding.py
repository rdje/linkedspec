"""Shared verified-bundle construction for generated LinkedSpec MCP bindings."""
...
class GenerationError(RuntimeError):
    """The neutral source bundle cannot produce a trustworthy language binding."""
...
def canonical_frames(path: Path, corpus: dict[str, Any]) -> dict[str, Any]:
    ...
    require(
        ...
        "canonical frame order is invalid",
    )
    ...
    for frame_id, row in zip(order, rows, strict=True):
        ...
        require(
            row.decode("utf-8") == canonical_text(value),
            f"canonical frame {frame_id} is not canonical JSON",
        )
...
def build_bundle() -> dict[str, Any]:
    """Return the canonical, digest-verified runtime subset for every binding."""
