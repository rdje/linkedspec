...
log() {
 printf '[progressive-span-dispatch-six] %s\n' "$*"
}

fail() {
 printf '[progressive-span-dispatch-six] ERROR: %s\n' "$*" >&2
 exit 1
}
...
log "checking the neutral progressive span-dispatch contract, rollout, and mutations"
bash tools/run_python_project_data.sh tools/check_progressive_span_dispatch_contract.py

log "checking the exact Perl progressive span-dispatch consumer"
PERL5LIB= prove -Iperl t/progressive_span_dispatch_perl_contract.t

log "checking the exact cfg-enabled Rust progressive span-dispatch consumer"
RUSTFLAGS='--cfg linkedspec_progressive_span_dispatch_red' "$CARGO_CMD" test --manifest-path rust/Cargo.toml -p linkedspec-runtime --test progressive_span_dispatch_contract

log "checking the exact Dart progressive span-dispatch consumer"
...
log "checking the exact Julia progressive span-dispatch consumer"
...
log "checking the shared progressive span-dispatch consumer on PUC Lua"
bash tools/run_lua_project_data.sh puc lua/test/progressive_span_dispatch_contract_test.lua

log "checking the shared progressive span-dispatch consumer on LuaJIT"
...
log "all six progressive span-dispatch runtime routes and support ledgers pass"
