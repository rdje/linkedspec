...
check_no_untracked_ci_inputs() {
 ...
 status_line=$(git status --short --untracked-files=all -- tools/check_progressive_span_dispatch_six_runtime.sh)
 if [[ "$status_line" == '?? '* ]]; then
  printf '[ci] ERROR: untracked CI input: %s\n' "${status_line#?? }" >&2
  found=1
 fi

 status_line=$(git status --short --untracked-files=all -- tools/check_staged_ast_enrichment_six_runtime.sh)
 if [[ "$status_line" == '?? '* ]]; then
  printf '[ci] ERROR: untracked CI input: %s\n' "${status_line#?? }" >&2
  found=1
 fi

 status_line=$(git status --short --untracked-files=all -- tools/check_typed_authoring_model_six_runtime.sh)
 if [[ "$status_line" == '?? '* ]]; then
  printf '[ci] ERROR: untracked CI input: %s\n' "${status_line#?? }" >&2
  found=1
 fi
...
require_tracked_file tools/check_progressive_span_dispatch_contract.py
require_tracked_file tools/check_recognition_transaction_contract.py
require_tracked_file tools/check_staged_ast_enrichment_contract.py
...
require_tracked_file tools/check_progressive_span_dispatch_six_runtime.sh
require_tracked_file tools/check_staged_ast_enrichment_six_runtime.sh
require_tracked_file tools/check_typed_authoring_model_six_runtime.sh
...
require_tracked_file capability_conformance/progressive_span_dispatch_contract.json
require_tracked_file capability_conformance/recognition_transaction_contract.json
...
progressive_span_dispatch_driver_matches=$(grep -nE '/(Users|home)/[^[:space:]]*|[[:alpha:]]:\\\\' tools/check_progressive_span_dispatch_six_runtime.sh || true)
[[ -z "$progressive_span_dispatch_driver_matches" ]] || fail "machine-specific absolute path(s) in progressive span-dispatch recurring driver:
$progressive_span_dispatch_driver_matches"
staged_ast_enrichment_driver_matches=$(grep -nE '/(Users|home)/[^[:space:]]*|[[:alpha:]]:\\\\' tools/check_staged_ast_enrichment_six_runtime.sh || true)
[[ -z "$staged_ast_enrichment_driver_matches" ]] || fail "machine-specific absolute path(s) in staged-AST enrichment recurring driver:
$staged_ast_enrichment_driver_matches"
typed_authoring_model_driver_matches=$(grep -nE '/(Users|home)/[^[:space:]]*|[[:alpha:]]:\\\\' tools/check_typed_authoring_model_six_runtime.sh || true)
[[ -z "$typed_authoring_model_driver_matches" ]] || fail "machine-specific absolute path(s) in typed authoring-model recurring driver:
$typed_authoring_model_driver_matches"
...
mutation_driver_matches=$(grep -nE '/(Users|home)/[^[:space:]]*|[[:alpha:]]:\\\\' tools/check_mutation_six_runtime.sh || true)
[[ -z "$mutation_driver_matches" ]] || fail "machine-specific absolute path(s) in mutation recurring driver:
$mutation_driver_matches"
...
bash -n tools/check_mutation_six_runtime.sh
...
log "checking backend-neutral progressive span-dispatch contract"
...
log "running exact Lua typed source-location value, projection, and recursive-observation admission consumers on PUC Lua"
...
log "running exact Lua typed source-location value, projection, and recursive-observation admission consumers on LuaJIT"
...
if [[ "${LINKEDSPEC_RUN_PROGRESSIVE_SPAN_MATRIX:-0}" == "1" ]]; then
 log "running optional six-runtime progressive span-dispatch matrix (LINKEDSPEC_RUN_PROGRESSIVE_SPAN_MATRIX=1)"
 require_tracked_file tools/check_progressive_span_dispatch_six_runtime.sh
 bash "$REPO_ROOT/tools/check_progressive_span_dispatch_six_runtime.sh"
else
 log "skipping optional six-runtime progressive span-dispatch matrix (set LINKEDSPEC_RUN_PROGRESSIVE_SPAN_MATRIX=1 when all backend toolchains are available)"
fi

if [[ "${LINKEDSPEC_RUN_STAGED_AST_ENRICHMENT_MATRIX:-0}" == "1" ]]; then
 log "running optional six-runtime staged-AST enrichment matrix (LINKEDSPEC_RUN_STAGED_AST_ENRICHMENT_MATRIX=1)"
 require_tracked_file tools/check_staged_ast_enrichment_six_runtime.sh
 bash "$REPO_ROOT/tools/check_staged_ast_enrichment_six_runtime.sh"
else
 log "skipping optional six-runtime staged-AST enrichment matrix (set LINKEDSPEC_RUN_STAGED_AST_ENRICHMENT_MATRIX=1 when all backend toolchains are available)"
fi

if [[ "${LINKEDSPEC_RUN_TYPED_AUTHORING_MODEL_MATRIX:-0}" == "1" ]]; then
 log "running optional composed typed authoring-model matrix (LINKEDSPEC_RUN_TYPED_AUTHORING_MODEL_MATRIX=1)"
 require_tracked_file tools/check_typed_authoring_model_six_runtime.sh
 bash "$REPO_ROOT/tools/check_typed_authoring_model_six_runtime.sh"
else
 log "skipping optional composed typed authoring-model matrix (set LINKEDSPEC_RUN_TYPED_AUTHORING_MODEL_MATRIX=1 to run all six existing recurring authorities)"
fi
...
if [[ "${LINKEDSPEC_RUN_RECOGNITION_TRANSACTION_MATRIX:-0}" == "1" ]]; then
 log "running optional six-runtime recognition-transaction matrix (LINKEDSPEC_RUN_RECOGNITION_TRANSACTION_MATRIX=1)"
 require_tracked_file tools/check_recognition_transaction_six_runtime.sh
 bash "$REPO_ROOT/tools/check_recognition_transaction_six_runtime.sh"
else
 log "skipping optional six-runtime recognition-transaction matrix (set LINKEDSPEC_RUN_RECOGNITION_TRANSACTION_MATRIX=1 when all backend toolchains are available)"
fi

if [[ "${LINKEDSPEC_RUN_MUTATION_MATRIX:-0}" == "1" ]]; then
 log "running optional six-runtime mutation matrix (LINKEDSPEC_RUN_MUTATION_MATRIX=1)"
 require_tracked_file tools/check_mutation_six_runtime.sh
 bash "$REPO_ROOT/tools/check_mutation_six_runtime.sh"
else
 log "skipping optional six-runtime mutation matrix (set LINKEDSPEC_RUN_MUTATION_MATRIX=1 when all backend toolchains are available)"
fi
