...
require_tracked_file tools/check_callable_codeblock_four_backend.sh
...
if [[ "${LINKEDSPEC_RUN_CALLABLE_CODEBLOCK_MATRIX:-0}" == "1" ]]; then
 log "running optional four-backend callable-codeblock matrix (LINKEDSPEC_RUN_CALLABLE_CODEBLOCK_MATRIX=1)"
 require_tracked_file tools/check_callable_codeblock_four_backend.sh
 bash "$REPO_ROOT/tools/check_callable_codeblock_four_backend.sh"
else
 log "skipping optional four-backend callable-codeblock matrix (set LINKEDSPEC_RUN_CALLABLE_CODEBLOCK_MATRIX=1 when all four toolchains are available)"
fi
