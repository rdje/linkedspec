...
spec_file::
 ...
 -> lifecycle_block_line { push(lifecycle_block_line, current) }
 -> lifecycle_block  { push(lifecycle_block, current) }
 -> lifecycle_fluent { push(lifecycle_fluent, current) }
 -> standalone_lifecycle_block { push(standalone_lifecycle_block, current) }
...
# ---- complete-line lifecycle block; wins ties with generic bare edges --------
lifecycle_block_line: /(?m:^[ \t]*(I|LS|LE|LX|E|EX|IT)[ \t]*(?<blkLBL>\{(?:[^{}"']++|"(?:\\.|[^"])*+"|'(?:\\.|[^'])*+'|(?&blkLBL))*+\})[ \t]*(?=\r?$))/
 I.return(hash("type", "lifecycle", "marker", entry_group(0), "code", entry_group(1), "source_form", "explicit"))
