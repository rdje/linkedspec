...
fn compiled_rule_json_roundtrip() {
    let rule = CompiledRule {
        ...
        acode_dispatch: vec![AcodeEntry { regex_idx: 0, child_label: "Child".into(), child_regex_idx: 0, code: None, has_parent_regex: true }],
...
fn runtime_value_json_conversion() {
    ...
    assert_eq!(nv.to_json(), serde_json::Value::Number(serde_json::Number::from(42)));
    ...
    // Bool
    assert_eq!(RuntimeValue::Bool(true).to_json(), serde_json::Value::Bool(true));
}
...
fn runtime_value_as_number() {
    ...
    assert_eq!(RuntimeValue::Scalar("not a number".into()).as_number(), None);
}
...
fn runtime_value_len() {
    ...
    assert_eq!(RuntimeValue::Array(vec![RuntimeValue::Scalar("a".into()), RuntimeValue::Scalar("b".into())]).len(), 2);
