...
3. structured attached-block composite flow (`if(cond) { ... } elseif(cond2) { ... } else() { ... }`) on action-edge and lifecycle block surfaces, with `else { ... }` accepted as the lighter final-branch alias, mixed per-branch carrier chains such as `if(cond) { ... } elseif(cond2) ... else { ... }` supported too, and the same family available as the final call on fluent action-edge and lifecycle chains.
...
- and those same two inline-switch structured branch-body carriers are now regression-locked in parity for nested composite `if(...)` / `elseif(...)` flow too,
- and those same two inline-switch structured branch-body carriers are now regression-locked in parity for nested composite `if(...)` / `elseif(...)` flow carrying that deeper alternating marker `if(...) ... switch(...) ... endif()` nesting shape too,
...
The outer block accepts both of these styles:
...
The same surface now works as the final fluent call too:
...
- and attached switch branch blocks are now regression-locked in parity across the two outer switch families themselves for nested composite `if(...)` / `elseif(...)` flow too,
...
- and attached switch branch blocks are now directly regression-locked across the two outer switch families for that broader multi-`case(...)` nested inline-composite `switch(...)` shape inside nested composite `if(...)` / `elseif(...)` flow too,
- and attached switch branch blocks are now directly regression-locked across the two outer switch families for that same broader multi-`case(...)` nested marker-style `switch(...) ... endswitch()` shape inside nested composite `if(...)` / `elseif(...)` flow too,
...
That same attached-block composite family is no longer all-or-nothing about branch carriers. Representative supported shapes now include:
