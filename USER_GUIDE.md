...
The `GenericFilter` grouping helpers are now package-owned too. `Plugin::GenericFilter` owns the `group_by`, `group_by_port`, `group_by_ioclock`, and `group_byRE` actions used by `HUtils::GenericFilter(...)` and `TableSort::GenericFilter(...)`. Repo-owned Perl callers no longer construct `genericfilter_*` plugin names or route those calls through `LinkedSpec::run_plugin(...)` / `LinkedSpec::get_plugin(...)`, and the old `plugin/genericfilter.plg` wrapper has now been deleted. This is the preferred migration shape for extracted plugin behavior: keep real executable logic in a normal package owner, keep `.plg` wrappers only where compatibility still needs the old names, and avoid direct `PPlugin` access in new code.

That last clause is intentional. A `.pm` owner does not automatically require a permanent `.plg` shadow. `Plugin::String`, `Plugin::CGI`, `Plugin::GenericFilter`, `Plugin::MSOffice`, `Plugin::VHDLConst`, and non-plugin owner `InteractivePrompt` are now pure package-owner surfaces for their repo-owned helper behavior: `plugin/string.plg`, `plugin/cgi.plg`, `plugin/genericfilter.plg`, `plugin/msoffice.plg`, `plugin/vhdconst_eval.plg`, and `plugin/yesno.plg` are no longer part of the shipped legacy plugin corpus. Use this as the direction for future extractions: keep `.plg` only while a real remaining legacy caller needs the old registration name, and treat `Plugin::*` as migration scaffolding rather than a permanent namespace when a clearer non-plugin owner exists.
...
Office automation helpers follow it too. The old `plugin/msoffice.plg` wrapper is gone; code that needs the historical Excel automation helper should call `Plugin::MSOffice::excel_start(...)` directly. That package owns the lazy `Win32::OLE` dependency boundary instead of hiding it behind an unqualified `excel_start` plugin action.

VHDL constant helpers follow it as well. The old `plugin/vhdconst_eval.plg` wrapper is gone; code that needs VHDL constant extraction or hash-value substitution should call `Plugin::VHDLConst::evaluate_constant_values(...)` or `Plugin::VHDLConst::substitute_hash_values(...)` directly. The former print action exists as `Plugin::VHDLConst::print_constant_values_for_conf(...)`, not as a legacy plugin registration.
