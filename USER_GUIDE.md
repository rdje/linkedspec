...
3. **Legacy or raw compatibility forms**
   - Compatibility only.
   - Examples: `return call(rule)`, `push(rule)`, `$CAPTURE`, `capture_from(name)`, `capture_take(name)`, `mark_here(name)`, `clear_mark(name)`, `BACKTRACK()`, or older raw wrappers such as `$retv = call(rule)`.
