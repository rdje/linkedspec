...
When the failure happens during `compiler_pipeline:spec_descr`, the structured payload can now also preserve `rule_label` when the compiler still knows which parsed rule entry was being compiled at that moment. Likewise, when `compiler_pipeline:build_final_descr` fails inside default `spec_gdata(...)`, the payload can preserve `rule_label` for the active rule whose combined regex dependencies were being built. The same attribution now reaches one stage later too: if `compiler_pipeline:validate_gdata_references` rejects the generated descriptor by returning false while the validation seam still knows the owning compiled rule, the payload can preserve that `rule_label` and carry a more specific owning-rule `detail` string instead of only the old generic “returned false” fallback. That same `validate_gdata_references` stage can now also preserve `rule_label` when a nested rule-local validation call throws while the seam still knows which compiled rule it was validating, and when validation already knows the exact failing gdata/rule-check cause it now reuses that more specific validation `summary` too instead of always collapsing back to the generic “Generated parser validation failed” banner. That makes descriptor-build failures easier to attribute without scraping `detail` just to discover which rule was active.
