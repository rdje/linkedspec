...
```perl
my $gdata_state = LinkedSpec::Compiler::build_dependency_regex_map($state, { return_state => 1 });
...
Likewise, final descriptor assembly keeps its `gdata` compilation defaults inside `LinkedSpec::Compiler`; normal callers do not need to provide a separate `build_dependency_regex_map` callback or depend on an older `LinkedSpec::build_dependency_regex_map(...)` façade helper.
