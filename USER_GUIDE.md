...
3. **Legacy or raw compatibility forms**
   - Compatibility only.
  - Examples: `return call(rule)`, `push(rule)`, `$CAPTURE`, `capture_from(name)`, `capture_len_from(name)`, `capture_take(name)`, `capture_between(start_mark, end_mark)`, `capture_len_between(start_mark, end_mark)`, `capture_take_between(start_mark, end_mark)`, `mark_here(name)`, `mark_match_start(name)`, `mark_copy(target_mark, source_mark)`, `clear_mark(name)`, `mark_exists(name)`, `mark_pos(name)`, `entry_text()`, `entry_group(index)`, `entry_groups()`, `entry_named(name)`, `entry_named_map()`, `entry_len()`, `entry_start_pos()`, `entry_end_pos()`, `match_text()`, `match_group(index)`, `match_groups()`, `match_named(name)`, `match_named_map()`, `match_len()`, `match_start_pos()`, `match_end_pos()`, `BACKTRACK()`, or older raw wrappers such as `$retv = call(rule)`.
...
For the worked long-form guide to the current rule-label sigils, blind-call `=> child_rule` orchestration patterns, and split-boundary behavior, read [`USER_GUIDE_RuleModesAndSplit.md`](USER_GUIDE_RuleModesAndSplit.md). That guide explains today’s supported `:&`, explicit `AND`, explicit `AND+`, `:|`, `:+`, `:*`, `:?`, explicit `OR`, bounded `OR{...}` forms, bounded `AND{...}` forms, advanced blind-call wrapper shapes, `@capture_from_here`, and named `@mark(name)` in one place, while also documenting `@move_pos` as the preserved compatibility alias, `capture_from(name)` as the stable named-read helper, `capture_len_from(name)` as the stable current-edge length helper, `capture_take(name)` as the advancing named-read helper, `capture_between(start_mark, end_mark)` as the explicit two-mark span helper, `capture_len_between(start_mark, end_mark)` as the explicit two-mark span-length helper, `capture_take_between(start_mark, end_mark)` as the explicit two-mark span plus advancing-start helper, `mark_here(name)` as the explicit post-match named-write helper, `mark_match_start(name)` as the explicit current-match-left-edge helper, `mark_copy(target_mark, source_mark)` as the explicit named-boundary copy helper, `clear_mark(name)` as the explicit named-clear helper, `mark_exists(name)` as the explicit named-presence helper, `mark_pos(name)` as the explicit named-position read helper, `entry_text()` as the explicit current-immediate-match text read helper, `entry_group(index)` as the explicit current-immediate-match positional capture-group read helper, `entry_groups()` as the explicit current-immediate-match capture-group-list read helper, `entry_named(name)` as the explicit current-immediate-match named-capture read helper, `entry_named_map()` as the explicit current-immediate-match named-capture-hash read helper, `entry_len()` as the explicit current-immediate-match width read helper, `entry_start_pos()` as the explicit current-immediate-match left-edge read helper, `entry_end_pos()` as the explicit current-immediate-match right-edge read helper, `match_text()` as the explicit current-local-match text read helper, `match_group(index)` as the explicit current-local-match positional capture-group read helper, `match_groups()` as the explicit current-local-match capture-group-list read helper, `match_named(name)` as the explicit current-local-match named-capture read helper, `match_named_map()` as the explicit current-local-match named-capture-hash read helper, `match_len()` as the explicit current-local-match width read helper, `match_start_pos()` as the explicit current-local-match left-edge read helper, `match_end_pos()` as the explicit current-local-match right-edge read helper, and several worked checkpoint/capture examples.
...
- `entry_named_map()`
  - helper form that snapshots the whole current immediate-match named-capture hash directly when an explicit helper spelling is clearer than passing raw `IMATCH_HASH` around.
- `entry_len()`
...
- `match_named_map()`
  - helper form that snapshots the whole current local named-capture hash directly when an explicit helper spelling is clearer than passing raw `LMATCH_HASH` around.
- `match_len()`
