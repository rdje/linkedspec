...
3. **Legacy or raw compatibility forms**
   - Compatibility only.
  - Examples: `return call(rule)`, `push(rule)`, `$CAPTURE`, `capture_slice()`, `capture_slice_len()`, `capture_slice_until_cursor()`, `capture_slice_until_cursor_len()`, `capture_slice_pos()`, `capture_slice_line()`, `capture_slice_col()`, `start_capture_slice()`, `capture_take()`, `capture_rest()`, `capture_rest_len()`, `capture_from(name)`, `capture_len_from(name)`, `capture_until_cursor_from(name)`, `capture_until_cursor_len_from(name)`, `capture_take(name)`, `capture_rest_from(name)`, `capture_rest_len_from(name)`, `capture_between(start_mark, end_mark)`, `capture_len_between(start_mark, end_mark)`, `capture_take_between(start_mark, end_mark)`, `mark_here(name)`, `mark_match_start(name)`, `mark_copy(target_mark, source_mark)`, `clear_mark(name)`, `mark_exists(name)`, `mark_pos(name)`, `mark_line(name)`, `mark_col(name)`, `cursor_pos()`, `cursor_line()`, `cursor_col()`, `cursor_rest()`, `cursor_rest_len()`, `entry_text()`, `entry_line()`, `entry_col()`, `entry_group(index)`, `entry_groups()`, `entry_named(name)`, `entry_has(name)`, `entry_map()`, `entry_len()`, `entry_start_pos()`, `entry_end_pos()`, `entry_end_line()`, `entry_end_col()`, `match_text()`, `match_line()`, `match_col()`, `match_group(index)`, `match_groups()`, `match_named(name)`, `match_has(name)`, `match_map()`, `match_len()`, `match_start_pos()`, `match_end_pos()`, `match_end_line()`, `match_end_col()`, `BACKTRACK()`, or older compatibility aliases such as `capture_from_rule_start()`.
...
For the worked long-form guide to the current rule-label sigils, blind-call `=> child_rule` orchestration patterns, and split-boundary behavior, read [`USER_GUIDE_RuleModesAndSplit.md`](USER_GUIDE_RuleModesAndSplit.md). That guide explains today’s supported `:&`, explicit `AND`, explicit `AND+`, `:|`, `:+`, `:*`, `:?`, explicit `OR`, bounded `OR{...}` forms, bounded `AND{...}` forms, advanced blind-call wrapper shapes, preferred `@capture_slice`, and named `@mark(name)` in one place, while also documenting compatibility aliases `@capture_from_here` / `@move_pos`, preferred anonymous capture helpers `capture_slice()` / `capture_slice_len()` / `capture_slice_until_cursor()` / `capture_slice_until_cursor_len()` / `capture_slice_pos()` / `capture_slice_line()` / `capture_slice_col()`, explicit anonymous capture-boundary move helper `start_capture_slice()` with compatibility alias `capture_slice_here()`, explicit anonymous capture-boundary advancing helper `capture_take()`, explicit anonymous capture-boundary tail helpers `capture_rest()` / `capture_rest_len()` with compatibility aliases `capture_slice_length()` / `capture_rest_length()` / `capture_from_rule_start()` / `capture_len_from_rule_start()`, `capture_from(name)` as the stable named-read helper, `capture_len_from(name)` as the stable current-edge length helper, `capture_until_cursor_from(name)` as the stable named-mark through-cursor helper, `capture_until_cursor_len_from(name)` as the stable named-mark through-cursor width helper, `capture_take(name)` as the advancing named-read helper, `capture_rest_from(name)` as the stable named-mark tail helper through end-of-input, `capture_rest_len_from(name)` as the stable named-mark tail-width helper through end-of-input, `capture_between(start_mark, end_mark)` as the explicit two-mark span helper, `capture_len_between(start_mark, end_mark)` as the explicit two-mark span-length helper, `capture_take_between(start_mark, end_mark)` as the explicit two-mark span plus advancing-start helper, `mark_here(name)` as the explicit post-match named-write helper, `mark_match_start(name)` as the explicit current-match-left-edge helper, `mark_copy(target_mark, source_mark)` as the explicit named-boundary copy helper, `clear_mark(name)` as the explicit named-clear helper, `mark_exists(name)` as the explicit named-presence helper, `mark_pos(name)` as the explicit named-position read helper, `mark_line(name)` as the explicit named-line read helper, `mark_col(name)` as the explicit named-column read helper, `cursor_pos()` as the explicit current-cursor position read helper, `cursor_line()` as the explicit current-cursor line read helper, `cursor_col()` as the explicit current-cursor column read helper, `cursor_rest()` / `cursor_rest_len()` as the explicit current-cursor tail text/width read helpers, `entry_text()` as the explicit current-immediate-match text read helper, `entry_line()` as the explicit current-immediate-match line read helper, `entry_col()` as the explicit current-immediate-match column read helper, `entry_group(index)` as the explicit current-immediate-match positional capture-group read helper, `entry_groups()` as the explicit current-immediate-match capture-group-list read helper, `entry_named(name)` as the explicit current-immediate-match named-capture read helper, `entry_map()` as the preferred explicit current-immediate-match named-capture-hash read helper with compatibility alias `entry_named_map()`, `entry_len()` as the explicit current-immediate-match width read helper, `entry_start_pos()` as the explicit current-immediate-match left-edge read helper, `entry_end_pos()` / `entry_end_line()` / `entry_end_col()` as the explicit current-immediate-match right-edge position/line/column read helpers, `match_text()` as the explicit current-local-match text read helper, `match_line()` as the explicit current-local-match line read helper, `match_col()` as the explicit current-local-match column read helper, `match_group(index)` as the explicit current-local-match positional capture-group read helper, `match_groups()` as the explicit current-local-match capture-group-list read helper, `match_named(name)` as the explicit current-local-match named-capture read helper, `match_map()` as the preferred explicit current-local-match named-capture-hash read helper with compatibility alias `match_named_map()`, `match_len()` as the explicit current-local-match width read helper, `match_start_pos()` as the explicit current-local-match left-edge read helper, `match_end_pos()` / `match_end_line()` / `match_end_col()` as the explicit current-local-match right-edge position/line/column read helpers, and several worked checkpoint/capture examples.
