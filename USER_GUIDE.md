...
More generally, marker-style `if(...) ... endif()` and marker-style `switch(...) ... endswitch()` are intended to allow arbitrarily deep mutual nesting in structured block contexts. The current regression suite now locks a representative deeper alternating chain across action-edge and the full lifecycle family. In other words, the language does not impose a fixed semantic nesting cap here; the only practical limits are the usual runtime recursion and resource ceilings.
