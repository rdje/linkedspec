...
That continuity now covers parser-factory callback exceptions too. If `validate_spec_name(...)`, `resolve_spec_path(...)`, `load_spec_content(...)`, or the delegated `compile_spec(...)` callback throws, `get_parser(...)` now returns `undef` and records the corresponding parser-factory failure stage in `last_error` instead of leaking a raw callback die. The same parser-factory rule now also preserves validator-specific false-return details on the front door: if `validate_spec_name(...)` rejects the requested parser name without throwing, `last_error` now keeps the validator’s own `summary` and `detail` instead of collapsing back to a generic “validate_spec_name rejected the requested parser name” wrapper. The same compile-stage rule still applies to silent `undef` returns too: if the delegated `compile_spec(...)` path comes back `undef` without first writing a deeper structured payload, parser-factory records its own `parser_factory:compile_spec` fallback; if the deeper compile/runtime owner already wrote a structured `last_error` payload before throwing or before returning `undef`, that deeper payload is preserved rather than being overwritten by a generic parser-factory wrapper error.
