...
`LinkedSpec::ActionRewriter` itself also no longer imports `LinkedSpec::Deps` at module load time, so require-only consumers stay on the extracted ActionIR owner modules without pulling the remaining parser-factory-only dep wiring into `%INC`.
