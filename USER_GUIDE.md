...
For the worked long-form guide to the current rule-label sigils, blind-call `=> child_rule` orchestration patterns, and split-boundary behavior, read [`USER_GUIDE_RuleModesAndSplit.md`](USER_GUIDE_RuleModesAndSplit.md). That guide explains today’s supported `:&`, explicit `AND`, explicit `AND+`, `:|`, `:+`, `:*`, `:?`, explicit `OR`, bounded `OR{...}` forms, bounded `AND{...}` forms, advanced blind-call wrapper shapes, preferred `@capture_slice`, and named `@mark(name)` in one place, while also documenting compatibility aliases `@capture_from_here` / `@move_pos`, preferred anonymous capture helpers `capture_slice()` / `capture_slice_len()` / `capture_slice_line()`, explicit anonymous capture-boundary move helper `start_capture_slice()` with compatibility alias `capture_slice_here()`, explicit anonymous capture-boundary tail helpers `capture_rest()` / `capture_rest_len()` with compatibility aliases `capture_slice_length()` / `capture_rest_length()` / `capture_from_rule_start()` / `capture_len_from_rule_start()`, `capture_from(name)` as the stable named-read helper, `capture_len_from(name)` as the stable current-edge length helper, `capture_take(name)` as the advancing named-read helper, `capture_between(start_mark, end_mark)` as the explicit two-mark span helper, `capture_len_between(start_mark, end_mark)` as the explicit two-mark span-length helper, `capture_take_between(start_mark, end_mark)` as the explicit two-mark span plus advancing-start helper, `mark_here(name)` as the explicit post-match named-write helper, `mark_match_start(name)` as the explicit current-match-left-edge helper, `mark_copy(target_mark, source_mark)` as the explicit named-boundary copy helper, `clear_mark(name)` as the explicit named-clear helper, `mark_exists(name)` as the explicit named-presence helper, `mark_pos(name)` as the explicit named-position read helper, `cursor_line()` as the explicit current-cursor line read helper, `entry_text()` as the explicit current-immediate-match text read helper, `entry_line()` as the explicit current-immediate-match line read helper, `entry_group(index)` as the explicit current-immediate-match positional capture-group read helper, `entry_groups()` as the explicit current-immediate-match capture-group-list read helper, `entry_named(name)` as the explicit current-immediate-match named-capture read helper, `entry_map()` as the preferred explicit current-immediate-match named-capture-hash read helper with compatibility alias `entry_named_map()`, `entry_len()` as the explicit current-immediate-match width read helper, `entry_start_pos()` as the explicit current-immediate-match left-edge read helper, `entry_end_pos()` as the explicit current-immediate-match right-edge read helper, `match_text()` as the explicit current-local-match text read helper, `match_line()` as the explicit current-local-match line read helper, `match_group(index)` as the explicit current-local-match positional capture-group read helper, `match_groups()` as the explicit current-local-match capture-group-list read helper, `match_named(name)` as the explicit current-local-match named-capture read helper, `match_map()` as the preferred explicit current-local-match named-capture-hash read helper with compatibility alias `match_named_map()`, `match_len()` as the explicit current-local-match width read helper, `match_start_pos()` as the explicit current-local-match left-edge read helper, `match_end_pos()` as the explicit current-local-match right-edge read helper, and several worked checkpoint/capture examples.
