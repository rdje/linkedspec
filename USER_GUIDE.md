...
backend admission is complete under `.11.8.4`. Five-backend callable recurring/public admission is complete;
the same governed matrix covers all five implementations, including PUC Lua and LuaJIT.
...
The legacy `ActionRewriter` module — a thin forwarding shim whose entrypoints all delegated into `RuleIR::EmitContext` — was deleted in Phase 1 (`PHASE1-PARSER-CORE-ISOLATION.2`); the focused helper-rewrite entrypoint is now `RuleIR::EmitContext::rewrite_action_code_for_compat(...)`, and `EmitContext::_rewrite_action_code_with_diagnostics(...)` still preserves the historical two-argument rewrite-helper call shape for direct compatibility callers.
...
Pure scalar assembly is part of that same contract too: `cat(...)` lets `.spec` rules build canonical string values from normalized scalar fragments across assignment sources, direct `return(payload)` expressions, and comparison inputs on both action-edge and lifecycle surfaces, so string construction no longer needs temporary array staging or ad hoc host-language interpolation. Strings pass unchanged, booleans become `1`/`0`, and finite numbers use stable decimal text (`-0.0` becomes `0`, `1.0` becomes `1`). Null, array, harray, and codeblock arguments are not scalar text and make the whole `cat` result null. The older source-spelled `concat(...)` form is retired and current runtimes diagnose it.
...
The visible `glc_xcel2hm` action still lives in `plugin/qcflow.plg`, but its duplicate-suppressed qclog insertion helper no longer appears as `qc_pushonce` in the dynamic plugin registry. That behavior now lives in `QC::Flow::push_once(...)`, the budget-check callbacks that used to appear as `qc_budget_check` and `qc_budget_check_01match_code` now live in `QC::Flow::budget_check(...)` and `QC::Flow::budget_check_single_or_zero_match(...)`, the filter-expression formatter that used to appear as `qcflow_filter_handler` now lives in `QC::Flow::filter_handler(...)`, the clock CTS summary helper that used to appear as `qcflow_clock_ctsinfo` now lives in `QC::Flow::clock_cts_info(...)`, the qclog workbook/link writer that used to appear as `qcflow_links_n_qclog` now lives in `QC::Flow::write_qclog_links(...)`, and the qclog table-preparation helper that used to appear as `qcflow_qclogdata` now lives in `QC::Flow::prepare_qclog_data(...)`. The same cleanup now covers the old Tcl/FANX helpers too: `QC::TclInterconn::append_interconnect_tcl(...)`, `load_fanx(...)`, and `fanx_info(...)` own that behavior directly, and the obsolete standalone `plugin/tcl4interconn.plg` wrapper is gone from the shipped legacy corpus.

The setup/hold timing helpers now follow the same rule across the former mixed legacy files. The visible `setup_hold_tmax_tmin` action still lives in `plugin/setup_hold_tmax_tmin.plg`, but the obsolete `plugin/tssio.plg` wrapper is gone now that `Timing::SetupHold::write_tssio(...)` owns the visible `tssio` report/action body directly. The private timing formulas no longer appear as `DxCy`, `DiCi`, `DiCo`, `DoCi`, `DoCo`, `tss_setup_hold`, or `tss_tmax_tmin` dynamic plugin registrations, and the `tssio` report logic no longer lives in a `.plg` body.

STAN/report-timing backend setup and clock-matrix cell formatting follow it as well. The visible STAN backend actions still live in `plugin/stan_backend.plg`, and related legacy actions in `plugin/skew.plg` plus `plugin/duty_cycle_degradation.plg` still exist, but the private setup helper no longer appears as `stan_backend_start` in the dynamic plugin registry, and the clock-matrix formatter no longer appears as the same-file `minmax_clockmx_cellcode` helper. That behavior now lives in `Timing::StanBackend::start(...)` and `Timing::StanBackend::clock_matrix_cell_code(...)`, preserving the launch banner, HTTP host/port setup, `stan_backend_table2ss` configuration load, min/max slack summary text, and first-100 path sheet allocation while letting repo-owned callers name the timing-domain owner directly.

The STAN OMAP2430C backend frequency-detail, STA-frequency, TCK-delay, and no-path helpers follow the same rule and also show the naming cleanup policy. The visible `old_default` and `dm_measures` actions still live in `plugin/stan_omap2430c_backend.plg`, but their private helpers no longer appear as misspelled dynamic plugin names such as `freqency_detailed`, `freqency_detailed_paths`, `drive_freqency_detailed`, or `get_freqency_detailed_fname`; the STA-frequency traversal callback no longer appears as `stafrequency`; the TCK-delay writer no longer appears as `drive_tckdelays`; the no-path checker no longer appears as `drive_nopath_check` or `nopath_check`; and the port-timing traversal callback no longer appears as `portiming`. That behavior now lives in `Timing::StanOmap2430cBackend` behind corrected callable names such as `collect_sta_frequency(...)`, `write_tck_delays(...)`, `write_no_path_check(...)`, `record_no_path_check(...)`, and `filter_port_timing_paths(...)`, while historical LOF section strings and data keys keep their existing spelling where external files may depend on them.
...
- Exact `array(name)` / `hash(name)` selectors remain temporarily readable in old non-shipped fixtures/corpora
  while tracked-source migration finishes. Their removal is settled; never introduce them in new code. Perl,
  Rust, Dart, Julia, and Lua all execute the selector-free mutation/result contract, and all 15 affected shipped
  specs are now selector-free. Remaining tracked sources migrate before hard rejection.
