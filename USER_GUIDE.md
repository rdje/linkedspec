...
The same rule also applies inside mixed legacy files. The former real historical `http` action now lives in `Plugin::HTTP::print_file_links_for_conf(...)`, so `plugin/http.plg` is no longer shipped as a legacy registry wrapper. `plugin/lighttpd.plg` still contains the real historical `lighttpd` action, but its package-backed helper names `set_http_hostport` and `set_http_localhost` are no longer registered as legacy `.plg` helpers. Repo-owned `.plg` code now calls `Plugin::HTTP::httplink(...)`, `Plugin::HTTP::set_http_hostport(...)`, or `Plugin::HTTP::set_http_localhost(...)` directly when it needs that behavior.
