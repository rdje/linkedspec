...
3. **Legacy or raw compatibility forms**
   - Compatibility only.
  - Examples: `return call(rule)`, `push(rule)`, `$CAPTURE`, `capture_slice()`, `capture_slice_len()`, `capture_slice_pos()`, `capture_slice_line()`, `start_capture_slice()`, `capture_rest()`, `capture_rest_len()`, `capture_from(name)`, `capture_len_from(name)`, `capture_take(name)`, `capture_between(start_mark, end_mark)`, `capture_len_between(start_mark, end_mark)`, `capture_take_between(start_mark, end_mark)`, `mark_here(name)`, `mark_match_start(name)`, `mark_copy(target_mark, source_mark)`, `clear_mark(name)`, `mark_exists(name)`, `mark_pos(name)`, `mark_line(name)`, `cursor_pos()`, `cursor_line()`, `entry_text()`, `entry_line()`, `entry_group(index)`, `entry_groups()`, `entry_named(name)`, `entry_has(name)`, `entry_map()`, `entry_len()`, `entry_start_pos()`, `entry_end_pos()`, `match_text()`, `match_line()`, `match_group(index)`, `match_groups()`, `match_named(name)`, `match_has(name)`, `match_map()`, `match_len()`, `match_start_pos()`, `match_end_pos()`, `BACKTRACK()`, or older compatibility aliases such as `capture_from_rule_start()`.
...
Use `@capture_slice` when one moving anonymous capture boundary is enough at the rule-paragraph level. Use `capture_slice()` when the rule wants the old raw `$IPOS` capture pattern explicitly without introducing a named checkpoint first. Use `capture_slice_len()` when the rule wants the numeric width of that same anonymous capture-boundary span without materializing the substring itself; `capture_slice_length()` remains supported as a longer alias, and older `capture_from_rule_start()` / `capture_len_from_rule_start()` spellings remain supported for compatibility. Use `capture_slice_pos()` when the rule should read the numeric position of that same current anonymous capture boundary directly as data, and use `capture_slice_line()` when the rule should read the 1-based line number where that same anonymous capture slice currently starts. Use `start_capture_slice()` when the rule should begin a new anonymous capture slice explicitly from inside a lifecycle or action block, instead of spelling `assign(s(IPOS), cursor_pos())` directly; `capture_slice_here()` remains supported as a compatibility alias. Use `capture_rest()` when the rule should read from the current anonymous capture boundary through end-of-input, and `capture_rest_len()` when it only needs that remaining width; `capture_rest_length()` remains supported as a longer compatibility alias. Use `@mark(name)` when the checkpoint meaning deserves a real name, when more than one checkpoint may coexist inside the same rule, or when the same rule needs several stable named left edges. Use `capture_from(name)` when that named checkpoint should stay stable, `capture_len_from(name)` when the rule should read the numeric width of that same current-edge span without materializing the substring itself, `capture_take(name)` when a later same-rule action should both return the current span and advance that named checkpoint like a named split cursor, `capture_between(start_mark, end_mark)` when the rule should read the span between two explicit named checkpoints instead of using the current match as the right edge, `capture_len_between(start_mark, end_mark)` when the rule should read the numeric width of that same explicit two-mark span instead of materializing the substring, `capture_take_between(start_mark, end_mark)` when that same explicit two-mark read should also advance the start mark to the stored end mark afterward, `mark_here(name)` when the rule should move that named checkpoint explicitly without bundling the write into the read itself, `mark_match_start(name)` when the rule should store the left edge of the current match rather than the post-match parser position, `mark_copy(target_mark, source_mark)` when the rule should copy one remembered boundary into another named checkpoint explicitly, `clear_mark(name)` when the rule should drop that named checkpoint explicitly before later reads, `mark_exists(name)` when the rule should branch or record metadata based on whether that named checkpoint is currently present, `mark_pos(name)` when the rule should read the stored numeric checkpoint position itself, and `mark_line(name)` when the rule should read the 1-based line number of that named checkpoint directly without round-tripping through the numeric position first. Use `cursor_pos()` when the rule should read the current parser cursor position directly as data, `cursor_line()` when the rule should read the current parser cursor line directly as data, `entry_text()` when the rule should read the current immediate match text directly as data, `entry_line()` when the rule should read the current immediate match line directly as data, `entry_group(index)` when the rule should read one immediate-match capture group directly as data, `entry_len()` when the rule should read the current immediate match width directly as data, `entry_start_pos()` when the rule should read the left edge of the current immediate match directly as data, `entry_end_pos()` when the rule should read the right edge of the current immediate match directly as data, `match_text()` when the rule should read the current local match text directly as data, `match_line()` when the rule should read the current local match line directly as data, `match_group(index)` when the rule should read one current-local-match capture group directly as data, `match_len()` when the rule should read the current local match width directly as data, `match_start_pos()` when the rule should read the left edge of the current local match directly as data, and `match_end_pos()` when the rule should read the right edge of the current local match directly as data. The long-form split/rule-mode guide now includes worked examples for anonymous capture-slice reads, anonymous capture-slice length reads, anonymous capture-slice position reads, anonymous capture-slice line reads, explicit anonymous capture-boundary movement, anonymous capture-boundary tail reads, same-rule named capture, current-edge span-length reads, explicit two-mark span-length reads, explicit named-boundary copy after length reads, advancing separator-delimited capture, explicit two-mark spans, advancing explicit two-mark spans, left-edge end markers, same-name reuse across different rules, missing-mark behavior, explicit numeric position reads, explicit named-checkpoint line reads, direct current-immediate-match text reads, direct current-immediate-match line reads, direct current-immediate-match group reads, direct current-immediate-match width reads, direct current-immediate-match boundary reads, direct current-local-match text reads, direct current-local-match line reads, direct current-local-match group reads, direct current-local-match width reads, direct current-local-match boundary reads, direct current-cursor position reads, direct current-cursor line reads, direct flow branching on named-mark presence, direct immediate/local named-capture presence checks, and direct immediate/local named-capture map snapshots.
...
For the worked long-form guide to the current rule-label sigils, blind-call `=> child_rule` orchestration patterns, and split-boundary behavior, read [`USER_GUIDE_RuleModesAndSplit.md`](USER_GUIDE_RuleModesAndSplit.md). That guide explains today’s supported `:&`, explicit `AND`, explicit `AND+`, `:|`, `:+`, `:*`, `:?`, explicit `OR`, bounded `OR{...}` forms, bounded `AND{...}` forms, advanced blind-call wrapper shapes, preferred `@capture_slice`, and named `@mark(name)` in one place, while also documenting compatibility aliases `@capture_from_here` / `@move_pos`, preferred anonymous capture helpers `capture_slice()` / `capture_slice_len()` / `capture_slice_pos()` / `capture_slice_line()`, explicit anonymous capture-boundary move helper `start_capture_slice()` with compatibility alias `capture_slice_here()`, explicit anonymous capture-boundary tail helpers `capture_rest()` / `capture_rest_len()` with compatibility aliases `capture_slice_length()` / `capture_rest_length()` / `capture_from_rule_start()` / `capture_len_from_rule_start()`, `capture_from(name)` as the stable named-read helper, `capture_len_from(name)` as the stable current-edge length helper, `capture_take(name)` as the advancing named-read helper, `capture_between(start_mark, end_mark)` as the explicit two-mark span helper, `capture_len_between(start_mark, end_mark)` as the explicit two-mark span-length helper, `capture_take_between(start_mark, end_mark)` as the explicit two-mark span plus advancing-start helper, `mark_here(name)` as the explicit post-match named-write helper, `mark_match_start(name)` as the explicit current-match-left-edge helper, `mark_copy(target_mark, source_mark)` as the explicit named-boundary copy helper, `clear_mark(name)` as the explicit named-clear helper, `mark_exists(name)` as the explicit named-presence helper, `mark_pos(name)` as the explicit named-position read helper, `mark_line(name)` as the explicit named-line read helper, `cursor_pos()` as the explicit current-cursor position read helper, `cursor_line()` as the explicit current-cursor line read helper, `entry_text()` as the explicit current-immediate-match text read helper, `entry_line()` as the explicit current-immediate-match line read helper, `entry_group(index)` as the explicit current-immediate-match positional capture-group read helper, `entry_groups()` as the explicit current-immediate-match capture-group-list read helper, `entry_named(name)` as the explicit current-immediate-match named-capture read helper, `entry_map()` as the preferred explicit current-immediate-match named-capture-hash read helper with compatibility alias `entry_named_map()`, `entry_len()` as the explicit current-immediate-match width read helper, `entry_start_pos()` as the explicit current-immediate-match left-edge read helper, `entry_end_pos()` as the explicit current-immediate-match right-edge read helper, `match_text()` as the explicit current-local-match text read helper, `match_line()` as the explicit current-local-match line read helper, `match_group(index)` as the explicit current-local-match positional capture-group read helper, `match_groups()` as the explicit current-local-match capture-group-list read helper, `match_named(name)` as the explicit current-local-match named-capture read helper, `match_map()` as the preferred explicit current-local-match named-capture-hash read helper with compatibility alias `match_named_map()`, `match_len()` as the explicit current-local-match width read helper, `match_start_pos()` as the explicit current-local-match left-edge read helper, `match_end_pos()` as the explicit current-local-match right-edge read helper, and several worked checkpoint/capture examples.
