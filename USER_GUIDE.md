...
3. **Legacy or raw compatibility forms**
   - Compatibility only.
   - Examples: `return call(rule)`, `push(rule)`, `$CAPTURE`, `capture_from(name)`, `capture_take(name)`, `capture_between(start_mark, end_mark)`, `capture_take_between(start_mark, end_mark)`, `mark_here(name)`, `mark_match_start(name)`, `clear_mark(name)`, `mark_exists(name)`, `BACKTRACK()`, or older raw wrappers such as `$retv = call(rule)`.
