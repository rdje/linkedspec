...
When the failure happens during `compiler_pipeline:spec_descr`, the structured payload can now also preserve `rule_label` when the compiler still knows which parsed rule entry was being compiled at that moment. That means descriptor-build failures are easier to attribute without scraping `detail` just to discover which rule was active.
