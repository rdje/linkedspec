...
Those same compile-time attributed failures can now also carry `handler_source_label` when LinkedSpec knows the failing rule label even before an exact handler variant has been selected. In that case the structured payload uses the stable synthetic label `LinkedSpec::generated_handler:<rule_label>`, which gives callers one direct generated-handler identity string for logging and attribution without forcing them to reconstruct it from `rule_label` manually. That makes descriptor-build failures easier to attribute without scraping `detail` just to discover which rule was active.
