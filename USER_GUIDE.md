...
That same story now reaches the standalone low-level `spec_descr(...)` helper too when you opt into `runtime_ctx_ref => \$ctx`. A thrown custom `compile_spec_entry(...)` callback, a malformed non-throwing descriptor tuple, an invalid non-CODE callback contract, or a malformed non-ARRAY parsed-entry container there now all normalize into `compiler_pipeline:spec_descr`. When rule iteration has already started, that payload preserves the active parsed `rule_label` when known; when failure happens earlier, it still falls back to the selected `top_rule` for the same label-scoped `handler_source_label` identity `LinkedSpec::generated_handler:<rule_label>` or `LinkedSpec::generated_handler:<top_rule>`.
