...
- 2026-03-22: Started Phase 4 with one intentionally small first-class checkpoint slice instead of trying to design the whole capture API up front. `.spec` now has named `@mark(name)` checkpoints in addition to anonymous `@capture_from_here`, the compatibility helper surface now has `capture_from(name)` for “text since that named mark,” and `LinkedRE::or(...)` now preserves shared mark state through later matches so child-rule calls can keep using a parent-established checkpoint.
- 2026-03-23: Expanded the named-checkpoint documentation immediately after landing the first slice. The main user guide now teaches the exact current semantics that matter in practice: `capture_from(name)` captures to the left edge of the current match, `@mark(name)` is a later-slot surface rather than a same-slot action helper, and the feature now has multiple worked examples so it reads like usable authoring guidance rather than a bare syntax note.
