...
examples spanning native parsing, semantic query/observation and authorized MCP.
MCP still takes a host-created index. The semantic checker includes the README
but its finite markers/denials miss older pending prose; existing .41 owners
retain correction and actual-claim regression obligations. No runtime repair.
...
lines26→27. Actual clean-source rollover, exact history reconstruction and
production-validator boundaries govern landing. All other limits and prior
records remain unchanged; PGEN/RGX build-on-update stays owned by startup .80.
...
## 2026-09-10 — recognition and observation proof boundaries

Dart .1.43 completes recognition/observation consumers. Private effect tests remain separate from runtime integration. Their emitted examples use strict offline independent callers; repeated-action emission runs direct/traced without a separate analyzer. All 22 tests pass; existing .2.4 remains open.
...
25→26 under ADR0111. Governed rollover is remeasured from clean 4d5af8e9;
independent reconstruction preserves the complete source and prior records.
All byte/aggregate limits remain unchanged. Exact validator and canonical proof
govern admission before Dart .1.25; no source-reading credit is added.
