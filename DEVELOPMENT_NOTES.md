...
- One recurring driver owns both mechanisms because their atomicity boundary is composed. Run neutral authorities
  before the fixed Perl/Rust/Dart/Julia/PUC-Lua/LuaJIT routes, then generated/capability/language support ledgers.
- Request both Rust integration targets in one Cargo invocation; separate invocations redundantly rebuild the
  generated parser carrier and repeat its already-owned 1,870-`pgen` / 26-`rgx-core` warning stream.
- A Dart test process that waits silently with almost no CPU and no child may be using ignored `.dart_tool`
  metadata from an older SDK. Compare `dart --version` with `generatorVersion`, then refresh through project-
  routed `pub get --offline`; never fall back to network or a global/off-volume cache.
- The first run showed SDK 3.13.3 versus metadata 3.9.2. After the offline refresh, write 9 and bang 11 passed
  immediately. The host sampler's automatically created `/tmp` report was consumed and deleted exactly, with an
  empty residue census; [[dart-sdk-local-metadata-refresh]] retains the causal fact.
- Canonical CI always requires, path-audits, and syntax-checks the driver; opt-in
  `LINKEDSPEC_RUN_MUTATION_MATRIX=1` executes it exactly once. Seventeen mutations guard authority, route, support,
  storage, owner, driver, CI, and live-artifact drift. Public teaching and parent `.19` remain exclusively `.19.9`.
- The complete record triggers the official notes rollover. Segment `4985-18da440ab1c4` preserves 239 clean-HEAD
  lines; ADR `0101` advances only finite collection/manifest capacity 22→23 / 21→22 with all ceilings unchanged.
