...
- 2026-04-11: Executed the README/SESSION_BOOTSTRAP architecture refresh path. The current deep read confirms `LinkedSpec.pm` remains a thin facade, the active compiler path now speaks in `build_compiled_rule_table(...)` plus compiled dependency-regex state, and the remaining `spec_descr` / `gdata` names are isolated to bootstrap parser internals in `LinkedSpec::BootstrapSpec` / `BootstrapSpec::Core`. `ARCHITECTURE_STATE.md` and the public owner-tree chapter now call that boundary out explicitly so future cleanup treats it as bootstrap-local naming clarity, not an active compiler-state compatibility issue.
