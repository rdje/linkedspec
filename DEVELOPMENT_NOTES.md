...
- The existing seven-group consumer is renamed from `test_dormant` to `dart/test`; its fixture, dedicated node,
  static/live transaction policy, fresh host authority, and four native/reconstructed/generated-plan/emitted routes
  remain byte-identical. No dormant duplicate remains.
- Canonical CI requires the admitted path, logs one exact Dart admission marker, and invokes its repository-routed
  command exactly once. Ordinary package discovery now executes the same proof.
- Neutral governance promotes only Dart: rollout is 4/9, mutations are 103, Rust/Dart carriers remain 9+8,
  Julia/Lua guards remain 2 groups/8 paths, and ten outward guards plus all 26 diagnostics stay exact.
- The Perl consumer adds one Dart path/rollout snapshot and passes 127 assertions. Rust's cfg consumer snapshots the
  same 4/9 boundary. The dormant Dart authority consumer now distinguishes its own absent route from the admitted
  carrier and remains 4/4.
- Fatal Dart analysis, ordinary 415, exact carrier 7/7, authority 4/4, Perl 127/127, exact cfg Rust 1/1 in 132.16s,
  typed 11/3/152, and recognition 138/250/58 pass. The staged receipt-bound canonical gate closes admission and
  parent `.14.6.4`; Julia `.14.6.5` is next only after the clean atomic-271 boundary.
- Canonical attempt one passes the Rust carrier and then exits 65 because Dart is launched from the repository root.
  The repaired exact invocation enters `dart/` and uses the same project-data wrapper plus package-relative test.
- Mandatory note rollover creates immutable segment 4992 and exceeds only the prior 15-file / 14-manifest-line
  controls. ADR `0083` admits exactly 16 files / 15 manifest lines; all aggregate, per-file, byte, lifecycle,
  owner, storage-authority, and verifier controls stay fixed.
