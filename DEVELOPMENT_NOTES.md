...
- 2026-04-10: Added `push_call(Rule)` as the compact convention-over-configuration helper for "call a child rule and push the result into the current rule array", with `push_call(Rule, index)` for shaped child payloads that need one indexed element and explicit-target `push_call(target, Rule[, index])` when the destination is a separate array. Migrated the small raw push-call accumulator wrappers in `specs/tkgui.spec` and `specs/regdef.spec`, plus the indexed quoted-string accumulator in `specs/ebnf.spec::logging_annotation`, to that helper. The rules keep their AST shapes, but the accumulator edges now read as modern helper DSL and classify through canonical `PUSH` instead of relying on compatibility wrapper recognition. Future resume can continue the same compatibility-surface cleanup on larger specs such as `ds_vhistory.spec` or the mixed state-update push-call bands in `simenv.spec`, but those should be handled as separate risk-managed slices.
...
- 2026-04-09: Finished the active dependency-regex naming cleanup across the live descriptor path with no compatibility alias preserved there. The outward descriptor key is now `dependency_regex_map`, per-rule compiled dependency mappings are now `dependency_refs`, and the remaining RuleIR emit-context holdout is now `DEPENDENCY_REFS`. Future resume should treat `gdata` as historical/bootstrap vocabulary only unless a slice is explicitly about older notes or bootstrap internals.
