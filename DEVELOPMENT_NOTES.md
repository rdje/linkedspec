...
- 2026-04-09: Deepened the FSMGen mdBook from scaffold-level prose into code-grounded reference material. `docs/fsmgen-book/src/model/fsm-source-model.md` now documents the active source model directly from `fsm_initialize(...)`, `fsm_walk(...)`, `dtree_walk(...)`, `top_exec(...)`, and `plugin/fsmgen.plg`, including paragraph families, decision-tree node kinds, hierarchy node kinds, and the inline mini-languages for ports/maps/generics/plugins. Future resume should keep filling the FSMGen book the same way: chapter-by-chapter, tied to the real active code surfaces, not with vague placeholder summaries.
- 2026-04-09: Added a dedicated FSMGen mdBook under `docs/fsmgen-book/` instead of trying to keep all FSMGen documentation inside the top-level LinkedSpec docs. The new book now has a stable `book.toml`, chapter hierarchy, and seeded overview/usage/model/reference/development pages driven from `perl/FSMGen.pm`, `plugin/fsmgen.plg`, and the visible FSMGen config surfaces. Future resume should treat that mdBook as the primary place to deepen FSMGen-specific user-facing documentation.
...
- 2026-04-07: Tightened the default `spec_gdata(...)` seam inside final descriptor assembly. It now validates descriptor shape explicitly instead of relying on incidental Perl reference errors, so malformed per-rule `gdata` / dependency / referenced-`re` structures now fail with targeted `build_final_descr` detail while preserving the active `rule_label` and label-scoped `handler_source_label` continuity through `run_get_pipeline(...)`. Future resume should treat default `spec_gdata(...)` contract validation as covered structured Phase 5 diagnostics surface now instead of assuming that seam only matters when `LinkedRE::or(...)` or another inner callback throws.
...
- 2026-04-06: Locked the same late generic compiler-failure attribution story through the file-oriented path too. `get_parser(...)` already preserved the deeper compiler payload, but we did not have a focused regression proving that resolved-spec continuity keeps `spec_path`, selected `top_rule`, and the label-only `LinkedSpec::generated_handler:<top_rule>` identity together on a later generic `build_final_descr` failure. Future resume should treat that file-oriented late-compiler continuity as covered now instead of rediscovering it by hand.
- 2026-04-05: Continued the same Phase 5 diagnostics-attribution line into the later generic compiler seams too. `Compiler.pm` now falls back to the selected `top_rule` for `handler_source_label` on unattributed `spec_descr`, `build_final_descr`, and `validate_gdata_references` failures when no rule label is known yet. Regression coverage now locks that directly and through public `LinkedSpec::Get(...)` continuity. Future resume should treat late generic compiler-failure handler attribution as covered structured-diagnostics continuity now instead of assuming only rule-attributed late compiler failures preserve handler identity.
...
- 2026-04-02: Continued the real Phase 5 diagnostics line with synthetic generated-handler attribution on compile-time rule failures. `Compiler.pm` now preserves `handler_source_label` not only on runtime handler/runtime parser failures with a known handler variant, but also on attributed `spec_descr`, `build_final_descr`, and `validate_gdata_references` failures whenever LinkedSpec still knows the failing rule label. Future resume should treat label-only forms like `LinkedSpec::generated_handler:Top` as part of the stable structured-diagnostics contract now instead of assuming `handler_source_label` is runtime-only.
...
- 2026-03-21: Continued the same Phase 5 line back into compile-time descriptor assembly. `Compiler.pm` now traps exceptions thrown while `spec_descr(...)` is compiling parsed rule entries and while `_build_final_descr(...)` is assembling the final descriptor / `gdata`, then normalizes those failures into the same `runtime_ctx->{last_error}` `compiler_pipeline` payload family (`spec_descr` / `build_final_descr`) instead of letting them escape as raw outer dies. This is a good fit for the diagnostics-continuity goal because callers using `runtime_ctx_ref => \$ctx` now get the same stable failure channel even when the compiler dies inside injected or lower-level descriptor-build callbacks.
...
- 2026-04-02: Continued the same Phase 5 diagnostics line one step further on the throw path too. `validate_gdata_references(...)` can now notify the same failure callback before rethrowing a nested `validate_rule_definition(...)` exception, which lets `Compiler.pm` preserve `rule_label` even when `compiler_pipeline:validate_gdata_references` fails by trapped die instead of only by false return. That keeps the generated-descriptor validation seam consistent with the earlier `spec_descr` / `build_final_descr` rule-attribution work.
