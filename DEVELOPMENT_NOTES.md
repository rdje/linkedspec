...
- Twelve managed jobs finish: full suites 1671/1672 retain the known PUC5.5 nil failure; two helper loads and 48 exact fixture observations are separate evidence.
- The malformed-event NIL ternary retains its table in six missing-field cases; actual absent controls correctly reject. Derivation instrumentation measures one materialization and no input hashing.
- LUA-STARTUP-READING.1.46 owns .2.34/.1/.2 intake and the exact clean-HEAD notes rollover; replay lives in docs/knowledge/lua-semantic-foundation-observation-fixture-reading.md.
...
- Six constructor/snapshot, two mark-setter and one progress observation per host stay separate from 30 valid controls each and public reachability claims.
- The unchanged recognition consumer passes 246 checks per host; CLI conformance passes 66 default-environment process cases per host, counted separately.
- LUA-STARTUP-READING.1.15 owns finite-state repair intake .2.12, stale-comment extension .2.1 and safe notes rollover; exact replay lives in docs/knowledge/lua-wire-cli-recognition-reading-and-finite-state-gap.md.
