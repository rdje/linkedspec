...
- 2026-04-17: Moved the FSMGen dynamic plugin-list parser out of `plugin/fsmgen.plg::getop_plugin_list` and into `perl/FSMGen.pm` as `FSMGen::getop_plugin_list(...)`. The legacy `.plg` entry is now only a thin package-owner wrapper, while the package owner preserves `+type=plugin#args...` bucket construction, the default `LinkedSpec::get_plugin(...)` resolver, and the unresolved-name no-op fallback. Tests can inject a resolver directly, so this path no longer needs the legacy plugin runtime just to verify the parser behavior.
