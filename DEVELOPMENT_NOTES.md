...
- 2026-04-11: Continued the package-backed helper retirement path inside the remaining HTTP-related `.plg` files. Repo-owned legacy actions in `plugin/http.plg`, `plugin/rtl.plg`, `plugin/stan_backend.plg`, and `plugin/tree.plg` now call `Plugin::HTTP::httplink(...)`, `Plugin::HTTP::set_http_hostport(...)`, or `Plugin::HTTP::set_http_localhost(...)` directly instead of relying on unqualified helper names registered through legacy plugin discovery. The real `http` and `lighttpd` actions remain in their `.plg` files for now, but the thin package-backed helper registrations `httplink`, `set_http_hostport`, and `set_http_localhost` are gone. Future plugin work should keep making this distinction: preserve a real legacy action only while it still exists as behavior, but retire helper registration names once all repo-owned callers can use the package owner explicitly.
...
- 2026-04-11: Continued the plugin/resource modernization track by extracting the generic table grouping plugin body into `Plugin::GenericFilter`. The legacy `plugin/genericfilter.plg` file is now only a thin compatibility wrapper for `genericfilter_group_by`, `genericfilter_group_by_port`, `genericfilter_group_by_ioclock`, and `genericfilter_group_byRE`, while `HUtils::GenericFilter(...)` and `TableSort::GenericFilter(...)` call `Plugin::GenericFilter::dispatch(...)` directly instead of bouncing through `LinkedSpec::run_plugin(...)` / `get_plugin(...)`. Future plugin extraction work should keep using this shape: move executable behavior into normal `Plugin::*` package ownership, keep `.plg` files only as temporary compatibility registration, and prefer direct package calls from repo-owned Perl code.
...
- 2026-03-24: Started the first real `.plg`-to-`.pm` extraction slice instead of only rerouting callers. `LinkedSpec::Plugin::String` now owns the old `string.plg` behavior (`var_subst`, `var_subst_test`), `string.plg` is reduced to thin compatibility wrappers, and `cgi.plg` now calls the package-owned `var_subst(...)` directly. Future resume should keep using that migration shape: move logic into a package owner, leave a small `.plg` wrapper only where compatibility still matters, and then steer repo-owned callers to the package/API instead of the wrapper.
