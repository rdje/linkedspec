...
- 2026-04-18: Extracted the visible `tssio` report/action body out of `plugin/tssio.plg` and into `perl/Timing/SetupHold.pm` as `Timing::SetupHold::write_tssio(...)`. The legacy `.plg` entry is now only a thin compatibility wrapper, while the package owner preserves `tssio.lof`, per-path `sta_<n>.lof` sheets, historical banner/error strings, and internal workbook-link formatting without loading `PPlugin`.
