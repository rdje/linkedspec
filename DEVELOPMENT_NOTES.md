...
- 2026-04-11: Deleted `plugin/vhdconst_eval.plg` after moving VHDL constant extraction and hash-value substitution into `Plugin::VHDLConst`. `plugin/mbist.plg` now calls `Plugin::VHDLConst::evaluate_constant_values(...)`, and `plugin/regtest.plg` now calls `Plugin::VHDLConst::substitute_hash_values(...)` instead of `LinkedSpec::run_plugin('hvalue_substitute', ...)`. The old print action shape is preserved as `Plugin::VHDLConst::print_constant_values_for_conf(...)` for explicit package callers, but the helper names are no longer kept as legacy `.plg` registrations without repo-owned callers.
- 2026-04-11: Deleted `plugin/msoffice.plg` after moving its only repo-owned caller, `plugin/spyglass.plg::spyglass_waive`, to `Plugin::MSOffice::excel_start(...)`. The new package owner keeps the historical active Excel instance preference and lazy `Win32::OLE` boundary while avoiding a permanent `excel_start` plugin registration with no internal caller. Future Office automation helpers should live under package ownership first, with `.plg` registration kept only for concrete legacy compatibility needs.
...
- 2026-04-11: Removed the next pure package-backed `.plg` helper wrappers instead of preserving compatibility surfaces with no repo-owned caller. `plugin/string.plg` is gone now that `Plugin::String::var_subst(...)` and `var_subst_test(...)` own all repo-owned string-helper usage, and `plugin/genericfilter.plg` is gone now that `Plugin::GenericFilter` plus direct `HUtils::GenericFilter(...)` / `TableSort::GenericFilter(...)` dispatch owns the GenericFilter behavior. Regression coverage now locks wrapper absence and keeps behavior coverage on the package owners themselves. Future plugin cleanup should keep using this bar: if the `.plg` file is only a package-backed registration shell and no repo-owned caller needs the legacy plugin name, delete it.
...
- 2026-04-11: Took the plugin-removal direction one step past extraction by deleting the obsolete `plugin/cgi.plg` compatibility wrapper. `Plugin::CGI::file_list_path2http(...)` now owns all repo-owned usage directly, `Plugin::String::var_subst_test(...)` already calls it as a normal package function, and no repo-owned code still needs `file_list_path2http` as a legacy plugin-registration name. Future plugin work should not create a `.plg` shadow by default after a `.pm` owner exists; keep a wrapper only while a concrete repo-owned legacy caller still needs that name.
- 2026-04-11: Continued the plugin/resource modernization track by extracting the generic table grouping plugin body into `Plugin::GenericFilter`. `HUtils::GenericFilter(...)` and `TableSort::GenericFilter(...)` call `Plugin::GenericFilter::dispatch(...)` directly instead of bouncing through `LinkedSpec::run_plugin(...)` / `get_plugin(...)`; the temporary `plugin/genericfilter.plg` compatibility wrapper from the extraction step has since been removed. Future plugin extraction work should keep using this shape: move executable behavior into normal `Plugin::*` package ownership, keep `.plg` files only while a concrete legacy caller still needs the old name, and prefer direct package calls from repo-owned Perl code.
