...
## 2026-09-06 — SESSION-STARTUP-READING.3.2.53 — registry metadata evolves independently of its container version

The container remains a version-1 registry, while fixed and variadic definitions use distinct metadata and typed final-codeblock declarations receive a separate outward version-3 projection. The spec-returned body payload and job must match the declaration before staged body parsing. Legacy PPlugin discovery and PathSearch remain separate from that compiler authority and from registered plugin dispatch.
...
Six controls qualify the passing suites; .37 owns resource and source-detail review, repairs, and recurring closeout.
...
Fresh controls confirm both branches and typed mutation values; historical scalar-slot storage and unconditional child-call notes are qualified.
...
The empty-source split results also differ; .33 owns contract/impact review and repair, with exact paired controls in Knowledge.

## 2026-09-06 — SESSION-STARTUP-READING.3.2.27 — compatibility bridges and finite numeric proof

AST dispatch enters the guarded compatibility catalog; scalar Numeric calls and inline aggregate reducers remain distinct paths.
The 55-case fixture leaves .20 open; successful map generation with a setpgid warning adds establishment verification to .7.
...
Existing Knowledge gains retrieval keys and qualifies dated migration spellings; exact evidence belongs to the task.
...
- Public is_empty on quoted zero disagrees with its bound twin; numeric zero reads host $0 and changes result
  with the process-local program-name seed. Repairs `.16.1`/`.16.2` own typed emptiness and literal recognition.
- Required complete-record rollover preserves clean source bytes; its finite route-capacity step uses an indexed
  ADR and receipt-bound canonical proof. Production/book remain unchanged until required reading completes.
- Nested sandbox initialization fails with status 71 in the restricted harness, including a no-op control;
  permitted execution passes both that control and the unchanged full six-family containment oracle. This is an
  execution prerequisite, not a reason to weaken the profile. The failed canonical attempt yields no receipt.
...
- Contracts resolves required callbacks before concatenating fourteen groups; staged/progressive groups delegate
  to dedicated owners. The existing lowering card records source order without claiming new runtime admission.
- Six untruncated suffix ranges complete the file; 64 Perl files remain unread. ControlFlow is next.
