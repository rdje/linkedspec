...
- 2026-03-22: Started Phase 4 with the first named checkpoint slice. `.spec` files now support `@mark(name)` alongside anonymous `@capture_from_here`, the compatibility helper surface now includes `capture_from(name)` for “text since that named checkpoint,” runtime match info now carries shared mark state forward across later matches and child-rule calls, and regression coverage now locks bootstrap/RuleIR lowering plus an end-to-end named-mark child-call capture flow.
- 2026-03-23: Expanded the author-facing docs for the new named checkpoint slice so it is easier to adopt. The rule-mode guide and ActionIR contract guide now explain the exact current `capture_from(name)` semantics, the “later slot, not same slot” timing rule for `@mark(name)`, and multiple worked examples including parent-to-child capture and separator-delimited capture.
