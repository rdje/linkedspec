...
- Activated admission task-tree-first from exact clean carrier atomic 262 `075cb7b8`; retrieved the neutral,
  private authority/carrier, dormant-RED, recognition, language-inventory, CI, ADR, book, and Knowledge owners.
- Required and syntax-checked `t/progressive_span_dispatch_perl_contract.t`, logged its exact admission route, and
  executed the unchanged 125-assertion consumer once in canonical CI.
- Promoted only Perl: progressive governance is 2/9/86 with four still-pending backend groups/14 paths, ten
  outward guards, 26 diagnostics, and unchanged generated, typed, recurrence, and public boundaries.
- Recomposed recognition effects to 138 node rows (134 current plus four dedicated), 250 call rows, and 58
  mutations. `PROGRESSIVE_DISPATCH_SPAN` is the sole current rejected `parser_registry_or_staged_dispatch` node;
  uncommitted recognition remains forbidden and no call row is admitted.
- The first canonical run caught Rust's stale 133-node metadata snapshot before runtime behavior. Rust, Dart,
  Julia, PUC Lua, and LuaJIT admitted consumers now all assert the recomposed 134/138 node counts in lockstep.
- Retained `dispatch_span` as the fifteenth non-public Perl classification. Private Perl intrinsic admission does
  not prove equal Dart/Julia/Lua shared-helper support; exact shared recurrence owns later reclassification.
- Fixed one direct-dependent prose collision: the recognition checker globally reserves its historical stale
  phrase, so progressive guide prose uses unambiguous `2/9 complete (neutral and Perl)` wording.
- Mandatory change-history pressure rollover archives 244 clean-HEAD lines as immutable segment 4993 and leaves
  the current shard at 233/512 lines without changing accepted chronology.
- ADR `0082` authorizes only the resulting finite change-history capacity 19→20 files and 18→19 manifest lines;
  all root, byte, per-segment, aggregate, lifecycle, owner, verifier, and storage controls remain unchanged.
- Focused authority/carrier/transaction proof passes 193 assertions, generated-source proof passes 6, and neutral,
  typed, recognition, generated, capability, language, Knowledge, and mdBook checks pass. Exact staged
  receipt-bound canonical CI is the final admission proof.
