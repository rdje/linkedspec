...
- Pure extracted helper wrappers are being deleted once no repo-owned caller needs the old plugin name: `plugin/string.plg`, `plugin/cgi.plg`, and `plugin/genericfilter.plg` are now gone while `Plugin::String`, `Plugin::CGI`, and `Plugin::GenericFilter` own the behavior directly.
- `Plugin::HTTP` is following the same boundary beyond helper subdefs now: repo-owned `.plg` actions call it directly for `httplink`, `set_http_hostport`, and `set_http_localhost`, and the former `plugin/http.plg` file-link action now lives in `Plugin::HTTP::print_file_links_for_conf(...)`, so `plugin/http.plg` is gone while `plugin/lighttpd.plg` remains only for the real legacy `lighttpd` action.
...
`Plugin::String`, `Plugin::CGI`, and `Plugin::GenericFilter` now show the preferred destination for pure helper extractions: the package owner keeps the behavior, while the old `.plg` registration wrapper is deleted once no repo-owned code still needs the legacy plugin name.

`Plugin::HTTP` now shows the same destination applied to a formerly real legacy action, not just thin helper wrappers. The package owns `httplink`, `set_http_hostport`, `set_http_localhost`, and the former `http` file-link action through `print_file_links_for_conf(...)`, so `plugin/http.plg` has been removed. `plugin/lighttpd.plg` remains only because it still owns the real `lighttpd` action, with the package-backed HTTP helper registrations already removed. Repo-owned `.plg` callers that need HTTP helpers call `Plugin::HTTP` explicitly.
