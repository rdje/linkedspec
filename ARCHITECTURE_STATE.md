...
- Shipped `.plg` files now have a corpus-level source lock against direct `LinkedSpec::get_plugin(...)` calls, in addition to the existing direct `PPlugin->get(...)` lock. Repo-owned legacy plugin files should keep moving to explicit package owners instead of using the plugin bridge for private helper reuse.
