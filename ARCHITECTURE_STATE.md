...
`language.map_leaves_receiver_mutation` after the unchanged neutral contracts became executable through five
backend implementations and six runtime routes: Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT. The capability
manifest therefore has 20 rows and 100 pass / 0 partial / 0 gap backend states. Each row names ADR `0036`, its
frozen mechanism authority, the shared composition authority, and exact production/test references for all five
backend implementations; the shared-Lua entries explicitly record both independent ABIs.

The capability checker pins both authority IDs, format versions, still-neutral status strings, canonical JSON
digests, the composition dependency paths/digests, row order, backend references, and Lua route note. Sixteen
admission mutations reject omission, duplication, reordering, stale state, proof substitution, and authority
drift. This is capability-state movement only: the immutable contract bytes and production runtime behavior do
not change.

`FUTURE-PARITY-BACKLOG.19.8` now adds one repository-routed recurring proof. It validates both neutral authorities,
executes exact Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT consumers in fixed order, then checks generated-source,
capability, and language-coverage ledgers. Seventeen additional mutations pin all three historical artifact
digests/statuses, route/support order, project-data initialization, owner/driver state, and exact-one canonical CI
registration. No production or frozen-authority byte moves. `.19.9` binds current public teaching back to those
unchanged authorities through one recurring checker and closes parent `.19`. The required engineering-notes
rollover creates immutable segment
`4985-18da440ab1c4`; ADR `0101` advances only finite collection/manifest capacity to 23/22 while preserving every
byte and aggregate ceiling.
...
generated plans, emitted modules, and the primary CLI. It originally passed 436 assertions per ABI;
the adjacent proof now passes 438 per ABI after replacing its two stale raw-bang assertions with the exact typed
invalid-arguments diagnostic. The complete Lua gate, CLI 66x2, corpus 105/105, and repository-local storage proof
pass. The separate `map_leaves!` receiver carrier is implemented below. Both mechanisms are capability-admitted
under `.19.7`, recur exactly under `.19.8`, and are public-current under `.19.9`.

## Lua implements controlled `map_leaves!` receiver mutation on both ABIs

`FUTURE-PARITY-BACKLOG.19.6.2` advances shared Lua to `linkedspec-map-leaves-mutation-v1`. One dedicated
`receiver_mutation_chain` retains the bare receiver, mutation call, direct typed callback block, ordinary
continuation, authored source, and half-open Unicode-scalar spans. Action contracts and static projection visit
the node; compiler, direct-runtime, public `SpecFile` reconstruction, generated-plan, and source-emitter
boundaries reject malformed or reserved carrier state before execution.

Visible bindings keep stable identities across ordinary writes. Callback scopes and user-function parameters get
fresh identities, so the guard follows the resolved receiver rather than spelling. Direct assignment/append,
nested write/bang, mutation helpers, array-end methods, regex substitution, and all binding-target pipelines
reject before operand, segment, or RHS evaluation only when they target that active identity. Unrelated writes
and distinct same-spelling scopes remain ordinary.

The runtime maps a detached original-shape snapshot in sorted-key or index order and recurses only through the
root kind. Every callback receives copied `value`, `path`, `depth`, and `key` or `index`; its detached result
replaces the leaf without revisit. Complete success publishes once, releases the guard, returns another copy,
and then runs ordinary continuation. Callback/re-entrant failure leaves the receiver unchanged and releases the
guard; continuation failure preserves an already completed commit.

Permanent proof passes 530 assertions per ABI across all 4/14/5 syntax, 10 success, 8 failure, 18 guarded-write,
six callback-composition, and one continuation-composition cases plus detachment, corrupted state,
native/reconstructed/generated-plan/emitted-module/primary-CLI routes, staged user-function bodies, and the exact
zero-regex-parent dispatch invariant. The unchanged neutral oracles reject 167 base plus 592 composition
mutations, and the complete Lua local gate passes both ABIs. The known Lua 5.1 200-local ceiling is preserved by
placing the implementation under the existing private `typed_source.receiver_mutation` namespace. Portable
capability is admitted under `.19.7`, exact six-runtime recurrence is complete under `.19.8`, and public no-drift
is closed under `.19.9`.
...
167 base plus 592 composition mutations. Lua has since advanced through the same contract as described above;
portable capability is now admitted under `.19.7`, exact six-runtime recurrence is complete under `.19.8`, and
public no-drift is closed under `.19.9`.
...
`ActionWritePathSegment` retains the typed expression, exact authored source, and half-open Unicode-scalar span;
one `ActionAssignNestedAccessExpr` owns every one- or many-segment bracket write. Parser, callable/contract,
compiled-state, generated-plan, source-emitter, and direct runtime boundaries preserve and validate that carrier.
...
passes. Julia `map_leaves!` and both Lua mechanisms have since advanced as described above; portable capability
is admitted under `.19.7`, exact six-runtime recurrence is complete under `.19.8`, and public no-drift is closed
under `.19.9`.

## Dart implements controlled `map_leaves!` receiver mutation

`FUTURE-PARITY-BACKLOG.19.4.2` advances Dart to `linkedspec-map-leaves-mutation-v1`. One dedicated
`ActionReceiverMutationChainExpr` retains the bare receiver, mutation call, typed callback, ordinary continuation,
exact authored source, and half-open Unicode-scalar spans. Contract/callable resolution, compiler, generated-plan,
source-emitter, and direct runtime boundaries validate the carrier and fail closed on malformed state.

Runtime bindings have stable identities across writes and fresh identities across scoped callback/function-local
lifetimes. Assignment, append, nested write/bang, helpers, array-end methods, and binding-target pipelines reject
before operand/segment/RHS evaluation only when they resolve to the active receiver identity; unrelated and same-
spelling shadow identities remain legal.
...
oracles. Julia and Lua have since advanced through the same contract; portable capability is now admitted under
`.19.7`, exact six-runtime recurrence is complete under `.19.8`, and public no-drift is closed under `.19.9`.

## Dart implements frozen typed nested-write vivification

`FUTURE-PARITY-BACKLOG.19.4.1` advances Dart to `linkedspec-write-vivification-v1`. One
`ActionWritePathSegment` retains the typed expression, exact authored source, and half-open Unicode-scalar span;
one `ActionAssignNestedAccessExpr` owns every one- or many-segment bracket write. Parser, callable, compiled-state,
generated-plan, emitted-source, and direct runtime boundaries reject malformed typed carriers.
...
`map_leaves!` have since composed with their respective nested-write paths as described above; Lua now implements
both mechanisms too. Portable capability is now admitted under `.19.7`, exact six-runtime recurrence is complete
under `.19.8`, and public no-drift is closed under `.19.9`.
...
base and 592 composition mutations. `.19.3.3-.4.0` have since closed the oracle/root-semantics cleanup and external
macOS launch-latency classification.
...
`WritePathSegment` retains source, half-open Unicode-scalar span, and typed expression; one
`AssignNestedAccess` owns every one- or many-segment write. Parser diagnostics are exact, and compiler/callable,
serde, source-emitter, decoded generated-plan, and direct engine boundaries fail closed on malformed typed nodes.

The engine evaluates segments left-to-right then RHS, snapshots binding presence/value afterward, and builds on
an isolated clone. Evaluated strings select harrays and nonnegative integral numbers select dense arrays. Absent
roots/intermediates create only from those kinds; bound null, wrong kinds, invalid selectors, and gaps return exact
typed errors without a partial path publication. Completed expression effects remain ordinary state, and
successful binding/result/initial/RHS aggregates detach.

The permanent five-test Rust contract projects the frozen 5 AST / 7 syntax / 11 success / 16 structural-failure
inventory plus evaluation, bound-null, fresh-function, detachment, fail-closed, and carrier proof across native,
serialized, generated-plan, emitted-source, and independently compiled emitted Rust. The complete 197-test
...
Rust `map_leaves!` has since advanced as described above; Dart, Julia, and Lua now implement both mechanisms,
while portable capability is admitted under `.19.7`, exact six-runtime recurrence is complete under `.19.8`, and
public no-drift is closed under `.19.9`.
...
write mutations, 167 base + 592 composition mutations, and Phase 0 1,032/1,032. Rust, Dart, Julia, and Lua now
implement both mechanisms as described above. Portable capability is admitted under `.19.7`, exact six-runtime
recurrence is complete under `.19.8`, and public-current no-drift is closed under `.19.9`.
...
At the neutral `.19.1.3` boundary, the exact non-bang control returned `{"leaf":[]}` on Perl, Rust, Dart, Julia,
PUC Lua, and LuaJIT. The bang source then stopped before callback nested-write lowering. Perl `.19.2.1-.2`, Rust
`.19.3.1-.2`, Dart `.19.4.1-.2`, Julia `.19.5.1-.2`, and Lua `.19.6.1-.2` have since advanced both mechanisms
without moving capability/public admission. The `.19.4.2` audit also corrected
the control's inert parent `/x/` to the established zero-regex `Top` loop whose edge selects `Done`'s regex; only
fixture spelling changed.
...
At the freeze boundary the one-token bang twin was unsupported everywhere. Perl, Rust, Dart, Julia, and Lua have
since implemented the exact authority. No portable capability, facade, schema, MCP, or public admission moves.
Shared composition `.19.1.3` freezes the cross-mechanism boundary
above.
...
`tools/check_standalone_lifecycle_block_five_backend.sh`; capability truth at that feature boundary was 90/0/0.
Mutation admission `.19.7` later advances the current census to 100/0/0.
...
the declaration effect. `.2` adds one unexported caller-frozen authority that resolves only supplied alias/relative/
...
Rust leaves `FUTURE-PARITY-BACKLOG.14.7.4.0-.4` now implement and privately admit the next complete backend path.
One exact consumer runs in ordinary Cargo discovery and in canonical CI. Its final path keeps the neutral inventory
and current function-body-v1 queue/
resolve/load/compile/execute/cache/stitch plus wrong-top context GREEN, while the v1 registry denies `expr-v1` at
resolve. Exact scalar-assignment `parse_job(text_expr, hash(literal options))` lowers exclusively to
...
text and convert immediately to same-source Unicode-scalar direct or nonempty ordered-derived provenance.

Private `staged_ast_enrichment.rs` accepts one caller-frozen immutable snapshot of normalized candidate outcomes
and already-compiled opaque callbacks. Resolution is pure alias/declaring-relative/ordered-root/provider
selection; top/version/capability/policy/source-detail fields only narrow authority. Default top precedes the v2
job digest, and an invocation-local cache stores only immutable callback plans under the neutral identity. One
complete depth prepares every job and stitch target before execution, orders typed paths/provenance/job ids,
isolates sibling cursor/mark/capture/variable state, detaches and node-bounds results, then atomically applies all
four result and three failure policies. Failed callbacks, results, and partial work never enter the cache; newly
returned markers remain inert under the one-depth API. `enrich_recursively` validates whole depths before
callbacks, schedules returned markers only at the next depth, and carries exact parser/top/payload-digest/full-
provenance lineage. Exact repeats and non-decreasing same-parser/top lineage reject. One cancellation identity,
clock/deadline, remaining steps, calls, depth, result nodes, and diagnostic-byte authority spend across every
depth; expiring callback contexts provide safe points and direct/ordered-derived source projection without false
contiguous spans. A private host-only seed attaches the authority after the complete parent result on native,
reconstructed, validated generated-plan, and independently compiled emitted routes. Every top-level execution
constructs a fresh frozen registry/cache and recursive authority; equal detached records, zero-hit/one-miss
run-local caches, distinct callback/cancellation/clock identities, cross-result mutation isolation, and emitted
logical-source authority absence are executable proof. The first production caller removes the temporary outer
...
completed alias/relative/ordered-root/provider outcomes plus already-compiled opaque callbacks, pure selection,
selected-top-before-v2 identity, exact plan-only caching, complete-depth sibling isolation, detached atomic
stitching, and all four result plus three failure policies.

`.3` preserves the one-depth API and adds breadth-first recurrence under active parser/top/exact-text-digest/full-
provenance tuples, exact cycle and strict-decrease checks, one non-resetting cancellation/deadline/step/call/depth/
...
Julia leaves `FUTURE-PARITY-BACKLOG.14.7.6.0-.3` now freeze and implement the same private declaration,
current-depth boundary, and recursive authority.
...
independently included emitted-module routes agree. A separate unexported authority owns tuple-frozen caller-
completed candidates and already-compiled callbacks, pure selected-top-before-id resolution, an immutable-plan
cache, complete-depth target reservation and typed order, fresh sibling state, detached results, and all four
result/three failure policies. A separate recursive entrypoint queues only successfully detached returned markers
by exact stitch path, carries resolved-parser/top/payload-digest/full-provenance lineage, rejects cycles and non-
decreasing repeats, and shares cancellation/deadline/steps/calls/depth/result/diagnostic bounds across complete
breadth-first depths. Callback authority expires after return or throw; safe points and direct/ordered-derived
source projection cannot escape. The run is 386 GREEN/one exact `.14.7.6.4` fresh-carrier/production/admission/
rollout RED. Julia stays `dormant_red`; neutral governance remains 92 mutations, and rollout, generated format,
public/outward behavior, and other backends stay unchanged.
...
is therefore 92 mutations and `.14.7.6.4` is the next frontier.
...
`{label,family}`. One repository-routed caller now independently resolves offline, strictly analyzes, and executes
...
The accepted topology keeps every stable root. Chronological sources receive strict repository-relative JSONL
manifests whose immutable segments record clean source commit/blob, line span/count, bytes, SHA-256, current
replacement, and exact root-derived query; concatenation must reproduce the Git source byte-for-byte. Live status
...
- `2026-07-29` MCP machine-contract realization: `FUTURE-PARITY-BACKLOG.10.9.1.1` encodes ADR `0055` once at
  ...
  capability projection), 35 canonical LF frames, ten raw-byte adversarial inputs, ten lifecycle cases, all five
...
- `2026-07-28` Lua calls/staging/generated composition closure: no-change
  ...
  authored Unicode/source/call identity, resolution/signature/shape, staged/generated, freeze/detachment, privacy,
  corruption, and no-execution guarantees. Complete Lua, primary 5x2x66, Unicode 10/10, all six unchanged ledgers,
  and canonical Rust 81.19s + Dart 1/1 + Julia 416/29.0s + reference primary 66x2 + Phase 0 1,031/648s pass. No
  production/test/fixture/API/query/observation/format/ledger file changes. Parent `.10.7.4` is composition-closed;
  behavior-free immutable-query authority audit `.10.7.5.0` is next.
- `2026-07-28` Lua staged/generated semantic provenance: `FUTURE-PARITY-BACKLOG.10.7.4.2` completes the existing
  ...
  observation, generated format, rollout, or admission; committed-owner closeout `.10.7.4.3` is complete.
...
- `2026-07-20` semantic-introspection neutral-contract refresh: ADRs `0049`/`0050` and
  ...
  errors. Queries read no path, compile/execute nothing, emit no trace, and expose no mutable/host state. Perl
