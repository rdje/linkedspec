...
- Shipped `.plg` files now have a corpus-level source lock against direct public plugin-bridge dispatch calls (`LinkedSpec::get_plugin(...)`, `LinkedSpec::run_plugin(...)`, and `LinkedSpec::dispatch_plugin_autoload_name(...)`), in addition to the existing direct `PPlugin->get(...)` lock. Repo-owned legacy plugin files should keep moving to explicit package owners instead of using the plugin bridge for private helper reuse.
