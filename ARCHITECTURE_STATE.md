...
- `Timing::StanOmap2430cBackend` now owns the former internal STAN OMAP frequency-detail helper family and port-timing traversal callback from `plugin/stan_omap2430c_backend.plg`; the visible `old_default` legacy action calls corrected package functions and the package-owned port-timing callback directly while the historical LOF/data strings keep their existing spelling for file compatibility.
...
`Timing::StanOmap2430cBackend` continues that cleanup in `plugin/stan_omap2430c_backend.plg`. The visible `old_default` action still lives in the legacy action file, but the private frequency-detail helper family is no longer registered as `freqency_detailed`, `freqency_detailed_paths`, `drive_freqency_detailed`, or `get_freqency_detailed_fname`, and the port-timing traversal callback is no longer registered as `portiming`. The package owner now exposes corrected frequency-detail function names plus `filter_port_timing_paths(...)` for the callable API while preserving historical LOF section strings and data keys where they are file/data compatibility contracts.
