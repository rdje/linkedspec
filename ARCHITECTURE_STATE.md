...
- Pure extracted helper wrappers are being deleted once no repo-owned caller needs the old plugin name: `plugin/string.plg`, `plugin/cgi.plg`, `plugin/genericfilter.plg`, `plugin/msoffice.plg`, and `plugin/vhdconst_eval.plg` are now gone while `Plugin::String`, `Plugin::CGI`, `Plugin::GenericFilter`, `Plugin::MSOffice`, and `Plugin::VHDLConst` own the behavior directly.
...
`Plugin::String`, `Plugin::CGI`, `Plugin::GenericFilter`, `Plugin::MSOffice`, and `Plugin::VHDLConst` now show the preferred destination for pure helper extractions: the package owner keeps the behavior, while the old `.plg` registration wrapper is deleted once no repo-owned code still needs the legacy plugin name.
