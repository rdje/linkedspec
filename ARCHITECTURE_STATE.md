...
- `Timing::StanOmap2430cBackend` now owns the former internal STAN OMAP frequency-detail helper family from `plugin/stan_omap2430c_backend.plg`; the visible `old_default` legacy action calls corrected package functions directly while the historical LOF/data strings keep their existing spelling for file compatibility.
