...
sub _method_chain_return_uses_general_payload {
 ...
 return $trimmed =~ /^(?:\[|\{|\"(?:\\.|[^\"])*\"|'(?:\\.|[^'])*'|-?\d+(?:\.\d+)?|scalar\s*\(|array\s*\(|hash\s*\(|trim\s*\(|lowercase\s*\(|uppercase\s*\(|length\s*\(|replace_substr\s*\(|num_abs\s*\(|num_floor\s*\(|num_ceil\s*\(|num_round\s*\(|num_add\s*\(|num_sub\s*\(|num_mul\s*\(|num_div\s*\(|num_mod\s*\(|num_clamp\s*\(|num_min\s*\(|num_max\s*\(|starts_with\s*\(|ends_with\s*\(|contains_substr\s*\(|matches\s*\(|coalesce_nonempty\s*\(|is_empty\s*\(|is_nonempty\s*\(|count\s*\(|first\s*\(|last\s*\(|tail\s*\(|concat_arrays\s*\(|contains\s*\(|count_keys\s*\(|sorted_keys\s*\(|sorted_values\s*\(|has_key\s*\(|merge_hash\s*\(|set_key\s*\(|rename_key\s*\(|drop_keys\s*\(|pick_keys\s*\(|join_values\s*\(|coalesce\s*\(|flat_array\s*\(|flat_hash\s*\(|flatten\s*\(|flat\s*\()/o ? 1 : 0
}
