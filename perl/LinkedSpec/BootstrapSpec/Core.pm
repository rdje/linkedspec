...
sub _method_chain_return_uses_general_payload {
 ...
 return $trimmed =~ /^(?:\[|\{|\"(?:\\.|[^\"])*\"|'(?:\\.|[^'])*'|-?\d+(?:\.\d+)?|array\s*\(|hash\s*\(|trim\s*\(|lowercase\s*\(|uppercase\s*\(|length\s*\(|replace_substr\s*\(|rm_prefix\s*\(|rm_suffix\s*\(|cat\s*\(|num_abs\s*\(|num_floor\s*\(|num_ceil\s*\(|num_round\s*\(|num_sum\s*\(|num_avg\s*\(|num_median\s*\(|num_range\s*\(|num_add\s*\(|num_sub\s*\(|num_mul\s*\(|num_div\s*\(|num_mod\s*\(|num_clamp\s*\(|num_min\s*\(|num_max\s*\(|abs\s*\(|floor\s*\(|ceil\s*\(|round\s*\(|sum\s*\(|avg\s*\(|median\s*\(|range\s*\(|add\s*\(|sub\s*\(|mul\s*\(|div\s*\(|mod\s*\(|clamp\s*\(|min\s*\(|max\s*\(|starts_with\s*\(|ends_with\s*\(|contains_substr\s*\(|matches\s*\(|coalesce_nonempty\s*\(|is_empty\s*\(|is_nonempty\s*\(|count\s*\(|first\s*\(|last\s*\(|take\s*\(|slice\s*\(|take_last\s*\(|drop_back\s*\(|drop_front\s*\(|concat_arrays\s*\(|split_tagged_records\s*\(|sorted\s*\(|reversed\s*\(|contains\s*\(|index_of\s*\(|count_keys\s*\(|sorted_keys\s*\(|sorted_values\s*\(|has_key\s*\(|merge_hash\s*\(|set_key\s*\(|rename_key\s*\(|drop_keys\s*\(|pick_keys\s*\(|join_values\s*\(|coalesce\s*\(|copy\s*\(|flat_array\s*\(|flat_hash\s*\(|flat\s*\(|(?:capture_slice|capture_slice_len|capture_slice_length|capture_slice_until_cursor|capture_slice_until_cursor_len|capture_take_until_cursor|capture_take_until_cursor_len|capture_take_rest|capture_take_rest_len|capture_take|capture_take_len|capture_rest|capture_rest_len|capture_rest_length|capture_slice_pos|capture_slice_line|capture_slice_col|capture_from_rule_start|capture_len_from_rule_start|capture_from|capture_len_from|capture_until_cursor_from|capture_until_cursor_len_from|capture_take_until_cursor_from|capture_take_until_cursor_len_from|capture_take_rest_from|capture_take_rest_len_from|capture_take_len_from|capture_rest_from|capture_rest_len_from|capture_between|capture_len_between|capture_take_between|capture_take_between_len|entry_text|match_text|entry_group|match_group|entry_groups|match_groups|input_text|input_len|input_slice)\s*\()/o ? 1 : 0
}
