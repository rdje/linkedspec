...
sub _lower_flow_composite_expr {
 ...
 if ($trimmed =~ /^(?:scalaref|scalar|array|hash|trim|lowercase|uppercase|count|first|last|contains|count_keys|sorted_keys|sorted_values|has_key|merge_hash|drop_keys|pick_keys|join_values|coalesce|array_copy|array_values)\s*\(/o) {
