...
# Purpose : Lazy-load one flow-expression dependency owner through the shared
#           owner-dispatch seam.
# Args    : ($pkg)
...
#           flow-expression helper callback.
# Args    : ($cb)
...
# Purpose : Lazy-load one flow-expression dependency owner and resolve one
...
sub _looks_like_array_value_expr {
 ...
 return 1 if $method =~ /^(?:array|array_copy|array_values|sorted|reversed|sorted_keys|sorted_values|tail|drop_front|take|slice|take_last|drop_last|drop_back|concat_arrays|split|split_each|trim_each|filter_nonempty|lowercase_each|uppercase_each|uniq|filter_match)$/o;
