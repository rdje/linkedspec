...
# Purpose : Lazy-load one contracts dependency owner through the shared
#           owner-dispatch seam.
# Args    : ($pkg)
...
# Purpose : Lazy-load one contracts dependency owner and resolve one callback
...
sub _build_return_contracts {
 ...
 return [
  ...
  },
  {
   ...
   ir_node            => 'RETURN',
   diag_name          => 'return',
   unresolved_pattern => qr/\breturn\s*\(\s*(?:\[|\{|\"|'|-?\d+(?:\.\d+)?|(?:scalar|s)\s*\(|(?:array|a)\s*\(|(?:hash|h)\s*\(|flat_array\s*\(|flat_hash\s*\(|flatten\s*\(|flat\s*\()/o,
   lower              => sub {
    my ($code) = @_;
