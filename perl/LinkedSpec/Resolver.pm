...
sub resolve_spec_path {
 ...
 unless ($spec_path) {
  if ($is_explicit_path || $is_explicit_spec_name) {
   if (-e $spec_name && !-f $spec_name) {
    ...
    my $summary = 'Spec path is not a file';
    my $detail = "spec='$spec_name' resolved='$spec_name' type='$path_type'";
    _notify_failure(
     $option,
     summary => $summary,
     detail => $detail,
   ...
   my $summary = 'Spec path not found';
   my $detail = "spec='$spec_name' resolved='<undef>'";
   _notify_failure(
    $option,
    summary => $summary,
    detail => $detail,
 ...
 unless ($ok) {
   my $summary = "Unable to resolve spec '$spec_name'";
   my $detail = "PathSearch load failed: $@";
   _notify_failure(
    $option,
    summary => $summary,
    detail => $detail,
  ...
  if ($@) {
   my $summary = "Unable to resolve spec '$spec_name'";
   my $detail = "PathSearch runtime failure: $@";
   _notify_failure(
    $option,
    summary => $summary,
    detail => $detail,
 ...
 if ($spec_path && -e $spec_path && !-f $spec_path) {
  ...
  my $summary = 'Spec path is not a file';
  my $detail = "spec='$spec_name' resolved='$spec_path' type='$path_type'";
  _notify_failure(
   $option,
   summary => $summary,
   detail => $detail,
 ...
 unless ($spec_path && -f $spec_path) {
  my $summary = 'Spec path not found';
  my $detail = "spec='$spec_name' resolved='".($spec_path // '<undef>')."'";
  _notify_failure(
   $option,
   summary => $summary,
   detail => $detail,
