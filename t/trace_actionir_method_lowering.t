...
subtest 'MethodLowering traces assignment and mutation decisions' => sub {
 ...
 like($hash_assign, qr/\$__ls_path_value = \$name;.*BindingRuntime::nested_write\(\$meta, .*"meta"/s, 'one-segment assignment lowers through the unified typed path binding');
 like($hash_assign_trace, qr/DECISION actionir:method_lowering:lower_hash_index_assignment_operator_statement:assignment:ast_hash_or_nested_access_assignment => TAKEN/, 'trace reports the unified AST nested-write assignment');
