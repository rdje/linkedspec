...
function M.parser_from_spec_source(source, options)
  ...
  local parsed = run_stage(
    ...
    function() return spec_parser.parse_spec(source) end
  )
  run_stage(
  ...
  local compiled = run_stage(
    ...
    function() return compiled_spec.compile_spec(parsed) end
  )
  return setmetatable({
...
local function build_default_parser()
  local loaded = run_stage(
    ...
    function()
      return spec_loader.load_spec(
        ...
        spec_loader.spec_load_options({ cwd = module_directory(), search_roots = {} })
      )
    end
  )
...
local function default_user_function_definition_parser()
  if default_parser == nil then default_parser = build_default_parser() end
  return default_parser
end

function M.default_parser_metadata()
  local parser = default_user_function_definition_parser()
  return json.harray({
...
function M.parse_user_function_definition_asts(source, parser)
  source = require_string(source, "user-function source")
  parser = require_parser(parser or default_user_function_definition_parser())
...
function M.parse_spec_with_staged_user_function_definitions(source, parser)
  local nodes = M.parse_user_function_definition_asts(source, parser)
  return staged_parser_registry.parse_spec_with_staged_user_function_definition_asts(source, nodes)
end
