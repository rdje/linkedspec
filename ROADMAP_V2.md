...
Dart `.1-.3` are complete at 4/7. Julia `.4.0` is the active behavior-free preflight; its core `.1` and routes
`.2` precede cursor-dependent final admission `.3`. Julia and Lua validation still block no-marker fallback, so
Lua/LuaJIT `.5` and composed public no-drift `.6` remain pending and the fallback is not yet uniform.
