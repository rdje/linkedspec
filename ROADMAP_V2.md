...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection remains exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6; MCP is complete/141, and parents `.10`, `.11`, `.14.1`, and `.24` are closed. Typed-source rollout remains 6 complete / 8 pending / 40 mutations with Perl/Rust/Dart admitted and Julia/Lua pending. Alias parity landed cleanly at `3a848631`; signoff-complete dormant Julia RED `.14.2.4.0.2` freezes a private non-colliding `LinkedSpecJulia.SourceLocation` namespace, all 3/7/6/3 value fixtures, four private diagnostics, detached exact 92+7 catalogs, and native/reconstructed/generated-plan behavior outside ordinary/canonical discovery. Core and projection selections parse completely and currently fail only for the absent namespace; production, schemas, registers, neutral/capability state, and public behavior do not move. Complete Julia passes its byte-fresh MCP binding, package, storage 19/5, CLI, corpus 105/105, and exact marker; language remains 246/105+1/122. The source-unchanged sole-facing book accurately leaves Julia values/projections pending and renders 79 files/14,168 KiB in separate audited blocks. The already-declared `.14.2.4.1-.3` core/routing/admission nodes are fully instantiated. Knowledge Map 787/6,455, all seven doctrines, containment/relocation, CLI 66x2, RAM 62%, and Phase 0 1,031/1,031 in 705 wallclock seconds pass. This leaf is intended atomic commit 159/300; immutable core `.14.2.4.1` follows only after clean landing. The nonurgent rendered-readability tree stays queued; `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
