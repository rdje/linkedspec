...
reject 98 mutations; static rule facts cross-check `linkedspec-rule-local-cursor-v1` after correction `.10.3.2.0`
...
rollout is 6 complete / 3 pending, backend admission is 6 complete / 0 pending, and `.10.3-.10.10` split all
...
advances to 6/20/98 at 6/9 + 6/6; `.10.7` is closed and recurring six-runtime proof `.10.8` is active
task-tree-first from clean `e335ea8b`. Its behavior-free plan freezes one exact repository-routed driver over all
admitted consumers, three 5x2 primary no-drift cases, generated/capability/language ledgers, canonical opt-in,
seven topology mutations, premature-MCP denial, and recurring-only promotion before implementation.
