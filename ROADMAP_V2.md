...
observation parent without behavior, format, rollout, or admission change. Exact Julia admission `.10.6.7` is
active from clean `48b7d96d`; governance remains unchanged until its ordered consumer and full signoff pass.
...
| Julia backend parity | `in progress` (semantic admission active) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root/cursor/repeated-action exact admissions and their recurring/public proof remain closed. Unicode-17 labels, opaque strict source/compiled outcome, all five private static targets, calls/staging/generated 22/25, immutable static query, typed native runtime capture, immutable observed-index derivation, generated/emitted observation propagation, and no-change parent closeout are complete through `.10.6.6.4`. All 20 typed/raw-neutral hashes and all 26 malformed query boundaries match; exact admission `.10.6.7` is active. Shared semantic governance stays rollout 4/9 and native admission 3/6 until that leaf passes. |
