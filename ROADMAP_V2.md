...
compile, path, traversal, or explanation semantics. Six fixture groups now derive 20 digest-locked responses and
reject 73 mutations; static rule facts cross-check `linkedspec-rule-local-cursor-v1` after correction `.10.3.2.0`
...
rollout is 3 complete / 6 pending, backend admission is 2 complete / 4 pending, and `.10.3-.10.10` split all
...
deep-equals the complete private 22/25 target. Composed calls closeout `.10.5.3.3` is next.
...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Lua `.8.4` closes full current backend parity at capability 80/0/0; diagnostic-output `.5.1`, logical-helper `.5.2`, and rule-local cursor/root/duplicate-slot/repeated-action governance are closed. ADRs `0049`/`0050` and neutral `.10.2` freeze six semantic fixture groups and 20 exact responses. Perl `.10.3` and Rust `.10.4` are admitted by exact 12-role consumers; governance is 73 mutations, rollout 3/9, and native admission 2/6. Dart audit `.10.5.0` froze the shared self-hosted label/current-corpus closure `.10.5.0.1`, Dart Unicode-label parity `.10.5.0.2`, and semantic leaves `.10.5.1-.10.5.6`. Shared `.10.5.0.1.0-.2` now generate/consume the exact 806-range class at all 12 canonical sites, freshness-lock four corpus inputs, reject prefix truncation, and pass the five-runtime default/POSIX matrix. Dart `.10.5.0.2.0-.4` generate and consume the matching classifier/scanner, preserve all positive/distinct labels through every identity route, reject every negative without broadening adjacent grammars, and pass composed Dart/canonical signoff without semantic promotion. Dart source/outcome parent `.10.5.1` is complete across behavior-free contract `.0`, strict copied input/private Unicode map `.1`, staged compiled-or-failed authority `.2`, and composed closeout `.3`. `SemanticIndex` owns strict source policy/mapping plus one staged parse/validate/compile/entry pass, immutable compiled-or-failed snapshot/native-diagnostic/entry/generated-v2-plan values, and no target execution or host-state exposure; closeout also makes the retired-selector scanner discover nonignored untracked sources. Dart static projection `.10.5.2` is composition-closed across all five private construction targets plus clone/privacy/lifecycle isolation. Calls audit `.10.5.3.0` freezes exact registry/ActionIR/sidecar/generated authorities and splits typed core `.1`, exact completion `.2`, and closeout `.3`; exact completion `.2` now deep-equals all 22 records / 25 relations with distinct staged payload/job/result and selected generated-plan provenance, and composed closeout `.3` is next. `.22` tracks the recurring mutable task-index marker-anchor defect; `.23` owns full-book semantic drift repair and stronger migration-example guards. Later semantic backend lanes inherit Unicode label alignment before fixture admission. Format-program and inter-match gap activation remain separate; mutation execution stays manual; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes plus cursor normalization/runtime/descriptor/generated/public removal and separate exact 15-role cursor/root admissions remain signed off. Repeated-action `.9.1.10.4` adds one exact 15-role native/loaded/reconstructed/emitted/generated/traced/primary/corpus admission: focused 162, package 3,711, primary 65x2, and corpus 105. Bare OR is minimum-one repetition, action returns collect per hit, lifecycle remains authoritative, and generated source stays v2. Dual-ABI Lua, recurring proof, and public no-drift are closed at the shared 8/0/54 ledger. |
