...
artifacts pass. Staged/generated `.2` remains next after the clean commit.
...
| Julia backend parity | `in progress` (rule-local cursor, root selection, and repeated-action results admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root/cursor/repeated-action exact admissions and their recurring/public proof remain closed. Unicode-17 labels, opaque strict source/compiled outcome, and all five private static targets are composition-closed. Calls/staging/generated authority is frozen at 22/25 and private typed core `.10.6.4.1` is exact at 18/16; staged/generated `.2`, closeout `.3`, then query/runtime/admission `.5-.7` remain. Shared semantic governance stays rollout 4/9 and native admission 3/6. |
