...
combined `.14.8` follow. Capacity `.0` freezes and canonical `.1` implements one equally bounded
`.14.6.5-.14.8` semantic task member without changing stable IDs, pressure ceilings, or backend behavior.
Canonical signoff is complete; independent `.2` now recomposes it before Julia task splitting.
