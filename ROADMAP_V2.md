...
| Trace observability | `active` (`.5.1` done; `.5.2-.3` pending) | Overall roadmap — engine observability / developer experience | Perl reference trace coverage and original Rust controls/events/parity proof remain as landed through `.4.5`. Corrective `.5.1` restores the ordinary progressive static validator on `compile_with_events` and locks exact ordinary/traced rejection equality while preserving valid results and trace schemas. `.5.2` owns missing gap-aware interpreted/generated `child_dispatch` lifecycle events. `.5.3` owns the stale private progressive-authority no-canonical-route assertion left behind by later Rust contract admission. Renew full Rust trace parity only after `.5.2`; complete `.5.3` before returning to staged Rust `.14.7.4.2`. |
