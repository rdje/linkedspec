...
limits; approved capacity implementation 4489f5e9 and independent proof a7d392a8 support `.7.4` admission. Canonical admission a67a18bf precedes completed [DART-STARTUP-READING.0](docs/tasks/DART-STARTUP-READING.md) decomposition: 55 pending children / 169 exact ranges. Dart `.1.51` completes 51/55 children, 74,528 fragments / 2,298,209 bytes. Dart .1.51 reads staged enrichment tests292-1791: 1,500 fragments / 50,285 baseline-identical bytes. All 19 tests and staged/typed-source neutral checks pass, covering private provenance, frozen authority, atomic stitching, recursive budgets and rebasing. Prior .2.17/.2.18/.2.19 defects remain open; whole-consumer execution adds no unread source credit or MCP exposure. Capacities remain canonical at 7d52c98e / 9c644ecb / bef5dafd under containment .8/.9/.10 and ADR0110/0111/0112; prior history and other limits remain. CI intake .80.0 is committed at eaf4331e. Director-approved .7 is implemented by containment .11 / ADR0113: exactly engineering files27→28 and manifest lines26→27; actual211-line/24521-byte source preservation and 22-validator checks pass, with the director’s explicit September 11 one-time focused/receipt exception. Capacity is committed at ad64f76f; next Dart .1.52 after the clean .1.51 handoff. Switch .2.1, regex .2.2, numeric trace .2.3 and recognition effect integration .2.4 repairs remain owned and gated; startup .54.3 retains broad regex recurrence. Confirmed attached-switch omission/default replacement is owned by Dart .2.1, with repair gated by startup reading/policy prerequisites. Startup `.3.4` remains pending until reading closeout. Repairs `.7`–`.30`, `.32`–`.47`, and
...
CI evidence intake `SESSION-STARTUP-READING.80.0` records eleven visible Rust test-build stages / 78m02s, six absent optional PGEN watches, and a separate newer-OS pre-main observation. Pending `.80.1-.4` implement the director-required PGEN/RGX build-on-submodule-update lifecycle, with no dependency compilation for repeated CI or LinkedSpec-only edits and `.81.1-.2` own current-OS causal diagnosis/remediation; all implementation gates remain. Canonical capacity commit `bef5dafd` passes nine doctrines, required consumers, CLI66x2 and Phase0 1032/1032 (1163s for Phase0 only; 25 optional skips). Containment `.11` closes under that explicit one-time focused exception at ad64f76f; next Dart `.1.52` after completed .1.51; the original CI intake is committed at eaf4331e and the build-on-update directive is now recorded without claiming implementation.
