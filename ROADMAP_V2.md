...
Progressive `.14.6.0-.1`, Perl `.2`, Rust `.3`, Dart `.4`, and Julia `.5.0-.4` are complete at
...
canonically. Shared Lua `.6`, recurrence `.7`, public `.8`, staged `.14.7`, and
...
format, rollout, typed, or outward movement. Canonical dual-ABI admission `.3` is next. Mandatory
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current. Progressive neutral + Perl + Rust + Dart + Julia remains 5/9/106; Julia `.14.6.5.0-.4` is closed. Shared Lua `.14.6.6.0` preserves the historical dual-ABI 85-pass/one-RED boundary, `.1` supplies one private shared authority at 273/273 per ABI, and dormant `.2` makes the final path GREEN at 178/178 per ABI through four fresh-authority routes without rollout movement. Admission `.3` is next; recomposition `.4`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
