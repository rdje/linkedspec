...
Dart nested write-vivification `.19.4.1` is next while Julia/Lua remain non-vivifying/non-bang.
...
Rust `.19.3` closes and Dart nested write-vivification `.19.4.1` is next.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Capability is 90/0/0; semantic introspection is closed at 9/0/128; typed source is 14/0/231. Progressive and staged language programs, standalone lifecycle blocks, task/index `.22`, and mdBook drift `.23` are closed. Write-vivification `.19.1.1`, `map_leaves!` receiver mutation `.19.1.2`, and strict composition `.19.1.3` freeze 105 write and 167 map mutations; the corrected current composition total is 592. Perl `.19.2.1-.2` and Rust `.19.3.1-.2` implement both mechanisms, including typed generated/emitted carriers and identity-guarded mutation. Rust oracle/root-semantics cleanup `.19.3.3` is canonical-signoff-complete; `.19.3.4.0` classifies older-artifact launch latency as external macOS policy/cache state, requires no repair, and closes Rust `.19.3`. Dart `.19.4.1` is next; Julia/Lua and portable/public admission remain pending. Codegen-inspector `.13.1` remains closed; bounded histories retain their separately authorized capacities. |
