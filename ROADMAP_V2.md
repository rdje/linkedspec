...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity is 80/0/0. Semantic introspection remains exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6; MCP is complete/141. Typed-source internal value/helper parent `.14.2` is composition-closed at 8 complete / 6 pending / 53 mutations across all six runtimes. Transaction audit `.14.3.0` proves the current cursor-only stack/rule-label-mark boundary, and `.14.3.1.0` ratifies the four exact `recognition_*` forms, falsey-safe match/payload split, linear token, closed 9/11 effects, invocation marks, and cursor-only progress. Executable neutral `.14.3.1.1` locks 128 current + 4 dedicated node rows, 246 call rows, token 8/17, effect graphs 6, marks 6, progress 8, and diagnostics 15. Rendered recomposition `.14.3.1.2` repairs one stale neutral-next sentence; guard `.14.3.1.2.0` locks the separate public projection. Perl `.14.3.2` is composition-complete with an exact canonical 51-test admission. Rust `.14.3.3` is composition-complete and landed at `02c612f5` as atomic 191/300: admission removes both custom cfgs, keeps the authority documentation-hidden, and runs the unchanged final-path consumer ordinarily and canonically at 12/12. Only Rust advances: current truth is 132 ActionIR rows / 246 calls / 42 semantic mutations / rollout neutral + Perl + Rust 3/9. Exhaustive sole-facing review advances public governance to three pages / eleven forbidden claims / 25 mutations after finding three adjacent Perl-only claims outside the earlier inventory; the separate capability guide is 1/4/8 and exact Rust registration/dormancy is 8 mutations. Rust-local and definitive canonical proof pass, including Perl 51/51, Rust 12/12, six-family containment, relocation, CLI 66x2, RAM 77%, and Phase 0 1,031/1,031 in 739 seconds. Dart retrieval then exposed one stale Perl-only `policy.current_boundary` that its checker also hard-coded. Corrective `.14.3.4.0.0` derives that exact boundary from expected rollout without changing behavior or counts and is signoff-complete for atomic 192/300 after definitive canonical proof (CLI 66x2, RAM 45%, Phase 0 1,031/1,031 in 721 seconds); dormant Dart RED `.14.3.4.0.1` follows only after clean landing. Dart, Julia, PUC Lua, LuaJIT, recurring composition, and final public no-drift remain RED. Reversed-span remains `.14.2`-owned and `.14.8` retains final public no-drift. The stale active-tree prose frontier is repaired and prevention is owned by `TASK-TREE-METADATA-HYGIENE.5`. The nonurgent rendered-readability tree stays queued; `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
