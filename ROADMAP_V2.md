...
contracts are frozen without public admission. Perl `.19.2.1` now implements the unchanged nested-write contract
only; Rust/Dart/Julia/Lua remain non-vivifying and Perl `map_leaves!` `.19.2.2` is next.
...
same-spelling shadow identity, and post-commit receiver-write failure are frozen at 593 composition mutations.
...
Rust, Dart, Julia, and Lua remain non-vivifying; no capability/public admission occurs. Perl `map_leaves!` `.19.2.2`
is next.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Capability is 90/0/0; semantic introspection is closed at 9/0/128; typed source is 14/0/231. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2`, complete internal Perl `.3`, Rust `.4`, Dart `.5`, Julia `.6`, shared Lua `.7`, exact five-source/six-route recurrence `.8`, public authoring/no-drift `.9`, and unchanged-owner recomposition `.10` are closed. Final combined `.14.8` composes all six existing recurring authorities and closes the exact 14-row authoring model without outward movement. Staged governance remains 9/9/123 plus public 6/17/10/129; generated v2 and function-body v1 remain unchanged. Standalone lifecycle-block `.15.0-.2` remains admitted; `.15.3` repairs the stale Lua `next()` fixture exposed by executable bare-block normalization through exact action-edge ownership without changing lifecycle/runtime behavior. Task/index `.22` and mdBook drift `.23` are closed. Write-vivification `.19.1.1`, `map_leaves!` receiver-mutation `.19.1.2`, and strict composition `.19.1.3` are frozen at 105 write, 167 map, and 593 composition mutations. Perl `.19.2.1` now implements only the unchanged nested-write contract with typed evaluated paths, presence-aware isolated dense commit, diagnostics, detachment, and rule/function state; other backends and public capability remain unchanged. Perl `map_leaves!` `.19.2.2` is next. Codegen-inspector repair `.13.1` restores all five documented block/chain executions through the explicit `BootstrapSpec::Core` and `RuleIR::EmitContext` owners with a recurring CI smoke and no public-facade expansion. Mandatory change-history segment `4989` is finitely authorized at 24/23 by ADR `0093`; engineering-notes segment `4987` is separately authorized at 21/20 by ADR `0097`. Reversed-span and unrelated hygiene/readability remain separately owned. |
