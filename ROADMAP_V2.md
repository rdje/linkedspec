...
limits; approved capacity implementation 4489f5e9 and independent proof a7d392a8 support `.7.4` admission. Canonical admission a67a18bf precedes completed [DART-STARTUP-READING.0](docs/tasks/DART-STARTUP-READING.md) decomposition: 55 pending children / 169 exact ranges. Dart `.1.36` completes 36/55 reading children (52,028 fragments / 1,599,420 bytes). Action-contract and callable tests are complete; compiled tests reach 418. All 38 selected tests and neutral callable7/11/9/7/4/8/23 checks pass, including fresh offline standalone emitted fixture/contextual/seven-error execution. No new defect; all earlier findings retain evidence and gates. Approved engineering-history capacity remains canonically committed at 9c644ecb under containment .9 / ADR0111. The director approved .6; containment .10 / ADR0112 admits exactly files31→32, manifest lines30→31/bytes17039→17615 from clean e55f7703. Fresh source reconstruction and 22 actual-validator executions preserve every earlier history byte. Containment .10 is canonically committed at bef5dafd; next .1.37 follows focused CI intake .80.0. Other limits and repair gates remain. The director-approved history exception is committed at 7d52c98e under containment `.8` / ADR0110. Switch .2.1, regex .2.2, numeric trace .2.3 and recognition effect integration .2.4 repairs remain owned and gated; startup .54.3 retains broad regex recurrence. Confirmed attached-switch omission/default replacement is owned by Dart .2.1, with repair gated by startup reading/policy prerequisites. Startup `.3.4` remains pending until reading closeout. Repairs `.7`–`.30`, `.32`–`.47`, and
...
CI evidence intake `SESSION-STARTUP-READING.80.0` records eleven visible Rust test-build stages / 78m02s, six absent optional PGEN watches, and a separate newer-OS pre-main observation. Pending `.80.1-.4` own correct Cargo reuse and `.81.1-.2` own current-OS causal diagnosis/remediation; all implementation gates remain. Canonical capacity commit `bef5dafd` passes nine doctrines, required consumers, CLI66x2 and Phase0 1032/1032 (1163s for Phase0 only; 25 optional skips). Dart `.1.37` is next after this focused intake.
