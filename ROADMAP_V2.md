...
consume mapping, rejects v1 before payload reconstruction, and removes private v1 execution. Current proof is
package 3,187, primary 65/65x2, corpus 105, and neutral 66/4+4/39. Option-removal `.5` deletes the caller-global
engine/loader/corpus/primary seam, supplies targeted legacy diagnostics, removes the help/request-trace field, and
preserves entry selection; cursor admission `.6` then precedes root admission `.9.1.1.2.4.3`.
...
| Dart backend parity | `done` (foundation and rule-local cursor admitted) | First future full-parity backend lane after Perl5 and Rust. | The original parity tree closed at 190 tests, 61/61 default/POSIX, and 105/105 corpus. Rule-local cursor `.9.1.5` now closes normalization, live/loaded/reconstructed execution, descriptor v1, generated-source v2, public/CLI override removal, and one exact 15-role admission. Current proof is package 271, primary 65x2, corpus 105/105, and neutral 67 files / 4 complete / 4 pending / 39 mutations. |
| Julia backend parity | `in progress` (cursor implementation through public removal complete; admission pending) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes plus cursor normalization/runtime/descriptor/generated/public removal `.9.1.6.1-.5` are signed off. Runtime derives policy at every entered rule; descriptor v1 and generated v2 carry no global override; legacy APIs/CLI fail exactly; package 3,187 and shared primary 65/65x2 pass. Rollout remains 4/4 until composed cursor admission `.6`, before root admission `.4.3`. |
