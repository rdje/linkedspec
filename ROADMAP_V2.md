...
nested-write `.19.6.1` now passes 436 assertions per ABI plus its complete gate; it remains non-bang and `.19.6.2`
is next.
...
nested-write `.19.6.1` now passes 436 assertions per ABI across native, reconstructed, generated-plan,
emitted-module, and CLI routes. Lua `map_leaves!` `.19.6.2` and portable/public admission remain pending.
