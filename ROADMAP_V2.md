...
canonically. Recurrence `.7`, public `.8`, staged `.14.7`, and combined `.14.8` follow. Capacity `.0` freezes,
...
unchanged, closes the parent, and hands off typed recurring proof `.14.6.7`. Mandatory
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current. Progressive private runtime admission is independently recomposed at 7/9/112 across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT; Julia `.14.6.5.0-.4` and shared Lua `.14.6.6.0-.4` are closed. Lua `.0` preserves the historical dual-ABI 85-pass/one-RED boundary, `.1` supplies one private shared authority at 273/273 per ABI, `.2` makes the final path GREEN at 178/178 per ABI through four dormant routes, `.3` admits that unchanged consumer exactly once per ABI, and `.4` proves every committed owner unchanged. Recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
