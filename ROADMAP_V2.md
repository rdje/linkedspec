...
| Trace observability | `active` (`.5.1-.2` done; `.5.3` pending) | Overall roadmap — engine observability / developer experience | Perl reference trace coverage and original Rust controls/events/parity proof remain as landed through `.4.5`. Corrective `.5.1` restores the ordinary progressive static validator on `compile_with_events` and locks exact ordinary/traced rejection equality. `.5.2` routes normal and gap-aware child entry through one traced local seam per Rust executor, restoring exactly one existing `child_dispatch` / result pair; trace controls pass 11/11 with unchanged results, so full Rust trace parity is current again. `.5.3` next repairs the stale private progressive-authority no-canonical-route assertion left by later Rust contract admission before staged Rust `.14.7.4.2`. |
