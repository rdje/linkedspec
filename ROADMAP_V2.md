...
| Inter-match gap capture | `active; Dart private native .4.2 signoff-complete for intended atomic 236` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl and Rust are privately admitted at governance 3/6/56 plus ten Rust admission mutations. Dart `.4.1` supplies authored/static/compiled identity; `.4.2` now supplies private same-authority native state, candidate-before-`LS`, post-`LE` commit, terminal tails, Unicode-scalar accessors, detached entry identity, rollback/nesting, and typed failures. The consumer remains dormant and ten Dart mutations lock no admission. Reconstructed/descriptor/generated `.4.3`, emitted `.4.4`, and primary/admission `.4.5` remain pending. Four later runtime routes and both public rows remain pending. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, generated plan v2 unchanged, and outward surfaces guarded. |
