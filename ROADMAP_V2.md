...
root-semantics cleanup `.19.3.3` is canonical-signoff-complete; measured macOS first-launch/cache-interference
audit `.19.3.4.0` is next before Rust parent closeout, while Dart/Julia/Lua remain non-vivifying/non-bang.
...
`.19.3.4.0` next owns controlled classification of macOS first-launch latency plus observed cache interference.
