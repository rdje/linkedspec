...
representative query. Leaf `.1` must extend those existing consumer identity roles to all twenty neutral
requests/digests and compose them through one repository-routed opt-in gate before coordinated thin-transport
promotion. No-change `.2` then recomposes the committed owners and closes `.10.9`; no production server,
transport byte, primary CLI, aggregator, or legacy behavior moves in the plan.
