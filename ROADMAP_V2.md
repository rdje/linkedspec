...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection remains exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6; MCP is complete/141. Typed-source internal value/helper parent `.14.2` is composition-closed at 8 complete / 6 pending / 53 mutations across all six runtimes. Transaction audit `.14.3.0` proves the current cursor-only stack/rule-label-mark boundary, and `.14.3.1.0` ratifies the four exact `recognition_*` forms, falsey-safe match/payload split, linear token, closed 9/11 effects, invocation marks, and cursor-only progress. Executable neutral `.14.3.1.1` locks 128 current + 4 dedicated node rows, 246 call rows, token 8/17, effect graphs 6, marks 6, progress 8, and diagnostics 15. Rendered recomposition `.14.3.1.2` repairs one stale neutral-next sentence; guard `.14.3.1.2.0` locks the separate public projection. Dormant Perl RED `.14.3.2.0` freezes 49 passing assertions / one missing-four-node failure; private authority `.14.3.2.1` adds opaque invocation/frame/token ownership and 293 nested TAP assertions; integration `.14.3.2.2` adds four dedicated ActionIR nodes, recursive effect closure, live/generated invocation state, falsey-safe results, and cursor-only typed progress. Admission `.14.3.2.3` now requires, syntax-checks, and executes the exact 51-test final-path consumer, promotes only Perl, and advances current truth to 41 semantic mutations / rollout 2/9 plus three public pages / eight forbidden claims / fourteen sequence mutations. Rust dormant RED `.14.3.3.0` landed as atomic 188/300 and guards the separate capability-guide surface at 1/2/6. Private authority `.14.3.3.1` landed at `fd8a1934` as atomic 189/300. Integration `.14.3.3.2` is signoff-complete for atomic 190/300: four dedicated cfg-private nodes, neutral effect/progress policy, live cursor/boundary/invocation-mark/acceptance binding, and native/reconstructed/generated-plan/independently compiled emitted carriers pass nested 12/12 while outer authority remains 7/7 and ordinary discovery stays zero-test. The complete Rust local and canonical gates pass; recognition 132/246/41, rollout 2/9, public 3/8/14, guide 1/2/6, manifests, canonical registration, and current support remain unchanged. Admission `.14.3.3.3` is next after clean landing. Rust authored support, Dart, Julia, PUC Lua, LuaJIT, recurring, and public-no-drift remain RED. Reversed-span remains `.14.2`-owned and `.14.8` retains final public no-drift. The stale active-tree prose frontier is repaired and prevention is owned by `TASK-TREE-METADATA-HYGIENE.5`. The nonurgent rendered-readability tree stays queued; `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
