...
Dart `.19.4.1` now implements nested write-vivification through native/reconstructed/generated/emitted/CLI routes;
Dart `map_leaves!` `.19.4.2` is next while Julia/Lua remain non-vivifying/non-bang.
...
write guard, original-shape atomic mapping, detached results, and generated-route proof. Dart, Julia, and Lua
remain non-vivifying/non-bang; no capability/public admission occurs. Perl `map_leaves!` `.19.2.2`
...
Rust `.19.3` closes. Dart `.19.4.1` now preserves the unchanged expression-bearing path and typed diagnostics
through native, reconstructed `SpecFile`, generated-plan, emitted-source, independently analyzed/executed caller,
and primary-CLI routes; complete Dart proof passes 450 tests, CLI 66x2, and corpus 105/105. Dart `map_leaves!`
`.19.4.2` is next, while Julia/Lua and portable/public admission remain pending.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Capability is 90/0/0; semantic introspection is closed at 9/0/128; typed source is 14/0/231. Progressive/staged/standalone/task-index/mdBook-drift lanes are closed. Write-vivification `.19.1.1`, `map_leaves!` `.19.1.2`, and composition `.19.1.3` freeze 105 / 167 / 592 mutations. Perl `.19.2.1-.2` and Rust `.19.3.1-.2` implement both mechanisms; Rust `.19.3.3-.4.0` closes root-semantics cleanup and external launch classification. Dart `.19.4.1` implements nested write-vivification through native, reconstructed, generated, emitted, and CLI routes with 450/450 package, CLI 66x2, and corpus 105/105 proof. Dart `map_leaves!` `.19.4.2` is next; Julia/Lua and portable/public admission remain pending. Codegen-inspector `.13.1` remains closed; bounded histories retain their separately authorized capacities. |
