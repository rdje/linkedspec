...
Julia, and dual-ABI Lua `.1-.5` are complete at 6/7 with 44 rejected mutations. Julia's separate cursor and root 15-role
...
in 641 seconds. Composed public no-drift `.6`
remains final; markerless fallback is implemented and backend-admitted but not yet publicly closed.
...
1,031/1,031 in 643 seconds. Cursor `.9.1.7` and admission `.5.3` are now complete; only no-drift `.6` remains,
so rollout is 6/7.
