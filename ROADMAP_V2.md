...
`FUTURE-PARITY-BACKLOG.9.1.1.2` owns the rollout. Executable neutral contract `.0` plus composed Perl, Rust, Dart,
and Julia `.1-.4` are complete at 5/7 with 39 rejected mutations. Julia's separate cursor and root 15-role
...
15-role admission `.5.3`. Composed public no-drift `.6` remains final; markerless fallback is not yet uniform.
...
1,031/1,031 in 643 seconds. Cursor `.9.1.7`, admission `.5.3`, and no-drift `.6` remain, so rollout stays 5/7.
...
KM 629/4,611, mdBook/four doctrines, and canonical Phase 0 1,031/1,031 in 621 seconds pass; `.3` is prepared for
commit before `.4` activates.
...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Lua `.8.4` closes full current backend parity at capability 80/0/0; diagnostic-output `.5.1` and logical-helper `.5.2` are closed at 8/0. ADR `0043` is current across all native/generated roles and public no-drift. Parenthesis-free conditions remain excluded; other `.5` helper decisions stay owned. Neutral cursor `.9.1.2`, Perl `.9.1.3`, Rust `.9.1.4`, Dart `.9.1.5`, and Julia `.9.1.6` are complete; rollout is 5/3 at 68 governed migration files with 44 mutations. Root selection has neutral plus Perl/Rust/Dart/Julia admitted at 5/7 with 39 mutations; Lua core `.9.1.1.2.5.1` and routes `.5.2` are implemented. Lua cursor preflight `.9.1.7.0` records the old 34/36 family, 5/13+2/4 normalization, and 0/7 diagnostic boundary. Typed normalization `.1` passes 258/258x2 over exact 36/18/6 semantics; intrinsic runtime `.2` passes 110/110x2 over 36 families, 8/8 child mechanisms, 2/2 structural replacements, and loaded/normalized/trace routes. Descriptor `.3` passes 875/875x2 and projects normalized family/policy/ownership/semantic edges without a global mode. Package 176/177x2, primary 32/65x4, corpus 105/105x2, and rollout stay fixed; generated/public/admission `.4-.6` remain ordered. Admission `.5.3` remains after cursor closeout. Identical dependency-regex identity is queued at `.9.1.8.1`. Format-program activation remains separate; mutation execution stays manual; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
