...
limits; approved capacity implementation 4489f5e9 and independent proof a7d392a8 support `.7.4` admission. Canonical admission a67a18bf precedes completed [DART-STARTUP-READING.0](docs/tasks/DART-STARTUP-READING.md) decomposition: 55 initially pending children / 169 exact ranges, now physically read. Dart `.1.55` completes 55/55 children, 80,297 fragments / 2,471,305 bytes. Startup .3.5.0 freezes Julia reading into 52 owned children / 146 ranges across all 95 baseline-identical files: 75,984 lines / 2,693,170 bytes. Independent reconstruction verifies every byte and range digest; physical Julia reading remains 0/52. A separate bounded JULIA-STARTUP-READING member fits existing controls; .4 owns future history/capacity pressure. Dart reading is closed under ADR0114, with all 69 repairs and its failed gate retained. Next Julia .1.1 reads the manifests and first README range. Capacities remain canonical at 7d52c98e / 9c644ecb / bef5dafd under containment .8/.9/.10 and ADR0110/0111/0112; prior history and other limits remain. CI intake .80.0 is committed at eaf4331e. Director-approved .7 is implemented by containment .11 / ADR0113: exactly engineering files 27→28 and manifest lines26→27; actual211-line/24521-byte source preservation and 22-validator checks pass, with the director’s explicit September 11 one-time focused/receipt exception. Capacity is committed at ad64f76f; Dart .3.2 / ADR0114 close reading; Julia decomposition is complete and next is JULIA-STARTUP-READING.1.1. Switch .2.1, regex .2.2, numeric trace .2.3 and recognition effect integration .2.4 repairs remain owned and gated; startup .54.3 retains broad regex recurrence. Confirmed attached-switch omission/default replacement is owned by Dart .2.1, with repair gated by startup reading/policy prerequisites. Startup `.3.4` is complete under ADR0114; all repairs remain open. Repairs `.7`–`.30`, `.32`–`.47`, and
...
CI evidence intake `SESSION-STARTUP-READING.80.0` records eleven visible Rust test-build stages / 78m02s, six absent optional PGEN watches, and a separate newer-OS pre-main observation. Pending `.80.1-.4` implement the director-required PGEN/RGX build-on-submodule-update lifecycle, with no dependency compilation for repeated CI or LinkedSpec-only edits and `.81.1-.2` own current-OS causal diagnosis/remediation; all implementation gates remain. Canonical capacity commit `bef5dafd` passes nine doctrines, required consumers, CLI66x2 and Phase0 1032/1032 (1163s for Phase0 only; 25 optional skips). Containment `.11` closes under that explicit one-time focused exception at ad64f76f; Dart reading closes under .3.2 / ADR0114; Julia has 52 owned reading groups and .1.1 is next; the original CI intake is committed at eaf4331e and the build-on-update directive is now recorded without claiming implementation.
