...
reject 65 mutations; static rule facts cross-check `linkedspec-rule-local-cursor-v1` after correction `.10.3.2.0`
...
rollout is 2 complete / 7 pending, backend admission is 1 complete / 5 pending, and `.10.3-.10.10` split all
...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Lua `.8.4` closes full current backend parity at capability 80/0/0; diagnostic-output `.5.1` and logical-helper `.5.2` are closed at 8/0. Rule-local cursor/root/duplicate-slot/repeated-action parents are closed with recurring/public proof. ADRs `0049`/`0050` and neutral `.10.2` freeze the semantic model/query through six fixture groups and 20 exact responses; checker governance is 65 mutations. Perl `.10.3` is admitted by one exact 12-role consumer, so rollout is 2/9 and native admission 1/6. Rust audit `.10.4.0` maps compiler/ActionIR/staged/generated/diagnostic/loader/runtime owners and splits source/static/calls/query/runtime/admission work before code. Toolbox leaf `.10.4.0.1` guards exact 6/20/65, rollout 2+7, admission 1+5, and Perl runtime/admission claims with omission/wrong-value proof. Completed `.10.4.0.2` implements ADR `0051` through generated Unicode 17.0.0 `XID_Continue` classification and exact Rust source/artifact/runtime proof. Completed `.10.4.1` adds copied strict-source mapping and opaque immutable compiled-or-failed authority with shared generated-v2 plan input. Completed `.10.4.2` adds exact private graph/privacy/failure/runtime-static v1 projection; completed `.10.4.3` composes typed calls/bindings/staged/generated provenance into the corrected 22-record/25-relation target without public query or promotion. `.10.4.4` follows for immutable query. `.22` tracks the recurring mutable task-index marker-anchor defect for structural repair; `.23` owns full-book semantic drift repair and stronger migration-example guards. Later semantic backend lanes inherit label alignment before fixture admission. Format-program and inter-match gap activation remain separate; mutation execution stays manual; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
