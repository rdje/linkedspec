...
| Inter-match gap capture | `active; Perl preflight .2.0 signoff-complete from db299789; .2.1 next` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Neutral `.1` landed through `db299789` unchanged at 1/8 rollout and 55 mutations. Toolbox-led `.2.0` reproduces current named/directive/accessor absence, exact legacy prefix/interstitial-without-tail behavior, descriptor eligibility, and generated lifecycle order. It freezes `.2.1` grammar/metadata plus dormant RED, `.2.2` same-guard live state and four private source-read ActionIR nodes, `.2.3` emitted/loaded parity, and `.2.4` Perl-only admission to 2/7 rollout and 56 mutations. Canonical signoff passes Phase 0 1,031/1,031 in 745 seconds and the exact opt-in pending route. This leaf changes no grammar/runtime/current count: recognition remains 133/246/58, language remains 122 public helpers, typed source remains 9/5/114, and all outward surfaces remain guarded. |
