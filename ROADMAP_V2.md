...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection remains exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6; MCP is complete/141. Typed-source rollout is 8 complete / 6 pending / 53 mutations: `.14.2.6` composes the six admitted internal runtimes through one repository-routed neutral→Perl→Rust→Dart→Julia→PUC-Lua→LuaJIT driver plus three support ledgers. Five backend source groups map to six runtime routes; canonical CI requires/syntax-checks the driver and exposes `LINKEDSPEC_RUN_TYPED_SOURCE_MATRIX=1`. Existing behavior, schema, README, storage, and hosted-workflow boundaries remain unchanged. The combined recurring/public no-drift row stays pending for `.14.8` with authored values, transactions, observation, and dispatch. Full recurrence, routing, sole-facing book 79/14,204 with separate blocks, Knowledge 790/6,526, all seven doctrines, canonical containment/relocation, CLI 66x2, RAM 78%, and Phase 0 1,031/1,031 in 669 seconds pass. Atomic commit 169/300 is the remaining landing step before `.14.2.7`. The nonurgent rendered-readability tree stays queued; `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
