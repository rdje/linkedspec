...
Director-requested [backend integration guides](docs/tasks/BACKEND-INTEGRATION-GUIDES.md) are active after clean conformance .1.34: inventory .0 is complete and Rust .2.1 is next. Common checkout/version/setup guidance and five native consumer guides will verify dependency preparation and reuse, API/results/errors, grammar packaging and outside-cwd use. The director authorizes this documentation activity before the remaining startup reading; complete it and resume conformance .1.35. ADR0040 keeps one canonical content owner and links the eventual companions. The inventory is complete; native guide and consumer implementation starts with Rust .2.1.

CI evidence intake `SESSION-STARTUP-READING.80.0` records eleven visible Rust test-build stages / 78m02s, six absent optional PGEN watches, and a separate newer-OS pre-main observation. Pending `.80.1-.4` implement the director-required PGEN/RGX build-on-submodule-update lifecycle, with no dependency compilation for repeated CI or LinkedSpec-only edits and `.81.1-.2` own current-OS causal diagnosis/remediation; all implementation gates remain. Canonical capacity commit `bef5dafd` passes nine doctrines, required consumers, CLI 66x2 and Phase0 1032/1032 (1163s for Phase0 only; 25 optional skips). Containment `.11` closes under that explicit one-time focused exception at ad64f76f; Dart reading closes under .3.2 / ADR0114; Julia reading is closed at 52/52 under ADR0117; Lua reading closes all 51 children under ADR0119; startup .3.7.0 owns 21 supporting groups, with SUPPORTING-SOURCE-READING.1.1 next; the original CI intake is committed at eaf4331e and the build-on-update directive is now recorded without claiming implementation.
