...
complete and independently recomposed at six private runtime rows / 7/9/112. Private pre-registered authority, globally rebased same-source views, narrowing ceilings, shared
...
promotes only typed `progressive_span_dispatch`, and makes typed governance 12/2/170 while behavioral governance
stays 7/9/112. Public `.8`, staged `.14.7`, and combined `.14.8` follow. Capacity `.0` freezes,
...
route; progressive public no-drift `.14.6.8` is next. Its mandatory engineering-notes rollover publishes segment
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170 with transaction safety, recursive observation, lossless-gap composition, and progressive recurrence current. Progressive private runtime admission remains 7/9/112 across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT; Julia `.14.6.5.0-.4`, shared Lua `.14.6.6.0-.4`, and typed recurrence `.14.6.7` are closed. Lua `.0` preserves the historical dual-ABI 85-pass/one-RED boundary, `.1` supplies one private shared authority at 273/273 per ABI, `.2` makes the final path GREEN at 178/178 per ABI through four dormant routes, `.3` admits that unchanged consumer exactly once per ABI, and `.4` proves every committed owner unchanged. Public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
