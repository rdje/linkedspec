...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity remains 80/0/0; semantic introspection remains closed at 128 mutations, rollout 9/9; typed source remains 9 complete / 5 pending / 114 mutations and recursive observation stays closed on all six runtimes. Behavior-free `.14.5.0` landed at `3d0384d1` and cleanly activated `INTER-MATCH-GAP-CAPTURE.1-.7` as sole named-slot/`@capture_gaps` owner. Neutral `.1` landed through `db299789` at 1 complete + 8 pending / 55 mutations. Perl `.2.1` now implements authored/static named slots, directive validation, and provenance behind a dormant consumer; live/runtime `.2.2`, emitted/loaded `.2.3`, and admission `.2.4` remain. `.14.5.1` promotes typed `gap_composition` only after gap admission `.7`. Recognition transaction `.14.3` stays closed at 133 = 129+4 / 246 / 58 and rollout 9/9 until the owned `.2.2` effect-census synchronization. Reversed-span and unrelated hygiene/readability and later backlog lanes remain separately owned. |
...
| Inter-match gap capture | `active; Perl authored/static .2.1 signoff-complete from 8f826923; atomic 223 ready` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Neutral `.1` remains 1/8 rollout and 55 mutations. Perl `.2.1` implements permanent `name=/regex/`, `Rule[name]`, and static `@capture_gaps` metadata with pinned Unicode 17 identity, exact diagnostics, five-field selector provenance, generated dependency provenance, and a 108-assertion dormant consumer plus 10 no-admission mutations. Canonical signoff passes all eight doctrines, containment/relocation, CLI 66x2, RAM 65%, Phase 0 1,031/1,031 in 755 seconds, and the exact opt-in route. It adds no live gap state/accessors, generated gap execution, recurring runtime route, or public admission. `.2.2` owns same-guard live state and four private source-read ActionIR nodes, `.2.3` emitted/loaded parity, and `.2.4` Perl-only admission. Recognition remains 133/246/58, language remains 122 public helpers, typed source remains 9/5/114, and all outward surfaces remain guarded. |
