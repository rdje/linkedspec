...
guarded `map_leaves!` passes 530 assertions per ABI plus the complete dual-ABI gate. Parent `.19.6` closes and
portable/public admission `.19.7` is next.
...
routes on both ABIs; nested-write passes 438 and bang passes 530 assertions per ABI. Portable/public admission
`.19.7` remains pending.
