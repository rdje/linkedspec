...
consumer is exact at 85-pass/one-RED on both ABIs without behavior or rollout movement. Private authority `.1`
passes 273/273 per ABI while remaining unexported and carrier-free; `.2` is next. Mandatory
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current. Progressive neutral + Perl + Rust + Dart + Julia is 5/9/106; Julia `.14.6.5.0-.4` is closed. Shared Lua `.14.6.6.0` freezes one dual-ABI 85-pass/one-RED consumer; `.1` supplies one private shared authority at 273/273 per ABI while preserving every carrier/rollout boundary. Lua carrier `.2` is next; admission `.3`, recomposition `.4`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
