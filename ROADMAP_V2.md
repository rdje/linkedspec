...
is task-tree active but decision-blocked: two frozen helper carriers assume implicit caller binding capture that
the admitted pure function-local contract forbids. The recommended correction uses existing explicit-target
`set` and caller-scoped `.with` carriers; no bang behavior or contract bytes have changed pending director choice.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Capability is 90/0/0; semantic introspection is closed at 9/0/128; typed source is 14/0/231. Progressive and staged language programs, standalone lifecycle blocks, task/index `.22`, and mdBook drift `.23` are closed. Write-vivification `.19.1.1`, `map_leaves!` receiver mutation `.19.1.2`, and strict composition `.19.1.3` remain frozen at 105 write, 167 map, and 593 composition mutations; Perl `.19.2.1` implements only nested writes. Active `.19.2.2` is decision-blocked because two frozen helper carriers assume implicit caller capture forbidden by the admitted pure function-local contract. Existing explicit-target `set` and caller-scoped `.with` can preserve the intended guard/commit proof; no bang behavior, neutral bytes, other backend, or public capability moves pending director choice. Codegen-inspector `.13.1` remains closed; bounded histories retain their separately authorized capacities. |
