...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity remains 80/0/0; semantic introspection remains closed at 128 mutations, rollout 9/9; typed source remains 9 complete / 5 pending / 114 mutations and recursive observation stays closed on all six runtimes. Behavior-free `.14.5.0` landed at `3d0384d1` and cleanly activated `INTER-MATCH-GAP-CAPTURE.1-.7` as sole named-slot/`@capture_gaps` owner. Neutral `.1` landed through `db299789`; Perl parent `.2` landed as atomic 226 at `eceb15ac`; Rust parent `.3` landed as atomic 232 at `2800e7c3`; Dart `.4.0-.4.5` now close authored metadata, private native execution, reconstructed/descriptor/generated carriers, independent emitted proof, primary parity, and exact nine-role private admission. Governance is 4 complete / 5 pending / 57 semantic mutations plus ten Rust and ten Dart admission mutations; ADR `0074` records why Dart adds mutation 57 instead of replacing a nonexistent premature row. Julia `.5` follows after the clean Dart landing; later runtimes and public rows remain pending. `.14.5.1` promotes typed `gap_composition` only after gap admission `.7`. Reversed-span and unrelated hygiene/readability and later backlog lanes remain separately owned. |
...
| Inter-match gap capture | `active; Dart parent .4 signoff-complete for intended atomic 239; Julia .5 follows after clean landing` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl, Rust, and Dart are privately admitted at governance 4/5/57 plus ten Rust and ten Dart admission mutations. Dart `.4.1-.4.5` supply authored/static/compiled identity, private same-authority native execution, normalized reconstruction, separate compatible descriptor projections, direct/traced generated execution, independent emitted proof, existing-primary parity, and one exact nine-role ledger. Its consumer runs ordinarily, canonically, and once after Rust in the rooted route. ADR `0074` preserves all prior mutations and adds the missing Dart regression as 57. Exact canonical CI closes parent `.4`; Julia `.5` follows only after atomic-239 commit/brief/clean proof. Both Lua ABIs, recurring/public rows, and every outward surface remain pending or unchanged. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, generated plan v2 and legacy descriptor/reference fields unchanged. |
