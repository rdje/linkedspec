...
`.10.7.1.0` now freezes one generated Lua-5.1-compatible 806-range classifier, exactly five parser
routes, one authoritative four-role validator, a shared diagnostic, and the identity/negative/isolation proof
split. Generated classifier and native-route implementation `.10.7.1.1` is next after the clean planning commit.
