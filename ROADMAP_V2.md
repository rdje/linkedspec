...
| Inter-match gap capture | `active; Dart parent .4 landed at 43ed1c8f; behavior-free Julia .5.0 active` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl, Rust, and Dart are privately admitted at governance 4/5/57 plus ten Rust and ten Dart admission mutations. Julia `.5.0` freezes `.5.1-.5.5`: authored/static provenance and dormant consumer; same-recognition-authority native state; normalized reconstruction/descriptors/generated execution; one repository-routed emitted host; and existing-primary/nine-role admission. The plan keeps `RecognitionFrameState` cursor/boundary/marks, reuses `SourceAuthority`, and retains format 2 `{label,family}` plans. No Julia behavior or rollout moves in `.5.0`. Both Lua ABIs, recurring/public rows, and every outward surface remain pending or unchanged. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, and legacy descriptor/reference fields unchanged. |
