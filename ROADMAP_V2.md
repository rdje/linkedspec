...
combined `.14.8` follow. Julia task splitting first waits for independent
`FUTURE-PARITY-PARTITION-CAPACITY.0-.2` to add and recompose one equally bounded `.14.6.5-.14.8` semantic
member without changing stable IDs, pressure ceilings, or backend behavior.
