...
routes. Descriptor global-mode v0 and generated v1 remain staged; private v1 plus outer CLI/corpus compatibility
stay isolated until `.4-.5`. Current proof is package 2,320 plus the frozen help mismatch, primary 32/65x2,
corpus 105, and neutral 67/4+4/39; cursor `.3-.6` then precedes root admission `.9.1.1.2.4.3`.
...
| Julia backend parity | `in progress` (normal cursor runtime complete; artifact/public admission pending) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes and cursor preflight/normalization are signed off. Runtime `.9.1.6.2` derives policy at every normal live/loaded/normalized/recursive/traced entry and passes 36 families, 8/8 parent-child, and 2/2 structural rows. Descriptor v0, generated v1, explicit outer compatibility, shared primary 32/65x2, and rollout 4/4 remain for `.3-.6` before root admission `.4.3`. |
