...
contracts are frozen without public admission. Perl `.19.2.1-.2` implements both unchanged mechanisms. Rust
`.19.3.1` now implements nested-write vivification through typed parsed/serialized/generated/emitted carriers;
Rust `map_leaves!` `.19.3.2` is next, while Dart/Julia/Lua remain non-vivifying/non-bang.
...
Rust `.19.3.1` now consumes the unchanged nested-write authority through one typed segment/node representation,
native/serialized/generated-plan/emitted/independently compiled execution, exact diagnostics, and fail-closed
carrier validation. Its permanent contract, 197-test integration target, and 105-fixture corpus pass. Dart,
Julia, and Lua remain non-vivifying; no capability/public admission occurs. Perl `map_leaves!` `.19.2.2`
...
mutations, and Phase 0 1,032/1,032. Rust `map_leaves!` `.19.3.2` follows.
