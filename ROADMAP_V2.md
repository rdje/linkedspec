...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170 with transaction safety, recursive observation, lossless-gap composition, and progressive recurrence current. Progressive private six-runtime admission, exact recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Julia `.14.6.5.0-.4`, shared Lua `.14.6.6.0-.4`, typed recurrence `.14.6.7`, and public closeout `.14.6.8` are closed. Lua `.0` preserves the historical dual-ABI 85-pass/one-RED boundary, `.1` supplies one private shared authority at 273/273 per ABI, `.2` makes the final path GREEN at 178/178 per ABI through four dormant routes, `.3` admits that unchanged consumer exactly once per ABI, and `.4` proves every committed owner unchanged. Staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
