...
- Still track these seams explicitly:
  ...
  - `perl/LinkedSpec/Validation.pm` still trails parts of the supported DSL surface and remains a Phase 2 hardening target, even though current rule-label syntax, rule-paragraph regex validation, earlier mixed-edge rejection, and malformed top-level edge-target indexing are now aligned there more closely.
