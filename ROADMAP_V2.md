...
Dart `.1-.3` are complete at 4/7. Julia preflight `.4.0` is cleanly committed and core `.4.1` signs off the
marker-optional resolver/failure/descriptor boundary; routes `.2` precede cursor-dependent final
admission `.3`. Lua validation still blocks no-marker fallback, while Julia composed routes remain pending, so
