...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection remains exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6; MCP is complete/141. Typed-source internal value/helper parent `.14.2` is composition-closed at 8 complete / 6 pending / 53 mutations across all six runtimes. Behavior-free `.14.3.0` is signoff-complete after auditing transaction safety before code: compatibility cursor stacks are cursor-only, recursive same-label marks share a rule-label bucket, termination guards do not emit the target progress records, and the complete ActionIR surface lacks a closed effect classification. The frozen v1 plan adds monotonic invocation frames, opaque owner/generation tokens, cursor/boundary/invocation-mark snapshots, staged recognition, fail-closed transitive effects, cursor-only progress, independent six-runtime admission, recurrence, and no-change closeout. No production/current behavior or schema/README/storage/workflow boundary moves. Reversed-span remains `.14.2`-owned; `.14.3.1.0` selects exact syntax/result exposure next; `.14.8` retains final public no-drift. Book 79/14,232 KiB, Knowledge 791/6,538, all seven doctrines, canonical CLI 66/66 twice, RAM 63%, and Phase 0 1,031/1,031 in 708 seconds pass. The nonurgent rendered-readability tree stays queued; `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
