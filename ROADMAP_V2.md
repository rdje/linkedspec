...
| Inter-match gap capture | `active; neutral parent .1 signoff-complete in intended atomic 221/300` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | `.1.0` landed the behavior-free plan at `31f3e664`, `.1.1` the executable artifact at `f58dfcb3`, and `.1.2` rooted recurrence/no-overclaim at `0490522b`. `.1.3` independently recomposes the unchanged format-1 model, 55 mutations, exact six pending skips, storage, public absence, duplicate-slot 59, and typed-source 9/5/114. Canonical CI passes CLI 66x2, RAM 61%, Phase 0 1,031/1,031 in 725 seconds, the exact opt-in route, and exit 0, closing neutral parent `.1`. Rollout remains 1 complete + 8 pending; named slots, `@capture_gaps`, accessors, parser/runtime behavior, and public claims remain absent. Perl `.2` is next after clean landing; `.3-.7` retain later runtime/public ownership. |
