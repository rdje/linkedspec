...
at `7aa9c578`; behavior-free Julia cursor preflight `.9.1.6.0` is verified and its clean commit precedes `.1-.6`
and root admission `.3`.
It measures 36 parsed headers but compact-`|` family drift, global seek, parent-child 5/8, structural 1/2,
descriptor global-mode v0, generated v1, package 56/57, primary 32/65x2, corpus 105, and neutral 67/4+4/39;
the frozen order isolates v1 until generated v2 owns the semantic bump.
...
| Julia backend parity | `in progress` (historical scoped milestone closed; global root/cursor obligations active) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes are signed off; current shared primary is exactly 32/65 twice with only 33 cursor-owned help/usage/trace failures. Dart admission is clean at `7aa9c578`; behavior-free cursor preflight records compact-`|` drift, global seek, 5/8 parent-child, 1/2 structural, descriptor v0, and generated v1, then freezes `.1-.6` before root admission `.4.3`. |
