...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2` and complete private Perl `.3`, Rust `.4`, and Dart `.5` parents are closed. Julia `.14.7.6.0-.3` now provide the dormant declaration, caller-frozen current-depth authority, and separate breadth-first recurrence with exact lineage, shared bounds, expiring safe points, and source projection at 386 GREEN before only `.4` fresh carriers, integration, admission, rollout, and parent closure remain RED. Ordinary Julia and canonical CI still omit the consumer; rollout, neutral 92-mutation governance, v1/v2 formats, and public/outward behavior remain unchanged. Julia carrier/admission `.14.7.6.4` is next. Mandatory segment `4990` remains finitely authorized at 23/22 by ADR `0091`; Lua `.7`, recurrence `.8`, public `.9`, recomposition `.10`, final `.14.8`, reversed-span, and unrelated hygiene/readability remain separately owned. |
...
| Staged linked parsing track | `in progress` (first prototype complete) | Language-neutral parser composition: stage-N specs parse anchored outer structure, emit source-provenance text islands, and route those payloads to one or more later `.spec` parsers through deterministic parse jobs. | `.1-.4` adopt the language-neutral architecture and `.5.1-.6` complete the narrow function-body prototype. The executable neutral program and private Perl/Rust/Dart general implementations are admitted. Julia `.14.7.6.0-.3` now provide its dormant exclusive declaration, exact native-offset-backed typed provenance, caller-frozen one-depth engine, and separate breadth-first recurrence with strict lineage, shared resources, safe points, and source rebasing. The consumer is 386 GREEN/one `.4` carrier/admission RED and remains outside ordinary/canonical discovery; Julia rollout and the 92-mutation oracle do not move. Fresh carrier integration and admission `.14.7.6.4` is next; recurring proof and public `parse_job(...)` authoring remain ordered future work. |
