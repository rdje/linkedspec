...
Progressive `.14.6.0-.1`, Perl `.2`, Rust `.3`, Dart `.4`, Julia `.5.0-.4`, and Lua admission `.6.0-.3` are
complete at six private runtime rows / 7/9/112. Private pre-registered authority, globally rebased same-source views, narrowing ceilings, shared
cancellation/budgets, bounded decreasing chains, detached results, dedicated nodes, four carriers, and exact
admissions are current on those four backends without outward or typed-row movement. Dart `.4.0-.2` preserve its
historical RED, authority, and four carriers; `.4.3` admits the unchanged seven-group consumer ordinarily and
canonically. Lua recomposition `.6.4`, recurrence `.7`, public `.8`, staged `.14.7`, and
combined `.14.8` follow. Capacity `.0` freezes, canonical `.1` implements, and focused `.2` independently
...
273-assertion-per-ABI authority dormant. Independent recomposition `.4` is next. Mandatory
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current. Progressive private runtime admission is 7/9/112 across Perl, Rust, Dart, Julia, PUC Lua, and LuaJIT; Julia `.14.6.5.0-.4` is closed. Shared Lua `.14.6.6.0` preserves the historical dual-ABI 85-pass/one-RED boundary, `.1` supplies one private shared authority at 273/273 per ABI, `.2` makes the final path GREEN at 178/178 per ABI through four dormant routes, and `.3` admits that unchanged consumer exactly once per ABI in ordinary and canonical proof. Recomposition `.4`, recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
