...
Standalone lifecycle-block audit `.15.0` ratifies a rule-item-leading `{ ... }` as exact future syntax sugar for
`I { ... }` at the same authored position. It is not current executable syntax: Perl rejects it, while the four
native parsers' `PlainBlock` carriers are dormant/inert. Perl/Rust implementation and Rust duplicate-`I` order
repair belong only to `.15.1`; Dart/Julia/Lua, self-hosted grammar, public teaching, and final no-drift belong to
`.15.2`. Attached edge blocks and function/callable/nested braces keep their established owners.
