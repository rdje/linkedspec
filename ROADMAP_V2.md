...
Director-requested [backend integration guides](docs/tasks/BACKEND-INTEGRATION-GUIDES.md) are active after clean conformance .1.34: inventory .0 and Rust setup/file/deployment .2.1-.2.2 are complete; the director-requested Rust push is complete and Perl .1.1-.1.2 are verified. Dart .3.1-.3.2, Julia .4.1-.4.2 and Lua setup .5.1 are verified; ADR0121 admits the six approved history controls. Intake .8.1 records seven ARCHOGEN and three SEMULITH reports; Rust workspace onboarding .8.2 is repaired and verified with unchanged dependency pins; public integration correction .8.3 is committed at a1166ee1d; prerequisite navigation and qualified checkout guidance .8.4 are verified. Lua deployment .5.2 is verified in working/clean PUC and LuaJIT consumers; .5.3 diagnoses captured wrapper stderr and hands evidence to existing startup .7 without claiming a fix. Common .6 is committed at fbb135d63 after canonical CI, CLI66x2 and Phase0 1032/1032. Boundary .8.5 removes private workspace probes/current premises with nine own-manifest controls and public RGX/native proof; Lua fixture isolation .8.6 precedes independent final .7. Final independent .7 retains parent closeout and canonical push. Common checkout/version/setup guidance and five native consumer guides will verify dependency preparation and reuse, API/results/errors, grammar packaging and outside-cwd use. The director authorizes this documentation activity before the remaining startup reading; complete it and resume conformance .1.35. ADR0040 keeps one canonical content owner and links the eventual companions. The native Rust example passes two four-value runs; normal Cargo dependency builds are authorized. Clean committed-source Rust preparation and actual file/deployment checks pass; the remaining backend guides retain their own acceptance. Strict Lispish document/token parsing remains pending under startup .83.1-.83.3.

The director requests an early push after the complete Rust/Lispish guide is verified under .2.2,
including clean pinned-source preparation and native file parsing. Notify the pushed commit,
then continue the remaining backend guides and final .7 before returning to reading.
