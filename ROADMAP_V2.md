...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2` and complete private Perl `.3`, Rust `.4`, Dart `.5`, and Julia `.6` parents are closed. Shared Lua `.14.7.7.0-.1` now freeze one stable Lua-5.1-compatible consumer and implement its exclusive private marker/provenance carrier: PUC Lua and LuaJIT each pass 392 assertions and fail only the same `.14.7.7.2` caller-frozen authority RED. Neutral governance rejects 98 mutations; ordinary/canonical discovery, both rollout rows, generated v2, function-body v1, and public/outward behavior remain unchanged or pending. Mandatory change-history segment `4990` remains finitely authorized at 23/22 by ADR `0091`; mandatory engineering-notes segment `4989` is finitely authorized at 19/18 by ADR `0092`. Lua resolution/cache/policies `.7.2` is next, while recurrence `.8`, public `.9`, recomposition `.10`, final `.14.8`, reversed-span, and unrelated hygiene/readability remain separately owned. |
