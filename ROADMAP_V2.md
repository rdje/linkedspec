...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2` and complete private Perl `.3`, Rust `.4`, Dart `.5`, and Julia `.6` parents are closed. Julia `.14.7.6.0-.4` provide the declaration, caller-frozen current-depth authority, breadth-first recurrence, and fresh native/reconstructed/generated-plan/emitted production authority. Its 491/491 consumer is admitted once in ordinary/canonical discovery, Julia rollout is complete at 5/9, and neutral governance rejects 97 mutations. Function-body v1, generated v2, and public/outward behavior remain unchanged. Mandatory change-history segment `4990` remains finitely authorized at 23/22 by ADR `0091`; mandatory engineering-notes segment `4989` is finitely authorized at 19/18 by ADR `0092`. Lua `.7` is next, while recurrence `.8`, public `.9`, recomposition `.10`, final `.14.8`, reversed-span, and unrelated hygiene/readability remain separately owned. |
...
| Staged linked parsing track | `in progress` (first prototype complete) | Language-neutral parser composition: stage-N specs parse anchored outer structure, emit source-provenance text islands, and route those payloads to one or more later `.spec` parsers through deterministic parse jobs. | `.1-.4` adopt the language-neutral architecture and `.5.1-.6` complete the narrow function-body prototype. The executable neutral program and private Perl/Rust/Dart/Julia general implementations are admitted. Julia `.14.7.6.0-.4` provide its exclusive declaration, exact native-offset-backed typed provenance, caller-frozen one-depth engine, breadth-first recurrence, and one fresh host-only seed across native/reconstructed/generated-plan/emitted production routes. The consumer is 491/491 in ordinary/canonical discovery; Julia rollout is complete and the oracle rejects 97 mutations. Shared Lua `.14.7.7` is next; recurring proof and public `parse_job(...)` authoring remain ordered future work. |
