...
the marker-optional resolver/failure/descriptor boundary plus composed route reuse before cursor-dependent final
admission `.3`. Lua validation still blocks no-marker fallback, while Julia topology
...
inventory is 66 files with 34 effective mutations; only composed admission remains `.6`.
...
| Dart backend parity | `done` (foundation; later cursor admission active) | First future full-parity backend lane after Perl5 and Rust. | The original parity tree closed at 190 tests, 61/61 default/POSIX, and 105/105 corpus. Rule-local cursor `.9.1.5.1-.5` now implements normalization, live/loaded/reconstructed execution, descriptor v1, generated-source v2, and public/CLI override removal. Current proof is 260/260 package, 63/63 primary default/POSIX, and 105/105 corpus; only composed admission `.9.1.5.6` remains. |
| Julia backend parity | `in progress` (historical scoped milestone closed; global root/cursor obligations active) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes are signed off; current shared primary is exactly 32/65 twice with only 33 cursor-owned help/usage/trace failures. Cursor `.9.1.6` and root admission `.4.3` remain explicit Julia parity work. |
