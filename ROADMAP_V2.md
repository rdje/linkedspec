...
package 3,133 plus the frozen help mismatch, primary 32/65x2, corpus 105, and neutral 67/4+4/39; cursor `.5-.6`
then precedes root admission `.9.1.1.2.4.3`.
...
| Julia backend parity | `in progress` (cursor through generated v2 complete; public admission pending) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes plus cursor normalization/runtime/descriptor/generated `.9.1.6.1-.4` are signed off. Runtime derives policy at every entered rule; descriptor v1 projects exact normalized facts; generated v2 derives policy from minimal label/family rows, rejects v1 before reconstruction, and has no serialized cursor override. Explicit outer compatibility, shared primary 32/65x2, and rollout 4/4 remain for `.5-.6` before root admission `.4.3`. |
