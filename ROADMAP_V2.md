...
| Inter-match gap capture | `active; Dart authored metadata .4.1 signoff-complete for intended atomic 234` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl and Rust are privately admitted at governance 3/6/56 plus ten Rust admission mutations. Dart `.4.1` adds named/anonymous authored order, selector provenance, logical source identity, static diagnostics, directive eligibility, and compiled metadata while its final consumer remains dormant; ten Dart dormancy mutations lock no admission. Tiered verification is complete and native `.4.2` is next under focused proof; reconstructed/descriptor/generated `.4.3`, emitted `.4.4`, and primary/admission `.4.5` remain pending. Four later runtime routes and both public rows remain pending. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, generated plan v2 unchanged, and outward surfaces guarded. |
