...
path/host denial. Completed negative/isolation audit `.10.7.1.3.0` freezes one pre-existing body-fluent suffix-loss defect:
`.Töp()`/`.A·B()` validate as ASCII-prefix method `T`/`A` because the adapter discards the returned remainder.
Active narrow whole-token repair `.3.1` precedes exhaustive dual-ABI proof `.3.2` and may not widen fluent identifiers.
