...
reject 89 mutations; static rule facts cross-check `linkedspec-rule-local-cursor-v1` after correction `.10.3.2.0`
...
rollout is 5 complete / 4 pending, backend admission is 4 complete / 2 pending, and `.10.3-.10.10` split all
...
logical costs, and portable typed errors at query 283/focused 1,204 per ABI. Its signoff passes primary 5x2x66,
Unicode 10/10, all six generated ledgers, the complete elevated local gate (Rust admission 1/1 in 78.18 s, Dart
1/1, Julia 416/416 in 27.3 s, containment, moved-root, reference 66x2, and Phase 0 1,031/1,031 in 622 s), the mdBook
build, and Knowledge Map 728 facts / 5,814 keys. Public/raw `.10.7.5.3` is next.
...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Lua `.8.4` closes full current backend parity at capability 80/0/0; diagnostic-output `.5.1`, logical-helper `.5.2`, and rule-local cursor/root/duplicate-slot/repeated-action governance are closed. ADRs `0049`/`0050` and neutral `.10.2` freeze six semantic fixture groups and 20 exact responses. Perl `.10.3`, Rust `.10.4`, Dart `.10.5`, and Julia `.10.6` are admitted by exact 12-role consumers; governance is 89 mutations, rollout 5/9, and native admission 4/6. Lua Unicode `.10.7.1`, opaque source/outcome `.10.7.2`, private static projection `.10.7.3`, and calls/staging/generated `.10.7.4` are composition-closed without ledger movement; exact private 22/25/10 recomposes at focused 920 per ABI. Query audit `.10.7.5.0` freezes detached authority, vocabulary, 19 hashes, 26 raw boundaries, and dual-ABI representation policy; private `.10.7.5.1` matches nine non-traversal hashes and private `.10.7.5.2` completes all 19 static hashes with pages/traversal/budgets/costs at focused 1,204 per ABI without a public query name. Raw/public `.10.7.5.3` is next. `.22` tracks the recurring mutable task-index marker-anchor defect; `.23` owns full-book semantic drift repair and stronger migration-example guards. Format-program and inter-match gap activation remain separate; mutation execution stays manual; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
