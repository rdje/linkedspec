...
reject 53 mutations; static rule facts cross-check `linkedspec-rule-local-cursor-v1` after correction `.10.3.2.0`
repaired stale default-family and no-edge ownership rows. Staged payload/job/result records are explicitly separate
from generated artifacts. Neutral
rollout is 1 complete / 8 pending, backend admission is 0 complete / 6 pending, and `.10.3-.10.10` split all
...
capture. `.10.3.1` now implements opaque strict source/map/compiled-or-failed construction without path reads,
execution, records, queries, or admission. Corrected static projection `.10.3.2.1` follows; `.10.3.3-.10.3.6`
retain the remaining Perl work.
...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Lua `.8.4` closes full current backend parity at capability 80/0/0; diagnostic-output `.5.1` and logical-helper `.5.2` are closed at 8/0. ADR `0043` is current across all native/generated roles and public no-drift. Parenthesis-free conditions remain excluded; other `.5` helper decisions stay owned. Rule-local cursor rollout is closed at 75 files / 8 complete + 0 pending / 60 mutations with exact selected 5x2x5 recurring and public proof. Root selection is closed at 7/0/54 and duplicate slots at 7/0/59. Repeated-action rollout is closed at 8 complete / 0 pending through 54 mutations; `tools/check_repeated_action_result_five_backend.sh` composes every runtime/ABI, one selected 5x2 primary case, and three support ledgers. Semantic parents `.9.1.1`, `.9.1.10`, `.9.1`, and `.9` are closed. ADRs `0049`/`0050` and neutral `.10.2` freeze the semantic model/query through six fixture groups and 20 exact responses; correction `.10.3.2.0` repairs default-family/no-edge facts and adds cross-contract proof at 53 mutations. Perl `.10.3.0` maps exact authorities, `.10.3.1` implements opaque strict source/map/outcome construction, and `.10.3.2.1` is the next static projection owner. Rollout remains 1 complete / 8 pending and backend admission 0 complete / 6 pending. Format-program and inter-match gap activation remain separate; mutation execution stays manual; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
