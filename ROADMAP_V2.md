...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170 with transaction safety, recursive observation, lossless-gap composition, and progressive recurrence current. Progressive private six-runtime admission, exact recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit `.14.7.0`, diagnostic repair `.1`, neutral authority `.2`, complete Perl `.14.7.3.0-.4`, Rust `.14.7.4.0-.4`, and Dart `.14.7.5.0-.3` are closed. The Perl oracle is 143/143 across native, normalized-descriptor, generated-plan, and independently loaded emitted fresh-authority routes with exact phase-0/canonical admission. Rust's complete private authority and fresh four-route carriers are admitted. Dart now has one inert live-proven marker plus a separate private caller-frozen recursive engine: pure selection, default-top-before-id, plan-only caching, typed order/fresh sibling state, detached atomic policies, breadth-first next-depth queues, strict cycle/decrease lineage, shared cancellation/resources, expiring safe points, and direct/ordered-derived source rebasing pass. Complete-depth target reservation also rejects cross-plan conflicts before callbacks. The same dormant consumer is `+18 -1`; only `.14.7.5.4` fresh production carriers, ordinary/canonical admission, and Dart rollout remain RED. Neutral governance remains 85 mutations and promotes only Perl plus Rust; function-body v1/generated v2 and public/outward behavior remain unchanged. Mandatory segment `4990` is finitely authorized at 23/22 by ADR `0091`. Final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
