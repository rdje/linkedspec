...
| Inter-match gap capture | `active; Julia private native .5.2 signoff-complete for intended atomic 242` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl, Rust, and Dart are privately admitted at governance 4/5/57. Julia `.5.1` carries exact authored/static/compiled provenance, and `.5.2` now executes private same-recognition-authority gap state, lifecycle, entry identity, rollback/nesting, scalar spans, and four zero-argument accessors behind ten dormancy mutations. Those private helpers remain outside the exact 246-name supported ActionIR inventory. Absolute loaded requests retain path opacity through logical basenames; legacy descriptors/references and generated format 2 `{label,family}` remain unchanged. Reconstructed/descriptor/generated `.5.3`, emitted `.5.4`, and primary/admission `.5.5` remain pending, as do Lua and public rows. Recognition stays 137/246/58, public helpers 122, and typed source 9/5/114. |
