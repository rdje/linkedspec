...
Independent Lua recomposition `.5`, recurrence `.8`, public authoring `.9`, and final recomposition `.10` follow.
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2` and complete private Perl `.3`, Rust `.4`, Dart `.5`, and Julia `.6` parents are closed. Shared Lua `.14.7.7.0-.4` now implement and admit one stable Lua-5.1-compatible consumer at 888/888 per ABI through exact once-per-ABI ordinary/canonical topology. Neutral governance rejects 106 mutations with all private backend lifecycle and rollout rows complete; generated v2, function-body v1, and public/outward behavior remain unchanged. Mandatory change-history segment `4989` is finitely authorized at 24/23 by ADR `0093`; mandatory engineering-notes segment `4989` is separately authorized at 19/18 by ADR `0092`. Independent Lua recomposition `.7.5`, staged recurrence `.8`, public `.9`, recomposition `.10`, final `.14.8`, reversed-span, and unrelated hygiene/readability remain separately owned. |
