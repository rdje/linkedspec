...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6. MCP is complete/141; parents `.10`, `.11`, `.14.1`, and `.24` are closed. Typed-source rollout remains 4/10 with 38 mutations. Rust projection `.14.2.2.2` is signoff-complete from clean `2d9401e2`: one per-input immutable authority now backs all 92 canonical helpers and seven aliases while UTF-8-byte registers, public results, schemas, and native/reconstructed/generated-plan identities remain unchanged. Nested projection 4/4, core 2/2, ordinary zero-test, dedicated alias carrier 1/1, complete Rust/corpus/generated/storage/CLI 66x2, sole-facing book/HTML, and all doctrines pass. Canonical CI passes containment/relocation, CLI 66/66 twice, RAM 51%, and Phase 0 1,031/1,031 in 654 seconds; only commit/clean remains. Exact consumer registration plus `rust_runtime` promotion belongs only to `.14.2.2.3`. The nonurgent rendered-readability tree stays queued. `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
