...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 12/2/170. Progressive private six-runtime admission, recurrence, and public no-drift are closed at 9/9/116 plus public 6/12/10/60 without an outward API. Staged audit/diagnostic/neutral `.14.7.0-.2` and complete private Perl `.3`, Rust `.4`, and Dart `.5` parents are closed. Julia `.14.7.6.0-.1` now freeze and implement the private declaration boundary: one exclusive marker, native-capture-proven Unicode-scalar direct/ordered-derived provenance, four equal detached logical routes, and 131 GREEN assertions before only `.2` authority remains RED. Ordinary Julia and canonical CI still omit the consumer; rollout, neutral 92-mutation governance, v1/v2 formats, and public/outward behavior remain unchanged. Julia authority/policies `.14.7.6.2` is next. Mandatory segment `4990` remains finitely authorized at 23/22 by ADR `0091`; Lua `.7`, recurrence `.8`, public `.9`, recomposition `.10`, final `.14.8`, reversed-span, and unrelated hygiene/readability remain separately owned. |
...
| Staged linked parsing track | `in progress` (first prototype complete) | Language-neutral parser composition: stage-N specs parse anchored outer structure, emit source-provenance text islands, and route those payloads to one or more later `.spec` parsers through deterministic parse jobs. | `.1-.4` adopt the language-neutral architecture and `.5.1-.6` complete the narrow function-body prototype. The executable neutral program and private Perl/Rust/Dart general implementations are admitted. Julia `.14.7.6.0-.1` now provide its dormant exclusive declaration node and exact native-offset-backed typed provenance across native, reconstructed, generated-plan, and independently included emitted routes. The consumer is 131 GREEN/one `.2` authority RED and remains outside ordinary/canonical discovery; Julia rollout and the 92-mutation oracle do not move. Julia resolution/cache/result/failure `.14.7.6.2` is next; recurring proof and public `parse_job(...)` authoring remain ordered future work. |
