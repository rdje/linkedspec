...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity remains 80/0/0; semantic introspection remains closed at 128 mutations, rollout 9/9; typed source remains 9 complete / 5 pending / 114 mutations and recursive observation stays closed on all six runtimes. `INTER-MATCH-GAP-CAPTURE.1-.6` is complete through Lua admission `.6.5`, landed as atomic 251 at `c33f6664`: neutral plus all six private runtimes execute at 7/2/60, with ten Rust, ten Dart, ten Julia, and sixteen Lua admission mutations. Behavior-free `.7.0` is canonical-signoff-complete and `.7.1` recurring is next before `.14.5.1` promotes typed `gap_composition`. Lua storage remains 19 owners / three dual-ABI native modules. Reversed-span and unrelated hygiene/readability and later backlog lanes remain separately owned. |
...
| Inter-match gap capture | `active; .7.0 canonical-signoff-complete from clean atomic 251 c33f6664; .7.1 next` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | All six private runtimes pass at governance 7/2/60. `.7.0` audits that committed boundary, corrects already-stale current route prose, and freezes `.7.1` recurring admission at 8/1/61; `.7.2` public language/compatibility/no-drift at shared calls 250, public Perl 126, gap 9/0/63 and public 6/12/10/29; `.7.3` independent no-change closeout and typed-source handoff. Focused and exact staged canonical proof pass; format 2, storage 19/three native modules, facades/schemas/MCP/CLI/README/capability, and typed source 9/5/114 remain unchanged in `.7.0`. |
