...
policy seams, and `.1-.4` implementation/admission order. `.10.9.2.1` now implements the generated binding,
private schema runtime, secure registry, and decoded dispatch; strict stdio/lifecycle `.10.9.2.2` is next. Six fixture groups now derive 20 digest-locked responses and
...
dependency order; in-process registry/decoded dispatch `.10.9.2.1` is complete with canonical signoff, with
strict stdio/lifecycle `.10.9.2.2` next.
