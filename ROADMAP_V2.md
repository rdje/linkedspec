...
routes. Generated v1 remains staged; its private compatibility path plus outer CLI/corpus compatibility stay
isolated until `.4-.5`. Descriptor `.3` now publishes cursor v1 from normalized compiled state with exact
direct/loaded/normalized bytes. Current proof is package 3,129 plus the frozen help mismatch, primary 32/65x2,
corpus 105, and neutral 67/4+4/39; cursor `.4-.6` then precedes root admission `.9.1.1.2.4.3`.
...
| Julia backend parity | `in progress` (normal cursor runtime/descriptor complete; generated/public admission pending) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes plus cursor normalization/runtime/descriptor `.9.1.6.1-.3` are signed off. Runtime derives policy at every normal live/loaded/normalized/recursive/traced entry; descriptor v1 projects exact family/policy/ownership/edges with direct/loaded/normalized byte identity. Generated v1, explicit outer compatibility, shared primary 32/65x2, and rollout 4/4 remain for `.4-.6` before root admission `.4.3`. |
