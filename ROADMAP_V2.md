...
`FUTURE-PARITY-BACKLOG.9.1.1.2` owns the rollout. Executable neutral contract `.0` plus composed Perl, Rust, and
Dart `.1-.3` are complete at 4/7. Julia preflight `.4.0` is cleanly committed and core/routes `.4.1-.2` sign off
the marker-optional resolver/failure/descriptor boundary plus composed route reuse. Dart cursor admission is clean
at `7aa9c578`; Julia cursor preflight `.9.1.6.0` and normalization `.1` are clean through `ac7010ee`. Intrinsic
runtime `.2` derives all 36 entered-rule
policies and passes all eight parent-child plus both structural rows across live/loaded/normalized/recursive/trace
routes. Descriptor `.3` publishes cursor v1 from normalized compiled state with exact direct/loaded/normalized
bytes. Generated-source `.4` now emits v2/format 2 from minimal family rows, derives the exact five seek/five
consume mapping, rejects v1 before payload reconstruction, and removes private v1 execution. Option-removal `.5`
deletes the caller-global
engine/loader/corpus/primary seam, supplies targeted legacy diagnostics, removes the help/request-trace field, and
preserves entry selection. Admission `.6` composes one exact 15-role consumer, advances only Julia, and closes the
cursor parent at package 3,291, primary 65/65x2, corpus 105, and neutral 67/5+3/44. Root admission
`.9.1.1.2.4.3` is next.
Lua validation still blocks no-marker fallback, while Julia topology
admission waits for cursor migration, so
Lua/LuaJIT `.5` and composed public no-drift `.6` remain open and the fallback is not yet uniform.
...
- Typical `.spec` authoring keeps regexes small and readable: zero-regex coordination,
  ...
  returned-AST staged enrichment. `FUTURE-PARITY-BACKLOG.14.1-.14.4` owns complete
  documentation and proof beyond the current narrow function-body prototype.
...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Lua `.8.4` closes full current backend parity at capability 80/0/0; diagnostic-output `.5.1` and logical-helper `.5.2` are closed at 8/0. ADR `0043` is current across all native/generated roles and public no-drift. Parenthesis-free conditions remain excluded; other `.5` helper decisions stay owned. Neutral cursor `.9.1.2`, Perl `.9.1.3`, Rust `.9.1.4`, Dart `.9.1.5`, and Julia `.9.1.6` are complete; rollout is 5/3 at 67 governed migration files with 44 mutations. Root selection has neutral plus Perl/Rust/Dart/Julia at 5/7 with 39 mutations; Lua `.9.1.1.2.5` is next after Julia's clean admission commit. Identical dependency-regex identity is queued at `.9.1.8.1`. Format-program activation remains separate; mutation execution stays manual; inspector `.13.1` and authoring `.14`/`.15` remain queued. |
| Inter-match gap capture | `direction ratified; implementation dependency-gated` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot and its lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or regex/edge adjacency. | ADR `0045` and `INTER-MATCH-GAP-CAPTURE.0` align history, guides, and book without behavior change, including the actual legacy matrix: Perl anonymous scope is rule-level, Lua is preceding-slot-local, and Rust/Dart/Julia do not execute marker members natively. Accepted future `@capture_gaps` and spacing-insensitive `name=/regex/` → `Rule[name]` named slots are unimplemented; exact boundary/failure, lifecycle-slot access, typed spans, diagnostics, migration, and five-backend rollout remain `.1-.7` behind full cursor-program completion and explicit activation. |
...
| Julia backend parity | `in progress` (rule-local cursor and root-selection mechanisms admitted) | Second future full-parity backend lane after Dart under ADR `0021`; native in-memory Julia library first under ADR `0022`. | Standalone corpus is 105/105. Root core/routes plus cursor normalization/runtime/descriptor/generated/public removal and separate exact 15-role cursor/root admissions are signed off. Root selection fixtures use entry lifecycle `I`; runtime derives cursor policy at every entered rule; descriptor v1 and generated v2 carry no global override; legacy APIs/CLI fail exactly. Package 3,428 and shared primary 65/65x2 pass. Cursor rollout is 5/3 at 67 files and 44 mutations; root rollout is 5/7 with 39 mutations. Canonical Phase 0 passes 1,031/1,031 in 616 seconds; the clean Julia root commit precedes Lua root migration. |
