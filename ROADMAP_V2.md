...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity is 80/0/0; semantic introspection is 6 groups / 20 responses / 128 mutations, rollout 9/9, with native admission 6/6; MCP is complete/141. Typed source is 9 complete / 5 pending and rejects 114 mutations. Recursive observation remains closed across all six runtimes. Behavior-free `.14.5.0` is signoff-complete after auditing current numeric slot identity, named-surface absence, historical Perl gaps, marker divergence, and both recurring gates, and freezes one clean handoff: `INTER-MATCH-GAP-CAPTURE.1-.7` owns all named-slot/`@capture_gaps` syntax, lifecycle, compatibility, runtime, carrier, and public admission; only after `.7` does `.14.5.1` promote typed `gap_composition`. Brackets stay the selector namespace and dot stays fluent rule behavior. No parser, compiler, runtime, public surface, rollout, or current claim moves in `.14.5.0`; canonical proof passes containment/relocation, CLI 66x2, and Phase 0 1,031/1,031. Recognition transaction `.14.3` stays closed at 133 = 129+4 / 246 / 58 and rollout 9/9. Reversed-span remains `.14.2`-owned. Unrelated hygiene/readability and later backlog lanes remain separately owned. |
...
| Inter-match gap capture | `direction ratified; clean pivot selected after FUTURE-PARITY-BACKLOG.14.5.0` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot and its lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or regex/edge adjacency. | ADR `0045` and decision leaf `.0` preserve the history and exact marker-divergence boundary. All prerequisites are satisfied. Behavior-free `FUTURE-PARITY-BACKLOG.14.5.0` selects `.1-.7` as the next clean tree and sole owner of named-slot syntax, `@capture_gaps` lifecycle/compatibility, prefix/interstitial/tail and empty spans, failure/commit/recursion, six-runtime implementation, carriers, and public admission. Numeric `Rule[N]` remains positional compatibility; named `Rule[name]` is stable identity; dot is not a selector alias. Typed-source composition follows only after `.7`. |
