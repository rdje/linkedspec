...
limits; approved capacity implementation 4489f5e9 and independent proof a7d392a8 support `.7.4` admission. Canonical admission a67a18bf precedes completed [DART-STARTUP-READING.0](docs/tasks/DART-STARTUP-READING.md) decomposition: 55 initially pending children / 169 exact ranges, now physically read. Dart `.1.55` completes 55/55 children, 80,297 fragments / 2,471,305 bytes. Dart .3.1 independently verifies all 55 reading commits and 115 baseline-identical files: 80,297 fragments / 2,471,305 bytes. All 100 touched Knowledge cards and 62 prior repair nodes are preserved. Separate diagnostics pass 461 tests, storage25/47, CLI66 twice and corpus105; the complete Dart gate fails formatting in six tests and strict analysis reports two SDK deprecations. New .2.24/.2.25 own repairs. Formal .3.2 and startup .3.4 await a separate verification decision. Capacities remain canonical at 7d52c98e / 9c644ecb / bef5dafd under containment .8/.9/.10 and ADR0110/0111/0112; prior history and other limits remain. CI intake .80.0 is committed at eaf4331e. Director-approved .7 is implemented by containment .11 / ADR0113: exactly engineering files27→28 and manifest lines26→27; actual211-line/24521-byte source preservation and 22-validator checks pass, with the director’s explicit September 11 one-time focused/receipt exception. Capacity is committed at ad64f76f; next Dart .3.2 verification decision after the committed .3.1 audit. Switch .2.1, regex .2.2, numeric trace .2.3 and recognition effect integration .2.4 repairs remain owned and gated; startup .54.3 retains broad regex recurrence. Confirmed attached-switch omission/default replacement is owned by Dart .2.1, with repair gated by startup reading/policy prerequisites. Startup `.3.4` remains pending until reading closeout. Repairs `.7`–`.30`, `.32`–`.47`, and
...
CI evidence intake `SESSION-STARTUP-READING.80.0` records eleven visible Rust test-build stages / 78m02s, six absent optional PGEN watches, and a separate newer-OS pre-main observation. Pending `.80.1-.4` implement the director-required PGEN/RGX build-on-submodule-update lifecycle, with no dependency compilation for repeated CI or LinkedSpec-only edits and `.81.1-.2` own current-OS causal diagnosis/remediation; all implementation gates remain. Canonical capacity commit `bef5dafd` passes nine doctrines, required consumers, CLI66x2 and Phase0 1032/1032 (1163s for Phase0 only; 25 optional skips). Containment `.11` closes under that explicit one-time focused exception at ad64f76f; next Dart `.3.2` verification decision after the .3.1 audit; the original CI intake is committed at eaf4331e and the build-on-update directive is now recorded without claiming implementation.
