...
| Dart backend parity | `done` (foundation; later cursor evolution active) | First future full-parity backend lane after Perl5 and Rust. | The original parity tree closed at 190 tests, 61/61 default/POSIX, and 105/105 corpus. The later rule-local cursor rollout is deliberately staged: current package proof is 257/1 only at reference-migrated shared help, primary is 30/63 twice until `.9.1.5.5`, corpus remains 105/105, and normalization/execution/descriptor `.1-.3` are implemented. |
