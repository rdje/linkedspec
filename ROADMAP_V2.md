...
| Inter-match gap capture | `active; Julia carrier .5.3 signoff-complete for intended atomic 243` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl, Rust, and Dart are privately admitted at governance 4/5/57. Julia `.5.1-.5.3` now carry exact authored/static/compiled provenance, private same-recognition-authority native state/lifecycle, normalized reconstruction, fresh detached `regex_slots`/`capture_gaps`/five-field selector descriptors, and direct/traced generated-v2 execution behind ten dormancy mutations. Loaded and inline descriptors retain truthful basename/`inline` logical identities; legacy `resolved_edges`, `{label,idx}` refs, exact 246 supported ActionIR names, and format 2 `{label,family}` plans remain unchanged. Emitted `.5.4` and primary/admission `.5.5` remain pending, as do Lua and public rows. Recognition stays 137/246/58, public helpers 122, and typed source 9/5/114. |
