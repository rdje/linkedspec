...
the marker-optional resolver/failure/descriptor boundary plus composed route reuse. Dart cursor admission
`.9.1.5.6` is signed off; its clean commit precedes Julia cursor `.9.1.6` and root admission `.3`. Lua validation still blocks no-marker fallback, while Julia topology
