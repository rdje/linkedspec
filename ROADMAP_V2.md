...
| Inter-match gap capture | `active; Julia emitted .5.4 signoff-complete for intended atomic 244` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl, Rust, and Dart are privately admitted at governance 4/5/57. Julia `.5.1-.5.4` now carry exact authored/static/compiled provenance, private same-recognition-authority native state/lifecycle, normalized reconstruction, compatible descriptors, same-engine generated-v2 execution, and independently loaded emitted direct/traced proof behind ten dormancy mutations. Ten value/two typed-error modules pass in one repository-routed offline host; storage is exactly 20/5 with cleanup. Loaded/inline identity, legacy descriptors/refs, exact 246 supported ActionIR names, and format 2 `{label,family}` plans remain unchanged. Primary/admission `.5.5`, Lua, and public rows remain pending. Recognition stays 137/246/58, public helpers 122, and typed source 9/5/114. |
