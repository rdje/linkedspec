...
reject 55 mutations; static rule facts cross-check `linkedspec-rule-local-cursor-v1` after correction `.10.3.2.0`
repaired stale default-family and no-edge ownership rows. Generated-plan correction `.10.3.3.0` uses the same
contract's v2 authority to replace stale/illegal calls family `and_acode` with exact `default`. Staged payload/job/result records are explicitly separate
