...
| Inter-match gap capture | `active; Rust private native .3.2 signoff-complete for intended atomic 229` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl parent `.2` landed at `eceb15ac`; the full private consumer runs ordinarily and in the rooted route at governance 2/7/56. Rust `.3.1` landed authored/static/compiled metadata at `95127e1d`; `.3.2` now implements invocation-local native candidate/commit/tail state, Unicode accessors, detached slots, rollback, and nested isolation on the existing recognition authority. Its final consumer remains ignored and absent from canonical/recurring/facade execution until `.3.5`; `.3.3-.3.4` still own generated/reconstructed and emitted behavior. Five later runtime routes and both public rows remain pending. Recognition remains 137/246/58, language 122 public helpers, typed source 9/5/114, generated plan v2 unchanged, and all outward surfaces guarded. |
