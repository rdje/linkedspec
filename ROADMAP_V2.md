...
consumer is exact at 85-pass/one-RED on both ABIs without behavior or rollout movement; `.1` is next. Mandatory
...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability is 80/0/0; semantic introspection is closed at 9/0/128; typed source is 11/3/152 with transaction safety, recursive observation, and lossless-gap composition current. Progressive neutral + Perl + Rust + Dart + Julia is 5/9/106; Julia `.14.6.5.0-.4` is closed with private authority, four carriers, ordinary/canonical admission, and independent recomposition. Shared Lua `.14.6.6.0` freezes one Lua-5.1-compatible 85-pass/one-RED consumer on both ABIs and `.1-.4` own authority, carriers, admission, and recomposition. Lua `.1` is next; recurrence `.7`, public `.8`, staged `.14.7`, final `.14.8` no-drift, reversed-span and unrelated hygiene/readability remain separately owned. |
