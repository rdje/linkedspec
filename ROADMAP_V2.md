...
Standalone lifecycle-block audit `.15.0` ratifies a rule-item-leading `{ ... }` as exact syntax sugar for
`I { ... }` at the same authored position. Perl/Rust `.15.1` now implements one neutral twin contract, preserves
source/line/ActionIR provenance and every earlier brace owner, rejects malformed twins equivalently, and repairs
Rust duplicate-`I` order by appending into the existing preamble ABI. Dart/Julia/Lua, self-hosted grammar, public
teaching, and final no-drift remain owned by `.15.2`; their legacy plain carriers remain inert.
