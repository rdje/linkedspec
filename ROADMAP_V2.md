...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity remains 80/0/0; semantic introspection remains closed at 128 mutations, rollout 9/9; typed source remains 9 complete / 5 pending / 114 mutations and recursive observation stays closed on all six runtimes. `INTER-MATCH-GAP-CAPTURE.1-.6` is complete through Lua admission `.6.5`, canonical-signoff-complete from atomic 250 `798aeeee`: neutral plus all six private runtimes now execute at 7/2/60, with ten Rust, ten Dart, ten Julia, and sixteen Lua admission mutations. Lua storage remains 19 owners / three dual-ABI native modules. `.14.5.1` promotes typed `gap_composition` only after public/recurring gap admission `.7`. Reversed-span and unrelated hygiene/readability and later backlog lanes remain separately owned. |
...
| Inter-match gap capture | `active; Lua admission .6.5 canonical-signoff-complete from atomic 250 798aeeee` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | All six private runtimes are admitted at governance 7/2/60. Shared Lua carries logical authored/compiled metadata, invocation-local native execution, normalized reconstruction, compatible descriptors, same-engine generated-v2 execution, independent emitted proof, unchanged-primary parity, and one exact nine-role consumer passing 392 assertions per ABI in ordinary/canonical/rooted routes. Existing preceding-slot events retain post-action/pre-`LE` timing. Recurring/public rows remain pending for `.7`; supported ActionIR 246, format 2, storage 19/three native modules, outward surfaces, and typed source 9/5/114 stay unchanged. |
