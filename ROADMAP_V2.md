...
static digests. Public `.10.5.4.3` now exposes `capabilities`, typed `query`, and raw-neutral `queryNeutral` through
one detached evaluator at all 19 static digests and 26 validation boundaries. Closeout `.4` is next; runtime
observation/rollout/admission stay absent.
