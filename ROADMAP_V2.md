...
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 110 mutations, rollout 8/9, and native admission 6/6. MCP ADRs `0054`/`0055` freeze one modern `2026-07-28` contract, five implementations, and six runtime admissions; neutral `.10.9.1` and all implementation parents `.10.9.2-.6` are closed. ADR `0062` and `.10.9.7.1.0` prove the former 17/20 blocker. Authorized repair `.1` fixes the exact shared boundaries and all-twenty consumer proof `.2` is green across all six runtimes. Routed `.3` now composes those owners through rooted `tools/check_mcp_six_runtime.sh`, canonical opt-in, and omission-sensitive cross-ledger governance; focused proof is green and formal MCP state is 5/5 implementations + 6/6 runtimes with rollout complete/141. Unchanged closeout `.4` and MCP-parent `.2` remain ordered; the weaker claim is not selected. Director-approved typed source-location direction `.14.0.1` is complete and `.14.1-.8` remain pending. Public no-drift, `.22`, `.23`, the format program, inter-match gaps, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
