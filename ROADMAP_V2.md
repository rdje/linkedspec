...
| Future parity backlog | `in progress` | Deferred/future parity lanes after the closed language-reference, terse-format, and five-backend implementation trees. | Five-backend capability parity remains 80/0/0; semantic introspection remains closed at 128 mutations, rollout 9/9; typed source remains 9 complete / 5 pending / 114 mutations and recursive observation stays closed on all six runtimes. `INTER-MATCH-GAP-CAPTURE.1-.5` is landed through atomic 245 at `a6ff2614`: neutral, Perl, Rust, Dart, and Julia are complete. Shared Lua behavior-free plan `.6.0` is active from that clean boundary and freezes `.6.1-.6.5` without behavior movement. Governance stays 5 complete / 4 pending / 58 semantic mutations plus ten Rust, ten Dart, and ten Julia admission mutations; Lua storage is currently 18 owners / three dual-ABI native modules. `.14.5.1` promotes typed `gap_composition` only after gap admission `.7`. Reversed-span and unrelated hygiene/readability and later backlog lanes remain separately owned. |
...
| Inter-match gap capture | `active; Lua metadata .6.1 signoff-complete from atomic 246 5089a360` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl, Rust, Dart, and Julia are privately admitted at governance 5/4/58. Shared Lua now carries logical source identity, pinned-Unicode named/anonymous declarations, selector/directive provenance, source-aware diagnostics, compiled slot/directive/five-field identity, and one 178-assertion dual-ABI dormant consumer guarded by ten mutations. `.6.2-.6.5` still own the one recognition-frame/token native path, normalized/descriptor/generated-v2 execution, fresh-process emitted proof with storage 18→19, and existing-primary/nine-role admission. Existing preceding-slot events retain post-action/pre-`LE` timing. Both Lua runtimes and public rows remain pending; supported ActionIR 246, format 2, descriptors, storage 18, outward surfaces, and typed source 9/5/114 stay unchanged. |
