...
Director-requested [backend integration guides](docs/tasks/BACKEND-INTEGRATION-GUIDES.md) are active after clean conformance .1.34: inventory .0 and Rust setup/native consumer .2.1 are complete; Lispish/deployment .2.2 is next. Common checkout/version/setup guidance and five native consumer guides will verify dependency preparation and reuse, API/results/errors, grammar packaging and outside-cwd use. The director authorizes this documentation activity before the remaining startup reading; complete it and resume conformance .1.35. ADR0040 keeps one canonical content owner and links the eventual companions. The native Rust example passes two four-value runs; normal Cargo dependency builds are authorized. Fresh-clone preparation and remaining backend guides retain their own acceptance.
