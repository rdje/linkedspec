...
| Inter-match gap capture | `active; Dart plan .4.0 signoff-complete for intended atomic 233` | Historical “super split” means automatic source-gap access between repeated OR/default action-edge matches. The target rule owns the selected regex slot/lifecycle; the enclosing rule owns repeated selection/gap orchestration. It is unrelated to blind calls or adjacency. | Perl and Rust are privately admitted at governance 3/6/56 plus ten Rust admission mutations. Rust composes all nine roles; the mixed input `alpha, beta | gamma\n- delta` proves separator-agnostic item extraction with exact gaps. Behavior-free Dart `.4.0` reproduces raw-syntax/unknown-helper absence and current LS-before-selection order, then assigns authored/static `.4.1`, native `.4.2`, reconstructed/descriptor/generated `.4.3`, emitted `.4.4`, and primary/admission `.4.5`. No Dart behavior or rollout moves yet. Four later runtime routes and both public rows remain pending. Recognition stays 137/246/58, public helpers 122, typed source 9/5/114, generated plan v2 unchanged, and outward surfaces guarded. |
