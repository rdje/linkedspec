...
| mdBook destination-root safety | `in progress; intake signoff-complete` | Make custom mdBook output validation and execution use one repository-derived resolution base. | `MDBOOK-DESTINATION-ROOT-ALIGNMENT.0` durably records the exact book-root-validation/repository-root-execution mismatch and smallest RED/repair plan without behavior change. `.1` follows its clean commit to preserve default, absolute, and environment behavior while proving all relative CLI forms and hostile rejections, then return the clean frontier to typed-source Perl work. The nonurgent readability audit remains separate. |
| Future parity backlog | `in progress` | Deferred/future lanes after the closed language-reference, terse-format, and five-backend implementation trees: staged parsing, function extensions, helper caveats, plugin fate, richer oracle candidates, spec-derived parser/stimuli validation, AND/OR edge-default design, deep semantic introspection with MCP projection, generic first-class codeblocks, compatibility retirement, toolbox reliability, structural/progressive/staged authoring closure, the Unicode structured-text program, and explicit Rust mutation campaigns. | Five-backend capability parity is 80/0/0. Semantic introspection is exact at six fixture groups / 20 responses / 128 mutations, rollout 9/9, and native admission 6/6. MCP is complete/141; parents `.10`, `.11`, `.14.1`, and `.24` are closed. Typed source-location neutral/public composition and behavior-free `.14.2.0` planning are closed. Audited correction `.14.2.0.1` is signoff-complete at live 3/11 with 37 mutations and exact backend admission owners. mdBook destination safety intake `.0` is signoff-complete and repair `.1` follows its clean commit before Perl RED; `.14.2.1-.7` and `.14.3-.8` remain pending. The nonurgent rendered-readability tree stays queued. `.22`, `.23`, format/inter-match work, manual mutation execution, inspector `.13.1`, and `.15` remain separately owned. |
