...
Behavior-free Perl audit `FUTURE-PARITY-BACKLOG.10.9.2.0` and ADR `0057` now freeze how the first native
implementation will consume this bundle without reading it at runtime: a deterministic generated private Perl
binding plus a strict schema/wire runtime feed an in-process `LinkedSpec::MCPServer`. No generated binding or
server exists in this planning leaf; `.10.9.2.1` owns the first implementation bytes.

Canonical local CI requires the manifest, all five subordinate neutral artifacts, both programs, and runs the two
commands above unconditionally in materializer-then-independent-validator order. The recurring tool-topology test
rejects a missing materializer and reversed order, so stale generated bytes cannot bypass contract validation.
