...
because one Lua source must qualify independently on PUC Lua and LuaJIT. Current exact state is Perl complete at
1/5 implementations and 1/6 runtimes, every other row pending, and shared `thin_mcp_transport` rollout pending
under `.10.9.7`. It pins the unchanged transport root digest, exact case counts, source owners, proof owners, and
one twelve-role admission topology. Run:
...
The checker rejects 28 ledger, status, ownership, source-authority, role, tracked-input, and canonical-order
mutations. The admission consumer composes existing public-server behavior and neutral artifacts exactly once per
role; it compares Perl results to direct native capabilities/query and does not become a second semantic or wire
oracle. Production MCP source remains fenced from parser construction, descriptors, substitution handlers,
parser-source dumps, trace, shell/process execution, and arbitrary path reads. The only documented production
`sysopen` is fail-closed `/dev/urandom` for handles.

Canonical local CI requires the manifest, all subordinate neutral artifacts, the implementation ledger, and every
program/test above. It runs materializer -> independent validator -> generated Perl binding -> binding/dispatch/
stdio proof -> admission-ledger checker -> Perl admission in that exact order. The recurring tool-topology test
rejects omissions and reversed order, so neither stale generated bytes nor a premature status promotion can bypass
contract and admission validation.
