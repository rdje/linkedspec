...
implementations and 6/6 runtimes complete, with shared `thin_mcp_transport` rollout pending under `.10.9.7`.
...
The checker rejects 114 ledger, status, ownership, source-authority, role, skipped-proof, tracked-input, and
...
semantic or wire oracle. The current identity role proves capabilities and one representative semantic query;
ADR `0062` and signoff-complete behavior-free `.10.9.7.0` require `.10.9.7.1` to strengthen those same consumers to all twenty
neutral semantic cases before recurring rollout can advance. Production MCP source remains fenced from parser construction, descriptors,
...
The recurring tool-topology test rejects omissions and reversed order, so neither
stale generated bytes nor premature status promotion can bypass contract and admission validation.
...
altering the transport contract. Their exact admissions advance only their implementation/runtime rows. Dart's
parent is closed; Julia no-change closeout `.10.9.5.4` is next before the clean handoff to shared Lua `.10.9.6`.
