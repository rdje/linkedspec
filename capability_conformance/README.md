...
zero complete / eight pending. Perl now consumes the neutral artifact through typed ActionIR/runtime proof, so the
current ledger is one complete / seven pending: Rust, Dart, Julia, Lua, generated/primary projection, recurring
composed proof, and public no-drift remain owned by `FUTURE-PARITY-BACKLOG.5.2.3-.9`.
