...
because one Lua source must qualify independently on PUC Lua and LuaJIT. Current exact state is Perl, Rust, Dart,
and Julia complete at 4/5 implementations and 4/6 runtimes; Lua, PUC Lua, and LuaJIT remain pending, and shared
`thin_mcp_transport` rollout remains pending under `.10.9.7`. Julia admission `.10.9.5.3` composes the existing
generated binding/runtime, native secure decoded server, and strict stdio without changing production behavior.
The ledger pins the unchanged transport root digest, exact
...
The checker rejects 79 ledger, status, ownership, source-authority, role, skipped-proof, tracked-input, and
...
semantic or wire oracle. Production MCP source remains fenced from parser construction, descriptors,
...
Dart uses core secure randomness, and Julia uses `RandomDevice`, all without filesystem authority.

Canonical local CI requires the manifest, all subordinate neutral artifacts, the implementation ledger, and every
program/test above. It runs neutral validation and all four generated bindings before Perl/Rust/Dart/Julia focused
proof; each Rust/Dart/Julia admission then precedes the admission-ledger checker, and Perl admission follows it.
