...
- `capture_take_until_cursor_len_from(name)` returns the numeric width of that same named-mark through-cursor span and then advances the named mark to the current parser position,
...
- `capture_take_between_len(start_mark, end_mark)` returns the numeric width of that same explicit two-mark span and then advances the start mark to the stored end mark,
...
## Worked Example: Current Local Match Boundary Reads
Sometimes the rule does not want a stored checkpoint at all. It just wants the boundaries of the current local match as immediate data.

That is what `match_start_pos()` and `match_end_pos()` are for.

```text
explicit_match_boundaries::AND
 I { declare(scalar, stage, body_start_pos, body_end_pos) }
 /foo\(/
 /\w+/
 /\)/
 -> explicit_match_boundaries[0] { assign(scalar(stage), "open") }
 -> explicit_match_boundaries[1] { assign(scalar(stage), "body"); assign(scalar(body_start_pos), match_start_pos()); assign(scalar(body_end_pos), match_end_pos()) }
 -> explicit_match_boundaries[2] { return(array("?explicit_match_boundaries:", scalar(body_start_pos), scalar(body_end_pos), match_start_pos(), match_end_pos())) }
```
...
the practical reading is:
- at `-> explicit_match_boundaries[1]`, the current local match is `bar`, so `match_start_pos()` returns `4` and `match_end_pos()` returns `7`,
- at `-> explicit_match_boundaries[2]`, the current local match is `)`, so `match_start_pos()` returns `7` and `match_end_pos()` returns `8`,
- and none of those reads touch the named-mark bucket at all.

This is the direct-current-boundary pattern:
- use `mark_pos(name)` when the rule wants a previously stored checkpoint,
...
- use `entry_group(index)` when the rule wants one capture group from that same immediate entry match,
...
## Worked Example: Current Immediate Match Group Read
Sometimes a child rule wants one capture group from the immediate entry match that led into the rule, even while later local matches inside that child rule have their own capture groups.

That is what `entry_group(index)` is for.

```text
entry_vs_local_groups::AND
 /(foo)\(/
 -> entry_vs_local_groups[0] { return(call(entry_vs_local_groups_body)) }

entry_vs_local_groups_body::AND
 I { declare(scalar, entry_group_0, body_group_0, body_group_1) }
 /(\w)(\w+)/
 /(\))/
 -> entry_vs_local_groups_body[0] { assign(scalar(entry_group_0), entry_group(0)); assign(scalar(body_group_0), match_group(0)); assign(scalar(body_group_1), match_group(1)) }
 -> entry_vs_local_groups_body[1] { return(array("?entry_vs_local_groups_body:", scalar(entry_group_0), scalar(body_group_0), scalar(body_group_1), match_group(0), entry_group(0), entry_group(1), match_group(1))) }
```

When building this exact inline example directly, select `top_rule => entry_vs_local_groups` so the entry rule is explicit.
...
the practical reading is:
- when `entry_vs_local_groups_body` starts, its immediate entry match is still `foo(` and its immediate capture-group list still contains `foo`, so `entry_group(0)` is `foo`,
- at `-> entry_vs_local_groups_body[0]`, the current local match is `bar`, so `match_group(0)` is `b` and `match_group(1)` is `ar`,
- at `-> entry_vs_local_groups_body[1]`, the current local match is `)`, so `match_group(0)` is `)` while `entry_group(0)` still keeps the same entry capture group,
- and missing group indexes stay `undef` instead of forcing the rule to branch first.
...
- and use `entry_map()` / `match_map()` when the whole named-capture hash is the clearer boundary snapshot.
...
That is what `entry_named(name)` and `match_named(name)` are for.

```text
entry_vs_local_named_groups::AND
 /(?<prefix>foo)\(/
 -> entry_vs_local_named_groups[0] { return(call(entry_vs_local_named_groups_body)) }

entry_vs_local_named_groups_body::AND
 I { declare(scalar, entry_prefix, body_first, body_rest) }
 /(?<first>\w)(?<rest>\w+)/
 /(?<close>\))/
 -> entry_vs_local_named_groups_body[0] { assign(scalar(entry_prefix), entry_named(prefix)); assign(scalar(body_first), match_named(first)); assign(scalar(body_rest), match_named(rest)) }
 -> entry_vs_local_named_groups_body[1] { return(array("?entry_vs_local_named_groups_body:", scalar(entry_prefix), scalar(body_first), scalar(body_rest), match_named(close), entry_named(prefix), entry_named(missing_name), match_named(rest))) }
```

When building this exact inline example directly, select `top_rule => entry_vs_local_named_groups` so the entry rule is explicit.
...
the practical reading is:
- when `entry_vs_local_named_groups_body` starts, its immediate entry match is still `foo(` and its immediate named-capture hash still contains `prefix => foo`, so `entry_named(prefix)` is `foo`,
- at `-> entry_vs_local_named_groups_body[0]`, the current local match is `bar`, so `match_named(first)` is `b` and `match_named(rest)` is `ar`,
- at `-> entry_vs_local_named_groups_body[1]`, the current local match is `)`, so `match_named(close)` is `)` while `match_named(rest)` is now `undef`,
- and missing named captures stay `undef` instead of forcing the rule to branch first.

This is the direct-named-group pattern:
- use `entry_named(name)` when the rule wants one named capture from the immediate entry match that led into the current rule,
...
That is what `entry_map()` and `match_map()` are for.
...
entry_vs_local_named_maps_body::AND
 ...
 /(?<first>\w)(?<rest>\w+)/
 -> entry_vs_local_named_maps_body[0] { assign(hash(entry_named_seen), entry_map()); assign(hash(body_named_seen), match_map()) }
 /(?<close>\))/
 -> entry_vs_local_named_maps_body[1] { return(array("?entry_vs_local_named_maps_body:", scalar(hash(entry_named_seen), "prefix"), scalar(hash(body_named_seen), "first"), scalar(hash(body_named_seen), "rest"), scalar(match_map(), "close"), join_values(",", sorted_keys(entry_map())), join_values(",", sorted_keys(match_map())))) }
```
...
the practical reading is:
- when `entry_vs_local_named_maps_body` starts, its immediate entry named-capture hash still contains `prefix => foo`, so `entry_map()` snapshots that entry-side named boundary directly,
- at `-> entry_vs_local_named_maps_body[0]`, the current local match is `bar`, so `match_map()` snapshots `first => b` and `rest => ar`,
- at `-> entry_vs_local_named_maps_body[1]`, the current local match is now `)`, so `match_map()` has moved on to `close => )`,
...
- use `entry_map()` when the rule wants the whole immediate entry named-capture hash that led into the current rule,
- use `match_map()` when the rule wants the whole currently active local named-capture hash instead,
...
That is what `entry_len()` is for.

```text
entry_vs_local_width::AND
 I { declare(scalar, stage) }
 /foo\(/
 -> entry_vs_local_width[0] { assign(scalar(stage), "open"); return(call(entry_vs_local_width_body)) }

entry_vs_local_width_body::AND
 I { declare(scalar, entry_width, body_width) }
 /\w+/
 /\)/
 -> entry_vs_local_width_body[0] { assign(scalar(entry_width), entry_len()); assign(scalar(body_width), length(match_text())) }
 -> entry_vs_local_width_body[1] { return(array("?entry_vs_local_width_body:", scalar(entry_width), scalar(body_width), entry_len(), length(match_text()))) }
```

When building this exact inline example directly, select `top_rule => entry_vs_local_width` so the entry rule is explicit.
...
the practical reading is:
- when `entry_vs_local_width_body` starts, its immediate entry match is still `foo(`, so `entry_len()` is `4`,
- at `-> entry_vs_local_width_body[0]`, the current local match is `bar`, so `length(match_text())` is `3`,
- at `-> entry_vs_local_width_body[1]`, the current local match is `)`, so `length(match_text())` is `1`,
- and the immediate entry width stays `4` throughout the child rule.

This is the direct-entry-width pattern:
- use `entry_len()` when the rule wants the width of the immediate entry match that led into the current rule,
- use `length(match_text())` when the rule wants the width of the currently active local match instead,
- and use `capture_len_from(name)` when the rule wants the width of a remembered span between rule-local boundaries instead.
...
## Worked Example: Current Local Match Group Read
Sometimes the rule wants one capture group from the current local match itself, not a stored checkpoint span and not a capture group from the immediate entry match.

That is what `match_group(index)` is for.

```text
explicit_match_groups::AND
 I { declare(scalar, stage, first_piece, second_piece) }
 /foo\(/
 /(\w)(\w+)/
 /(\))/
 -> explicit_match_groups[0] { assign(scalar(stage), "open") }
 -> explicit_match_groups[1] { assign(scalar(stage), "body"); assign(scalar(first_piece), match_group(0)); assign(scalar(second_piece), match_group(1)) }
 -> explicit_match_groups[2] { return(array("?explicit_match_groups:", scalar(first_piece), scalar(second_piece), match_group(0), match_group(1))) }
```
...
the practical reading is:
- at `-> explicit_match_groups[1]`, the current local match is `bar`, so `match_group(0)` is `b` and `match_group(1)` is `ar`,
- at `-> explicit_match_groups[2]`, the current local match is `)`, so `match_group(0)` is `)` and `match_group(1)` is `undef`,
- and those reads do not consult or mutate the named-mark bucket.

This is the direct-current-group pattern:
- use `entry_group(index)` when the rule wants one capture group from the immediate entry match that led into the current rule,
- use `match_group(index)` when the rule wants one capture group from the current local match only,
- use `capture_from(name)` when the rule wants a remembered span,
- and use `match_text()` / `match_len()` when whole-match text or width is clearer than group-level reads.
...
Use `capture_take_until_cursor_len()` when:
- one rolling anonymous capture boundary is enough for the whole rule,
...
- the rule only needs the width of that explicit two-mark span instead of the span text,
- and the start mark should still roll forward to the stored end mark after the read.
...
- or the rule wants a backend-neutral replacement for raw `length($IMATCH)` in normal user-facing `.spec` code.
